    ...
    match index_read_result {
        ...
        Ok(ref bytes) => {
            ...
            match (catalog_hash, blockquote_hash) {
                ...
                (Some(b2), Some(b3)) => {
                    // BC-5.39.010 v1.14 PC13: directional two sub-cases.
...
///
/// # BC trace
/// BC-5.39.010 v1.14 PC40: volatile-input precondition.
...
///
/// PC40 (BC-5.39.010 v1.14): if the story's `inputs:` list contains any volatile
...
mod tests {
    ...
    /// AC-009 MUTANT: STORY-INDEX internally inconsistent blocks (BC-5.39.010 v1.14 PC13b).
    ///
    /// BC-5.39.010 v1.14 PC13 directional carve-out:
    ...
    fn test_BC_5_39_010_arm_b1_hash_mismatch_blocks() {
        ...
        // PC13b three-provenance note (invariant 11, BC-5.39.010 v1.14)
        assert!(
    ...
    // BC-5.39.010 v1.14 §B3 invariant: B3 must be extracted from the prose line.
    // -----------------------------------------------------------------------
    ...
    // BC-5.39.010 v1.14 precondition 17: input-hash value must be hex (0-9a-f only).
    // -----------------------------------------------------------------------
    ...
    // BC-5.39.010 v1.14 PC16: catalog lookup must match the CANONICAL S-NNN.NNN row
    ...
    // -----------------------------------------------------------------------
    // PC40 (BC-5.39.010 v1.14 amendment): volatile-input precondition.
    //
    ...
    ///
    /// BC-5.39.010 v1.14 PC40: volatile inputs → advisory + Continue (skip comparison).
    #[test]
    ...
    //
    // BC-5.39.010 v1.14 (ADR-037 §Decision 2) specifies six canonical volatile-input
    ...
    /// ADR-037 §Decision 2 and BC-5.39.010 v1.14 PC40. It must NOT be volatile.
    ///
    ...
    //   per BC-5.39.010 v1.14 PC40 ... Update input-hash manually when non-volatile
    //   inputs change."
    //
    // Prescribed text (ADR-037 §Decision 4 / BC-5.39.010 v1.14 PC40 note):
    ...
    fn test_BC_5_39_010_arm_b1_volatile_advisory_prescribed_text() {
        ...
        assert!(
            ...
            F-S2107-P3-012: current message uses 'BC-5.39.010 v1.14 PC40' instead. \
