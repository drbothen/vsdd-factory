...
**Story spec path lookup:** Story spec files follow the naming pattern `.factory/stories/S-{story-id}-{slug}.md`. The slug is part of the filename but is not always known in advance. To locate the spec for a given story ID, use a case-insensitive glob — e.g., `Glob('.factory/stories/S-{story-id}-*.md')` — trying both upper and lower case prefix variants (`S-`/`s-`) if the first attempt returns zero results. This returns exactly one file for well-formed story IDs. If both case variants return zero results, report a scope-resolution error and halt.
