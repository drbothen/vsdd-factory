...
2. **Verify basename of the embedded worktree-abs-path matches story-id.** The orchestrator derives the worktree root via `git rev-parse --show-toplevel` and embeds it as `worktree-abs-path` in the identity tuple. You (read-only, no Bash) compare the basename of that embedded `worktree-abs-path` against the dispatched story-id — you do NOT execute git yourself. The exact alignment rule: the basename MUST equal the story-id (e.g., `S-12.08`) OR begin with the story-id followed by a `-` separator (e.g., `S-12.08-slug`). Any other basename is a mismatch — emit a `dispatch-error` and halt.
...
**Story spec path lookup:** Story spec files follow the naming pattern `.factory/stories/S-{story-id}-{slug}.md`. The slug is part of the filename but is not always known in advance. To locate the spec for a given story ID, use a case-insensitive glob anchored to the canonical repo root — e.g., `Glob('<canonical-repo-root>/.factory/stories/[Ss]-{story-id}-*.md')` — trying both upper and lower case prefix variants (`S-`/`s-`) if the first attempt returns zero results. This returns exactly one file for well-formed story IDs. If both case variants return zero results, report a scope-resolution error and halt.
