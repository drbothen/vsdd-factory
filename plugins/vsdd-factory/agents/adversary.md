...
Before reading any feature-code evidence or producing findings, you MUST verify that you are operating against the correct git tree. This preflight guards against two failure modes documented in issues #169 and #176: (a) accidentally reading `.factory/` content found in the story worktree — no `.factory/` directory is created at worktree-checkout time (gitignored on the product branch), so any such content is live shadow-write evidence (issue #523 class) that MUST be reported as a defect signal but MUST NOT be used as spec ground-truth — using worktree `.factory/` paths as spec ground-truth produces hallucinated "absent file" findings for specs that exist on `factory-artifacts` (#169); (b) reading the wrong feature checkout and producing a false-GREEN review for a different story's diff.
...
4. **Read spec/ADR/BC ground-truth from canonical factory-artifacts, NOT from any `<worktree-abs-path>/.factory/` path.** `git worktree add` checks out NOTHING under `.factory/` — `.factory/` is gitignored on the product branch, so no shadow directory is created at worktree-creation time. Any content found at `<worktree-abs-path>/.factory/` is therefore evidence of a live CWD-relative shadow write (issue #523 class) and MUST be reported as a defect signal, not dismissed as a pathing artifact. Spec files, ADR files, BC files, and story specs MUST be read from `<canonical-repo-root>/.factory/` — the canonical factory-artifacts path embedded in the identity tuple. The spec ground-truth — including STORY specs in `.factory/stories/` — comes ONLY from `<canonical-repo-root>/.factory/`. Any worktree `.factory/` content is live shadow-write evidence subject to BC-6.26.001 Invariant 5 — the `step-g-cleanup.md §G.1` teardown preflight exists to catch this class before `git worktree remove` destroys it; report it as a defect signal regardless of whether teardown has been attempted.
...
6. **Path-corroborate all "absent file" findings before reporting — corroboration target depends on artifact class.** Any finding claiming an absent file, a missing deliverable, or a missing ADR MUST be path-corroborated before reporting. The corroboration target differs by artifact class:
   - **SPEC / ADR / BC / VP absence** — corroborate against `<canonical-repo-root>/.factory/...` (the canonical factory-artifacts path). These files do NOT live in the worktree (`git worktree add` checks out nothing under `.factory/`); any `.factory/` content found in the worktree is live shadow-write evidence (issue #523 class) and MUST be reported as a defect signal — not used as spec ground-truth. A SPEC/ADR/BC/VP absence finding based only on worktree `.factory/` path reads MUST NOT be reported; re-corroborate against the canonical-repo-root instead.
   - **FEATURE CODE / demo-evidence absence** — corroborate against `<worktree-abs-path>/...` (the feature checkout). These files do NOT live on factory-artifacts; corroborating against the canonical-repo-root for feature code is the wrong target and will always show absence.
   A finding of the form "missing ADR for X", "missing deliverable Y", or "absent source file Z" that is NOT path-corroborated against the correct target for its artifact class MUST NOT be reported. Pathing-artifact absences are not findings.

> **Gate-imposed authoring constraint (F-S2104-P28-M06 — F-S2104-P27-B01 ordinal-position guards):** The ordinal positions of Rules 3–6 in this `#### Worktree-Identity Preflight` section are load-bearing. Six guards in `worktree-identity-preflight.bats` pin specific rule numbers and fire fail-closed (false RED, not false GREEN) if a rule is renumbered: `_guard_d_worktree_rooted` requires "worktree-rooted" on a `^3\.` line; `_guard_e_checks_out_nothing` requires "checks out NOTHING under" on a `^4\.` line; `_guard_e_factory_artifacts` requires "factory-artifacts" on a `^4\.` line; `_guard_e_canonical_repo_root` requires "canonical-repo-root" on a `^4\.` line; `_guard_f_case_insensitive` requires "case-insensitive" on a `^5\.` line; `_guard_g_path_corroborated` requires "path-corroborated" on a `^6\.` line. Inserting a rule anywhere between Rules 3 and 6, or reordering Rules 3–6, renumbers the list and fires all six guards simultaneously. Append new rules after Rule 6; do not insert between Rules 3 and 6.
