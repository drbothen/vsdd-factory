...
2. **Verify basename of the embedded worktree-abs-path matches story-id (case-insensitive).** The orchestrator derives the worktree root via `git rev-parse --show-toplevel` and embeds it as `worktree-abs-path` in the identity tuple. You (read-only, no Bash) compare the basename of that embedded `worktree-abs-path` against the dispatched story-id — you do NOT execute git yourself. The comparison is **case-insensitive** (story IDs may appear in mixed case in worktree directory names). The exact alignment rule: the basename MUST equal the story-id (e.g., `S-12.08`) OR begin with the story-id followed by a `-` separator (e.g., `S-12.08-slug`), compared case-insensitively. Any other basename is a mismatch — emit a `dispatch-error` and halt.
