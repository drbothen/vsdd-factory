#!/usr/bin/env bats
# fail-malformed-cite.bats — AC-12: hook blocks on malformed cite in Closes line with no
#                             structured ID pattern; block cites D-419(c)
#
# Traces to:
...
#
# Fixture: lessons.md entry with "**Closes:** fixed the thing" (freeform prose, no structured ID)
...
setup() {
  ...
  FIXTURE_SRC="${BATS_TEST_DIRNAME}/../fixtures/validate-closes-completeness/fail-malformed-cite"
  WORK="$(mktemp -d)"
...
_lessons_md_envelope() {
  printf '{"event_name":"PostToolUse","tool_name":"Edit","session_id":"fail-malformed-cite","tool_input":{"file_path":".factory/cycles/v1.0-brownfield-backfill/lessons.md","content":""},"tool_response":{"exit_code":0,"stdout":"","stderr":""}}'
}

# ---------------------------------------------------------------------------
# AC-12: malformed cite "fixed the thing" in Closes line => exits 2 (block)
# Traces to BC-5.39.007 postcondition 8
# ---------------------------------------------------------------------------

@test "AC-12 FAIL: hook blocks on malformed cite with no structured ID pattern in Closes line" {
  _require_artifacts
  _setup_fixture
  _write_registry
...
# ---------------------------------------------------------------------------
# AC-12: block message cites D-419(c)
# ---------------------------------------------------------------------------

@test "AC-12 FAIL: block message cites D-419 for malformed cite ID" {
  _require_artifacts
  _setup_fixture
  _write_registry
  cp "$WASM_PLUGIN" "$WORK/hook-plugins/"
