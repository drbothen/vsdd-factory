...
worktree. Proceed only on a PASS preflight result (per the §G.1 discrimination-chain protocol:
nothing at the path or empty-and-clean directory → proceed;
symlink or non-directory at the path → BLOCKED (PC2b); stray files → BLOCKED; find error → HALT).
