...
Record the Red Gate outcome in `<canonical-repo-root>/.factory/cycles/<cycle-id>/<story-id>/implementation/red-gate-log.md` following `${CLAUDE_PLUGIN_ROOT}/templates/red-gate-log-template.md`. Dispatch `state-manager` to write and commit this file; do NOT write it from a story-worktree CWD.
...
- `<canonical-repo-root>/.factory/cycles/<cycle-id>/<story-id>/implementation/red-gate-log.md` — Red Gate verification log
