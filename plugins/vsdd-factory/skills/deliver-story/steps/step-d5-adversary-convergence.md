...
- Story spec (`<canonical-repo-root>/.factory/stories/<STORY-ID>-*.md`) — canonical repo-root path only (NOT from any worktree `.factory/` path — such content is live shadow-write evidence, not a snapshot)
