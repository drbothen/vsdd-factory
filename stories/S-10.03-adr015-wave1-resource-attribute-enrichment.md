...
### AC-001: All Resource attributes stamped at startup with no null fields (traces to ADR-015 D-15.2 §Resource attributes + "No Resource field absent or null") (traces: BC-1.12.003 postcondition 1)
...
### AC-002: vcs.repository.url.full fallback cascade implemented (traces to ADR-015 D-15.2.c) (traces: BC-1.12.003 postcondition 2)
...
### AC-003: host.id fallback cascade implemented with full Windows registry lookup (traces to ADR-015 D-15.2.c §host.id cascade — OQ-2 resolved 2026-05-04) (traces: BC-1.12.003 postcondition 3)
...
### AC-004: worktree.id is SHA-256 of absolute worktree path (traces to ADR-015 D-15.2 Resource attributes table + D-15.2.c) (traces: BC-1.12.003 invariant 4)
...
### AC-005: Per-event identity fields stamped at emit time (traces to ADR-015 D-15.2 §Per-event attributes) (traces: BC-1.12.004 postcondition 1)
...
### AC-006: event.category compile-time registry correct (traces to ADR-015 D-15.2.a + D-15.2.b) (traces: BC-1.12.004 postcondition 2)
...
|-------|-------|-----------|------------|-------|
| [BC-1.12.003](../specs/behavioral-contracts/ss-01/BC-1.12.003.md) | factory-dispatcher::resource_attributes::startup_stamping — all 15 OTel Resource attributes stamped at dispatcher startup with deterministic fallback cascade; no Resource field absent or null | SS-01 | CAP-030 | Wave 1 — Resource attribute startup stamping (AC-001, AC-002, AC-003, AC-004) |
| [BC-1.12.004](../specs/behavioral-contracts/ss-01/BC-1.12.004.md) | factory-dispatcher::emit_event::per_event_host_stamping_and_internal_bifurcation — per-event OTel fields stamped at emit time; emit_internal Some/None bifurcation post-FileSink-rewire; event.category derived from compile-time registry | SS-01 | CAP-029 | Wave 1 — per-event stamping; Resource context consumed here (AC-005, AC-006) |
| [BC-1.12.005](../specs/behavioral-contracts/ss-01/BC-1.12.005.md) | factory-dispatcher::host::emit_event::host_field_override_visibility — event.host_overrides field stamped on domain events + vsdd.internal.host_field_override.v1 lifecycle event emitted when host overrides plugin-supplied host-owned fields (ADR-015 D-15.3) | SS-01 | CAP-029 | Wave 1 — host-override visibility depends on Resource attribute context established here (AC-007) |

### AC-007: host_field_override_visibility depends on startup Resource context (traces: BC-1.12.005 postcondition 2 — event.host_overrides absence rule when no fields overridden)

The `event.host_overrides` override-visibility mechanism (from S-10.02/BC-1.12.005)
depends on the startup Resource attribute context established by this story. The host
can only detect that a plugin supplied a conflicting host-owned field because the
host-stamped Resource value was computed at startup here (BC-1.12.003 postcondition 4).
`event.host_overrides` is absent from events where no host-owned field was overridden;
it is present (non-empty array) when at least one override occurred.

**Falsifiable test:** Integration test: emit an event from a plugin that supplies NO
host-owned fields — assert `event.host_overrides` is absent from the event record.
Confirms the Resource context was stamped correctly so the comparison can occur.
...
| v1.2 | 2026-05-06 | D-313 Phase 1c POLICY 8 propagation: behavioral_contracts array populated with 3 BCs (BC-1.12.003, BC-1.12.004, BC-1.12.005). BC table added. ACs traced (AC-001/002/003/004→BC-1.12.003; AC-005/006→BC-1.12.004; AC-007 new→BC-1.12.005). New AC-007 added for BC-1.12.005 coverage. Phase 1a (D-310) BCs propagated. BC-1.12.004 anomaly: BC Traceability names S-10.02 AND S-10.03; both stories updated. |
| v1.3 | 2026-05-06 | D-316 POLICY 8 sweep: BC H1 + capability anchor changes from D-314/D-315 propagated to body BC table (no new BCs added — frontmatter array unchanged). BC-1.12.003 CAP-003 → CAP-030 (D-315 re-anchor to CAP-030). BC-1.12.004 CAP-003 → CAP-029. BC-1.12.005 CAP-003 → CAP-029. F-14 wave-terminology note added. |
| v1.4 | 2026-05-06 | D-330 — F-11 fix: AC-007 trace target corrected BC-1.12.005 postcondition 4 → postcondition 2. PC4 is the immutability rule (event.host_overrides is itself host-stamped and cannot be overridden by a plugin); PC2 is the no-override absence rule (event.host_overrides is ABSENT when the host overrides no fields) — which is what this AC's falsifiable test actually exercises. |
