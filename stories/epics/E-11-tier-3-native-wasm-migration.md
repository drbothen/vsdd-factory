...
|----------|-------|--------|-----------|--------|--------|
| S-11.01 | Perf baseline + W-17 bundle ceiling (reuse W-16 model) | TBD | E-9 W-16 closure | S-11.02..S-11.07 | draft |
| S-11.02 | Batch B-1: artifact guard hooks (protect-bc, protect-vp) | TBD | S-11.01 | — | draft |
| S-11.03 | Batch B-2: process-discipline PostToolUse hooks (convergence-tracker, purity-check) | TBD | S-11.01 | — | draft |
| S-11.04 | Batch B-3: identity guard hooks (brownfield-discipline, factory-branch-guard) | TBD | S-11.01 | — | draft |
| S-11.05 | Native port: protect-secrets (dual-event PreToolUse:Bash+Read) | TBD | S-11.01 | — | draft |
| S-11.06 | Native port: check-factory-commit (PreToolUse:Bash, git log) | TBD | S-11.01 | — | draft |
| S-11.07 | Native port: red-gate (High complexity, state-file + path matching) | TBD | S-11.01 | — | draft |
| S-11.08 | Native port: destructive-command-guard (High complexity, command tokenization) + adapter retirement | TBD | S-11.02..S-11.07 | — | draft |

**Story count: 8** (S-11.01 + S-11.02..S-11.08)

> **Note on S-11.08:** destructive-command-guard is the last Tier 3 hook and also the
...
> Story-writer authors S-11.02..S-11.08 in subsequent bursts following adversarial
> convergence per ADR-013.

## Hook Inventory (10 hooks)

### Batch B-1 (S-11.02): Artifact guard hooks (PreToolUse:Edit|Write, block)
...
### Batch B-2 (S-11.03): Process-discipline PostToolUse hooks (PostToolUse:Edit|Write, continue)
...
### Batch B-3 (S-11.04): Identity guard hooks (PreToolUse:Edit|Write, block)
...
### Solo S-11.05: protect-secrets (dual-event, PreToolUse:Bash+Read, block)
...
### Solo S-11.06: check-factory-commit (PreToolUse:Bash, block)
...
### Solo S-11.07: red-gate (PreToolUse:Edit|Write, block)
...
### Solo S-11.08: destructive-command-guard + adapter retirement
...
> **Adapter retirement:** S-11.08 also performs the pre-deletion audit (zero
...
retiring the adapter in S-11.08.
...
5. Bundle size growth within the W-17 latency-primary + advisory-ceiling model
   ...
   revised model; measured by S-11.01; enforced per-batch).
6. `legacy-bash-adapter` crate deleted at S-11.08 close (Phase H). TD-014 closed.
...
|-------|-------|-----------|
| S-11.01 | (perf baseline) | pre-work; confirms W-16 perf model extends to W-17 |
| S-11.02 | protect-bc, protect-vp | both PreToolUse:Edit\|Write block-mode; near-identical logic (BC frontmatter lifecycle_status check) |
| S-11.03 | convergence-tracker, purity-check | both PostToolUse:Edit\|Write continue-mode; Tier 2 residuals with homogeneous pattern |
| S-11.04 | brownfield-discipline, factory-branch-guard | both PreToolUse:Edit\|Write block-mode; identity guard semantics (path/branch allowlist) |
| S-11.05 | protect-secrets | dual-event (Bash+Read) registration is unique; warrants solo story |
| S-11.06 | check-factory-commit | git subprocess dependency requires exec_subprocess ABI; warrants solo story |
| S-11.07 | red-gate | High complexity; state-file YAML parse + 4-way path normalization; warrants solo |
| S-11.08 | destructive-command-guard + adapter retirement | High complexity; also carries Phase H adapter deletion work |
...
except that S-11.08 triggers Phase H as part of adapter retirement.
...
1. Update STORY-INDEX S-8.20 → `superseded_by: E-11 S-11.02`
2. Update STORY-INDEX S-8.21 → `superseded_by: E-11 S-11.02`
3. Update STORY-INDEX S-8.22 → `superseded_by: E-11 S-11.05`
4. Update STORY-INDEX S-8.23 → `superseded_by: E-11 S-11.07`
5. Update STORY-INDEX S-8.24 → `superseded_by: E-11 S-11.04`
6. Update STORY-INDEX S-8.25 → `superseded_by: E-11 S-11.04`
7. Update STORY-INDEX S-8.26 → `superseded_by: E-11 S-11.06`
8. Update STORY-INDEX S-8.27 → `superseded_by: E-11 S-11.08`
9. Add E-11 and S-11.01..S-11.08 rows to STORY-INDEX.
...
|---------|-------------|-----------|--------|------------|
| R-W16-001 | bats orphan migration: bats tests for `.sh` hooks become orphans after WASM port | HIGH | MED | Deferred to Phase H. Each story spec includes a bats orphan deletion checklist task. S-11.08 triggers Phase H. |
...
| R-W16-003 | Latency regression: 10 new WASM plugins may regress cold-start p95 beyond 500ms or exceed 30MB hard kill-switch | LOW | HIGH | Same latency-primary + advisory model as W-16 (ADR-014 R-8.09 revised). S-11.01 measures W-17 baseline. Wave paused if cold-start regresses >10%. |
...
| R-W17-001 | PreToolUse block-mode regression: a wrong `permissionDecision` (false deny) on a PreToolUse hook immediately blocks the user's tool call — no batch effect, no recover path until the plugin is rolled back | HIGH | HIGH | Every block-mode story (S-11.02, S-11.04, S-11.05, S-11.06, S-11.07, S-11.08) MUST include negative test fixtures (false-block scenario) for EVERY blocking condition. Adversarial review is required before implementation dispatch. Mirror E-8 F-011 pattern for negative fixture ACs. |
| R-W17-002 | destructive-command-guard shell tokenization: the hook's command-pattern matching relies on shell-aware tokenization (token splitting, quoting). Rewriting in pure Rust regex may miss tokenization edge cases (quoted args, heredocs) | MED | HIGH | S-11.08 story spec MUST enumerate all 11+ command-pattern families from the bash original as explicit test vectors. Shell tokenization is not a 1:1 regex port — see Library Table entry for `shlex`. Adversarial review required. |
| R-W17-003 | protect-secrets dual-event registration: two `[[hooks]]` entries for one plugin; dispatcher must route both correctly | LOW | MED | S-11.05 story spec must verify dual-registration in hooks-registry.toml (both Bash and Read entries point to same `.wasm`). Integration test covers both event paths. Mirror how `protect-secrets.sh` dual-registration was originally verified in E-8. |
| R-W17-004 | adapter retirement sequencing: S-11.08 deletes adapter crate. If any hook in S-11.02..S-11.07 left a lingering adapter entry, deletion causes runtime dispatch failure | MED | HIGH | S-11.08 pre-deletion audit: `grep -r 'legacy-bash-adapter' hooks-registry.toml` must return zero matches before deletion proceeds. This is a hard pre-condition in S-11.08 ACs. |
...
| AC | Statement |
|----|-----------|
| AC-1 | All 10 hooks (8 Tier 3 PreToolUse + 2 Tier 2 PostToolUse residuals) have native WASM equivalents in `crates/hook-plugins/<name>/` delivered by S-11.02..S-11.08 |
...
| AC-3 | W-17 bundle growth within the latency-primary + advisory-ceiling model per ADR-014 R-8.09 revised: cold-start p95 ≤ 500ms (hard gate, from S-11.01 baseline); advisory soft cap ≤ 100% cumulative growth at end of W-17 (shared ceiling with W-16 per ADR-014 R-8.09); hard kill-switch ≤ 30MB. Per-wave telemetry `(bundle_size_delta_bytes, cold_start_p95_delta_ms)` published from S-11.01 baseline. Wave paused if cold-start regresses >10%. |
| AC-4 | All 7 batched/solo stories (S-11.02..S-11.08) pass adversarial convergence per ADR-013 before implementation dispatch |
| AC-5 | Every block-mode hook story (S-11.02, S-11.04, S-11.05, S-11.06, S-11.07, S-11.08) has at least one negative (false-block) test fixture per blocking condition in its story ACs. Mirror E-8 F-011 pattern. |
...
| AC-7 | Legacy bash adapter crate (`crates/hook-plugins/legacy-bash-adapter/`) deleted at S-11.08 close. Pre-deletion audit confirms zero `[[hooks]]` entries use `legacy-bash-adapter.wasm`. TD-014 closed. |
| AC-8 | `.sh` files remain on disk per R-W16-001 (Phase H deferred); no `.sh` bash hook files deleted in W-17 stories (except Phase H triggers at S-11.08). |
...
| AC-10 | S-11.08 T-0 STOP CHECK verifies that S-11.02..S-11.07 are all in `merged` status and zero legacy-bash-adapter registry entries remain before S-11.08 implementation begins. |
...
|---------|---------|---------|-------------|
| regex | workspace (1.10+) | Pattern matching for protect-bc/protect-vp (lifecycle_status), red-gate (path patterns), destructive-command-guard (command families) | S-11.02, S-11.07, S-11.08 |
| serde_json | workspace | stdin JSON deserialization (HookPayload via vsdd-hook-sdk) | S-11.02..S-11.08 |
| serde_yaml | workspace (0.9.x) | YAML parse for red-gate (wave-state.yaml red[] list) | S-11.07 |
| vsdd-hook-sdk | 0.2.0 (post-S-8.10) | Plugin ABI shim (read_file, emit_event, log, exec_subprocess) | S-11.02..S-11.08 |
| vsdd-hook-sdk (subprocess facet) | 0.2.0+ | exec_subprocess wrapper for check-factory-commit (git log) | S-11.06 |
| shlex | 1.x (TBD — see OQ-11-003) | POSIX shell-aware tokenization for destructive-command-guard | S-11.08 |
| bats-core | >=1.10 (CI) | Bats orphan checklist verification (bash hooks remain on disk) | Per-story (D-11.5) |
| wc (POSIX) | system | Bundle-size measurement (`wc -c < file`); portable across macOS BSD and Linux GNU | S-11.01 |
| hyperfine | >=1.18 | Latency benchmarking harness (cold-start p95 measurement per wave) | S-11.01 |
| git | system | check-factory-commit subprocess via host::exec_subprocess (BC-1.05.001..034 + BC-1.05.035) | S-11.06 |
...
> is adopted, it must be added to workspace `Cargo.toml`. Adversarial review on S-11.08
...
| legacy-bash-adapter crate (deleted at S-11.08) | `crates/hook-plugins/legacy-bash-adapter/` | N/A — deleted |
...
E-9 (W-16 Tier 2 closure — 23 validate-*.sh hooks native; adapter still present)
    ↓
S-11.01 (W-17 perf baseline + bundle ceiling)
    ↓ [blocks S-11.02..S-11.07]
S-11.02, S-11.03, S-11.04, S-11.05, S-11.06, S-11.07  ← all parallel, depends_on S-11.01
    ↓ [all must be merged before S-11.08]
S-11.08 (destructive-command-guard + adapter retirement + Phase H)
    ↓
TD-014 CLOSED
```
...
> (adapter retirement at S-11.08 requires all Tier 2 entries to be already native),
> but the hook port stories S-11.02..S-11.07 may begin in parallel with E-9 Wave 1
> implementation if the adapter crate remains on disk. The hard sequencing constraint
> is: S-11.08 (adapter deletion) MUST occur after both E-9 Wave 1 AND S-11.02..S-11.07
> are merged.

**Topological order (wave scheduling):**
- Wave 0: S-11.01 (depends on E-9 W-16 rc.X closure being done)
- Wave 1: S-11.02, S-11.03, S-11.04, S-11.05, S-11.06, S-11.07 (all depend on S-11.01; parallel)
- Wave 2: S-11.08 (depends on all Wave 1 stories merged + pre-deletion audit passes)
...
Mechanical renumber of all 8 E-11 story IDs +1 due to collision with existing
S-11.00 stub in STORY-INDEX (verify-sha-currency.sh Rust port, filed D-297,
unanchored to any epic). POLICY 1 (append-only, no recycling) prevented reusing
S-11.00 for the perf-baseline pre-work story.

Renumbering:
- S-11.00 (perf baseline) → S-11.01
- S-11.01 (B-1: protect-bc + protect-vp) → S-11.02
- S-11.02 (B-2: convergence-tracker + purity-check) → S-11.03
- S-11.03 (B-3: brownfield-discipline + factory-branch-guard) → S-11.04
- S-11.04 (protect-secrets) → S-11.05
- S-11.05 (check-factory-commit) → S-11.06
- S-11.06 (red-gate) → S-11.07
- S-11.07 (destructive-command-guard + adapter retirement) → S-11.08

Dependency-graph topology preserved (perf baseline still pre-blocks all batch
stories; adapter retirement still depends on all batch closures). Total story
count unchanged at 8.

Resolution committed by state-manager at 14bb9c4 (STORY-INDEX side); this entry
synchronizes the epic body. No semantic changes.
