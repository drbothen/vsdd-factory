...
> S-8.02 v1.3 (status=draft, pass-4 fix burst applied 2026-05-01). Pass-1: 13 findings closed (4H+5M+3L+1NIT); 322→383 lines (+61). Pass-2: 6 findings (2H+3M+1L); 383→427 lines (+44). Pass-3: NITPICK_ONLY (0H+0M+2L+2NIT); clock held 1_of_3 (no fix burst required). Pass-4: 4 findings (1H+1M+2L); 427→436 lines (+9). Pass-4 fixes: F-S802-P4-001 [HIGH] SS-04 "Hook Plugins"→"Plugin Ecosystem" (2 occurrences, POLICY 6, ARCH-INDEX:77 verified); F-S802-P4-002 [MED] Token Budget refreshed (story-spec ~2,800→~12,000; Total ~9,600→~18,800); F-S802-P4-003 [LOW] AC-007 perf-log recording target added (perf-log.md, S-8.00 convention); F-S802-P4-004 [LOW] AC-008 [process-gap] T-11 added (BC-7.03.045 amendment obligation). input-hash e441e99 → 5ae44ad. adv-s8.02-p4.md persisted.
...
> S-8.05 v1.4 (status=draft, pass-4 fix burst applied 2026-05-01). Pass-1: 12 findings closed (4H+5M+2L+1NIT); 338→395 lines (+57). Pass-2: 4 findings (0H+3M+1L); all closed in v1.2. Pass-3: 5 findings (2H+1M+2L); all closed in v1.3. Pass-4: 4 findings (2H+0M+2L); all closed in v1.4. v1.4 fixes: [F-S805-P4-001] AC-008 and EC-005 reworded — emit_event returns `()` (fire-and-forget); prohibition on `?`/`unwrap()`/`expect()` dropped (nonsensical on unit return); bare statement form preferred per capture-commit-activity sibling; [F-S805-P4-002] WASI entry-point pinned — AC-001, T-2, and File Structure now specify `[lib]` + `[[bin]]` + `src/main.rs` trampoline calling `vsdd_hook_sdk::__internal::run(on_hook)`; [F-S805-P4-003] v1.2 changelog path-fix annotated as superseded; [F-S805-P4-004] T-8 capability-block removal marked pending-intent-verification. adv-s8.05-p4.md persisted.
