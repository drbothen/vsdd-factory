...
> S-8.01 v1.3 (status=draft, pass-3 fix burst applied 2026-05-01). Pass-1: 14 findings closed (4H+6M+3L+1NIT); 344→380 lines (+36). Pass-2: 4 findings (0H+2M+2L); 380→419 lines (+39). Pass-3: 7 findings (1H+3M+2L+1NIT); input-hash bcf4e0a → 7b31f6f. Key pass-3 fixes: F-S801-P3-001 [HIGH] SS-04 "Hook Plugin Ecosystem" → "Plugin Ecosystem" (3 occurrences, POLICY 6); F-S801-P3-002 [MED] AC-005 rewritten to enumerate all 7 bats cases (a-g) matching T-5; F-S801-P3-003 [MED] cross-CAP stretch disclosure adds SS-02 (host::emit_event SDK surface); F-S801-P3-004 [MED] T-3 Rust snippets: agent_name: String annotated, len_str temp bound outside slice; F-S801-P3-005 [LOW] Token Budget ~2,500→~4,000 (419 lines), Total ~9,850→~11,350; F-S801-P3-006 [LOW] Wave Assignment section added with explicit wave:15 [process-gap] D-13 disclosure; F-S801-P3-007 [NIT] subsumed by 001. adv-s8.01-p3.md persisted.

> S-8.02 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30; pass-2 SUBSTANTIVE 2026-04-30). Pass-1: 13 findings closed (4H+5M+3L+1NIT); 322→383 lines (+61). Pass-2: 6 findings (2H+3M+1L); clock held 0_of_3. Key pass-2 findings: AC-006 missing NEXT_STEP>=10 wildcard bats fixture (HIGH); AGENT 3-arm fallback chain not pinned (HIGH); grep -c line-vs-match semantics ambiguous (MED); AC-005 stderr only 1 of 11 lines pinned (MED); EC-002 truncation hint stale (MED). Pass-2 fix burst pending. adv-s8.02-p2.md persisted. D-174.

> S-8.03 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30; pass-2 SUBSTANTIVE 2026-04-30). Pass-1: 13 findings closed (4H+5M+3L+1NIT); 295→356 lines (+61). Pass-2: 9 findings (2H+4M+2L+1NIT); clock held 0_of_3. Key pass-2 findings: emit_event variadic description technically wrong — SDK is slice-of-tuples not variadic (HIGH); [hooks.capabilities] block removal scope contradicts sibling pattern (HIGH, pass-1 partial closure regression); workspace Cargo.toml members missing (MED); is_ascii_whitespace 0x0B divergence from POSIX (MED); EC-005 fire-and-forget SDK reality (MED). Pass-2 fix burst pending. adv-s8.03-p2.md persisted. D-174.

> S-8.04 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30; pass-2 SUBSTANTIVE 2026-04-30). **HIGHEST RISK — D-6 BLOCKER (host::write_file SDK extension required).** Pass-1: 17 findings closed (7H+6M+3L+1NIT); 352→485 lines (+133). Pass-2: 11 findings (3H+5M+2L+1NIT); clock held 0_of_3. Key pass-2 findings: depends_on references non-existent S-8.SDK-write-file ID (HIGH); wave-state-gate-null.yaml fixture has no content spec (HIGH, story-writer prior); bash ERE precedence test lacks concrete input/output (HIGH, story-writer prior); BC-7.03.083 postcondition "Requires jq AND python3" contradicts WASM port (MED); BC line numbers stale 877-894 vs actual 942-948 (MED). Pass-2 fix burst pending; BLOCKED on D-6. adv-s8.04-p2.md persisted. D-174.

> S-8.05 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30; pass-2 SUBSTANTIVE 2026-04-30). Pass-1: 12 findings closed (4H+5M+2L+1NIT); 338→395 lines (+57). Pass-2: 4 findings (0H+3M+1L); clock held 0_of_3. Key pass-2 findings: RESULT field dual-fallback semantics not specified (last_assistant_message ?? result ?? "") (MED); AC-003 agent-pattern dot vs hyphen notation ambiguous (MED); AC-007 case (e) input shape unspecified (MED); T-8 silent on top-level [hooks.capabilities] env_allow block removal (LOW). Pass-2 fix burst pending. adv-s8.05-p2.md persisted. D-174.
...
> S-8.07 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30; pass-2 SUBSTANTIVE 2026-04-30). Pass-1: 14 findings closed (6H+5M+2L+1NIT); 358→433 lines (+75). Pass-2: 11 findings (4H+4M+2L+1NIT) — HIGHEST HIGH count in pass-2 batch; clock held 0_of_3. CRITICAL: SS-04 labeled "Hook SDK" — ARCH-INDEX says SS-04=Plugin Ecosystem; SDK surface belongs in SS-02 (HIGH); vsdd-hook-sdk path "../../crates/hook-sdk" resolves to non-existent crates/crates/hook-sdk — cargo build-breaking (HIGH); workspace Cargo.toml members registration missing — cargo build-breaking (HIGH); read_file max_bytes+timeout_ms mandatory args unspecified (HIGH). Pass-2 fix burst pending. adv-s8.07-p2.md persisted. D-174.

> S-8.08 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30; pass-2 SUBSTANTIVE 2026-04-30). Pass-1: 12 findings closed (4H+5M+2L+1NIT); 343→385 lines (+42). Pass-2: 9 findings (2H+5M+2L); clock held 0_of_3. Key pass-2 findings: SS-04 mislabeled "cross-CAP stretch" — it is the primary anchor for crates/hook-plugins/ (HIGH); AC-002 conflates BC-7.03.079 invariant-1 (identity stability) with hook-path-lifecycle — distinct invariants (HIGH); vsdd-hook-sdk path specifier missing from Cargo.toml (MED); wasm32-wasip1 Architecture Compliance row absent (MED); wave [process-gap] sibling parity drift (LOW). Pass-2 fix burst pending. adv-s8.08-p2.md persisted. D-174.

> S-8.09 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30; pass-2 SUBSTANTIVE 2026-04-30). **D-6 BLOCKER (host::write_file SDK extension required).** Pass-1: 16 findings closed (6H+7M+2L+1NIT); 542→647 lines (+105). Pass-2: 9 findings (4H+4M+1L); clock held 0_of_3. CRITICAL: SS-04 mis-anchored to "Hook SDK" — ARCH-INDEX SS-04=Plugin Ecosystem; SDK surface belongs in SS-02 (HIGH); BC-7.03.071 invariants still fabricated — third attempt; pass-1 F-009 closed with different fabrication (HIGH REGRESSION); assumption_validations references unregistered "OQ-write_file" (HIGH); depends_on lacks SDK extension story while sibling S-8.04 declares it — sibling divergence (HIGH). Pass-2 fix burst pending; BLOCKED on D-6. adv-s8.09-p2.md persisted. D-174.
