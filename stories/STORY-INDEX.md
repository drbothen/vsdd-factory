...
> **S-8.10 v1.1 CONVERGENCE_REACHED at pass-4 (2026-05-01, D-182):** Pass-2: 7 findings (0H+2M+4L+1NIT); pass-3: 4 NITPICK_ONLY (clock 1/3); pass-4: 4 NITPICK_ONLY 0 new findings (clock 2/3→3/3 = CONVERGENCE_REACHED per ADR-013 plateau-as-convergence pattern). Trajectory 18→7→4→4. Status flip BLOCKED on OQ-A1: PO must author BC-2.02.011 (host::write_file ABI invariant) in `.factory/specs/behavioral-contracts/ss-02/` before status can flip draft → ready (Spec-First Gate S-7.01). adv-s8.10-p4.md persisted.
>
...
> S-8.05 v1.5 (status=draft, SUBSTANTIVE pass-6, D-182 — NOT converging). Trajectory 12→7→5→4→4→5. Pass-1..5: prior findings closed. Pass-6 SUBSTANTIVE 2 HIGH + 2 MED + 1 LOW novel deeper-layer findings: F-S805-P6-001 HIGH jq `//` null-as-alternative semantics divergence (`Option::or` misses present-but-`Value::Null`); F-S805-P6-002 HIGH `envelope` undefined — typed-projection mismatch with `HookPayload` typed struct (implementer cannot write working code); F-S805-P6-003 MED AC-007 case (e) uses `event_type` (SDK uses `event_name`) + value `PostToolUse` (hook fires on `SubagentStop`); F-S805-P6-004 MED AC-008 still allows `let _ =` form (partial-fix propagation gap); F-S805-P6-005 LOW AC-001 omits `name` field. v1.6 fix burst required (all 5 items). Clock HELD at 0/3. adv-s8.05-p6.md persisted.
