...
> S-8.01 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). 14 findings closed (4H+6M+3L+1NIT); 344→380 lines (+36). Universal patterns applied. Key fixes: wasm32-wasip1; hooks.json positive verification; subsystems+=SS-04; CAP-022 stretch; wave:15 [process-gap]; input-hash convention; AC-005 perf gate dropped (E-8 AC-7 Tier 1 exclusion); emit_event signature mapped; BC trace re-anchored. Clock 0_of_3 (held; pass-2 dispatch next). D-173.

> S-8.02 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). 13 findings closed (4H+5M+3L+1NIT); 322→383 lines (+61). Universal patterns applied. Key fixes: wasm32-wasip1; hooks.json positive verification; subsystems+=SS-04; AC-005 hint string completed; 10th catch-all case arm added; BC-7.03.046 title ASCII/Unicode contract aligned; jq-missing degradation contract added; emit_event signature mapped; capability justification corrected. Clock 0_of_3 (held; pass-2 dispatch next). D-173.

> S-8.03 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). 13 findings closed (4H+5M+3L+1NIT); 295→356 lines (+61). Universal patterns applied. Key per-story fixes: byte-vs-char parity defect resolved (wc -c = bytes; Rust .chars().count() = Unicode scalars; parity contract pinned); multiline regex anchor parity fixed (bash grep ^ per-line vs Rust default); AC-005 BC re-anchored (BC-7.03.082 emit_event postcondition); +AC-007 malformed JSON graceful exit (aligns with S-8.01 sibling); T-3 whitespace-classifier contradiction resolved; [process-gap] disclosures added. Clock 0_of_3 (held; pass-2 dispatch next). D-173.

> S-8.04 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). **HIGHEST RISK STORY — 17 findings closed (7H+6M+3L+1NIT); 352→485 lines (+133). BLOCKED pending host::write_file SDK extension (D-6 Option A).** Universal patterns applied. Key per-story fixes: YAML semantic trichotomy pinned (None/missing-key/"not_started"); TOCTOU + non-atomic write contract disclosed; bash ERE alternation precedence clarified; EC-003 emission contract corrected; python3 drift addressed; Library table TBD row added for host::write_file; depends_on extended with SDK extension story TBD. D-6 Option A blocker explicitly disclosed in story. Clock 0_of_3 (held; BLOCKED until SDK PR merges). D-173.

> S-8.05 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). 12 findings closed (4H+5M+2L+1NIT); 338→395 lines (+57). Universal patterns applied. Key per-story fixes: AC-006 Check 3a verdict accumulation logic corrected; stderr format now enumerates verbatim 3-line bash remediation block; binary_allow cleanup completed (bash/jq/gh all listed in tasks); stretch-anchor SS-07 claim corrected post-port; emit failure semantics specified. Clock 0_of_3 (held; pass-2 dispatch next). D-173.

> S-8.06 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). 11 findings closed (4H+5M+1L+1NIT); 354→419 lines (+65). Universal patterns applied. Key per-story fixes: hooks.json positive verification in AC-002; BC-7.03.076 vs AC-001 binary_allow=empty contradiction resolved; AC-006 fabricated claim removed and BC-aligned; AC count reduced 6→4 (over-decomposition fix; AC-002+AC-006 collapsed); std::fs used (no host::write_file needed for session-learning); EC-005 stdin WASI SIGPIPE risk documented. Clock 0_of_3 (held; pass-2 dispatch next). D-173.

> S-8.07 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). 14 findings closed (6H+5M+2L+1NIT); 358→433 lines (+75). Universal patterns applied. Key per-story fixes: serde_yaml pinned at 0.9.34 (deprecated by Dtolnay 2024; TD entry logged for future migration; chosen over serde_yaml_ng for current API compatibility); emit_event signature mapped; python3 soft-dep path mapped; read_file capability declaration added; wave:15 [process-gap] disclosure added. Clock 0_of_3 (held; pass-2 dispatch next). D-173.

> S-8.08 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). 12 findings closed (4H+5M+2L+1NIT); 343→385 lines (+42). Universal patterns applied. Key per-story fixes: hooks.json positive verification across ACs/Tasks/File-list; Goal updated to reflect registry-only reality; AC-002 BC-7.03.079 invariant 1 re-anchored correctly; SS-07 ownership claim corrected post-port; latency goal made advisory; T-6 exec_subprocess removal aligned; perf gate made advisory (Tier 1 exclusion). Clock 0_of_3 (held; pass-2 dispatch next). D-173.

> S-8.09 v1.1 (status=draft, pass-1 fix burst applied 2026-04-30). **BLOCKED pending host::write_file SDK extension (D-6 Option A); 16 findings closed (6H+7M+2L+1NIT); 542→647 lines (+105).** Universal patterns applied. Key per-story fixes: Library table TBD row added for host::write_file; depends_on extended with SDK extension story TBD; OQ-6 audit doc schema defined; AC-009 "may be revised" qualifier removed (deterministic); AC-007 9-pattern coverage — all 9 patterns enumerated + all 9 have bats fixture stubs; AC-002 blast-radius safeguard added; AC-011 sequencing restructured (cannot audit what it depends on at close — audit scope narrowed); AC-012 find command bounded; AC-008 BC trace fixed (BC-7.03.071 invariant 2 enumerated); EC-006 silent write failure handling added per AGENT-SOUL; regression-gate tool-filter specified. Clock 0_of_3 (held; BLOCKED until SDK PR merges). D-173.
