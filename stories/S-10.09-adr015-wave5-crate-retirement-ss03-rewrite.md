...
### AC-001: sink-otel-grpc crate physically deleted (traces to ADR-015 D-15.1 §Retired (Wave 5)) (traces: BC-1.12.007 postcondition 4)
...
### AC-002: Router, SinkRegistry, DlqWriter types removed from sink-core (traces to ADR-015 D-15.1 §Wave 5) (traces: BC-1.12.007 postcondition 4)
...
### AC-003: No workspace crate depends on deleted types (traces to ADR-015 D-15.1 §TD-015-a) (traces: BC-1.12.007 invariant 1)
...
|-------|-------|-----------|------------|-------|
| [BC-1.12.007](../specs/behavioral-contracts/ss-01/BC-1.12.007.md) | factory-dispatcher::deprecation_lifecycle::wave1_call_graph_invariant — Router, SinkRegistry, DlqWriter, and sink-otel-grpc NOT called from any production code path after Wave 1; deprecated crates excluded from default-members; TD-015-a CI gate (tool selected D-318: cargo-call-stack; implementation deferred to Wave 5) | SS-01 | CAP-029 | Wave 5 — physical deletion completes the two-phase lifecycle started at Wave 1 (S-10.02); AC-001/002/003/007 |

### AC-007: Wave 1 call-graph invariant completed via physical deletion (traces: BC-1.12.007 postcondition 1)

The call-graph invariant established by S-10.02 (Wave 1) is permanently completed by
this story. After physical deletion of `sink-otel-grpc` and `Router`/`SinkRegistry`/
`DlqWriter` types, there is no possibility of any code path calling these APIs —
not from `main.rs`, and not from any other workspace crate. The TD-015-a prerequisite
check (cargo-metadata audit for intra-workspace re-coupling) MUST be performed (T-1)
before deletion to prevent workspace breakage per BC-1.12.007 invariant 2.

**Falsifiable test:** After deletion, `cargo build --workspace` exits 0. Final grep
confirms zero hits for `Router::`, `SinkRegistry::`, `DlqWriter::` in `crates/`.
`publish = false` in `sink-otel-grpc/Cargo.toml` is moot post-deletion (crate gone).
...
~12% of a 200k context window. Well within budget.
...
| v1.1 | 2026-05-06 | D-313 Phase 1c POLICY 8 propagation: behavioral_contracts array populated with 1 BC (BC-1.12.007). BC table added. AC-001/002 traced to BC-1.12.007 postcondition 4; AC-003 traced to BC-1.12.007 invariant 1. New AC-007 added for BC-1.12.007 postcondition 1 (Wave 5 physical-deletion perspective). Phase 1b (D-313) BC propagated. BC-1.12.007 anomaly: BC Traceability names S-10.02 AND S-10.09; both stories updated. |
| v1.2 | 2026-05-06 | D-316: POLICY 8 sweep for BC-1.12.007 D-315 changes (CAP-003 → CAP-029; F-9 P1 + Inv1 sharpening: callgraph invariant enforcement now specifies cargo-call-stack as primary verification tool). F-14 wave-terminology note added. |
