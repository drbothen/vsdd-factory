...
| [BC-1.12.009](../specs/behavioral-contracts/ss-01/BC-1.12.009.md) | factory-dispatcher::dual_emit::pair_identity_contract — event.correlation_id / event.deprecated_by / event.replaces_deprecated_alias field semantics; five-state event classification (paired-current / paired-deprecated / orphaned-current-half / orphaned-deprecated-half / non-paired); malformed → orphaned-half downgrade rule; consumer degradation rule for orphaned halves (ADR-015 D-15.2.e v1.5) | SS-01 | CAP-029 | Wave 2 — dual-emit pair identity contract; consumer classification rules (AC-002, AC-010) |
...
| BC files (2 BCs) | ~4,000 |
| 11 plugin source files (partial reads) | ~12,000 |
| S-10.01 measurement outputs | ~3,000 |
| Test file context | ~4,000 |
| **Total** | **~37,000** |

~18% of a 200k context window. Within budget but approaching split threshold for a
