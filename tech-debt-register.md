...
## TD-074 — Three sibling WaveEntry struct definitions that can drift

**Severity:** MEDIUM
**Status:** OPEN — deferred to future maintenance cycle
**Adopted:** 2026-05-10
**Filed in:** F5 pass-4 fix-burst (feature/F5-pass-3-cycle-hardening)

### Problem Statement

Three independent Rust struct definitions exist for the same `wave-state.yaml` schema:

1. `crates/vsdd-context-resolvers/src/wave_context.rs::WaveEntry` (public, Deserialize)
2. `crates/hook-plugins/update-wave-state-on-merge/src/lib.rs::WaveEntry` (private, Serialize+Deserialize)
3. `crates/hook-plugins/warn-pending-wave-gate/src/lib.rs::WaveEntry` (private, Deserialize)

TD-073 (now RESOLVED) fixed the SYMPTOM: schema disagreement between MAPPING form and
SEQUENCE form. But the root cause — three independent definitions — remains. If any one
diverges in the future (e.g., a new field added to one but not others), the schema
mismatch can silently recur.

The `warn-pending-wave-gate` struct has fewer fields (only `wave` and `gate_status`)
than the producer (`update-wave-state-on-merge`) because it only needs those fields.
This is valid today due to `#[serde(default)]`, but adds an implicit contract that
must be maintained.

### Resolution Path

Hoist into a shared workspace crate `vsdd-wave-state` (or add to `vsdd-hook-sdk`) with:
- Canonical `WaveEntry` + `WaveState` structs
- Serde feature flag (to avoid pulling serde into non-serde contexts)
- All three consumers depend on it

This is a MEDIUM blast-radius refactor. Appropriate for a future maintenance wave
(Path 3 — refactor cycle) or as part of a story that already touches wave-state
processing.

### References

- F-P4-006 finding (adversarial review pass-4)
- `TODO(TD-074)` comments in all three sibling files
- TD-073 (resolved — MAPPING vs SEQUENCE fix) — this is the structural follow-up
