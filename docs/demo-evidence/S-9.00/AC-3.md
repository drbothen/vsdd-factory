...
bash .factory/measurements/measure-bundle-sizes.sh \
  ...
  jq '{all_hook_plugins_wasm_bytes, dispatcher_bytes, grand_total_bytes}'
```
...
| Current `all_hook_plugins_wasm_bytes` | 8,704,199 | Well above advisory cap (by design — advisory cap was pre-W-15 baseline) |
...
- Current `all_hook_plugins_wasm_bytes` = 8,704,199 — exceeds advisory cap because W-15 already ported 17 plugins. The advisory cap model applies to future growth, not to block the already-shipped W-15 work.
...
PASS — JSON output has distinct `all_hook_plugins_wasm_bytes`, `grand_total_bytes`, and `dispatcher_bytes` fields. Equality check: 8,704,199 + 12,250,912 = 20,955,111 (grand_total_bytes verified). Gate model committed to `perf-baseline-w16.md` with all required fields. Bats test AC-3 passes.
