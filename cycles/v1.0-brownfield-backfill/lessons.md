...
## L-BB-namespace-migrations-must-sweep-vp-traceability-and-harness-blocks [process-gap] [codified D-811]

**Category:** process-gap

**Status:** CODIFIED D-811

**Lesson:** When a path/namespace migration lands in a BC (like D-775 hooks/→bin/ for BC-5.42.001 v1.2), the sweep enumeration MUST include VP §Traceability bullets AND §Proof Harness Skeleton code blocks citing the migrated paths. Harness code blocks are load-bearing for test-writers even though they are fenced code, not prose.

**Context:** E-19 pass-55 F-P55-001 found VP-094 v1.1 citing `plugins/vsdd-factory/hooks/check-stale-verdict.sh` and `plugins/vsdd-factory/hooks/enforce-merge-strategy.sh` in both its §Traceability bullet (1 site) and its §Proof Harness Skeleton bats invocations (5 sites). The canonical namespace per BC-5.42.001 §Architecture Anchors, S-19.01 §File Structure, and ADR-030 §Decision 2/3 is `plugins/vsdd-factory/bin/`. The §Traceability label also read "Hook scripts:" when "Enforcement scripts:" is correct for bin/-semantics.

**Root cause:** D-775 introduced the BC-5.42.001 v1.2 migration establishing the hooks/→bin/ split for pr-manager orchestrator-invoked scripts. The D-775 sweep enumerated BC §Architecture Anchors and story §File Structure but did not enumerate VP §Traceability bullets or §Proof Harness Skeleton code blocks as sweep sites. VP-094 was created at v1.0 before D-775 (2026-07-06). Neither F-P42-001 (D-797, BC-version-pin stable-anchor sweep) nor subsequent passes enumerated harness code blocks as a sweep category. The finding survived 32 passes (23–54). This extends the D-809 lesson (L-BB-description-bearing-anchor-prose-derives-from-target-SoT) by one axis: §Proof Harness Skeleton code blocks are also description-bearing anchor prose for sweep purposes, not just §Traceability prose bullets.

**Gate:** When a namespace/path migration is committed in a BC or ADR, the migration sweep must enumerate ALL of: (1) BC §Architecture Anchors — already covered; (2) story §File Structure — already covered by D-809; (3) VP §Traceability bullets — required per D-809; (4) **VP §Proof Harness Skeleton code blocks** — newly required per D-811; (5) sibling VPs sharing the same anchor story — covered by F-P42-001 discipline. Detection predicate: `grep -rn "<old-path>" .factory/specs/verification-properties/VP-*.md` checking BOTH prose and fenced code blocks (harness `run bash <path>` and `# script: <path>` patterns).

**Prevention:** (1) Every namespace/path migration that touches a BC §Architecture Anchors field must trigger a grep sweep of all VP files anchored to that BC — checking BOTH prose and fenced code blocks. (2) §Proof Harness Skeleton code blocks contain `run bash <script-path>` and `# script: <path>` patterns; these are structural test-harness anchors that must be updated when the target script moves namespaces. (3) The class-sweep classification table (genuine-hook vs bin/-semantics) is a useful discrimination tool when checking VP clusters.

**Anchors:** D-811; F-P55-001 (MEDIUM POLICY 4 v1.4.4 + POLICY 5 v1.3.3 regression; VP-094 v1.1 §Traceability bullet + 5 §Proof Harness Skeleton lines stale hooks/→bin/); architect 364cfd2c (fix: VP-094 v1.1→v1.2; 6 sites corrected; class sweep 8 genuine-hook VPs clean); D-775 (BC-5.42.001 v1.2 hooks/→bin/ migration origin); VP-094 v1.2 (fix); VP-INDEX v2.60→v2.61 (SM D-811 annotation).

**Cites:** D-811; F-P55-001 (MEDIUM); POLICY 4 v1.4.4 (description-bearing anchor-prose parity — extended to harness code blocks); POLICY 5 v1.3.3 (regression: D-775 migration propagation gap); D-809 L-BB-description-bearing-anchor-prose-derives-from-target-SoT (direct precedent: POLICY 4 derivation principle extended from VP §Traceability prose to VP §Proof Harness code blocks); D-797 L-BB-vp-source-bc-sibling-sweep (companion: BC-pin stable-anchor discipline); TD-VSDD-091 (volatile-pin prohibition — stable structural anchors).

**Closes:** D-811 (F-P55-001 CLOSED architect 364cfd2c VP-094 v1.1→v1.2; 6 stale hooks/→bin/ sites corrected; §Traceability label relabeled; input-hash 9eff742→e2f422f; class sweep 8 genuine-hook VPs CLEAN; VP-INDEX v2.60→v2.61; L-BB codified). `[process-gap; namespace-migration; hooks-to-bin; VP-traceability-bullet; VP-proof-harness-skeleton; sweep-site; description-bearing-anchor-prose; POLICY-4; POLICY-5-regression; harness-code-blocks; D-775-propagation-gap; self-application]`

---

## L-BB-anchor-prose-parity-includes-diagnostic-strings-and-exit-codes [process-gap] [codified D-812]

**Category:** process-gap

**Status:** CODIFIED D-812

**Lesson:** POLICY 4 v1.4.4 anchor-prose-parity extends to sentinel-string VALUES and exit-code INTEGERS in VP §Property Statement PS-* descriptions and §Proof Harness test assertions. These are load-bearing test-fidelity anchors because they determine whether the proof harness tests for the correct observable behavior. The source of truth is the source BC §VP-table canonical test vectors and anchored ADR §Decision behavioral specifications.

**Context:** E-19 pass-56 F-P56-001 found VP-094 v1.2 §Property Statement PS-B asserting `VERDICT_STALE` (stale) as the sentinel string for the stale-verdict halt path, where the canonical sentinel per BC-5.42.001 §Postcondition 2 + ADR-030 §Decision 2 is `STALE_READY_VERDICT`. VP-094 §Property Statement PS-C asserted `MERGE_STRATEGY_REQUIRED` as the sentinel for the release-PR merge-strategy enforcement path, where the canonical sentinel per BC-5.42.001 §Description (c) + ADR-030 §Decision 3 is `RELEASE_PR_SQUASH_FORBIDDEN`. VP-094 also asserted exit code 2 (fail-closed block-intent) where ADR-030 §Decision 2 and §Decision 3 specify exit 1 (fail-closed without block-intent). PS-B directed STALE_READY_VERDICT to stdout where ADR-030 §Decision 2 specifies stderr. §Proof Harness Skeleton propagated all stale values across 16 total sites. The stale content inherited from VP-094 v1.0 initial creation survived 32 passes (22–55).

**Root cause:** The prior 9 standing gates (D-811 roster) verified path/namespace anchor parity (D-806/D-797/D-811), description-bearing prose parity for module: and §Traceability bullet text (D-809), BC-version-pin stable-anchor form (D-797), and harness code block sweep (D-811) — but none extracted and compared the LITERAL sentinel string values or exit-code integers cited inside §Property Statement PS-* descriptions and §Proof Harness test assertions against the source BC §VP-table postcondition canonical form. A VP's §Property Statement can silently inherit wrong sentinel strings from its initial creation if the source BC's §VP-table test vectors were updated after the VP was created, and no gate checks the value parity. BC-5.42.001 §Canonical Test Vectors and §VP-table rows are the authoritative source for the exact sentinel strings and exit codes that a VP must assert; they changed between VP-094 v1.0 and v1.2 but the VP prose/harness was never synchronized.

**Gate (10th standing gate):** For any VP fix burst, extract VP §Property Statement PS-* sentinel strings and exit codes; compare against source BC §Canonical Test Vectors §VP-table postcondition rows AND anchored ADR §Decision behavioral specifications. Expected: exact string match. Any mismatch = POLICY 4 v1.4.4 MEDIUM mis-anchor. Detection predicate:
- Grep VP §Property Statement for sentinel strings: `grep -n "SENTINEL_STRING\|exit [0-9]" VP-NNN.md`
- Compare against BC §VP-table: `grep -n "sentinel\|exit code\|exit [0-9]" BC-N.NN.NNN.md`
- Compare against ADR §Decision behavioral clauses: `grep -n "exit [0-9]\|sentinel\|emits.*to stderr\|emits.*to stdout" ADR-NNN*.md`

**Prevention:** (1) When creating a VP, extract ALL sentinel strings and exit codes from the source BC §VP-table postcondition rows and the anchored ADR §Decision behavioral specs. (2) When a BC or ADR is updated (e.g., a new version adds a canonical sentinel string or changes an exit code), enumerate all VPs that source from that BC and check §Property Statement and §Proof Harness assertions for value parity. (3) The 10th standing gate is now part of the mandatory Commit-E roster per D-812.

**Anchors:** D-812; F-P56-001 (MEDIUM POLICY 4 v1.4.4 novel axis: content-of-anchored-postcondition parity; VP-094 v1.2 PS-B VERDICT_STALE/exit-2/stdout + PS-C MERGE_STRATEGY_REQUIRED/exit-2; 16 sites; BC-5.42.001 §VP-table self-contradiction); architect 93d3ca03 (fix: VP-094 v1.2→v1.3; 16 sites corrected; class sweep VP-095..VP-101 ALL CLEAN; input-hash e2f422f UNCHANGED); VP-094 v1.3 (fix); VP-INDEX v2.61→v2.62 (SM D-812 annotation); BC-5.42.001 v1.6 (source: §VP-table canonical test vectors; §Postcondition 2 STALE_READY_VERDICT; §Description (c) RELEASE_PR_SQUASH_FORBIDDEN; ADR-030 §Decision 2/3 exit-1 fail-closed).

**Cites:** D-812; F-P56-001 (MEDIUM POLICY 4 v1.4.4); POLICY 4 v1.4.4 (description-bearing anchor-prose parity — newly extended to sentinel-string values and exit-code integers; 10th standing gate axis); D-809 L-BB-description-bearing-anchor-prose-derives-from-target-SoT (direct precedent: POLICY 4 derivation principle — D-812 extends from path/prose to sentinel-string/exit-code VALUES); D-811 L-BB-namespace-migrations-must-sweep-vp-traceability-and-harness-blocks (companion: harness code blocks are sweep sites; D-812 extends further to assert correct VALUES inside those blocks); BC-5.42.001 v1.6 (SoT: §VP-table canonical test vectors); ADR-030 v1.3 §Decision 2/3 (SoT: behavioral exit-code specification).

**Closes:** D-812 (F-P56-001 CLOSED architect 93d3ca03 VP-094 v1.2→v1.3; 16 sites corrected: VERDICT_STALE→STALE_READY_VERDICT ×5, MERGE_STRATEGY_REQUIRED→RELEASE_PR_SQUASH_FORBIDDEN ×3, exit-2→exit-1 ×4+prose, PS-C message text canonicalized, stdout→stderr fix; class sweep VP-095..101 ALL CLEAN; input-hash e2f422f UNCHANGED; VP-INDEX v2.61→v2.62; 10th standing gate codified). `[process-gap; sentinel-string; exit-code; content-of-anchored-postcondition; POLICY-4-v1.4.4; proof-harness-assertions; property-statement; BC-VP-table; ADR-behavioral-spec; test-fidelity-anchor; 10th-gate; value-parity; self-application]`

---

## L-BB-invocation-signature-migrations-must-sweep-vp-proof-harness-skeleton [process-gap] [codified D-815]

**Category:** process-gap

**Status:** CODIFIED D-815

**Lesson:** POLICY 4 v1.4.5 anchor-prose-parity extends to §Proof Harness script invocation-form: the argument signature (positional vs named-flag, including required positional arguments like `<pr_number>`) of script invocations inside VP §Proof Harness @test blocks must match the ADR §Decision canonical invocation form. A mismatched invocation form defeats the test's ability to validate the load-bearing behavior (a script cannot test what it cannot invoke correctly). When an ADR adjudicates an invocation-form migration (e.g., named-flag → positional), propagation directives MUST enumerate VP §Proof Harness INVOCATION SIGNATURES as sweep sites.

**Context:** E-19 pass-57 F-P57-001 found VP-094 v1.3 §Proof Harness Skeleton all five script-invoking @test blocks invoking scripts with superseded named-flag signatures: `check-stale-verdict.sh --covered-sha "$covered_sha" --live-sha "$live_sha"` and `enforce-merge-strategy.sh --branch "release/v1.0.0-rc.23" --strategy "squash"`. The canonical invocation per ADR-030 §Decision 2 is `check-stale-verdict.sh <pr_number> <covered_sha>` (positional; live HEAD SHA obtained internally via `gh pr view <pr_number> --json headRefOid`). The canonical invocation per ADR-030 §Decision 3 is `enforce-merge-strategy.sh <pr_number> [--merge|--squash|--rebase]` (branch name obtained internally via `gh pr view <pr_number> --json headRefName`). No `<pr_number>` argument was passed at all. The named-flag `--live-sha` defeats the design: `check-stale-verdict.sh` exists precisely to fetch the live HEAD via `gh pr view`; a `--live-sha` parameter would reduce it to a pure two-value comparison that cannot detect a real advanced HEAD in production. The D-812 fix (pass-56) corrected sentinel-string VALUES and exit-code INTEGERS inside these same harness lines but did not sweep the invocation signature itself, confirming that sentinel/exit parity (10th gate) is a distinct sub-axis from invocation-form parity (11th gate).

**Root cause:** ADR-030 v1.1 changelog recorded the named→positional signature migration (F-W1V-002) with propagation directives issued for BC-5.42.001 §Architecture Anchors and S-19.01 §Architecture Mapping. VP-094 §Proof Harness was not named in those directives. VP-094 was created at v1.0 using the pre-migration named-flag form. The named-flag form survived ADR-030 v1.1 adoption (the migration registered its own propagation scope but excluded VP §Proof Harness). The 10 prior standing gates (D-812 roster) verified path/namespace parity (D-811), sentinel-string/exit-code value parity (D-812), and description-bearing prose parity (D-809) — but none compared the invocation-form (argument structure) of harness script calls against ADR §Decision canonical invocation declarations. Sentinel/exit parity checking touched the same harness lines without recognizing the invocation-form as a separate dimension.

**Gate (11th standing gate):** For any VP fix burst touching script-invoking §Proof Harness @test blocks, or for any ADR update adjudicating an invocation-form migration: grep VP §Proof Harness script invocation lines for `--` named-flag patterns; compare against ADR §Decision canonical invocation form (positional vs named-flag, argument order, required arguments). Expected: zero unexpected named-flag patterns vs ADR canonical form. Any mismatch = POLICY 4 v1.4.5 MEDIUM mis-anchor. Detection predicate:
- Grep VP §Proof Harness for script invocations: `grep -n "run bash\|result=\$(bash" VP-NNN.md`
- For each invocation, extract argument form and compare against ADR §Decision `**Invocation:**` clause
- Flag any `--named-flag` patterns that contradict the ADR canonical positional form
- Also verify `<pr_number>` or equivalent required leading positional argument is present when ADR specifies it

**Prevention:** (1) When authoring or modifying a VP §Proof Harness, extract the canonical invocation form from the ADR §Decision `**Invocation:**` clause — use that exact positional/named-flag signature with a `gh`-mocked test double. (2) When an ADR adjudicates an invocation-form migration, the propagation directives MUST enumerate: (a) BC §Architecture Anchors; (b) story §File Structure; (c) VP §Proof Harness invocation signatures — all three are equally load-bearing anchors for the script's interface contract. (3) Pre-pass class-sweep completeness (D-798 4th standing gate) must now include invocation-signature-form parity as a sweep sub-axis whenever the perimeter includes a VP with script-invoking @test blocks.

**Anchors:** D-815; F-P57-001 (MEDIUM POLICY 4 v1.4.5 + POLICY 5; VP-094 v1.3 all five script-invoking @test blocks named-flag vs ADR-030 §Decision 2/3 positional; blast radius 1 file); architect 6716b14b (fix: VP-094 v1.3→v1.4; all five @test blocks rewritten to positional SoT form + gh mock; O-P57-001 VP-094-B false-RED exit-code capture → bats run/$status; O-P57-002 PS-C 'exits 0 unconditionally' → 'delegates to gh pr merge and propagates its exit code'; class sweep VP-095..VP-101 + E-19 stories CLEAN; input-hash e2f422f UNCHANGED); VP-094 v1.4 (fix); VP-INDEX v2.62→v2.63 (SM D-815 annotation); ADR-030 v1.3 §Decision 2/3 (SoT: canonical positional invocation forms + gh-fetch design); BC-5.42.001 v1.6 (SoT: §Description positional form); policies.yaml v1.4.5 (POLICY 4 INVOCATION-SIGNATURE-FORM PARITY verification_step extension).

**Cites:** D-815; F-P57-001 (MEDIUM POLICY 4 v1.4.5 + POLICY 5); POLICY 4 v1.4.5 (description-bearing anchor-prose parity — newly extended to §Proof Harness script invocation-form; 11th standing gate axis); D-812 L-BB-anchor-prose-parity-includes-diagnostic-strings-and-exit-codes (direct precedent: POLICY 4 derivation principle — D-815 extends from sentinel-string/exit-code VALUES to invocation-form STRUCTURE; both are load-bearing test-fidelity anchors inside the same §Proof Harness @test block); D-811 L-BB-namespace-migrations-must-sweep-vp-traceability-and-harness-blocks (companion: harness code blocks are sweep sites; D-815 adds invocation-form as another sweep dimension); ADR-030 v1.1 F-W1V-002 (origin: named→positional migration whose propagation directives omitted VP §Proof Harness); TD-VSDD-091 (volatile-pin prohibition — structural anchor discipline extends to invocation-form).

**Closes:** D-815 (F-P57-001 CLOSED architect 6716b14b VP-094 v1.3→v1.4; all five script-invoking @test blocks rewritten to positional SoT form + gh mock; O-P57-001 fixed: VP-094-B bats run/$status; O-P57-002 fixed: PS-C 'delegates to gh pr merge'; class sweep VP-095..101 + E-19 stories CLEAN; input-hash e2f422f UNCHANGED; VP-INDEX v2.62→v2.63; policies.yaml v1.4.4→v1.4.5 INVOCATION-SIGNATURE-FORM PARITY; 11th standing gate codified). `[process-gap; invocation-signature; positional-form; named-flag-superseded; ADR-decision-canonical-invocation; VP-proof-harness-skeleton; gh-mock; pr_number-required; propagation-directive-gap; POLICY-4-v1.4.5; POLICY-5-regression; 11th-gate; invocation-form-parity; self-application]`

---

### L-BB-adr-anchor-fields-must-propagate-with-governing-adr [process-gap]

**Title:** ADR Propagation Directives Must Enumerate VP §Source Contract / §Traceability `**ADR:**` Anchor Fields

**Lesson:** When an ADR is created or updated, its propagation directives MUST enumerate VP §Source Contract and §Traceability `**ADR:**` anchor fields for every VP whose §Property Statement cites that ADR. A VP that cites an ADR in its §Property Statement PS-* clauses but names a different artifact (or nothing) in its §Source Contract `**ADR:**` bullet and §Traceability `**ADR:**` bullet contains a same-document contradiction — an internal incoherence that is HIGH severity under POLICY 4 semantic-anchoring-integrity.

**Context:** E-19 pass-58 F-P58-001 found VP-094 v1.4 §Source Contract `**ADR:**` bullet stating `RELEASING.md §Merge requirements (merge-strategy invariant is an established release procedure constraint, not an ADR-documented decision; BC-5.42.001 formalizes it)`. This claim is stale/false: ADR-030 §Decision 3 documents the merge-strategy enforcement decision verbatim, and ADR-030 §Decision 2 documents the stale-verdict decision (exit 1 fail-closed) — neither has RELEASING.md provenance. VP-094 §Property Statement PS-B cites "per ADR-030 §Decision 2" and PS-C cites "per ADR-030 §Decision 3". VP-094 §Traceability omits the `**ADR:**` bullet entirely. This creates a same-document contradiction: §Property Statement acknowledges ADR-030 governs two of three postconditions, while §Source Contract denies it. Sibling VPs VP-095, VP-097, VP-098 all cite their governing ADRs in both §Source Contract and §Traceability.

**Root cause:** ADR-030 v1.1 changelog issued propagation directives "for BC-5.42.001 §Architecture Anchors and S-19.01 §Architecture Mapping" (F-W1V-002). These directives did not enumerate VP-094 §Source Contract/§Traceability `**ADR:**` anchor fields. VP-094 was created (v1.0) before ADR-030 was in place. When ADR-030 v1.1 was adopted, its propagation scope listed only BC §Architecture Anchors and story §Architecture Mapping — not VP §Source Contract/§Traceability. The structured `**ADR:**` fields (3 fields: §Source Contract + §Traceability per sibling convention) are analogous to §Architecture Anchors in BCs and §Architecture Mapping in stories — they are the VP's structured governance traceability fields. Passes 42/55/56/57 all corrected other VP-094 fields (source_bc volatile pin, namespace path, sentinel/exit, invocation-form) but none caught the §Source Contract ADR-anchor denial because no standing gate checked this field.

**Gate (12th standing gate):** For any VP fix burst or pass perimeter including a VP whose §Property Statement cites an ADR: verify (a) §Source Contract `**ADR:**` bullet names the same ADR, (b) §Traceability has a `**ADR:**` bullet naming the same ADR, (c) no 'not an ADR-documented decision' claim when an ADR §Decision governs the postcondition. Sibling convention: VP-095 §Source Contract + §Traceability both cite `ADR-025 Decision 14`; VP-097 both cite `ADR-025 Decision 13`; VP-098 both cite `ADR-025 Decision 13`. Detection predicate:
- `grep -A5 "## Source Contract" VP-NNN.md | grep "\\*\\*ADR:\\*\\*"` → must name governing ADR
- `grep -A5 "## Traceability" VP-NNN.md | grep "\\*\\*ADR:\\*\\*"` → must name same governing ADR
- `grep "not an ADR-documented decision" VP-NNN.md` → must be empty when VP §Property Statement cites an ADR §Decision
- Compare §Source Contract ADR bullet vs §Property Statement PS-* ADR cites → must name same ADR

**Prevention:** (1) When creating a VP for a BC governed by an ADR, populate §Source Contract `**ADR:**` and §Traceability `**ADR:**` with the governing ADR + Decision reference on first authoring — do not leave as 'not applicable' or pointing at a procedural document like RELEASING.md. (2) When an ADR is created or its §Decision clauses are adopted, the propagation directives MUST enumerate: (a) BC §Architecture Anchors; (b) story §Architecture Mapping; (c) VP §Source Contract `**ADR:**` fields; (d) VP §Traceability `**ADR:**` fields — for all VPs whose §Property Statement cites that ADR. (3) Same-VP self-consistency check: if §Property Statement cites `ADR-NNN §Decision M`, §Source Contract `**ADR:**` must cite `ADR-NNN §Decision M` (not RELEASING.md or another artifact at the same hierarchy level).

**Anchors:** D-817; F-P58-001 (HIGH POLICY 4 semantic-anchoring-integrity; VP-094 v1.4 §Source Contract ADR-anchor RELEASING.md → should be ADR-030; §Traceability ADR bullet absent; same-document contradiction with §Property Statement PS-B/PS-C; blast radius 2 sections within VP-094); architect 3558b9ca (fix: VP-094 v1.4→v1.5; §Source Contract `**ADR:**` → `ADR-030 §Decision 1 + §Decision 2 + §Decision 3`; §Traceability `**ADR:**` bullet added; sibling class-sweep VP-095..VP-101 PASS; POLICY 5 v1.3.4 sweep gate: zero non-historical matches; input-hash e2f422f UNCHANGED); VP-094 v1.5 (fix); VP-INDEX v2.63→v2.64 (SM D-817 annotation); policies.yaml v1.4.5→v1.4.6 (POLICY 4 ADR-ANCHOR-FIELD PARITY verification_step extension; 12th standing gate); O-P58-001 [process-gap] (missing 12th gate — codified this burst).
