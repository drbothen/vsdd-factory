...
**Cascade trajectory:** ~41 → 14 → 8 → 3 → 5 → 2 NIT → 1 NIT → 1 HIGH → **0 CLEAN** (FIRST TRUE CLEAN of cascade). CRITICAL=0 sustained 8 consecutive passes. HIGH=0 RESTORED at pass-9. STREAK 0/3 → **1/3 FIRST ADVANCE POST-RESET**. D-494 POLICY 14 extension empirically validated by adversary independent gate execution (all 4 indexes PASS). Cure-extension parsimony confirmed (no INV-021 needed). 4-index BC v2.47/VP v2.04/STORY v3.51/ARCH v2.13. D-495 codified 2026-05-20.
...
- M3 BC cascade (BC-5.39.006 + BC-5.39.007 + BC-5.39.008): **PASS-9 PERSISTED D-495** — 9 passes; trajectory ~41→14→8→3→5→2 NIT→1 NIT→1 HIGH→0 CLEAN; CRIT=0 sustained 8 passes; HIGH=0 RESTORED at pass-9; 5 INV classes (INV-016→017→018→019→020); pass-9: FIRST TRUE CLEAN; POLICY 14 empirically validated; STREAK 1/3 FIRST ADVANCE POST-RESET; need 2 more CLEAN/NITPICK for 3-CLEAN convergence; next: pass-10; BC-INDEX v2.47 + VP-INDEX v2.04 + STORY-INDEX v3.51 + ARCH-INDEX v2.13; D-range D-001..D-495
