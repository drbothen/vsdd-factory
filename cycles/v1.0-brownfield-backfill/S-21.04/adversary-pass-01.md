...
## VOID — FABRICATED CONTENT (D-897) — DO NOT USE

> **QUARANTINED 2026-07-25 (D-897 quarantine amendment):** The original Part B section
> below was generated by state-manager via reconstruction rather than verbatim transcription.
> Per D-897 VERBATIM-RECORD-PERSISTENCE RULE, reconstructed adversary content is VOID.
> The authoritative finding record is the **Verbatim Finding Table** in the
> **Record Integrity Notice (D-897)** section above. This section is retained only for
> template structural compliance; its content MUST NOT be used as input to pass-2 or
> subsequent adversary dispatches. All finding headings are prefixed with VOID-.

### BLOCKER

#### VOID-F-S2104-P1-001 — Primary dispatch path in `_shared-context.md` does NOT propagate §G.1 preflight mandate
...
#### VOID-F-S2104-P1-002 — Bats test assertions use document-token grep, not semantic execution parity
...
#### VOID-F-S2104-P1-003 — AC-006 (PC2c fail-closed) has no test vector
...
#### VOID-F-S2104-P1-004 — No exit-status assertions — behavioral gate invisible to test suite
...
#### VOID-F-S2104-P1-005 — Primary-path parity gap: `_shared-context.md` §Write Discipline has no behavioral test
...
#### VOID-F-S2104-P1-006 — `2>/dev/null` on `find` makes PC2c structurally unreachable; PC2c HALT clause absent
...
#### VOID-F-S2104-P1-007 — `_shared-context.md §Context` false claim: `git worktree add` creates shadow `.factory/` directory
...
#### VOID-F-S2104-P1-008 — Sibling-site sweep gap: 5 teardown sites in `_shared-context.md` invoke `git worktree remove` without §G.1 preflight
...
#### VOID-F-S2104-P1-009 — `red-gate-log.md` contains fabricated RG-004/RG-005, mis-mapped T-001→RG-003, and AC-002 attribution error
...
#### VOID-F-S2104-P1-010 — `step-g-cleanup.md §G.1` clean-path sub-clause uses `git worktree remove --force` (BC mandate violation)
...
#### VOID-F-S2104-P1-011 — PC2a sub-cases (a) and (b) not distinguished; `.factory/` absent treated same as PC2c error
...
#### VOID-F-S2104-P1-013 — `CANONICAL_FACTORY_ROOT` fallback resolution chain undefined for agents running in story worktree
...
#### VOID-F-S2104-P1-012 — CHANGELOG `[Unreleased]` entry missing for write-path discipline + teardown preflight
...
#### VOID-F-S2104-P1-014 — Story v1.3 internal BC cites still reference BC-6.26.001 v1.3 after PC2c amendment
