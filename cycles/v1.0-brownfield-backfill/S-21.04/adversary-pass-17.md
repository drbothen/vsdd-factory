...
| F-S2104-P17-002 | BLOCKER | test-writer | 2e70faa8 + 1859ef70 + c89bef22 | Gate PW-B fail-closed rewrite at 2e70faa8: directive-token whitelist replaced by prohibition-token requirement (`FORBIDDEN\|Forbidden\|forbidden\|MUST NOT\|prohibited\|never\|forbid`); prohibited-target class extended to syntactic-form class (`story-worktree CWD`, `shadow subtree`, `worktree-local`, `in-worktree`, `story worktree CWD`, `worktree CWD`); M-P17-C + M-P17-C-control both RED. word-boundary `(^\|[^[:alnum:]])[Ii]n-worktree` added at 1859ef70 (orchestrator-verified residual: false-match on `<main-worktree-path>` template discovered during orchestrator verification of primary leg). `[Ww]orktree-local` bracket-class added at c89bef22 (test-writer-found residual: sentence-initial `Worktree-local` after `. ` splitter not caught by lowercase-only pattern; discovered during test-writer self-audit, 5 sites: grep pattern + comments + error message). Gate 1(d) closes conditional-scoping axis. Gate 2a tightened. Evidence that mutant-battery discipline is working as designed: two successive verification iterations surfaced progressively narrower gaps post-primary-leg |
