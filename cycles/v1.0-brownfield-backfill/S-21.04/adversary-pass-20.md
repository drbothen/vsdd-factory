...
**Open blocker:** Adversary-agent output delivery is a confirmed OPEN BLOCKER for the cascade. Remediation anchor: adversary delivery failure must be diagnosed before cascade can proceed normally. See lesson `L-BB-adversary-agent-delivery-silent`.

**ROOT CAUSE CONFIRMED (D-926; formally codified D-927):** `plugins/vsdd-factory/agents/adversary.md` frontmatter line 5 pins `model: opus`. In this environment the pin does not resolve — the subagent terminates on a terminal API error after retries and returns nothing, matching the idle failure shape of adv-p20/adv-p20b/adv-p20c. Correlation: every agent dispatched with `model: sonnet` (state-manager, test-writer, devops-engineer) delivered; adversary dispatched with `model: opus` failed ×3. Validated fix: dispatching the adversary with an explicit `model: sonnet` override (Agent-tool `model` param takes precedence over frontmatter) delivered correctly. Smoke-test stdout: `DELIVERY: OK / POLICY COUNT: 22 / POLICY 22 NAME: subagent_report_fidelity_literal_shell` — agent read real files (both values independently correct). Blast radius: seven agents pin `model: opus` — `adversary`, `codebase-analyzer`, `holdout-evaluator`, `formal-verifier`, `pr-reviewer`, `research-agent`, `spec-reviewer` — silently disabling most of the factory quality-gate layer. Engine-level P0. **Model-diversity deviation:** the `model: sonnet` override mitigation satisfies delivery but forfeits the stated "genuine perspective diversity" property; every future pass using this override MUST disclose this deviation here in MANDATORY PROVENANCE DISCLOSURE. **ARCHITECTURE DECISION REQUIRED (route to architect — do not decide here):** which model these 7 agents should pin, and whether agents should fail loudly rather than silently on unresolvable model pins. Register as a story; route to architect.
