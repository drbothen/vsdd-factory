...
|---------|----------|--------------------|
| F-S2104-P16-001 | BLOCKER | test-writer 9ab1aa32 — negation-transparency gates (Gate 1(a) now explicit MUST NOT check; `MUST[[:space:]]+use` with no negation-token between MUST and canonical absolute); sentence-complete polarity (every sentence in block checked for prohibited write targets, not only mandate sentence); abbreviation-protected sentence splitter (`cf\.\|i\.e\.\|e\.g\.\|§[0-9]\.` protected before split); M-P16-A verbatim RED (Gate 1(a) fires on `MUST NOT use canonical absolute`); M-P16-C2 verbatim RED (abbreviation-split sentence: MUST canonical absolute in one fragment and CWD target in next — Gate 5 fires) |
| F-S2104-P16-002 | HIGH | test-writer 9ab1aa32 — Gate 7(a) added: requires `\*\*Forbidden:\*\*` + `file_path="\.factory/` same line in `$spec_path_section`; Gate 7(b) added: no line with `file_path="\.factory/` may match `\*\*Correct:\*\*`; Gate 3 tightened to scope `relative path` conjunction to lines also containing `file_path="\.factory/`; M-P16-D verbatim RED (Gate 7(a) fails — CWD-relative bullet relabeled `**Correct:**`); Gate 7(a) deletion mutant RED |
| F-S2104-P16-003 | HIGH | test-writer 9ab1aa32 — anchor-uniqueness gate added: `_extract_write_discipline_prohibition_block` asserts `All.*\.factory.*artifact writes` matches exactly once in `$spec_path_section`; `#### Write Discipline` child-heading bounding added so extractor is restricted to the `#### Write Discipline` subsection, excluding read-discipline prose; M-P16-B out-of-section decoy RED (uniqueness gate fires + bounding prevents capture); in-section decoy placed inside `#### Write Discipline` before normative paragraph: uniqueness gate fires RED |
| F-S2104-P16-004 | HIGH | story-writer c9def56d — story v1.21: AC-001 Gate cell rewritten to 15-gate set reflecting HEAD predicates (negation-explicit Gate 1(a); mandate-sentence Gate 1(b) prohibited-subject alternation; sentence-scoped domain normalization named explicitly; Gates 4/5 per-sentence; Gates 6(a)/(b) two-part traversal polarity; Gates 7(a)/(b) CWD-relative-bullet polarity; anchor-uniqueness gate; `#### Write Discipline` bounding; no `per extracted line`/`per line` language; same-burst predicate-gate cell coupling note added) |
| F-S2104-P16-005 | MEDIUM | state-manager D-916 (this burst) — red-gate-log v1.13→v1.14: (a) closing completeness claim `All gates: independent, polarity-complete, zero degrees of freedom.` replaced with qualified inventory statement naming the gap; (b) `M-P15-A proof (RED)` block relabeled `M-P15-A-simplified proof (RED) [Correction at v1.14 (D-916)]`; adversary-verbatim M-P15-A (5-line text from adversary-pass-15.md Part A) recorded in Pass-16 TIER 1 with re-proof stdout; obligation-indexed AC-001 coverage table added |
| F-S2104-P16-006 | LOW | state-manager D-916 (this burst) — adversary-pass-15.md §Fix Mapping row F-S2104-P15-005 corrected: unlanded bats-comment clause dropped; error-acknowledgment clause `[Corrected at D-916 per F-S2104-P16-006: ...]` added |
