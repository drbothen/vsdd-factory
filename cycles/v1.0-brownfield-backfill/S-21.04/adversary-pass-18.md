...
Fix wave landed at `fixes_landed_head: a4ec37d3` (test-writer) + `5a2da843` (story-writer). All 7 findings FIXED.

| Finding | Severity | Fix | Landed at | Status |
|---------|----------|-----|-----------|--------|
| F-S2104-P18-001 | BLOCKER | write-directive gate added (open-trigger POSITIVE; any sentence with write-directive+action must carry prohibition or canonical-absolute escape); PW-B's closed trigger backstopped on the write-directive axis | `a4ec37d3` (test-writer) + `5a2da843` (story-writer) | FIXED |
| F-S2104-P18-002 | BLOCKER | (a) fence exclusion removed from `write_discipline_prose_nosplit` — fenced code now IN domain for all section-wide negative gates; M-P18-C now RED at bats line 820; (b) balanced-fence POSITIVE assertion added — odd fence count blocks before any domain evaluation; M-P18-C(b) now RED at bats line 668 | `a4ec37d3` (test-writer) + `5a2da843` (story-writer) | FIXED |
| F-S2104-P18-003 | HIGH | boundary-rule sentence-splitter (`perl -pe 's/\.[[:space:]]+(?=[A-Z*\`\[])/.\n/g'`) replaces `sed 's/\. /\n/g'` throughout — splits only before `[A-Z*\`\[]`, so `No. 523` (digit follows) is NOT split; M-P18-B and all six abbreviation variants (No./etc./vs./al./Inc./§4.1.) now RED at bats line 915 | `a4ec37d3` (test-writer) + `5a2da843` (story-writer) | FIXED |
| F-S2104-P18-004 | HIGH | `rendered_write_discipline` domain introduced — strips HTML-comment spans AND link-reference-definition lines before any positive gate evaluates; anchor-uniqueness and empty-block guard re-scoped to rendered domain; M-P18-D now RED at bats line 695 (link-ref-def hidden anchor → count=0) | `a4ec37d3` (test-writer) + `5a2da843` (story-writer) | FIXED |
| F-S2104-P18-005 | HIGH | Gate 2b(c) re-scoped section-wide (matching Gate 2b(a) domain); trigger widened to include `prohibition\|prohibited\|the rule\|this rule\|the constraint\|above`; adversative class widened to 9 members; Gate 2b(a) nullification class widened to 25 members (adds supplanted, supersede, does not bind, does not govern, no longer binds, descriptive only, advisory only, pre-#NNN); M-P18-E/F now RED at bats line 869; M-P18-G now RED at bats line 897 | `a4ec37d3` (test-writer) + `5a2da843` (story-writer) | FIXED |
| F-S2104-P18-006 | MEDIUM | red-gate-log v1.15→v1.16: Battery table M-P17-A row corrected (Gate PW-B primary + write-directive gate secondary; Gate 5 claim dropped at 2 sites); Gate-indexed audit table Gate 5 row corrected (M-P17-A dropped; true Gate-5 vectors listed; SEQUENCE-SHADOWED DEFENSE-IN-DEPTH noted) | state-manager closure burst (D-920) | FIXED |
| F-S2104-P18-007 | MEDIUM | red-gate-log v1.16: D-918 input-hash placeholders corrected to literal `4b26b3b` at both sites (`last_amended` + `modified[D-918]`); all sibling rows record literal values | state-manager closure burst (D-920) | FIXED |
