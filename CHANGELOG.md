...
- **S-21.10 — `failure_policy` dispatcher registry schema extension** (BC-1.01.016 v1.2, E-21 W5, ADR-039 Phase 1): Dispatcher registry: new per-plugin `failure_policy` TOML field (`fail-closed` | `fail-open`); absent field defaults to `fail-open`; unknown values rejected at parse time (ADR-039 Decision 1+2 Phase 1).
