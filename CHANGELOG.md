...
- **S-21.07 — validate-cross-site-correspondence pass-5 adversarial fix burst** (BC-5.39.010 v1.8, E-21 W4): Closes six adversarial findings from pass-3. **F-S2107-P3-001 (BLOCKER):** `extract_bc_index_version` rearchitected to three-state `BcIndexVersionState` enum (`RowAbsent` / `RowPresentNoVersion` / `Version(String)`) with escape-aware column-count-anchored split; prior two-state `Option<String>` conflated absent-row and no-version-cell into a single `None`, causing ≥1,712 false BLOCKs on 5-column BC-INDEX rows (the normal state for ~98% of BC-INDEX entries). **F-S2107-P3-002 (HIGH):** `arm_b::is_volatile_path` rewritten with exact six-pattern const slice per ADR-037 §Decision 2 (adds ARCH-INDEX.md; removes blanket `.factory/cycles/**` wildcard; removes VP-INDEX.md); volatile advisory message updated to ADR-037 §Decision 4 prescribed text with `ADR-037 §Decision 2` cite and `Class B BLOCK suspended`. **F-S2107-P3-004 (HIGH):** `arm_a2` BC-ID detection replaced with word-boundary check so `BC-5.39.010` no longer matches `BC-5.39.0101`. **F-S2107-P3-009 (HIGH):** `dispatch::is_frontmatter_parity_target` epic branch now requires all three path components (`.factory`, `stories`, `epics`) and validates basename against `^E-[0-9]+-.*\.md$`. **F-S2107-P3-011 (MEDIUM):** `is_canonical_vp_filename` gains explicit `filename == "VP-INDEX.md"` early-return guard per PC34 bullet 2 normative REQUIRED defence-in-depth requirement. **F-S2107-P3-015 (MEDIUM):** `arm_b::extract_story_id_from_table_row` migrated to `parse_story_id_len` for canonical `S-[0-9]+\.[0-9]+` validation, preventing non-story table rows from producing spurious story-ID matches. Two-phase PC13 version-extraction in `arm_a2` replaces the prior left-to-right optional-v scanner, eliminating three false-positive collision classes (Story-ID traces, DEFERRED annotations, BC-section fragments). Stale `BC-5.39.010 v1.2` cites updated to v1.8 across Cargo.toml, lib.rs, main.rs, and red-gate-log.md.
