...
| D-941 SESSION-WRAP-PAUSED 2026-07-29 (session-wrap burst; SM this-commit) | state-manager | COMPLETE | (1) POLICY 16 GLOBAL-MAX GATE: `grep -n "^## D-" .factory/cycles/v1.0-brownfield-backfill/decision-log.md \| sort -t'-' -k2 -n \| tail -3` → 13760:## D-938 / 13860:## D-939 / 13953:## D-940; D-940 confirmed max → D-941 allocated. (2) PIPELINE PAUSED: human /wrap directive 2026-07-29; NOTE: `pipeline:` field STALE since D-878 (2026-07-22) through D-931..D-940 (see decision-log.md for full range); lesson L-BB-pipeline-field-no-liveness-signal recorded. (3) D-930 checkpoint archived to session-checkpoints.md. (4) STATE.md v6.72→v6.73: last_amended COMPACTED. (5) L-BB-pipeline-field-no-liveness-signal appended to lessons.md. (6) 4-INDEX UNCHANGED: BC v4.37/VP v2.72/STORY v4.272/ARCH v3.37. trajectory-tail →6→17→11→7. parent-commit: c95eda57. |
...
| factory-artifacts | see `git -C .factory log -1` | D-941-BURST this commit (pushed 2026-07-29). Prior: D-940-BURST `c95eda57`. NOTE: current HEAD not self-cited per TD-VSDD-053. |
| feature/S-21.04 | c7c61688 | feat(S-21.04): pass-27 CLOSED; cascade in progress; NO PR open (correct: mid-cascade); pushed clean 2026-07-29 |
...
| v1.0-brownfield-backfill | brownfield | **D-941-SESSION-WRAP-PAUSED 2026-07-29. PIPELINE PAUSED. develop 948f0fb1; main 80e5cd7b; merged_count 107; BC-INDEX v4.37; VP-INDEX v2.72; STORY-INDEX v4.272; ARCH-INDEX v3.37; streak 0/3 (27 passes); trajectory-tail →6→17→11→7. S-21.04 LOCAL adversary cascade pass-27 CLOSED D-940. feature/S-21.04 @ c7c61688 pushed, NO PR. NEXT: Pass-28 adversary dispatch (human directive: grind to 3 clean).** | D-941 SESSION-WRAP-PAUSED 2026-07-29; D-940 pass-27 CLOSED; D-939 pass-26 CLOSED; D-938 pass-25 CLOSED (regression; streak reset); D-937 pass-24 CLOSED; D-936 pass-23 CLOSED; D-935 pass-22 CLOSED; D-927 D-931 falsification burst; D-931..D-934 passes 21→22 arc; D-863..D-930 (see decision-log.md for full range). |
...
> D-001..D-606 (see decision-log.md for full range): `cycles/v1.0-brownfield-backfill/decision-log.md` + `decisions-log-archive.md`. D-379..D-454 (see decision-log.md for full range) (F5): `cycles/v1.0-feature-engine-discipline-pass-1/decision-log.md`. All archived per D-430(a) compaction bursts. D-607..D-941 (see decision-log.md for full range): this Decisions Log + decision-log.md SoT.

| ID | Decision | Summary | Phase | Date |
|----|----------|---------|-------|------|
| D-941 | SESSION-WRAP-PAUSED. Single-commit session-wrap burst (TD-VSDD-053) 2026-07-29. (1) POLICY 16 GLOBAL-MAX GATE: `grep -n "^## D-" .factory/cycles/v1.0-brownfield-backfill/decision-log.md \| sort -t'-' -k2 -n \| tail -3` → 13760:## D-938 / 13860:## D-939 / 13953:## D-940; D-940 confirmed max → D-941 allocated. (2) PIPELINE PAUSED: human /wrap directive 2026-07-29; NOTE: `pipeline:` field STALE since D-878 (2026-07-22) through D-931..D-940 (see decision-log.md for full range); lesson L-BB-pipeline-field-no-liveness-signal recorded. (3) D-930 checkpoint archived to session-checkpoints.md. (4) STATE.md v6.72→v6.73: last_amended COMPACTED (prior 73KB chain→compact entry; full chain SoT decision-log.md D-862 compaction note + Decisions Log table). (5) L-BB-pipeline-field-no-liveness-signal appended to lessons.md. (6) 4-INDEX UNCHANGED: BC v4.37/VP v2.72/STORY v4.272/ARCH v3.37. trajectory-tail →6→17→11→7. parent-commit: c95eda57. | SESSION-WRAP-PAUSED 2026-07-29; S-21.04 pass-27 CLOSED (D-940); streak 0/3; PIPELINE PAUSED post-D-940; SRC FULL REPLACEMENT; STATE.md v6.72→v6.73; 4-index UNCHANGED BC v4.37/VP v2.72/STORY v4.272/ARCH v3.37 | D-941-SESSION-WRAP-PAUSED | 2026-07-29 |
...
| **[D-863 hook false-positive] validate-dispatch-advance regex matches word-final-D + hyphenated-year substrings** | OPEN 2026-07-20 — D-863 | `APPROVED-2026` triggers bogus D-NNNN decision token. Root fix: scope D-chain regex. Anchor: next maintenance sweep / hook-hardening story. |
...
## Session Resume Checkpoint (2026-07-29 — D-941-SESSION-WRAP-PAUSED; PIPELINE PAUSED)
...
- Pass-23: 11 findings
- Pass-24: 7 findings (starvation break at B4)
- Pass-25: 8 findings (regression; streak reset; POLICY-15 restore-leg H04)
- Pass-26: 7 findings (H04 CLOSED at D-939; streak 0/3)
...
**CAUTION: `pipeline: PAUSED` is NOT a reliable liveness signal.** It was set at D-878 (2026-07-22) and remained stale through D-931..D-940 (see decision-log.md for full range) (6 adversary passes + 8 record bursts while pipeline was ACTIVE). D-417(b) strict dispatch-side advance only modifies `phase:` + `current_step:`, not `pipeline:`. Use `phase:` field and this SRC as the authoritative liveness indicators.
