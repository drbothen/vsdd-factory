...
| **Started** | 2026-04-25 |
| **Last Updated** | 2026-06-12 — D-555 adversary pass 8+9 cleanup: C0/H0 (guard functionally converged); M1 canonical→block_with_fix; L1 dead unsafe removal; L2 log_warn 8 fail-open paths; L4 whitespace trim→Block; stale-doc sweep; L3 test-name reconcile; S-17.04 v1.6→v1.7; impl green 5a704b6a 37 unit+7 bats; STORY-INDEX v3.99→v4.00; LOCAL streak 2/3. |
| **Current Phase** | D-555 S-17.04-V1.7-ADVERSARY-PASS-8-9-CLEANUP 2026-06-12 — Pass 8+9 cleanup codified: C0/H0; M1/L1/L2/L4/stale-doc/L3; S-17.04 v1.7 (37 unit+7 bats; 19 ACs); STORY-INDEX v3.99→v4.00. LOCAL streak 2/3. Next: final confirming adversary pass 10 → 3-CLEAN → PR → merge → rc.21. |
...
| v1.0-brownfield-backfill | brownfield | **D-555 S-17.04 v1.7 + pass 8+9 cleanup CODIFIED 2026-06-12; LOCAL adversary streak 2/3; final confirming pass then PR→merge→rc.21** | S-15.03 PRIORITY-A COMPLETE D-508; rc.20 SHIPPED D-528; #128+#130+#169+#176+#170(S17.01+S17.02+S17.03) all MERGED; E-17 delivery complete: BC-5.40.001+BC-4.13.001+BC-6.23.001 ACTIVE; ADR-025 v1.6 (Decision 12 WASM guard passes 1-9 corrected; env-free trigger verified; guard functionally converged); S-17.04 v1.7 FINAL (19 ACs; 8pts; SS-04+SS-05; 37 unit+7 bats; feature/S-17.04 @ f627a1c5 IN-FLIGHT NOT merged; branch contains v1.1 spec — must rebase to v1.7 before final pass dispatch); BC-INDEX v2.72; STORY-INDEX v4.00; ARCH-INDEX v2.27; **rc release PENDING** to ship all to operator cache. |
...
> D-001..D-549: `cycles/v1.0-brownfield-backfill/decision-log.md` + `decisions-log-archive.md` (D-550..D-555 also in decision-log.md SoT)
...
> D-379..D-454 (F5): `cycles/v1.0-feature-engine-discipline-pass-1/decision-log.md` <!-- D-452(e) umbrella-range-auto-advance; D-511..D-555 per-burst D-range advances archived to decision-log.md; D-555 ADVERSARY-PASS-8-9-CLEANUP 2026-06-12 D-range→D-555 -->
...
## Session Resume Checkpoint (2026-06-12 — D-555 S-17.04 v1.7 + adversary pass 8+9 cleanup CODIFIED; STORY-INDEX v4.00; ARCH-INDEX v2.27; LOCAL adversary streak 2/3; resume = final confirming adversary pass 10 → 3-CLEAN → PR → merge → rc.21)
...
**ADVERSARY PASS 8+9 CLEANUP CODIFIED (D-555 2026-06-12). Guard functionally CONVERGED.** Re-cascade passes 8+9 (post-P0-fix; same-family Claude) returned C0/H0 — the P0 env-free suffix trigger is verified correct by T-6 real-WASM absolute-path bats e2e. All pass 8+9 findings were quality/cleanup: (M1) `canonical_block_message`/`canonical_continue_message` formatted `block_with_fix` inline — single source of truth gap closed by delegating to `HookResult::block_with_fix/block_continue` from hook-sdk; (L1) dead `unsafe { std::env::remove_var("CLAUDE_PROJECT_DIR") }` in unit test removed (env-var era artifact); (L2) `log_warn!` added to all 8 fail-open paths (observability parity with verify-factory-lock); (L4) whitespace-only `timestamp:` / `expires_at:` → `.trim().is_empty()` → Block (closes consistency gap); stale-doc sweep (trigger comments → suffix/equality description); (L3) Red Gate Test Table test-names reconciled (6 occurrences across 4 locations; normative table drives test-writer stub naming → S-17.04 v1.7). impl green `5a704b6a` (37 unit + 7 bats). **LOCAL adversary streak now 2/3** — one final confirming pass required for 3-CLEAN.

**S-17.04 v1.7 CODIFIED.** Branch `feature/S-17.04-mid-burst-heartbeat-renewal-wiring` @ `f627a1c5` still pushed to origin. Worktree at `.worktrees/S-17.04`. **Note:** the branch still contains the v1.1 spec (bash-gate approach). The worktree must be rebased/force-updated to match the v1.7 story spec before final adversary dispatch. S-17.04 v1.7 deliverables: D10 (SKILL renew step retained), D13 (state-manager.md cross-ref updated for D16), D15 (`factory-lock-parse` shared crate at `crates/factory-lock-parse/`), D16 (`verify-state-timestamp-refresh` WASM plugin + registry; env-free trigger equals OR ends_with; correct payload fields; `..` normalization; `guard_ran` sentinel; canonical messages delegate to SDK; `log_warn` on 8 fail-open paths; whitespace trim→Block), D17 (21 Rust unit + 6 bats minimum; 37 shipped unit + 7 bats). 19 ACs, 8 pts, SS-04+SS-05.

**ADR-025 v1.6 DECISION 12 FULLY CORRECTED (content revision, passes 1-9 complete, 2026-06-12).** Per-tool payload extraction: Write→`tool_input.content` (full file body); Edit→on-disk STATE.md with `old_string` replaced by `new_string` (first occurrence; `replace_all` honored); MultiEdit→on-disk STATE.md with each `edits[]` element applied sequentially. Trigger: env-free — normalized `file_path` EQUALS `.factory/STATE.md` OR ENDS WITH `/.factory/STATE.md` (handles absolute paths without env dependency). Registry `[hooks.capabilities.read_file]` is `path_allow`-ONLY. Priorities: verify-factory-lock=142, verify-state-timestamp-refresh=143. Canonical-path step 5: `..` segment-stack resolution. LockExpiryStale: blocks when lock held AND proposed `expires_at` absent OR empty OR byte-identical stale. `factory-lock-parse` crate at `crates/factory-lock-parse/`. `guard_ran` sentinel on all Continue paths. Canonical messages delegate to SDK `block_with_fix/block_continue`. `log_warn` on all 8 fail-open paths. Whitespace-only fields trigger Block.
...
- **4-index (post-D-555):** BC-INDEX v2.72, VP-INDEX v2.06, STORY-INDEX v4.00, ARCH-INDEX v2.27.
...
- **D-555 carry:** ADVERSARY PASS 8+9 CLEANUP CODIFIED. Guard functionally CONVERGED: re-cascade C0/H0. M1 canonical_*_message→SDK block_with_fix delegation; L1 dead unsafe env_remove_var removed; L2 log_warn 8 fail-open paths; L4 whitespace trim→Block; stale-doc sweep; L3 test-name reconcile (6 occurrences). S-17.04 v1.6→v1.7 CODIFIED (19 ACs; 37 unit+7 bats shipped; impl green 5a704b6a). STORY-INDEX v4.00. LOCAL adversary streak 2/3 — one final confirming adversary pass 10 required for 3-CLEAN. Do NOT dispatch test-writer/implementer; worktree rebase to v1.7 before final adversary dispatch.
- **D-554 carry (prior):** ADR-025 v1.6 DEEP-PASS-7 P0 GUARD-INERT FIX (std::env::var(CLAUDE_PROJECT_DIR) dead in WASI sandbox; env-free trigger EQUALS OR ends_with; AC-019 empty-timestamp→Block; AC-018 absolute-path bats e2e mandate). S-17.04 v1.6 CODIFIED (19 ACs; 27 min/36 shipped; impl green 96eb1a0a). ARCH-INDEX v2.27. STORY-INDEX v3.99. LOCAL adversary streak 0/3 per D-554.
...
- Brownfield: D-001..D-555 — `cycles/v1.0-brownfield-backfill/decision-log.md`. Latest: D-554 ADR-025 v1.6 ADVERSARY DEEP-PASS-7 P0 WASM ENV-DEAD-CODE FIX + S-17.04 v1.5→v1.6; **D-555 ADVERSARY PASS 8+9 CLEANUP CODIFIED 2026-06-12 — re-cascade C0/H0 (guard functionally converged); M1 canonical→block_with_fix; L1/L2/L4/stale-doc/L3; S-17.04 v1.6→v1.7; impl green 5a704b6a; STORY-INDEX v3.99→v4.00; LOCAL streak 2/3.**
...
| STORY-INDEX | v4.00 | D-555: S-17.04 v1.6→v1.7 (pass 8+9 cleanup; test-name reconcile 6 occurrences; normative table drives test-writer stub naming; 37 unit+7 bats; impl green 5a704b6a) |
...
- **ADR-025 v1.6 REVISED:** `.factory/specs/architecture/decisions/ADR-025-single-writer-factory-locklease-prevent-concurrent-session-races-on-factory-artifacts-orphan-branch.md` — D-554 corrections (same version; §12.7 R6 env-free trigger; §12.8 EC-006; §12.9 AC-018+AC-019 mandate); ARCH-INDEX v2.27; UNCHANGED at D-555
- **S-17.04 story:** `.factory/stories/S-17.04-mid-burst-heartbeat-renewal-wiring.md` v1.7 DRAFT — STORY-INDEX v4.00; E-17 W4; 19 ACs; 21 Red Gate minimum Rust unit + 6 bats minimum; 37 unit+7 bats shipped; 8pts; SS-04+SS-05; ADR-025 v1.6 Decision 12 fully corrected passes 1-9; LOCAL adversary streak 2/3
...
8. **LOCAL adversary streak 2/3 (D-555 — pass 8+9 C0/H0 codified).** ADR-025 v1.6 Decision 12 fully corrected (passes 1-9 complete) + S-17.04 v1.7 codified (test-name reconcile; canonical messages delegate to SDK; log_warn 8 fail-open paths; whitespace trim→Block; 19 ACs; 37 unit+7 bats). Worktree rebase to v1.7 spec → final confirming adversary pass 10 → 3-CLEAN → PR → merge → rc.21.
9. **4-index confirmed D-555:** BC-INDEX v2.72, VP-INDEX v2.06, STORY-INDEX v4.00, ARCH-INDEX v2.27.
...
11. **Latest decision D-555.** All #128+#130+#169+#176+#170(S17.01+S17.02+S17.03) MERGED; REQUIRE rc.21+ for operator reach. S-17.04 v1.7 IN-FLIGHT NOT merged (final confirming adversary pass 10 next).

### §12. Pending Work Items — Strict Resume Ordering (refreshed 2026-06-12 D-555)

| Step | Item | Tier | Gate | Status |
|------|------|------|------|--------|
| **1** | **S-17.04 final confirming adversary pass 10** | **adversary** | worktree rebased to v1.7 spec | Rebase feature/S-17.04 worktree to v1.7 spec; dispatch adversary pass 10 against S-17.04 v1.7 + impl `5a704b6a` → if C0/H0: LOCAL streak 3/3 → 3-CLEAN satisfied → PR → squash-merge → state-manager post-merge burst → rc.21 release |
...
> Previous checkpoint (D-554 ADR-025-V1.6-DEEP-PASS-7-P0-WASM-ENV-DEAD-CODE-FIX-S-17.04-V1.6-2026-06-12) archived to: `cycles/v1.0-brownfield-backfill/session-checkpoints.md`
