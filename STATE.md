...
- **factory-artifacts HEAD:** `[D-610-SHA-TBD]` (D-610 E-18 F2 FULLY COMPLETE / L-F2-no-bypass-on-edit-failure codified 2026-06-16; prior: `9a23df77` D-609 SHA-patch; prior-prior: `49ac4355` D-609 E-18 CONFIRMING-PASS FIX BURST)
...
- **Verify on resume:** `git rev-parse --short origin/develop` → expect `7e99f6ef`; `git rev-parse --short origin/main` → expect `caf06c68`; `git -C .factory log -1 --format='%h'` → expect D-610 SHA (post SHA-patch commit)
