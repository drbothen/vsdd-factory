...
| **Started** | 2026-04-25 |
| **Last Updated** | 2026-06-12 — D-554 ADR-025 v1.6 adversary deep-pass-7 P0 WASM env-var dead-code fix: guard was inert in production (std::env::var always Err in WASM sandbox); env-free trigger adopted (equals OR ends_with); AC-019 empty-timestamp→Block; AC-018 absolute-path bats e2e mandate; impl green 96eb1a0a 31 unit+7 bats; S-17.04 v1.5→v1.6; ARCH-INDEX v2.26→v2.27; STORY-INDEX v3.98→v3.99; LOCAL adversary streak 0/3; TDD re-implementation next. |
| **Current Phase** | D-554 ADR-025-V1.6-DEEP-PASS-7-P0-WASM-ENV-DEAD-CODE-FIX-S-17.04-V1.6 2026-06-12 — Adversary deep-pass-7 P0 guard-inert fix codified: env-free trigger (equals OR ends_with /.factory/STATE.md); AC-019 empty-timestamp→Block; AC-018 absolute-path bats e2e; impl green 96eb1a0a; S-17.04 v1.6 (19 ACs). ARCH-INDEX v2.26→v2.27; STORY-INDEX v3.98→v3.99. LOCAL streak 0/3. Next: rebase worktree to v1.6 spec → TDD re-implementation → LOCAL re-cascade pass-8 toward 3-CLEAN → PR → merge → rc.21. |
...
| factory-artifacts | 64c5c587 | D-554 ADR-025 v1.6 deep-pass-7 P0 WASM env-dead-code fix + S-17.04 v1.6 2026-06-12; prior: `edc397a2` D-553; prior-prior: `aa3cd62f` D-552 |
...
| v1.0-brownfield-backfill | brownfield | **D-554 ADR-025 v1.6 deep-pass-7 P0 WASM env-dead-code fix + S-17.04 v1.6 CODIFIED 2026-06-12; LOCAL adversary streak 0/3; S-17.04 v1.6 awaiting TDD re-implementation** | S-15.03 PRIORITY-A COMPLETE D-508; rc.20 SHIPPED D-528; #128+#130+#169+#176+#170(S17.01+S17.02+S17.03) all MERGED; E-17 delivery complete: BC-5.40.001+BC-4.13.001+BC-6.23.001 ACTIVE; ADR-025 v1.6 (Decision 12 WASM guard passes 1-7 corrected; env-free trigger adopted; Decision 11 Mech-2 WITHDRAWN; D-539 multi-family satisfied); S-17.04 v1.6 CORRECTED (19 ACs; 8pts; SS-04+SS-05; env-free trigger; AC-018+AC-019; feature/S-17.04 @ f627a1c5 IN-FLIGHT NOT merged; branch contains v1.1 spec — must rebase to v1.6 before TDD); BC-INDEX v2.72; STORY-INDEX v3.99; ARCH-INDEX v2.27; **rc release PENDING** to ship all to operator cache. |
...
> D-001..D-549: `cycles/v1.0-brownfield-backfill/decision-log.md` + `decisions-log-archive.md` (D-550+D-551+D-552+D-553+D-554 also in decision-log.md SoT)
...
> D-379..D-454 (F5): `cycles/v1.0-feature-engine-discipline-pass-1/decision-log.md` <!-- D-452(e) umbrella-range-auto-advance; D-511..D-554 per-burst D-range advances archived to decision-log.md; D-554 ADR-025-DEEP-PASS-7-P0-FIX 2026-06-12 D-range→D-554 -->
...
## Session Resume Checkpoint (2026-06-12 — D-554 ADR-025 v1.6 deep-pass-7 P0 WASM env-dead-code fix + S-17.04 v1.6 CODIFIED; ARCH-INDEX v2.27; STORY-INDEX v3.99; LOCAL adversary streak 0/3; resume = rebase worktree to v1.6 spec → S-17.04 v1.6 TDD re-implementation → LOCAL adversary re-cascade pass-8 toward 3-CLEAN → PR → merge → rc.21)
...
**ADR-025 v1.6 ADVERSARY DEEP-PASS-7 P0 GUARD-INERT FIX CODIFIED (D-554 2026-06-12).** Deep-pass-7 found the critical P0: `verify-state-timestamp-refresh` was **inert in production**. Root cause: §12.7 R6 step 1 called `std::env::var("CLAUDE_PROJECT_DIR")` to strip the project root prefix. In the WASI sandbox, `WasiCtxBuilder` in `crates/factory-dispatcher/src/invoke.rs` constructs the context with `preopened_dir` only — neither `.env()` nor `.inherit_env()` is called — so `std::env::var` always returns `Err(NotPresent)` at runtime. Claude Code emits absolute `file_path` values (e.g. `/Users/alice/vsdd-factory/.factory/STATE.md`). The dead env-strip meant absolute paths were never normalized → guard always Continued → guard was a no-op in every real Claude Code session. Masked by native unit tests that set `CLAUDE_PROJECT_DIR` in native process env (works in native, not in WASM). Fix: remove env-var strip; env-free trigger: normalized path EQUALS `.factory/STATE.md` OR ENDS WITH `/.factory/STATE.md` (handles relative and absolute; no env dependency; no capability required). `host::env`+`env_allow` route explicitly rejected (ADR-025 v1.3 silent-no-op footgun class). AC-019 added: proposed `timestamp:` present but empty → Block TimestampStale. LOCAL adversary cascade streak **still 0/3** — re-cascade REQUIRED after TDD re-implementation with v1.6 spec.

**S-17.04 v1.6 CODIFIED — TDD RE-IMPLEMENTATION NEXT.** Branch `feature/S-17.04-mid-burst-heartbeat-renewal-wiring` @ `f627a1c5` still pushed to origin. Worktree at `.worktrees/S-17.04`. **Note:** the branch still contains the v1.1 spec (bash-gate approach). The worktree must be rebased/force-updated to match the v1.6 story spec before test-writer dispatch. S-17.04 v1.6 deliverables: D10 (SKILL renew step retained), D13 (state-manager.md cross-ref updated for D16), D15 (`factory-lock-parse` shared crate at `crates/factory-lock-parse/`), D16 (`verify-state-timestamp-refresh` WASM plugin + registry; env-free trigger equals OR ends_with; correct payload fields per AC-011..015 + AC-016/017; `..` normalization; `guard_ran` sentinel on all Continue paths), D17 (21 Rust unit + 6 bats minimum; 30+6=36 shipped). 19 ACs, 8 pts, SS-04+SS-05.

**ADR-025 v1.6 DECISION 12 FULLY CORRECTED (content revision, passes 1-7 complete, 2026-06-12).** Per-tool payload extraction: Write→`tool_input.content` (full file body); Edit→on-disk STATE.md with `old_string` replaced by `new_string` (first occurrence; `replace_all` honored); MultiEdit→on-disk STATE.md with each `edits[]` element applied sequentially. Trigger: env-free — normalized `file_path` EQUALS `.factory/STATE.md` OR ENDS WITH `/.factory/STATE.md` (handles absolute paths without env dependency). Registry `[hooks.capabilities.read_file]` is `path_allow`-ONLY. Priorities: verify-factory-lock=142, verify-state-timestamp-refresh=143. Canonical-path step 5: `..` segment-stack resolution. LockExpiryStale: blocks when lock held AND proposed `expires_at` absent OR empty OR byte-identical stale. `factory-lock-parse` crate at `crates/factory-lock-parse/`. `guard_ran` sentinel on all Continue paths.
...
- **4-index (post-D-554):** BC-INDEX v2.72, VP-INDEX v2.06, STORY-INDEX v3.99, ARCH-INDEX v2.27.
...
- **D-554 carry:** ADR-025 v1.6 DEEP-PASS-7 P0 GUARD-INERT FIX (std::env::var(CLAUDE_PROJECT_DIR) dead in WASI sandbox; env-free trigger EQUALS OR ends_with; AC-019 empty-timestamp→Block; AC-018 absolute-path bats e2e mandate). S-17.04 v1.6 CODIFIED (19 ACs; 27 min/36 shipped; impl green 96eb1a0a). feature/S-17.04 @ f627a1c5 PUSHED (branch contains v1.1 spec — must rebase to v1.6 before TDD re-dispatch). ARCH-INDEX v2.27. STORY-INDEX v3.99. LOCAL adversary streak 0/3 — do NOT dispatch test-writer/implementer until worktree rebased to v1.6 spec.
- **D-553 carry:** ADR-025 v1.6 PASS-4+5 BUILD/TEST-HARNESS CORRECTIONS (P5-H1 factory-lock-parse→crates/factory-lock-parse/; P5-M1 CI_REQUIRE_ARTIFACTS=1 hard-fail; P4-H1 guard_ran sentinel; P4-O1 Red-Gate reconcile). S-17.04 v1.5 CODIFIED. ARCH-INDEX v2.26. STORY-INDEX v3.98.
...
- Brownfield: D-001..D-554 — `cycles/v1.0-brownfield-backfill/decision-log.md`. Latest: D-553 ADR-025 v1.6 ADVERSARY PASS 4+5 BUILD/TEST-HARNESS CORRECTIONS; **D-554 ADR-025 v1.6 ADVERSARY DEEP-PASS-7 P0 WASM ENV-DEAD-CODE FIX + S-17.04 v1.5→v1.6 2026-06-12 — std::env::var(CLAUDE_PROJECT_DIR) dead in WASI sandbox; env-free trigger (EQUALS OR ends_with); AC-019 empty-timestamp→Block; impl green 96eb1a0a; ARCH-INDEX v2.26→v2.27; STORY-INDEX v3.98→v3.99; LOCAL streak 0/3.**
...
| STORY-INDEX | v3.99 | D-554: S-17.04 v1.5→v1.6 (adversary deep-pass-7 P0 guard-inert fix; env-free trigger; AC-018+AC-019; 19 ACs; 27 min/36 shipped; impl green 96eb1a0a) |
| ARCH-INDEX | v2.27 | D-554: ADR-025 v1.6 deep-pass-7 P0 WASM env-dead-code fix (§12.7 R6 rewritten — env-free trigger; §12.8 EC-006; §12.9 AC-018+AC-019 mandate; INV-019 cure (a)/(b)/(c)) |

### §9. Critical Anchors

- **factory-artifacts HEAD:** 64c5c587 D-554 burst; prior: `edc397a2` D-553; prior-prior: `aa3cd62f` D-552
- **develop HEAD:** `60fd0233` (D-547 PR #183 S-17.03 squash-merge 2026-06-11)
- **feature/S-17.04 HEAD:** `f627a1c5` PUSHED to origin; worktree `.worktrees/S-17.04` (contains v1.1 spec — rebase to v1.6 before TDD dispatch)
...
- **ADR-025 v1.6 REVISED:** `.factory/specs/architecture/decisions/ADR-025-single-writer-factory-locklease-prevent-concurrent-session-races-on-factory-artifacts-orphan-branch.md` — D-554 corrections (same version; §12.7 R6 env-free trigger; §12.8 EC-006; §12.9 AC-018+AC-019 mandate); ARCH-INDEX v2.27
- **S-17.04 story:** `.factory/stories/S-17.04-mid-burst-heartbeat-renewal-wiring.md` v1.6 DRAFT — STORY-INDEX v3.99; E-17 W4; 19 ACs; 27 Red Gate minimum (21 Rust unit + 6 bats); 36 shipped; 8pts; SS-04+SS-05; ADR-025 v1.6 Decision 12 fully corrected passes 1-7; LOCAL adversary streak 0/3
...
8. **LOCAL adversary streak 0/3 (D-554 — deep-pass-7 P0 guard-inert fix codified).** ADR-025 v1.6 Decision 12 fully corrected (passes 1-7 complete) + S-17.04 v1.6 codified (env-free trigger; AC-018+AC-019; 19 ACs). Worktree rebase to v1.6 → test-writer 27 Red Gate stubs (21 Rust unit + 6 bats) → implementer D15/D16/D10/D13 (env-free trigger; guard_ran sentinel) → re-cascade LOCAL pass-8 toward 3-CLEAN → PR → merge.
9. **4-index confirmed D-554:** BC-INDEX v2.72, VP-INDEX v2.06, STORY-INDEX v3.99, ARCH-INDEX v2.27.
...
11. **Latest decision D-554.** All #128+#130+#169+#176+#170(S17.01+S17.02+S17.03) MERGED; REQUIRE rc.21+ for operator reach. S-17.04 v1.6 IN-FLIGHT NOT merged (TDD re-implementation next).

### §12. Pending Work Items — Strict Resume Ordering (refreshed 2026-06-12 D-553)

| Step | Item | Tier | Gate | Status |
|------|------|------|------|--------|
| **1** | **S-17.04 v1.6 TDD re-implementation** | **implementation** | worktree rebased to v1.6 spec | Rebase feature/S-17.04 worktree to v1.6 spec; test-writer 27 Red Gate stubs (21 Rust unit + 6 bats) → implementer D15/D16/D10/D13 (factory-lock-parse at crates/factory-lock-parse/; env-free trigger equals OR ends_with; correct payload fields per AC-011..015; AC-016/017 LockExpiryStale absent+empty; AC-010 +MultiEdit; `..` normalization step 5; guard_ran sentinel all Continue paths; AC-019 empty-timestamp→Block) → LOCAL adversary re-cascade pass-8 0/3 → 3-CLEAN → PR → merge |
...
> Previous checkpoint (D-553 ADR-025-V1.6-PASS-4-5-BUILD-HARNESS-CORRECTIONS-S-17.04-V1.5-2026-06-12) archived to: `cycles/v1.0-brownfield-backfill/session-checkpoints.md`
