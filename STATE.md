...
| **Started** | 2026-04-25 |
| **Last Updated** | 2026-06-12 — D-553 ADR-025 v1.6 adversary pass 4+5 (deep-probe) build/test-harness corrections: P5-H1 factory-lock-parse relocated crates/hook-plugins/→crates/factory-lock-parse/; P5-M1 CI_REQUIRE_ARTIFACTS=1 hard-fail; P4-H1 guard_ran stderr sentinel all 10 Continue paths; P4-O1 Red-Gate count reconcile (19 mandated/33 shipped); impl green 2c4977a3; S-17.04 v1.4→v1.5; ARCH-INDEX v2.25→v2.26; STORY-INDEX v3.97→v3.98; LOCAL adversary streak 0/3; re-cascade pending. |
| **Current Phase** | D-553 ADR-025-V1.6-PASS-4-5-BUILD-HARNESS-CORRECTIONS-S-17.04-V1.5 2026-06-12 — Adversary pass 4+5 deep-probe build/test-harness corrections codified: factory-lock-parse relocation; CI_REQUIRE_ARTIFACTS=1 hard-fail; guard_ran sentinel; Red-Gate reconcile; impl green 2c4977a3; S-17.04 v1.5. ARCH-INDEX v2.25→v2.26; STORY-INDEX v3.97→v3.98. LOCAL streak 0/3. Next: TDD re-implementation → LOCAL re-cascade 0/3 → PR → rc.21. |
...
| factory-artifacts | `TBD-D553` | D-553 ADR-025 v1.6 pass-4+5 build/test-harness corrections + S-17.04 v1.5 2026-06-12; prior: `aa3cd62f` D-552; prior-prior: `f7a4cc1a` D-551 |
...
| v1.0-brownfield-backfill | brownfield | **D-553 ADR-025 v1.6 pass-4+5 deep-probe corrections + S-17.04 v1.5 CODIFIED 2026-06-12; LOCAL adversary streak 0/3; S-17.04 v1.5 awaiting TDD re-cascade** | S-15.03 PRIORITY-A COMPLETE D-508; rc.20 SHIPPED D-528; #128+#130+#169+#176+#170(S17.01+S17.02+S17.03) all MERGED; E-17 delivery complete: BC-5.40.001+BC-4.13.001+BC-6.23.001 ACTIVE; ADR-025 v1.6 (Decision 12 WASM guard passes 1-5 corrected; Decision 11 Mech-2 WITHDRAWN; D-539 multi-family satisfied; factory-lock-parse relocation pass 5); S-17.04 v1.5 CORRECTED (17 ACs; 8pts; SS-04+SS-05; D15 corrected path; D16/D17; feature/S-17.04 @ f627a1c5 IN-FLIGHT NOT merged; branch contains v1.1 spec — must rebase to v1.5 before TDD); BC-INDEX v2.72; STORY-INDEX v3.98; ARCH-INDEX v2.26; **rc release PENDING** to ship all to operator cache. |
...
> D-001..D-549: `cycles/v1.0-brownfield-backfill/decision-log.md` + `decisions-log-archive.md` (D-550+D-551+D-552+D-553 also in decision-log.md SoT)
...
> D-379..D-454 (F5): `cycles/v1.0-feature-engine-discipline-pass-1/decision-log.md` <!-- D-452(e) umbrella-range-auto-advance; D-511..D-553 per-burst D-range advances archived to decision-log.md; D-553 ADR-025-PASS-4-5-CORRECTIONS 2026-06-12 D-range→D-553 -->
...
## Session Resume Checkpoint (2026-06-12 — D-553 ADR-025 v1.6 pass-4+5 deep-probe corrections + S-17.04 v1.5 CODIFIED; ARCH-INDEX v2.26; STORY-INDEX v3.98; LOCAL adversary streak 0/3; resume = S-17.04 v1.5 TDD re-implementation → LOCAL adversary re-cascade pass-6 toward 3-CLEAN → PR → merge → rc.21)
...
**ADR-025 v1.6 ADVERSARY PASS 4+5 (DEEP-PROBE) BUILD/TEST-HARNESS CORRECTIONS CODIFIED (D-553 2026-06-12).** Guard LOGIC confirmed clean by Claude same-family passes 3-5 + Gemini cross-family (no logic defects). Deep-probe found 4 build/test-harness gaps: (P5-H1) `factory-lock-parse` was a lib-only crate (no `[[bin]]`, no WASM output) placed under `crates/hook-plugins/` — this inflated the CI WASM floor-count gate's expected count, breaking CI; correct path is `crates/factory-lock-parse/` alongside other non-plugin workspace crates; CI floor-count now derives from `[[bin]]`-bearing crates only; ADR-025 §12.5 / D15 path corrected. (P5-M1) bats `_require_artifacts` was skip-as-green — guard test would silently pass if artifacts absent; `CI_REQUIRE_ARTIFACTS=1` hard-fail added; CI e2e suite runs with artifacts present; S-17.04 CI scaffold section updated. (P4-H1) `plugins_run=1` assertion was false-green — `on_error=continue` means a guard crash also exits 0, so exit-code alone cannot confirm the guard ran; `guard_ran` stderr sentinel added to all 10 Continue paths in `verify-state-timestamp-refresh`; T-1/T-3 bats assert sentinel; S-17.04 T-1/T-3 task rows updated. (P4-O1) Red-Gate count reconcile — 19 Rust unit mandated + 9 GREEN control/symmetry tests = 28 shipped Rust unit (extras are correct, not mandate inflation); 5 bats mandated; story minimum stays 19 Rust unit + 5 bats = 24; S-17.04 test count table updated. LOCAL adversary cascade streak **still 0/3** — re-cascade REQUIRED after TDD re-implementation with v1.5 spec.

**S-17.04 v1.5 CODIFIED — TDD RE-IMPLEMENTATION NEXT.** Branch `feature/S-17.04-mid-burst-heartbeat-renewal-wiring` @ `f627a1c5` still pushed to origin. Worktree at `.worktrees/S-17.04`. **Note:** the branch still contains the v1.1 spec (bash-gate approach). The worktree must be rebased/force-updated to match the v1.5 story spec before test-writer dispatch. S-17.04 v1.5 deliverables: D10 (SKILL renew step retained), D13 (state-manager.md cross-ref updated for D16), D15 (`factory-lock-parse` shared crate at `crates/factory-lock-parse/`), D16 (`verify-state-timestamp-refresh` WASM plugin + registry; correct payload fields per AC-011..015 + AC-016/017; `..` normalization; verify-factory-lock +MultiEdit; `guard_ran` sentinel on all Continue paths), D17 (19 Rust unit + 5 bats tests; 28 shipped Rust unit total). 17 ACs, 8 pts, SS-04+SS-05.

**ADR-025 v1.6 DECISION 12 FULLY CORRECTED (content revision, passes 1-5 complete, 2026-06-12).** Per-tool payload extraction: Write→`tool_input.content` (full file body); Edit→on-disk STATE.md with `old_string` replaced by `new_string` (first occurrence; `replace_all` honored); MultiEdit→on-disk STATE.md with each `edits[]` element applied sequentially. Trigger field is always `file_path` (never `new_content`). Registry `[hooks.capabilities.read_file]` is `path_allow`-ONLY. Priorities: verify-factory-lock=142, verify-state-timestamp-refresh=143. Canonical-path step 5: `..` segment-stack resolution. LockExpiryStale: blocks when lock held AND proposed `expires_at` absent OR empty OR byte-identical stale. `factory-lock-parse` crate at `crates/factory-lock-parse/` (not under hook-plugins/). `guard_ran` sentinel on all Continue paths.
...
- **4-index (post-D-553):** BC-INDEX v2.72, VP-INDEX v2.06, STORY-INDEX v3.98, ARCH-INDEX v2.26.
...
- **D-553 carry:** ADR-025 v1.6 PASS-4+5 BUILD/TEST-HARNESS CORRECTIONS (P5-H1 factory-lock-parse→crates/factory-lock-parse/; P5-M1 CI_REQUIRE_ARTIFACTS=1 hard-fail; P4-H1 guard_ran sentinel; P4-O1 Red-Gate reconcile). S-17.04 v1.5 CODIFIED. feature/S-17.04 @ f627a1c5 PUSHED (branch contains v1.1 spec — must rebase to v1.5 before TDD re-dispatch). ARCH-INDEX v2.26. STORY-INDEX v3.98. LOCAL adversary streak 0/3 — do NOT dispatch test-writer/implementer until worktree rebased to v1.5 spec.
...
- **D-553 (2026-06-12):** ADR-025 v1.6 ADVERSARY PASS 4+5 DEEP-PROBE BUILD/TEST-HARNESS CORRECTIONS. S-17.04 v1.4→v1.5. ARCH-INDEX v2.25→v2.26. STORY-INDEX v3.97→v3.98. impl green 2c4977a3. LOCAL adversary streak 0/3. Re-cascade pending.
...
- Brownfield: D-001..D-553 — `cycles/v1.0-brownfield-backfill/decision-log.md`. Latest: D-552 ADR-025 v1.6 GEMINI CROSS-FAMILY ADVERSARY PASS 2 CORRECTIONS; **D-553 ADR-025 v1.6 ADVERSARY PASS 4+5 (DEEP-PROBE) BUILD/TEST-HARNESS CORRECTIONS + S-17.04 v1.4→v1.5 2026-06-12 — P5-H1 factory-lock-parse relocation crates/factory-lock-parse/; P5-M1 CI_REQUIRE_ARTIFACTS=1 hard-fail; P4-H1 guard_ran sentinel; P4-O1 Red-Gate reconcile; impl green 2c4977a3; ARCH-INDEX v2.25→v2.26; STORY-INDEX v3.97→v3.98; LOCAL streak 0/3.**
...
| STORY-INDEX | v3.98 | D-553: S-17.04 v1.4→v1.5 (adversary pass-4+5 deep-probe corrections; factory-lock-parse path corrected; CI_REQUIRE_ARTIFACTS=1; guard_ran sentinel; Red-Gate reconcile) |
| ARCH-INDEX | v2.26 | D-553: ADR-025 v1.6 pass-4+5 deep-probe build/test-harness corrections (D15 factory-lock-parse path crates/hook-plugins/→crates/factory-lock-parse/; §12.5 path rationale; §12.9 guard_ran sentinel; INV-019 cure (a)/(b)/(c)) |

### §9. Critical Anchors

- **factory-artifacts HEAD:** `TBD-D553` D-553 burst; prior: `aa3cd62f` D-552; prior-prior: `f7a4cc1a` D-551
- **develop HEAD:** `60fd0233` (D-547 PR #183 S-17.03 squash-merge 2026-06-11)
- **feature/S-17.04 HEAD:** `f627a1c5` PUSHED to origin; worktree `.worktrees/S-17.04` (contains v1.1 spec — rebase to v1.5 before TDD dispatch)
...
- **ADR-025 v1.6 REVISED:** `.factory/specs/architecture/decisions/ADR-025-single-writer-factory-locklease-prevent-concurrent-session-races-on-factory-artifacts-orphan-branch.md` — D-553 corrections (same version; D15 factory-lock-parse path crates/factory-lock-parse/; §12.5 path rationale; §12.9 guard_ran sentinel); ARCH-INDEX v2.26
- **S-17.04 story:** `.factory/stories/S-17.04-mid-burst-heartbeat-renewal-wiring.md` v1.5 DRAFT — STORY-INDEX v3.98; E-17 W4; 17 ACs; 24 Red Gate tests (19 Rust unit + 5 bats); 8pts; SS-04+SS-05; ADR-025 v1.6 Decision 12 corrected+extended; LOCAL adversary streak 0/3
...
8. **LOCAL adversary streak 0/3 (D-553 — pass-4+5 deep-probe corrections codified).** ADR-025 v1.6 Decision 12 fully corrected (passes 1-5 complete) + S-17.04 v1.5 codified. Worktree rebase to v1.5 → test-writer 24 Red Gate stubs → implementer D15/D16/D10/D13 (factory-lock-parse at crates/factory-lock-parse/; guard_ran sentinel) → re-cascade LOCAL pass-6 toward 3-CLEAN → PR → merge.
9. **4-index confirmed D-553:** BC-INDEX v2.72, VP-INDEX v2.06, STORY-INDEX v3.98, ARCH-INDEX v2.26.
...
11. **Latest decision D-553.** All #128+#130+#169+#176+#170(S17.01+S17.02+S17.03) MERGED; REQUIRE rc.21+ for operator reach. S-17.04 v1.5 IN-FLIGHT NOT merged (TDD re-implementation next).

### §12. Pending Work Items — Strict Resume Ordering (refreshed 2026-06-12 D-553)

| Step | Item | Tier | Gate | Status |
|------|------|------|------|--------|
| **1** | **S-17.04 v1.5 TDD re-implementation** | **implementation** | worktree rebased to v1.5 spec | Rebase feature/S-17.04 worktree to v1.5 spec; test-writer 24 Red Gate stubs (19 Rust unit + 5 bats) → implementer D15/D16/D10/D13 (factory-lock-parse at crates/factory-lock-parse/; correct payload fields per AC-011..015; AC-016/017 LockExpiryStale absent+empty; AC-010 +MultiEdit; `..` normalization step 5; guard_ran sentinel all Continue paths) → LOCAL adversary re-cascade 0/3 → 3-CLEAN → PR → merge |
...
> Previous checkpoint (D-552 ADR-025-V1.6-PASS-2-CORRECTIONS-S-17.04-V1.4-2026-06-12) archived to: `cycles/v1.0-brownfield-backfill/session-checkpoints.md`
