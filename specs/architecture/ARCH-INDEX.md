...
| ADR-024 | Dispatcher log-dir worktree-aware resolution, CLAUDE_PLUGIN_ROOT fail-loud contract, internal-error dedup, and destructive-guard shadow exception — 5-level resolution order (VSDD_LOG_DIR override → FACTORY_ROOT override → basename-is-.factory guard → walk-up ancestor → git-worktree-main-root subprocess [200ms timeout] → cwd fallback); fail-loud-but-continue on absent CLAUDE_PLUGIN_ROOT; per-session dedup of `internal.dispatcher_error` (fixed-cap HashSet<u64>, cap 1024); shadow exception in destructive-op guard scoped to `.factory/.factory` substring — **ACCEPTED 2026-06-09; gates issue #130; requires rc release for operator-level cache** | SS-01, SS-03, SS-07 | decisions/ADR-024-dispatcher-log-dir-resolution-and-plugin-root-fail-loud.md |
