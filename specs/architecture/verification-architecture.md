...
All 140 verification properties, organized by subsystem. Each VP entry states: title,
proof method, BC postcondition/invariant anchor, and current status.

> **VP count invariant:** This catalog lists exactly 140 VPs (VP-001..VP-140) across
> all subsystems. The per-method totals in §3 must sum to 140.
...
| VP-140 | Item-Count Trigger Off-By-One (`current_item_count + 1 > N`) and `low_water_mark` Rotation-Target Config Validation (Default `floor(N/2)`; Fail-Loud on `>= N` or Negative) — dedicated PC8 item-count-trigger VP; item-count-shape analogue of VP-116's `"flat"` byte-boundary role (no F4-provisional numeric dependency) | unit-test | BC-1.18.005 PC8/INV4/INV5/EC-008/EC-010/EC-011 | draft |
...
> **Arithmetic invariant:** per-method counts must sum to total_vps (140).
