...
1. After parsing `resolvers-registry.toml`, the function obtains the directory
   ...
   parameter name in `load_registry` — verified: `pub fn load_registry(&self, path: &Path) -> Result<(ResolverRegistry, Vec<LoadWarning>), ResolverLoadError>` — exactly one parameter `path: &Path`; no additional parameters).
...
| 1.10 | 2026-06-22 | architect | S-18.14 pass-18 fix burst: (F-1 MAJOR POLICY 5) §Decision 1 Addendum step 1 phantom `, ...` parameter list removed — normative text cited `pub fn load_registry(&self, path: &Path, ...)` implying additional parameters; ground truth (resolver_loader.rs:281-284): exactly ONE parameter `path: &Path`; corrected citation: `pub fn load_registry(&self, path: &Path) -> Result<(ResolverRegistry, Vec<LoadWarning>), ResolverLoadError>` with exact one-parameter list; 'verified' attestation updated to match actual signature; sibling-sweep of ADR normative body for `, ...`-in-paramlist occurrences (`grep -n ', \.\.\.)' ADR-024-*.md`) → zero additional normative sites. (O-1) ARCH-INDEX ADR-024 row v1.3 summary segment `toml_path.parent()` corrected to `path.parent()` for cross-cell consistency with v1.9 toml_path→path correction. ARCH-INDEX v2.71→v2.72. |
