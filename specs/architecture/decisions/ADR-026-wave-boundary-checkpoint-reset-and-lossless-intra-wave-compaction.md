...
- Add to PC1 (or a new PC adjacent to the field-validation PC): "**Conditional field validation:** When the gate detects EPIC-COMPLETE context (determined by: prior HANDOFF.md on factory-artifacts was absent OR prior HANDOFF.md had non-empty `next_wave_stories`; AND current write has `next_wave_stories: []`), the gate adds `epic_status` to the required field set and validates it equals `complete`. When NOT in EPIC-COMPLETE context, the gate must FAIL with `UnexpectedEpicStatus` if `epic_status` is present (presence on a non-final wave is malformed)."
