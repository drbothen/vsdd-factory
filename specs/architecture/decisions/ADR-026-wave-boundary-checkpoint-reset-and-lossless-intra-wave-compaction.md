...
4. **Bash 'commit' subcommand** (`wave-handoff.sh --commit [args]`): Stages HANDOFF.md (already on disk from the Write tool call in step 2) and wave-state.yaml (on disk from step 3) and creates a **single atomic git commit** via `commit_to_artifacts`. This preserves **BC-5.41.002 PC6 atomicity**: both files are staged and committed in ONE git commit regardless of the fact that HANDOFF.md was written by the Write tool and wave-state.yaml was written by bash. Atomicity is maintained at the git commit boundary, not at the write boundary — the two writes landing separately on disk before the single commit is not a PC6 violation.
...
Step 5: Commit HANDOFF.md + wave-state.yaml atomically (Bash: wave-handoff.sh --commit)
...
**Atomicity-preservation argument (BC-5.41.002 PC6):** BC-5.41.002 PC6 requires HANDOFF.md and wave-state.yaml to be committed in a SINGLE git commit, not committed separately. The agent-orchestrated flow satisfies this: HANDOFF.md is written by the Write tool and lands on the factory-artifacts working tree; wave-state.yaml is written by bash and lands on the same working tree; the `--commit` subcommand stages BOTH files and creates ONE commit. The two writes arriving at different times (steps 3 and 4) do not create two commits — only step 5 creates a commit. This is identical in effect to the pre-restructure flow where both files were bash-written before a single `commit_to_artifacts` call. PC6 atomicity holds.
