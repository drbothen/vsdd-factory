...
### Decision 14 — Pure-parse invariant consistency gate: detection algorithm for S-18.08 (S-18.08 redesign)
...
**Decision:** The correct gate detection algorithm uses THREE sequential layers, all of which must be satisfied to classify a line as a genuine violation. The algorithm is specified below as a drop-in replacement for S-18.08 AC-001..AC-005.

#### Layer 1 — Normative section extraction (BC scan only)

For BC files, extract ONLY the normative behavioral sections before scanning. Normative sections are delimited by their H2 headings:
- **START anchors (inclusive):** `## Preconditions`
- **END anchor (exclusive):** `## Related BCs`

Extraction command:
```bash
awk '/^## Preconditions$/{ found=1 } found && /^## Related BCs$/{ exit } found{ print }' "$BC_FILE"
```

This covers: §Preconditions, §Postconditions, §Invariants, §Edge Cases, §Canonical Test Vectors — the complete behavioral specification. It excludes: §Description (narrative context), §Related BCs, §Architecture Anchors, §Story Anchor, §VP Anchors, §Verification Properties, §Traceability, §Changelog.
...
For a **BC file** (normative sections only):
...
HITS=$(awk '/^## Preconditions$/{ found=1 } found && /^## Related BCs$/{ exit } found{ print }' "$BC_FILE" \
  ...
  | grep -Eiv "no |not |NOT |without|never|does not|MUST NOT|is NOT|cannot|do NOT|only from|exclusively" \
  | wc -l)
[ "$HITS" -eq 0 ] || echo "FAIL: $BC_FILE has $HITS genuine pure-parse violations"
```
...
**Honest statement of residual risk:** This lexical gate cannot detect every possible violation. A violation phrased using a non-enumerated read verb (e.g., "queries", "consults", "examines") would not be caught. The verb list (`reads?|loads?|fetches|derives?|access(es)?|retrieves?|opens?|parses?`) covers the domain of BC prose actually observed across the SS-04 BC corpus. If future BCs use unusual verbs, the gate's verb pattern must be extended. The gate is verified correct on the current BC-4.14.001/BC-4.15.001/VP-083/VP-081/VP-091 corpus as of ADR-026 v1.30.

**Positive control verification:** The algorithm correctly detects the following injected violation:
> "The gate reads wave context directly from sprint-state.yaml before parsing the payload."
Returns HITS=1.

**Empirical validation (as of ADR-026 v1.30):** All five assertions return 0 on the current clean corpus:
- BC-4.14.001 normative sections: 0 genuine violations
- BC-4.15.001 normative sections: 0 genuine violations
- VP-083 whole-file: 0 genuine violations
- VP-081 whole-file: 0 genuine violations
- VP-091 whole-file: 0 genuine violations
