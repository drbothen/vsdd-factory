...
**SEC-002 requirement:** The `command_preview` field emitted in both channels (PC-B-B1 stderr and PC-B-B2 plugin.log) MUST have secrets redacted before the preview string is constructed. Redaction occurs BEFORE truncation (redact-then-truncate ordering mandate). This ordering prevents a secret that straddles the 120-character truncation boundary from being partially exposed in the truncated output.

**Algorithm — no-regex two-pass redaction (non-configurable):**

The redaction algorithm is hard-coded in the WASM plugin source. It MUST NOT be made configurable at runtime (no config-file override, no hooks-registry.toml config key for the trigger list). The trigger list is a compile-time constant. Extending it requires a source code change and a new release.

**Pass 1 — flag-argument masking:**

Scan the command string for flag tokens from the following hard-coded trigger list. Each trigger is matched by exact case-sensitive prefix equality against whitespace-delimited tokens.
...
**Rationale for including `--key`:** `--key` is used by several CLIs (including curl-like tools and cloud SDKs) as a short form for API key or encryption key arguments. False-negative risk (masking a flag that is not a secret in a specific invocation) is preferred over false-positive risk (leaking a secret). Over-mask is the accepted trade-off.

**Pass 2 — inline assignment and Authorization header masking:**

After Pass 1, scan the resulting string for the following patterns and replace the value portion with `***REDACTED***`:
...
**Mask token:** The literal string `***REDACTED***` (15 characters, all ASCII). This token is hard-coded and not configurable.

**False-negative acknowledgment:** This algorithm does not detect all possible secret forms. It does not parse environment variable assignments (e.g., `MY_SECRET=abc cmd`), positional arguments, or secrets embedded in subshell expansions (`$(cat /secret)`). These cases are acknowledged out-of-scope for v1. The algorithm targets the highest-probability leakage patterns (flag-argument pairs and Authorization headers) without attempting exhaustive detection. Adding detection for new patterns requires a source code change.

**Invariant preservation — command_preview ≤120-char contract (EC-020 truncation-interaction):**

The redacted string (output of Pass 2) is the input to the truncation step (BC-4.15.001 Invariant 4 / VP-091 §5). The ≤120-character truncation invariant applies to the REDACTED string. Because `***REDACTED***` (15 chars) may be longer than the secret it replaces, the redacted command string may be longer than the original. Truncation is applied after redaction — the truncated preview is therefore always ≤120 characters (+ optional `…`) and never contains a partial secret token. The `message` field in the plugin.log record derives from the already-redacted `command_preview` and therefore requires no separate redaction step.
...
|---------|------|--------|--------|
| v1.27 | 2026-06-26 | architect | (SEC-002) §Decision 12 SEC-002 Redaction Sub-clause appended. Specifies redact-then-truncate ordering mandate; no-regex two-pass algorithm (Pass 1 flag-argument masking with hard-coded trigger list including `--key`; Pass 2 Authorization/Bearer inline masking); mask token `***REDACTED***`; non-configurable trigger list; false-negative acknowledgment; EC-020 truncation-interaction invariant (redacted string is input to truncation; `message` field inherits redacted `command_preview` and needs no separate redaction). VP-091 §6 authored in same burst. §Traceability ARCH-INDEX provenance leg pending state-manager propagation. Refs: SEC-002, VP-091 §6, BC-4.15.001, S-18.06, E-18, issue-173. |
