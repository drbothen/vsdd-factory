...
| `precompact_flush_sha` | string (40-char hex) OR null | `precompact-flush-log` append-only side-channel file (last-line SHA field) | Three-case null rule (CV-P14-001; BC-5.41.001 §PC2/PC5/EC-006/EC-011 three-case null rule): **(1) 40-char hex** — SHA of last commit written by `precompact-flush.sh` in this wave, corroborated against last-line field 2 of `.factory/hooks/precompact-flush-log` (field 4 must equal `commit`) — VALID. **(2) null when wave_id=1** — permitted unconditionally (no PreCompact fired before first wave close; log may be absent). **(3) null when wave_id>1 AND log genuinely absent** — verified via `test -f .factory/hooks/precompact-flush-log` in shell-context (or `host::read_file` returning FileNotFound in WASM-context); log absence means no PreCompact flush ever ran this wave — ADVISORY warning emitted, no hard block. **(4) null when wave_id>1 AND log EXISTS with a valid last-line (FIELD-4 == `commit`)** — corroboration failed: SHA present in log does not match null field → **HARD BLOCK (PrecompactShaMismatch)**. **(5) Log corruption** (FIELD-4 absent, empty, or ≠`commit`) → stale entry → prefix-match-alone → **EXEMPT** (consistent with DI-025/BC-5.41.003/§F-P4-004 EC-003 corruption arm — do NOT block). |
...
**Wave-1 and Genuine-Log-Absence special cases (CV-P14-001; three-case null rule):** The `precompact_flush_sha` field permits `null` in two distinct scenarios:

1. **wave_id = 1 (first wave):** No prior HANDOFF.md exists and no PreCompact flush has fired. `null` is permitted unconditionally. The `wave-handoff` skill documents this in its output header. The log (`.factory/hooks/precompact-flush-log`) may be absent; its absence is not verified for wave_id=1.

2. **wave_id > 1 AND precompact-flush-log genuinely absent:** The log file does not exist, verified mechanically via `test -f .factory/hooks/precompact-flush-log` (shell-context) or `host::read_file` returning FileNotFound (WASM-context). Genuine log absence means no PreCompact flush ever ran during this wave. `null` is permitted with an ADVISORY warning emitted to stderr — no hard block. The absence must be verified by filesystem probe, NOT by operator attestation.

A `null` value is a **HARD BLOCK (PrecompactShaMismatch)** for wave_id > 1 when the log EXISTS and its last line has a valid commit SHA (FIELD-4 == `commit`). Log corruption (FIELD-4 absent, empty, or ≠ `commit`) is treated as a stale entry and falls through to EXEMPT (prefix-match-alone) per §F-P4-004/EC-003 — the corruption→EXEMPT arm is unchanged.
