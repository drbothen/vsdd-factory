...
**Pass 3 — Authorization/Cookie header masking:**

For any whitespace-delimited token (or quoted argument) whose lowercased form starts with `authorization:`, `cookie:`, or `set-cookie:`, mask everything after the first `:` with `***REDACTED***`. Also handle the two-token sequence where the first token lowercased is exactly one of `authorization:`, `cookie:`, `set-cookie:` and the value is the next whitespace-delimited token — in that case mask the next token. This covers all Authorization schemes (Basic, Bearer, ApiKey, etc.) uniformly without scheme enumeration.
