...
**ACCEPTED — all open questions resolved via LOCKED DECISIONS D1–D5 (human-authorized F2 gate). F2 adversarial-pass-4 revision (v1.4) complete. E-18 story decomposition may proceed (F3). Implementation dispatch ready.**
...
6. Appends `<ISO-timestamp> <SHA> <cycle>/<step> commit` to the append-only side-channel log (`.factory/hooks/precompact-flush-log`) BEFORE the `git push` step. This write is local filesystem and precedes the push to establish crash-consistent ordering. The fourth field (`commit`) is the output of `git cat-file -t <SHA>` executed HERE in the shell hook (which CAN exec git). WASM corroboration validators (`validate-burst-log`, `validate-dispatch-advance`) read this pre-embedded token statically via `host::read_file` — they do NOT exec `git cat-file` (WASM has no `exec_subprocess`). **Append failure exits 2 (block compaction):** If the append to `precompact-flush-log` fails, the hook exits 2 — not 0. Rationale: an append failure leaves the log in a state where its last-line SHA is a PRIOR flush's SHA (SHA_A), but the just-committed HEAD on factory-artifacts is SHA_B. BC-5.41.003 PC1(a) then sees a mismatch (last-line SHA_A ≠ HEAD SHA_B) and cannot grant the exemption, causing a false-positive `MULTI_COMMIT_CHAIN_NOT_ALLOWED` block on the next state-manager burst. Blocking compaction on append failure is the correct response — it is a rare local filesystem event that signals something is wrong, and allowing compaction with an inconsistent log introduces the exact regression CAP-032 Part B exists to prevent. The `on_error = "continue"` fail-open at the harness level applies ONLY to full hook crashes (before any exit code is emitted) — not to deliberate exit 2 signals.
...
## F-P4-004 PO Wording (exact replacement text for BC-5.41.003 PC1 and Inv1)

This section provides the exact replacement wording for BC-5.41.003 PC1 and Inv1 to close both F-P4-001 (coherence) and O-P4-004 (WASM/shell clarity). The PO MUST apply these changes to BC-5.41.003 in the same burst. The PO MUST NOT add any wording implying WASM execs `git cat-file` — that exec happens only in the shell hook.
...
>
> **(b) Log present but last-line SHA stale (write-before-push crash):** If the last-line SHA field is present and field 4 equals `commit` but the SHA does not match the exempted commit's SHA → the exempted commit is NOT corroborated → NOT EXEMPT (mismatch; suspicious prefix-only commit). If the last-line SHA field is present but field 4 does NOT equal `commit` (write-before-push crash: SHA in log but not yet a real commit) → treat as stale entry → fall through to case (c).
>
> **(c) Log genuinely absent or last line empty:** If the log is genuinely absent or the last line is empty → prefix match alone is sufficient for EXEMPT determination. This covers first-flush-ever (no prior log) and hook-crash scenarios where the log could not be written at all. Note: under BC-7.07.001 PC8 (append failure → exit 2), the log is always commit-consistent when compaction proceeds. Case (c) applies only to hook-crash-before-append (which triggers `on_error=continue` at the harness level).
>
...
Add a new row after EC-003 row 4:

> `HEAD="PreCompact flush v1.0/S-18.04 ..."; precompact-flush-log last-line field 4 does NOT equal "commit" (write-before-push crash: append succeeded but SHA not yet a real commit) → EXEMPT (stale-entry case; prefix match alone sufficient).`
