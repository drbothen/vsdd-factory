...
modified:
  - "2026-06-29 (v1.37) — architect adjudication (S-18.11 full-graph depth; human directive 'full rigor, option b'): §Decision 3a AMENDED AGAIN. Intra-partition ordering basis changed from partition-restricted edges (definition a) to full-graph dependency depth (definition b). Wave depth of story S is now defined as 1 + max(depth of all predecessors in the FULL depends_on graph, including cross-partition/supersession edges), with root stories (no depends_on) = depth 1. This is a global graph property independent of partition placement. Intra-partition sort key becomes (full-graph-wave-depth ASC, story-id lex ASC) for BOTH Partition A and Partition B. Partition placement (terminal first) is UNCHANGED. Cross-partition supersession edges from S-3.01/S-3.02/S-3.03/S-4.07/S-4.08 to S-3.04 (partial, `superseded_by: ADR-015`) are NOW INCLUDED in the depth computation — e.g. S-3.01 full-graph depth = depth(S-2.08) + 1 = 10, and S-3.04 (partial) contributes depth 4 as a predecessor. The TopoViolation guard detection logic (EC-010: tolerate supersession edges, abort genuine anomalies) is UNCHANGED — only the depth-computation basis changes. The current sprint-state.yaml Partition A does NOT conform to definition (b): 10 monotonicity violations confirmed by mechanical spot-check (S-9.00 depth=5 placed before S-9.30/S-0.02 depth=1/2; S-18.06/S-18.07/S-18.13 depth=1 placed inside depth-4/5/6 blocks). Re-sort is REQUIRED (implementer scope). Refs: S-18.11, human directive 2026-06-29 'full rigor option b', BC-5.41.004 PC3, SKILL.md Step 5, E-18, issue-173. [Prior: 2026-06-29 (v1.36) — architect re-adjudication (S-18.11 EC-010 supersession-edge narrowing; human directive): §Decision 3a AMENDED. The v1.35 topo-validity claim ('zero terminal→non-terminal dependency violations — mechanically verified') was FACTUALLY INCORRECT. The real vsdd-factory STORY-INDEX graph contains terminal→non-terminal edges: S-3.01 (merged), S-3.02 (merged), S-3.03 (merged), S-4.07 (merged), S-4.08 (merged) all carry `depends_on:` entries referencing S-3.04 (status: partial). S-3.04 carries `superseded_by: ADR-015` frontmatter — it is an abandoned-by-supersession story (AC-001 Router::submit never wired in main.rs; ADR-015 retired Router/multi-sink in favour of single-stream OTel-aligned emission; see S-3.04 supersession_note). These are LEGITIMATE supersession edges — the dependent stories merged using the shipped portion of S-3.04; the unfinished Router-wiring remainder was retired by ADR-015. Human directive (2026-06-29): S-3.04 STAYS status: partial (alive-but-paused); ADR-015 revisit (Router/multi-sink revival) is a SEPARATE effort AFTER E-18. S-3.04 is NOT reclassified to any terminal status. BC-5.41.004 EC-010 is narrowed accordingly (see §Decision 3a body). Forward-reference note: ADR-015 (Router/multi-sink retirement) is slated for a POST-E-18 revisit; until that revisit, S-3.04 intentionally remains `partial` with `superseded_by: ADR-015`, and the supersession edges from S-3.01/S-3.02/S-3.03/S-4.07/S-4.08 to S-3.04 are spec-compliant under the narrowed EC-010. Refs: S-18.11 EC-010 human directive, S-3.04 frontmatter, ADR-015, BC-5.41.004 EC-010, E-18, issue-173."
  - "2026-06-28 (v1.35) — architect adjudication (S-18.11 pass-6 F-P6-002 ordering-contract incompatibility): §Decision 3a (NEW) codifies the canonical two-partition sprint-state.yaml ordering rule — resolves the incompatibility between BC-5.41.004 PC3 (strict wave-ascending topo-sort) and BC-5.41.001 PC2/P-SPRINT-STATE-WAVE-ORDER/WaveOrderUnverifiable guard (terminal-contiguous-leading-prefix required). Canonical rule: (i) ALL terminal stories first (partition A), ordered wave-ascending topo-sort + lex tie-break within the terminal partition; then (ii) ALL non-terminal stories (partition B), ordered wave-ascending topo-sort + lex tie-break within the non-terminal partition. Topo-validity confirmed: zero terminal→non-terminal dependency violations in the vsdd-factory graph (mechanically verified). [CORRECTED by v1.36: the 'zero violations' claim was factually wrong — S-3.01/S-3.02/S-3.03/S-4.07/S-4.08 all depend_on S-3.04 (partial); these are legitimate supersession edges tolerated by narrowed EC-010.] This rule satisfies both the WaveOrderUnverifiable guard (no post-boundary terminal entries) and provides deterministic intra-partition ordering. BC-5.41.004 PC3 must be amended by product-owner to two-partition rule. BC-5.41.001 PC2/P-SPRINT-STATE-WAVE-ORDER are compatible as-is (guard only requires terminal-contiguous-prefix; two-partition satisfies this). S-18.11 AC-002 and SKILL.md Step 5 must be updated to two-partition algorithm. wave_id correctness confirmed: single terminal block → one completed wave group → wave_id = 2 (unchanged). Refs: S-18.11 pass-6 F-P6-002, BC-5.41.004 PC3, BC-5.41.001 PC2/P-SPRINT-STATE-WAVE-ORDER, parse-sprint-state.sh WaveOrderUnverifiable, E-18, issue-173."
  - "2026-06-28 (v1.34) — architect adjudication (S-18.11 pass-2 obs-1): §Decision 2 `wave_id` source column clarified — wave GROUP ordinal semantics made explicit; wave_id = completed_wave_groups + 1, NOT leading_terminal_story_count + 1; canonical example added (103 merged stories in one contiguous block → wave_id = 2, not 104); `derive_wave_id` transition-counting algorithm named as canonical implementation. No behavioral change to the algorithm — the merged `parse-sprint-state.sh` `derive_wave_id` was already correct; this amendment closes the S-18.11 AC-006 story-count-vs-wave-count ambiguity before T-1 bats tests are written. Refs: S-18.11 pass-2 obs-1, BC-5.41.001 PC2, parse-sprint-state.sh derive_wave_id, E-18, issue-173."
  - "2026-06-27 (v1.33) — state-manager governance burst (D-707): §Decision 14 spec-hygiene normalization (LOCAL adv P3 O-P3-002/O-P3-003): §Decision 14 grep snippets normalized `\\s+`→`[[:space:]]+` and `\\s*`→`[[:space:]]*` (6 lines) for BSD/macOS + GNU grep portability; portability note added to Layer 2 read-verb prose. No logic change. AC snippets in S-18.08 normalized in parallel (story v1.8→v1.9). Validated 0/0/0 + 3 positive controls. STORY-INDEX v4.91→v4.92; ARCH-INDEX provenance leg appended v2.83→v2.84. Refs: D-707, O-P3-002, O-P3-003, S-18.08 v1.9, ADR-026 §Decision 14, E-18, issue-173."
  - "2026-06-27 (v1.32) — state-manager governance burst (D-706): §Decision 14 verb-set reconciliation (LOCAL adv P2 F-P2-001): read-verb pattern expanded to canonical 8-verb form `(reads?|loads?|fetches|derives?|access(es)?|retrieves?|opens?|parses?)` in all three command-block occurrences (BC-loop inner, VP-scan, positive-control snippet) + prose mention. Opens/parses positive-control verification rows added. Empirical validation banner updated to v1.32. S-18.08 v1.7→v1.8; ARCH-INDEX provenance leg appended v2.82→v2.83. Refs: D-706, F-P2-001, S-18.08 v1.8, ADR-026 §Decision 14, E-18, issue-173."
  - "2026-06-27 (v1.31) — state-manager governance burst (D-705): §Decision 14 REFINEMENT (LOCAL adv P1 F-P1-001/F-P1-002, architect-refined): discovery anchored to ## Invariants section (excludes BC-INDEX + SS-07 prose); Layer-1 awk whitelist terminator (robust to '(Recommended)' headings); fail-loud scannability guard added to AC-003 loop. S-18.08 v1.6→v1.7; ARCH-INDEX provenance leg appended v2.81→v2.82. Refs: D-705, F-P1-001, F-P1-002, S-18.08 v1.7, ADR-026 §Decision 14, E-18, issue-173."
  - "2026-06-27 (v1.30) — state-manager governance burst (D-704): §Decision 14 authored by architect — pure-parse invariant consistency gate detection algorithm (3-layer: normative-section extraction + verb+substrate collocation + negation/comment exclusion); S-18.08 AC-001..005 redesign anchor; human-adjudicated S-18.08 gate redesign. ARCH-INDEX provenance leg appended v2.80→v2.81. Refs: D-704, S-18.08 v1.6, ADR-026 §Decision 14, E-18, issue-173."
last_amended: "2026-06-29 (v1.37) — architect adjudication (S-18.11 full-graph depth; human directive 'full rigor, option b'): §Decision 3a AMENDED — intra-partition ordering basis changed from partition-restricted edges (definition a) to full-graph dependency depth (definition b); wave depth = longest-path depth over full depends_on graph (root=1; depth(S)=1+max(depth(predecessors))); supersession edges included in depth computation; partition placement (terminal first) unchanged; TopoViolation guard detection logic unchanged; re-sort of sprint-state.yaml Partition A required (implementer). [Prior: 2026-06-29 (v1.36) — architect re-adjudication (S-18.11 EC-010 supersession-edge narrowing; human directive): §Decision 3a AMENDED — v1.35 'zero topo-violations' claim corrected; EC-010 narrowed to tolerate superseded-by-ADR dependencies (S-3.01/S-3.02/S-3.03/S-4.07/S-4.08 → S-3.04 partial); hard-abort preserved for genuine non-superseded anomalies. S-3.04 STAYS partial; ADR-015 revisit is POST-E-18. See modified[] row for full detail. [Prior: 2026-06-28 (v1.35) — architect adjudication (S-18.11 pass-6 F-P6-002): §Decision 3a (NEW) two-partition sprint-state.yaml ordering rule codified — resolves BC-5.41.004 PC3 vs BC-5.41.001 PC2/WaveOrderUnverifiable incompatibility; wave_id unaffected. [CORRECTED by v1.36: topo-validity claim was wrong.] [Prior: 2026-06-28 (v1.34) — architect adjudication (S-18.11 pass-2 obs-1): §Decision 2 wave_id source column clarified — wave GROUP ordinal semantics made explicit (wave_id = completed_wave_groups + 1, NOT story count + 1); canonical example added; derive_wave_id transition-counting named as canonical implementation. No behavioral change. Refs: S-18.11, E-18, issue-173. [Prior: 2026-06-27 (v1.33) — state-manager governance burst (D-707): §Decision 14 spec-hygiene normalization (LOCAL adv P3 O-P3-002/O-P3-003): grep snippets normalized `\\s+`→`[[:space:]]+` and `\\s*`→`[[:space:]]*` for BSD/macOS portability; portability note added. No logic change. S-18.08 v1.8→v1.9; ARCH-INDEX v2.83→v2.84. Refs: D-707, O-P3-002, O-P3-003, E-18, issue-173. [Prior: 2026-06-27 (v1.32) — state-manager governance burst (D-706): §Decision 14 verb-set reconciliation (LOCAL adv P2 F-P2-001): read-verb pattern expanded to canonical 8-verb form `(reads?|loads?|fetches|derives?|access(es)?|retrieves?|opens?|parses?)` in all three command-block occurrences (BC-loop inner, VP-scan, positive-control snippet) + prose mention. Opens/parses positive-control verification rows added. Empirical validation banner updated to v1.32. S-18.08 v1.7→v1.8; ARCH-INDEX provenance leg appended v2.82→v2.83. Refs: D-706, F-P2-001, S-18.08 v1.8, ADR-026 §Decision 14, E-18, issue-173. [Prior: 2026-06-27 (v1.31) — state-manager governance burst (D-705): §Decision 14 REFINEMENT (LOCAL adv P1 F-P1-001/F-P1-002, architect-refined): discovery anchored to ## Invariants section (excludes BC-INDEX + SS-07 prose); Layer-1 awk whitelist terminator (robust to '(Recommended)' headings); fail-loud scannability guard added to AC-003 loop. S-18.08 v1.6→v1.7; ARCH-INDEX provenance leg appended v2.81→v2.82. Refs: D-705, F-P1-001, F-P1-002, S-18.08 v1.7, ADR-026 §Decision 14, E-18, issue-173. [Prior: 2026-06-27 (v1.30) — state-manager governance burst (D-704): §Decision 14 authored by architect — pure-parse invariant consistency gate detection algorithm (3-layer: normative-section extraction + verb+substrate collocation + negation/comment exclusion); S-18.08 AC-001..005 redesign anchor; human-adjudicated S-18.08 gate redesign. ARCH-INDEX provenance leg appended v2.80→v2.81. Refs: D-704, S-18.08 v1.6, ADR-026 §Decision 14, E-18, issue-173. [Prior: 2026-06-26 (v1.29) — fix burst (architect): (F-RD2-001/F-RD2-002) §Decision 12 SEC-002 Redaction Sub-clause Pass 3 replaced with bounded-consumption rule: inline-value form masks everything after `:` within the same token; following-token form masks the immediate next token (plus scheme token for two-token scheme+credential sequences) and STOPS at the first subsequent token that starts with `-` (CLI flag), contains `://` (URL), or closes a quoted value — prevents over-consumption (F-RD2-001: URL after unquoted auth value destroyed; F-RD2-002: malformed unbalanced-quote swallows command tail). Test vectors added: Bearer+tok+URL (URL preserved), Bearer+tok+--verbose (flag preserved), unbalanced-quote (stops at flag/end), quoted `-H` form (still masked). VP-091 §6 PC6d revised in same burst. ARCH-INDEX provenance leg pending state-manager propagation. Refs: F-RD2-001, F-RD2-002, VP-091 §6, BC-4.15.001, S-18.06, E-18, issue-173. [Prior: 2026-06-26 (v1.28) — fix burst (architect): (SEC-002 revised) §Decision 12 SEC-002 Redaction Sub-clause replaced with final canonical 4-pass design: best-effort-advisory framing (not a security boundary; aligned with GitLab/Warp best-effort masking guidance); mask token `***REDACTED***` (14 ASCII chars, corrected from erroneous earlier counts); Pass 1 flag-argument masking (flag name lowercased, equals-or-contains trigger keywords; bare `--key` REMOVED — false-positive trap; no bare short flags); Pass 2 env-var assignment-prefix masking (compound triggers only; bare KEY not matched; allowlist SSH_AUTH_SOCK/SSH_ASKPASS/*_SERVICE_HOST checked first); Pass 3 Authorization/Cookie/Set-Cookie header masking; Pass 4 inline URL credential masking (`://user:pass@host` → `://***REDACTED***@host`); provider-prefix shape detection (sk-/AKIA/ghp_) explicitly deferred as documented future enhancement; EC-020 redact-then-truncate ordering invariant preserved. VP-091 §6 revised to 7 postconditions (PC6a–PC6g) in same burst. §Traceability ARCH-INDEX provenance leg pending state-manager propagation. Refs: SEC-002, VP-091 §6, BC-4.15.001, S-18.06, E-18, issue-173. [Prior: 2026-06-26 (v1.27) — fix burst (architect): (SEC-002) §Decision 12 SEC-002 Redaction Sub-clause appended: redact-then-truncate ordering mandate; no-regex two-pass algorithm (Pass 1 flag-argument masking with trigger list including `--key`, `--token`, `--password`, `--secret`, `--api-key`, `--auth`, `--bearer`, `--credential`, `--private-key`, `--access-key`, `--secret-key`; Pass 2 inline assignment/Authorization/Bearer masking); mask token `***REDACTED***`; hard-coded non-configurable trigger list; false-negative acknowledgment; invariant-preservation note. VP-091 §6 authored in same burst. [Prior: 2026-06-25 (v1.26) — fix burst (architect): (adv P2 F-P2-002) §Decision 7 generator-handling text corrected: post-retirement hooks register via POST_HISTORICAL_SCRIPTS + skip_names (check-harness-version precedent), not full revert-to-develop — the band_for()/legacy-JSON edits are the drift hazard and are reverted, POST_HISTORICAL emits nothing. [Prior: 2026-06-25 (v1.25) — fix burst (architect): §Decision 7 amended — phantom-field anchoring fix (F-P1-001/S-18.05): `last_verified_develop_sha:` is NOT a STATE.md frontmatter field; it is a HANDOFF.md field. The hook `postcompact-reanchor.sh` was prescribed (BC-7.07.002 PC1, VP-089 §1, S-18.05 AC-001/AC-009) to read this field from `git show factory-artifacts:.factory/STATE.md`, but the field is absent — causing `sha=UNKNOWN` on every real PostCompact firing. Fix: the re-anchor block SHA MUST be sourced from `git rev-parse origin/develop` (or `refs/remotes/origin/develop`) executed at hook time. This is the live authoritative develop HEAD; it requires no STATE.md schema change; it matches the hook's existing git-based read-only approach. No `last_verified_develop_sha:` field is added to STATE.md. (F-P1-003/S-18.05): `[hooks.capabilities]` env_allow corrected — removed `GIT_DIR` (was added to support the bats test fixture via GIT_DIR env injection, a test artifact not a production need; production git does not require GIT_DIR to be set explicitly); added `GIT_CONFIG_GLOBAL` and `XDG_CONFIG_HOME` (needed by git subprocess to locate its own config, matching Decision 6's canonical pattern). (F-P1-003/S-18.05): `[hooks.capabilities.read_file]` replaced with `[hooks.capabilities.write_file]` — the hook reads STATE.md via `git show` subprocess (covered by exec_subprocess, not read_file); the only direct filesystem write is the JSONL log. Canonical `write_file.path_allow` set to `.factory/logs/` (log write target). §Traceability ARCH-INDEX v2.76→v2.77 provenance leg appended. Refs: F-P1-001, F-P1-003, BC-7.07.002 PC1, VP-089 §1, S-18.05 AC-001/AC-009/AC-010, DI-024, E-18, issue-173. [Prior: 2026-06-19 (v1.24) — fix burst (architect): §Decision 8 amended — EPIC-COMPLETE carve-out added to Step 5 (`--commit`) of the canonical SKILL.md step structure and to the `--commit` subcommand normative prose (F-SP13-P5-001). The v1.23 Step 5 demanded BOTH HANDOFF.md + wave-state.yaml but Step 4 is explicitly skipped on EPIC-COMPLETE, meaning wave-state.yaml is never written on that path — so `--commit` would hard-abort on a missing wave-state.yaml, making epic closure impossible. Fix: Step 5 now carries an explicit EPIC-COMPLETE carve-out parallel to Step 4's: on the HAS-NEXT-WAVE path, `--commit` stages BOTH HANDOFF.md + wave-state.yaml and hard-aborts if EITHER is absent; on the EPIC-COMPLETE path (`epic_status: complete` / `next_wave_stories: []`), `--commit` stages HANDOFF.md ALONE (and removes any stale wave-state.yaml per current wave-handoff.sh behavior), hard-aborting ONLY on missing HANDOFF.md. A single-file EPIC-COMPLETE commit is still ONE atomic commit — not a BC-5.41.002 PC6 violation (PC6 forbids two separate commits; a single-file commit is still a single commit). The BC-4.14.001 replacement cite text in §BC Traceability Cite Convention is updated to reflect the EPIC-COMPLETE commit-arm distinction. §Traceability ARCH-INDEX v2.59→v2.60 provenance leg appended. Refs: F-SP13-P5-001, S-18.13, BC-5.41.002 PC6, E-18, issue-173. [Prior: 2026-06-19 (v1.23) — fix burst (architect): §Decision 8 amended — agent-orchestrated emit→Write→commit control flow specified (S-18.13 redesign). The v1.22 normative rule stated that `write-handoff.sh` MUST 'route its HANDOFF.md output through the Write tool', which was impossible for a monolithic bash run: `wave-handoff.sh` ends in `main \"$@\"` and has no agent seam between Step 4 (write_handoff call) and Step 6 (commit_to_artifacts). The correct mechanism requires restructuring the skill into an agent-orchestrated model: (1) Bash 'emit' subcommand — `wave-handoff.sh --emit-handoff` computes all HANDOFF.md fields with full anti-fabrication cross-checks and emits the assembled YAML payload to stdout; NO disk write, NO commit; (2) Agent Write-tool step — SKILL.md numbered agent step captures stdout payload and invokes the Claude Code Write tool to write `${ARTIFACTS_WT}/HANDOFF.md`; the PostToolUse `validate-wave-handoff-completeness` gate fires here; (3) Bash 'emit-wave-state' subcommand — `wave-handoff.sh --emit-wave-state` (or existing shell logic) writes wave-state.yaml to disk via bash; (4) Bash 'commit' subcommand — `wave-handoff.sh --commit` stages HANDOFF.md (on disk from Write tool) + wave-state.yaml (on disk from step 3) and creates a SINGLE atomic git commit (BC-5.41.002 PC6 atomicity preserved). Fail-loud-no-fallback obligation unchanged: if Write tool is unavailable, emit hard error `HandoffWriteToolUnavailable`; no bash-redirect fallback permitted; no partial HANDOFF.md committed. §Traceability ARCH-INDEX v2.58→v2.59 provenance leg appended. Refs: F-S1802-02, S-18.13, BC-4.14.001, BC-5.41.002 PC6, E-18, issue-173. [Prior: 2026-06-19 (v1.22) — fix burst (architect): §Decision 8 amended — HANDOFF.md write-path constraint added (F-S1802-02 / S-18.13). The `wave-handoff` skill MUST write HANDOFF.md via the Claude Code Write tool (not bash redirection) so that the PostToolUse `validate-wave-handoff-completeness` gate fires on every HANDOFF.md write. Bash redirection (`} > \"$output_path\"` in the `write-handoff` skill component) emits NO PostToolUse event, rendering BC-4.14.001 completeness gate functionally inert in production. The producer MUST NOT silently fall back to bash redirection if the Write tool is unavailable — it must fail loudly (see S-18.13 EC-003). Refs: F-S1802-02, S-18.13, BC-4.14.001, E-18, issue-173. [Prior: 2026-06-16 (v1.21) — fix burst (architect): S-18.10 ADR-026 §Deliverables row added — check-state-health settings.json CLAUDE_AUTOCOMPACT_PCT_OVERRIDE verification (BC-6.25.001) explicitly owned in §Deliverables table with correct story S-18.10 (replaces stale 'S-18.03 or S-18.07' provisional row). VP-092 v1.0 authored in same burst (behavioral-invariant; unit-test; SS-06; DI-020; BC-6.25.001; S-18.10). §Traceability VP-INDEX provenance leg appended: v2.33→v2.34 (VP-092 v1.0 authored — check-state-health CLAUDE_AUTOCOMPACT_PCT_OVERRIDE settings.json verification; SS-06; DI-020; unit-test; S-18.10; total_vps 91→92; unit-test 45→46). ARCH-INDEX provenance leg appended: v2.50→v2.51 (ADR-026 v1.20→v1.21 S-18.10 §Deliverables row + VP-092 v1.0). F-MAJOR-003 coverage gap closed. Refs: BC-6.25.001 v1.0, VP-092 v1.0, S-18.10, E-18, CAP-032, issue-173. [Prior: 2026-06-16 (v1.20) — fix burst (architect): (F3 OQ-4 human directive) §Decision 12 amended — 'No BC required for v1 advisory' clause replaced with 'BC-4.15.001 required' clause. The advisory output IS a contractable postcondition: BC-4.15.001 specifies always-Continue (never blocks), dual-channel DelegationRecommended advisory on match (stderr B-1 + plugin.log B-2), no advisory on no-match (PC-A), non-Bash no-op (PC-D). §Deliverables table S-18.06 row updated: 'no BC in v1' → 'BC-4.15.001'. v1→blocking-promotion remains amend-not-replace per §Future Mode (non-normative). §Traceability ARCH-INDEX provenance leg appended: v2.49→v2.50 (ADR-026 v1.19→v1.20 §Decision 12 BC-4.15.001 amendment). Refs: F3 OQ-4, BC-4.15.001 v1.0, VP-091 v1.0, S-18.06, E-18, CAP-032, issue-173. [Prior: 2026-06-15 (v1.19) — fix burst (architect): (F-P37-001 MEDIUM semantic-anchoring/scope) SS-01 added to `anchors:` and `subsystems_affected:` frontmatter. Root cause: Decision 11 ('S-18.00: Dispatcher routing addition for PreCompact/PostCompact') is an explicit architectural decision about the factory-dispatcher binary in `crates/factory-dispatcher/src/` — unambiguously SS-01 (Hook Dispatcher Core). The prior `anchors:` list included only SS-04/SS-05/SS-06/SS-07, which by the §SS-08 Scope Clarification criterion ('enumerates subsystems where architectural decisions are made') should have included SS-01. Corroborating sources all confirm SS-01 scope: BC-1.15.001 subsystem=SS-01; VP-086 scope SS-01,SS-04; CAP-032 declares Subsystems SS-01,SS-04,SS-05,SS-06,SS-07; Deliverables table S-18.00 row Subsystem(s)=SS-01. §SS-08 Scope Clarification extended to explain the SS-01 anchors inclusion and to correct the stale 'potentially SS-01 per S-18.00 outcome' hedge in §Traceability (the outcome is now definitive per Decision 11). §Traceability ARCH-INDEX provenance leg appended: v2.46→v2.47 (ADR-026 v1.18→v1.19 F-P37-001). Refs: F-P37-001, BC-1.15.001, VP-086, CAP-032, E-18, issue-173. [Prior: 2026-06-15 (v1.18) — fix burst (architect): (F-P31-001 exhaustive sweep) §Decision 6 step 5 normative stale-term fix — 'append-only side-channel log' → 'append-only log' (canonical). This site is in the normative body of §Decision 6 (not in §Crash-Consistency Design); it was missed by the v1.17 sweep which adjudicated §Crash-Consistency Design section hits as historical but did not catch this §Decision 6 body occurrence. The precompact-flush-log descriptor has been 'append-only log' across all sibling normative bodies (BC-5.41.001 v1.13, BC-5.41.003 v1.7, VP-085 v1.7) since the F-P28-002 sweep. §Traceability ARCH-INDEX provenance leg appended: v2.45→v2.46 (ADR-026 v1.17→v1.18 F-P31-001). Refs: F-P31-001, VP-085 v1.7, BC-5.41.001 v1.13, BC-5.41.003 v1.7, E-18, issue-173. [Prior: 2026-06-15 (v1.17) — fix burst (architect): (F-P28-002-sibling MEDIUM) ADR-026 normative side-channel→precompact-flush-log term migration: two stranded normative-present-tense 'side-channel file' residue sites fixed. (1) §BC Traceability Cite Convention table BC-5.41.001 Replacement text cell: 'precompact_flush_sha hard cross-check against side-channel file' → 'precompact_flush_sha hard cross-check against precompact-flush-log (append-only)' — NORMATIVE-fixed (this is the PO-facing replacement text for BC-5.41.001; the stale term would propagate into the BC verbatim). (2) §Decision 2 HANDOFF.md schema table `precompact_flush_sha` Source column: '`precompact-flush-log` append-only side-channel file (last-line SHA field)' → '`precompact-flush-log` (append-only) (last-line SHA field)' — NORMATIVE-fixed (the point-file 'side-channel file' qualifier is stale since F-P2-003/Decision A; the canonical name is precompact-flush-log (append-only) per DI-025, BC-5.41.001, BC-5.41.003, BC-7.07.001). All other 'side-channel' occurrences in the §Crash-Consistency Design section adjudicated as historical-rationale (explicitly describing the OLD superseded point-file design or showing old BC text to be replaced) — retained per sweep classification. §Traceability ARCH-INDEX provenance leg appended: v2.44→v2.45 (ADR-026 v1.16→v1.17 F-P28-002-sibling). Refs: F-P28-002-sibling, DI-025, BC-5.41.001 PC5, BC-5.41.003, BC-7.07.001, ADR-026 §Decision A, E-18, issue-173. [Prior: 2026-06-15 (v1.16) — fix burst (architect): (F-P25-002 MEDIUM) §Decision 9 EPIC-COMPLETE-before-wave_id==1 carve-out: adjudicated (a) — single-wave epics CAN occur; the WASM gate MUST evaluate the EPIC-COMPLETE branch (`next_wave_stories: []`) BEFORE applying the wave_id==1 no-op. The prior ordering (no-op checks first per Inv3) leaves the EPIC-COMPLETE discriminator unreachable when wave_id==1, causing `epic_status: complete` to be never validated on a single-wave EPIC-COMPLETE write. §Decision 9 amended to make the new evaluation order canonical: (1) non-HANDOFF.md target → no-op; (2) EPIC-COMPLETE detection (`next_wave_stories: []`) → if EPIC-COMPLETE, validate `epic_status` even when wave_id==1; (3) wave_id==1 AND NOT EPIC-COMPLETE → no-op; (4) wave_id>1 → full validation. This preserves pure-parse purity (all checks are PAYLOAD-ONLY) and closes the gap where the completeness gate — whose purpose is preventing an incomplete handoff from being committed — was a no-op on the single-wave EPIC-COMPLETE case. (F-P25-003 LOW) §VP Allocations sibling-sweep completed: VP-082/VP-084/VP-085 abbreviated titles updated to verbatim VP H1 headings (VP-082: 'PreCompact Flush Commits to factory-artifacts Before Compaction Proceeds'; VP-084: 'PreCompact Flush Commit Is Lifecycle-Distinct From State-Manager Burst Commit'; VP-085: 'PreCompact Flush Hook Is Hermetic — Reads STATE.md+Git Only, Never In-Context Reasoning'), completing the F-P22-001 pass-22 sweep that had synced VP-081/VP-083 only. §Traceability ARCH-INDEX provenance leg appended: v2.43→v2.44 (ADR-026 v1.15→v1.16 F-P25-002 + F-P25-003). [Prior: 2026-06-15 (v1.15) — fix burst (architect): (F-P22-001 LOW) §VP Allocations table: VP-081 title corrected to 'Wave Cannot Close Without Verified Handoff (wave_id > 1)' (added '(wave_id > 1)' qualifier matching current VP-081 H1 verbatim); VP-083 title corrected to 'Completeness Gate Is No-Op on Wave-1 or Non-HANDOFF.md Writes' (added 'or Non-HANDOFF.md Writes' matching current VP-083 H1 verbatim). (F-P23-001 LOW) §Decision 6 numbered step list re-sequenced to canonical commit→capture SHA_B→append→push→exit-0 order per BC-7.07.001 Inv3: prior step 5 referenced push-failure as an exit-2 trigger before step 6 introduced the append-before-push ordering and without `git push` as an explicit numbered step; re-sequenced as step 4 (git commit), step 5 (append log BEFORE push — still carries the SHA-pinned append-failure guard), step 6 (`git push` to remote — explicit numbered step), with exit-2 conditions stated inline at each step that can fail (commit failure at step 4, append failure at step 5, push failure at step 6). §Traceability ARCH-INDEX provenance leg appended: v2.42→v2.43 (ADR-026 v1.14→v1.15). Field-4-provenance reconciliation (same burst, same version v1.15): §Decision 6 step 5 field-4-mechanism description revised — over-corrected 'NO live git cat-file -t execution is required (though the shell CAN exec git)' framing replaced with canonical: shell embeds field-4 token via `git cat-file -t <SHA_B>` (value: `commit`) OR by embedding the literal string `commit`, since the type is definitionally `commit` for a just-created commit; both approaches yield identical field-4; WASM corroboration validators read this pre-embedded token statically — they do NOT exec `git cat-file`. Now internally consistent with §Crash-Consistency Design lines 525/529 (which describe field-4 as the `git cat-file -t` output), BC-7.07.001 PC8 ('NOT the output of a live git cat-file -t call at validation time' — scoped to validation time/WASM, not write time), and DI-025 v1.16 BLOCKER ('embedded by precompact-flush.sh at flush time via git cat-file -t'). VP-082 v1.10→v1.11 (F-P24-001 MEDIUM + field-4-canonical reconciliation; PC-A parenthetical corrected — architect). VP-084 v1.8→v1.9 (F-P24-003 LOW; split-anchor SS-04/SS-05 confirmed + in-file justification added — architect). Refs: F-P22-001, F-P23-001, F-P24-001, F-P24-003, VP-081 H1, VP-083 H1, BC-7.07.001 Inv3, BC-7.07.001 PC8, DI-025 v1.16, ADR-026 §Crash-Consistency Design. [Prior: 2026-06-15 (v1.14) — fix burst (architect): (F-P20-002 LOW pending-intent) §Wave-Identity-Derivation third bullet ('For "wave 1 / first wave" discrimination') cross-note added: clarifies that the prior-HANDOFF.md-absence proxy is a CALLER-SIDE substrate used by the shell wave-handoff skill to DERIVE wave_id before writing the payload, and that `payload.wave_id == 1` is the WASM gate's own PAYLOAD-ONLY discriminator (established by F-P10-001/Decision 9). The two substrates are reconcilable: prior-HANDOFF.md absence is the caller's derivation signal → caller embeds wave_id=1 in payload → WASM gate reads `payload.wave_id == 1`. The proxy is not a competing substrate — it is the upstream input from which the caller produces the payload field the gate consumes. §Traceability ARCH-INDEX provenance leg appended: v2.41 to v2.42 (ADR-026 v1.13 to v1.14 F-P20-002 fix). Sibling-strand sweep: VP-081 Precondition already correctly describes the SHELL CALLER / WASM GATE division (no edit needed); BC-4.14.001 already sanctioned by ADR-026 §Decision 9 (no routing needed). Refs: F-P20-002, ADR-026 §Decision 9, BC-4.14.001, VP-081 v1.5. [Prior: 2026-06-15 (v1.13) — fix burst (architect): (F-P19-002 MEDIUM BC-ADR contradiction) §Decision 9 heading retitled from 'no-op on wave-1 / HANDOFF.md absent' to 'no-op on wave-1 / non-HANDOFF.md writes': the prior heading embedded obsolete file-absence proxy framing superseded by F-P10-001 and by VP-083 v1.5 corrected title. §Decision 9 third bullet ('Wave close attempted...Block with HandoffMissing') removed: HandoffMissing is a SHELL wave-gate error code (BC-5.41.001 PC9/EC-014) emitted when wave-gate is invoked to close a wave but HANDOFF.md is absent on factory-artifacts — the check is SHELL-SIDE, performed before any WASM gate invocation; the validate-wave-handoff-completeness WASM gate validates a Write/Edit payload, fires only on HANDOFF.md writes (BC-4.14.001 EC-011), cannot detect file-absence, and NEVER emits HandoffMissing. Third bullet replaced with: (b) non-HANDOFF.md write target no-op bullet per BC-4.14.001 EC-011; (c) explicit 'Semantic boundary — HandoffMissing is SHELL-SIDE' paragraph documenting the disjoint (HandoffMissing=SHELL/file-absent, HandoffIncomplete=WASM/present-but-incomplete), superseding any prior framing that attributed HandoffMissing to the WASM gate. §BC Traceability Cite Convention table BC-4.14.001 row §Decision 9 description updated to payload-only framing and HandoffMissing/HandoffIncomplete boundary. §Risk Mitigations F1-R3 row updated to reflect two distinct WASM-gate no-op conditions and shell-side HandoffMissing attribution. §Traceability ARCH-INDEX provenance leg appended: v2.40 to v2.41 (ADR-026 v1.12 to v1.13 F-P19-002 fix). Comprehensive tree-wide sibling-strand sweep conducted — findings: VP-081 PC-A 'HANDOFF.md absent' label correctly asserts shell-side behavior; BC-4.14.001/BC-5.41.001/BC-INDEX HandoffMissing/HandoffIncomplete attributions all CORRECT; no out-of-ADR edits required. Refs: F-P19-002, BC-4.14.001 EC-011, BC-5.41.001 PC9/EC-014, VP-083 v1.5, E-18, issue-173. [Prior: 2026-06-15 (v1.12) — fix burst (architect): (F-P17-001 MEDIUM) §Decision 2 `precompact_flush_sha` null rule made wave-agnostic for the log-present-and-valid case. Prior rule had a wave-1 unconditional-null carve-out (case 2: 'null when wave_id=1 — permitted unconditionally') that reopened the fabricated/dropped-SHA failure class (DI-021/CAP-032) for wave 1: a hallucinating session could write `precompact_flush_sha: null` at wave-1 close while a genuine flush SHA existed in `precompact-flush-log`. This was asymmetric with wave_id>1 which already HARD-BLOCKed the same scenario. Fix: the three-state uniform null rule now makes the wave_id discriminator survive ONLY for the log-absent case: (1) log genuinely ABSENT — wave_id=1 → null VALID (silent, normal first-wave); wave_id>1 → null ADVISORY only, no hard block; (2) log present but last-line CORRUPT (FIELD-4 absent/empty/≠`commit`) → stale entry → EXEMPT, ANY wave_id (DI-025 arm unchanged); (3) log present with VALID last-line (FIELD-4==`commit`) → null → HARD BLOCK (PrecompactShaMismatch), ANY wave_id INCLUDING wave_id=1. The 'no PreCompact fired before first wave close' unenforced assumption removed from rationale; replaced with explicit statement that mid-wave-1 compaction is a supported Part B scenario. §Decision 2 schema row Anti-fabrication column rewritten; Wave-1 and Genuine-Log-Absence special-cases note rewritten to match three-state rule. WASM gate boundary (BC-4.14.001 pure-parse) is NOT implicated — the null-vs-log check is BC-5.41.001 PC5 shell-side; DI-025 corruption→EXEMPT arm is unchanged. No VP update required (no VP covers the PrecompactShaMismatch null-SHA path by name — routing note provided in fix report). [Prior: v1.11 — F2 adversarial-pass-15 fix burst (architect): (F-P15-002 LOW) §Decision 2 `precompact_flush_sha` Anti-fabrication column de-versioned: load-bearing volatile pin `BC-5.41.001 v1.8 PC2/PC5/EC-006/EC-011` replaced with stable-anchor form `BC-5.41.001 §PC2/PC5/EC-006/EC-011 three-case null rule` per TD-VSDD-091 / POLICY 19. No behavioral change — the three-case logic text is unchanged. [Prior: v1.10 — CV-P14-001 follow-up (architect): §Decision 2 `precompact_flush_sha` schema row and Wave-1 special case note reconciled to BC-5.41.001 v1.8 three-case null-SHA rule. The schema row Anti-fabrication column now encodes the three-case logic: (1) 40-char hex SHA corroborated against precompact-flush-log last-line — VALID; (2) null when wave_id=1 (no PreCompact fired) — permitted; (3) null when wave_id>1 AND precompact-flush-log genuinely absent (verified via `test -f`) — ADVISORY warning, no hard block; (4) null when wave_id>1 AND log EXISTS with valid last-line FIELD-4==commit — HARD BLOCK (PrecompactShaMismatch). Log corruption (FIELD-4 absent/empty/≠commit) → stale entry → prefix-match-alone → EXEMPT (consistent with DI-025/BC-5.41.003/EC-003). Wave-1 note expanded to 'Wave-1 and Genuine-Log-Absence special cases' covering the wave>1 advisory-on-genuine-absence path. §F-P4-004/§Decision A/§Crash-Consistency corruption→EXEMPT and SHA-mismatch→NOT-EXEMPT arms are unchanged. Refs: CV-P14-001, BC-5.41.001 v1.8, E-18. [Prior: 2026-06-14 (v1.9) — F2 adversarial-pass-10 fix burst (architect): (F-P10-001 MAJOR) §Decision 9 wave-1 no-op discriminator corrected to PAYLOAD-ONLY: the validate-wave-handoff-completeness WASM gate computes `is_first_wave = (payload.wave_id == 1)` from the current HANDOFF.md payload being written — NO sprint-state.yaml read, NO factory-artifacts HANDOFF.md read, NO filesystem or git access. Rationale: `wave_id` is Field 1 of the 9 base required fields; the caller (wave-handoff skill) derives it from real substrate (sprint-state.yaml topo-sort ordinal or STATE.md pass number) BEFORE writing HANDOFF.md. The WASM gate reads it back from the Write/Edit tool-call payload. The 'prior HANDOFF.md absence / sprint-state.yaml ordinal' framing was a PROXY for wave-1 that required external reads, violating BC-4.14.001 Invariant 1 (pure-parse). The payload-only `wave_id == 1` discriminator closes the gap: the gate never reads git, filesystem, sprint-state.yaml, or factory-artifacts. The same payload-only principle was established for the EPIC-COMPLETE discriminator by F-P7-001/F-P8-001; F-P10-001 applies it to the sibling wave-1 discriminator. VP-083 `is_first_wave: bool` wording is consistent with this model (`is_first_wave` is computed as `payload.wave_id == 1` by the gate itself). §Decision 9 text amended to document payload-only wave_id==1 discriminator. §F-P10-001 PO Wording section added with exact BC-4.14.001 change spec (rewrite Description, PC3, PC4, PC8, Inv3, EC-001/002/003/005/006/010, test vectors, VP-083/VP-081 row descriptions). (F-P10-002 NOTE) §Decision 6 push-step contract: ADR §Decision 6 step 5 already prescribes `exit 2` on push failure; the 'commit local; push to remote → push failure = exit 2' boundary is now stated explicitly in §F-P10-002 PO Note section to enable PO BC-7.07.001 alignment (BC-7.07.001 PC6/PC8 mislabeling of commit-vs-push is PO-owned). (F-P10-003 MEDIUM) VP-082 append-failure Postcondition E added: VP-082.md v1.3→v1.4 — PC E: 'on precompact-flush-log append failure → if HEAD==SHA_B: git reset --soft SHA_B^ + exit 2; if HEAD≠SHA_B: no reset + exit 2 (human intervention) per §F-P6-006'. VP-INDEX v2.13→v2.14 (VP-082 row description updated, NN-2 POLICY 17 parity). ARCH-INDEX v2.36→v2.37. Refs: F-P10-001, F-P10-002, F-P10-003, issue-173, E-18. [Prior: 2026-06-14 (v1.8) — F2 adversarial-pass-9 fix burst (architect): (F-P9-001 MEDIUM) §F-P5-002 PO Wording PC8 block and §Decision A annotated as SUPERSEDED by §F-P6-006 SHA-pinned form: both `HEAD~1` tokens replaced/annotated — §Decision A live text now prescribes the SHA-pinned `SHA_B^` guard (per §F-P6-006) and labels the HEAD~1 form as historical-only; §F-P5-002 PC8 block given explicit SUPERSEDED-by-§F-P6-006 banner and test-vector note updated to assert `SHA_B^` (not `HEAD~1`) + concurrent-commit path. (F-P9-002 LOW) VP-INDEX VP-082 Full Index row description amended to include append-failure → SHA-pinned-reset (SHA_B^) + concurrent-commit-guard behavior; VP-INDEX v2.12→v2.13. (F-P9-003 MINOR) ARCH-INDEX v2.35→v2.36 ADR-026 v1.7→v1.8 amendment row; §Traceability provenance leg appended (ADR v1.7→v1.8; VP-INDEX v2.12→v2.13). Refs: F-P9-001, F-P9-002, issue-173, E-18. [Prior: 2026-06-14 (v1.7) — F2 adversarial-pass-7 fix burst (architect): (F-P7-001 MAJOR) §F-P6-004 PO Wording BC-4.14.001 change-spec amended: EPIC-COMPLETE discriminator changed from filesystem-read prescription (prior HANDOFF.md on factory-artifacts absent OR non-empty next_wave_stories) to PAYLOAD-ONLY discriminator (current payload next_wave_stories: [] → EPIC-COMPLETE; non-empty → non-EPIC-COMPLETE). BC-4.14.001 Invariant 1 pure-parse constraint satisfied: WASM gate reads only the Write/Edit tool-call payload; no git read, no filesystem read of prior HANDOFF.md. Richer terminal-state judgment (broken-sprint vs genuine final wave) remains in shell-context wave-gate/wave-handoff BC-5.41.002. UnexpectedEpicStatus on non-EPIC payload and MissingEpicStatus on EPIC payload remain coherent with payload-only discriminator. (F-P7-002 MAJOR) §Traceability downstream-index provenance trace completed: VP-INDEX line appended v2.11→v2.12 leg (pass-6 cite-convention migration); ARCH-INDEX line appended v2.33→v2.34 (ADR-026 v1.5→v1.6) and v2.34→v2.35 (ADR-026 v1.6→v1.7 this pass-7 fix). ARCH-INDEX v2.34→v2.35. VP-INDEX unchanged. Refs: F-P7-001, F-P7-002, issue-173, E-18. [Prior: 2026-06-14 (v1.6) — F2 adversarial-pass-6 fix burst (architect): (GOVERNANCE DECISION) BC Traceability ADR cite convention adopted: BC Traceability rows referencing this ADR MUST cite `ADR-026 §Decision N` (stable section anchor) WITHOUT a load-bearing version number. Rationale: the ADR version number is a volatile pin — it increments on every fix-burst while the behavioral content of the cited Decision N is unchanged; carrying it in BCs generates a false-positive finding class each pass (F-P6: BC-5.41.001, BC-5.41.002, BC-5.41.003, BC-6.24.001, BC-7.07.002, BC-1.15.001 all flagged as stale v1.4 while ADR was at v1.5). Under TD-VSDD-091 (anti-volatile-pin), version numbers that decay on every ADR bump are forbidden in load-bearing citations. The stable behavioral anchor is the section heading (e.g., `§Decision 2`, `§Decision 6`), not the version tag. An informational `(as of vX.Y)` parenthetical is PERMITTED only when it is explicitly marked non-load-bearing and is not used as a gate criterion by any validator or adversary check. Uniform PO cite text for all 8 E-18 BCs is specified in §BC Traceability Cite Convention below. (F-P6-006 MAJOR) Concurrent-commit race guard for reset-on-append-failure: Decision 6 step 6 strengthened — the soft reset MUST be pinned to the captured flush SHA, not to a relative `HEAD~1` offset. Root cause: `git reset --soft HEAD~1` assumes HEAD equals the local flush commit (SHA_B); but the PreCompact hook fires between LLM turns, and `factory_lock` is opt-in (absent = no lock). A concurrent state-manager burst could land a commit on factory-artifacts between SHA_B and the attempted reset, advancing HEAD to SHA_C (state-manager commit). `git reset --soft HEAD~1` would then discard SHA_C (silent commit loss, SOUL.md #4 violation — strictly worse than the original orphan problem). Fix: capture SHA_B immediately after `git commit` succeeds; on append failure, reset ONLY if `$(git rev-parse HEAD) == SHA_B`; if HEAD has moved (HEAD != SHA_B), do NOT reset — exit 2 immediately with error message `precompact-flush: append failed after concurrent commit advanced HEAD; SHA_B=<sha>; human intervention required`. Also reconciled the overstated claim that `--soft preserves same staged state across retry`: STATE.md may change between the failed attempt and the retry (the intervening turn may call state-manager), so the retry commits a potentially different tree. The correct statement: `--soft restores the working tree and index to the state they were in when the local commit was created; if STATE.md or other files change between the soft-reset and the retry, the retry commit will include those additional changes`. (F-P6-004 MAJOR) HANDOFF.md schema field-count reconciliation: The schema has 9 base required fields (wave_id, last_verified_develop_sha, active_bcs, next_wave_stories, open_decisions, pending_fixes, process_gaps, precompact_flush_sha, factory_lock_holder). The `epic_status` field is a CONDITIONAL 10th field: required ONLY on the final/EPIC-COMPLETE wave (when `next_wave_stories: []` and all stories are terminal). On non-final waves, `epic_status` must be absent (not `null`, not `complete` — absent). Statements of `all 9 required fields` remain correct for non-final waves. The validate-wave-handoff-completeness WASM gate (BC-4.14.001) must conditionally require `epic_status: complete` ONLY when it detects EPIC-COMPLETE context (no prior pending/draft stories remain). The HANDOFF.md schema table in Decision 2 is amended to add `epic_status` as a conditional field with explicit conditionality rule. BC-4.14.001 and BC-5.41.001 change specs for PO are in §F-P6-004 PO Wording below. [Prior: 2026-06-14 (v1.5) — F2 adversarial-pass-5 fix burst (architect): (F-P5-002 MAJOR) Orphaned un-logged flush commit eliminated via reset-on-append-failure. DECISION: §Decision A in §Crash-Consistency Design now prescribes `git reset --soft HEAD~1` BEFORE `exit 2` when the log append fails. Rationale: without the reset, SHA_B is a real local commit that is never logged (append failed) and never pushed (exit 2 blocked push). On the next retry, SHA_C is committed, logged, and pushed. `validate-burst-log` reads only the LAST LINE of the log (SHA_C); when it sees SHA_B in the HEAD/HEAD^ pair it cannot find SHA_B in the log, and BC-5.41.003 PC1 case (b) fails to grant the exemption → false-positive MULTI_COMMIT_CHAIN_NOT_ALLOWED block. The soft reset restores HEAD to the pre-flush commit, eliminating SHA_B from the local history; the next retry commits SHA_C cleanly. No working-tree changes are lost (--soft). Decision 6 step 6 updated to prescribe: on append failure, reset --soft HEAD~1 then exit 2. (F-P5-002 MAJOR) Stale-entry justification re-grounded. The v1.4 §Crash-Consistency Design (BC-5.41.003 change spec) describes BC-5.41.003 PC1 case (b) `write-before-push crash` as `field-4 ≠ commit`. This justification is mechanically impossible: the shell hook always appends the literal string `commit` (the output of `git cat-file -t <SHA>` on a local commit that already exists) as field-4 at append time; the append succeeds or fails atomically. A partial-write that writes a non-`commit` field-4 would require filesystem-level truncation or corruption — not a `write-before-push crash`. The re-grounded description: field-4 ≠ commit arises ONLY from file corruption or truncation; the fallback (treat as stale, allow bare-prefix match) is still correct but must be attributed to the real cause. BC-5.41.003 PC1 case (b) label changed from `write-before-push crash` to `log corruption or truncation`. §F-P5-002 PO Wording below has exact replacement text for PO. (O-P5-002 MINOR) §Crash-Consistency Design §Decision A subsection heading added: `### Decision A — Append-only side-channel log replaces point-file (F-P2-003; reset-on-append-failure added F-P5-002)`. This provides the stable anchor DI-025 Justification cites as `ADR-026 §Crash-Consistency Design §Decision A`. [Prior: 2026-06-14 (v1.4) — F2 adversarial-pass-4 fix burst (architect): (F-P4-001 MAJOR) Append-failure ↔ exemption incoherence resolved. DECISION: append failure is durability-relevant → BC-7.07.001 PC8 now exits 2 on append failure (not fail-open). Rationale: log append is a local filesystem write that almost never fails; on failure, blocking compaction is correct (the log would be inconsistent, causing a false-positive chain-block on the next burst — the exact regression CAP-032 Part B exists to prevent); fail-open semantics remain ONLY for full hook crashes via on_error=continue. BC-5.41.003 PC1/Inv1 keep the three-gate logic (prefix + last-line SHA match + stale-entry fallback) but the stale-entry case now arises only from push failure (not append failure), so the log is always consistent when compaction proceeds. Decision 6 §step 6 and the §Crash-Consistency Design BC-7.07.001 change spec updated: append failure → exit 2. §VP Allocations note updated: VP-INDEX allocated at v2.08; now at v2.10 (no VP change in this pass). (O-P4-004 MINOR) §Crash-Consistency Design BC-5.41.003 change spec: added explicit statement that WASM corroboration reads the embedded fourth field from the log line and does NOT exec `git cat-file` — the shell hook (which CAN exec git) writes the token at flush time; WASM reads it statically. This is the canonical division: shell execs git, WASM reads the pre-embedded token. PC1/Inv1 replacement wording for PO in §F-P4-004 PO Wording below. (O-P4-001 MINOR) §VP Allocations stale narrative fixed: `VP-INDEX bumped to v2.08` is historical; annotated `(allocated at v2.08; VP-INDEX now at v2.10 — no VP change in this pass)`. [Prior: 2026-06-14 (v1.3) — F2 adversarial-pass-3 COMPLETE-SWEEP fix burst: (F-P3-003 BLOCKER) Decision 2 HANDOFF.md schema `precompact_flush_sha` source column: `last-precompact-flush-sha` → `precompact-flush-log` append-only log last-line SHA field. Decision 6 step 6: redesigned to four-field log line format `<ISO-timestamp> <SHA> <cycle>/<step> <cat-file-type>` with embedded `git cat-file -t` token — enables WASM corroboration without live git exec (O-P3-004 closure). All three TOML `path_allow` blocks corrected: `last-precompact-flush-sha` → `precompact-flush-log` (Decision 6 read_file + write_file + Decision 7 read_file). (F-P3-002 BLOCKER) BC-5.41.003 corroboration spec updated: WASM reads four-field log line, checks SHA (field 2) + embedded `commit` token (field 4) — no live `git cat-file` exec from WASM. (F-P3-001 BLOCKER adjudicated) Terminal-Wave Discriminator expanded: terminal set = `{merged, withdrawn, cancelled}`. `cancelled` added as legitimate terminal state with rationale. BC-5.41.002 v1.2 is already correctly aligned (no PO action needed). ADR-026 now authoritative. (O-P3-001 closure) Pruning ownership attached: S-18.04 MUST include explicit pruning AC for `precompact-flush-log` (no dangling sentence). (O-P3-004 closure) WASM capability constraint documented: `validate-burst-log` and `validate-dispatch-advance` are WASM with no `exec_subprocess`; four-field log format resolves. BC change spec for BC-7.07.001, BC-5.41.003, BC-5.41.001 updated with four-field format. [Prior: 2026-06-14 (v1.2) — F2 adversarial-pass-2 revision: (F-P2-001) Canonical wave-identity derivation expression authored as a single normative §Wave-Identity Derivation section (all BCs/VPs/DIs cite verbatim; eliminates drift between partial re-anchors). (F-P2-002) VP inputs: path corrected from shorthand `ADR-026.md` to real slug `.factory/specs/architecture/decisions/ADR-026-wave-boundary-checkpoint-reset-and-lossless-intra-wave-compaction.md` across VP-081..VP-086 (TASK 3). VP-081/082/083/085 swept: all `current_wave` references in Preconditions, Invariants, and test-harness fixtures replaced with canonical derivation substrate (see §Wave-Identity Derivation); VP files bumped to v1.2. (F-P2-003) Crash-consistency design for side-channel file: Decision 6 redesigned with append-only side-channel log (`.factory/hooks/precompact-flush-log`) — entries survive hook crash; BC-5.41.003 EC-003 must corroborate against the log, not a single-SHA point file; BC-7.07.001 side-channel write step redesigned as append-only. Exact BC change specs in §Crash-Consistency Design below. (F-P2-004) Terminal-wave discriminator added to Decision 2 and Decision 4: empty `next_wave_stories` AND all stories `merged` or `withdrawn` → EPIC-COMPLETE success path; empty `next_wave_stories` AND stories in other states → hard error (broken sprint-state). BC-5.41.002 EC-001 and BC-6.24.001 must be updated per §Terminal-Wave Discriminator below. [Prior: 2026-06-14 (v1.1) — F2 adversarial-pass-1 revision: (F-1) Re-anchored wave-identity to real substrate; (F-2) Reconciled with ADR-025 opt-in lock model; (F-3) next_wave_stories real derivation specified; (F-4) Bounded timeout_ms=30000; (F-5) Harness-version runtime assertion; (F-6) PostCompact re-anchor best-effort; (F-7) TOML registration corrected; (F-9) SS-08/S-18.06/S-18.07 scope; (F-10) VP-086 allocated; (F-11) 83% clamp MEDIUM-confidence; (F-15) Prerequisite-verification table. [Prior: 2026-06-14 (v1.0) — Initial ADR.]]]]]"
---
...
| BC-4.14.001 | `ADR-026 v1.5 Decision 8 ..., Decision 9 ...` | `ADR-026 §Decision 8 (WASM for completeness gate; deterministic parse-heavy validation; shell for flush; HANDOFF.md MUST be written via the Claude Code Write tool as part of an agent-orchestrated emit→Write→commit flow — bash redirection emits no PostToolUse event and is forbidden; the skill is restructured to bash subcommands with SKILL.md as genuine numbered-step agent orchestrator; emit-handoff computes+emits payload to stdout; agent Write tool writes to disk and triggers PostToolUse gate; commit subcommand has TWO arms: HAS-NEXT-WAVE arm stages BOTH HANDOFF.md+wave-state.yaml in ONE atomic commit (hard-abort if EITHER absent); EPIC-COMPLETE arm stages HANDOFF.md ALONE in ONE atomic commit (removes any stale wave-state.yaml; hard-abort only on missing HANDOFF.md) — a single-file EPIC-COMPLETE commit is still ONE atomic commit, not a PC6 violation (PC6 forbids two separate commits); no silent fallback to bash redirection permitted per F-S1802-02/S-18.13; F-SP13-P5-001), §Decision 9 (no-op on wave-1 / non-HANDOFF.md writes; wave-1 discriminator PAYLOAD-ONLY: payload.wave_id==1; no sprint-state.yaml read, no factory-artifacts probe, no git access; wave_id absent → fail-closed HandoffIncomplete; non-HANDOFF.md write target → no-op; note: HandoffMissing is SHELL wave-gate code in BC-5.41.001 PC9, DISJOINT from this WASM gate which cannot detect file-absence)` |
| BC-5.41.001 | `ADR-026 v1.4 Decision 2 ..., Decision 9 ..., Decision 1` | `ADR-026 §Decision 2 (HANDOFF.md schema + anti-fabrication cross-checks; wave_id from real substrate; factory_lock_holder nullable; precompact_flush_sha hard cross-check against precompact-flush-log (append-only); 9 base required fields + epic_status conditional on EPIC-COMPLETE wave per v1.6), §Decision 9 (wave-1 no-op), §Decision 1 (wave-boundary reset is primary mechanism)` |
...
|-------|------|------------------------|----------------------|
| `wave_id` | integer | Product pipelines: wave GROUP ordinal derived by `wave-handoff` from sprint-state.yaml dependency order. The ordinal is the count of completed contiguous-terminal wave groups in sprint-state.yaml + 1. A "wave group" is a maximal contiguous block of terminal stories (merged/withdrawn/cancelled) that precedes the first non-terminal entry. Under P-WAVE-BARRIER-INVARIANT, one contiguous terminal block = one completed wave; therefore wave_id = completed_wave_groups + 1, NOT leading_terminal_story_count + 1. For vsdd-factory's real sprint-state.yaml (103 merged stories in one contiguous block followed by 51 non-terminal stories): wave_id = 2 (one completed wave group). Engine self-referential: pass number from STATE.md `current_step:` | Cross-checked: `wave_id` in HANDOFF.md must match the value wave-handoff skill computed from the real sprint-state.yaml or STATE.md — not from any phantom `current_wave:` frontmatter field. The `parse-sprint-state.sh` `derive_wave_id` function (counting terminal→non-terminal TRANSITIONS, not raw story count) is the canonical implementation of this wave-group ordinal. |
...
### Decision 3a — Canonical sprint-state.yaml ordering rule: two-partition (terminal-first, full-graph wave-depth+lex within each partition)

**Decision (S-18.11 pass-6 F-P6-002 ordering-contract adjudication; amended v1.37 — human directive 'full rigor, option b'):** The canonical ordering rule for the `stories:` list in `sprint-state.yaml` is a **two-partition order**:

1. **Partition A — terminal stories** (status: `merged`, `withdrawn`, `cancelled`): ALL terminal stories appear first as a single contiguous block, ordered by **(full-graph wave-depth ASC, story-ID lex ASC)**. See wave-depth definition below. The contiguous terminal block satisfies the `P-SPRINT-STATE-WAVE-ORDER` precondition and the `WaveOrderUnverifiable` guard in `parse-sprint-state.sh` `derive_wave_id` — the guard requires only that no terminal entry appears after the first non-terminal entry.

2. **Partition B — non-terminal stories** (status: `draft`, `ready`, `in-progress`, `partial`, `blocked`): ALL non-terminal stories appear after the terminal block, ordered by **(full-graph wave-depth ASC, story-ID lex ASC)** — same sort key, same wave-depth definition.

**Wave-depth definition (definition b — full graph, v1.37):**

> The **full-graph wave-depth** of story S is the length of the longest path from S to any root in the FULL `depends_on` graph (all edges, across all story statuses, including cross-partition supersession edges):
>
> - Root stories (no `depends_on` entries, or empty `depends_on: []`) have depth **1**.
> - All other stories: `depth(S) = 1 + max(depth(P) for all P in S.depends_on)`.
>
> This is a global graph property computed once over the full dependency graph, independent of which partition S is placed in. The depth computation includes cross-partition edges (e.g., a terminal story that `depends_on` a superseded non-terminal story contributes the non-terminal story's depth to the terminal story's own depth).

**Why full-graph depth (not partition-restricted), and why two-partition (not strict global sort):**

The strict global wave-ascending topo-sort is incompatible with vsdd-factory's real project state: this project has non-monotonic completion (early epics with low-depth `draft` stories, later epics with high-depth `merged` stories), which would interleave terminal and non-terminal stories and trigger `WaveOrderUnverifiable`. The two-partition rule resolves the incompatibility:

- It satisfies `WaveOrderUnverifiable` (all terminal entries precede all non-terminal entries — no post-boundary terminal).
- It gives deterministic intra-partition ordering (full-graph wave-depth ASC + lex within each partition).
- It remains consistent with the `P-SPRINT-STATE-WAVE-ORDER` precondition: the derive_wave_id algorithm counts terminal→non-terminal transitions, and with two-partition there is exactly one such transition boundary.
- The `wave_id` derivation is unaffected: one contiguous terminal block followed by non-terminal stories = one completed wave group = `wave_id = 2`.

Full-graph depth (definition b) rather than partition-restricted depth (definition a) is chosen because (per human directive 2026-06-29 'full rigor, option b') a story's position in a partition should reflect its TRUE dependency depth in the overall project graph — including the contributions of superseded predecessors — not an artificially truncated depth that ignores cross-partition edges. This produces a semantically meaningful ordering within each partition.

**Coherence with partition placement + supersession edge tolerance:**

Partition placement is UNCHANGED. A terminal story (Partition A) may have a `depends_on` edge pointing to a superseded non-terminal story (Partition B) — this is TOLERATED by EC-010 (see below). Under definition (b), this cross-partition supersession edge IS counted toward the terminal story's full-graph depth, even though the dep is placed later in the file. The result is coherent and deterministic:

- Depth is a global graph property computed independently of file placement.
- Placement is determined by partition membership (terminal/non-terminal), then within each partition by (full-graph-depth ASC, lex ASC).
- The cross-partition supersession edge moves the terminal story's depth up (e.g., S-3.01 depends on S-3.04 partial + S-2.08; S-3.04 depth=4, S-2.08 depth=9, so S-3.01 depth=10, not 9 under partition-restricted). The terminal story appears later in Partition A than it would under definition (a), but still before ALL non-terminal stories (Partition B boundary is unchanged).
- No consumer reads `depends_on` edges from sprint-state.yaml; consumers read only `status:` values and list position.

**Topo-validity with supersession edges (S-18.11 EC-010 re-adjudication — human directive 2026-06-29):** The vsdd-factory STORY-INDEX.md graph contains terminal→non-terminal `depends_on` edges: S-3.01 (merged), S-3.02 (merged), S-3.03 (merged), S-4.07 (merged), and S-4.08 (merged) all reference S-3.04 (status: `partial`). These are LEGITIMATE SUPERSESSION EDGES: S-3.04 carries `superseded_by: ADR-015` frontmatter indicating that its unfinished Router-wiring work was retired by ADR-015 (single-stream OTel-aligned emission), while its shipped portion (AC-001 host fn) was consumed by the dependent stories. These edges are INCLUDED in full-graph depth computation and TOLERATED by the TopoViolation guard.

**TopoViolation guard — narrowed definition (BC-5.41.004 EC-010; UNCHANGED from v1.36):** The producer hard-aborts only on GENUINE anomalies — a terminal story that `depends_on` a non-terminal story that does NOT carry `superseded_by:` frontmatter. Supersession edges are tolerated and emitted normally. The depth computation NOW includes supersession edges; the guard detection logic (check for `superseded_by:` before deciding tolerate-or-abort) is unchanged. Only the depth-computation basis changed from restricted to full-graph.

**Superseded dependency — cross-partition placement note:** When a terminal story in Partition A `depends_on` a superseded non-terminal story in Partition B, the terminal story appears before its superseded non-terminal dependency in the output file (Partition A precedes Partition B). This is expected and correct: the partition boundary is determined by terminal/non-terminal status, not by dependency direction.

**Human directive — S-3.04 / ADR-015 forward-reference:** Per human directive (2026-06-29), S-3.04 intentionally remains `status: partial` with `superseded_by: ADR-015`. It is NOT reclassified to any terminal status. The ADR-015 revisit (Router/multi-sink revival) is a SEPARATE effort deferred to AFTER E-18 convergence.

**Sprint-state.yaml conformance (v1.37 assessment — resolved):** The pre-def-b `.factory/stories/sprint-state.yaml` Partition A did not conform to definition (b); a mechanical spot-check confirmed 10 monotonicity violations (e.g., S-9.00 full-graph depth=5 placed before S-9.30 depth=1 and S-0.02 depth=2; S-18.06/S-18.07/S-18.13 depth=1 placed inside depth-4/5/6 blocks). The implementer re-sorted Partition A and Partition B to (full-graph-depth ASC, story-ID lex ASC) def-b order (S-18.11 T-4). Conformance is independently verified by bats RG-9 (`test_partitions_sorted_by_full_graph_depth_def_b`), which computes full-graph depths from STORY-INDEX and asserts both partitions are (depth ASC, lex ASC) with done-first contiguity; 14/14 green, `derive_wave_id=2`. This is a closed action.

**What does NOT change:**
- `derive_wave_id` implementation in `parse-sprint-state.sh` — no code change required.
- `WaveOrderUnverifiable` guard in `parse-sprint-state.sh` — no code change required; guard fires only on post-boundary terminal entries.
- `wave_id = 2` for vsdd-factory's current sprint-state.yaml — confirmed correct under two-partition.
- BC-5.41.001 PC2, P-SPRINT-STATE-WAVE-ORDER, WaveOrderUnverifiable guard — no amendment needed; terminal-contiguity requirement is satisfied by Partition A.
- TopoViolation guard detection logic (BC-5.41.004 EC-010 — check `superseded_by:` before tolerate/abort) — unchanged.

**What changes (v1.37):**
- BC-5.41.004 PC3: amend intra-partition ordering to full-graph wave-depth (definition b) as defined above. Product-owner owns.
- wave-scheduling SKILL.md Step 5: update ordering algorithm from partition-restricted topo-sort to full-graph depth + (depth, lex) sort key. Implementer owns (T-5).
- sprint-state.yaml (T-4 re-sort): re-sorted Partition A and Partition B to (full-graph-depth ASC, lex ASC) def-b order — 10 violations resolved. COMPLETED (S-18.11 T-4); verified by bats RG-9 14/14 green.

**Refs:** S-18.11 pass-6 F-P6-002, S-18.11 v1.37 human directive 'full rigor option b' (2026-06-29), S-18.11 EC-010 supersession-edge narrowing (human directive 2026-06-29), BC-5.41.004 PC3/EC-010, BC-5.41.001 PC2/P-SPRINT-STATE-WAVE-ORDER, `parse-sprint-state.sh` `WaveOrderUnverifiable`, S-3.04, ADR-015, E-18, issue-173.
...
4. Commits to `factory-artifacts` with message: `PreCompact flush <cycle>/<step> <timestamp>`. The git commit is a LOCAL operation (no network). Immediately after commit: captures `SHA_B=$(git -C <factory-artifacts-worktree> rev-parse HEAD)` — this capture MUST happen at this step before step 5 (append), so SHA_B is available if the append fails. **Exit 2 on commit failure (local failure — lock contention, permission error, etc.)**: hook exits 2 (block compaction); no append and no push attempted.
5. Appends `<ISO-timestamp> <SHA_B> <cycle>/<step> commit` to the append-only log (`.factory/hooks/precompact-flush-log`) BEFORE the `git push` step. This write is local filesystem and precedes the push to establish crash-consistent ordering. The fourth field (`commit`) is the object-type token embedded by the shell hook at write time — the shell embeds this token by executing `git cat-file -t <SHA_B>` (value: `commit`), or equivalently by embedding the literal string `commit` directly, since the type is definitionally `commit` for a just-created commit object; both approaches yield the same field-4 value. WASM corroboration validators (`validate-burst-log`, `validate-dispatch-advance`) read this pre-embedded token statically via `host::read_file` — they do NOT exec `git cat-file` (WASM has no `exec_subprocess`). **Append failure: SHA-pinned reset-then-exit-2 (F-P5-002 + F-P6-006).** If the append to `precompact-flush-log` fails, the hook MUST: (a) compare CURRENT_HEAD = `git -C <worktree> rev-parse HEAD` to SHA_B (captured at step 4); (b) if CURRENT_HEAD == SHA_B: execute `git -C <worktree> reset --soft SHA_B^` (SHA-PINNED, not `HEAD~1`); then exit 2; (c) if CURRENT_HEAD != SHA_B: a concurrent commit landed — do NOT reset; exit 2 immediately with error message `precompact-flush: append failed; concurrent commit advanced HEAD; SHA_B=<sha_b>; human intervention required`; (d) if the reset itself fails: exit 2; human intervention required. Rationale for SHA-pinned form: `HEAD~1` is a relative reference that resolves to whatever HEAD~1 is at reset time, which is the concurrent commit SHA_C if a state-manager burst landed between the flush commit and the reset — discarding SHA_C silently (SOUL.md #4). The SHA-pinned `SHA_B^` always resolves to the parent of the KNOWN flush commit, which is the pre-flush HEAD regardless of any intervening commits. See §F-P6-006 Concurrent-Commit Race Guard for full analysis. The `on_error = "continue"` fail-open at the harness level applies ONLY to full hook crashes (before any exit code is emitted) — not to deliberate exit 2 signals.
6. Executes `git push` to synchronize the local commit (SHA_B) to the remote `factory-artifacts` branch (NETWORK). **Exit 2 on push failure (network error, remote rejection, authentication failure):** hook exits 2 (block compaction). At this point the local commit and log entry are already intact — NOT rolled back. Retry is push-only (no re-commit, no re-append). **Exit 0 on success:** flush landed; hook exits 0 (allow compaction to proceed). Exits 0 also when no state changes exist since last flush (no-op path — commit, append, and push are all skipped).
...
**Re-anchor block SHA source — canonical (F-P1-001 / v1.25):**

The re-anchor block format is:
...
Where:
- `<current_cycle>` and `<current_step>` are read from `git show factory-artifacts:.factory/STATE.md` frontmatter fields.
- `<develop_sha>` is sourced from `git rev-parse origin/develop` (or `refs/remotes/origin/develop`) executed at hook invocation time — NOT from any STATE.md field.

**Rationale (F-P1-001):** `last_verified_develop_sha:` is a HANDOFF.md field (Decision 2 schema), NOT a STATE.md frontmatter field. STATE.md frontmatter contains only `current_step:`, `current_cycle:`, `phase:`, and document-metadata fields. Prescribing the hook to read `last_verified_develop_sha:` from `git show factory-artifacts:.factory/STATE.md` causes `sha=UNKNOWN` on every real PostCompact firing — the same phantom-field anchoring defect class that §Wave-Identity Derivation forbids for `current_wave`. The correct source is `git rev-parse origin/develop` at hook time: it is the live authoritative develop HEAD, requires no STATE.md schema change, and is already within the hook's read-only git-subprocess capability profile. On git failure (e.g., network unavailable), the hook falls through to `sha=UNKNOWN` with `status=warn` (DI-024 advisory; exit 0 always). This is not a guaranteeable field — the hook is advisory-only and not in the CAP-032 chain.

**Hook registration spec (corrected TOML schema per F-7; env_allow + capabilities corrected v1.25):**
...
**env_allow rationale (F-P1-003 / v1.25):** `GIT_CONFIG_GLOBAL` and `XDG_CONFIG_HOME` are required by the git subprocess to locate its own config (same entries as Decision 6 §precompact-flush). `GIT_DIR` is NOT in the canonical env_allow — it was included by the S-18.05 implementer to support the bats test fixture (which passes a bare repo path via `GIT_DIR` env injection), a test artifact only. Production git does not require `GIT_DIR` to be set explicitly; omitting it from env_allow is correct. If a specific bats fixture requires `GIT_DIR`, the fixture must use a different mechanism (e.g., `-C <dir>` flag or `--git-dir` git argument) rather than relying on a production env_allow entry.

**write_file vs read_file rationale (F-P1-003 / v1.25):** The hook reads STATE.md from `factory-artifacts` exclusively via `git show factory-artifacts:.factory/STATE.md` — this is a git subprocess invocation covered by `exec_subprocess`, NOT by `read_file`. No direct `read_file` capability is needed for the STATE.md read. The only direct filesystem write is the JSONL log file at `.factory/logs/postcompact-reanchor-YYYY-MM-DD.jsonl` — this requires `write_file` capability. Prior versions incorrectly listed `read_file` for `.factory/STATE.md` and `.factory/hooks/precompact-flush-log` (the former is git-mediated; the latter is not used by this hook at all). Corrected to `write_file` with `path_allow = [".factory/logs/"]`.
...
**Generator retirement — canonical TOML is hooks-registry.toml, not generator output (F-P1-004 / v1.25; corrected v1.26):** `plugins/vsdd-factory/hooks-registry.toml` is the **human-edited source of truth** as declared in its own file header ("The v0.79.x→v1.0 migration generator has been retired; edit this file directly"). This is non-negotiable. The S-18.05 implementer also added a PostCompact entry to `scripts/generate-registry-from-hooks-json.sh` (added `PostCompact)` case to `band_for()`) and `scripts/legacy/hooks-json-pre-templating.json` (added a PostCompact block). Those edits are a drift hazard — the generator is retired and running it would produce a DIFFERENT registry entry than the checked-in canonical one (different env_allow, no `async` field, missing capabilities block). The latent regeneration-drift foot-gun must be eliminated: **the S-18.05 implementer MUST revert the `band_for()` PostCompact case and the legacy-JSON block**; then register `postcompact-reanchor` in the generator's `POST_HISTORICAL_SCRIPTS` array and in `generate-registry.bats` `skip_names` — mirroring the existing `check-harness-version` precedent — so the generator's orphan-check stays green WITHOUT emitting a regenerated stanza. The `band_for()`/legacy-JSON edits are the drift hazard and are reverted; `POST_HISTORICAL_SCRIPTS` emits nothing, so no drift is introduced. The PostCompact entry exists only in `hooks-registry.toml` as the canonical single source of truth. There is no generator-to-registry parity lint at present; a follow-up story should add a CI check asserting that `generate-registry-from-hooks-json.sh --dry-run` (if retained at all) is not executable or is explicitly marked read-only. [Process gap recommendation: register as follow-up work anchored to S-18.07 or a new S-18.15 story — "generator marked no-op or deleted after retirement declaration"; not deferred without an anchor.]
...
**Why bash redirection cannot be used — gate-event analysis (F-S1802-02):** The `validate-wave-handoff-completeness` WASM gate is registered as a PostToolUse handler on `Write` and `Edit` tool events. It fires ONLY when the Claude Code harness emits a PostToolUse event for one of those tool calls. Bash redirection (`} > "$output_path"` or `echo ... > path` in a `Bash` tool call) does NOT emit a Write or Edit PostToolUse event — it emits a `Bash` PostToolUse event for which the completeness gate is NOT registered. This means any HANDOFF.md written via bash redirection bypasses the BC-4.14.001 completeness gate entirely, rendering it functionally inert in production. The v1.22 normative rule ('the `write-handoff` skill component MUST route its HANDOFF.md output through the Write tool') was architecturally correct in intent but was written without verifying the execution model: `wave-handoff.sh` is a **monolithic bash run** ending in `main "$@"`, with Steps 4–6 in `main()` being bash function calls. There is no agent seam inside a monolithic bash run where a Write tool call can be interleaved. Making the PostToolUse gate fire requires restructuring the skill's execution model.

**Normative control flow — agent-orchestrated emit→Write→commit (S-18.13; v1.23):** The `wave-handoff` skill MUST be restructured from a monolithic bash run into an agent-orchestrated flow. The mechanism is a bash subcommand split with the agent as the coordinator between the emit phase and the commit phase. The canonical four-step flow is:

1. **Bash 'emit-handoff' subcommand** (`wave-handoff.sh --emit-handoff [args]`): Computes all HANDOFF.md fields with the full anti-fabrication cross-check suite (same logic as current `write_handoff()` in `lib/write-handoff.sh`). Emits the assembled YAML payload to **stdout**. Does NOT write any file to disk. Does NOT commit. Hard-errors on any anti-fabrication failure exactly as the current `write_handoff()` does; on error, exits non-zero with a human-readable error message — no partial payload is emitted.

2. **Agent Write-tool invocation** (SKILL.md numbered agent step): The agent captures the stdout payload from step 1 and invokes the Claude Code **Write tool** with the assembled HANDOFF.md content, targeting `${ARTIFACTS_WT}/HANDOFF.md`. The PostToolUse `validate-wave-handoff-completeness` gate fires here. If the gate blocks (HandoffIncomplete), the agent surfaces the gate error and the wave-close is aborted — HANDOFF.md is on disk but not committed. If the gate returns Continue, the flow proceeds to step 3.

3. **Bash 'emit-wave-state' subcommand** (`wave-handoff.sh --emit-wave-state [args]`, or existing `write_wave_state()` logic invoked separately): Produces `wave-state.yaml` and writes it to `${ARTIFACTS_WT}/wave-state.yaml` via bash. This step is a bash-only disk write; it does NOT need to emit a PostToolUse event. The PostToolUse gate only applies to HANDOFF.md (BC-4.14.001 EC-011). This step is skipped on EPIC-COMPLETE paths (no wave-state.yaml on final wave).

4. **Bash 'commit' subcommand** (`wave-handoff.sh --commit [args]`): Creates a **single atomic git commit** via `commit_to_artifacts`. The subcommand has two distinct arms based on EPIC-COMPLETE context (F-SP13-P5-001):
   - **HAS-NEXT-WAVE arm** (non-EPIC-COMPLETE; `next_wave_stories` non-empty): Stages HANDOFF.md (on disk from the Write tool call in step 2) AND wave-state.yaml (on disk from step 3) and commits both atomically in ONE git commit. Hard-aborts if EITHER file is absent from the working tree before staging — missing HANDOFF.md OR missing wave-state.yaml both cause a non-zero exit with an explicit error message.
   - **EPIC-COMPLETE arm** (`epic_status: complete` / `next_wave_stories: []`): Stages HANDOFF.md ALONE (wave-state.yaml was never written — step 3 was skipped on this path). Removes any stale wave-state.yaml from the working tree if present (consistent with current `wave-handoff.sh` behavior for EPIC-COMPLETE close). Hard-aborts ONLY on missing HANDOFF.md. Creates ONE git commit containing HANDOFF.md only.
   This preserves **BC-5.41.002 PC6 atomicity** on both paths: each arm creates exactly ONE git commit regardless of how many files it stages. A single-file EPIC-COMPLETE commit is still ONE atomic commit — it is not a PC6 violation. PC6 forbids two SEPARATE commits (e.g., committing HANDOFF.md alone then wave-state.yaml alone in a second commit); a commit that stages one file because only one file exists on the EPIC-COMPLETE path is the correct single-commit behavior. On the HAS-NEXT-WAVE path, both files land in one commit as before.

**SKILL.md restructuring — genuinely agent-orchestrated numbered steps:** `SKILL.md` MUST be rewritten from its current descriptive "Behavior Overview" list (which describes what the monolithic bash run does) to **normative numbered agent steps** that the agent executes in sequence. The agent is the coordinator that invokes the bash subcommands and the Write tool in the correct order. The skill's `allowed-tools` already includes `Write, Bash`; the restructuring does not change the permission list, only the behavioral prescription.

**Canonical SKILL.md step structure (normative):**
```
Step 1: Parse sprint-state.yaml and classify stories (Bash: wave-handoff.sh --parse-sprint-state)
Step 2: Emit HANDOFF.md payload to stdout (Bash: wave-handoff.sh --emit-handoff)
Step 3: Write HANDOFF.md to factory-artifacts (Write tool: content = stdout from Step 2)
         ← PostToolUse validate-wave-handoff-completeness gate fires here
Step 4: Write wave-state.yaml to factory-artifacts (Bash: wave-handoff.sh --emit-wave-state)
         [skipped on EPIC-COMPLETE path]
Step 5: Commit atomically (Bash: wave-handoff.sh --commit)
         ← single git commit; BC-5.41.002 PC6 atomicity
         HAS-NEXT-WAVE path:   stages BOTH HANDOFF.md + wave-state.yaml;
                                hard-aborts if EITHER is absent
         EPIC-COMPLETE path:   stages HANDOFF.md ALONE (removes stale wave-state.yaml
                                if present); hard-aborts ONLY on missing HANDOFF.md
                                [parallel to "[skipped on EPIC-COMPLETE path]" on Step 4]
         NOTE: a single-file EPIC-COMPLETE commit is still ONE atomic commit —
               not a PC6 violation (F-SP13-P5-001)
```

**Failure and partial-write edge cases:**
- **Session dies after Write tool (step 3) but before wave-state.yaml write (step 4):** HANDOFF.md is on disk on factory-artifacts but not committed. On resume, the agent re-runs from the SKILL.md step sequence. The `--emit-handoff` recomputes the payload; the Write tool overwrites the partial HANDOFF.md; the gate re-fires. The `--commit` subcommand's EC-015 idempotent guard (existing, in `commit_to_artifacts`) handles the case where HANDOFF.md content is unchanged. No silent inconsistency.
- **Session dies after wave-state.yaml write (step 4) but before commit (step 5):** Both files are on disk. On resume, `--commit` stages both and commits atomically. EC-015 guard handles byte-identical re-run. No silent inconsistency.
- **Write tool unavailable at step 3:** The agent MUST surface `HandoffWriteToolUnavailable` and abort. No bash-redirect fallback is permitted. Wave-close does NOT proceed. The `--emit-handoff` subcommand completes (payload was emitted to stdout) but the Write tool call fails; the agent logs the failure and surfaces the hard error. See S-18.13 EC-016.
- **Gate blocks at step 3 (HandoffIncomplete):** Agent surfaces gate error. HANDOFF.md is on disk but uncommitted; wave-close is aborted. Operator corrects and re-invokes from Step 2.

**No silent fallback permitted (unchanged from v1.22):** The agent MUST NOT fall back to bash redirection (`echo ... >`, `tee`, `>` redirect) if the Write tool is unavailable or fails. This is a SOUL.md §4 silent-failure prohibition. See S-18.13 EC-016 and BC-5.41.001 EC-016.

**Atomicity-preservation argument (BC-5.41.002 PC6):** BC-5.41.002 PC6 requires that the committed wave-close artifacts land in a SINGLE git commit, not committed separately. The agent-orchestrated flow satisfies this on both paths: on the HAS-NEXT-WAVE path, HANDOFF.md is written by the Write tool and wave-state.yaml is written by bash; the `--commit` subcommand stages BOTH files and creates ONE commit. On the EPIC-COMPLETE path, only HANDOFF.md is written (step 4 is skipped); the `--commit` subcommand stages HANDOFF.md ALONE and creates ONE commit — a single-file commit is still a single commit and satisfies PC6. PC6's prohibition is on committing the files in TWO SEPARATE commits (one for HANDOFF.md, then a second for wave-state.yaml); it is not a prohibition on committing fewer files when the EPIC-COMPLETE design legitimately omits wave-state.yaml. The two writes arriving at different times (steps 3 and 4) do not create two commits — only step 5 creates a commit, regardless of how many files are staged. PC6 atomicity holds on both paths (F-SP13-P5-001).

**Rationale:** The v1.22 amendment correctly identified the gate-bypass failure class (F-S1802-02) but prescribed a solution ('write-handoff.sh MUST route output through the Write tool') that is impossible for a monolithic bash run. The v1.23 amendment specifies the **structurally implementable mechanism**: split the bash run into subcommands, make SKILL.md the genuine agent-orchestration layer, and have the agent invoke the Write tool between the emit and commit phases. This is the only design that (a) makes the PostToolUse gate fire on the production path, (b) preserves BC-5.41.002 PC6 single-commit atomicity, (c) handles partial-write failure modes without silent inconsistency, and (d) fails loudly when the Write tool is unavailable. Every prior mechanism that kept the bash run monolithic is structurally incapable of satisfying all four constraints simultaneously.

---

### Decision 9 — validate-wave-handoff-completeness: no-op on wave-1 / non-HANDOFF.md writes

**Decision:** The `validate-wave-handoff-completeness` WASM gate fires as a PostToolUse gate on Write/Edit operations that produce `HANDOFF.md` on `factory-artifacts`. Its evaluation order and blocking behavior is:

**Canonical evaluation order (PAYLOAD-ONLY; all checks are pure-parse — no filesystem or git access):**

1. **Non-HANDOFF.md write target → no-op immediately:** When the tool call does not target `HANDOFF.md` (per BC-4.14.001 EC-011), return `Continue` unconditionally. The gate validates a Write/Edit payload; it cannot probe for file presence or absence on factory-artifacts.

2. **EPIC-COMPLETE detection BEFORE wave_id==1 no-op (F-P25-002):** The gate evaluates `next_wave_stories: []` (EPIC-COMPLETE discriminator) as the SECOND check — BEFORE the wave_id==1 no-op. Rationale: single-wave epics CAN occur in this engine; an epic whose only wave is wave 1 may reach terminal status (all stories merged/withdrawn/cancelled) without proceeding to wave 2. On a single-wave EPIC-COMPLETE write (`wave_id=1`, `next_wave_stories:[]`, `epic_status:complete`), skipping the EPIC-COMPLETE branch means `epic_status` is never validated — the completeness gate's purpose (preventing a partial/incomplete handoff from being committed) becomes a no-op on the single-wave-final case. The EPIC-COMPLETE check is PAYLOAD-ONLY (`next_wave_stories: []` from the current payload) and reads no filesystem or git resource — it is sound to apply it at wave_id==1.
   - **If EPIC-COMPLETE (`next_wave_stories: []`):** Validate `epic_status: complete` per PC2a, regardless of `wave_id`. This applies even when `wave_id == 1`. If `epic_status` is absent → block `MissingEpicStatus`; if present but not `complete` → block `HandoffIncomplete: epic_status malformed`. If `epic_status` is valid, continue to full 9-base-field validation.
   - **If NOT EPIC-COMPLETE (`next_wave_stories` is non-empty):** Proceed to step 3.

3. **Payload `wave_id == 1` AND NOT EPIC-COMPLETE → no-op:** Return `Continue` unconditionally without performing field validation. This prevents friction on the first wave where no prior handoff exists AND no EPIC-COMPLETE closure is being asserted.

4. **Payload `wave_id > 1` (not first wave, not EPIC-COMPLETE at step 2):** Validate all 9 base required fields are present and syntactically correct. Block with `HandoffIncomplete` if any required field is missing or malformed.

5. **`wave_id` ABSENT from payload → fail-closed:** The gate does NOT treat absent `wave_id` as wave-1. It proceeds to full validation, blocking with `HandoffIncomplete: ["wave_id"]` (and any other missing required fields).
...
**EPIC-COMPLETE evaluation at wave_id==1 (F-P25-002):** The wave_id==1 no-op exists to prevent false-positive blocking on normal first-wave writes (no prior handoff to validate). This rationale does NOT apply when the first-wave HANDOFF.md is also the EPIC-COMPLETE closure: in that case, the operator IS asserting a closure condition (`epic_status: complete`) and the gate's purpose — ensuring the closure assertion is well-formed — applies equally regardless of `wave_id`. `epic_status` validation is about the CURRENT closure's completeness; it is independent of any prior handoff. Applying it at wave_id==1 is sound and preserves pure-parse purity (EPIC-COMPLETE detection is PAYLOAD-ONLY from `next_wave_stories: []`).

**Rationale:** This closes F1 regression risk §4.1 R3 — the gate must not add friction for short single-wave runs or for users who have not yet produced a HANDOFF.md. The gate only activates the no-op on wave_id==1 when NOT in EPIC-COMPLETE context. Single-wave EPIC-COMPLETE writes are validated for `epic_status` as intended by PC2a. The payload-only approach closes F-P10-001 (no WASM filesystem reads) and F-P25-002 (single-wave EPIC-COMPLETE gap).
...
**BC-4.15.001 required (amended F3 OQ-4):** The F2 text stated "No BC required for v1 advisory." This clause is REPLACED. The advisory output IS a contractable postcondition — the gate's behavioral obligations are precisely specified: it MUST always return Continue (Invariant 2), MUST emit a dual-channel DelegationRecommended advisory (stderr B-1 + plugin.log B-2) on first pattern match (PC-B), MUST emit no advisory on no-match (PC-A), and MUST be a no-op on non-Bash tool calls (PC-D). These are enforceable postconditions suitable for a BC. Product-owner has authored BC-4.15.001 (v1.0, 2026-06-16) covering all four behavioral obligations. S-18.06 story is correctly scoped as advisory-only (never blocks in v1). The architectural judgment — advisory mode in v1, blocking-promotion deferred to F3 adversarial calibration — is UNCHANGED. Only the BC-requirement documentation obligation changes: advisory output is contractable; BC-4.15.001 is required now, not at blocking-promotion time. If v1→blocking-promotion occurs, product-owner amends BC-4.15.001 (amend-not-replace per BC lifecycle / POLICY 1 append-only) with updated normative postconditions and new §Changelog row; BC-4.15.001 is NOT retired.
...
#### §Decision 12 SEC-002 Redaction Sub-clause — command_preview Secret Redaction

**Best-effort advisory framing (not a security boundary):** This redaction mechanism is a best-effort advisory measure, consistent with the GitLab CI masked-variable and Warp terminal best-effort masking guidance. It is NOT a security boundary. It does not guarantee that every possible secret form is detected; it reduces the probability that common high-risk patterns appear in plugin.log advisory records. False negatives are acknowledged and documented below. Operators requiring hard secret isolation must not rely solely on this mechanism.

**SEC-002 requirement:** The `command_preview` field emitted in both channels (PC-B-B1 stderr and PC-B-B2 plugin.log) MUST have secrets redacted before the preview string is constructed. Redaction occurs BEFORE truncation (redact-then-truncate ordering mandate per EC-020). This ordering prevents a secret that straddles the 120-character truncation boundary from being partially exposed in the truncated output.

**Algorithm — no-regex 4-pass redaction (non-configurable):**

The redaction algorithm is hard-coded in the WASM plugin source using pure standard string operations (no regex engine). It MUST NOT be made configurable at runtime (no config-file override, no hooks-registry.toml config key). All trigger keyword lists are compile-time constants. Extending them requires a source code change and a new release.

The four passes are applied sequentially to the command string. Each pass operates on the output of the previous pass. Passes are non-iterating (single scan each).

**Pass 1 — flag-argument masking:**

Split the command string on ASCII whitespace. For each token that starts with `-` (i.e., is a flag):

1. Strip leading dashes to get the flag name. If the name contains `=`, the name is the portion before the first `=`.
...
**Bare `--key` is NOT a trigger** (false-positive trap: `sort --key`, `cut --key`, `awk --key` and similar well-known utilities use `--key` non-secretly). Bare short flags (`-p`, `-u`, `-t`) are NOT masked regardless of name.

**Pass 2 — env-var assignment-prefix masking:**

For tokens at the start of the command that match the pattern `IDENT=value` (an assignment token appearing before the first non-assignment command token), where `IDENT` uppercased **contains** one of the following compound trigger substrings:
...
- `SSH_AUTH_SOCK`
- `SSH_ASKPASS`
- Any identifier ending in `_SERVICE_HOST` (e.g., `KUBERNETES_SERVICE_HOST`, `MY_API_SERVICE_HOST`)
...
**Pass 3 — Authorization/Cookie header masking (bounded consumption):**

For any whitespace-delimited token whose lowercased form starts with `authorization:`, `cookie:`, or `set-cookie:`, apply the following bounded-consumption rule. This covers all Authorization schemes (Basic, Bearer, ApiKey, etc.) uniformly without scheme enumeration.
...
**Rationale (F-RD2-001/F-RD2-002):** An unquoted Authorization value can span two tokens (scheme token + credential token); the prior "mask the next token (singular)" rule left the credential exposed. Unbounded consumption in the other direction destroyed trailing positionals and URLs. This bounded rule masks the full credential while preserving non-secret trailing CLI flags and URL arguments in the preview. On malformed input it fails safe rather than consuming indefinitely.

**Test vectors (for product-owner BC-4.15.001 and test-writer to mirror):**

| Input | Expected command_preview |
|-------|--------------------------|
| `curl -H "Authorization: Bearer eyJtoken" https://api.example.com` | `curl -H "Authorization:***REDACTED***" https://api.example.com` (quoted inline Form A; URL preserved) |
| `curl Authorization: Bearer tok https://api.example.com` | `curl Authorization: ***REDACTED*** https://api.example.com` (Form B: `Bearer` and `tok` both masked; `https://…` is a b2-stop token — preserved) |
| `curl Authorization: Bearer tok --verbose` | `curl Authorization: ***REDACTED*** --verbose` (Form B: `Bearer` and `tok` masked; `--verbose` is a b1-stop token — preserved) |
| `curl "Authorization: Bearer tok --flag` (unbalanced quote) | consumption stops at `--flag` (b1-stop) or end; tail not swallowed |

**Pass 4 — inline URL credential masking:**

For any whitespace-delimited token containing `://`: if there is an `@` character after the `://` and before the next `/` (or end of token), mask the userinfo substring between `://` and `@` (the `user:pass` portion) with `***REDACTED***`, yielding `scheme://***REDACTED***@host/path`. Covers `https://user:pass@host`, `postgres://user:pass@host/db`, and equivalent URL forms.

**Mask token:** The literal string `***REDACTED***` — exactly **14 ASCII characters** (`***` = 3, `REDACTED` = 8, `***` = 3; total = 14). This token is hard-coded and not configurable.

**Provider-prefix shape detection deferred (documented future enhancement):** Detection of provider-specific secret shapes (e.g., `sk-` OpenAI prefix, `AKIA` AWS access key prefix, `ghp_` GitHub PAT prefix) is explicitly out of scope for v1. These patterns would require additional shape-matching passes that introduce false-positive risk on legitimate non-secret tokens with the same prefix. The keyword-based passes above catch the most common leakage vectors for these providers indirectly (via `--token`, `--api-key`, `--secret`, `ACCESS_KEY`, etc.). Provider-prefix detection may be added in a future release as a separate named pass.

**False-negative acknowledgment:** This algorithm does not detect all possible secret forms. Undetected patterns include: positional arguments containing secrets, secrets embedded in subshell expansions (`$(cat /secret)`), secrets in here-documents or multi-line strings, environment variable assignments that use non-standard naming (e.g., `MY_SPECIAL_CRED=abc` where `MY_SPECIAL_CRED` does not contain a compound trigger). These are acknowledged out-of-scope for v1. The algorithm targets the highest-probability leakage patterns (flag-argument pairs, env-assignment prefixes, Authorization/Cookie headers, URL credentials) without attempting exhaustive detection.

**Invariant preservation — command_preview ≤120-char contract (EC-020 truncation-interaction):**

The redacted string (output of Pass 4) is the input to the truncation step (BC-4.15.001 Invariant 4 / VP-091 §5). The ≤120-character truncation invariant applies to the POST-REDACTION string. Because `***REDACTED***` (14 chars) may be longer or shorter than the secret it replaces, the redacted string length may differ from the original; truncation is always applied after redaction. The truncated preview is therefore always ≤120 characters (+ optional `…`) and never contains a partial secret token at a truncation boundary. The `message` field in the plugin.log record derives from the already-redacted `command_preview` and therefore requires no separate redaction step.
...
### Decision 14 — Pure-parse invariant consistency gate: detection algorithm for S-18.08 (S-18.08 redesign v2)

**Context (S-18.08 gate flaw):** The S-18.08 gate-enforcement story as originally designed uses a noun-only `grep -Ev "^## <section>"` exclusion pattern that is broken in three ways: (1) it does not extract normative sections exclusively — section body content that is NOT a heading line is included; (2) it flags a gate's own trigger object (e.g., `HANDOFF.md` as the BC-4.14.001 PostToolUse target) identically to a genuine substrate-read violation; (3) it cannot distinguish prohibition/affirmation sentences ("no external filesystem or sprint-state.yaml access is performed") from genuine violation sentences ("the gate reads wave_id from sprint-state.yaml").

**Post-v1.30 adversarial findings (v1.31 revision):** Two MEDIUM defects were identified in the v1.30 AC-003 dynamic-discovery leg: (1) **Layer-1 boundary fails on SS-07 heading variant** — ~1,712 SS-07 BCs use `## Related BCs (Recommended)` (with parenthetical suffix), which the prior `## Related BCs$` dollar-anchor never matches; awk over-extracts from `## Preconditions` to EOF for all SS-07 BCs, scanning non-normative sections (Verification Properties, Traceability, Changelog) and defeating the section-scoping guarantee. (2) **Over-broad discovery plus vacuous false-clean** — the prior `grep -rl "pure.parse"` discovery (unescaped `.`) matched 194 files including `BC-INDEX.md` (an index doc with no `## Preconditions`) and 190 SS-07 BCs that contain "pure parse/decision logic" as an architectural prose description, not an invariant declaration. For any file lacking `## Preconditions`, Layer-1 awk emits ZERO lines, producing HITS=0 silently — the gate cannot distinguish "scanned and clean" from "structurally un-scannable, vacuously clean". Both defects are corrected below.

**Decision:** The correct gate detection algorithm uses THREE sequential layers plus a scoped discovery step and a fail-loud scannability check, all of which must be satisfied to classify a line as a genuine violation. The algorithm is specified below as a drop-in replacement for S-18.08 AC-001..AC-005.

#### Discovery — Invariants-section-anchored BC discovery

**Correct discovery (v1.31):** Search for BCs that DECLARE the pure-parse invariant in their normative `## Invariants` section. This precisely identifies BCs where pure-parse is a behavioral requirement, excludes index docs, and excludes BCs that only mention "pure parse" as architectural prose.

Discovery command:

```bash
find .factory/specs/behavioral-contracts -name "BC-*.md" -print0 \
...
**Why Invariants-section anchoring is correct:** BCs that genuinely enforce the pure-parse invariant declare it in `## Invariants` using a phrase like "pure-parse WASM function" or "Pure-parse; no filesystem, subprocess, or context access". BCs that only contain "pure parse" as an architectural concept (e.g., SS-07 BCs describing "pure parse/decision logic") or capability-constraint mentions (e.g., BC-5.41.001 changelog) do NOT declare it in their Invariants section. The `## Invariants` section is the correct normative anchor.

**Why the prior discovery was wrong:** `grep -rl "pure.parse"` matched 194 files:
- 190 SS-07 BCs: matched "pure parse/decision logic" in non-Invariants prose (architectural description)
- 1 SS-05 BC (BC-5.41.001): matched "pure-parse capability constraint" in changelog
- 1 index doc (BC-INDEX.md): index file with no normative sections at all
- 2 genuine targets (BC-4.14.001, BC-4.15.001): correctly matched

The Invariants-section-anchored discovery returns exactly 2 files — BC-4.14.001 and BC-4.15.001 — on the current corpus, and auto-discovers any future BC that adds a pure-parse Invariants declaration.

#### Scannability check — Fail-loud on un-scannable discovered files

Before scanning any discovered file, verify it contains `## Preconditions`. A discovered file that lacks this heading produces ZERO awk output — the gate would vacuously pass, indistinguishable from a genuinely clean file.

```bash
if ! grep -q "^## Preconditions$" "$BC_FILE"; then
  echo "FAIL: $BC_FILE was discovered as a pure-parse BC but lacks ## Preconditions — structurally un-scannable"
  exit 1
fi
```

This check ensures every discovered BC is structurally amenable to Layer-1 extraction. Any future BC that declares pure-parse in its Invariants but lacks a Preconditions section (a structural defect) is surfaced loudly rather than vacuously passed.

#### Layer 1 — Normative section extraction (BC scan only)

For BC files, extract ONLY the normative behavioral sections before scanning. The extraction uses a **whitelist terminator**: stop at the first `## ` heading that is NOT one of the known normative behavioral section names. This is robust to all current BC structural variants in the corpus.

**Normative behavioral section names (whitelist):**
`Preconditions`, `Postconditions`, `Invariants`, `Edge Cases`, `Error Paths`, `Canonical Test Vectors`

**START anchor (inclusive):** `## Preconditions`
**END trigger:** any `## ` heading NOT in the normative whitelist above
...
This covers: §Preconditions, §Postconditions, §Invariants, §Edge Cases, §Error Paths (where present), §Canonical Test Vectors — the complete normative behavioral specification. It excludes ALL non-normative sections regardless of their order or naming variant: §Description, §Related BCs, §Related BCs (Recommended), §Architecture Anchors, §Architecture Anchors (Recommended), §Story Anchor, §Story Anchor (Recommended), §VP Anchors, §VP Anchors (Recommended), §Verification Properties, §Traceability, §Changelog.

**Why the prior `## Related BCs$` END anchor was wrong:** The BC corpus has two structural patterns:
- SS-04 style (119 BCs): order is `… Canonical Test Vectors → Related BCs → Architecture Anchors → …`. The `## Related BCs$` anchor fired correctly.
- SS-07 style (1,712 BCs): order is `… Canonical Test Vectors → Verification Properties → Traceability → Related BCs (Recommended) → …`. The `## Related BCs$` dollar-anchor NEVER matches `## Related BCs (Recommended)`, causing awk to over-extract from `## Preconditions` to EOF (106 lines vs. 32 lines for the correct normative-only range). The non-normative sections Verification Properties and Traceability were silently scanned.

The whitelist terminator correctly handles both patterns: for SS-04 BCs it stops at `## Related BCs`; for SS-07 BCs it stops at `## Verification Properties` — both are the first non-normative heading after `## Canonical Test Vectors`.

For VP files, scan the whole file (no section extraction). The verb-collocation + negation filter layers (below) are sufficient to distinguish legitimate test-fixture setup comments from genuine violations.

#### Layer 2 — Verb-collocation requirement (not noun-only)

A genuine pure-parse violation requires a READ VERB collocated with a substrate noun within the same line. A line that contains a substrate noun WITHOUT a read verb is describing the substrate (to identify what the gate does NOT do, to name the gate's own input, or to state the prohibition) and is not a violation.

Read-verb pattern (uses `[[:space:]]+` for BSD/macOS + GNU grep cross-platform portability; `\s+` is not honored by BSD grep):
```
(reads?|loads?|fetches|derives?|access(es)?|retrieves?|opens?|parses?)[[:space:]]+.{0,80}(sprint-state\.yaml|git-log|git-cat-file)
```

Substrate nouns scanned:
- `sprint-state\.yaml` — shell-layer substrate that a pure-parse WASM gate MUST NOT read
- `git-log` — git execution a pure-parse gate MUST NOT perform
- `git-cat-file` — git execution a pure-parse gate MUST NOT perform

Note on `HANDOFF.md` and `factory-artifacts`: these nouns are EXCLUDED from the substrate-noun set for BC scans because they appear legitimately in pure-parse BC normative sections as (a) the gate's registered trigger object (PostToolUse target), (b) the payload being validated, and (c) integration test fixture paths. They cannot be reliably distinguished from violation uses by lexical analysis alone. The critical pure-parse substrates — `sprint-state.yaml`, `git-log`, `git-cat-file` — are the correct violation targets.

#### Layer 3 — Negation-cue exclusion (removes affirmations)

Lines that pass Layer 2 (have a read verb + substrate noun) but also contain a negation cue are AFFIRMATIONS of the pure-parse invariant, not violations. Exclude them:

```bash
grep -Eiv "no |not |NOT |without|never|does not|MUST NOT|is NOT|cannot|do NOT|only from|exclusively"
```

Additionally, exclude Rust/bash code comment lines (lines where the first non-whitespace characters are `//`) — these appear in VP Proof Harness sections and describe what the SHELL CALLER (not the WASM gate) reads from substrate:

```bash
grep -Ev "^[[:space:]]*//"
```

#### Complete machine-executable gate command (single scan pipeline)

For a **BC file** (discovery + scannability check + normative sections only):

```bash
# Step 1: Discovery — find all BCs declaring pure-parse in ## Invariants
...
# Step 2: Scan each discovered BC
OVERALL_STATUS=0
while IFS= read -r BC_FILE; do
  # Scannability check — fail-loud if ## Preconditions absent
  if ! grep -q "^## Preconditions$" "$BC_FILE"; then
    echo "FAIL: $BC_FILE was discovered as a pure-parse BC but lacks ## Preconditions — structurally un-scannable"
    OVERALL_STATUS=1
    continue
...
HITS=$(grep -Ei "(reads?|loads?|fetches|derives?|access(es)?|retrieves?|opens?|parses?)[[:space:]]+.{0,80}(sprint-state\.yaml|git-log|git-cat-file|factory-artifacts)" "$VP_FILE" \
  ...
  | grep -Ev "^[[:space:]]*//" \
  | wc -l)
[ "$HITS" -eq 0 ] || echo "FAIL: $VP_FILE has $HITS genuine pure-parse violations"
```

Note: `factory-artifacts` IS included in the VP verb-collocation scan because VP Property sections may legitimately describe integration test substrates in a way that a verb collocation with `factory-artifacts` (not a Rust comment) would indicate a genuine violation.

#### Rationale for algorithm design

**Why verb-collocation instead of noun-only grep:** The noun-only approach yields ~42–59 false positives per BC file because `HANDOFF.md`, `sprint-state.yaml`, and `factory-artifacts` appear legitimately in prohibition statements ("no sprint-state.yaml access"), gate-trigger descriptions ("targets HANDOFF.md"), and test vector inputs ("Write to HANDOFF.md"). The verb-collocation approach requires active-voice read semantics — "reads X from sprint-state.yaml" — which is the only form a genuine violation takes.

**Why negation-cue exclusion is needed in addition to verb-collocation:** Pure-parse BCs commonly phrase their invariant as "the gate does NOT read from sprint-state.yaml" — this sentence contains a read verb AND a substrate noun but is an affirmation. The negation filter removes it.

**Why `//` comment exclusion is needed for VPs:** Integration VP files (VP-081) include Rust test fixture setup comments that describe the shell caller's substrate access ("// Setup (SHELL CALLER substrate): sprint-state.yaml…"). These comments are load-bearing descriptions of what the SHELL layer does — not of what the WASM gate does. The `//` exclusion prevents false positives on these comments without requiring VP section extraction.

**Why Invariants-section-anchored discovery is future-proof:** The discovery pattern `awk '/^## Invariants$/{found=1;next} found && /^## /{exit} found{print}' | grep -i "pure-parse"` auto-discovers any new BC that declares pure-parse as a behavioral invariant, without requiring manual enumeration. When a new pure-parse gate BC is authored (as required by §Decision 8), it will automatically be picked up if it correctly declares the invariant in `## Invariants`. If a new BC lacks the Invariants declaration despite being a pure-parse gate, the gate (correctly) does not discover it — a spec authoring defect, not a gate defect.

**Honest statement of residual risk:** This lexical gate cannot detect every possible violation. A violation phrased using a non-enumerated read verb (e.g., "queries", "consults", "examines") would not be caught. The verb list (`reads?|loads?|fetches|derives?|access(es)?|retrieves?|opens?|parses?`) covers the domain of BC prose actually observed across the SS-04 BC corpus. If future BCs use unusual verbs, the gate's verb pattern must be extended. The gate is verified correct on the full BC corpus (1,973 BC files) as of ADR-026 v1.31.

**Positive control verification:** The algorithm correctly detects the following injected violations in a BC's normative sections:
> "The gate reads wave context directly from sprint-state.yaml before parsing the payload."
Returns HITS=1. Verified against a synthetic BC file with SS-07 structural pattern (`## Related BCs (Recommended)` heading variant) — the whitelist Layer-1 terminates at `## Verification Properties` before the non-normative sections, and the positive-control line in `## Edge Cases` is correctly extracted and detected.

Additionally (v1.32 reconciliation — previously unverifiable with 6-verb form):
> "The gate opens sprint-state.yaml to retrieve the wave context."
Returns HITS=1.
> "The gate parses git-log output to derive the commit range."
Returns HITS=1.
Both confirm that `opens?` and `parses?` verbs — present in the prose spec but absent from the v1.31 command block — are now correctly detected by the reconciled 8-verb gate.

**Empirical validation (as of ADR-026 v1.32, run against full corpus — 8-verb gate):**

```
Discovery: 2 BCs (BC-4.14.001, BC-4.15.001) — excludes BC-INDEX.md and all 190 SS-07 prose-mention files
Scannability: 2/2 pass (both have ## Preconditions)
BC-4.14.001 normative sections: HITS=0 (PASS)
BC-4.15.001 normative sections: HITS=0 (PASS)
VP-083 whole-file: HITS=0 (PASS)
VP-081 whole-file: HITS=0 (PASS)
VP-091 whole-file: HITS=0 (PASS)
...
#### Relationship to §Decision 8

§Decision 8 establishes that pure-parse WASM gates MUST NOT perform filesystem, git, or context reads — this is the invariant being enforced. §Decision 14 specifies how to verify that BC specifications correctly represent this invariant (i.e., no BC nominally declares pure-parse while also specifying substrate reads in its normative behavioral sections). §Decision 14 is the consistency-gate companion to §Decision 8's runtime design choice.

#### Story anchor

S-18.08 (pure-parse invariant consistency gate). The story-writer MUST replace S-18.08 AC-001..AC-005 with the corrected machine-checkable assertions derived from this decision (see S-18.08 story file for the authoritative AC text after the story-writer adopts this design).

---

## SS-08, SS-01, and S-18.07 Scope Clarification (F-9, F-P37-001)
...
**Why SS-01 IS in `anchors:` (F-P37-001):** Decision 11 ('S-18.00: Dispatcher routing addition for PreCompact/PostCompact') is an explicit architectural decision about the factory-dispatcher binary (`crates/factory-dispatcher/src/`) — the source code artifact owned by SS-01 (Hook Dispatcher Core). The decision prescribes that the dispatcher binary MUST route `PreCompact` and `PostCompact` events to registered plugins and specifies that S-18.00 is the resolution vehicle if routing support is absent. This is unambiguously an architectural decision made about SS-01: it dictates what event types SS-01's event-routing layer must enumerate. Corroborating evidence: (1) the Deliverables table row for S-18.00 explicitly lists Subsystem(s)=SS-01; (2) VP-086 (Dispatcher Exit-2 Propagation for PreCompact Block-Intent) has scope SS-01,SS-04 and is the load-bearing verification property for the entire PreCompact blocking mechanism; (3) BC-1.15.001 carries subsystem=SS-01 for the exit-2 propagation contract; (4) CAP-032 declares Subsystems SS-01,SS-04,SS-05,SS-06,SS-07 as the full scope of this capability. SS-01 belongs in `anchors:` under the same criterion that includes SS-04..SS-07 — architectural decisions are made here. SS-08 is excluded because no architectural decision in this ADR is made about the templates/rules layer. The `anchors:` distinction between SS-01 and SS-08 is: SS-01 is architecturally impacted (Decision 11 modifies the dispatcher routing layer); SS-08 is documentationally touched (S-18.07 updates documentation under SS-08 targets) but no architectural commitment lands in SS-08.
...
| `validate-heavy-op-delegation` blocking mode | Requires false-positive calibration before blocking | S-18.06 F3 adversarial review → promotion if rate acceptable; BC-4.15.001 amended (amend-not-replace) at promotion time |
...
| `validate-heavy-op-delegation` WASM crate (advisory mode; BC-4.15.001) | S-18.06 | SS-04 |
...
| check-state-health settings.json `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` verification (BC-6.25.001) | S-18.10 | SS-06 |
...
| VP | Title | BC | Type |
|----|-------|-----|------|
| VP-081 | Wave Cannot Close Without Verified Handoff (wave_id > 1) | BC-5.41.001, BC-4.14.001 | safety/integration |
| VP-082 | PreCompact Flush Commits to factory-artifacts Before Compaction Proceeds | BC-7.07.001 | safety/integration |
| VP-083 | Completeness Gate Is No-Op on Wave-1 or Non-HANDOFF.md Writes | BC-4.14.001 | invariant/unit-test |
| VP-084 | PreCompact Flush Commit Is Lifecycle-Distinct From State-Manager Burst Commit | BC-5.41.003 | invariant/integration |
| VP-085 | PreCompact Flush Hook Is Hermetic — Reads STATE.md+Git Only, Never In-Context Reasoning | BC-7.07.001 | safety/unit-test |
...
- **Subsystems affected:** SS-01, SS-04, SS-05, SS-06, SS-07 (SS-01 confirmed affected per Decision 11 dispatcher routing requirement and VP-086/BC-1.15.001/CAP-032 scope; prior 'potentially SS-01 per S-18.00 outcome' hedge was stale — Decision 11 is definitive regardless of S-18.00 outcome)
...
- **VP-INDEX:** v2.07→v2.08 (VP-086 added); v2.08→v2.09 (VP-081..VP-085 v1.2 inputs-path fix; VP-081/082/083/085 current_wave sweep); v2.09→v2.10 (VP-082 v1.2→v1.3 last-precompact-flush-sha→precompact-flush-log; VP-084 v1.2→v1.3 flush-wave prefix→canonical format; VP-085 v1.2→v1.3 last-precompact-flush-sha→precompact-flush-log); v2.10→v2.10 (no VP change in pass-4; VP-INDEX remains at v2.10); v2.10→v2.11 (VP-084 v1.4→v1.5 F-P5-004: harness re-anchored from validate-burst-log.sh → dispatcher WASM invocation; VP-INDEX v2.10→v2.11); v2.11→v2.12 (pass-6 cite-convention migration: VP-084 v1.5→v1.6 cite de-versioned; VP-INDEX v2.11→v2.12); v2.12→v2.13 (pass-9 F-P9-002: VP-082 description amended to include append-failure → SHA-pinned reset (SHA_B^) + concurrent-commit guard; VP-INDEX v2.12→v2.13); v2.13→v2.14 (pass-10 F-P10-003: VP-082 v1.3→v1.4 Postcondition E added; VP-INDEX row updated; total_vps 86 unchanged; VP-INDEX v2.13→v2.14); v2.32→v2.33 (VP-091 v1.0 authored — validate-heavy-op-delegation always-Continue + dual-channel DelegationRecommended advisory + no-match no-emission + non-Bash no-op + command_preview Inv4; SS-04; DI-020; unit-test; S-18.06; total_vps 90→91; unit-test 44→45); v2.33→v2.34 (VP-092 v1.0 authored — check-state-health CLAUDE_AUTOCOMPACT_PCT_OVERRIDE settings.json verification; PC1 absent-ADVISORY, PC2 value>80-ADVISORY, PC3 value<=80-PASS, PC4 project-local-precedence, PC5+Inv1+Inv5 never-blocks+row-always-emitted; SS-06; DI-020; unit-test; BC-6.25.001; S-18.10; total_vps 91→92; unit-test 45→46)
- **ARCH-INDEX:** v2.28→v2.29 (ADR-026 v1.0→v1.1); v2.29→v2.30 (ADR-026 v1.1→v1.2 F-P2 revision); v2.30→v2.31 (ADR-026 v1.2→v1.3 F-P3 COMPLETE-SWEEP: canonical §Wave-Identity Derivation + precompact-flush-log append-only + EPIC-COMPLETE cancelled terminal state); v2.31→v2.32 (ADR-026 v1.3→v1.4 F-P4 revision: append-failure→exit-2; WASM/shell division explicit; §F-P4-004 PO wording; §VP Allocations stale-narrative fixed); v2.32→v2.33 (ADR-026 v1.4→v1.5 F-P5 revision: reset-on-append-failure; stale-entry re-grounding; §Decision A anchor; VP-084 v1.4→v1.5 harness re-anchor); v2.33→v2.34 (ADR-026 v1.5→v1.6 GOVERNANCE cite-convention + F-P6-006 SHA-pinned guard + F-P6-004 epic_status conditional field); v2.34→v2.35 (ADR-026 v1.6→v1.7 F-P7-001 payload-only EPIC-COMPLETE discriminator + F-P7-002 provenance trace completion); v2.35→v2.36 (ADR-026 v1.7→v1.8 F-P9-001 §F-P5-002 PC8 + §Decision A HEAD~1 superseded annotations; F-P9-002 VP-082 description + VP-INDEX v2.12→v2.13); v2.36→v2.37 (ADR-026 v1.8→v1.9 F-P10-001 Decision 9 payload-only wave-1 discriminator + §F-P10-001 PO wording + §F-P10-002 push-step contract note; F-P10-003 VP-082 v1.3→v1.4 Postcondition E; VP-INDEX v2.13→v2.14); v2.37→v2.38 (ADR-026 v1.9→v1.10 CV-P14-001 §Decision 2 precompact_flush_sha schema row + Wave-1/Genuine-Log-Absence note reconciled to BC-5.41.001 v1.8 three-case null-SHA rule); v2.38→v2.39 (ADR-026 v1.10→v1.11 F-P15-002 de-versioned stable-anchor cite); v2.39→v2.40 (ADR-026 v1.11→v1.12 F-P17-001 wave-agnostic null rule: log-present-and-valid HARD BLOCK now applies to ANY wave_id including wave_id=1; wave_id discriminator survives only for log-absent case); v2.40→v2.41 (ADR-026 v1.12→v1.13 F-P19-002 §Decision 9 MEDIUM fix: heading retitled 'non-HANDOFF.md writes'; HandoffMissing misattribution to WASM gate corrected — HandoffMissing is BC-5.41.001 PC9 SHELL-SIDE; disjoint boundary paragraph added; §BC Traceability Cite Convention table + §Risk Mitigations F1-R3 row updated); v2.41→v2.42 (ADR-026 v1.13→v1.14 F-P20-002 §Wave-Identity-Derivation cross-note: prior-HANDOFF.md-absence proxy is CALLER-SIDE derivation signal; WASM gate PAYLOAD-ONLY discriminator `payload.wave_id == 1` is the authoritative gate behavior — two substrates are reconcilable not competing; §Decision 9 remains canonical for WASM gate behavior); v2.42→v2.43 (ADR-026 v1.14→v1.15 F-P22-001 §VP Allocations VP-081/VP-083 title corrections; F-P23-001 §Decision 6 step-order re-sequenced to canonical commit→append→push→exit per BC-7.07.001 Inv3 — `git push` now explicit numbered step 6; field-4 literal-commit framing corrected in step 5 body; VP-082 v1.10→v1.11 PC-A parenthetical canonical-field-4 fix; VP-084 v1.8→v1.9 split-anchor justification); v2.43→v2.44 (ADR-026 v1.15→v1.16 F-P25-002 §Decision 9 EPIC-COMPLETE-before-wave_id==1 carve-out — canonical evaluation order (1) non-HANDOFF.md→no-op (2) EPIC-COMPLETE branch even at wave_id==1 (3) wave_id==1+NOT-EPIC-COMPLETE→no-op (4) wave_id>1→validate; F-P25-003 §VP Allocations VP-082/VP-084/VP-085 abbreviated titles corrected to verbatim VP H1 headings; VP-082 v1.11→v1.12 F-P25-004 PC-A 'actual HEAD'→'candidate commit SHA'; VP-083 v1.7 no change required — Property §1 wave_id==1 no-op is not the new EPIC-COMPLETE carve-out path, which is handled pre-no-op by PC2a); v2.44→v2.45 (ADR-026 v1.16→v1.17 F-P28-002-sibling normative side-channel→precompact-flush-log term migration: §BC Traceability Cite Convention BC-5.41.001 replacement text cell + §Decision 2 `precompact_flush_sha` Source column — two normative-present-tense 'side-channel file' residue sites fixed); v2.45→v2.46 (ADR-026 v1.17→v1.18 F-P31-001 exhaustive sweep: §Decision 6 step 5 normative 'append-only side-channel log' → 'append-only log' — missed by v1.17 sweep which adjudicated §Crash-Consistency Design hits as historical but did not catch this §Decision 6 body occurrence); v2.46→v2.47 (ADR-026 v1.18→v1.19 F-P37-001: SS-01 added to `anchors:` + `subsystems_affected:` frontmatter; §SS-08 Scope Clarification extended to explain SS-01 anchors inclusion via Decision 11 dispatcher routing architectural decision; §Traceability 'potentially SS-01' hedge corrected to definitive SS-01 confirmation); v2.49→v2.50 (ADR-026 v1.19→v1.20 F3 OQ-4: §Decision 12 'No BC required' clause replaced with 'BC-4.15.001 required' — advisory output IS contractable; §Deliverables S-18.06 row 'no BC in v1'→'BC-4.15.001'; §v2 Deferrals blocking-mode row updated to 'amend-not-replace BC-4.15.001 at promotion time'; VP-091 v1.0 authored in same burst); v2.50→v2.51 (ADR-026 v1.20→v1.21: S-18.10 §Deliverables row added — check-state-health settings.json CLAUDE_AUTOCOMPACT_PCT_OVERRIDE verification (BC-6.25.001) explicit ownership; stale 'S-18.03 or S-18.07' provisional row replaced; VP-092 v1.0 authored in same burst; F-MAJOR-003 coverage gap closed); v2.57→v2.58 (ADR-026 v1.21→v1.22 §Decision 8 Write-tool write-path constraint); v2.58→v2.59 (ADR-026 v1.22→v1.23 §Decision 8 agent-orchestrated emit→Write→commit control flow: v1.22 normative rule corrected — monolithic bash run has no agent seam; correct mechanism is bash subcommand split `--emit-handoff`/`--emit-wave-state`/`--commit` with SKILL.md as genuine agent-orchestration layer; atomicity-preservation argument for BC-5.41.002 PC6; partial-write failure edge cases specified; SKILL.md numbered step structure canonical); v2.59→v2.60 (ADR-026 v1.23→v1.24 F-SP13-P5-001: §Decision 8 `--commit` EPIC-COMPLETE carve-out — Step 5 SKILL.md step structure annotated with two-arm `--commit` behavior; normative prose for `--commit` subcommand amended with HAS-NEXT-WAVE arm (stages BOTH; hard-abort if EITHER absent) and EPIC-COMPLETE arm (stages HANDOFF.md ALONE; removes stale wave-state.yaml; hard-abort only on missing HANDOFF.md); atomicity-preservation argument updated: single-file EPIC-COMPLETE commit is ONE atomic commit, not a PC6 violation; BC-4.14.001 cite-convention replacement text updated to reflect two-arm distinction); v2.76→v2.77 (ADR-026 v1.24→v1.25 F-P1-001/F-P1-003/F-P1-004 §Decision 7 phantom-field fix: `last_verified_develop_sha` SHA source corrected to `git rev-parse origin/develop` (not STATE.md field); env_allow corrected (GIT_DIR removed — test artifact; GIT_CONFIG_GLOBAL + XDG_CONFIG_HOME added — git subprocess config); read_file→write_file capability correction (STATE.md read is git-subprocess; only log write needs write_file); generator retirement note added (dead code revert instruction for implementer); VP-089 v1.0→v1.1 §1 SHA source updated; Refs: F-P1-001, F-P1-003, F-P1-004, BC-7.07.002 PC1, VP-089 §1, S-18.05, DI-024); v2.77→v2.78 (ADR-026 v1.25→v1.26 adv P2 F-P2-002: §Decision 7 generator-handling text corrected — POST_HISTORICAL_SCRIPTS+skip_names registration pattern per check-harness-version precedent; band_for()/legacy-JSON are the drift hazard and are reverted; POST_HISTORICAL emits nothing; VP-089 v1.1→v1.2 §2 log-schema field swept last_verified_develop_sha→develop_sha)
...
|---------|------|--------|--------|
| v1.33 | 2026-06-27 | state-manager | D-707 governance burst: §Decision 14 spec-hygiene normalization (LOCAL adv P3 O-P3-002/O-P3-003) — grep snippets normalized `\s+`→`[[:space:]]+` and `\s*`→`[[:space:]]*` (6 lines: Layer 2 prose read-verb note, BC-loop inner grep, VP-scan grep, VP comment grep ×2, positive-control snippet grep) for BSD/macOS + GNU grep cross-platform portability; portability note added to Layer 2 read-verb pattern prose. No logic change; validated 0/0/0 genuine violations + 3 positive controls. S-18.08 AC snippets normalized in parallel (story v1.8→v1.9). §Traceability ARCH-INDEX v2.83→v2.84 provenance leg appended. Refs: D-707, O-P3-002, O-P3-003, S-18.08 v1.9, ADR-026 §Decision 14, E-18, issue-173. |
| v1.32 | 2026-06-27 | state-manager | D-706 governance burst: §Decision 14 verb-set reconciliation (LOCAL adv P2 F-P2-001, architect-reconciled) — read-verb pattern in all three command-block occurrences (BC-loop inner, VP-scan, positive-control snippet) expanded to canonical 8-verb form `(reads?|loads?|fetches|derives?|access(es)?|retrieves?|opens?|parses?)` (adds `opens?` and `parses?`); prose "honest residual risk" statement updated to cite 8-verb set; opens/parses positive-control verification rows added (both yield HITS=1); empirical validation banner updated to v1.32. S-18.08 v1.7→v1.8. §Traceability ARCH-INDEX v2.82→v2.83 provenance leg appended. Refs: D-706, F-P2-001, S-18.08 v1.8, ADR-026 §Decision 14, E-18, issue-173. |
| v1.31 | 2026-06-27 | state-manager | D-705 governance burst: §Decision 14 REFINEMENT (LOCAL adv P1 F-P1-001/F-P1-002, architect-refined) — discovery anchored to `## Invariants` section (awk scans only that section of each BC-*.md; structurally excludes BC-INDEX.md and ~190 SS-07 prose-mentions that `grep -rl "pure.parse"` previously matched); Layer-1 awk whitelist terminator (stops at any `## ` heading NOT in `{Preconditions, Postconditions, Invariants, Edge Cases, Error Paths, Canonical Test Vectors}`, robust to `## Related BCs (Recommended)` and all variant headings); fail-loud scannability guard added to AC-003 loop (discovered BC lacking `## Preconditions` emits `FAIL: structurally un-scannable`, sets OVERALL_STATUS=1). S-18.08 v1.6→v1.7. §Traceability ARCH-INDEX v2.81→v2.82 provenance leg appended. Refs: D-705, F-P1-001, F-P1-002, S-18.08 v1.7, ADR-026 §Decision 14, E-18, issue-173. |
| v1.30 | 2026-06-27 | state-manager | D-704 governance burst: §Decision 14 authored by architect — pure-parse invariant consistency gate 3-layer detection algorithm (normative-section extraction + verb+substrate collocation + negation/comment exclusion); S-18.08 AC-001..005 redesign anchor; human-adjudicated S-18.08 gate redesign. §Traceability ARCH-INDEX v2.80→v2.81 provenance leg appended. Refs: D-704, S-18.08 v1.6, ADR-026 §Decision 14, E-18, issue-173. |
| v1.29 | 2026-06-26 | architect | (F-RD2-001/F-RD2-002) §Decision 12 SEC-002 Redaction Sub-clause Pass 3 replaced with bounded-consumption rule. Form A (inline value): mask after `:` within same token; stop — no further-token consumption. Form B (following-token): mask immediate next token(s); stop at first token starting with `-` (b1-CLI flag), containing `://` (b2-URL), or closing a quoted value (b3). At minimum the immediate next token is masked. Malformed/unbalanced-quote: fails safe — stops at next b1/b2 token or end-of-tokens. Rationale: F-RD2-001 (unquoted `Authorization: Bearer tok https://host` was destroying the URL); F-RD2-002 (unbalanced-quote was swallowing command tail). Test vectors added (Bearer+tok+URL, Bearer+tok+--verbose, unbalanced-quote, quoted -H form). VP-091 §6 PC6d revised in same burst. Refs: F-RD2-001, F-RD2-002, VP-091 §6, BC-4.15.001, S-18.06, E-18, issue-173. |
| v1.28 | 2026-06-26 | architect | (SEC-002 revised) §Decision 12 SEC-002 Redaction Sub-clause replaced with final canonical 4-pass design. Best-effort-advisory framing added (not a security boundary; aligned with GitLab/Warp masking guidance). Mask token corrected to 14 ASCII chars. Pass 1: flag-argument masking — lowercased flag name equals-or-contains compound trigger substrings; bare `--key` REMOVED (false-positive trap); bare short flags excluded. Pass 2: env-var assignment-prefix masking — compound triggers on IDENT only; bare KEY not matched; allowlist (SSH_AUTH_SOCK, SSH_ASKPASS, *_SERVICE_HOST) checked first. Pass 3: Authorization/Cookie/Set-Cookie header masking (all schemes uniformly). Pass 4: inline URL credential masking (`://user:pass@host` → `://***REDACTED***@host`). Provider-prefix shape detection (sk-/AKIA/ghp_) explicitly deferred as documented future enhancement. EC-020 redact-then-truncate ordering invariant preserved. VP-091 §6 revised to 7 postconditions (PC6a–PC6g) in same burst. Refs: SEC-002, VP-091 §6, BC-4.15.001, S-18.06, E-18, issue-173. |
| v1.27 | 2026-06-26 | architect | (SEC-002) §Decision 12 SEC-002 Redaction Sub-clause appended (superseded by v1.28). Two-pass design: Pass 1 flag-argument masking (including bare --key); Pass 2 Authorization/Bearer masking. Mask token stated as 15 chars (incorrect). VP-091 §6 v1.3 authored in same burst (superseded by VP-091 §6 v1.4). |
| v1.26 | 2026-06-25 | architect | (adv P2 F-P2-002) §Decision 7 generator-handling text corrected: "identical to develop-branch versions" clause replaced with the correct POST_HISTORICAL_SCRIPTS + generate-registry.bats skip_names pattern (check-harness-version precedent). The band_for() PostCompact case and legacy-JSON block are the drift hazard and are reverted; POST_HISTORICAL_SCRIPTS emits nothing, so no drift is introduced. VP-089 v1.1→v1.2 §2 log-schema swept last_verified_develop_sha→develop_sha (incomplete v1.1 propagation closed). §Traceability ARCH-INDEX v2.77→v2.78 provenance leg appended. Refs: adv P2 F-P2-001, F-P2-002, VP-089 v1.2, BC-7.07.002 PC2 v1.13, S-18.05, E-18, issue-173. |
| v1.25 | 2026-06-25 | architect | §Decision 7 amended — three phantom-field and capabilities defects fixed (F-P1-001, F-P1-003, F-P1-004). (F-P1-001) SHA source in re-anchor block corrected: `last_verified_develop_sha:` is a HANDOFF.md field, NOT a STATE.md frontmatter field — prescribing the hook to read it from STATE.md caused `sha=UNKNOWN` on every real PostCompact firing. Canonical source is `git rev-parse origin/develop` at hook invocation time; no STATE.md schema change required. (F-P1-003) `env_allow` corrected: removed `GIT_DIR` (test-fixture artifact; not a production requirement); added `GIT_CONFIG_GLOBAL` and `XDG_CONFIG_HOME` (required for git subprocess config location, matching Decision 6 canonical pattern). `[hooks.capabilities.read_file]` replaced with `[hooks.capabilities.write_file]` — STATE.md is read via `git show` subprocess (covered by exec_subprocess, not read_file); only the JSONL log write requires write_file. (F-P1-004) Generator retirement note added: scripts/generate-registry-from-hooks-json.sh and scripts/legacy/hooks-json-pre-templating.json edits in S-18.05 worktree are dead code; implementer must revert them; hooks-registry.toml is the sole canonical SoT per its own header. Process-gap: register follow-up story to mark or delete the retired generator. VP-089 v1.0→v1.1 §1 SHA source and precondition updated in same burst. §Traceability ARCH-INDEX v2.76→v2.77 provenance leg appended. Refs: F-P1-001, F-P1-003, F-P1-004, BC-7.07.002 PC1, VP-089 §1, S-18.05 AC-001/AC-009/AC-010, DI-024, E-18, issue-173. |
| v1.24 | 2026-06-19 | architect | §Decision 8 amended — EPIC-COMPLETE carve-out added to Step 5 (`--commit`) in the canonical SKILL.md step structure and in the `--commit` subcommand normative prose (F-SP13-P5-001). The v1.23 Step 5 hard-aborted if wave-state.yaml was absent, but Step 4 is explicitly `[skipped on EPIC-COMPLETE path]`, making epic closure impossible. Fix: `--commit` now has two arms: HAS-NEXT-WAVE stages BOTH HANDOFF.md + wave-state.yaml (hard-abort if EITHER absent); EPIC-COMPLETE stages HANDOFF.md ALONE (removes stale wave-state.yaml; hard-abort only on missing HANDOFF.md). Atomicity-preservation argument updated: a single-file EPIC-COMPLETE commit is ONE atomic commit — not a PC6 violation (PC6 forbids two separate commits). §BC Traceability Cite Convention BC-4.14.001 replacement text updated to reflect two-arm `--commit` distinction. §Traceability ARCH-INDEX v2.59→v2.60 provenance leg appended. Refs: F-SP13-P5-001, S-18.13, BC-5.41.002 PC6, E-18, issue-173. |
| v1.23 | 2026-06-19 | architect | §Decision 8 amended — agent-orchestrated emit→Write→commit control flow specified (S-18.13 redesign). The v1.22 normative rule ('write-handoff.sh MUST route HANDOFF.md output through the Write tool') was impossible for the monolithic bash run: `wave-handoff.sh` ends in `main "$@"` with no agent seam between Steps 4–6. Correct mechanism: bash subcommand split (`--emit-handoff` computes payload → stdout; `--emit-wave-state` writes wave-state.yaml; `--commit` stages both and creates ONE git commit) with SKILL.md restructured to genuine numbered agent steps. The Write tool is invoked by the agent between emit-handoff and emit-wave-state; PostToolUse gate fires here. BC-5.41.002 PC6 atomicity preserved: single `--commit` call stages both files regardless of whether they were agent-written or bash-written. Partial-write edge cases specified (session-die-after-Write, gate-block-at-Write, Write-tool-unavailable). §Traceability ARCH-INDEX v2.58→v2.59 provenance leg appended. Refs: F-S1802-02, S-18.13, BC-4.14.001, BC-5.41.002 PC6, E-18, issue-173. |
| v1.22 | 2026-06-19 | architect | §Decision 8 amended — HANDOFF.md Write-tool write-path constraint added (F-S1802-02 / S-18.13): the `wave-handoff` skill MUST write HANDOFF.md via the Claude Code Write tool (not bash redirection) so that the PostToolUse `validate-wave-handoff-completeness` gate fires on every HANDOFF.md write. Bash redirection emits no PostToolUse event, rendering the BC-4.14.001 completeness gate functionally inert in production. The producer MUST NOT silently fall back to bash redirection if the Write tool is unavailable — it must fail loudly (see S-18.13 EC-003). §Traceability ARCH-INDEX v2.57→v2.58 provenance leg appended. Refs: F-S1802-02, S-18.13, BC-4.14.001, E-18, issue-173. |
| v1.21 | 2026-06-16 | architect | S-18.10 §Deliverables row: stale provisional 'CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=70 settings.json verification in check-state-health | S-18.03 or S-18.07 | SS-06' row replaced with canonical 'check-state-health settings.json CLAUDE_AUTOCOMPACT_PCT_OVERRIDE verification (BC-6.25.001) | S-18.10 | SS-06'. Closes F-MAJOR-003 coverage gap — ADR-026 §Decision 5 / §F-11 mandated deliverable now has explicit story ownership and BC citation in §Deliverables. VP-092 v1.0 authored in same burst: behavioral-invariant (unit-test; SS-06; DI-020; BC-6.25.001; S-18.10) covering PC1/PC2/PC3/PC4/PC5+Inv1+Inv5 with machine-stable bats harness. §Traceability VP-INDEX v2.33→v2.34; ARCH-INDEX v2.50→v2.51. Refs: BC-6.25.001 v1.0, VP-092 v1.0, S-18.10, E-18, CAP-032, issue-173. |
| v1.20 | 2026-06-16 | architect | F3 OQ-4 human directive: §Decision 12 amended — 'No BC required for v1 advisory' clause replaced with 'BC-4.15.001 required' clause. The advisory output IS a contractable postcondition: BC-4.15.001 (v1.0, 2026-06-16, product-owner) specifies always-Continue (Invariant 2 / never blocks), dual-channel DelegationRecommended advisory on match (PC-B: stderr B-1 + plugin.log B-2), no advisory on no-match (PC-A), non-Bash no-op (PC-D), command_preview ≤120-char truncation (Invariant 4). §Deliverables table S-18.06 row: 'no BC in v1' → 'BC-4.15.001'. §v2 Deferrals blocking-mode row: 'BC authored at promotion time' → 'BC-4.15.001 amended (amend-not-replace) at promotion time'. Architectural judgment unchanged: advisory-only in v1; blocking-promotion deferred to F3 adversarial calibration (§Decision 12 rationale preserved). VP-091 v1.0 authored in same burst: verifies all four BC-4.15.001 normative postconditions + Invariant 4 via unit-test (bats; factory-dispatcher WASM invocation; SS-04; DI-020; S-18.06). §Traceability ARCH-INDEX v2.49→v2.50; VP-INDEX v2.32→v2.33. Refs: F3 OQ-4, BC-4.15.001 v1.0, VP-091 v1.0, S-18.06, E-18, CAP-032, issue-173. |
| v1.19 | 2026-06-15 | architect | F-P37-001 MEDIUM: SS-01 added to `anchors:` and `subsystems_affected:` frontmatter. §SS-08 Scope Clarification extended (SS-01 Decision 11 anchors inclusion rationale). §Traceability ARCH-INDEX v2.46→v2.47. |
| v1.18 | 2026-06-15 | architect | F-P31-001: §Decision 6 step 5 normative 'append-only side-channel log' → 'append-only log'. §Traceability ARCH-INDEX v2.45→v2.46. |
| v1.17 | 2026-06-15 | architect | F-P28-002-sibling: §BC Traceability Cite Convention BC-5.41.001 cell + §Decision 2 `precompact_flush_sha` Source column — two normative 'side-channel file' residue sites fixed. §Traceability ARCH-INDEX v2.44→v2.45. |
| v1.16 | 2026-06-15 | architect | F-P25-002 §Decision 9 EPIC-COMPLETE-before-wave_id==1 canonical evaluation order. F-P25-003 §VP Allocations title corrections. §Traceability ARCH-INDEX v2.43→v2.44. |
| v1.15 | 2026-06-15 | architect | F-P22-001 §VP Allocations VP-081/VP-083 titles. F-P23-001 §Decision 6 step-order canonical. Field-4 literal-commit framing. §Traceability ARCH-INDEX v2.42→v2.43. |
| v1.14 | 2026-06-15 | architect | F-P20-002 §Wave-Identity-Derivation cross-note (CALLER-SIDE vs WASM PAYLOAD-ONLY substrates reconciled). §Traceability ARCH-INDEX v2.41→v2.42. |
| v1.13 | 2026-06-15 | architect | F-P19-002 §Decision 9 MEDIUM: HandoffMissing SHELL-SIDE attribution; heading 'non-HANDOFF.md writes'. §Traceability ARCH-INDEX v2.40→v2.41. |
| v1.12 | 2026-06-15 | architect | F-P17-001: wave-agnostic null rule for log-present-and-valid case (HARD BLOCK any wave_id). §Traceability ARCH-INDEX v2.39→v2.40. |
| v1.11 | 2026-06-15 | architect | F-P15-002: §Decision 2 `precompact_flush_sha` ADR cite de-versioned (stable anchor). §Traceability ARCH-INDEX v2.38→v2.39. |
| v1.10 | 2026-06-15 | architect | CV-P14-001: §Decision 2 null-SHA three-case rule reconciled to BC-5.41.001 v1.8. §Traceability ARCH-INDEX v2.37→v2.38. |
| v1.9 | 2026-06-14 | architect | F-P10-001: §Decision 9 payload-only wave-1 discriminator. F-P10-002 push-step contract note. F-P10-003 VP-082 v1.3→v1.4. §Traceability ARCH-INDEX v2.36→v2.37. |
| v1.8 | 2026-06-14 | architect | F-P9-001: §F-P5-002 PC8 + §Decision A HEAD~1 superseded annotations (SHA-pinned form from §F-P6-006 is normative). §Traceability ARCH-INDEX v2.35→v2.36. |
| v1.7 | 2026-06-14 | architect | F-P7-001: EPIC-COMPLETE payload-only discriminator. F-P7-002: §Traceability downstream-index provenance trace completed. ARCH-INDEX v2.34→v2.35. |
| v1.6 | 2026-06-14 | architect | GOVERNANCE: BC Traceability Cite Convention adopted (TD-VSDD-091). F-P6-006: SHA-pinned concurrent-commit guard. F-P6-004: epic_status conditional 10th field. §Traceability ARCH-INDEX v2.33→v2.34. |
| v1.5 | 2026-06-14 | architect | F-P5-002: reset-on-append-failure. Stale-entry label corrected. §Decision A anchor added. VP-084 v1.4→v1.5. §Traceability ARCH-INDEX v2.32→v2.33. |
| v1.4 | 2026-06-14 | architect | F-P4-001: append-failure → exit 2. WASM/shell division explicit. §F-P4-004 PO wording. §VP Allocations stale-narrative fixed. §Traceability ARCH-INDEX v2.31→v2.32. |
| v1.3 | 2026-06-14 | architect | F-P3 COMPLETE-SWEEP: canonical §Wave-Identity Derivation; precompact-flush-log append-only; EPIC-COMPLETE `cancelled` terminal state. §Traceability ARCH-INDEX v2.30→v2.31. |
| v1.2 | 2026-06-14 | architect | F-P2: Canonical §Wave-Identity Derivation section; VP current_wave sweep; crash-consistency append-only log design. §Traceability ARCH-INDEX v2.29→v2.30. |
| v1.1 | 2026-06-14 | architect | F-P1: Re-anchored wave-identity; ADR-025 lock reconciliation; harness capability confirmation; PostCompact best-effort; TOML corrections; VP-086 allocated. §Traceability ARCH-INDEX v2.28→v2.29. |
| v1.0 | 2026-06-14 | architect | Initial ADR for issue #173 / E-18 context-durability. Decisions 1–13, HANDOFF.md schema, Wave-Identity Derivation, Crash-Consistency Design, VP Allocations VP-081..VP-086. |
