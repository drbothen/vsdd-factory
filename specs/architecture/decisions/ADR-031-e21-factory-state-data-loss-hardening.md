...
- **INV-E21-004 (Worktree Teardown Preflight):** Before `git worktree remove <story-worktree>`,
  ...
  POSIX `test -d` follows symlinks so a symlink-to-directory satisfies `[ -d ]`; POSIX `find`
  without `-H`/`-L` does not descend symlinks and returns empty output, falsely authorizing
  teardown and allowing `rm -rf` to destroy the symlink target);
  (3) real directory → run `find "<worktree-path>/.factory/" -type f` (trailing slash —
  defense-in-depth; `2>/dev/null` FORBIDDEN; bats harness machine-rejects suppressed form);
...
- **PC2 — Teardown preflight:** Before `git worktree remove <story-worktree>`, apply the
  ...
  POSIX `test -d` follows symlinks; POSIX `find` without `-H`/`-L` returns empty for symlinks
  → false PC2a escape);
