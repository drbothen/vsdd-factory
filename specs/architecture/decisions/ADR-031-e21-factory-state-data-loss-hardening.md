...
+ INV-E21-004).** Because `.factory/` is listed in `.gitignore` on the product branch,
`git worktree add` checks out nothing at `<worktree-path>/.factory/`; no shadow directory
exists after worktree creation. An agent writing to `.factory/**` using a CWD-relative path
from the story-worktree root silently creates and populates the shadow subdirectory rather than
writing to the main-checkout `.factory/` worktree. Crucially, git's clean-state check in
`git worktree remove` does not flag gitignored content as "untracked" — the gitignored-shadow
is a false negative in git's own sanity check — so `git worktree remove` deletes the worktree
and the shadow `.factory/` subtree it contains without warning.
...
**Decision 1 — E-21 invariant catalog.** Define six cross-cutting invariants as the
...
- **INV-E21-004 (Worktree Teardown Preflight):** Before `git worktree remove <story-worktree>`,
  apply the 4-step fail-closed discrimination chain (normative source: BC-6.26.001 PC2):
  (1) `[ ! -e "<worktree-path>/.factory" ]` — nothing at path → PC2a(a) proceed immediately;
  (2) `[ -L "<worktree-path>/.factory" ]` (symlink, ANY target type) OR non-directory inode at
  path → PC2b BLOCKED, `find` NOT invoked (the `[ -L ]` test MUST precede any `[ -d ]` test:
  POSIX `test -d` follows symlinks so a symlink-to-directory satisfies `[ -d ]`; without the
  `[ -L ]` guard the step would fall through to the `find` branch; with the mandated
  trailing-slash form, POSIX pathname resolution dereferences a symlink-to-directory, causing
  `find` to enumerate files from the target directory outside the worktree boundary — an
  out-of-scope traversal; `rm -rf` removes only the symlink entry, not the target);
  (3) real directory → run `find "<worktree-path>/.factory/" -type f` (trailing slash;
  `2>/dev/null` FORBIDDEN; bats harness machine-rejects suppressed form);
  (4) empty output → PC2a(b) proceed; non-empty output → PC2b BLOCKED; `find` exits non-zero →
  PC2c HALT, fail-closed (path-nonexistence unreachable after step 1; symlink-at-path unreachable
  after step 2). The git worktree command silently deletes the shadow subtree.
...
- **PC2 — Teardown preflight:** Before `git worktree remove <story-worktree>`, apply the
  4-step fail-closed discrimination chain per BC-6.26.001 PC2:
  (1) `[ ! -e "<worktree-path>/.factory" ]` → nothing at path → PC2a(a) proceed;
  (2) `[ -L "<worktree-path>/.factory" ]` (symlink, ANY target type) OR non-directory inode →
  PC2b BLOCKED, `find` NOT invoked (the `[ -L ]` test MUST precede any `[ -d ]` test:
  POSIX `test -d` follows symlinks, so a symlink-to-directory satisfies `[ -d ]`; with the
  mandated trailing-slash form, POSIX pathname resolution dereferences a symlink-to-directory,
  causing `find` to enumerate files from the target directory outside the worktree boundary —
  an out-of-scope traversal that cannot reliably detect stray worktree content);
  (3) real directory → `find "<worktree-path>/.factory/" -type f` (trailing slash; `2>/dev/null`
  FORBIDDEN; bats harness machine-rejects suppressed form);
  (4) empty output → PC2a(b) proceed; non-empty output → PC2b BLOCKED; `find` exits non-zero →
  PC2c HALT (fail-closed; path-nonexistence unreachable after step 1; symlink-at-path unreachable
  after step 2).

**Enforcement surfaces (five, mirroring BC-6.26.001 §Architecture Anchors):**

- **Primary protocol — `plugins/vsdd-factory/skills/deliver-story/steps/_shared-context.md`:**
  The §Spec-Path Discipline clause is extended with an explicit write-discipline mandate covering
  all `.factory/**` writes (INV-E21-002 instantiation; S-21.04 deliverable).

- **Primary protocol — `plugins/vsdd-factory/skills/deliver-story/steps/step-g-cleanup.md`:**
  The teardown preflight sub-step is added before the `git worktree remove` dispatch
  (INV-E21-004 instantiation; discrimination-chain logic is defined solely here per §Rationale
  dispatch-point ruling; S-21.04 deliverable).

- **Executor-side verification-and-delegation — `plugins/vsdd-factory/agents/devops-engineer.md`:**
  Before issuing `git worktree remove` on a story worktree, devops-engineer MUST have obtained a
  PASS §G.1 preflight result; when not evident from dispatch context, the agent executes the §G.1
  procedure by reference only — no re-implementation of discrimination-chain logic (which remains
  solely in step-g-cleanup.md). Driving obligation: BC-6.26.001 Precondition 3 ("no exceptions"
  per Invariant 2). For the full qualification see §Rationale — executor-side verification-and-
  delegation is defense-in-depth; caller-side gating via step-g-cleanup.md §G.1 is PRIMARY.

- **Awareness surface — `plugins/vsdd-factory/agents/adversary.md`:** The adversary agent must
  recognize the gitignored-shadow mechanism and discrimination-chain protocol when reviewing
  story-worktree lifecycle operations (BC-6.26.001 Invariant 5 / story AC-009).

- **Awareness surface — `plugins/vsdd-factory/skills/adversarial-review/SKILL.md`:** The
  adversarial-review skill must incorporate the shadow-factory discrimination chain as a known
  failure class when reviewing deliver-story step execution (BC-6.26.001 Invariant 5 /
  story AC-009).
...
**Qualification — executor-side verification-and-delegation layer (F-S2104-P6-005 adjudication):**
This ruling does not preclude a complementary executor-side verification layer. An executor
(devops-engineer) that receives a dispatch to execute `git worktree remove` on a story worktree
MUST verify that a PASS §G.1 preflight result is available. When not evident from the dispatch
context, the executor invokes the §G.1 procedure BY REFERENCE — the 4-step discrimination chain
logic stays exclusively in `step-g-cleanup.md §G.1`; devops-engineer does not re-implement it.
This is verification-and-delegation, not co-location of `.factory/` inventory logic. The coupling
concern in point (2) above ("embedding `.factory/` inventory logic there") is not triggered by a
delegation call: the `.factory/`-specific knowledge remains owned solely by `step-g-cleanup.md`.
Caller-side gating via `step-g-cleanup.md §G.1` remains PRIMARY; the executor layer is
defense-in-depth. Driving invariant: BC-6.26.001 Invariant 2 ("no exceptions" — `git worktree
remove` MUST NOT execute on a story worktree without a confirmed PASS preflight result, regardless
of which agent holds the dispatch context). Stable cite: story S-21.04 AC-008.
[Closes adversary F-P2-007. Qualification closes F-S2104-P6-005.]
...
5. **Post-rebase gate host surface (F-P1-006 ruling):** The diff-integrity gate for INV-E21-005
   ...
   [DEFERRED — E-21 W2 WAVE GATE: BC-5.44.001 v1.3 and S-21.02 v1.1 both cite
   "ADR-031 v1.1 §Consequences #5" — a load-bearing ADR version pin. This violates POLICY 19
   (TD-VSDD-091 anti-volatile-pin: narrative spec content must cite behavioral anchors, not
   versioned-file forms). Both target artifacts are outside S-21.04 perimeter. Tracked here for
   E-21 W2 wave gate sweep. Stable cite form: "ADR-031 §Consequences #5 (post-rebase gate host
   surface)". Do NOT edit BC-5.44.001 or S-21.02 before the wave gate. Recorded at ADR-031
   v1.14 by architect.]
...
|---------|------|-------------|
| 1.15 | 2026-07-28 | F-S2104-P26-H02 three retracted BC-6.26.001 v1.12/v1.13 mechanism claims corrected (architect; D-938). (1) §Decision 1 INV-E21-004 step (2): retracted claim "POSIX find without -H/-L does not descend symlinks and returns empty output, falsely authorizing teardown and allowing rm -rf to destroy the symlink target" replaced with verified mechanism — without the `[ -L ]` guard the step falls through to `find`; with the mandated trailing-slash form, POSIX pathname resolution dereferences a symlink-to-directory, enumerating files from outside worktree boundary (out-of-scope traversal); `rm -rf` removes only the symlink entry, not the target. (2) §Decision 4 PC2 step (2): retracted claim "POSIX find without -H/-L returns empty for symlinks → false PC2a escape" replaced with verified mechanism — POSIX test -d follows symlinks (symlink-to-directory satisfies [ -d ]); mandated trailing-slash find dereferences the symlink via POSIX pathname resolution, enumerating files from target outside worktree boundary (out-of-scope traversal that cannot reliably detect stray worktree content). (3) §Decision 1 step (3): "trailing slash — defense-in-depth" removed — trailing slash dereferences rather than protects; [ -L ] guard in step 2 is the actual protection. All three corrections are consistent with BC-6.26.001 v1.12 §Description step 2 + §Postconditions non-directory/symlink-path paragraph + EC-008/T-7 contrast text. [ -L ] guard placement, PC2a/PC2b/PC2c routing, and all enforcement surface obligations unchanged. ARCH-INDEX v3.36→v3.37. |
| 1.14 | 2026-07-28 | F-S2104-P25-M03 §Decision 1 lead-in corrected five→six invariants (architect). INV-E21-006 appended at v1.1 was not swept into the §Decision 1 lead-in aggregation cell — identical count-sweep class as §Decision 7 Four→Five fix (v1.2). TD-VSDD-060 whole-document count sweep: §Context "Five independent failure modes" (5 issues: #342/#365/#358/#523/#588 — CORRECT); §Decision 2 "two independent layers" (2 — CORRECT); §Decision 4 "Two required agent actions" (PC1+PC2 — CORRECT); §Decision 5 "Two required protocol elements" (2 — CORRECT); §Decision 7 "Five new capability entries" (CAP-034..CAP-038 — CORRECT); §Decision 8 "Two required post-action assertions" (2 — CORRECT); §Rationale "four invariants" (INV-E21-002/003/004/005 — CORRECT). §Decision 1 "five" only defect found. §Consequences #5 DEFERRED tracking note added: BC-5.44.001 v1.3 + S-21.02 v1.1 cite "ADR-031 v1.1 §Consequences #5" — load-bearing version pin; POLICY 19 / TD-VSDD-091 violation; outside S-21.04 perimeter; tracked for E-21 W2 wave gate. D-937. |
| 1.13 | 2026-07-25 | F-S2104-P10-005 §Decision 4 five-surface enumeration (architect): §Decision 4 extended to enumerate all five obligation surfaces with obligation classes — primary protocol (×2: `_shared-context.md`, `step-g-cleanup.md`), executor-side verification-and-delegation (×1: `agents/devops-engineer.md`; §Rationale qualification pointer; caller-side gating PRIMARY), awareness surfaces (×2: `agents/adversary.md` and `skills/adversarial-review/SKILL.md`; BC-6.26.001 Invariant 5 / story AC-009). Mirrors BC-6.26.001 v1.11 five-surface classification for consistency. |
| 1.12 | 2026-07-25 | F-S2104-P6-005 adjudication (architect): §Rationale F-P2-007 qualified — caller-side primary ruling preserved; executor-side verification-and-delegation layer (AC-008; BC-6.26.001 Invariant 2 "no exceptions") reconciled as non-coupling: §G.1 logic stays in step-g-cleanup.md; devops-engineer invokes by reference, does not re-implement inventory logic. Whole-file sweep: no other text contradicts executor layer. |
| 1.11 | 2026-07-25 | F-S2104-P5-001 closure (architect): INV-E21-004 and §Decision 4 PC2 propagated to BC-6.26.001 v1.7 4-step discrimination chain — existence pre-test (`[ ! -e ]`), symlink/non-directory guard (`[ -L ]` MUST precede any `[ -d ]` test, `find` NOT invoked when symlink/non-directory found), trailing-slash `find "<worktree-path>/.factory/" -type f` on real directory only, PC2c unreachability note (path-nonexistence unreachable after step 1; symlink-at-path unreachable after step 2). Pre-v1.6 find-first three-branch construction and `non-path-absent` PC2c carve-out removed from both sites. TD-VSDD-060 class sweep: find-first constructions, non-path-absent phrases, non-trailing-slash `.factory" -type f` forms, absent-DIR phrasing — zero live occurrences post-fix (changelog rows exempt). |
