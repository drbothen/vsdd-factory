...
§12.3 annotation first-sentence correction in v1.22.
...
> **ADR-032 supersedes-in-part (v1.20):** ADR-032 implements payload-targeted enforcement,
...
> the Edit explicitly sets `timestamp:` at column 0. If YES, full reconstruction and
> comparison (this section) applies. If NO, the Edit is payload-neutral and returns `Continue`
> immediately — no reconstruction, no fragment check. The §12.2 warning targets a hypothetical
> implementation that treats ALL Edits as fail-open regardless of payload; ADR-032's
> payload-targeted approach closes that gap for explicit-timestamp Edits while eliminating
> spurious blocks for body-only Edits.
...
> **ADR-032 supersedes-in-part (v1.22):** For `Edit` and `MultiEdit` tools, ADR-032 Decision 1
...
> the guard returns `Continue` immediately, bypassing all §12.3 rows for that invocation. The §12.3 rows below
