...
host-ABI additive code + guard read-cap amendment in v1.7, and one host-function additive
amendment in v1.8.
...
**Signature:** `read_prefix(path: &str, max_bytes: u64) → i64`
...
| `CAPABILITY_DENIED (-1)` | Path not covered by `read_file.path_allow` capability block |
...
matters; for frontmatter-only parse (Decision 14), truncation at 262144 bytes is
functionally harmless because the frontmatter `---` closing delimiter appears before that
bound in all observed and worst-case STATE.md sizes.

**Capability block:** WASM plugins reuse the existing `[hooks.capabilities.read_file]`
block (with `path_allow`) — no new TOML capability key is introduced.
...
`host::read_prefix` eliminates all `OUTPUT_TOO_LARGE` risk on large STATE.md files,
making `STATE_MD_MAX_BYTES` a soft prefix-read bound rather than a hard file-size cap.
...
| D18 | `host::read_prefix` host function | `crates/factory-dispatcher/src/host.rs` (new `read_prefix` function); `crates/hook-sdk/src/host.rs` (new extern `read_prefix` declaration) | Additive host function per Decision 15. Signature: `read_prefix(path: &str, max_bytes: u64) → i64`. Contract: never returns `OUTPUT_TOO_LARGE`; truncates output to `max_bytes` bytes and returns truncated byte count. Returns `NOT_FOUND (-5)` for absent path (per Decision 13); `CAPABILITY_DENIED (-1)` for path not in `read_file.path_allow`; `INTERNAL_ERROR (-99)` for I/O fault. Reuses same `[hooks.capabilities.read_file]` capability key as `read_file` — no new TOML key. HOST_ABI_VERSION = 1 unchanged. Bats unit tests MUST cover: (a) prefix ≤ file_length → byte-exact prefix content returned, exit 0; (b) prefix > file_length → entire file content returned without error, exit 0; (c) absent path → NOT_FOUND (-5), exit 0; (d) path outside `path_allow` → CAPABILITY_DENIED (-1), exit 0; (e) `verify-factory-lock` plugin replaces `read_file` call with `read_prefix` and STATE.md frontmatter is parsed correctly at 262144 byte bound. |
