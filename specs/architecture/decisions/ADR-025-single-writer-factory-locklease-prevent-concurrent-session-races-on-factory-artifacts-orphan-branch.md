...
(`verify-factory-lock` and `verify-state-timestamp-refresh`) supply max_bytes=8192 per
BC-4.13.001 §Precondition 3 Phase-B — sufficient for any realistic STATE.md frontmatter
block (the `---` closing delimiter always appears well before 8192 bytes). The 262144-byte
figure is the Phase-A `host::read_file` cap from Decision 14 and is not the call-site
argument for `read_prefix`.
...
`host::read_prefix` (max_bytes=8192 per BC-4.13.001 §Precondition 3 Phase-B) eliminates
all `OUTPUT_TOO_LARGE` risk on large STATE.md files. `STATE_MD_MAX_BYTES` is removed in
its entirety at S-19.07 (BC-4.13.001 Phase-B); the 8192 call-site argument in the
