...
§12.3 annotation first-sentence correction in v1.22, one §12.3 Continue-phrasing parity correction in v1.23, one §12.2 supersedes-in-part two-field definition + Continue-phrasing correction in v1.24, and one boundary-operator correction (`now > expires_at` → `now >= expires_at`; mirrored `now <= expires_at` → `now < expires_at`) in v1.25.
...
- **Guard logic:** reads `STATE.md` via `host::read_file`, parses the `factory_lock`
  frontmatter block, checks `holder != current_git_email` and `now < expires_at`
  (`is_expired(now, expires_at)` returns `false`; boundary consistent with Decision 5). If
...
- The guard computes `now >= expires_at` on every check (`is_expired(now, expires_at)` at
  `crates/hook-plugins/verify-factory-lock/src/lib.rs`; EC-002: the exact-expiry instant
  `now == expires_at` is treated as expired). An expired lock is treated as absent — the
  check passes and the operation proceeds.
