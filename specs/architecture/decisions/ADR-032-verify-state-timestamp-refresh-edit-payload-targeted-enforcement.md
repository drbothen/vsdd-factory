...
related_adrs:
  - ADR-025 (single-writer factory lock/lease — STATE.md write discipline; factory_lock schema; §12.2/§12.3 superseded-in-part by ADR-032 for payload-neutral Edit/MultiEdit; amended to v1.24 for §12.2 supersedes-in-part two-field definition + §12.3 Continue-phrasing parity)
...
neither field is set), the guard skips Steps 4–7 entirely, then returns `Continue` at the payload-scan step (inserted after code-inline Step 3). A `guard_ran (continue: payload-neutral)`
...
- If `!sets_timestamp && !sets_factory_lock` (payload-neutral): skip Steps 4–7 entirely
  (returns Continue at the payload-scan step, inserted after code-inline Step 3).
...
- Slight weakening of the per-Edit guarantee: a burst that issues only body-only Edits (zero
  ...
  scoped into the Implementer Work Spec below) fires on Bash git-commit events; `detect_git_commit_event`
  qualifies ANY PostToolUse Bash command containing `"git"`, `" commit"`, and `".factory"` — including
  develop-branch commits whose command text mentions `".factory"`. The advisory reads the factory-artifacts
  worktree HEAD via `build_git_context(factory_dir)` and compares STATE.md `timestamp:` between HEAD and
  HEAD^, detecting "burst committed without advancing timestamp" at the per-commit boundary — the correct
  detection point for the ADR-025 §12.2 invariant. Advisory-only; `on_error=continue`; no blocking.
  Develop-branch Bash commands mentioning `".factory"` can trigger a spurious advisory fire; the advisory
  keys off the factory-artifacts worktree's OWN latest commit rather than the triggering command and is
  self-describing (the emitted message MUST describe the factory-artifacts HEAD state, not attribute to the
  triggering command). **Option (a) accepted-with-disclosure (F-ADR032-P6R-002):** spurious fires are
  bounded (advisory-only, `on_error=continue`) and self-identifying. False positives on factory-artifacts
  commits that do not touch STATE.md are eliminated by Pre-condition 2 (`state_md_in_commit == true`,
...
- **Timestamp-deletion sub-case — explicit disclosure (CWE-693 LOW, accepted-with-disclosure; PR #742 Finding 3 + security-review adjudication 2026-07-21).** An Edit/MultiEdit whose `old_string` contains the `timestamp:` line and whose `new_string` does NOT set `timestamp:` (field deletion — the field is removed rather than advanced) is payload-neutral per Decision 1: `sets_timestamp == false` → skip Steps 4–7 → `Continue`. The enforcement anchor field can therefore be stripped from STATE.md frontmatter without triggering a block. Residual nuance beyond the general payload-neutral disclosure: after a committed deletion, the on-disk STATE.md has no `timestamp:` field. ADR-025 §12.3's "timestamp: absent in on-disk content → Continue" row means the NEXT Edit that explicitly sets `timestamp:` also passes enforcement unconditionally (the absent-in-on-disk row exits early with `Continue`, regardless of the value being set — the guard does not re-engage until a Write path or a lock re-acquire). AC-021 does NOT fire for the deletion commit: after a committed deletion, `head_state_timestamp` is empty/absent and `head_parent_state_timestamp` has a value — these are not byte-identical, so the advisory treats the absent-field commit as an "advancement." The deletion sub-case therefore has no automated compensating control at the per-Edit, per-commit, or per-subsequent-Edit level. Bounding controls: (i) state-manager process discipline never issues an Edit whose purpose is to delete the `timestamp:` field — the operation is outside the defined state-manager workflow; (ii) `factory-lock-write.sh` `acquire` fails loudly with a `SchemaViolation` when `timestamp:` is absent from STATE.md frontmatter, surfacing any prior deletion on the next lock-acquisition attempt; (iii) the Write-path enforcement remains unconditional (Decision 2) — a full-file Write is blocked if the proposed content lacks or does not advance `timestamp:`. Implementation: behavior is pinned by the `ac020_edit_timestamp_line_deletion_payload_neutral_continues` and `ac020_multiedit_timestamp_line_deletion_payload_neutral_continues` unit tests (Edit + MultiEdit paths) added on the PR #742 branch.

### Status

ACCEPTED 2026-07-20. Strict 3-CLEAN convergence achieved per BC-5.39.001: passes 9, 10, and 11 each returned CLEAN (B0/H0/M0/L0) by independent fresh-context adversaries against frozen commit bc7f6d8b (ADR-032 pin 16533eeb…; ADR-025 pin e73e19f2…; ARCH-INDEX pin 31711996…). Each reviewer explicitly staked the acceptance gate. Convergence mode: human-directed STRICT (superseding an earlier asymptotic-acceptance selection). Cascade totals: 11 passes total (8 genuine fresh-context adversary + P5 aborted-on-moving-target + 2 additional), 7 fix bursts, 41 findings closed, 3 integrity events (D-867/D-868).

Implementation authorized via fix-pr-delivery (D-869); route to `vsdd-factory:implementer` per §Implementer Work Spec (unchanged). An rc.24 release build is required for the operator-cache to receive the new hook plugin. The four Red Gate tests must fail against the current unmodified `guard_logic` and pass only after the fix, and the full existing unit suite (50 tests as of this writing; `grep -c '#\[test\]' src/lib.rs → 50`) must remain green.
...
- **ADR-025 §12.2** (every STATE.md commit must advance `timestamp:`; authoritative source of the
  ...
  **TimestampStale rows** apply ONLY when `sets_timestamp == true` — a factory_lock-only Edit
  ...
  ADR-032 is payload-targeted, not fragment-only. See ADR-025 v1.24 §12.3 for the matching
...
```
$ grep -nE '^priority = (15[0-9]|160)$' plugins/vsdd-factory/hooks-registry.toml
939:priority = 150
958:priority = 152
980:priority = 152
1004:priority = 151
...
**Pre-condition 1 (trigger heuristic scope):** `detect_git_commit_event` qualifies ANY PostToolUse
Bash command containing `"git"`, `" commit"`, and `".factory"` — including develop-branch commits
whose command text mentions `".factory"`. The advisory keys off the factory-artifacts worktree's OWN
latest commit (via `build_git_context(factory_dir)`) regardless of which Bash command triggered the
event. Spurious fires on develop-branch commits are possible (bounded: advisory-only, `on_error=continue`).
Pre-condition 2 (`state_md_in_commit == "true"`) is the PRIMARY discriminator: only events where the
factory-artifacts HEAD commit included STATE.md changes can emit the advisory. No separate discriminator
field is needed; Option (a) accepted-with-disclosure (see §Consequences Negative bullet (c)).
...
- `state_md_in_commit` absent or `"false"` (STATE.md not modified in the factory-artifacts
  worktree's HEAD commit; Pre-condition 2 not met; note: develop-branch Bash commands
  mentioning `.factory` CAN qualify Pre-condition 1 — the advisory keys off the
  factory-artifacts worktree's OWN HEAD regardless of which Bash command triggered the
  event; if that HEAD commit did not include STATE.md, this condition returns `Continue`;
  spurious advisory fires bounded: advisory-only, `on_error=continue`)
...
|---------|------|--------|---------|
| 1.14 | 2026-07-21 | architect | PR #742 security-review follow-up: §Consequences Negative timestamp-deletion sub-case explicit disclosure added. CWE-693 LOW, accepted-with-disclosure per security-review adjudication 2026-07-21 (Finding 3 from PR #742 fresh-eyes review). An Edit/MultiEdit whose `old_string` contains the `timestamp:` line and whose `new_string` does not set `timestamp:` (field deletion) is payload-neutral per Decision 1 → `Continue`; the enforcement anchor field can be stripped without a block. No automated compensating control at per-Edit, per-commit, or per-subsequent-Edit level; bounding controls named: (i) state-manager never deletes `timestamp:`; (ii) `factory-lock-write.sh` `acquire` fires `SchemaViolation` on absent `timestamp:`; (iii) Write-path enforcement unconditional (Decision 2). Behavior pinned by `ac020_edit_timestamp_line_deletion_payload_neutral_continues` + `ac020_multiedit_timestamp_line_deletion_payload_neutral_continues` unit tests on PR #742 branch. Append-only; no normative Decision or Work Spec content changed. ARCH-INDEX v3.24→v3.25. |
| 1.13 | 2026-07-20 | state-manager | D-869 acceptance burst: ADR-032 ACCEPTED 2026-07-20. Strict 3-CLEAN convergence (BC-5.39.001) at frozen commit bc7f6d8b — passes 9/10/11 CLEAN B0/H0/M0/L0 by independent fresh-context adversaries; human-directed STRICT mode (superseding asymptotic-acceptance selection). Cascade totals: 11 passes, 7 fix bursts, 41 findings closed, 3 integrity events (D-867/D-868). Human gate: Accept ADR-032; implement via fix-pr-delivery; draft follow-up stories (separate burst). status: proposed→accepted. §Status body refreshed. No Decision or Work Spec content changed. ARCH-INDEX v3.23→v3.24. |
| 1.12 | 2026-07-20 | architect | F-ADR032-P7R fix burst 6: 4-finding closure per genuine adversary pass 7. F-ADR032-P7R-001 (HIGH): §Second deliverable fail-open bullet corrected — "develop-branch commits are structurally excluded because `.factory/STATE.md` is not part of the develop working tree" contradicted the accepted-with-disclosure Pre-condition 1 design; replaced with correct text: develop-branch Bash commands mentioning `.factory` CAN qualify Pre-condition 1; advisory keys off factory-artifacts worktree OWN HEAD regardless of trigger; if that HEAD commit did not include STATE.md, Pre-condition 2 returns Continue; spurious fires bounded (advisory-only, on_error=continue). F-ADR032-P7R-002 (MEDIUM): ADR-025 §12.2 supersedes-in-part annotation corrected — header (v1.20)→(v1.24); single-field "Edit explicitly sets `timestamp:` at column 0" replaced with two-field condition (neither `timestamp:` nor `factory_lock:` in any new_string); "returns `Continue` immediately" → "returns `Continue` after skipping the timestamp and lock-expiry checks"; factory_lock-only Edit path explicitly noted (skips Steps 4–6, still runs Step 7). ADR-025 amended to v1.24 in lock-step. F-ADR032-P7R-003 (MEDIUM): ARCH-INDEX ADR-032 row leading current-state summary corrected — "return `Continue` immediately" → "return `Continue` after skipping the timestamp and lock-expiry checks"; "(ADR-025 amended to v1.22)" → v1.24; AMENDED v1.12 entry added; ADR-025 row AMENDED trail extended v1.23→v1.24. ARCH-INDEX v3.21→v3.22. F-ADR032-P7R-004 (LOW): v1.11 Changelog P6R-003 rationale correction note appended per append-only discipline — "code-inline scheme has no Step 8" misstated ground truth; code-inline Step 8 exists: `guard_logic` function has labeled comment "// Step 8: All checks passed — allow the write" (verified by `grep -n 'Step 8' crates/hook-plugins/verify-state-timestamp-refresh/src/lib.rs` → 983:comment); v1.11 body fix itself remains correct since payload-neutral Edits return `Continue` at the payload-scan step (inserted after code-inline Step 3), not by falling through to Step 8; v1.11 row text left intact. ADR-025 amended to v1.24 in lock-step. ARCH-INDEX v3.21→v3.22. |
| 1.11 | 2026-07-20 | architect | F-ADR032-P6R fix burst (fix burst 5): 4-finding closure per genuine adversary pass 6. F-ADR032-P6R-001 (HIGH): AC-021 evidence block command corrected — `grep -n "^priority = " ... \| grep -E "15[0-9]"` replaced with `grep -nE '^priority = (15[0-9]\|160)$'`; true stdout unchanged (12 matches; 159 unoccupied confirmed). F-ADR032-P6R-002 (HIGH): §Consequences Negative bullet (c) + Pre-condition 1 corrected — `detect_git_commit_event` qualifies ANY Bash command containing "git"+" commit"+".factory" including develop-branch commits; spurious advisory fires possible (bounded: advisory-only, on_error=continue); advisory keys off factory-artifacts worktree HEAD; Option (a) accepted-with-disclosure; advisory message MUST describe factory-artifacts HEAD state. F-ADR032-P6R-003 (MEDIUM): Decision 1 + Decision 3 "(code-inline numbering)" annotations corrected — code-inline scheme has no Step 8; corrected to "payload-scan step (inserted after code-inline Step 3)"; both sites fixed; sibling sweep confirms no additional occurrences in either ADR. F-ADR032-P6R-004 (LOW): §Source/Origin ADR-025 reference updated to v1.23 (§12.3 Continue-phrasing parity correction delegated to ADR-025 v1.23). ADR-025 amended to v1.23 in lock-step. ARCH-INDEX v3.20→v3.21. |
