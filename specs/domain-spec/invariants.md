...
Enforcement owner: SS-05 (validate-burst-log + validate-dispatch-advance apply the exemption); SS-07 (precompact-flush.sh produces the canonical commit message prefix, runs `git cat-file -t <SHA>` to embed the object-type token, and appends a structured line to `.factory/hooks/precompact-flush-log`). BC range: BC-5.41.003, BC-7.07.001.
