...
- The detector reads FIELD-4 (the `commit` token) of that last line STATICALLY — the shell hook (`precompact-flush.sh`) embedded that token at flush time; the WASM plugins (`validate-burst-log`, `validate-dispatch-advance`) perform NO live git exec (they have no `exec_subprocess` capability). The WASM check is: the last-line FIELD-4 token equals the candidate SHA string, and the last-line FIELD-5 token is `commit`.
- Both conditions must hold for the exemption to be granted: last-line presence in the append-only log AND last-line FIELD-5 token equals `commit`.
- A last-line FIELD-5 token that is absent, empty, or anything other than `commit` indicates log corruption or truncation; treat as stale/absent and deny the exemption.
