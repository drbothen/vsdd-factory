...
- The detector then runs `git cat-file -t <SHA>` and confirms the output is `commit`.
- Both conditions must hold for the exemption to be granted: last-line presence in the append-only log AND `git cat-file -t` returning `commit`.
- A SHA recorded in the log but absent from git (i.e., `git cat-file -t` fails or returns anything other than `commit`) indicates a write-before-push crash; treat as stale/absent and deny the exemption.
