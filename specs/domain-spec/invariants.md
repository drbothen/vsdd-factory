...
last_amended: "2026-06-28 (v1.26 — S-18.11 T-2 BC authorship (product-owner): POLICY 2 bidirectional DI→BC backward-link update: DI-020 BC range += BC-5.41.004 (sprint-state.yaml producer format obligation; upholds DI-020 by ensuring producer-side conformance closes the context-durability chain). DI-023 BC range += BC-5.41.004 + enforcement-owner prose += wave-scheduling skill (sprint-state.yaml producer). Both backward-links close the POLICY 2 gap: BC-5.41.004 Traceability cites DI-020+DI-023; this edit closes the reciprocal backward link. [Prior: v1.25 — F-P2-004 STORY PASS-2 FIX: DI-020 BC range += BC-6.25.001 (check-state-health preventive-advisory verification that CLAUDE_AUTOCOMPACT_PCT_OVERRIDE env-var prerequisite is configured; closing POLICY 2 bidirectional DI→BC backward-link gap: forward link BC-6.25.001→DI-020 existed in BC Traceability; backward link DI-020 BC range was missing BC-6.25.001). [Prior: v1.24 — D-615 E-18 STORY PASS-1 FIX WAVE INTEGRATION BURST: DI-020 Cited by += VP-092 (BC-6.25.001 check-state-health CLAUDE_AUTOCOMPACT_PCT_OVERRIDE advisory check; domain_invariants: [DI-020]; S-18.10; advisory-only; never blocks; pure settings.json read; upholds DI-020 by ensuring autocompact threshold configuration is visible to the pipeline operator). POLICY 2 bidirectional DI→VP traceability maintained.]])"
phase: 1.3
inputs:
...
> and the SS-NN subsystem that enforces it, OR (for DIs authored from
> post-crystallization capabilities such as CAP-032 / DI-020..025) to its
> `Source:` capability + ADR rather than a BR-NN business rule.
...
Enforcement owner: SS-05 (wave-gate + wave-handoff skill) for wave-boundary transitions; SS-07 (precompact-flush.sh) for intra-wave PreCompact transitions; SS-04 (validate-wave-handoff-completeness WASM gate) as parse-layer enforcer; SS-06 (check-state-health skill — BC-6.25.001) as preventive-advisory verifier that the CLAUDE_AUTOCOMPACT_PCT_OVERRIDE env-var prerequisite is configured, preventing the Part-B mid-wave-compaction losslessness arm from degrading silently. BC range: BC-5.41.001, BC-5.41.003, BC-5.41.004, BC-4.14.001, BC-1.15.001, BC-6.25.001.
...
**Cited by:** VP-081 (domain_invariants: [DI-020, DI-021, DI-023] — SHA pinned in precompact-flush-log as external ground truth for anti-fabrication), VP-082 (domain_invariants: [DI-021, DI-022, DI-025] — precompact-flush pins SHA in log for anti-fabrication/external-ground-truth), VP-085 (domain_invariants: [DI-021, DI-022, DI-025] — hermetic flush hook maintains anti-fabrication SHA corroboration).

**DI-022 — The PreCompact flush derives all flushed state exclusively from durable persisted sources**
The PreCompact flush hook (`precompact-flush.sh`) must read its inputs from STATE.md on the filesystem and from git commands only. It must not read from `custom_instructions`, environment variables set by the LLM, tool-call results visible only to the current context window, or any other in-context mechanism. This hermetic constraint closes the anti-deadlock risk (a flush that requires in-context state to operate would itself be unusable at the moment of compaction when that state is being lost). Lock-renewal before the flush commit is a mandatory step WHEN a factory lock is held (`factory_lock:` present in STATE.md frontmatter); when no lock is held (`factory_lock:` absent), the lock-renewal step is skipped entirely (no-op). Lock-renewal failure (when a lock is held) is advisory only and must not prevent the flush commit from proceeding.
...
**Cited by:** VP-082 (domain_invariants: [DI-021, DI-022, DI-025] — flush hook derives state exclusively from durable persisted sources, hermetic state derivation), VP-085 (domain_invariants: [DI-021, DI-022, DI-025] — hermetic state derivation from durable persisted sources).
...
Enforcement owner: SS-05 (wave-handoff skill; derives wave_id and next-wave story list from real substrate; wave-scheduling skill — sprint-state.yaml producer) and SS-06 (rehydrate-wave skill; reads wave-state.yaml from git, no RAG fallback). BC range: BC-5.41.001 (wave_id source), BC-5.41.002 (stories list derivation + no RAG), BC-5.41.004 (sprint-state.yaml producer topo-sort from STORY-INDEX.md depends_on; no phantom wave: field; no RAG status derivation), BC-6.24.001 (git-sourced manifest + no RAG + exact list semantics).
...
**Cited by:** VP-081 (domain_invariants: [DI-020, DI-021, DI-023] — handoff-completeness gate tracks session-context identity from real substrate fields), VP-087 (domain_invariants: [DI-023] — wave-state.yaml atomic production and real-substrate stories derivation per BC-5.41.002; stories list derives from sprint-state.yaml+dependency-graph.md, not LLM memory), VP-088 (domain_invariants: [DI-023] — rehydrate-wave reads wave-state.yaml from factory-artifacts via git, exact-list injection, no RAG fallback per BC-6.24.001; wave/phase identity derives from real persisted substrate).
...
**Cited by:** VP-089 (domain_invariants: [DI-024] — postcompact-reanchor.sh is best-effort advisory; reads STATE.md from factory-artifacts via git show (never working-tree); emits re-anchor block to stdout; makes NO factory-artifacts commits (commit-free invariant); exits 0 on all error paths; per BC-7.07.002; resolves BC-7.07.002 DEFERRED-VP disposition per D-607 F2 gate).

**DI-025 — PreCompact flush commits are lifecycle-orthogonal to state-manager burst commits**
A `PreCompact flush <cycle>/<step> <ISO-timestamp>` commit on `factory-artifacts` is produced by the PreCompact flush hook (BC-7.07.001) as a harness-event-triggered lifecycle commit, not as part of a state-manager burst (the A/B/C/D/E sequence per TD-VSDD-053). The MULTI_COMMIT_CHAIN_NOT_ALLOWED detector in `validate-burst-log` and `validate-dispatch-advance` must treat any commit whose subject matches the prefix `PreCompact flush ` as exempt from chain detection. The exemption is NOT a bare-prefix match alone: corroboration against the append-only log `.factory/hooks/precompact-flush-log` is required per BC-5.41.003 EC-003. Specifically:
- The detector reads `.factory/hooks/precompact-flush-log` (if the file exists) and checks whether the candidate SHA appears as the LAST LINE of that file.
- The detector reads FIELD-4 (the `commit` token) of that last line STATICALLY — the shell hook (`precompact-flush.sh`) embedded that token at flush time; the WASM plugins (`validate-burst-log`, `validate-dispatch-advance`) perform NO live git exec (they have no `exec_subprocess` capability). The WASM check is: the last-line FIELD-2 token equals the candidate SHA string (exact 40-char hex match), AND the last-line FIELD-4 token equals the literal string `commit`. Both must hold for EXEMPT. There is no FIELD-5 — the log line has exactly 4 fields.
- Both conditions must hold for the exemption to be granted: last-line presence in the append-only log AND last-line FIELD-4 token equals `commit`.
- A last-line FIELD-4 token that is absent, empty, or anything other than `commit` indicates log corruption or truncation; treat the log entry as stale and fall through: prefix match alone is sufficient — EXEMPT (same path as the no-log-yet case below). A SHA-mismatch where FIELD-4 IS `commit` (i.e., a syntactically valid log entry whose FIELD-2 does not match the candidate SHA) is NOT EXEMPT — this is the anti-forgery case, not the corruption case.
...
A commit bearing the `PreCompact flush ` prefix that presents a syntactically valid log entry (FIELD-4 = `commit`) whose FIELD-2 SHA does not match the candidate SHA is not exempt (anti-forgery). A commit whose log entry is corrupted or truncated (FIELD-4 absent/empty/≠ `commit`) is treated as stale and is exempt via prefix-match-alone. The prefix match is case-sensitive on the raw commit subject; it is not NLP inference. The exemption — prefix-match AND append-log last-line SHA corroboration (static FIELD-2/FIELD-4 read, no live git exec in WASM) when the log exists — must be implemented symmetrically in both hooks. Shell-context hooks (wave-gate, wave-handoff skills) MAY exec git for their own purposes; this exception applies to those non-WASM contexts only. A burst-log entry must not cite a PreCompact flush commit as Commit A/B/C/D/E.
Enforcement owner: SS-05 (validate-burst-log + validate-dispatch-advance apply the exemption); SS-07 (precompact-flush.sh produces the canonical commit message prefix, embeds the object-type token `commit` into the log line (the shell hook MAY use `git cat-file -t <SHA_B>` to derive this value, or embed the literal string `commit` directly — both yield `commit` for a just-created commit; the shell CAN exec git, unlike WASM validators which cannot), and appends a structured line to `.factory/hooks/precompact-flush-log`). BC range: BC-5.41.003, BC-7.07.001.
Justification: DI-025 is a business invariant because the TD-VSDD-053 single-commit-per-burst discipline is a core factory-governance rule. Without an explicit lifecycle boundary, every PreCompact flush commit would produce a false-positive MULTI_COMMIT_CHAIN_NOT_ALLOWED block, making the factory unworkable after any compaction event. The lifecycle boundary is the domain-level rule that keeps both disciplines (durability + governance) simultaneously satisfied. Source: ADR-026 §Crash-Consistency Design §Decision A (as of v1.6) (append-only log replaces point-file); CAP-032 (capabilities.md §CAP-032); TD-VSDD-053.
**Cited by:** VP-082 (domain_invariants: [DI-021, DI-022, DI-025] — PreCompact flush commit is lifecycle-orthogonal to state-manager burst commit), VP-084 (domain_invariants: [DI-020, DI-025] — commit-token field-4 verified by WASM via static read; lifecycle-orthogonal commits), VP-085 (domain_invariants: [DI-021, DI-022, DI-025] — lifecycle-orthogonal flush commits), VP-090 (domain_invariants: [DI-025] — precompact-flush-log pruning via precompact-flush-prune.sh preserves last-line invariant (most-recent SHA survives prune) so validate-burst-log and validate-dispatch-advance can always corroborate most-recent flush SHA via FIELD-2/FIELD-4 static read; per BC-7.07.001 Inv7).

## CHANGELOG

| Version | Date | Change |
|---------|------|--------|
| v1.25 | 2026-06-16 | F-P2-004 STORY PASS-2 FIX (business-analyst): DI-020 BC range += BC-6.25.001. Added SS-06 (check-state-health skill) as a named enforcement owner with a clause noting it is the preventive-advisory verifier that the CLAUDE_AUTOCOMPACT_PCT_OVERRIDE env-var prerequisite is configured, preventing the Part-B mid-wave-compaction losslessness arm from degrading silently. Closes POLICY 2 bidirectional DI→BC backward-link gap: BC-6.25.001's Traceability forward-linked DI-020 (added at v1.24 via VP-092 Cited by); DI-020's BC range was missing the reciprocal backward link. DI-020 normative invariant statement unchanged. |
| v1.24 | 2026-06-16 | D-615 E-18 STORY PASS-1 FIX WAVE INTEGRATION BURST: DI-020 `Cited by:` += VP-092 (BC-6.25.001 check-state-health CLAUDE_AUTOCOMPACT_PCT_OVERRIDE advisory check; domain_invariants: [DI-020]; S-18.10; advisory-only; never blocks; pure settings.json read; upholds DI-020 by ensuring autocompact threshold configuration is visible to the pipeline operator). POLICY 2 bidirectional DI→VP traceability maintained. |
| v1.23 | 2026-06-16 | D-612 INTEGRATION BURST (state-manager): DI-020 `Cited by:` += VP-091 (domain_invariants: [DI-020]; validate-heavy-op-delegation WASM gate; BC-4.15.001; S-18.06; advisory-only; pure-parse; never blocks; upholds DI-020 by design). POLICY 2 bidirectional DI→VP traceability maintained. |
| v1.22 | 2026-06-16 | D-607 F2 E-18 spec-completion INTEGRATION BURST (state-manager): DI-023 `Cited by:` += VP-087 (BC-5.41.002 wave-state.yaml atomic production + real-substrate stories derivation), VP-088 (BC-6.24.001 rehydrate-wave git-sourced manifest + no RAG fallback). DI-024 `Cited by:` placeholder "(VP deferred to F3 / S-18.05 per BC-7.07.002 DEFERRED-VP disposition)" replaced with VP-089 (BC-7.07.002 postcompact-reanchor.sh best-effort advisory + commit-free invariant; resolves DEFERRED-VP). DI-025 `Cited by:` += VP-090 (BC-7.07.001 Inv7 precompact-flush-log pruning; last-line invariant preserved across prune). POLICY 2 bidirectional DI→VP traceability complete for DI-023/024/025. |
| v1.21 | 2026-06-15 | F-P40-001 MEDIUM + O-P40-001 LOW fix: (F-P40-001) Added `**Cited by:** VP-NNN[, VP-NNN...]` back-reference lines to DI-020..DI-025, completing POLICY-2 bidirectional DI→VP traceability. DI-019 already carried a `Cited by:` line establishing the norm; DI-020..025 lacked it despite VP-081..086 forward-referencing them in `domain_invariants:` frontmatter (gap introduced at F-P19-003). Verified mapping against VP-INDEX Full Index Domain Invariant column (VP-081: [DI-020,DI-021,DI-023]; VP-082: [DI-021,DI-022,DI-025]; VP-083: [DI-020]; VP-084: [DI-020,DI-025]; VP-085: [DI-021,DI-022,DI-025]; VP-086: [DI-020]). DI-024 has no citing VP yet — `Cited by:` line reads "(VP deferred to F3 / S-18.05 per BC-7.07.002 DEFERRED-VP disposition)" so the absence is explained. (O-P40-001) Preamble updated to acknowledge that DIs authored from post-crystallization capabilities (DI-020..025, CAP-032) trace to their `Source:` capability + ADR rather than a BR-NN business rule — the prior preamble claim ("each traces to its BR-NN rule in business-rules.md") was inaccurate for this cohort. |
| v1.20 | 2026-06-15 | F-P24-002 MEDIUM fix (re-aligned, correcting an over-correction): DI-025 Enforcement-owner clause (line 212) re-aligned to canonical (B). The prior edit incorrectly extended the WASM-only no-git-exec prohibition to the shell hook, stating "no live git cat-file execution" for `precompact-flush.sh`. The v1.16 BLOCKER (F-P5-001) excised live git exec from WASM plugins ONLY — the shell hook (`precompact-flush.sh`) retains exec capability and MAY use `git cat-file -t <SHA_B>` at write time to derive the `commit` token, or may embed the literal string `commit` directly (both yield `commit` for a just-created commit). Revised enforcement-owner clause now reads: "the shell hook MAY use `git cat-file -t <SHA_B>` to derive this value, or embed the literal string `commit` directly — both yield `commit` for a just-created commit; the shell CAN exec git, unlike WASM validators which cannot." DI-025 is now internally consistent: shell MAY exec at write time; WASM validators (SS-05) read FIELD-2/FIELD-4 statically and never exec; FIELD-4==`commit` is the structural corroboration; FIELD-2 SHA match is the anti-forgery gate. Reconciles DI-025 with ADR-026 §Decision 6 / §Crash-Consistency lines 525–529 and BC-7.07.001 PC8 ("NOT a live cat-file call at validation time" = WASM-scoped, (B)-consistent). |
| v1.19 | 2026-06-15 | F-P16-005 LOW fidelity fix: DI-022 conditional-no-op qualifier added per BC-7.07.001/ADR-025/ADR-026. Lock-renewal sentence updated from unconditional "mandatory step" to conditional "mandatory step WHEN a factory lock is held (`factory_lock:` present); no-op when no lock is held (`factory_lock:` absent)." Advisory-failure clause scoped to the lock-held case. No behavioral change; wording now matches BC-7.07.001 PC3/Inv3/EC-009 and ADR-025 opt-in lock model. |
| v1.18 | 2026-06-14 | F2 pass-13 BEHAVIORAL fix (F-P13-001 BLOCKER): DI-025 FIELD-4-corruption outcome corrected. Prior text routed FIELD-4-absent/empty/≠`commit` to "deny the exemption" (NOT EXEMPT) — contradicting BC-5.41.003 PC1 case (b)/(c), Inv1 gate (3), ADR-026 §F-P4-004 EC-003, all of which route corruption to stale-entry → prefix-match-alone → EXEMPT. Line 209 corrected: FIELD-4-corruption now routes to stale/fall-through → prefix-alone → EXEMPT, explicitly distinct from the SHA-mismatch-with-valid-FIELD-4 case (anti-forgery, stays NOT EXEMPT). Line 211 corrected: the "is not exempt" sentence now covers only the syntactically-valid-FIELD-4 SHA-mismatch (anti-forgery) case; FIELD-4-corruption case is explicitly EXEMPT via prefix-match-alone. The stale-entry→prefix-alone path is now internally consistent with the no-log-yet case (line 210) and with the enforcing BC/ADR. |
| v1.17 | 2026-06-14 | F2 pass-6 BEHAVIORAL fix (F-P6-003 BLOCKER): DI-025 field-numbering error corrected. Precompact-flush-log line format has exactly 4 fields (`<ISO-timestamp> <SHA> <cycle>/<step> commit`): SHA is FIELD-2 (not FIELD-4), `commit` token is FIELD-4 (not FIELD-5), there is no FIELD-5. Three occurrences fixed: (1) WASM check clause corrected to FIELD-2=SHA + FIELD-4=`commit`, with explicit "no FIELD-5" statement; (2) "Both conditions" clause updated FIELD-5→FIELD-4; (3) "absent/empty/other" corruption clause updated FIELD-5→FIELD-4. Parenthetical on line 211 updated FIELD-4/FIELD-5→FIELD-2/FIELD-4. Intro at line 206 ("detector reads FIELD-4, the `commit` token") was already correct and left unchanged. Justification source cite updated: `ADR-026 v1.5 §Crash-Consistency Design §Decision A` → `ADR-026 §Crash-Consistency Design §Decision A (as of v1.6)` (drop load-bearing version token per new cite convention; informational parenthetical retained). |
| v1.16 | 2026-06-14 | F2 pass-5 BEHAVIORAL fix (F-P5-001 BLOCKER + O-P5-002): DI-025 corrected to remove two git-exec clauses (`git cat-file -t <SHA>` EXEC) that incorrectly prescribed live git execution inside WASM plugins. Replaced with static FIELD-4/FIELD-5 read semantics: WASM plugins read the pre-embedded object-type token from the last line of the append-only log (embedded by `precompact-flush.sh` at flush time via `git cat-file -t`); WASM has no `exec_subprocess` capability. Shell-context exception (wave-gate/wave-handoff skills MAY exec git) documented. "write-before-push crash" condition (mechanically impossible — append occurs after the commit exists locally) replaced with "log corruption or truncation" as the correct failure mode. Enforcement owner updated: SS-07 description now reflects that `precompact-flush.sh` embeds the object-type token into the log line. Justification source cite bumped ADR-026 v1.4→v1.5 §Crash-Consistency Design §Decision A (O-P5-002: `### Decision A` anchor now exists in ADR-026 v1.5). |
