...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-490"
ss_section: "Artifact and verification skills"
skill: "perf-check"
---

# Behavioral Contract BC-6.14.013: perf-check: Identity — bench regression + resource profiling + budget compliance

## Description

Runs benchmark suite, binary size, startup time, memory profiling, compile time, test suite performance. Acceptance: Frontmatter `name: perf-check`; `disable-model-invocation: true`.

## Preconditions

1. Performance validation requested.

## Postconditions

1. Runs benchmark suite, binary size, startup time, memory profiling, compile time, test suite performance.

## Invariants

1. Frontmatter `name: perf-check`; `disable-model-invocation: true`.
