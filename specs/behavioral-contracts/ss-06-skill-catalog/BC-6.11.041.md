...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-583"
ss_section: "Factory operations and dashboards skills"
skill: "policy-add"
---

# Behavioral Contract BC-6.11.041: policy-add: Output appends to policies.yaml; runs validate after; reports next steps

## Description

Appends YAML block. Runs `/vsdd-factory:policy-registry validate` to confirm. Reports ID, name, severity, enforced_by, and next steps (implement lint hook, document rationale). Acceptance: Append-only operation.
...
1. Appends YAML block. Runs `/vsdd-factory:policy-registry validate` to confirm. Reports ID, name, severity, enforced_by, and next steps (implement lint hook, document rationale).
