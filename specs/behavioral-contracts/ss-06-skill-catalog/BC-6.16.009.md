...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-483"
ss_section: "Phase orchestration and mode skills"
skill: "next-step"
---

# Behavioral Contract BC-6.16.009: next-step: Uses lobster-parse to enumerate workflow steps with dependencies

## Description

Uses `${CLAUDE_PLUGIN_ROOT}/bin/lobster-parse <file> '.workflow.steps[] | {name, agent, depends_on, task}'` to enumerate steps with their dependencies. Acceptance: lobster-parse jq query specified.

## Preconditions

1. Workflow file resolved.

## Postconditions

1. Uses `${CLAUDE_PLUGIN_ROOT}/bin/lobster-parse <file> '.workflow.steps[] | {name, agent, depends_on, task}'` to enumerate steps with their dependencies.

## Invariants

1. lobster-parse jq query specified.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/skills/onboard-observability/SKILL.md` (132 LOC)` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Audit Source ID** | BC-AUDIT-483 |
| **Source Line(s)** | 22-26 |
