...
|--------|----------|-------------|
| VP-001 | TBD — assertion derived from acceptance: "Resulting JSON has `agent == "vsdd-factory:orchestrator:orchestrator"` and `vsdd-factory.{activated_platform,activated_at,activated_plugin_version}` present; pre-existing keys still present." | manual |
