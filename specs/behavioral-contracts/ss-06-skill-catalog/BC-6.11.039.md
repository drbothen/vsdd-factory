...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-581"
ss_section: "Factory operations and dashboards skills"
skill: "policy-add"
---

# Behavioral Contract BC-6.11.039: policy-add: Severity HIGH or MEDIUM only; HIGH violations block convergence

## Description

Severity is `HIGH` or `MEDIUM` only. HIGH: violations block convergence. MEDIUM: violations are findings but may not block alone. Acceptance: Two-value enum.
...
1. Severity is `HIGH` or `MEDIUM` only. HIGH: violations block convergence. MEDIUM: violations are findings but may not block alone.
