...
| [BC-4.14.001](ss-04/BC-4.14.001.md) | validate-wave-handoff-completeness WASM gate blocks HandoffIncomplete on PostToolUse HANDOFF.md writes (9 base required fields + epic_status conditional on EPIC-COMPLETE); no-op on wave-1 and non-HANDOFF.md writes | draft | CAP-032 | S-18.02 | v1.0 \| v1.1 \| v1.2 \| v1.3 \| v1.4 \| v1.5 \| v1.6 \| v1.7 |
...
| [BC-7.07.001](ss-07/BC-7.07.001.md) | precompact-flush.sh fires synchronously on PreCompact; hermetic (STATE.md + git only); renews factory lock when held (no-op when absent); commits with `PreCompact flush <cycle>/<step>` message; appends commit SHA to precompact-flush-log before push; on append failure: SHA-pinned reset (`git reset --soft SHA_B^` when HEAD==SHA_B; no-reset + human-intervention exit 2 when HEAD!=SHA_B); exit 2 on commit failure; fail-open on crash | draft | CAP-032 | S-18.04 | v1.0 \| v1.1 \| v1.2 \| v1.3 \| v1.4 \| v1.5 \| v1.6 |
