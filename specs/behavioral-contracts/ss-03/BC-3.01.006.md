...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:310"
subsystem: "SS-03"
capability: "TBD"
...
# Behavioral Contract BC-3.01.006: file sink path template substitutes `{date}`, `{name}`, `{project}` and rejects unknown placeholders

> Section: Sink registry and base contracts
> Source BC (audit ID): BC-AUDIT-046

## Description

`resolve_path_template` returns `FileSinkError::UnknownPlaceholder`.

## Preconditions

1. path_template with at least one unknown `{xyz}` placeholder.

## Postconditions

1. `resolve_path_template` returns `FileSinkError::UnknownPlaceholder`.
...
| **Source line(s)** | TBD |
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-046 |
| **Pass-3 source** | `pass-3-behavioral-contracts.md:310` |
| **Extraction Date** | 2026-04-25 |

**Evidence (from pass-3):**

> `sink-file/lib.rs::resolve_path_template` + tests.
