...
origin: brownfield
extracted_from: "pass-3-deep-rust-tests.md:956"
subsystem: "SS-03"
capability: "TBD"
...
# Behavioral Contract BC-3.02.010: sink-file::tag_enrichment_writes_tags_onto_every_event: configured tags `env=prod,team=factory` l

> Section: File sink behavior
> Source BC (audit ID): BC-AUDIT-2377

## Description

Given Sink config with tags = `{env: "prod", team: "factory"}`. When One event submitted + flushed.. Then Written line has `env == "prod"` and `team == "factory"` alongside the producer-supplied `type == "commit.made"`.

## Preconditions

1. Sink config with tags = `{env: "prod", team: "factory"}`.

## Postconditions

1. Written line has `env == "prod"` and `team == "factory"` alongside the producer-supplied `type == "commit.made"`.

## Invariants

1. Operators can attribute per-sink source data via static tags without touching the producer.
...
|-------|----------------|----------|
| Sink config with tags = `{env: "prod", team: "factory"}`. | Written line has `env == "prod"` and `team == "factory"` alongside the producer-supplied `type == "commit.made"`. | happy-path |
| TBD | TBD | edge-case |
| TBD | TBD | error |
...
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-2377 |
| **Pass-3 source** | `pass-3-deep-rust-tests.md:956` |
| **Extraction Date** | 2026-04-25 |
| **Pinned test** | ``crates/sink-file/src/lib.rs::tests::tag_enrichment_writes_tags_onto_every_event`` |
| **Test type** | unit |
