...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:322"
subsystem: "SS-03"
capability: "TBD"
...
# Behavioral Contract BC-3.01.008: file sink failures recorded into Mutex<Vec<SinkFailure>>

> Section: Sink registry and base contracts
> Source BC (audit ID): BC-AUDIT-048

## Description

SinkFailure with path, reason, ts pushed; consumer drains via `take_failures`.

## Preconditions

1. Write fails (e.g., permission denied on append).

## Postconditions

1. SinkFailure with path, reason, ts pushed; consumer drains via `take_failures`.
...
| **Source line(s)** | TBD |
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-048 |
| **Pass-3 source** | `pass-3-behavioral-contracts.md:322` |
| **Extraction Date** | 2026-04-25 |

**Evidence (from pass-3):**

> `sink-file/lib.rs::SinkFailure` + worker error path. Pending S-4.4 wires this to `internal.sink_error` events.
