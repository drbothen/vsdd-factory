...
# Behavioral Contract BC-3.03.005: Producer-side `submit` is fully non-blocking via `try_send`; overflow increments `queue_full_coun

> Section: OTLP gRPC sink batching and lifecycle
> Source BC (audit ID): BC-AUDIT-141

## Description

`Sink::submit` calls `sender.try_send(Message::Event(enriched)).is_err()` → if Err, `shared.queue_full_count.fetch_add(1, Relaxed)`. No producer-side blocking, no panic, no allocation beyond the event itself. Tests read `queue_full_count` to verify backpressure.

## Preconditions

1. Internal mpsc bounded at `queue_depth.max(1)` (default 1000).

## Postconditions

1. `Sink::submit` calls `sender.try_send(Message::Event(enriched)).is_err()` → if Err, `shared.queue_full_count.fetch_add(1, Relaxed)`. No producer-side blocking, no panic, no allocation beyond the event itself. Tests read `queue_full_count` to verify backpressure.
...
| **Source line(s)** | TBD |
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-141 |
| **Pass-3 source** | `pass-3-behavioral-contracts-deep-r1.md:520` |
| **Extraction Date** | 2026-04-25 |

**Evidence (from pass-3):**

> `crates/sink-otel-grpc/src/lib.rs::OtelGrpcSink::submit` (Sink::submit impl with try_send + queue_full_count); `crates/sink-otel-grpc/src/lib.rs::OtelGrpcSink::queue_full_count` (public accessor).
...
| v1.1 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 6 — migrated 1 body cite: `sink-otel-grpc/src/lib.rs:406-419` + `:351-355` → `::OtelGrpcSink::submit`, `::OtelGrpcSink::queue_full_count`. |
