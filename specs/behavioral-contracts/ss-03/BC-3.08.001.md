...
- **PC3 OPERATOR_OVERRIDE**: Retroactive reconciliation via RAW_DELETE_DETECTED: the dispatcher-native `reconcile_raw_delete` (same pre-check's marker-absent branch) finds the marker absent on next evaluation with an unmatched `plugin.indeterminate` in the FileSink log (emission point corrected v1.2 — never the WASM gate plugin).
...
| `TTL_EXPIRED` | `deadman` | Dispatcher-native `check_and_clear_expired_marker` pre-check auto-delete (BC-1.18.003 PC4; ADR-048 §D4 v1.2) | `check_and_clear_expired_marker` in `indeterminate_marker.rs` — dispatcher-native, called from `executor.rs`'s tier-execution loop BEFORE the Arm 1/Arm 2 WASM gate plugin runs; NOT emitted from inside `evaluate_gate` |
| `OPERATOR_OVERRIDE` | `operator` | Retroactive: dispatcher-native `reconcile_raw_delete` finds marker absent + unmatched `plugin.indeterminate` (BC-1.18.003 PC3; ADR-048 §D4 v1.2) | `reconcile_raw_delete` in `indeterminate_marker.rs` — dispatcher-native, invoked from the same pre-check's marker-absent branch; NOT emitted from inside the WASM gate plugin |
...
**Emission path**: `emit_marker_cleared` emits via the same FileSink/InternalLog path as `plugin.indeterminate` (Events 8 and 9 share the same emission infrastructure), and — per ADR-048 §D4 v1.2 — is called exclusively from dispatcher-native code (`delete_marker_if_pass`, `check_and_clear_expired_marker`, `reconcile_raw_delete`, all in `indeterminate_marker.rs`); never from inside the WASM gate plugin via the `emit_event` host ABI. The crash-path native TTL check in the dispatcher (`block_if_marker_check`, Decision 1 — crash + expired marker → Allow) does NOT emit `marker.cleared(TTL_EXPIRED)` — only the dispatcher-native pre-check (`check_and_clear_expired_marker`) emits it.
...
| Marker TTL expires; dispatcher-native `check_and_clear_expired_marker` pre-check auto-deletes marker (before the Arm 1/Arm 2 WASM gate plugin runs) | `marker.cleared` event in events-*.jsonl; `clear_mode="TTL_EXPIRED"`, `actor_type="deadman"`, `trace_id` from marker TOML, `reason=null`; all mandatory fields present; no event if native crash-path TTL-allow fires (no marker delete on crash path) | marker-cleared-ttl-expired |
