...
All nine event types carry the following dispatcher-owned fields on the wire. These fields are injected by the host (see `emit_event.rs` enrichment path) and are never supplied by plugins (they are RESERVED_FIELDS — see §Implementation Notes):
...
| `session_id` | UUID v4 string | Claude Code session identifier from the hook envelope context (`ctx.session_id`). Present on all nine event types (O-P15-001). |
| `plugin_name` | string | Name of the plugin registry entry, injected by the host. Present on plugin-context events (1, 4, 5, 6, 7, and 8) only; absent from dispatcher-startup events (2 + 3) which have no plugin context. |
...
The §Common Fields appear on the wire for ALL nine event types except where noted. Wire-format examples in §Postconditions show:
...
- All nine event types: `trace_id` + `session_id` explicitly shown (verified by VP-079 payload conformance).
...
**Sink destination**: All nine events are routed to `events-*.jsonl` via FileSink. They are NOT routed to the dispatcher-internal debug stream (which is opt-in per DI-007 amended). The VP-028 sink-fan-out invariant applies: if multiple sinks are configured, all nine events must fan out to all applicable sinks.
...
**Trigger**: The `.factory/unvalidated-mutation.marker` file is cleared by one of three paths defined in BC-1.18.003:
...
- **PC4 TTL_EXPIRED**: The gate plugin's normal-path TTL-check branch auto-deletes an expired marker (TTL-loudness — previously silent; now audited per ADR-048 §Decision 4).
- **PC3 OPERATOR_OVERRIDE**: Retroactive reconciliation via RAW_DELETE_DETECTED: gate plugin finds marker absent on next evaluation with an unmatched `plugin.indeterminate` in the FileSink log.

**Full behavioral governance (clear predicates, scoping rules, audited-clear obligations, RAW_DELETE_DETECTED reconciliation) is BC-1.18.003 (PC1, PC3, PC4); this catalog entry registers the wire-format/field-shape authority only.**
...
```json
{
  ...
  "clear_mode": "<string — one of: REVALIDATED | TTL_EXPIRED | OPERATOR_OVERRIDE>",
  "actor_type": "<string — one of: validator | deadman | operator>",
  "reason": "<string | null — mandatory when clear_mode=OPERATOR_OVERRIDE (RAW_DELETE_DETECTED text); null otherwise>",
  "timestamp": "<ISO-8601 UTC — time of the clear event, not the original INDETERMINATE event>"
}
```

**Mandatory fields**: `type`, `trace_id`, `session_id`, `plugin_name`, `artifact_path`, `clear_mode`, `actor_type`, `reason` (conditional: mandatory for OPERATOR_OVERRIDE, null/omitted otherwise), `timestamp`.

**`clear_mode` / `actor_type` correspondence:**

| `clear_mode` | `actor_type` | Trigger | Emission point |
|---|---|---|---|
| `REVALIDATED` | `validator` | `delete_marker_if_pass` removes marker after PASS (BC-1.18.003 PC1) | `emit_marker_cleared` in `indeterminate_marker.rs` after `remove_file` succeeds |
| `TTL_EXPIRED` | `deadman` | Gate plugin normal-path TTL auto-delete (BC-1.18.003 PC4) | Plugin TTL-check branch after auto-delete |
| `OPERATOR_OVERRIDE` | `operator` | Retroactive: gate eval finds marker absent + unmatched `plugin.indeterminate` (BC-1.18.003 PC3) | RAW_DELETE_DETECTED reconciliation path in gate plugin |

**`trace_id` semantics**: Links to the originating `plugin.indeterminate` event. For REVALIDATED and TTL_EXPIRED, the `trace_id` is read from the marker TOML `trace_id` field written at INDETERMINATE time. For OPERATOR_OVERRIDE, the `trace_id` is taken from the unmatched `plugin.indeterminate` record found in the FileSink log.

**`reason` semantics**: Null (or omitted) for REVALIDATED and TTL_EXPIRED. For OPERATOR_OVERRIDE, MUST be the literal string: `"RAW_DELETE_DETECTED: marker absent without prior marker.cleared event; inferred operator out-of-band clear"`.

**Proportionality**: No signed digests or dual-control required. Append-only FileSink event is proportionate for the cooperating-agent threat model (VSDD baseline). GitHub branch-protection and the `factory-artifacts` worktree provide tamper-evidence at the VCS layer (ADR-048 §Decision 4 rationale).

**Emission path**: `emit_marker_cleared` emits via the same FileSink/InternalLog path as `plugin.indeterminate` (Events 8 and 9 share the same emission infrastructure). The crash-path native TTL check in the dispatcher (`block_if_marker_check`, Decision 1 — crash + expired marker → Allow) does NOT emit `marker.cleared(TTL_EXPIRED)` — only the gate plugin's normal-path auto-delete emits it.

## Invariants

1. **`trace_id` is mandatory on all nine event types**: Per DI-017, every emitted event carries the UUID v4 from the invoking hook envelope. These nine events are no exception.
...
3. **Events do not affect dispatcher exit code**: All nine are observability-only. `plugin.async_block_discarded`, `plugin.timeout (async)`, `plugin.abandoned`, `plugin.completed` (async path), `plugin.fuel_headroom_warning`, `plugin.indeterminate`, and `marker.cleared` are logged and forgotten. `dispatcher.schema_mismatch` and `dispatcher.registry_invalid` accompany a hard exit (non-zero) but the event itself does not cause the exit — the validation failure does. For `plugin.indeterminate` with `failure_policy=fail-closed`, the block is issued by the marker-gate mechanism (BC-1.18.002), not by this event emission. `marker.cleared` is a pure audit event: it records a state change but never influences block decisions.
...
- BC-1.18.001 — event `plugin.indeterminate` (Event 8) triggering-condition/semantics authority (outcome classification, INDETERMINATE causes, fail-closed marker path); this BC provides the SS-03 wire-format/field-shape catalog authority only
- BC-1.18.002 — gate behavior when marker exists (follow-on from Event 8 fail-closed path)
- BC-1.18.003 — event `marker.cleared` (Event 9) clear-path/lifecycle authority (REVALIDATED/TTL_EXPIRED/OPERATOR_OVERRIDE clear predicates, audited-clear obligations, RAW_DELETE_DETECTED reconciliation); this BC provides the SS-03 wire-format/field-shape catalog authority only
...
- `crates/factory-dispatcher/src/indeterminate_marker.rs` + `crates/factory-dispatcher/src/host/emit_event.rs` — `marker.cleared` (Event 9) emission path: `emit_marker_cleared(clear_mode, marker, trace_id)` in `indeterminate_marker.rs` writes the JSON line via FileSink; called from `delete_marker_if_pass` (REVALIDATED clear), from the gate plugin's TTL-check branch (TTL_EXPIRED clear), and from the gate plugin's RAW_DELETE_DETECTED reconciliation path (OPERATOR_OVERRIDE retroactive)
- `crates/factory-dispatcher/src/registry.rs` — schema_mismatch and registry_invalid emission sites
- `crates/sink-core/src/` — FileSink fan-out path for all nine event types
...
- VP-079 — Payload schema conformance for all nine event types including `plugin.abandoned`, `plugin.completed` (async path), `plugin.fuel_headroom_warning`, `plugin.indeterminate`, and `marker.cleared`: each mandatory field is
...
- VP-028 — Sink fan-out invariant: once emitted, all nine event types (`plugin.async_block_discarded`, `dispatcher.schema_mismatch`, `dispatcher.registry_invalid`, `plugin.timeout` (async path), `plugin.abandoned`, `plugin.completed` (async path), `plugin.fuel_headroom_warning`, `plugin.indeterminate`, `marker.cleared`) reach every configured accepting sink (independent of VP-079's payload conformance check)
...
| EC-012 | Gate plugin's TTL-check branch determines marker `expires_at ≤ now` (TTL deadman) | `marker.cleared` emitted with `clear_mode="TTL_EXPIRED"`, `actor_type="deadman"`, `trace_id` from marker TOML, `plugin_name` from marker TOML, `artifact_path` from marker TOML, `reason=null`, `timestamp` = time of auto-delete. Crash-path native TTL-allow does NOT emit this event (BC-1.18.003 PC4). Full behavioral governance: BC-1.18.003 PC4. |
| EC-013 | Gate plugin's `RAW_DELETE_DETECTED` reconciliation path fires (marker absent, unmatched `plugin.indeterminate` in FileSink log) | `marker.cleared` emitted retroactively with `clear_mode="OPERATOR_OVERRIDE"`, `actor_type="operator"`, `trace_id` from matched `plugin.indeterminate` event, `reason` field MUST be non-null (human-readable explanation), `timestamp` = reconciliation time. Best-effort: only possible when FileSink log available. Full behavioral governance: BC-1.18.003 PC3. |
...
| Marker deleted via T1 re-validation (Edit/Write → plugin PASS → `delete_marker_if_pass`) | `marker.cleared` event in events-*.jsonl; `clear_mode="REVALIDATED"`, `actor_type="validator"`, `trace_id` matches originating `plugin.indeterminate` `trace_id`, `reason=null`; all mandatory fields present | marker-cleared-revalidated |
| Marker TTL expires; gate plugin TTL-check branch auto-deletes marker | `marker.cleared` event in events-*.jsonl; `clear_mode="TTL_EXPIRED"`, `actor_type="deadman"`, `trace_id` from marker TOML, `reason=null`; all mandatory fields present; no event if native crash-path TTL-allow fires (no marker delete on crash path) | marker-cleared-ttl-expired |
| Operator deletes marker out-of-band; gate plugin detects RAW_DELETE (marker absent + unmatched `plugin.indeterminate` in FileSink log) | `marker.cleared` event in events-*.jsonl; `clear_mode="OPERATOR_OVERRIDE"`, `actor_type="operator"`, `reason` non-null; emitted retroactively at reconciliation time | marker-cleared-operator-override |
...
| VP-079 | Payload schema conformance for all nine event types including `plugin.abandoned`, `plugin.completed` (async path), `plugin.fuel_headroom_warning`, `plugin.indeterminate`, and `marker.cleared` — mandatory fields present, non-null, type string correct. **VP-079 Event 9 staleness flag OPEN:** architect must add SITE_9 (`marker.cleared`), update mandatory-fields table (add `clear_mode`, `actor_type`, `reason`), update Property Statement to "nine events" per `vp_index_is_vp_catalog_source_of_truth`. | integration |
