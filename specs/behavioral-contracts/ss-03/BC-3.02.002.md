...
origin: brownfield
extracted_from: "pass-3-deep-rust-tests.md:868"
subsystem: "SS-03"
capability: "TBD"
...
# Behavioral Contract BC-3.02.002: sink-file::template_name_only: `{name}` substitutes to the sink's operator-assigned name

> Section: File sink behavior
> Source BC (audit ID): BC-AUDIT-2369

## Description

Given `{name}.jsonl`, name="audit-archive". When Resolved.. Then Returns `/var/log/audit-archive.jsonl`.

## Preconditions

1. `{name}.jsonl`, name="audit-archive".

## Postconditions

1. Returns `/var/log/audit-archive.jsonl`.

## Invariants

1. Operators can disambiguate per-sink output files via the sink name token.
...
|-------|----------------|----------|
| `{name}.jsonl`, name="audit-archive". | Returns `/var/log/audit-archive.jsonl`. | happy-path |
| TBD | TBD | edge-case |
| TBD | TBD | error |
...
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-2369 |
| **Pass-3 source** | `pass-3-deep-rust-tests.md:868` |
| **Extraction Date** | 2026-04-25 |
| **Pinned test** | ``crates/sink-file/src/lib.rs::tests::template_name_only`` |
| **Test type** | unit |
