...
|--------|----------|-------------|
| VP-001 | Given Operator overrides include `service.name="custom-svc"`, `deployment.env="dev"`. When `build_resource_attributes(&o | manual (existing test: `crates/sink-otel-grpc/src/lib.rs::tests::resource_attributes_merge_defaults_with_config`) |
