...
| EC-007 | `resolve_path_for_allowlist` in `path_util.rs` receives a path with no existing ancestor (root path or filesystem root not accessible) | Algorithm exhausts all ancestors without finding a canonicalizable one. Returns `false` (cannot determine if allowlisted). Dispatcher emits `internal.capability_denied` with reason `path_resolution_failed` (not `path_not_allowed`). |
