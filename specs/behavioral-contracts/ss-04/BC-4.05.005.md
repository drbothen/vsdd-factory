...
| EC-003 | Duplicate `SessionEnd` entries in `hooks-registry.toml` | Both entries are loaded into `Vec<RegistryEntry>`; routing is presumed first-entry-wins per `toml::from_str` `[[hooks]]` iteration order (same caveat as BC-4.04.005 EC-002 — plausible but not directly verified by VP-066 file-load harness). Operator must avoid duplicates manually. v1.1 candidate: registry validation duplicate-detection. |
...
**F-17 Generator Workflow (inherited):** `hooks-registry.toml` is human-edited source of truth as of v1.0.0. The SessionEnd entry is added by direct edit. The legacy generator script is retired and must not be used.
