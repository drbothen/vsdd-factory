...
| EC-006 | HANDOFF.md write; `precompact_flush_sha: null`; payload `wave_id: 2` (not first wave) | Continue if null is explicitly permitted in the field schema for wave > 1 (advisory only — anti-fabrication check on null is the skill's job, not the gate's) |
