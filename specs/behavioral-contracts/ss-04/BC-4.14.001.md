...
3. **No-op conditions are checked in canonical 5-step order**: The gate checks conditions in the following order: (1) non-HANDOFF.md target → no-op; (2) EPIC-COMPLETE detection (`next_wave_stories: []`) → validate PC2a even when wave_id==1; (3) wave_id==1 AND NOT EPIC-COMPLETE → no-op; (4) wave_id>1 → full validation; (5) wave_id ABSENT → fail-closed. The EPIC-COMPLETE check (step 2) precedes the wave_id==1 no-op (step 3). This ensures the gate validates `epic_status` on single-wave EPIC-COMPLETE writes. Wave-1 detection reads `wave_id` from the payload — it is NOT checked against sprint-state.yaml, factory-artifacts, or any external source. A gate that parses HANDOFF.md fully and then returns Continue when `wave_id == 1` (without checking EPIC-COMPLETE context first) is an incorrect implementation. Absent `wave_id` is NOT a no-op condition — it proceeds to full validation (fail-closed).
...
| EC-015 | HANDOFF.md write; `wave_id: 1`; `next_wave_stories: []` (EPIC-COMPLETE); all 9 base fields + `epic_status: complete` | Continue (single-wave-epic-complete-happy-path); all fields valid; wave-1 no-op does NOT fire because EPIC-COMPLETE branch is evaluated first |
| EC-016 | HANDOFF.md write; `wave_id: 1`; `next_wave_stories: []` (EPIC-COMPLETE); all 9 base fields; `epic_status` absent | `MissingEpicStatus`; exit 2 (single-wave-epic-complete-missing-epic-status); wave-1 no-op does NOT fire; EPIC-COMPLETE branch demands `epic_status: complete` regardless of wave_id |
...
| Write to HANDOFF.md; `wave_id: 1`; `next_wave_stories: []`; all 9 base fields + `epic_status: complete` | Continue (single-wave EPIC-COMPLETE happy path; EPIC-COMPLETE branch fires before wave-1 no-op) | single-wave-epic-complete-happy-path |
| Write to HANDOFF.md; `wave_id: 1`; `next_wave_stories: []`; all 9 base fields; `epic_status` absent | MissingEpicStatus; exit 2 (EPIC-COMPLETE requires epic_status even at wave_id==1) | single-wave-epic-complete-missing-epic-status |
...
|--------|----------|-------------|
| VP-083 | validate-wave-handoff-completeness returns Continue when `payload.wave_id == 1` AND NOT EPIC-COMPLETE (payload-only; no filesystem access); when `payload.wave_id == 1` AND `next_wave_stories: []` (EPIC-COMPLETE) the gate does NOT return Continue — it evaluates PC2a and validates `epic_status: complete`; also no-op on non-HANDOFF.md writes; when `wave_id` is ABSENT from payload the gate FAILS CLOSED (does NOT return Continue) | unit-test |
