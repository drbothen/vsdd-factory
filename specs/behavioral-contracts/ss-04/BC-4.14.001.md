...
| EC-015 | HANDOFF.md write; `wave_id: 1`; `next_wave_stories: []` (EPIC-COMPLETE); all 9 base fields + `epic_status: complete` | Continue; all fields valid; wave-1 no-op does NOT fire because EPIC-COMPLETE branch is evaluated first | single-wave-epic-complete-happy-path |
| EC-016 | HANDOFF.md write; `wave_id: 1`; `next_wave_stories: []` (EPIC-COMPLETE); all 9 base fields; `epic_status` absent | `MissingEpicStatus`; exit 2; wave-1 no-op does NOT fire; EPIC-COMPLETE branch demands `epic_status: complete` regardless of wave_id | single-wave-epic-complete-missing-epic-status |
