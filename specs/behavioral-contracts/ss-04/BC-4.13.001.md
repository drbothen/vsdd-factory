...
9. **`factory_lock` block parsing is fail-open**: Frontmatter parsing uses a YAML subset
   ...
   scanning upon encountering the second `---\n` delimiter — the extracted frontmatter slice
   excludes the delimiter bytes (scanning-abort and slice-boundary are distinct: the scanner
   stops when the delimiter is detected; the delimiter bytes are not included in the output;
   see Verification note) — and MUST NOT attempt to parse the file body. The lock verdict is derived exclusively from the frontmatter region;
   ...
   **Verification note (F-P2-011):** The correct test for this invariant is byte-boundary
   ...
   prefix `bytes 0..delimiter_start_offset` — up to but EXCLUDING the second `---\n`
   delimiter line (delimiter_start_offset = byte index of the `\n` that begins `\n---\n`);
   the slice includes the opening `---\n`. Tests MUST assert this byte-exact boundary;
   parity-with-full-file-parse verification remains FORBIDDEN.
