...
last_amended: "2026-06-26 (v1.4) — fix-burst (product-owner): (SEC-002 revised) INV5 rewritten to final canonical 4-pass best-effort-advisory design per ADR-026 §Decision 12 SEC-002 Redaction Sub-clause (ADR v1.28) + VP-091 §6 (VP v1.4). Best-effort-advisory framing added (not a security boundary). Mask token corrected to 14 ASCII chars (v1.3 incorrectly stated 15). Pass 1 revised: bare --key REMOVED (false-positive trap); compound trigger substrings (password, passwd, token, secret, api-key, apikey, api_key, client-secret, auth-token, access-token, access-key, secret-key, credential, passphrase, private-key); bare short flags excluded. Pass 2 added: env-var assignment-prefix masking (compound triggers on IDENT; bare KEY not matched; allowlist SSH_AUTH_SOCK/SSH_ASKPASS/*_SERVICE_HOST checked first). Pass 3 added: Authorization/Cookie/Set-Cookie header masking (all schemes). Pass 4 added: inline URL credential masking (://user:pass@host → ://***REDACTED***@host). Provider-prefix detection (sk-/AKIA/ghp_) explicitly deferred as documented future enhancement. EC-014..EC-020 replaced with EC-014..EC-021 (append-only; 8 ECs covering all 4 passes + negatives). Redaction test vectors revised. PC-B-B1/PC-B-B2 command_preview description updated; INV4 mask-token count corrected to 14 chars. Traceability ADR cite updated. VP Anchors updated to PC6a–PC6g. [Prior: 2026-06-26 (v1.3): INV5 two-pass design (superseded); mask token incorrectly stated as 15 chars. [Prior: 2026-06-17 (v1.2): PC-B-B1/PC-B-B2 heading promotion. [Prior: 2026-06-16 (v1.1): VP-091 proof_method fix.]]]"
phase: F3
inputs:
...
5. **Secret-redaction; 4-pass best-effort advisory; pure-function; no-regex; enumerable trigger lists (SEC-002):** The `command_preview` field in BOTH emission channels (PC-B-B1 stderr and PC-B-B2 plugin.log) MUST have secrets redacted before the preview string is constructed or truncated. Redaction occurs BEFORE truncation (redact-then-truncate ordering mandate per EC-020 / ADR-026 §Decision 12 SEC-002 Redaction Sub-clause).
   ...
   **Pass 1 — flag-argument masking:** Split the command string on ASCII whitespace. For each token starting with `-` (a flag token):
   ...
   **Bare `--key` is NOT a trigger** (`sort --key`, `cut --key`, and similar standard utilities use `--key` non-secretly; `key` alone does not appear in the trigger substring list). Bare short flags (`-p`, `-u`, `-t`, etc.) are NOT masked regardless of name.
...
| EC-016 | Bash command contains an `Authorization: Bearer` header, e.g., `grep -r . -H "Authorization: Bearer eyJtoken123"`; command matches `grep -r` | INV5 Pass 3 masks the bearer token: `command_preview` contains `Authorization:***REDACTED***` (or `Authorization: ***REDACTED***` per two-token form); raw token absent from both channels; gate returns Continue with advisory |
...
| Bash command: `grep -r . -H "Authorization: Bearer eyJtoken123"` | Continue; DelegationRecommended advisory; `command_preview` in plugin.log and stderr shows bearer token replaced with `***REDACTED***`; raw token absent from both channels (Pass 3) | redaction-pass3-authorization-header |
