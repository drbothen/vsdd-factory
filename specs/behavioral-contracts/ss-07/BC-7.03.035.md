...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:411"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.035: destructive-command-guard: blocks gh shared-state destructive ops (4 codes)

## Description

destructive-command-guard: blocks gh shared-state destructive ops (4 codes). Block codes `gh_repo_delete`, `gh_release_delete`, `gh_pr_close`, `gh_issue_delete`. Even closing a PR is blocked (must be done by user).

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1034` (extracted from `pass-3-deep-hooks.md` line 411).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: `gh repo delete`, `gh release delete`, `gh pr close`, `gh issue delete`.

## Postconditions

1. Behavior: Block codes `gh_repo_delete`, `gh_release_delete`, `gh_pr_close`, `gh_issue_delete`. Even closing a PR is blocked (must be done by user).
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 411 |
| **Audit ID** | `BC-AUDIT-1034` |
| **Source Line(s) (within hook)** | 306-330. |
