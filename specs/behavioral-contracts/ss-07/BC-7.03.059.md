...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:666"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.059: protect-secrets: blocks env\

## Description

protect-secrets: blocks env|grep for secret-shaped names. Block code `secret_var_grep`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1058` (extracted from `pass-3-deep-hooks.md` line 666).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-secrets.sh``.

## Preconditions

1. Trigger: Bash command contains `env|printenv|set` piped into `grep` with secret-shaped pattern.

## Postconditions

1. Behavior: Block code `secret_var_grep`.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `env \| grep TOKEN` blocked. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 666 |
| **Audit ID** | `BC-AUDIT-1058` |
| **Source Line(s) (within hook)** | 186-193. |
