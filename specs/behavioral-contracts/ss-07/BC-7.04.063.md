...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1666"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.063: validate-template-compliance: identity & registry binding

## Description

validate-template-compliance: identity & registry binding. Template compliance enforcer; on_error=block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1154` (extracted from `pass-3-deep-hooks.md` line 1666).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-template-compliance.sh``.
**Registry entry:** `hooks-registry.toml::validate-template-compliance` (PostToolUse, tool=`Edit|Write`, priority=400, timeout_ms=10000, on_error=**block**)..
...
1. Behavior: Template compliance enforcer; on_error=block.
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1666 |
| **Audit ID** | `BC-AUDIT-1154` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:436-454` → `hooks-registry.toml::validate-template-compliance`. |
