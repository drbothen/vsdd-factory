...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:145"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.010: capture-commit-activity: skips non-Bash, interrupted, or non-zero exits

## Description

capture-commit-activity: skips non-Bash, interrupted, or non-zero exits. Exit 0 immediately when (a) tool != "Bash", or (b) `interrupted == "true"`, or (c) explicit `exit_code != 0`. Documents Claude Code's lack of `exit_code` field; treats `interrupted: false` as success per harness contract.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1009` (extracted from `pass-3-deep-hooks.md` line 145).
**Hook script:** ``plugins/vsdd-factory/hooks/capture-commit-activity.sh``.

## Preconditions

1. Trigger: `tool_name`, `tool_response.exit_code`, `tool_response.interrupted` extracted.

## Postconditions

1. Behavior: Exit 0 immediately when (a) tool != "Bash", or (b) `interrupted == "true"`, or (c) explicit `exit_code != 0`. Documents Claude Code's lack of `exit_code` field; treats `interrupted: false` as success per harness contract.
2. Exit codes: 0 (skip).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Failed `git commit` (exit 1) does not produce a `commit.made` event. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 145 |
| **Audit ID** | `BC-AUDIT-1009` |
| **Source Line(s) (within hook)** | 41-57. |
