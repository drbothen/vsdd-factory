...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:896"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.080: track-agent-start: emits agent.start with subagent + best-effort story_id

## Description

track-agent-start: emits agent.start with subagent + best-effort story_id. Extracts `tool_input.subagent_type` and prompt; greps prompt for `S-[0-9]+\.[0-9]+` then `STORY-[0-9]+`. Emits `agent.start` with subagent and optional story_id.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1079` (extracted from `pass-3-deep-hooks.md` line 896).
**Hook script:** ``plugins/vsdd-factory/hooks/track-agent-start.sh``.

## Preconditions

1. Trigger: `tool_name == "Agent"`.

## Postconditions

1. Behavior: Extracts `tool_input.subagent_type` and prompt; greps prompt for `S-[0-9]+\.[0-9]+` then `STORY-[0-9]+`. Emits `agent.start` with subagent and optional story_id.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Dispatch of `pr-manager` for `S-6.07` produces `agent.start subagent=pr-manager story_id=S-6.07`. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 896 |
| **Audit ID** | `BC-AUDIT-1079` |
| **Source Line(s) (within hook)** | 27-44. |
...
| v1.2 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.08 (S-8.08 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
