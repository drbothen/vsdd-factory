...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:110"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.007: brownfield-discipline: blocks edits to `.reference/**`

## Description

brownfield-discipline: blocks edits to `.reference/**`. Emits `hook.block` event (reason=`reference_readonly`), prints diagnostic naming the file and reminding that reference codebases must be re-cloned upstream, exits 2.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1006` (extracted from `pass-3-deep-hooks.md` line 110).
**Hook script:** ``plugins/vsdd-factory/hooks/brownfield-discipline.sh``.

## Preconditions

1. Trigger: Edit/Write file_path matches regex `(^|/)\.reference/`.

## Postconditions

1. Behavior: Emits `hook.block` event (reason=`reference_readonly`), prints diagnostic naming the file and reminding that reference codebases must be re-cloned upstream, exits 2.
2. Exit codes: 2 on `.reference/` path.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `Write file_path=".reference/foo/main.go"` is rejected; `Write file_path="src/foo.go"` passes. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 110 |
| **Audit ID** | `BC-AUDIT-1006` |
| **Source Line(s) (within hook)** | 35-41. |
