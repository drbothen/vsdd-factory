...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:942"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.084: update-wave-state-on-merge: scopes to pr-manager + successful merge signal

## Description

update-wave-state-on-merge: scopes to pr-manager + successful merge signal. Other paths exit 0.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1083` (extracted from `pass-3-deep-hooks.md` line 942).
**Hook script:** ``plugins/vsdd-factory/hooks/update-wave-state-on-merge.sh``.

## Preconditions

1. Trigger: Agent name matches `*pr-manager*` AND result matches `STEP_COMPLETE: step=8.*status=ok|merged|squash.*merge`.

## Postconditions

1. Behavior: Other paths exit 0.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 942 |
| **Audit ID** | `BC-AUDIT-1083` |
| **Source Line(s) (within hook)** | 32-40. |
