...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:342"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.028: destructive-command-guard: blocks `rm -rf` on protected paths

## Description

destructive-command-guard: blocks `rm -rf` on protected paths. Allowlist exceptions: `.worktrees/` cleanup is permitted; build dirs (`target`, `node_modules`, `dist`, `build`, `.next`, `__pycache__`, `.pytest_cache`) when removed via `rm` are permitted.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1027` (extracted from `pass-3-deep-hooks.md` line 342).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: `rm` with recursive flag targeting `.factory/`, `.factory<EOL>`, `src/`, or `tests/`.

## Postconditions

1. Behavior: Allowlist exceptions: `.worktrees/` cleanup is permitted; build dirs (`target`, `node_modules`, `dist`, `build`, `.next`, `__pycache__`, `.pytest_cache`) when removed via `rm` are permitted.
2. Exit codes: 2 (block).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `rm -rf .factory` blocked; `rm -rf .worktrees/STORY-NNN/.factory/...` allowed; `rm -rf target` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 342 |
| **Audit ID** | `BC-AUDIT-1027` |
| **Source Line(s) (within hook)** | 88-106. |
