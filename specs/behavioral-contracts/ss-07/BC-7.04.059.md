...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1627"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.059: validate-subsystem-names: blocks story `subsystems:` array members not in canonical list

## Description

validate-subsystem-names: blocks story `subsystems:` array members not in canonical list. Per stray entry, error "Story subsystems: \"X\" does not match any SS-ID". Block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1150` (extracted from `pass-3-deep-hooks.md` line 1627).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-subsystem-names.sh``.

## Preconditions

1. Trigger: Story `subsystems:` array (inline or YAML list) element not in canonical list.

## Postconditions

1. Behavior: Per stray entry, error "Story subsystems: \"X\" does not match any SS-ID". Block.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1627 |
| **Audit ID** | `BC-AUDIT-1150` |
| **Source Line(s) (within hook)** | 110-143. |
