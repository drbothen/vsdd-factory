...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1685"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.065: validate-template-compliance: resolves template by document_type, then path pattern (15 patterns)

## Description

validate-template-compliance: resolves template by document_type, then path pattern (15 patterns). No template match → exit 0.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1156` (extracted from `pass-3-deep-hooks.md` line 1685).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-template-compliance.sh``.

## Preconditions

1. Trigger: Reads `document_type` from frontmatter; greps `$CLAUDE_PLUGIN_ROOT/templates/*.md` for matching document_type. If none, falls back to 15 path patterns: behavioral-contracts/BC-, verification-properties/VP-, stories/STORY-, holdout-scenarios/HS-, architecture/verification-coverage-matrix, architecture/verification-architecture, architecture/, domain-spec/, prd.md, product-brief.md, STATE.md, dtu-assessment.md, module-criticality.md.

## Postconditions

1. Behavior: No template match → exit 0.
2. Exit codes: 0 (skip).
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-template-compliance.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1685 |
| **Audit ID** | `BC-AUDIT-1156` |
| **Source Line(s) (within hook)** | 70-123. |
