...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:308"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.025: convergence-tracker: warns on zero findings on first pass

## Description

convergence-tracker: warns on zero findings on first pass. Adds to WARNINGS string; printed to stderr but does NOT block.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1024` (extracted from `pass-3-deep-hooks.md` line 308).
**Hook script:** ``plugins/vsdd-factory/hooks/convergence-tracker.sh``.

## Preconditions

1. Trigger: Pass==1 AND `New findings` + `Duplicate/variant findings` total == 0.

## Postconditions

1. Behavior: Adds to WARNINGS string; printed to stderr but does NOT block.
2. Exit codes: 0 (warn-only).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Pass-1 with `**New findings**: 0` and `**Duplicate**: 0` → stderr warn "Zero findings on first adversary pass — suspicious". | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 308 |
| **Audit ID** | `BC-AUDIT-1024` |
| **Source Line(s) (within hook)** | 87-96. |
