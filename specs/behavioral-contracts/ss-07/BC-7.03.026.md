...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:320"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.026: destructive-command-guard: identity & registry binding

## Description

destructive-command-guard: identity & registry binding. Catastrophic command guard; registered with on_error=block. binary_allow includes curl too.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1025` (extracted from `pass-3-deep-hooks.md` line 320).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.
**Registry entry:** `hooks-registry.toml::destructive-command-guard` (PreToolUse, tool=Bash, priority=50, timeout_ms=5000, on_error=block)..

## Preconditions

1. Hook event/tool match: PreToolUse Bash..
2. Trigger: Every Bash command.

## Postconditions

1. Behavior: Catastrophic command guard; registered with on_error=block. binary_allow includes curl too.
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 320 |
| **Audit ID** | `BC-AUDIT-1025` |
| **Source Line(s) (within hook)** | Header 1-19. |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:558-577` → `hooks-registry.toml::destructive-command-guard`. |
