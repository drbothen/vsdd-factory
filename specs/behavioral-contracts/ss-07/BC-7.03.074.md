...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:831"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.074: regression-gate: writes state file with status, timestamp, command

## Description

regression-gate: writes state file with status, timestamp, command. `jq -n '{status, timestamp, command}' > .factory/regression-state.json`. Timestamp is ISO-8601 UTC.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1073` (extracted from `pass-3-deep-hooks.md` line 831).
**Hook script:** ``plugins/vsdd-factory/hooks/regression-gate.sh``.

## Preconditions

1. Trigger: Status is pass or fail.

## Postconditions

1. Behavior: `jq -n '{status, timestamp, command}' > .factory/regression-state.json`. Timestamp is ISO-8601 UTC.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 831 |
| **Audit ID** | `BC-AUDIT-1073` |
| **Source Line(s) (within hook)** | 73-75. |
