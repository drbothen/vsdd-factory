...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1228"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.019: validate-finding-format: blocks legacy STORY-NNN-FIX-NNN format

## Description

validate-finding-format: blocks legacy STORY-NNN-FIX-NNN format. Error per legacy ID; suggest current `FIX-P<PHASE>-<SEQ>` format.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1110` (extracted from `pass-3-deep-hooks.md` line 1228).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-finding-format.sh``.

## Preconditions

1. Trigger: File contains `STORY-NNN-FIX-NNN` pattern.

## Postconditions

1. Behavior: Error per legacy ID; suggest current `FIX-P<PHASE>-<SEQ>` format.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1228 |
| **Audit ID** | `BC-AUDIT-1110` |
| **Source Line(s) (within hook)** | 69-75. |
