...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:85"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.005: block-ai-attribution: jq absence is graceful no-op

## Description

block-ai-attribution: jq absence is graceful no-op. Exit 0 silently. Hook degrades to allow rather than error. Same idiom in 38 of 44 hooks.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1004` (extracted from `pass-3-deep-hooks.md` line 85).
...
1. Behavior: Exit 0 silently. Hook degrades to allow rather than error. Same idiom in 38 of 44 hooks.
2. Exit codes: 0.
3. Error policy: Effectively continue (allow); registry says `block` but script body never reaches block path.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | With jq missing, every Bash command passes through without diagnostic. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 85 |
| **Audit ID** | `BC-AUDIT-1004` |
| **Source Line(s) (within hook)** | 18-20. |
