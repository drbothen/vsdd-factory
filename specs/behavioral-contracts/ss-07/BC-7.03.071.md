...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:800"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.071: regression-gate: identity & registry binding

## Description

regression-gate: identity & registry binding. Records test pass/fail to `.factory/regression-state.json`. Warn-only on regression. on_error=continue.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1070` (extracted from `pass-3-deep-hooks.md` line 800).
**Hook script:** ``plugins/vsdd-factory/hooks/regression-gate.sh``.
**Registry entry:** `hooks-registry.toml::regression-gate` (PostToolUse, no tool filter, priority=230, timeout_ms=5000, on_error=continue)..

## Preconditions

1. Hook event/tool match: PostToolUse Bash (filtered in body)..
2. Trigger: Any PostToolUse; early-exits if tool != Bash.

## Postconditions

1. Behavior: Records test pass/fail to `.factory/regression-state.json`. Warn-only on regression. on_error=continue.
...
| **Source Document** | `pass-3-deep-hooks.md` line 800 |
| **Audit ID** | `BC-AUDIT-1070` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:97-114` → `hooks-registry.toml::regression-gate`. |
| v1.3 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.09 (S-8.09 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
