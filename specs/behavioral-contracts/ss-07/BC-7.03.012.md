...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:165"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.012: capture-commit-activity: detects `--amend` and emits `amended="true"`

## Description

capture-commit-activity: detects `--amend` and emits `amended="true"`. Sets `AMENDED="true"`; emit-event payload includes `amended="true"` field. Allows downstream dashboards to filter amends.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1011` (extracted from `pass-3-deep-hooks.md` line 165).
**Hook script:** ``plugins/vsdd-factory/hooks/capture-commit-activity.sh``.

## Preconditions

1. Trigger: Command regex matches `(^|[[:space:]])--amend([[:space:]=]|$)`.

## Postconditions

1. Behavior: Sets `AMENDED="true"`; emit-event payload includes `amended="true"` field. Allows downstream dashboards to filter amends.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `git commit --amend` produces an event with `amended="true"`; ordinary `git commit` produces an event without that field. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 165 |
| **Audit ID** | `BC-AUDIT-1011` |
| **Source Line(s) (within hook)** | 117-120. |
