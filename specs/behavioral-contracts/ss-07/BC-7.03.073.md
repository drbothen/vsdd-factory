...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:821"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.073: regression-gate: pass/fail derivation prefers exit_code; falls back to interrupted

## Description

regression-gate: pass/fail derivation prefers exit_code; falls back to interrupted. Status logic: `exit==0 → pass`; explicit non-zero → fail; `interrupted==true → fail`; `interrupted==false → pass`; otherwise unknown (no recording). Documents Claude Code's `interrupted` field shape.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1072` (extracted from `pass-3-deep-hooks.md` line 821).
**Hook script:** ``plugins/vsdd-factory/hooks/regression-gate.sh``.

## Preconditions

1. Trigger: Test runner matched.

## Postconditions

1. Behavior: Status logic: `exit==0 → pass`; explicit non-zero → fail; `interrupted==true → fail`; `interrupted==false → pass`; otherwise unknown (no recording). Documents Claude Code's `interrupted` field shape.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `cargo test` returning interrupted=false → recorded `pass`; returning interrupted=true → recorded `fail`. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/regression-gate.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 821 |
| **Audit ID** | `BC-AUDIT-1072` |
| **Source Line(s) (within hook)** | 49-64. |
...
| v1.2 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.09 (S-8.09 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
