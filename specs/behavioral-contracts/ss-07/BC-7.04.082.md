...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1849"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.082: validate-wave-gate-prerequisite: worker agents go through gate prerequisite check

## Description

validate-wave-gate-prerequisite: worker agents go through gate prerequisite check. First blocking earlier wave found → emit hook.block (`wave_gate_prerequisite_not_passed`, fields subagent/story_id/target_wave/blocking_wave/blocking_status). Multi-line stderr with two recovery options (run gate, mark deferred with rationale). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1173` (extracted from `pass-3-deep-hooks.md` line 1849).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-wave-gate-prerequisite.sh``.

## Preconditions

1. Trigger: subagent_type matches `*test-writer*`, `*implementer*`, `*demo-recorder*`, `*pr-manager*`, or `*devops-engineer*`. Story ID extracted from prompt (S-N.NN). wave-state.yaml resolved from cwd or `cd <path>` in prompt. Python finds story's wave; checks all earlier waves for `gate_status` not in `(passed, deferred)`.

## Postconditions

1. Behavior: First blocking earlier wave found → emit hook.block (`wave_gate_prerequisite_not_passed`, fields subagent/story_id/target_wave/blocking_wave/blocking_status). Multi-line stderr with two recovery options (run gate, mark deferred with rationale). Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Dispatch test-writer for S-2.05 (wave W2) while W1.gate_status=pending → block. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-wave-gate-prerequisite.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1849 |
| **Audit ID** | `BC-AUDIT-1173` |
| **Source Line(s) (within hook)** | 101-160. |
