...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1618"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.058: validate-subsystem-names: blocks BC `subsystem:` not in canonical SS-NN list

## Description

validate-subsystem-names: blocks BC `subsystem:` not in canonical SS-NN list. Error "BC file subsystem: \"X\" does not match any SS-ID". Block (`policy6_subsystem_name_mismatch`). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1149` (extracted from `pass-3-deep-hooks.md` line 1618).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-subsystem-names.sh``.

## Preconditions

1. Trigger: BC frontmatter `subsystem:` value not present in CANONICAL_IDS extracted from ARCH-INDEX `Subsystem Registry` table column 2.

## Postconditions

1. Behavior: Error "BC file subsystem: \"X\" does not match any SS-ID". Block (`policy6_subsystem_name_mismatch`). Exit 2.
2. Exit codes: 2.
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-subsystem-names.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1618 |
| **Audit ID** | `BC-AUDIT-1149` |
| **Source Line(s) (within hook)** | 90-108. |
