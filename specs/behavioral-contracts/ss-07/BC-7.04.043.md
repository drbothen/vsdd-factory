...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1466"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.043: validate-pr-review-posted: blocks `gh pr comment` fallback (not a review verdict)

## Description

validate-pr-review-posted: blocks `gh pr comment` fallback (not a review verdict). Error "Used 'gh pr comment' instead of 'gh pr review'". Block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1134` (extracted from `pass-3-deep-hooks.md` line 1466).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-pr-review-posted.sh``.

## Preconditions

1. Trigger: Result mentions `gh pr comment`.

## Postconditions

1. Behavior: Error "Used 'gh pr comment' instead of 'gh pr review'". Block.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1466 |
| **Audit ID** | `BC-AUDIT-1134` |
| **Source Line(s) (within hook)** | 44-46. |
