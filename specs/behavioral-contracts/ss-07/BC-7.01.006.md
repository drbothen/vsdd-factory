...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:451"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.01.006: validate-* family (24 validators on PostToolUse:Edit\

## Description

validate-* family (24 validators on PostToolUse:Edit|Write or all). Each validator exits 0 (pass) or 2 (block) with diagnostic on stderr; emits `hook.block` with reason.

**Source category:** Bash hook script contracts (broad-sweep).
**Audit ID:** `BC-AUDIT-068` (extracted from `pass-3-behavioral-contracts.md` line 451).
...
1. Each validator exits 0 (pass) or 2 (block) with diagnostic on stderr; emits `hook.block` with reason.
...
| **Source Document** | `pass-3-behavioral-contracts.md` line 451 |
| **Audit ID** | `BC-AUDIT-068` |
...
> Per-script source under `plugins/vsdd-factory/hooks/`. Sample (validate-novelty-assessment.sh) actively gating my own outputs in this session.
