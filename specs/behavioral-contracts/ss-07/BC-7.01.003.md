...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:433"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.01.003: regression-gate (PostToolUse) fails when bash command interrupted

## Description

regression-gate (PostToolUse) fails when bash command interrupted. Exit 2 with explanation; emits `hook.block`.

**Source category:** Bash hook script contracts (broad-sweep).
**Audit ID:** `BC-AUDIT-065` (extracted from `pass-3-behavioral-contracts.md` line 433).

## Preconditions

1. PostToolUse with `tool_response.interrupted = true`.

## Postconditions

1. Exit 2 with explanation; emits `hook.block`.
...
| **Source Document** | `pass-3-behavioral-contracts.md` line 433 |
| **Audit ID** | `BC-AUDIT-065` |
...
> `hooks/regression-gate.sh` (per CHANGELOG v1.0.0-beta.3).
