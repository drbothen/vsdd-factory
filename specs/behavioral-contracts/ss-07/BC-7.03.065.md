...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:734"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.065: red-gate: identity & registry binding

## Description

red-gate: identity & registry binding. TDD red-before-green enforcement, opt-in via `.factory/red-gate-state.json`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1064` (extracted from `pass-3-deep-hooks.md` line 734).
**Hook script:** ``plugins/vsdd-factory/hooks/red-gate.sh``.
**Registry entry:** `hooks-registry.toml::red-gate` (PreToolUse, tool=`Edit|Write`, priority=100, timeout_ms=5000, on_error=block)..
...
1. Behavior: TDD red-before-green enforcement, opt-in via `.factory/red-gate-state.json`.
2. Exit codes: 0 / 1 (jq missing) / 2 (strict violation).
...
| **Source Document** | `pass-3-deep-hooks.md` line 734 |
| **Audit ID** | `BC-AUDIT-1064` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:678-696` → `hooks-registry.toml::red-gate`. |
