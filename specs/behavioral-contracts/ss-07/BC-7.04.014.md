...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1173"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.014: validate-demo-evidence-story-scoped: blocks flat-level demo-evidence files

## Description

validate-demo-evidence-story-scoped: blocks flat-level demo-evidence files. Emit hook.block (`demo_evidence_not_story_scoped`); stderr "POL-010 VIOLATION: demo evidence must live under docs/demo-evidence/<STORY-ID>/". Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1105` (extracted from `pass-3-deep-hooks.md` line 1173).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-demo-evidence-story-scoped.sh``.

## Preconditions

1. Trigger: file_path matches `*/docs/demo-evidence/*` AND the relative path after `docs/demo-evidence/` contains no `/` (i.e., directly at top level).

## Postconditions

1. Behavior: Emit hook.block (`demo_evidence_not_story_scoped`); stderr "POL-010 VIOLATION: demo evidence must live under docs/demo-evidence/<STORY-ID>/". Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `docs/demo-evidence/evidence-report.md` blocked; `docs/demo-evidence/S-0.02/evidence-report.md` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1173 |
| **Audit ID** | `BC-AUDIT-1105` |
| **Source Line(s) (within hook)** | 35-53. |
