...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:441"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.038: factory-branch-guard: identity & registry binding

## Description

factory-branch-guard: identity & registry binding. Guards the worktree invariant: `.factory/` must be on `factory-artifacts` branch (or `.factory-project/` on `factory-project-artifacts`).

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1037` (extracted from `pass-3-deep-hooks.md` line 441).
**Hook script:** ``plugins/vsdd-factory/hooks/factory-branch-guard.sh``.
**Registry entry:** `hooks-registry.toml::factory-branch-guard` (PreToolUse, tool=`Edit|Write`, priority=60, timeout_ms=5000, on_error=block)..

## Preconditions

1. Hook event/tool match: PreToolUse Edit|Write..
2. Trigger: Edit/Write to any path containing `.factory/` or `.factory-project/`.

## Postconditions

1. Behavior: Guards the worktree invariant: `.factory/` must be on `factory-artifacts` branch (or `.factory-project/` on `factory-project-artifacts`).
2. Exit codes: 0 / 2.
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/factory-branch-guard.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 441 |
| **Audit ID** | `BC-AUDIT-1037` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:579-596` → `hooks-registry.toml::factory-branch-guard`. |
