...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:961"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.086: update-wave-state-on-merge: flips gate_status to pending when wave fully merged

## Description

update-wave-state-on-merge: flips gate_status to pending when wave fully merged. Sets `wave_data.gate_status = pending`, sets `state.next_gate_required = wave_name`, prints stderr reminder. Emits `hook.action` event (reason `wave_merge_recorded`) with `gate_transitioned=true` field.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1085` (extracted from `pass-3-deep-hooks.md` line 961).
**Hook script:** ``plugins/vsdd-factory/hooks/update-wave-state-on-merge.sh``.

## Preconditions

1. Trigger: `set(stories) == set(merged)` after append AND prior `gate_status` was `not_started` or None.

## Postconditions

1. Behavior: Sets `wave_data.gate_status = pending`, sets `state.next_gate_required = wave_name`, prints stderr reminder. Emits `hook.action` event (reason `wave_merge_recorded`) with `gate_transitioned=true` field.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Final story merge in W6 → `waves.W6.gate_status: pending` AND `next_gate_required: W6`. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 961 |
| **Audit ID** | `BC-AUDIT-1085` |
| **Source Line(s) (within hook)** | 87-101. |
...
| v1.2 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.04 (S-8.04 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
