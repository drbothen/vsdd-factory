...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:919"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.082: track-agent-stop: classifies result as ok\

## Description

track-agent-stop: classifies result as ok|empty|blocked, emits agent.stop. RESULT_LEN = trimmed-non-whitespace count of result. EXIT_CLASS = `empty` if 0, `blocked` if matches BLOCKED regex (same as pr-manager-completion-guard for parity), else `ok`. Emits `agent.stop subagent=… exit_class=… result_len=…`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1081` (extracted from `pass-3-deep-hooks.md` line 919).
**Hook script:** ``plugins/vsdd-factory/hooks/track-agent-stop.sh``.
...
1. Behavior: RESULT_LEN = trimmed-non-whitespace count of result. EXIT_CLASS = `empty` if 0, `blocked` if matches BLOCKED regex (same as pr-manager-completion-guard for parity), else `ok`. Emits `agent.stop subagent=… exit_class=… result_len=…`.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Empty result → `exit_class=empty`; "BLOCKED: dependency missing" → `exit_class=blocked`; normal result → `exit_class=ok`. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/track-agent-stop.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 919 |
| **Audit ID** | `BC-AUDIT-1081` |
| **Source Line(s) (within hook)** | 24-43. |
...
| v1.2 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.03 (S-8.03 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
