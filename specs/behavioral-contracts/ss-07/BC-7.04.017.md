...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1208"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.017: validate-finding-format: identity & registry binding

## Description

validate-finding-format: identity & registry binding. Finding ID format normalizer; on_error=continue.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1108` (extracted from `pass-3-deep-hooks.md` line 1208).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-finding-format.sh``.
**Registry entry:** `hooks-registry.toml::validate-finding-format` (PostToolUse, tool=`Edit|Write`, priority=290, timeout_ms=5000, on_error=continue)..
...
1. Behavior: Finding ID format normalizer; on_error=continue.
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1208 |
| **Audit ID** | `BC-AUDIT-1108` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:216-234` → `hooks-registry.toml::validate-finding-format`. |
