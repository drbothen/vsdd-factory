...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:765"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.068: red-gate: state file absent → skip

## Description

red-gate: state file absent → skip. Exit 0 — strict mode is opt-in.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1067` (extracted from `pass-3-deep-hooks.md` line 765).
**Hook script:** ``plugins/vsdd-factory/hooks/red-gate.sh``.

## Preconditions

1. Trigger: `.factory/red-gate-state.json` not on disk OR `mode != "strict"`.

## Postconditions

1. Behavior: Exit 0 — strict mode is opt-in.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 765 |
| **Audit ID** | `BC-AUDIT-1067` |
| **Source Line(s) (within hook)** | 52-66. |
