...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1694"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.066: validate-template-compliance: blocks missing frontmatter keys vs template

## Description

validate-template-compliance: blocks missing frontmatter keys vs template. MISSING_KEYS comma-joined; emit hook.block (`template_noncompliant`, fields template + missing_keys). Stderr lists count `present/total` and the missing list. Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1157` (extracted from `pass-3-deep-hooks.md` line 1694).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-template-compliance.sh``.

## Preconditions

1. Trigger: Template frontmatter keys differ from file frontmatter keys.

## Postconditions

1. Behavior: MISSING_KEYS comma-joined; emit hook.block (`template_noncompliant`, fields template + missing_keys). Stderr lists count `present/total` and the missing list. Exit 2.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1694 |
| **Audit ID** | `BC-AUDIT-1157` |
| **Source Line(s) (within hook)** | 130-157. |
