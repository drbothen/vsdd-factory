...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:372"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.031: destructive-command-guard: blocks `find -delete\

## Description

destructive-command-guard: blocks `find -delete|-exec rm` on protected paths. Block code `find_delete_protected`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1030` (extracted from `pass-3-deep-hooks.md` line 372).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: `find` with `-delete` or `-exec rm` AND command mentions `.factory`, `src`, or `tests` (word-boundary match).

## Postconditions

1. Behavior: Block code `find_delete_protected`.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `find .factory -name '*.tmp' -delete` blocked. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 372 |
| **Audit ID** | `BC-AUDIT-1030` |
| **Source Line(s) (within hook)** | 163-174. |
