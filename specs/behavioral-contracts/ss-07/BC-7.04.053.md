...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1571"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.053: validate-story-bc-sync: blocks frontmatter BC missing from body BC table

## Description

validate-story-bc-sync: blocks frontmatter BC missing from body BC table. Per missing BC, error to ERRFILE. Block (`policy8_bc_array_desync`). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1144` (extracted from `pass-3-deep-hooks.md` line 1571).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-story-bc-sync.sh``.

## Preconditions

1. Trigger: BC in `behavioral_contracts:` (or legacy `bcs:`) array but not in body BC table (greppable `BC-S.SS.NNN`).

## Postconditions

1. Behavior: Per missing BC, error to ERRFILE. Block (`policy8_bc_array_desync`). Exit 2.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1571 |
| **Audit ID** | `BC-AUDIT-1144` |
| **Source Line(s) (within hook)** | 91-95. |
