...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:638"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.056: protect-secrets: blocks copy/move of real .env (allows template→real)

## Description

protect-secrets: blocks copy/move of real .env (allows template→real). Block code `env_file_copy`. Direction-aware: `cp .env.example .env` allowed (source is template), `cp .env backup` blocked.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1055` (extracted from `pass-3-deep-hooks.md` line 638).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-secrets.sh``.

## Preconditions

1. Trigger: Bash command contains `cp|mv|rsync|scp` AND first positional arg basename is `.env`/`.envrc`/`.env.*` (not template).

## Postconditions

1. Behavior: Block code `env_file_copy`. Direction-aware: `cp .env.example .env` allowed (source is template), `cp .env backup` blocked.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 638 |
| **Audit ID** | `BC-AUDIT-1055` |
| **Source Line(s) (within hook)** | 132-161. |
