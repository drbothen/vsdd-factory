...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:863"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.077: session-learning: appends timestamped marker to .factory/sidecar-learning.md

## Description

session-learning: appends timestamped marker to .factory/sidecar-learning.md. If `.factory/sidecar-learning.md` does not exist, creates it with header. Then appends `- Session ended at <ISO8601-UTC> (awaiting /session-review)`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1076` (extracted from `pass-3-deep-hooks.md` line 863).
**Hook script:** ``plugins/vsdd-factory/hooks/session-learning.sh``.

## Preconditions

1. Trigger: `.factory/` directory exists.

## Postconditions

1. Behavior: If `.factory/sidecar-learning.md` does not exist, creates it with header. Then appends `- Session ended at <ISO8601-UTC> (awaiting /session-review)`.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Each session end produces exactly one new line in sidecar-learning.md. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/session-learning.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 863 |
| **Audit ID** | `BC-AUDIT-1076` |
| **Source Line(s) (within hook)** | 16-32. |
...
| v1.2 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.06 (S-8.06 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
