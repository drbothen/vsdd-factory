...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1142"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.011: validate-changelog-monotonicity: blocks rows with date older above newer (non-decreasing)

## Description

validate-changelog-monotonicity: blocks rows with date older above newer (non-decreasing). Error "date 'X' is newer than prior row 'Y' — changelog should be newest-first".

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1102` (extracted from `pass-3-deep-hooks.md` line 1142).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-changelog-monotonicity.sh``.

## Preconditions

1. Trigger: Date field on later row (newer entry expected to be at top) is greater than prior row's date. Format `YYYY-MM-DD`.

## Postconditions

1. Behavior: Error "date 'X' is newer than prior row 'Y' — changelog should be newest-first".
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Newer-at-top invariant: top row 2026-04-25, second row 2026-04-26 → block. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1142 |
| **Audit ID** | `BC-AUDIT-1102` |
| **Source Line(s) (within hook)** | 130-134. |
