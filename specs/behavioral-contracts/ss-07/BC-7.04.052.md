...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1560"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.052: validate-story-bc-sync: identity & registry binding

## Description

validate-story-bc-sync: identity & registry binding. Policy 8 enforcer (BC array bidirectional sync).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1143` (extracted from `pass-3-deep-hooks.md` line 1560).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-story-bc-sync.sh``.
**Registry entry:** `hooks-registry.toml::validate-story-bc-sync` (PostToolUse, tool=`Edit|Write`, priority=370, timeout_ms=5000, on_error=continue)..
...
1. Behavior: Policy 8 enforcer (BC array bidirectional sync).
2. Exit codes: 0 / 2.
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-story-bc-sync.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1560 |
| **Audit ID** | `BC-AUDIT-1143` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:376-394` → `hooks-registry.toml::validate-story-bc-sync`. |
