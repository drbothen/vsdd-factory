...
1. Behavior: Session-end safety net for forgotten wave gates.
2. Exit codes: Always 0 (advisory).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Stop event triggers; never blocks session end. | happy-path |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:799-816` → `hooks-registry.toml::warn-pending-wave-gate`. |
| v1.3 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.07 (S-8.07 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
