...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:605"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.053: protect-secrets: identity & dual registry binding

## Description

protect-secrets: identity & dual registry binding. Distinct tool dispatch in script body: `tool_name == "Read"` runs file-path check; `tool_name == "Bash"` runs command-content check. Otherwise exits 0.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1052` (extracted from `pass-3-deep-hooks.md` line 605).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-secrets.sh``.
**Registry entry:** `hooks-registry.toml::protect-secrets` (PreToolUse Bash, priority=80, on_error=block) **AND** `hooks-registry.toml::protect-secrets` (PreToolUse Read, priority=80, on_error=block) — duplicated for two distinct tools..

## Preconditions

1. Hook event/tool match: PreToolUse Bash AND PreToolUse Read..
2. Trigger: Every Bash or Read tool call.

## Postconditions

1. Behavior: Distinct tool dispatch in script body: `tool_name == "Read"` runs file-path check; `tool_name == "Bash"` runs command-content check. Otherwise exits 0.
2. Exit codes: 0 / 2.
3. Error policy: block.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Single hook script registered twice. Confirmed only such duplication in registry. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/protect-secrets.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 605 |
| **Audit ID** | `BC-AUDIT-1052` |
| **Source Line(s) (within hook)** | Header 1-22. |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:618-636` and `hooks-registry.toml:638-656` → `hooks-registry.toml::protect-secrets` (×2, Bash and Read tool entries). |
