...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1600"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.056: validate-subsystem-names: identity & registry binding

## Description

validate-subsystem-names: identity & registry binding. Policy 6 enforcer (canonical SS-NN registry).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1147` (extracted from `pass-3-deep-hooks.md` line 1600).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-subsystem-names.sh``.
**Registry entry:** `hooks-registry.toml::validate-subsystem-names` (PostToolUse, tool=`Edit|Write`, priority=380, timeout_ms=5000, on_error=continue)..
...
1. Behavior: Policy 6 enforcer (canonical SS-NN registry).
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1600 |
| **Audit ID** | `BC-AUDIT-1147` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:396-414` → `hooks-registry.toml::validate-subsystem-names`. |
