...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1063"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.003: validate-anchor-capabilities-union: blocks when frontmatter caps ≠ union over BC capability fields

## Description

validate-anchor-capabilities-union: blocks when frontmatter caps ≠ union over BC capability fields. Emits hook.block (`anchor_capabilities_mismatch`) with `expected` and `actual` fields. Stderr lists EXPECTED/ACTUAL CAP arrays and the BC→CAP mapping. Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1094` (extracted from `pass-3-deep-hooks.md` line 1063).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-anchor-capabilities-union.sh``.

## Preconditions

1. Trigger: Story has `anchor_bcs` (or `behavioral_contracts`) AND `anchor_capabilities`. For each BC, looks up `BC_DIR/${bc_id}*.md` and reads `capability:` field (single value or CSV). If union differs from anchor_capabilities → block.

## Postconditions

1. Behavior: Emits hook.block (`anchor_capabilities_mismatch`) with `expected` and `actual` fields. Stderr lists EXPECTED/ACTUAL CAP arrays and the BC→CAP mapping. Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Story anchors BC-2.04.007 (CAP-006) but lists `anchor_capabilities: [CAP-005]` → block. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-anchor-capabilities-union.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1063 |
| **Audit ID** | `BC-AUDIT-1094` |
| **Source Line(s) (within hook)** | 48-181. |
