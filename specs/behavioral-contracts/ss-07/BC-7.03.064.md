...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:722"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.064: purity-check: detects 16 forbidden side-effect patterns and warns

## Description

purity-check: detects 16 forbidden side-effect patterns and warns. Emits hook.block event with severity=warn, reason `pure_core_boundary_violation`, lists matched patterns. Stderr warn lists each hit; exit 0.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1063` (extracted from `pass-3-deep-hooks.md` line 722).
**Hook script:** ``plugins/vsdd-factory/hooks/purity-check.sh``.

## Preconditions

1. Trigger: File in pure/core path matches any of: `std::fs`, `std::env::var`, `std::process`, `std::io::stdin`, `reqwest`, `hyper::`, `tokio::spawn`, `tokio::time::sleep`, `tokio::fs`, `println!`, `eprintln!`, `File::open`, `File::create`, `fetch(`, `process.env`, `fs.readFile`, `fs.writeFile` (16 patterns; counted from PATTERNS array).

## Postconditions

1. Behavior: Emits hook.block event with severity=warn, reason `pure_core_boundary_violation`, lists matched patterns. Stderr warn lists each hit; exit 0.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | A `crates/foo/src/pure/bar.rs` with `println!` produces a warning pointing to SOUL.md. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/purity-check.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 722 |
| **Audit ID** | `BC-AUDIT-1063` |
| **Source Line(s) (within hook)** | 41-78. |
