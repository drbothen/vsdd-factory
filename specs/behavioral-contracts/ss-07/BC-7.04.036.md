...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1398"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.036: validate-pr-merge-prerequisites: identity & registry binding

## Description

validate-pr-merge-prerequisites: identity & registry binding. Pre-merge evidence guard; on_error=block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1127` (extracted from `pass-3-deep-hooks.md` line 1398).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-pr-merge-prerequisites.sh``.
**Registry entry:** `hooks-registry.toml::validate-pr-merge-prerequisites` (PreToolUse, tool=Agent, priority=120, timeout_ms=10000, on_error=block)..
...
1. Behavior: Pre-merge evidence guard; on_error=block.
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1398 |
| **Audit ID** | `BC-AUDIT-1127` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:718-736` → `hooks-registry.toml::validate-pr-merge-prerequisites`. |
