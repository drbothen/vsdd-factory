...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:483"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.042: handoff-validator: identity & registry binding

## Description

handoff-validator: identity & registry binding. Validates non-empty subagent output; on_error=block (script crash blocks the stop).

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1041` (extracted from `pass-3-deep-hooks.md` line 483).
**Hook script:** ``plugins/vsdd-factory/hooks/handoff-validator.sh``.
**Registry entry:** `hooks-registry.toml::handoff-validator` (SubagentStop, no tool filter, priority=910, timeout_ms=5000, on_error=block)..

## Preconditions

1. Hook event/tool match: SubagentStop..
2. Trigger: Every SubagentStop.

## Postconditions

1. Behavior: Validates non-empty subagent output; on_error=block (script crash blocks the stop).
2. Exit codes: 0 always (script body emits warnings only, never exits non-zero).
3. Error policy: block (registry); script always exits 0 — only a script-level crash would block.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Empty/short subagent results produce stderr WARN but never exit 2. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/handoff-validator.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 483 |
| **Audit ID** | `BC-AUDIT-1041` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:820-837` → `hooks-registry.toml::handoff-validator`. |
| v1.3 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.01 (S-8.01 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
