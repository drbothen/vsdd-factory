...
8. **precompact-flush-log append**: After the git commit succeeds, the hook APPENDS a new line to `.factory/hooks/precompact-flush-log` (creating the file if absent) containing four space-separated fields: `<ISO-timestamp> <SHA> <cycle>/<step> commit` where `<SHA>` is the 40-char hex of the resulting commit and `commit` is the literal string `commit` (the object type, pre-embedded by the shell hook — NOT the output of a live `git cat-file -t` call at validation time). **If the append fails, the hook writes a DIAGNOSTIC message to stderr (`precompact-flush: SHA append to precompact-flush-log failed; blocking compaction`) and exits 2 (block compaction).** An append failure means the log last-line remains the prior flush's SHA — BC-5.41.003 PC1(a) would see a SHA mismatch on the next MULTI_COMMIT_CHAIN check, causing a false-positive block (the regression CAP-032 prevents). Therefore append failure is durability-critical and must block. Prior lines are retained as history; only the last line is authoritative. The ONLY exit codes this hook ever produces are 0 (success, no-op, or STATE.md-unreadable fail-open) and 2 (git commit failure OR append failure blocks compaction). Exit 1 is not used.
...
| PreCompact event; git commit succeeds; precompact-flush-log append fails (e.g., filesystem permission error) | exit 2; stderr diagnostic `precompact-flush: SHA append to precompact-flush-log failed; blocking compaction`; block_intent=true propagated to harness | append-failure-block |
...
|--------|----------|-------------|
| VP-082 | precompact-flush.sh commits STATE.md + wave state before exiting; blocks (exit 2) on commit failure OR precompact-flush-log append failure; no-op when state is clean; renews factory lock per ADR-025 D11 | integration |
| VP-085 | precompact-flush.sh reads STATE.md+git only; ignores custom_instructions; determines current context from STATE.md `current_cycle:` + `current_step:` fields (no phantom `current_wave:` field); appends 4-field line `<ISO-timestamp> <SHA> <cycle>/<step> commit` to precompact-flush-log before push; exits 2 on append failure; exits 0 fail-open when STATE.md unreadable | unit-test |
...
| ADR | ADR-026 v1.4 Decision 6 (PreCompact shell hook; hermetic; blocking; fail-open on crash; lock renewal no-op when lock absent; on_error=continue) |
