...
8. **precompact-flush-log append (SHA-pinned concurrent-commit guard — F-P5-002 + F-P6-006)**: After the git commit succeeds, the hook immediately captures `SHA_B=$(git -C <factory-artifacts-worktree> rev-parse HEAD)`. It then APPENDS a new line to `.factory/hooks/precompact-flush-log` (creating the file if absent) containing four space-separated fields: `<ISO-timestamp> <SHA_B> <cycle>/<step> commit` where `commit` is the literal string `commit` (the object type, pre-embedded by the shell hook — NOT the output of a live `git cat-file -t` call at validation time). **If the append fails**, the hook executes the following SHA-pinned concurrent-commit guard:
   ...
   **Note on `--soft` staged-state behavior**: `--soft SHA_B^` restores the working tree index to the state staged for SHA_B. If STATE.md or other files changed between SHA_B creation and the reset (e.g., a concurrent state-manager turn), those changes appear as unstaged working-tree changes in the retry — they are NOT reverted. The retry commit WILL include those changes if they are re-staged. This is the correct behavior for the common case.
   The ordering is: (1) capture SHA_B, (2) git commit, (3) attempt append, (4) IF append fails: compare CURRENT_HEAD to SHA_B, (5) if equal: `reset --soft SHA_B^` then exit 2; if not equal: exit 2 with human-intervention message. Prior log lines are retained as history; only the last line is authoritative. The ONLY exit codes this hook ever produces are 0 (success, no-op, or STATE.md-unreadable fail-open) and 2 (git commit failure OR append-failure-after-SHA-pinned-reset OR concurrent-commit-guard-triggered blocks compaction). Exit 1 is not used.
...
| ADR | ADR-026 §Decision 6 (PreCompact shell hook; hermetic; blocking; fail-open on crash; lock renewal no-op when lock absent; on_error=continue; reset --soft pinned to captured SHA before exit 2 on append failure; concurrent-commit guard: only reset if HEAD == captured SHA_B per F-P6-006) |
