...
1. **STATE.md read from filesystem**: The hook reads `STATE.md` via direct filesystem read (`cat .factory/STATE.md` or equivalent); it determines `current_wave` from `STATE.md` frontmatter `current_wave:` field. It does NOT rely on in-context reasoning, environment variables set by the LLM, or piped session context.
...
3. **Factory lock renewed**: Before any git commit, the hook invokes `factory-lock-write.sh renew .factory/STATE.md` (ADR-025 D11 Mechanism 1). Lock renewal failure is treated as a non-fatal warning (advisory); the flush proceeds and attempts the commit regardless. Lock renewal failure does NOT cause exit 2.

4. **Factory-artifacts commit**: The hook stages and commits `STATE.md` (and any other factory-artifacts files with pending changes) to the `factory-artifacts` branch with commit message: `PreCompact flush wave-<N> <ISO-8601-timestamp>` where N is `current_wave` from STATE.md frontmatter.
...
3. **factory-lock-write.sh renew is mandatory step**: The lock renewal call must occur as a step before the git commit, not after. Order: (1) read STATE.md, (2) invoke `factory-lock-write.sh renew`, (3) git add, (4) git commit.

4. **Commit message format is canonical**: The commit message MUST match the pattern `PreCompact flush wave-<N> <ISO-timestamp>` exactly (with a single space between `wave-<N>` and the timestamp). This prefix is used by `validate-burst-log` and `validate-dispatch-advance` to exempt the commit from TD-VSDD-053 `MULTI_COMMIT_CHAIN_NOT_ALLOWED` detection (BC-5.41.003).
...
| EC-004 | factory-lock-write.sh renew fails (lock expired or not held) | Advisory warning to stderr; flush commit attempted anyway; exit 0 if commit succeeds |
...
| EC-007 | Two consecutive PreCompact events (rapid compactions) | Two flush commits on factory-artifacts with different timestamps; both exempt from TD-VSDD-053 via `PreCompact flush wave-` prefix; no multi-commit-chain violation |
...
|-------|----------------|----------|
| PreCompact event; STATE.md readable; pending changes on factory-artifacts | flush commit with msg `PreCompact flush wave-2 2026-06-14T...`; exit 0; SHA written to last-precompact-flush-sha | happy-path |
...
| L2 Domain Invariants | TBD-DI — hermetic flush + lock-renewal invariant; new invariant candidate for business-analyst |
| Architecture Module | SS-07 (Hook Bash Layer) — shell hook in `plugins/vsdd-factory/hooks/`; registry entry in `hooks-registry.toml` |
| ADR | ADR-026 v1.0 Decision 6 (PreCompact shell hook; hermetic; blocking; fail-open on crash; lock renewal; on_error=continue) |
