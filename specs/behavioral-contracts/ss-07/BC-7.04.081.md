...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1839"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.081: validate-wave-gate-prerequisite: adversary dispatches go through SHA-currency hook

## Description

validate-wave-gate-prerequisite: adversary dispatches go through SHA-currency hook. If hook missing, exit 0 (operator opt-in). Otherwise runs `bash $SHA_HOOK --project-root $SHA_PROJECT_ROOT`. Non-zero exit → emit hook.block (`sha_currency_failed`, subagent), prints multi-line stderr including hook output indented 2 spaces, recovery commands. Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1172` (extracted from `pass-3-deep-hooks.md` line 1839).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-wave-gate-prerequisite.sh``.

## Preconditions

1. Trigger: subagent_type matches `*adversary*`. Locates `verify-sha-currency.sh` at `.factory/hooks/` (cwd) or via `cd <project>` extracted from prompt.

## Postconditions

1. Behavior: If hook missing, exit 0 (operator opt-in). Otherwise runs `bash $SHA_HOOK --project-root $SHA_PROJECT_ROOT`. Non-zero exit → emit hook.block (`sha_currency_failed`, subagent), prints multi-line stderr including hook output indented 2 spaces, recovery commands. Exit 2.
2. Exit codes: 0 (pass) / 2 (SHA currency failed).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | adversary dispatch with stale .factory SHA → block; with clean SHA → pass. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1839 |
| **Audit ID** | `BC-AUDIT-1172` |
| **Source Line(s) (within hook)** | 47-100. |
