...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:689"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.061: protect-vp: denies edits to green VPs at .factory/specs/verification-properties/VP-*.md

## Description

protect-vp: denies edits to green VPs at .factory/specs/verification-properties/VP-*.md. Emits permissionDecision=deny citing SOUL.md #4 immutability and `Supersedes:` field convention.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1060` (extracted from `pass-3-deep-hooks.md` line 689).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-vp.sh``.

## Preconditions

1. Trigger: file_path matches `.factory/specs/verification-properties/VP-.*\.md$` AND file exists AND contains `^Status: green`.

## Postconditions

1. Behavior: Emits permissionDecision=deny citing SOUL.md #4 immutability and `Supersedes:` field convention.
2. Exit codes: 0 (deny in JSON).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Editing `VP-001.md` with Status: green produces deny envelope. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 689 |
| **Audit ID** | `BC-AUDIT-1060` |
| **Source Line(s) (within hook)** | 51-63. |
