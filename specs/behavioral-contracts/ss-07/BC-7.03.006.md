...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:98"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.006: brownfield-discipline: identity & registry binding

## Description

brownfield-discipline: identity & registry binding. Header documents intent: enforces .reference/** read-only invariant (brownfield ingest cloned source-of-truth). Registered with `block` error policy.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1005` (extracted from `pass-3-deep-hooks.md` line 98).
**Hook script:** ``plugins/vsdd-factory/hooks/brownfield-discipline.sh``.
**Registry entry:** `hooks-registry.toml::brownfield-discipline` (event=PreToolUse, tool=`Edit|Write`, priority=30, timeout_ms=5000, on_error=block)..

## Preconditions

1. Hook event/tool match: PreToolUse on Edit|Write (regex alternation)..
2. Trigger: Every Edit or Write tool invocation.

## Postconditions

1. Behavior: Header documents intent: enforces .reference/** read-only invariant (brownfield ingest cloned source-of-truth). Registered with `block` error policy.
2. Exit codes: 0 allow / 1 (jq missing) / 2 deny.
...
| **Source Document** | `pass-3-deep-hooks.md` line 98 |
| **Audit ID** | `BC-AUDIT-1005` |
| **Source Line(s) (within hook)** | 1-12. |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:518-536` → `hooks-registry.toml::brownfield-discipline`. |
