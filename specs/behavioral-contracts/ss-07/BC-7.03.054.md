...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:618"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.054: protect-secrets: blocks Read of `.env`/`.envrc`/`.env.*` (excludes `.example`/`.sample`/`.template`)

## Description

protect-secrets: blocks Read of `.env`/`.envrc`/`.env.*` (excludes `.example`/`.sample`/`.template`). `is_secret_env_filename` returns true for `.env`, `.envrc`, or `.env.*` (not `.env.example|.sample|.template`). Block code `env_file_read_direct`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1053` (extracted from `pass-3-deep-hooks.md` line 618).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-secrets.sh``.

## Preconditions

1. Trigger: Tool == Read AND `is_secret_env_filename(file_path)` returns true.

## Postconditions

1. Behavior: `is_secret_env_filename` returns true for `.env`, `.envrc`, or `.env.*` (not `.env.example|.sample|.template`). Block code `env_file_read_direct`.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Read `.env` blocked; Read `.env.example` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 618 |
| **Audit ID** | `BC-AUDIT-1053` |
| **Source Line(s) (within hook)** | 55-77. |
