...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:445"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.01.005: check-factory-commit warns when committing in `.factory/` without STATE.md update

## Description

check-factory-commit warns when committing in `.factory/` without STATE.md update. Outputs `additionalContext` advisory; exit 0 (non-blocking).

**Source category:** Bash hook script contracts (broad-sweep).
**Audit ID:** `BC-AUDIT-067` (extracted from `pass-3-behavioral-contracts.md` line 445).

## Preconditions

1. PreToolUse on Bash, command contains "git commit". Hook is registered as `event = "PreToolUse"` in `hooks-registry.toml` and `hooks.json`. Note: the script header comment incorrectly says `# PostToolUse hook` — this is a documentation error in the script body, not in the registry. The .factory/STATE.md content check still happens, but as advisory output (`additionalContext` JSON) rather than as a precondition filter.

## Postconditions

1. Outputs `additionalContext` advisory; exit 0 (non-blocking).
...
| **Source Document** | `pass-3-behavioral-contracts.md` line 445 |
| **Audit ID** | `BC-AUDIT-067` |
...
> `hooks/check-factory-commit.sh` (read directly).
