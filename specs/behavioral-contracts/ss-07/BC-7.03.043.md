...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:496"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.043: handoff-validator: warns on empty subagent result

## Description

handoff-validator: warns on empty subagent result. Emits hook.block event (severity=warn, reason `subagent_empty_result`), prints to stderr "subagent ... returned an empty result", exits 0.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1042` (extracted from `pass-3-deep-hooks.md` line 496).
**Hook script:** ``plugins/vsdd-factory/hooks/handoff-validator.sh``.

## Preconditions

1. Trigger: `last_assistant_message`/`result`/`output` field stripped of whitespace has length 0.

## Postconditions

1. Behavior: Emits hook.block event (severity=warn, reason `subagent_empty_result`), prints to stderr "subagent ... returned an empty result", exits 0.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | SubagentStop with empty result produces stderr warn but does not block. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 496 |
| **Audit ID** | `BC-AUDIT-1042` |
| **Source Line(s) (within hook)** | 37-46. |
...
| v1.2 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.01 (S-8.01 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
