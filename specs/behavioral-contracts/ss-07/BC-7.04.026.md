...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1300"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.026: validate-input-hash: BLOCKS on non-hex chars

## Description

validate-input-hash: BLOCKS on non-hex chars. Emits hook.block (`input_hash_invalid_format`, issue=chars). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1117` (extracted from `pass-3-deep-hooks.md` line 1300).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-input-hash.sh``.

## Preconditions

1. Trigger: Hash length 7 but does not match `^[0-9a-f]{7}$`.

## Postconditions

1. Behavior: Emits hook.block (`input_hash_invalid_format`, issue=chars). Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Hash `ABC1234` (uppercase) blocked; hash `XYZ` rejected (non-hex). | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1300 |
| **Audit ID** | `BC-AUDIT-1117` |
| **Source Line(s) (within hook)** | 97-104. |
