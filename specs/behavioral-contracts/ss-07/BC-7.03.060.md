...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:678"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.060: protect-vp: identity & registry binding

## Description

protect-vp: identity & registry binding. Twin of protect-bc, but for `.factory/specs/verification-properties/VP-*.md`. Identical envelope-based decision pattern.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1059` (extracted from `pass-3-deep-hooks.md` line 678).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-vp.sh``.
**Registry entry:** `hooks-registry.toml::protect-vp` (PreToolUse, tool=`Edit|Write`, priority=90, timeout_ms=5000, on_error=block)..
...
1. Behavior: Twin of protect-bc, but for `.factory/specs/verification-properties/VP-*.md`. Identical envelope-based decision pattern.
2. Exit codes: Always 0; decision in stdout JSON.
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/protect-vp.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 678 |
| **Audit ID** | `BC-AUDIT-1059` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:658-676` → `hooks-registry.toml::protect-vp`. |
