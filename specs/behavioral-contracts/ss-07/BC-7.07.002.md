...
modified:
  ...
  - "2026-06-15 (v1.10) — fix burst (product-owner): (F-P33-001) §Changelog: defensive skip-marker annotation added for v1.5 absence — coordinated-burst skip (only behaviorally-changed BCs bumped at pass-5 F-P5-002/003 burst); gap v1.4→v1.6 is intentional, not a lost row. Exhaustive 8-BC sibling-sweep per F-P33-001."
...
`postcompact-reanchor.sh` is an advisory shell hook registered under `event = "PostCompact"` in `hooks-registry.toml`. It fires after the harness has completed context compaction. Its sole purpose is to emit a concise re-anchor block to stdout (visible to the LLM as fresh context) that re-states `current_step` and `current_cycle` from `factory-artifacts` STATE.md (via `git show`), and the authoritative `develop_sha` from `git rev-parse origin/develop` — not from in-context memory. PostCompact is inherently non-blocking in the Claude Code harness; this hook cannot prevent or reverse compaction regardless of its exit code. It does NOT commit to `factory-artifacts` (commits are the PreCompact flush's responsibility). **This hook is explicitly BEST-EFFORT and is NOT part of the CAP-032 continuity-guarantee chain.** The CAP-032 guarantee rests exclusively on Part A (HANDOFF.md) and Part B (PreCompact flush). This hook's failure, crash, or absence has no effect on the CAP-032 guarantee. Its output is purely informational re-anchoring after the fact.
...
1. The hook is registered in `hooks-registry.toml` as:
   ...
   ```
   (Canonical capabilities block per ADR-026 §Decision 7. Note: `GIT_DIR` is NOT in `env_allow` — it was a bats test-fixture artifact, not a production need. STATE.md is read via `git show` subprocess (covered by `exec_subprocess`), not `read_file`. The only direct filesystem write is the JSONL log at `.factory/logs/`.)
...
1. **Re-anchor block emitted to stdout**: The hook emits a concise (2–5 line) re-anchor summary to stdout in a structured format readable by the LLM:
   ...
   ```
   `current_cycle` and `current_step` are read from `factory-artifacts` STATE.md via `git show factory-artifacts:.factory/STATE.md`, NOT from in-context knowledge. `develop_sha` (the `sha=` value) is sourced from `git rev-parse origin/develop` (or `refs/remotes/origin/develop`) executed at hook invocation time — NOT from any STATE.md field. `last_verified_develop_sha:` is a HANDOFF.md field (ADR-026 §Decision 2 schema), NOT a STATE.md frontmatter field; reading it from STATE.md yields empty string on every real PostCompact firing (F-P1-001 phantom-field anchoring defect). On git failure, the hook falls through to `sha=UNKNOWN` with `status=warn` (fail-open, DI-024 advisory; exit 0 always). Wave identity is derived — not stored as a `current_wave:` field. The hook emits `current_cycle:` and `current_step:` from STATE.md frontmatter; the human can derive the wave/pass ordinal from `current_step:` if needed.

2. **Log written to .factory/logs/**: The hook appends a structured log entry to `.factory/logs/postcompact-reanchor-YYYY-MM-DD.jsonl` (same daily-file pattern as the dispatcher internal log) with fields: `event`, `current_cycle`, `current_step`, `develop_sha`, `timestamp`, `status` ("ok" or "warn"). `current_cycle` and `current_step` are read from `factory-artifacts` STATE.md via `git show` (same git-sourced values as PC1's stdout output); `develop_sha` is sourced from `git rev-parse origin/develop` (same source as PC1's `sha=` value) — NOT from any STATE.md field. There is no `wave_id` field because the hook has no defined source for wave-ordinal data.
...
2. **Values sourced from git, not in-context**: `current_cycle:` and `current_step:` values in the re-anchor output must be read from `git show factory-artifacts:.factory/STATE.md` (factory-artifacts branch STATE.md frontmatter), never from the LLM's in-context knowledge or environment variables. `develop_sha` (the `sha=` value) must be sourced from `git rev-parse origin/develop` executed at hook invocation time — NOT from any STATE.md frontmatter field. `last_verified_develop_sha:` is a HANDOFF.md field (ADR-026 §Decision 2 schema), NOT a STATE.md frontmatter field; it must not be read from `git show factory-artifacts:.factory/STATE.md`. There is no `current_wave:` field in STATE.md — the hook reads `current_cycle:` and `current_step:` and emits those. This is the re-anchor's entire purpose: providing authoritative grounding after compaction potentially corrupted in-context state.
...
|-------|----------------|----------|
| PostCompact event; factory-artifacts STATE.md has `current_cycle: v1.0-example`, `current_step: phase-F2` (no `last_verified_develop_sha` field — that is a HANDOFF.md field); `git rev-parse origin/develop` returns `abc...def` | stdout: `[PostCompact Re-anchor] context=v1.0-example/phase-F2 sha=abc...def`; log entry appended with `develop_sha: abc...def`; exit 0 | happy-path |
...
| PostCompact event followed by LLM tool call | LLM context includes the re-anchor stdout; LLM uses verified sha/cycle/step from that block | re-anchor-visible |
...
|--------|----------|-------------|
| VP-089 | postcompact-reanchor.sh Emits Re-Anchor Block From Git-Sourced STATE.md, Appends Log Entry, Makes No factory-artifacts Commits, Exits 0 on All Error Paths — postcompact-reanchor.sh emits re-anchor block to stdout (git-sourced, not in-context: current_cycle+current_step from git show factory-artifacts STATE.md; develop_sha from git rev-parse origin/develop at hook time — NOT from last_verified_develop_sha in STATE.md, which is a HANDOFF.md field); appends structured entry to .factory/logs/postcompact-reanchor-YYYY-MM-DD.jsonl (fields: event, current_cycle, current_step, develop_sha, timestamp, status; no wave_id field); does NOT commit or push to factory-artifacts (Inv1 absolute prohibition); exits 0 on all error paths including factory-artifacts unreachable, STATE.md absent, git absent, log-write failure, hook crash (PC5/EC-002/EC-004/EC-005) | unit-test/bats |
...
|---------|------|--------|--------|
| v1.13 | 2026-06-25 | product-owner | (adv P1 F-P1-001/F-P1-003) SHA source corrected to git rev-parse origin/develop (last_verified_develop_sha is not a STATE.md field); PC1 re-anchor block template updated: sha=<develop_sha>; PC1 description updated to clarify develop_sha sourced from git rev-parse origin/develop at hook time. PC2 log schema: field renamed last_verified_develop_sha→develop_sha; description updated to clarify git-sourced values. Inv2: rewritten to specify current_cycle+current_step from git show factory-artifacts STATE.md, develop_sha from git rev-parse origin/develop — neither from env vars nor in-context; last_verified_develop_sha is HANDOFF.md field not STATE.md field. Precondition 1 capabilities block: full canonical TOML per ADR-026 §Decision 7 added (env_allow + exec_subprocess + write_file path_allow [".factory/logs/"]); GIT_DIR not in env_allow (test artifact); read_file replaced with write_file. §Description updated to replace last_verified_develop_sha mention with develop_sha + git rev-parse source. Per architect adjudication 539b9a9f. |
| v1.12 | 2026-06-16 | product-owner | DEFERRED-VP (F3, S-18.05) resolved — VP-089 now exists per F2 gate human directive. §VP Anchors prose replaced with active bullet reference to VP-089; §Verification Properties row updated from DEFERRED-VP to VP-089 with verbatim H1 title "postcompact-reanchor.sh Emits Re-Anchor Block From Git-Sourced STATE.md, Appends Log Entry, Makes No factory-artifacts Commits, Exits 0 on All Error Paths". Title-cite parity verified against VP-089 H1 verbatim. |
| v1.11 | 2026-06-15 | product-owner | (F-P35-002) §Changelog v1.5 skip-marker: de-enumerated explanatory clause — removed false BC-ID enumeration that claimed which specific BCs changed at the pass-5 burst (premise was false per F-P35-002); reworded to be self-contained without enumerating changed BCs, which makes the marker robust against premise errors. |
| v1.10 | 2026-06-15 | product-owner | (F-P33-001 MEDIUM) §Changelog: defensive skip-marker annotation added for v1.5 absence — v1.5 was a coordinated-burst skip; the jump v1.4→v1.6 is intentional, not a lost row. Exhaustive sibling-sweep of all 8 E-18 BCs per F-P33-001 obligation. |
