...
|--------|----------|-------------|
| VP-089 | postcompact-reanchor.sh Emits Re-Anchor Block From Git-Sourced STATE.md, Appends Log Entry, Makes No factory-artifacts Commits, Exits 0 on All Error Paths — postcompact-reanchor.sh emits re-anchor block to stdout from factory-artifacts STATE.md (git-sourced, not in-context: current_cycle+current_step+last_verified_develop_sha); appends structured entry to .factory/logs/postcompact-reanchor-YYYY-MM-DD.jsonl (no wave_id field); does NOT commit or push to factory-artifacts (Inv1 absolute prohibition); exits 0 on all error paths including factory-artifacts unreachable, STATE.md absent, git absent, log-write failure, hook crash (PC5/EC-002/EC-004/EC-005) | unit-test/bats |
