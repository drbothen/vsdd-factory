...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1798"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.077: validate-wave-gate-completeness: blocks gate_status:passed without gate_report

## Description

validate-wave-gate-completeness: blocks gate_status:passed without gate_report. Error "X: gate_status=passed but no gate_report path specified".

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1168` (extracted from `pass-3-deep-hooks.md` line 1798).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-wave-gate-completeness.sh``.

## Preconditions

1. Trigger: Wave passed AND no `gate_report:` value (python emits `<wave>:no_report`).

## Postconditions

1. Behavior: Error "X: gate_status=passed but no gate_report path specified".
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1798 |
| **Audit ID** | `BC-AUDIT-1168` |
| **Source Line(s) (within hook)** | 86-89. |
