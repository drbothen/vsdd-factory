...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:774"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.069: red-gate: blocks edits to files not in `red[]` (path normalization 4-way)

## Description

red-gate: blocks edits to files not in `red[]` (path normalization 4-way). All four lookups fail → emits hook.block (`red_gate_strict_violation`), prints "Write a failing test first, then add the path to .factory/red-gate-state.json under .red[]". Exit 2.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1068` (extracted from `pass-3-deep-hooks.md` line 774).
**Hook script:** ``plugins/vsdd-factory/hooks/red-gate.sh``.

## Preconditions

1. Trigger: State file has `mode: strict`; file_path is not in `.red[]` after trying 4 normalizations: 1. Raw `$FILE_PATH` 2. `${FILE_PATH#./}` strip leading `./` 3. `${FILE_PATH#$PWD/}` strip cwd prefix 4. `${FILE_PATH#$GIT_ROOT/}` strip git root prefix

## Postconditions

1. Behavior: All four lookups fail → emits hook.block (`red_gate_strict_violation`), prints "Write a failing test first, then add the path to .factory/red-gate-state.json under .red[]". Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Strict mode + `src/foo.rs` not in `red[]` → block; same with `red: ["src/foo.rs"]` → allowed. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/red-gate.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 774 |
| **Audit ID** | `BC-AUDIT-1068` |
| **Source Line(s) (within hook)** | 68-107. |
