...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1386"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.035: validate-pr-description-completeness: blocks unresolved {placeholder} tokens

## Description

validate-pr-description-completeness: blocks unresolved {placeholder} tokens. Error "Unresolved template placeholders: {p1}, {p2}, ...". Block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1126` (extracted from `pass-3-deep-hooks.md` line 1386).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-pr-description-completeness.sh``.

## Preconditions

1. Trigger: File contains `{[a-z_]+}` patterns (template placeholders).

## Postconditions

1. Behavior: Error "Unresolved template placeholders: {p1}, {p2}, ...". Block.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `pr-description.md` containing `{story_id}` triggers block. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1386 |
| **Audit ID** | `BC-AUDIT-1126` |
| **Source Line(s) (within hook)** | 58-63. |
