...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1196"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.016: validate-factory-path-root: blocks paths through .worktrees/<X>/.factory/

## Description

validate-factory-path-root: blocks paths through .worktrees/<X>/.factory/. Extracts worktree name; computes relative-from-.factory; emits hook.block (`factory_path_worktree_relative`). Stderr "FACTORY PATH ERROR — writing to worktree instead of project root" with the absolute project-root path the agent should have used. Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1107` (extracted from `pass-3-deep-hooks.md` line 1196).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-factory-path-root.sh``.

## Preconditions

1. Trigger: file_path contains `.factory/` AND ALSO contains `.worktrees/<name>/.factory/`.

## Postconditions

1. Behavior: Extracts worktree name; computes relative-from-.factory; emits hook.block (`factory_path_worktree_relative`). Stderr "FACTORY PATH ERROR — writing to worktree instead of project root" with the absolute project-root path the agent should have used. Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `.worktrees/STORY-001/.factory/STATE.md` blocked. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-factory-path-root.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1196 |
| **Audit ID** | `BC-AUDIT-1107` |
| **Source Line(s) (within hook)** | 41-66. |
