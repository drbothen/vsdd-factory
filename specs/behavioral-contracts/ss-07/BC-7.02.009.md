...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts-deep-r1.md:248"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.02.009: Native (non-legacy) hook plugins MUST link `vsdd-hook-sdk` and use the `#[hook]` macro (not curre...

## Description

Native (non-legacy) hook plugins MUST link `vsdd-hook-sdk` and use the `#[hook]` macro (not currently used in production registry). The plugin must be a Cargo crate whose entry function is annotated `#[hook]` and whose signature is exactly `fn(payload: HookPayload) -> HookResult`. Async / unsafe / multi-arg / non-`HookResult`-return signatures fail the proc-macro check at compile time. Sole production-ready candidate today is the stub `hook-plugins/capture-commit-activity` (S-3.1, not yet swapped in).

**Source category:** Validator hook class contracts.
**Audit ID:** `BC-AUDIT-107` (extracted from `pass-3-behavioral-contracts-deep-r1.md` line 248).

## Preconditions

1. A `[[hooks]]` entry references a non-`legacy-bash-adapter.wasm` plugin path.

## Postconditions

1. The plugin must be a Cargo crate whose entry function is annotated `#[hook]` and whose signature is exactly `fn(payload: HookPayload) -> HookResult`. Async / unsafe / multi-arg / non-`HookResult`-return signatures fail the proc-macro check at compile time. Sole production-ready candidate today is the stub `hook-plugins/capture-commit-activity` (S-3.1, not yet swapped in).
...
| **Source Document** | `pass-3-behavioral-contracts-deep-r1.md` line 248 |
| **Audit ID** | `BC-AUDIT-107` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hook-sdk-macros/src/lib.rs:38-95` → `crates/hook-sdk-macros/src/lib.rs::validate_signature`; `hook-sdk-macros/src/lib.rs:47-55` → `crates/hook-sdk-macros/src/lib.rs::hook`. |

#### Original Source Evidence Quote

> `crates/hook-sdk-macros/src/lib.rs::validate_signature` (enforces async/unsafe/arg-count/return-type); `crates/hook-sdk-macros/src/lib.rs::hook` (generates `fn main() { run(user_fn) }`); pass-6 DRIFT/story-coverage map confirms no native hook in production registry as of beta.4.
