...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:175"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.013: capture-commit-activity: emits `commit.made` event via emit-event helper

## Description

capture-commit-activity: emits `commit.made` event via emit-event helper. Calls `${CLAUDE_PLUGIN_ROOT}/bin/emit-event type=commit.made hook=capture-commit-activity matcher=Bash commit_sha=… branch=… [message_subject=…] [amended=true]`. Helper is fail-tolerant (`2>/dev/null || true`).

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1012` (extracted from `pass-3-deep-hooks.md` line 175).
**Hook script:** ``plugins/vsdd-factory/hooks/capture-commit-activity.sh``.

## Preconditions

1. Trigger: All filters passed and preamble parsed.

## Postconditions

1. Behavior: Calls `${CLAUDE_PLUGIN_ROOT}/bin/emit-event type=commit.made hook=capture-commit-activity matcher=Bash commit_sha=… branch=… [message_subject=…] [amended=true]`. Helper is fail-tolerant (`2>/dev/null || true`).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Successful commit produces a single line in `.factory/logs/events-*.jsonl` with `type=commit.made`. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 175 |
| **Audit ID** | `BC-AUDIT-1012` |
| **Source Line(s) (within hook)** | 122-133. |
