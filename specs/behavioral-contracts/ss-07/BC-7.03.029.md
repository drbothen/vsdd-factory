...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:352"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.029: destructive-command-guard: blocks `rm` on source-of-truth files (8 SOT files)

## Description

destructive-command-guard: blocks `rm` on source-of-truth files (8 SOT files). Block code `sot_delete`. Even non-recursive `rm STATE.md` is blocked.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1028` (extracted from `pass-3-deep-hooks.md` line 352).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: Any `rm` command mentioning `STATE.md`, `BC-INDEX.md`, `VP-INDEX.md`, `STORY-INDEX.md`, `ARCH-INDEX.md`, `HS-INDEX.md`, `L2-INDEX.md`, or `prd.md`.

## Postconditions

1. Behavior: Block code `sot_delete`. Even non-recursive `rm STATE.md` is blocked.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `rm STATE.md` blocked. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 352 |
| **Audit ID** | `BC-AUDIT-1028` |
| **Source Line(s) (within hook)** | 110-122. |
