...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:908"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.081: track-agent-stop: identity & registry binding

## Description

track-agent-stop: identity & registry binding. Telemetry; never blocks; on_error=continue.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1080` (extracted from `pass-3-deep-hooks.md` line 908).
**Hook script:** ``plugins/vsdd-factory/hooks/track-agent-stop.sh``.
**Registry entry:** `hooks-registry.toml::track-agent-stop` (SubagentStop, no tool filter, priority=930, timeout_ms=5000, on_error=continue)..

## Preconditions

1. Hook event/tool match: SubagentStop (all subagents)..
...
| **Source Document** | `pass-3-deep-hooks.md` line 908 |
| **Audit ID** | `BC-AUDIT-1080` |
| **Source Line(s) (within hook)** | Header 1-14. |
