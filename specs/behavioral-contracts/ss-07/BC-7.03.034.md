...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:402"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.034: destructive-command-guard: blocks `git rm` on living spec/story files

## Description

destructive-command-guard: blocks `git rm` on living spec/story files. Block code `git_rm_protected` with hint "use lifecycle status fields (retired, deprecated)".

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1033` (extracted from `pass-3-deep-hooks.md` line 402).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: `git rm` mentions `.factory/specs/`, `.factory/stories/`, or `.factory/STATE.md`.

## Postconditions

1. Behavior: Block code `git_rm_protected` with hint "use lifecycle status fields (retired, deprecated)".
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 402 |
| **Audit ID** | `BC-AUDIT-1033` |
| **Source Line(s) (within hook)** | 292-302. |
