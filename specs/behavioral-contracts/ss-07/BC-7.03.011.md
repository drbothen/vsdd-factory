...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:155"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.011: capture-commit-activity: matches `git commit` as real subcommand and parses preamble

## Description

capture-commit-activity: matches `git commit` as real subcommand and parses preamble. Scans every stdout line for `[<branch> <sha>] <subject>` pattern; SHA must match `^[0-9a-f]{7,40}$`. Extracts last bracket token as SHA, first as branch. Defensive re-validation. Skips when no preamble matches.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1010` (extracted from `pass-3-deep-hooks.md` line 155).
**Hook script:** ``plugins/vsdd-factory/hooks/capture-commit-activity.sh``.

## Preconditions

1. Trigger: Successful Bash command containing `git commit` (with shell-boundary regex avoiding inside-string false positives).

## Postconditions

1. Behavior: Scans every stdout line for `[<branch> <sha>] <subject>` pattern; SHA must match `^[0-9a-f]{7,40}$`. Extracts last bracket token as SHA, first as branch. Defensive re-validation. Skips when no preamble matches.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Output `[main abc1234] feat: foo` produces commit_sha=abc1234, branch=main, message_subject=`feat: foo`. Output without bracket line: no event. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 155 |
| **Audit ID** | `BC-AUDIT-1010` |
| **Source Line(s) (within hook)** | 62-105. |
