...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1073"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.004: validate-anchor-capabilities-union: missing BC files warn-only

## Description

validate-anchor-capabilities-union: missing BC files warn-only. Stderr "ANCHOR CAPABILITIES NOTE: BC files not found for: ..." but does not block. Allows new BCs not yet on disk.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1095` (extracted from `pass-3-deep-hooks.md` line 1073).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-anchor-capabilities-union.sh``.

## Preconditions

1. Trigger: Anchor BC file not found in BC_DIR.

## Postconditions

1. Behavior: Stderr "ANCHOR CAPABILITIES NOTE: BC files not found for: ..." but does not block. Allows new BCs not yet on disk.
2. Exit codes: 0 (continues processing).
...
| **Source Document** | `pass-3-deep-hooks.md` line 1073 |
| **Audit ID** | `BC-AUDIT-1095` |
| **Source Line(s) (within hook)** | 156-158. |
