...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1185"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.015: validate-factory-path-root: identity & registry binding

## Description

validate-factory-path-root: identity & registry binding. Catches relative-path .factory/ writes from inside worktrees. on_error=**block** (one of only 5 PostToolUse validators with block policy).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1106` (extracted from `pass-3-deep-hooks.md` line 1185).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-factory-path-root.sh``.
**Registry entry:** `hooks-registry.toml::validate-factory-path-root` (PostToolUse, tool=`Edit|Write`, priority=280, timeout_ms=5000, on_error=block)..
...
1. Behavior: Catches relative-path .factory/ writes from inside worktrees. on_error=**block** (one of only 5 PostToolUse validators with block policy).
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1185 |
| **Audit ID** | `BC-AUDIT-1106` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:196-214` → `hooks-registry.toml::validate-factory-path-root`. |
