...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1239"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.020: validate-index-self-reference: identity & registry binding

## Description

validate-index-self-reference: identity & registry binding. Advisory only — never blocks (script always exits 0).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1111` (extracted from `pass-3-deep-hooks.md` line 1239).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-index-self-reference.sh``.
**Registry entry:** `hooks-registry.toml::validate-index-self-reference` (PostToolUse, tool=`Edit|Write`, priority=300, timeout_ms=5000, on_error=continue)..
...
1. Behavior: Advisory only — never blocks (script always exits 0).
...
| **Source Document** | `pass-3-deep-hooks.md` line 1239 |
| **Audit ID** | `BC-AUDIT-1111` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:236-254` → `hooks-registry.toml::validate-index-self-reference`. |
