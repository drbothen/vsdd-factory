...
> `hooks-registry.toml:19-36` (capture-commit-activity entry: priority 110, on_error continue, exec_subprocess capability with `binary_allow = ["bash", "git", "gh", "jq"]`); broader stanza-count + on_error distribution from independent `grep | sort | uniq -c` (18 block / 27 continue).
