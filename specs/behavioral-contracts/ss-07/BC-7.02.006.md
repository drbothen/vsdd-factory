...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts-deep-r1.md:227"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.02.006: factory-dispatcher routing binds hooks via `[[hooks]]` entry: `name`, `event`, optional `tool` re...

## Description

factory-dispatcher routing binds hooks via `[[hooks]]` entry: `name`, `event`, optional `tool` regex, `priority`, `timeout_ms`, `on_error`. Routing tuple is `(name, event, tool?, priority, timeout_ms, on_error)`. Independently confirmed: registry has 45 entries, `priority` ranges 20..950, `on_error` is one of `block` (18 entries) or `continue` (27 entries). All 45 entries route via `plugin = "hook-plugins/legacy-bash-adapter.wasm"`. Per-entry capabilities live at `[hooks.capabilities]` and `[hooks.capabilities.exec_subprocess]`.

**Source category:** Validator hook class contracts.
**Audit ID:** `BC-AUDIT-104` (extracted from `pass-3-behavioral-contracts-deep-r1.md` line 227).

## Preconditions

1. A `hooks-registry.toml` `[[hooks]]` stanza loaded via `Registry::parse_str`.

## Postconditions

1. Routing tuple is `(name, event, tool?, priority, timeout_ms, on_error)`. Independently confirmed: registry has 45 entries, `priority` ranges 20..950, `on_error` is one of `block` (18 entries) or `continue` (27 entries). All 45 entries route via `plugin = "hook-plugins/legacy-bash-adapter.wasm"`. Per-entry capabilities live at `[hooks.capabilities]` and `[hooks.capabilities.exec_subprocess]`.
...
| **Source Document** | `pass-3-behavioral-contracts-deep-r1.md` line 227 |
| **Audit ID** | `BC-AUDIT-104` |
...
> `hooks-registry.toml::capture-commit-activity` (capture-commit-activity entry: priority 110, on_error continue, exec_subprocess capability with `binary_allow = ["bash", "git", "gh", "jq"]`); broader stanza-count + on_error distribution from independent `grep | sort | uniq -c` (18 block / 27 continue).
