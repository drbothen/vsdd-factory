...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:985"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.088: verify-git-push: blocks --force / -f, allows --force-with-lease

## Description

verify-git-push: blocks --force / -f, allows --force-with-lease. If `--force-with-lease` present, allowed. Else if `--force`, ` -f `, ` -f<NL>` or trailing ` -f` matches → block code `git_push_force` with hint to use `--force-with-lease`. Exit 2.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1087` (extracted from `pass-3-deep-hooks.md` line 985).
**Hook script:** ``plugins/vsdd-factory/hooks/verify-git-push.sh``.

## Preconditions

1. Trigger: `git push` command.

## Postconditions

1. Behavior: If `--force-with-lease` present, allowed. Else if `--force`, ` -f `, ` -f<NL>` or trailing ` -f` matches → block code `git_push_force` with hint to use `--force-with-lease`. Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `git push --force` blocked; `git push --force-with-lease` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 985 |
| **Audit ID** | `BC-AUDIT-1087` |
| **Source Line(s) (within hook)** | 56-66. |
