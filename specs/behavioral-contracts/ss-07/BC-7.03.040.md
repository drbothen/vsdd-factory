...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:463"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.040: factory-branch-guard: blocks if worktree on wrong branch

## Description

factory-branch-guard: blocks if worktree on wrong branch. Emits hook.block (reason `factory_wrong_branch`) with current_branch and expected_branch fields; recovery `git checkout $EXPECTED_BRANCH` printed. Exit 2.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1039` (extracted from `pass-3-deep-hooks.md` line 463).
**Hook script:** ``plugins/vsdd-factory/hooks/factory-branch-guard.sh``.

## Preconditions

1. Trigger: Worktree exists; `git -C $FACTORY_DIR rev-parse --abbrev-ref HEAD` returns a branch other than expected (`factory-artifacts` for `.factory`, `factory-project-artifacts` for `.factory-project`) and is not `unknown`.

## Postconditions

1. Behavior: Emits hook.block (reason `factory_wrong_branch`) with current_branch and expected_branch fields; recovery `git checkout $EXPECTED_BRANCH` printed. Exit 2.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 463 |
| **Audit ID** | `BC-AUDIT-1039` |
| **Source Line(s) (within hook)** | 78-89. |
