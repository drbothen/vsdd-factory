...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1261"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.022: validate-input-hash: identity & registry binding

## Description

validate-input-hash: identity & registry binding. Format-validates input-hash field. on_error=block (script crash blocks the write).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1113` (extracted from `pass-3-deep-hooks.md` line 1261).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-input-hash.sh``.
**Registry entry:** `hooks-registry.toml::validate-input-hash` (PostToolUse, tool=`Edit|Write`, priority=310, timeout_ms=10000, on_error=**block**)..
...
1. Behavior: Format-validates input-hash field. on_error=block (script crash blocks the write).
2. Exit codes: 0 / 2.
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-input-hash.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1261 |
| **Audit ID** | `BC-AUDIT-1113` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:256-274` → `hooks-registry.toml::validate-input-hash`. |
