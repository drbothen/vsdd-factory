...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:506"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.044: handoff-validator: warns on suspiciously short result (<40 chars)

## Description

handoff-validator: warns on suspiciously short result (<40 chars). Emits hook.block event severity=warn reason `subagent_truncated_result` with `result_len`. Stderr warn. Exit 0.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1043` (extracted from `pass-3-deep-hooks.md` line 506).
**Hook script:** ``plugins/vsdd-factory/hooks/handoff-validator.sh``.

## Preconditions

1. Trigger: Trimmed length 1..39 chars.

## Postconditions

1. Behavior: Emits hook.block event severity=warn reason `subagent_truncated_result` with `result_len`. Stderr warn. Exit 0.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 506 |
| **Audit ID** | `BC-AUDIT-1043` |
| **Source Line(s) (within hook)** | 48-53. |
