...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:560"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.049: protect-bc: identity & registry binding

## Description

protect-bc: identity & registry binding. Emits PreToolUse JSON envelope with permissionDecision (allow/deny) — uses Claude Code's structured envelope, not exit codes, to communicate decision (richer reason field).

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1048` (extracted from `pass-3-deep-hooks.md` line 560).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-bc.sh``.
**Registry entry:** `hooks-registry.toml::protect-bc` (PreToolUse, tool=`Edit|Write`, priority=70, timeout_ms=5000, on_error=block)..

## Preconditions

1. Hook event/tool match: PreToolUse Edit|Write..
2. Trigger: Every Edit/Write.

## Postconditions

1. Behavior: Emits PreToolUse JSON envelope with permissionDecision (allow/deny) — uses Claude Code's structured envelope, not exit codes, to communicate decision (richer reason field).
2. Exit codes: Always 0 — but stdout JSON encodes the deny.
3. Error policy: block (registry).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Hook emits `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow"}}` for non-BC files. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/protect-bc.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 560 |
| **Audit ID** | `BC-AUDIT-1048` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:598-616` → `hooks-registry.toml::protect-bc`. |
