...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:539"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.047: pr-manager-completion-guard: BLOCKED status is legitimate early exit

## Description

pr-manager-completion-guard: BLOCKED status is legitimate early exit. Exit 0 — BLOCKED reports are an authorized way to leave the lifecycle.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1046` (extracted from `pass-3-deep-hooks.md` line 539).
**Hook script:** ``plugins/vsdd-factory/hooks/pr-manager-completion-guard.sh``.

## Preconditions

1. Trigger: Result matches `^(Status:|##?\s*)?\s*BLOCKED`.

## Postconditions

1. Behavior: Exit 0 — BLOCKED reports are an authorized way to leave the lifecycle.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 539 |
| **Audit ID** | `BC-AUDIT-1046` |
| **Source Line(s) (within hook)** | 50-53. |
