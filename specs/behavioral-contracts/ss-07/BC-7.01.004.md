...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:439"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.01.004: protect-secrets (PreToolUse:Bash + PreToolUse:Read) blocks reads of dotenv / credentials

## Description

protect-secrets (PreToolUse:Bash + PreToolUse:Read) blocks reads of dotenv / credentials. Exit 2 with explanation.

**Source category:** Bash hook script contracts (broad-sweep).
**Audit ID:** `BC-AUDIT-066` (extracted from `pass-3-behavioral-contracts.md` line 439).

## Preconditions

1. PreToolUse on Bash or Read where target path matches secret patterns.

## Postconditions

1. Exit 2 with explanation.
...
| **Source Document** | `pass-3-behavioral-contracts.md` line 439 |
| **Audit ID** | `BC-AUDIT-066` |
...
> `hooks/protect-secrets.sh` (script not deeply read in this pass; behavior inferred from registration).
