...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:840"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.075: regression-gate: warns on pass→fail transition

## Description

regression-gate: warns on pass→fail transition. Emits hook.block severity=warn (`regression_gate_pass_to_fail`). Prints stderr warn with command and state file. Does NOT block (exit 0). Red-gate consumes the state if strict.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1074` (extracted from `pass-3-deep-hooks.md` line 840).
**Hook script:** ``plugins/vsdd-factory/hooks/regression-gate.sh``.

## Preconditions

1. Trigger: Prior status was `pass` and current is `fail`.

## Postconditions

1. Behavior: Emits hook.block severity=warn (`regression_gate_pass_to_fail`). Prints stderr warn with command and state file. Does NOT block (exit 0). Red-gate consumes the state if strict.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 840 |
| **Audit ID** | `BC-AUDIT-1074` |
| **Source Line(s) (within hook)** | 78-84. |
