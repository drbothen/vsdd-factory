...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1733"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.070: validate-vp-consistency: blocks VPs in INDEX missing from verification-architecture.md

## Description

validate-vp-consistency: blocks VPs in INDEX missing from verification-architecture.md. Error "VP X is in VP-INDEX but missing from verification-architecture.md". Block (`policy9_vp_inconsistency`). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1161` (extracted from `pass-3-deep-hooks.md` line 1733).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-vp-consistency.sh``.

## Preconditions

1. Trigger: Per VP-NNN in VP-INDEX, grep VERIF_ARCH for token absent.

## Postconditions

1. Behavior: Error "VP X is in VP-INDEX but missing from verification-architecture.md". Block (`policy9_vp_inconsistency`). Exit 2.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1733 |
| **Audit ID** | `BC-AUDIT-1161` |
| **Source Line(s) (within hook)** | 81-86. |
