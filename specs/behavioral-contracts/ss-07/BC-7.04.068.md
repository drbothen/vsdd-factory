...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1715"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.068: validate-vp-consistency: identity & registry binding

## Description

validate-vp-consistency: identity & registry binding. Policy 9 enforcer (VP-INDEX ↔ verification-architecture ↔ coverage-matrix consistency). Tempfile cleanup on EXIT trap preserves rc.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1159` (extracted from `pass-3-deep-hooks.md` line 1715).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-vp-consistency.sh``.
**Registry entry:** `hooks-registry.toml::validate-vp-consistency` (PostToolUse, tool=`Edit|Write`, priority=410, timeout_ms=10000, on_error=continue)..
...
1. Behavior: Policy 9 enforcer (VP-INDEX ↔ verification-architecture ↔ coverage-matrix consistency). Tempfile cleanup on EXIT trap preserves rc.
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1715 |
| **Audit ID** | `BC-AUDIT-1159` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:456-474` → `hooks-registry.toml::validate-vp-consistency`. |
