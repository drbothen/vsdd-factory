...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:851"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.076: session-learning: identity & registry binding

## Description

session-learning: identity & registry binding. Append-only marker for `/session-review`; never blocks; minimal binary_allow=[bash].

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1075` (extracted from `pass-3-deep-hooks.md` line 851).
**Hook script:** ``plugins/vsdd-factory/hooks/session-learning.sh``.
**Registry entry:** `hooks-registry.toml::session-learning` (Stop, no tool filter, priority=910, timeout_ms=5000, on_error=continue)..

## Preconditions

1. Hook event/tool match: Stop..
2. Trigger: Every session-end (Stop event).

## Postconditions

1. Behavior: Append-only marker for `/session-review`; never blocks; minimal binary_allow=[bash].
...
| **Source Document** | `pass-3-deep-hooks.md` line 851 |
| **Audit ID** | `BC-AUDIT-1075` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:780-797` → `hooks-registry.toml::session-learning`. |
| v1.3 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.06 (S-8.06 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
