...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1828"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.080: validate-wave-gate-prerequisite: identity & registry binding

## Description

validate-wave-gate-prerequisite: identity & registry binding. Wave gate prerequisite + adversary SHA-currency hygiene gate. on_error=block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1171` (extracted from `pass-3-deep-hooks.md` line 1828).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-wave-gate-prerequisite.sh``.
**Registry entry:** `hooks-registry.toml::validate-wave-gate-prerequisite` (PreToolUse, tool=Agent, priority=130, timeout_ms=10000, on_error=block)..

## Preconditions

1. Hook event/tool match: PreToolUse Agent..

## Postconditions

1. Behavior: Wave gate prerequisite + adversary SHA-currency hygiene gate. on_error=block.
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1828 |
| **Audit ID** | `BC-AUDIT-1171` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:738-756` → `hooks-registry.toml::validate-wave-gate-prerequisite`. |
