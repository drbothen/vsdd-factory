...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:812"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.072: regression-gate: matches 9 test runners

## Description

regression-gate: matches 9 test runners. Other commands exit 0 immediately.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1071` (extracted from `pass-3-deep-hooks.md` line 812).
**Hook script:** ``plugins/vsdd-factory/hooks/regression-gate.sh``.

## Preconditions

1. Trigger: Command contains `cargo test`, `cargo nextest`, `pytest`, `pnpm test`, `npm test`, `go test`, `just test`, `just ci`, or `yarn test`.

## Postconditions

1. Behavior: Other commands exit 0 immediately.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 812 |
| **Audit ID** | `BC-AUDIT-1071` |
| **Source Line(s) (within hook)** | 40-43. |
