...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:362"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.030: destructive-command-guard: blocks clobbering redirects to SOT files

## Description

destructive-command-guard: blocks clobbering redirects to SOT files. Block codes: `sot_clobber_redirect`, `sot_truncate_colon`, `sot_truncate_cmd`, `sot_clobber_cpnull`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1029` (extracted from `pass-3-deep-hooks.md` line 362).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: Per SOT file, any of: `>` clobber redirect (not `>>`), `: > FILE` truncate idiom, `truncate -s 0 FILE`, `cp /dev/null FILE`.

## Postconditions

1. Behavior: Block codes: `sot_clobber_redirect`, `sot_truncate_colon`, `sot_truncate_cmd`, `sot_clobber_cpnull`.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `> STATE.md` blocked; `>> STATE.md` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 362 |
| **Audit ID** | `BC-AUDIT-1029` |
| **Source Line(s) (within hook)** | 128-159. |
