...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:52"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.002: block-ai-attribution: substring gate on `git commit`

## Description

block-ai-attribution: substring gate on `git commit`. Only runs the rest of the script when `$COMMAND` contains the literal substring `git commit`. All other commands return exit 0 immediately.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1001` (extracted from `pass-3-deep-hooks.md` line 52).
**Hook script:** ``plugins/vsdd-factory/hooks/block-ai-attribution.sh``.

## Preconditions

1. Hook event/tool match: PreToolUse on Bash..
2. Trigger: stdin JSON parsed; `tool_input.command` extracted via jq.

## Postconditions

1. Behavior: Only runs the rest of the script when `$COMMAND` contains the literal substring `git commit`. All other commands return exit 0 immediately.
2. Exit codes: 0 (skip path).
3. Error policy: N/A (skip path).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Bash command without `git commit` produces exit 0 and no stderr output. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 52 |
| **Audit ID** | `BC-AUDIT-1001` |
| **Source Line(s) (within hook)** | 22-29. |
