...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:788"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.070: red-gate: jq parse error → fail-closed exit 1

## Description

red-gate: jq parse error → fail-closed exit 1. Captures stderr to a tempfile; if non-empty, prints "red-gate: failed to parse" and exits 1.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1069` (extracted from `pass-3-deep-hooks.md` line 788).
**Hook script:** ``plugins/vsdd-factory/hooks/red-gate.sh``.

## Preconditions

1. Trigger: `.red-gate-state.json` exists but jq cannot parse it (broken JSON).

## Postconditions

1. Behavior: Captures stderr to a tempfile; if non-empty, prints "red-gate: failed to parse" and exits 1.
2. Exit codes: 1.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Malformed JSON triggers exit 1, treated as block per registry. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 788 |
| **Audit ID** | `BC-AUDIT-1069` |
| **Source Line(s) (within hook)** | 56-64. |
