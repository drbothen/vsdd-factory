...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:453"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.039: factory-branch-guard: blocks if .factory/ is not a worktree

## Description

factory-branch-guard: blocks if .factory/ is not a worktree. Emits hook.block (reason `factory_not_worktree`), prints recovery `git worktree add $FACTORY_DIR factory-artifacts`. Exit 2.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1038` (extracted from `pass-3-deep-hooks.md` line 453).
**Hook script:** ``plugins/vsdd-factory/hooks/factory-branch-guard.sh``.

## Preconditions

1. Trigger: `.factory/` directory exists but lacks the `.git` marker file.

## Postconditions

1. Behavior: Emits hook.block (reason `factory_not_worktree`), prints recovery `git worktree add $FACTORY_DIR factory-artifacts`. Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Plain mkdir-created `.factory/` triggers the block. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 453 |
| **Audit ID** | `BC-AUDIT-1038` |
| **Source Line(s) (within hook)** | 68-76. |
