...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:701"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.062: purity-check: identity & registry binding

## Description

purity-check: identity & registry binding. Pure-core boundary advisory; warn-only. on_error=continue.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1061` (extracted from `pass-3-deep-hooks.md` line 701).
**Hook script:** ``plugins/vsdd-factory/hooks/purity-check.sh``.
**Registry entry:** `hooks-registry.toml::purity-check` (PostToolUse, tool=`Edit|Write`, priority=220, timeout_ms=5000, on_error=continue)..

## Preconditions

1. Hook event/tool match: PostToolUse Edit|Write..
2. Trigger: Edit/Write to any file.

## Postconditions

1. Behavior: Pure-core boundary advisory; warn-only. on_error=continue.
...
| **Source Document** | `pass-3-deep-hooks.md` line 701 |
| **Audit ID** | `BC-AUDIT-1061` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:77-95` → `hooks-registry.toml::purity-check`. |
