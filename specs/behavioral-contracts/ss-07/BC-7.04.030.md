...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1340"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.030: validate-novelty-assessment: blocks when section is missing

## Description

validate-novelty-assessment: blocks when section is missing. Adds error "Missing '## Novelty Assessment' section"; emits hook.block (`novelty_assessment_incomplete`). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1121` (extracted from `pass-3-deep-hooks.md` line 1340).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-novelty-assessment.sh``.

## Preconditions

1. Trigger: File does NOT contain `## Novelty Assessment`.

## Postconditions

1. Behavior: Adds error "Missing '## Novelty Assessment' section"; emits hook.block (`novelty_assessment_incomplete`). Exit 2.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1340 |
| **Audit ID** | `BC-AUDIT-1121` |
| **Source Line(s) (within hook)** | 53-55. |
