...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:427"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.01.002: capture-commit-activity (PostToolUse:Bash) emits `commit.made` on successful commits

## Description

capture-commit-activity (PostToolUse:Bash) emits `commit.made` on successful commits. Parses commit preamble `[<branch> <sha>] <message>` from any stdout line where bracket's last token is 7-40 hex chars. Emits `commit.made` with commit_sha, branch, message.

**Source category:** Bash hook script contracts (broad-sweep).
**Audit ID:** `BC-AUDIT-064` (extracted from `pass-3-behavioral-contracts.md` line 427).

## Preconditions

1. PostToolUse on Bash, command contained "git commit", `tool_response.interrupted = false` (or `exit_code == 0` for back-compat).

## Postconditions

1. Parses commit preamble `[<branch> <sha>] <message>` from any stdout line where bracket's last token is 7-40 hex chars. Emits `commit.made` with commit_sha, branch, message.
...
| **Source Document** | `pass-3-behavioral-contracts.md` line 427 |
| **Audit ID** | `BC-AUDIT-064` |
...
> `hooks/capture-commit-activity.sh` (per CHANGELOG v1.0.0-beta.3 "first-line-only stdout parser" fix).
