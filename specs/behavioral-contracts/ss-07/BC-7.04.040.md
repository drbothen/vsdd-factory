...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1439"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.040: validate-pr-review-posted: identity & registry binding

## Description

validate-pr-review-posted: identity & registry binding. Verifies pr-review.md was written AND `gh pr review` (not `gh pr comment`) was used.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1131` (extracted from `pass-3-deep-hooks.md` line 1439).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-pr-review-posted.sh``.
**Registry entry:** `hooks-registry.toml::validate-pr-review-posted` (SubagentStop, no tool filter, priority=950, timeout_ms=5000, on_error=continue)..

## Preconditions

1. Hook event/tool match: SubagentStop on pr-reviewer (filtered in body)..

## Postconditions

1. Behavior: Verifies pr-review.md was written AND `gh pr review` (not `gh pr comment`) was used.
2. Exit codes: 0 / 2.
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-pr-review-posted.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1439 |
| **Audit ID** | `BC-AUDIT-1131` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:896-913` → `hooks-registry.toml::validate-pr-review-posted`. |
| v1.3 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.05 (S-8.05 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
