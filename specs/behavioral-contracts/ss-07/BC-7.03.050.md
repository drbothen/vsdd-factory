...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:573"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.050: protect-bc: denies edits to green BCs at .factory/specs/behavioral-contracts/BC-*.md

## Description

protect-bc: denies edits to green BCs at .factory/specs/behavioral-contracts/BC-*.md. Emits permissionDecision=deny with reason citing spec-format.md immutability rule. Notes that to change a green BC, create a new BC with `Supersedes:` field.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1049` (extracted from `pass-3-deep-hooks.md` line 573).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-bc.sh``.

## Preconditions

1. Trigger: file_path matches `.factory/specs/behavioral-contracts/BC-.*\.md$` AND file exists AND contains `^Status: green`.

## Postconditions

1. Behavior: Emits permissionDecision=deny with reason citing spec-format.md immutability rule. Notes that to change a green BC, create a new BC with `Supersedes:` field.
2. Exit codes: 0 (decision in JSON).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Editing `BC-2.01.001.md` with `Status: green` produces deny envelope. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 573 |
| **Audit ID** | `BC-AUDIT-1049` |
| **Source Line(s) (within hook)** | 49-59. |
