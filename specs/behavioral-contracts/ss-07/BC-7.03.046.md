...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:529"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.046: pr-manager-completion-guard: counts STEP_COMPLETE emissions; passes if ≥8

## Description

pr-manager-completion-guard: counts STEP_COMPLETE emissions; passes if ≥8. `grep -c "STEP_COMPLETE:"` counts emissions. ≥8 passes (allows step 2 N/A skip on chore PRs). Otherwise proceeds to block.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1045` (extracted from `pass-3-deep-hooks.md` line 529).
**Hook script:** ``plugins/vsdd-factory/hooks/pr-manager-completion-guard.sh``.

## Preconditions

1. Trigger: Subagent is pr-manager.

## Postconditions

1. Behavior: `grep -c "STEP_COMPLETE:"` counts emissions. ≥8 passes (allows step 2 N/A skip on chore PRs). Otherwise proceeds to block.
2. Exit codes: 0 (pass).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Result with 9 `STEP_COMPLETE: step=N` lines exits 0; result with 7 proceeds to block path. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 529 |
| **Audit ID** | `BC-AUDIT-1045` |
| **Source Line(s) (within hook)** | 41-48. |
