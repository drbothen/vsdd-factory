...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1042"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.001: validate-anchor-capabilities-union: identity & registry binding

## Description

validate-anchor-capabilities-union: identity & registry binding. Story `anchor_capabilities` invariant validator. Bash 3.2 compatible (no associative arrays).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1092` (extracted from `pass-3-deep-hooks.md` line 1042).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-anchor-capabilities-union.sh``.
**Registry entry:** `hooks-registry.toml::validate-anchor-capabilities-union` (PostToolUse, tool=`Edit|Write`, priority=240, timeout_ms=10000, on_error=continue)..
...
1. Behavior: Story `anchor_capabilities` invariant validator. Bash 3.2 compatible (no associative arrays).
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1042 |
| **Audit ID** | `BC-AUDIT-1092` |
| **Source Line(s) (within hook)** | Header 1-15. |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:116-134` → `hooks-registry.toml::validate-anchor-capabilities-union`. |
