...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1859"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.083: validate-wave-gate-prerequisite: skips non-worker, non-adversary subagents

## Description

validate-wave-gate-prerequisite: skips non-worker, non-adversary subagents. Exit 0 (e.g., state-manager, pr-reviewer, github-ops dispatches pass through unchecked).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1174` (extracted from `pass-3-deep-hooks.md` line 1859).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-wave-gate-prerequisite.sh``.

## Preconditions

1. Trigger: subagent_type does not match any of the 6 worker patterns and does not match adversary pattern.

## Postconditions

1. Behavior: Exit 0 (e.g., state-manager, pr-reviewer, github-ops dispatches pass through unchecked).
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1859 |
| **Audit ID** | `BC-AUDIT-1174` |
| **Source Line(s) (within hook)** | 102. |
