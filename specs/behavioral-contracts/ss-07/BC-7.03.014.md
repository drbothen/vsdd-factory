...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:187"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.014: capture-pr-activity: identity & registry binding

## Description

capture-pr-activity: identity & registry binding. Telemetry-only (set +e, on_error=continue).

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1013` (extracted from `pass-3-deep-hooks.md` line 187).
**Hook script:** ``plugins/vsdd-factory/hooks/capture-pr-activity.sh``.
**Registry entry:** `hooks-registry.toml::capture-pr-activity` (PostToolUse, no tool filter, priority=120, timeout_ms=5000, on_error=continue)..

## Preconditions

1. Hook event/tool match: PostToolUse on Bash (filtered in script body)..
2. Trigger: Any PostToolUse; early-exits if tool != Bash.

## Postconditions

1. Behavior: Telemetry-only (set +e, on_error=continue).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Hook never blocks on any path. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 187 |
| **Audit ID** | `BC-AUDIT-1013` |
| **Source Line(s) (within hook)** | Header; tool filter at 33-36. |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:38-55` → `hooks-registry.toml::capture-pr-activity`. |
