...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:517"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.045: pr-manager-completion-guard: identity & registry binding

## Description

pr-manager-completion-guard: identity & registry binding. Detects FM4 (incomplete 9-step PR lifecycle); blocks via stderr injection; on_error=block.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1044` (extracted from `pass-3-deep-hooks.md` line 517).
**Hook script:** ``plugins/vsdd-factory/hooks/pr-manager-completion-guard.sh``.
**Registry entry:** `hooks-registry.toml::pr-manager-completion-guard` (SubagentStop, no tool filter, priority=920, timeout_ms=5000, on_error=block)..

## Preconditions

1. Hook event/tool match: SubagentStop on pr-manager subagents..
2. Trigger: Every SubagentStop; early-exits unless agent name matches `*pr-manager*` or `*pr_manager*`.

## Postconditions

1. Behavior: Detects FM4 (incomplete 9-step PR lifecycle); blocks via stderr injection; on_error=block.
2. Exit codes: 0 / 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 517 |
| **Audit ID** | `BC-AUDIT-1044` |
| **Source Line(s) (within hook)** | Header 1-17. |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:839-856` → `hooks-registry.toml::pr-manager-completion-guard`. |
| v1.3 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.02 (S-8.02 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
