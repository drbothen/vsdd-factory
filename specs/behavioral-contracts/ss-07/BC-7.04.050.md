...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1539"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.050: validate-state-size: identity & registry binding

## Description

validate-state-size: identity & registry binding

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1141` (extracted from `pass-3-deep-hooks.md` line 1539).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-state-size.sh``.
**Registry entry:** `hooks-registry.toml::validate-state-size` (PostToolUse, tool=`Edit|Write`, priority=360, timeout_ms=5000, on_error=continue). binary_allow includes git for HEAD comparison..
...
| **Source Document** | `pass-3-deep-hooks.md` line 1539 |
| **Audit ID** | `BC-AUDIT-1141` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:356-374` → `hooks-registry.toml::validate-state-size`. |
