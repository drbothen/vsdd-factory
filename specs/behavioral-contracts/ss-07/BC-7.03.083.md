...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:931"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.083: update-wave-state-on-merge: identity & registry binding

## Description

update-wave-state-on-merge: identity & registry binding. Advisory state-update; on_error=continue. Requires jq AND python3.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1082` (extracted from `pass-3-deep-hooks.md` line 931).
**Hook script:** ``plugins/vsdd-factory/hooks/update-wave-state-on-merge.sh``.
**Registry entry:** `hooks-registry.toml::update-wave-state-on-merge` (SubagentStop, no tool filter, priority=940, timeout_ms=10000, on_error=continue)..

## Preconditions

1. Hook event/tool match: SubagentStop on pr-manager (filtered in body)..
2. Trigger: Every SubagentStop.

## Postconditions

1. Behavior: Advisory state-update; on_error=continue. Requires jq AND python3.
...
| **Source Document** | `pass-3-deep-hooks.md` line 931 |
| **Audit ID** | `BC-AUDIT-1082` |
...
| v1.2 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 5 — migrated `hooks-registry.toml:877-894` → `hooks-registry.toml::update-wave-state-on-merge`. |
| v1.3 | 2026-05-09 | state-manager | F-P47-001 fix-burst-43: Traceability Stories TBD→S-8.04 (S-8.04 behavioral_contracts frontmatter cites this BC; bidirectional L-P28-001 propagation). |
