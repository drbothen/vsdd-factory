...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2227
section: "Rules: git-commits (Conventional Commits, no AI attribution)"
type: rule
---

# Behavioral Contract BC-8.22.009: rules/git-commits.md: NEVER use `gh pr merge --admin` without explicit per-merge user permission

## Description

NEVER use `--admin` flag (or any branch-protection bypass) on `gh pr merge` unless user explicitly grants permission for THAT specific merge in THAT moment. Prior permission does NOT carry forward — each use requires fresh explicit approval. Always ask before using. Applies to all 1898andCo repos.

## Preconditions

1. The rule from `plugins/vsdd-factory/rules/git-commits.md` is being applied to a code change, commit, or artifact.
2. The artifact under review falls within the scope of the rule ("Used by" field below).

## Postconditions

1. No automated `--admin` flag use.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/rules/git-commits.md` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/rules/git-commits.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 66–70
