...
origin: brownfield
extracted_from: "plugins/vsdd-factory/templates/wave-state-template.yaml"
subsystem: SS-08
capability: ""
...
removed: null
removal_reason: null
audit_id: BC-AUDIT-1908
section: "State and workflow templates"
type: template
---

# Behavioral Contract BC-8.14.009: wave-state-template: wave-state.yaml lifecycle tracker schema

## Description

Wave-state lifecycle YAML. Top-level: `current_wave`, `next_gate_required`, `waves` (map of wave-id → {`stories[]`, `stories_merged[]`, `gate_status` enum {not_started | pending | passed | deferred | failed}, `gate_date`, `gate_report`, `rationale` (required for deferred)}). Consumed by `validate-wave-gate-prerequisite.sh`, `update-wave-state-on-merge.sh`, `warn-pending-wave-gate.sh`. Initialize via copy to `.factory/wave-state.yaml`.

## Preconditions

1. Template file at `plugins/vsdd-factory/templates/wave-state-template.yaml` is being authored or validated.
2. Authoring agent or validation tool is reading the template definition.

## Postconditions

1. Top-level keys present; gate_status uses enum values; deferred status carries rationale.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/templates/wave-state-template.yaml` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/templates/wave-state-template.yaml` | **Type:** template | **Confidence:** HIGH | **Source line(s):** 1–30
