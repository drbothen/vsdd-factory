...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2206
section: "Rules: bash (no stderr suppression, no eval, dependency checks)"
type: rule
---

# Behavioral Contract BC-8.20.006: rules/bash.md: literal-string matching SHALL use `grep -F`

## Description

`grep -q` without `-F` treats the pattern as regex. The `.` in `Cargo.lock` matches any character. `Cargo.lock` matches `CargoXlock`. Use `-F` (fixed-string) when matching literals.
...
1. Literal-string greps use `-qF` or `-F`.
...
**Source metadata:** `plugins/vsdd-factory/rules/bash.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 94–104
