...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2232
section: "Rules: rust (forbid unsafe, edition, lints)"
type: rule
---

# Behavioral Contract BC-8.23.005: rules/rust.md: error handling — thiserror enums, per-crate `pub type Result<T>`, sanitize `Display` before client send

## Description

Use `thiserror` for error enums (structured semantic variants, not string bags). Define `pub type Result<T> = std::result::Result<T, CrateError>` per crate. `Display` impl is for internal logging only — sanitize before sending to clients.
...
1. Each crate has thiserror error enum + `Result<T>` alias.
...
**Source metadata:** `plugins/vsdd-factory/rules/rust.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 22–26
