...
A's fields before the overwrite and — **emission-point corrected v1.7, ADR-048 §D4 v1.5, F-P9-001:
ONLY AFTER `write_indeterminate_marker` returns `Ok(())`, not unconditionally at the pre-overwrite
read** — emits `marker.cleared(clear_mode=SUPERSEDED, actor_type=system)`
for A — preventing a later raw-delete of B's marker from causing `reconcile_raw_delete` to
mis-attribute A's silent supersession to a human `OPERATOR_OVERRIDE` that never happened. On
`Err(_)`, NEITHER `marker.cleared(SUPERSEDED)` nor `marker.written` is emitted (see PC5 and EC-018
below). All
...
5. **SUPERSEDED clear path (cross-pair marker overwrite) with `marker.cleared(SUPERSEDED)` audit
   ...
   `artifact_path`) BEFORE the temp+rename overwrite — this read is unavoidable, since the write
   itself overwrites the file the fields are read from. If the existing marker's
   `(plugin_name, artifact_path)` DIFFERS from the new event's, the caller emits `marker.cleared`
   (BC-3.08.001 Event 9) via `ctx.emit_internal` — **emission-point corrected v1.7, ADR-048 §D4
   v1.5, F-P9-001: ONLY AFTER `write_indeterminate_marker` returns `Ok(())`**, in the SAME
   write-success arm as PC1's sibling BC-1.18.001 §PC4 v1.4 `marker.written` emission (SUPERSEDED
   fires first, then `marker.written`) — with:
   ...
   `reason`-mandatory `clear_mode`), `timestamp` = time of the overwrite. **Write-failure
   non-fabrication (v1.7, ADR-048 §D4 v1.5, F-P9-001):** when `write_indeterminate_marker` returns
   `Err(_)` (EC-007 — atomic rename failure), NEITHER `marker.cleared(SUPERSEDED)` for A NOR
   `marker.written` for B is emitted — the OLD marker for A remains durably present and still
   enforcing its quarantine; emitting SUPERSEDED here would falsely claim it was overwritten, the
   exact fabricated-audit-record class (NIST AU-3/AU-10) this correction closes. Pre-v1.7, this
   postcondition described the emission as occurring unconditionally at the pre-overwrite READ,
   BEFORE the write was even attempted — a write failure could then leave a fabricated SUPERSEDED
   record on disk-truth-inconsistent grounds; see EC-018 for the direct regression test. **Same-pair exception:**
...
| EC-015 | PC5 path (v1.5) — marker for pair A `(plugin_a, artifact_a)` is overwritten by a NEW INDETERMINATE event for a DIFFERENT pair B `(plugin_b, artifact_b)` before A was ever cleared, AND `write_indeterminate_marker(&pair_b, ...)` returns `Ok(())` | `write_indeterminate_marker`'s caller emits `marker.cleared` for A with `clear_mode="SUPERSEDED"`, `actor_type="system"`, `trace_id`/`plugin_name`/`artifact_path` = A's OWN fields (not B's), `reason` non-null, via `ctx.emit_internal`, ONLY AFTER B's `write_indeterminate_marker` call returns `Ok(())` (**corrected v1.7, ADR-048 §D4 v1.5, F-P9-001** — previously described as before B's write completes), in the same write-success arm as B's `marker.written` (SUPERSEDED first, then `marker.written`). Marker is then overwritten with B's fields (last-writer-wins, BC-1.18.001 INV3). |
...
| EC-018 | PC5 negative control (v1.7, ADR-048 §D4 v1.5 — S-25.01 adversary pass 9 F-P9-001 MEDIUM) — marker for pair A `(plugin_a, artifact_a)` exists; a DIFFERENT pair B `(plugin_b, artifact_b)` goes INDETERMINATE fail-closed and its `write_indeterminate_marker(&pair_b, ...)` call returns `Err(_)` (e.g., disk full, permissions — EC-007) | NO `marker.cleared(SUPERSEDED)` is emitted for A, and NO `marker.written` is emitted for B — the OLD marker for A is still durably present on disk and still enforcing its quarantine; emitting SUPERSEDED here would be a fabricated audit record (NIST AU-3/AU-10) falsely claiming A's marker was overwritten. This is the direct regression test for F-P9-001: the pre-v1.7 formulation emitted SUPERSEDED unconditionally at the pre-overwrite field read, before the write was even attempted, so a subsequent write failure was structurally unrepresentable as a non-fabrication case — the fixture ordering itself (emit-then-write) masked the bug. Contrast EC-017 (which regression-tests the OPERATOR_OVERRIDE fabrication route and never attempts a write at all) — EC-018 forces the write to be attempted and to fail before any emission decision is made. |
...
| marker.cleared SUPERSEDED emission — cross-pair overwrite | Marker exists for pair A (plugin_name="regression-gate", artifact_path="/abs/A.md"); pair B (plugin_name="convergence-tracker", artifact_path="/abs/B.md") — different — goes INDETERMINATE fail-closed; `write_indeterminate_marker(&pair_b, ...)` returns `Ok(())` | ONLY AFTER B's marker write returns `Ok(())` (**corrected v1.7, ADR-048 §D4 v1.5, F-P9-001** — previously described as before B's write completes): `marker.cleared` emitted for A with `clear_mode="SUPERSEDED"`, `actor_type="system"`, `trace_id`/`plugin_name`/`artifact_path` = A's own fields, `reason` non-null, via `ctx.emit_internal`, in the same write-success arm as B's `marker.written` (EC-015; BC-1.18.001 INV3 corollary) |
...
| marker.cleared SUPERSEDED — write-failure non-fabrication negative control (v1.7, F-P9-001) | Marker exists for pair A (plugin_name="regression-gate", artifact_path="/abs/A.md"); pair B (plugin_name="convergence-tracker", artifact_path="/abs/B.md") — different — goes INDETERMINATE fail-closed; `write_indeterminate_marker(&pair_b, ...)` returns `Err(_)` (EC-007) | NO `marker.cleared(SUPERSEDED)` emitted for A and NO `marker.written` emitted for B — A's marker remains durably present and still enforcing; no fabricated audit record (EC-018; direct F-P9-001 regression test) |
...
- `crates/factory-dispatcher/src/indeterminate_marker.rs` — `delete_marker_if_pass(outcome, policy, dir, current_artifact_path: &str) -> Result<(), io::Error>`; idempotent `fs::remove_file` with `NotFound` swallowed; MUST compare the marker's `artifact_path` field against `current_artifact_path` (exact normalized absolute path equality) before deleting — if marker `artifact_path` is non-empty and does not match `current_artifact_path`, skip deletion (return Ok(())); if marker `artifact_path` is empty, skip the artifact check (vacuously satisfied); `should_write_marker(outcome, policy) -> bool` predicate used by executor to determine whether to write or skip marker. Also contains `emit_marker_cleared(clear_mode: ClearMode, marker: &MarkerContent, trace_id: &str) -> Result<()>` — called from `delete_marker_if_pass` (REVALIDATED), from `check_and_clear_expired_marker` (TTL_EXPIRED), from `reconcile_raw_delete` (OPERATOR_OVERRIDE), and — v1.5 — from `write_indeterminate_marker`'s caller (SUPERSEDED). `ClearMode` is an enum: `Revalidated | TtlExpired | OperatorOverride | Superseded` (v1.5 variant added). **v1.4 addition (ADR-048 §D4 v1.2 — human-ratified):** `check_and_clear_expired_marker(factory_root, now) -> io::Result<Option<MarkerFields>>` — dispatcher-native TTL pre-check: reads the marker via `read_all_marker_fields`; if present and `expires_at ≤ now`, deletes it (idempotent, swallow `NotFound`), emits `marker.cleared(TTL_EXPIRED)`, and returns `Some(fields)`; otherwise returns `None` without deleting. `reconcile_raw_delete(factory_root, ctx: &HostContext) -> io::Result<()>` — dispatcher-native OPERATOR_OVERRIDE reconciliation, invoked from the same pre-check's marker-absent branch: bounded, best-effort scan for an unmatched `plugin.indeterminate`; if found, emits `marker.cleared(OPERATOR_OVERRIDE)` via `emit_marker_cleared`.
  ...
  **v1.5 addition — SUPERSEDED trigger (F-P3-002 LOW):** `write_indeterminate_marker`'s caller (`executor.rs`, the marker-write callsite for INDETERMINATE outcomes) reads the EXISTING marker's fields (if any) BEFORE the temp+rename overwrite; if the existing marker's `(plugin_name, artifact_path)` differs from the new event's, it calls `emit_marker_cleared(ctx, ..., ClearMode::Superseded, "system", Some(reason))` — symmetrical with the read-before-delete `delete_marker_if_pass`'s REVALIDATED path. **v1.7 correction — emission-point (ADR-048 §D4 v1.5, F-P9-001):** the `emit_marker_cleared(..., ClearMode::Superseded, ...)` call is now gated identically to `emit_marker_written` (BC-1.18.001 §PC4 v1.4) — both are invoked only inside `write_indeterminate_marker`'s `Ok(())` arm (SUPERSEDED first, then `marker.written`), never at the pre-overwrite read and never on `Err(_)`. The pre-v1.7 call site invoked `emit_marker_cleared` unconditionally at the read, before the write was even attempted.
- `crates/factory-dispatcher/src/executor.rs` — calls `delete_marker_if_pass` after a PostToolUse PASS on a fail-closed plugin; the clear MUST be gated on (a) plugin_name match AND (b) artifact-path match (exact normalized absolute path equality, with empty-artifact_path fallback to name-only); BOTH callsites in the executor must apply these two conditions — missing the artifact check at either callsite causes premature quarantine discharge; same module as the INDETERMINATE classification in BC-1.18.001. **v1.4 addition:** the tier-execution loop calls `check_and_clear_expired_marker` for every `on_error == OnError::BlockIfMarker` registry entry BEFORE invoking the Arm 1/Arm 2 WASM gate plugin on the normal (non-crash) path. **v1.5 addition:** the marker-write callsite for INDETERMINATE fail-closed outcomes (feeding `write_indeterminate_marker`) reads the pre-overwrite marker fields and triggers the SUPERSEDED emission described above; `HostContext` (`base_ctx`) is already in scope at this callsite and at the `emit_indeterminate`/`emit_marker_cleared`/`check_and_clear_expired_marker`/`reconcile_raw_delete` call sites, so the `ctx: &HostContext` signature widening requires no new plumbing. **v1.7 correction (ADR-048 §D4 v1.5, F-P9-001):** the SUPERSEDED emission call is relocated within this same callsite to inside `write_indeterminate_marker`'s `Ok(())` result arm, immediately preceding the `emit_marker_written` call (BC-1.18.001 §PC4 v1.4) — both write-tied events now share the same success-gated invocation point.
...
| VP-108 | SUPERSEDED clear (PC5, v1.5; emission-point corrected v1.7): `write_indeterminate_marker`'s caller, given an existing marker for a DIFFERENT `(plugin_name, artifact_path)` pair, reads the existing marker's fields before overwrite and emits `marker.cleared(clear_mode=SUPERSEDED, actor_type=system, reason=non-null)` via `ctx.emit_internal` carrying the SUPERSEDED pair's own trace_id/plugin_name/artifact_path (not the new pair's), ONLY AFTER the new marker write returns `Ok(())` (**corrected v1.7, ADR-048 §D4 v1.5, F-P9-001** — previously described as before the write completes) (EC-015); same-pair re-INDETERMINATE emits nothing (EC-016). ADR-048 §D4 v1.3 F-P3-002 | unit-test |
| VP-108 | Negative control (PC8, v1.7, ADR-048 §D4 v1.5 — F-P9-001 direct regression test): given an existing marker for pair A and a cross-pair overwrite for pair B, `write_indeterminate_marker(&pair_b, ...)` FORCED to return `Err(_)` → NEITHER `marker.cleared(clear_mode=SUPERSEDED)` for A NOR `marker.written` for B is emitted (EC-018) | unit-test |
...
| 1.7 | 2026-09-01 | product-owner | ADR-048 §Decision 4 v1.5 Emission-Point Correction (S-25.01 adversary pass 9 F-P9-001 MEDIUM; architect adjudication; HUMAN-RATIFIED 2026-09-01, POLICY 22, D-1142). PC5's SUPERSEDED `marker.cleared` emission is relocated from the unconditional pre-overwrite field-read to inside `write_indeterminate_marker`'s `Ok(())` arm, symmetric with the sibling BC-1.18.001 §PC4 v1.4 `marker.written` "emit only after `Ok(())`" rule — both write-tied events now fire back-to-back in the same write-success arm (SUPERSEDED first, then `marker.written`); on `Err(_)`, NEITHER event is emitted. Root cause: the pre-v1.7 formulation read the OLD marker's fields and emitted SUPERSEDED unconditionally at that read, BEFORE `write_indeterminate_marker` was even called — so if the subsequent write then returned `Err(_)` (EC-007, disk full/permissions, swallowed best-effort), the OLD marker remained durably present and still enforcing its quarantine, yet a SUPERSEDED record had already been emitted falsely claiming it was overwritten — a fabricated audit record (NIST AU-3/AU-10), the exact class ADR-048 v1.3/v1.4 exist to eliminate; this is the un-swept sibling of the v1.6 `marker.written` discipline (TD-VSDD-060 sibling-sweep miss). (1) Description's Quaternary-path paragraph gains the emission-point correction clause. (2) PC5 rewritten: read-before-overwrite stated as unavoidable and unchanged; emission gated on `Ok(())`; new write-failure non-fabrication clause added. (3) EC-015 corrected to AFTER-`Ok(())` ordering. (4) New EC-018 added: PC5 negative control — the direct F-P9-001 regression test proving no fabrication occurs when the cross-pair write fails. (5) Canonical Test Vectors: SUPERSEDED cross-pair-overwrite row corrected; new write-failure negative-control row added. (6) Architecture Anchors: `indeterminate_marker.rs` bullet's v1.5-addition paragraph and `executor.rs` bullet's v1.5-addition paragraph both gain a v1.7 correction clause. (7) VP Anchors + Verification Properties: VP-108 SUPERSEDED row's fixture-ordering note updated; new VP-108 PC8 negative-control row added. (8) Traceability ADR row: ADR-048 §Decision 4 v1.5 citation added. No wire-format/field-shape change to `marker.cleared` itself — `clear_mode`, `actor_type`, and the `reason`-mandatory-for-SUPERSEDED rule are UNCHANGED; this is an emission-point/ordering correction, not a new `clear_mode` value. `marker.written`'s own v1.6 wire format and emission rule are unaffected — its "emit only after `Ok(())`" rule was already correct. Companion siblings (same burst): BC-3.08.001 v1.33 → v1.34 (Event 9 PC5 Trigger bullet, correspondence table, emission path, Event 10 Trigger paragraph, Architecture Anchors, EC-014/EC-016, Canonical Test Vectors, VP-108 PC8 row, Traceability ADR row all corrected to the same ordering) and BC-1.18.001 v1.4 → (bump) (Invariant 3 SUPERSEDED corollary + PC4 write-success-arm description corrected to the same ordering) — all three amendments jointly implement ADR-048 §D4 v1.5 / F-P9-001's fix, ground-truthed against the architect's ratified ADR-048 v1.5 and VP-108 v1.4. |
