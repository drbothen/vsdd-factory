...
| EC-007 | Implementer feeds canonical to allow-check at line 152 but leaves raw `cmd` in `execute_bounded(...)` call at line 173 | TOCTOU window NOT closed. Failing implementation; covered by VP for this BC. The implementer MUST replace `execute_bounded(cmd, args, ...)` → `execute_bounded(canonical_path.as_str(), args, ...)`. |
