...
**Panic semantics (v1 known limitation):** If `Path::canonicalize()` or any std-lib call in the canonicalize/allow-check/spawn chain panics (e.g., due to an unexpected OS-layer failure), the panic propagates uncaught to the wasmtime host-call boundary. Wasmtime converts the Rust panic to a wasmtime `Trap` and propagates it to the plugin caller. No `catch_unwind` wraps the host-call surface in `exec_subprocess.rs` (exec_subprocess.rs:230 — `Command::new` call is unguarded). The `internal.host_function_panic` event class (`INTERNAL_HOST_FUNCTION_PANIC` at internal_log.rs:83) is declared but the emit call is NOT implemented in `exec_subprocess`. Unhandled panics produce no observability event in v1. Track host-call panic-handling decision in **OQ-W16-008** (v1: panic propagates as wasmtime Trap; v2 candidate: `catch_unwind` wrapper emitting `INTERNAL_HOST_FUNCTION_PANIC` before re-raising). See EC-014.
