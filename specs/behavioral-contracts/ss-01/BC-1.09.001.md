...
| Property | Value |
|----------|-------|
| **Path** | `crates/factory-dispatcher/src/plugin_loader.rs::probe` (reads metadata + modified()); `crates/factory-dispatcher/src/plugin_loader.rs::PluginCache::get_or_compile` (entries.lock() check) |
| **Confidence** | HIGH |
...
| v1.1 | 2026-05-08 | implementer | TD-VSDD-091 Chunk 6 — migrated 1 body cite: `plugin_loader.rs:56-84` → `plugin_loader.rs::probe` (reads metadata + modified()); `plugin_loader.rs::PluginCache::get_or_compile` (entries.lock() check). |
