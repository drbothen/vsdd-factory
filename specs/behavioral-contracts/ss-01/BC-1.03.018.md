...
last_amended: "2026-08-22 (v1.4) — Wave-7 adversary pass-3 remediation, cycle
...
9. **PC9 — Mechanical intra-story ordering gate, DUAL-TRIGGER (ADR-039 §Decision 1
    ...
    **Detector precision — executable-code read pattern, not bare substring/grep (mirrored
    from S-21.23 AC-022 hardening, F-S2123-P3-003, LOW-folded-in for POLICY-8 fidelity):** the
    break-glass consultation code detection MUST recognize the `VSDD_BREAK_GLASS_GATE_BYPASS`
    reference via an EXECUTABLE-CODE read pattern — i.e. the token must appear in a position
    the Rust compiler would actually evaluate (a live `std::env::var`/conditional-check
    expression in the block-decision path) — NOT a bare substring or line-oriented `grep`
    match against the source text. A `VSDD_BREAK_GLASS_GATE_BYPASS` token appearing ONLY
    inside a source comment (`//` or `/* */`) — dead text the compiler never evaluates — MUST
    NOT satisfy this detection. A bare-grep detector would false-positive on a commented-out or
    merely-mentioned-in-a-comment reference, reporting the ordering rule as satisfied when the
    break-glass code is NOT actually live in the block-decision path — reopening exactly the
    self-lock hazard this gate exists to close, silently.
    ...
    **Why the dual trigger, not `failure_policy` alone (Q-A/F-S2123-P2-002):** the prior PC9
    ...
    (f) **LIVE-TREE-CONTROL:** the detector MUST be run against the actual
        ...
        exposure.
    (g) **COMMENT-ONLY control (NEW — mirrored from S-21.23 AC-022, F-S2123-P3-003):**
        synthetic registry with EITHER trigger present (e.g. `on_error="block"`, matching the
        live state) + a synthetic executor-source snippet where the
        `VSDD_BREAK_GLASS_GATE_BYPASS` token appears ONLY inside a `//` or `/* */` comment
        (e.g. `// TODO: consult VSDD_BREAK_GLASS_GATE_BYPASS here`) — NO actual executable
        `std::env::var` read or conditional check — → assert the gate STILL FIRES RED. This
        proves the detector is precise to EXECUTABLE code, not merely present-in-source-text:
        a detector implemented as a bare substring/grep match would incorrectly report PASS on
        this fixture (the token IS present in the file), which is the exact false-green defect
        this control exists to catch — the break-glass code is NOT actually live in the
        block-decision path despite the token's textual presence.
...
| Synthetic registry with `on_error="block"` present (matching live state); synthetic executor-source snippet with `VSDD_BREAK_GLASS_GATE_BYPASS` appearing ONLY inside a `//` comment (no executable read/check) | `test_no_agent_gate_fail_closed_flip_without_break_glass` FAILS (RED) — detector correctly recognizes the token is NOT in executable code despite textual presence; a bare-grep detector would false-positive PASS on this fixture | migration-window-gate (PC9 COMMENT-ONLY control, NEW — F-S2123-P3-003, mirrored from S-21.23 AC-022) |
