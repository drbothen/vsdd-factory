...
last_amended: "2026-08-23 (v1.5) — Wave-7 adversary pass-4 remediation, cycle
...
9. **PC9 — Mechanical intra-story ordering gate, DUAL-TRIGGER (ADR-039 §Decision 1
    ...
    **Detector precision — executable-code read pattern, not bare substring/grep; comment-stripping
    ...
    actually evaluate (a live `std::env::var`/conditional-check expression in the block-decision
    path) — NOT a bare substring or line-oriented `grep` match against the source text.
    **Comment-stripping is MANDATORY before applying the executable-code pattern check:** the
    ...
    `std::env::var("VSDD_BREAK_GLASS_GATE_BYPASS")` call appearing only inside a `//` comment is
    ...
    A `VSDD_BREAK_GLASS_GATE_BYPASS` token — or a complete `std::env::var(...)` call referencing it
    ...
    **Why the dual trigger, not `failure_policy` alone (Q-A/F-S2123-P2-002):** the prior PC9
    ...
    (f) **COMMENT-ONLY control (control-letter aligned to story AC-022, F-S2123-P4-001;
        ...
        comment — NO actual executable `std::env::var` read or conditional check — → assert
        ...
        **(f.ii) full-call-in-comment sub-case (NEW — F-S2123-P4-002):** a syntactically
            complete `std::env::var(...)` call referencing the variable appears ONLY inside a
            `//` comment (e.g. `// let _bypass =
            std::env::var("VSDD_BREAK_GLASS_GATE_BYPASS").ok();`) — the detector MUST fire
            RED. This is the most dangerous false-green variant: a bare substring/grep
            detector finds a complete, syntactically valid `env::var(...)` call and reports
            ...
            is the required mitigation — after stripping, the `env::var` call is removed from
