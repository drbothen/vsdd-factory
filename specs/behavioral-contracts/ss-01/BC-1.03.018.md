...
last_amended: "2026-08-24 (v1.6) — Wave-7 adversary pass-6/R5 remediation, cycle
v1.0-brownfield-backfill: (F-S2123-P6-002) LOW — PC9 detector-precision examples updated to
use `std::env::var_os` exclusively (the mandated accessor per Task 3/AC-014/EC-015; added
accessor-agnostic clarification: the detector keys on the `VSDD_BREAK_GLASS_GATE_BYPASS` token
in executable position, not the specific function name); (f.i)/(f.ii) comment-stripping
sub-cases preserved intact; updated: detector-precision paragraph, comment-stripping sentence,
(f.ii) sub-case description and mitigation sentence, Canonical Test Vectors (f.i)/(f.ii) rows.
(F-S2123-P6-003) LOW (POLICY 19 HIGH-floor risk) — load-bearing ADR-039 version pins
(`v1.9`, `v1.10`, `v1.16`) stripped from all active citation sites: H1 title, Preconditions 1
and 4, PC9 header, PC9 dual-trigger rationale paragraph, PC11 header, Invariant 1, Traceability
ADR row (three pins), Security row, Architecture Anchors (two sites); stable `ADR-039 §Decision
3` / `ADR-039 §AMD-004` section-anchor form used throughout active text; version tokens retained
only as `(as of vX.Y, non-load-bearing)` parentheticals where historical context has semantic
value, or dropped where RATIFIED date provides sufficient context; Changelog entries v1.0–v1.5
treated as immutable historical records. POLICY-15 grep attestation:
`grep -nE 'ADR-039[^)]*v1\.[0-9]+'` pre-patch stdout = 9 matches on lines
156/164/167/461/557/558/575/577/602 (all active-text load-bearing matches removed in this
patch). 5-leg parity: version v1.5→v1.6. BC-1.03.018 v1.6. [Prior:
2026-08-23 (v1.5) — Wave-7 adversary pass-4 remediation, cycle
v1.0-brownfield-backfill: (3a) F-S2123-P4-001 (MED) — PC9 control-letter drift
corrected: (f)=COMMENT-ONLY and (g)=LIVE-TREE, aligned to story AC-022's LIVE-TREE=(g)
partition; prior BC had (f)=LIVE-TREE and (g)=COMMENT-ONLY, causing a reader to defer to
the wrong control letter. Swept VP-TBD, Traceability ADR row, Story Anchor, VP Anchors.
(3b) F-S2123-P4-002 (MED) — PC9 detector false-green for commented-out full calls:
mandated comment-stripping (removal of // and /* */ comment spans) BEFORE applying the
executable-code pattern check, so a complete `std::env::var(\"VSDD_BREAK_GLASS_GATE_BYPASS\")`
call inside a comment does not satisfy detection. COMMENT-ONLY control description
strengthened to cover the full-call-in-comment sub-case. New Canonical Test Vector row
added (full-call-in-comment → gate FIRES RED). Closes CWE-636 false-green hazard. (3c)
F-S2123-P4-005 (LOW, PO confirm) — PC3 `all` explicitly stated as a trimmed comma-token
(same rule as PC2), confirming `all` is name-scoped shorthand detected by the identical
token-matching path as any named gate, not a separate parsing mode. H1 enriched per
POLICY 7. No PC renumbering; PC3/PC8/PC9 amendments corrective/additive; PC1-PC2,
PC4-PC7, PC10-PC12 UNCHANGED. Does NOT touch BC-1.03.017 (separate product-owner burst,
same task), S-21.x story bodies/ACs/frontmatter (story-writer domain, dispatched
separately per POLICY 8 propagation), or any INDEX/STATE.md (state-manager domain).
Input-hash flagged stale for state-manager reconciliation. BC-1.03.018 v1.5. [Prior:
2026-08-22 (v1.4) — Wave-7 adversary pass-3 remediation, cycle
v1.0-brownfield-backfill: (2a) F-S2123-P3-001 (HIGH) — the `all` wildcard's scope-restriction
guarantee had no proof it preserves a NON-named blocking plugin's fail-closed enforcement: PC8
extended to state explicitly that `all` (PC3) is name-scoped shorthand for the two named gates
only, NOT a blanket suppress-every-block switch — under `all`, a non-named blocking plugin
(e.g. `validate-factory-path-root`) MUST have its block STAND AND MUST NOT receive a
`break_glass.activated` audit record. Closes a compound CWE-636 (silent fail-open for
non-named plugins) + CWE-863 (Incorrect Authorization — scope silently expands beyond its
declared two-gate boundary) hazard a naive `all` implementation (suppress-every-tier-outcome
rather than name-scoped) would introduce, with no audit trail — materially worse than the
exact-name case PC8 already covered, since `all` is the operationally more likely value an
operator reaches for under a live self-lock. New Canonical Test Vector row
(`all`×non-named-plugin negative control). (2b) F-S2123-P3-003 (LOW-folded-in for POLICY-8
fidelity) — S-21.23's AC-022 hardened PC9 with a 7th COMMENT-ONLY control and an
executable-code-read-pattern detector-precision requirement (not bare-grep) that this BC's PC9
did not carry, creating a story-superset-of-BC drift. Mirrored both into PC9: (i) added an
explicit detector-precision requirement — the break-glass consultation code MUST be recognized
via an executable-code read pattern, not a bare substring/grep match, so a
`VSDD_BREAK_GLASS_GATE_BYPASS` token appearing only inside a source comment does NOT satisfy
detection; (ii) added control (g) COMMENT-ONLY, proving the detector correctly fires RED on a
comment-only fixture despite the token's textual presence. Control count reconciled from six to
SEVEN, matching S-21.23's actual (and correct) test coverage. New Canonical Test Vector row
(comment-only control). Swept sibling sites for TD-VSDD-060 consistency: VP-TBD (Verification
Properties row + VP Anchors, six-control→seven-control + `all`-wildcard mention), Traceability
ADR row (new F-S2123-P3-001/003 citations), Security row, Stories row, Story Anchor section. H1
enriched per POLICY 7 with the `all`-wildcard-scope and detector-precision clauses. No PC
renumbering; PC8 and PC9 amendments are corrective/additive within their existing scope; PC1-PC7,
PC10-PC12 UNCHANGED. Does NOT touch BC-1.03.017 (separate product-owner burst, same task), the
S-21.x story bodies/ACs/frontmatter (story-writer's domain, dispatched separately this burst per
POLICY 8 propagation — S-21.23's AC-022 citation should re-anchor to confirm this BC now matches
its control set), or any INDEX/STATE.md (state-manager's domain). This content change means this
BC's own `input-hash` will go stale as a result and is flagged for state-manager reconciliation
same-burst. BC-1.03.018 v1.4. [Prior: 2026-08-22 (v1.3) — Wave-7 adversary pass-2 remediation, cycle
...
1. ADR-039 §Decision 3 break-glass minimum-viable definition is RATIFIED (POLICY 22, 2026-08-19; as of v1.10, non-load-bearing) — no further design pass is required to derive this BC's postconditions.
...
3. **PC3 — Override containing the trimmed token `all` → both named gates bypassed (F-S2123-P4-005, `all`-as-trimmed-comma-token):** `VSDD_BREAK_GLASS_GATE_BYPASS=all` — where `all` is matched using the same comma-split-and-trim rule as PC2 (the override value is split on commas and each token is trimmed of leading/trailing whitespace before comparison; `all` is therefore a case-sensitive exact token match after trimming, identical to how any named gate is matched, not a separate parsing mode) — suppresses both `validate-wave-gate-prerequisite`'s and `validate-pr-merge-prerequisites`' block contributions for that invocation, equivalent to naming both explicitly. `all` is a reserved name-scoped shorthand for the two named gates; it is not a blanket "suppress any plugin whose name appears" interpretation — PC8 makes the scope restriction explicit and testable.
...
8. **PC8 — Scope restriction: override has no effect on any plugin other than the two named gates, INCLUDING under the `all` wildcard (F-S2123-P3-001, HIGH, Wave-7 pass-3 remediation):** Setting `VSDD_BREAK_GLASS_GATE_BYPASS` to the exact `name` of any registered plugin OTHER than `validate-wave-gate-prerequisite` or `validate-pr-merge-prerequisites` (e.g., `validate-factory-path-root`, or any `on_error="continue"` advisory plugin) has NO effect on that other plugin's block decision — the mechanism is hard-scoped to the two named gates at the implementation level, not a generic "bypass any plugin by name" facility. This closes an over-broad-implementation risk: a naive implementation that treats ANY matching plugin name as bypassable would silently defeat fail-closed enforcement for plugins ADR-039 never intended to be bypassable (e.g., `validate-factory-path-root`, `validate-input-hash`, `validate-template-compliance`).
    **This scope restriction applies IDENTICALLY when the override is set to the literal `all`
    (F-S2123-P3-001):** `all` (PC3) is name-scoped shorthand for "both of the two named gates,"
    NOT a blanket "suppress every blocking plugin" switch. Under
    `VSDD_BREAK_GLASS_GATE_BYPASS=all`, a NON-named blocking plugin (e.g.
    `validate-factory-path-root`, `validate-input-hash`, `validate-template-compliance`) MUST
    have its block STAND — `block_intent=true` from that plugin's contribution, UNAFFECTED by
    the override — AND MUST NOT have a `break_glass.activated` event emitted attributing
    bypass/audit to it; only the two named gates' contributions are suppressed and audited
    under `all`.
    **Why this needed an explicit assertion (F-S2123-P3-001):** prior to this correction, no PC
    or test proved the `all` path preserves a non-named plugin's block — PC8's original framing
    tested only exact-non-matching-NAME inputs (the existing PC4/PC8 negative controls), never
    the `all` literal specifically. A naive `all` branch implemented as "suppress every outcome
    in the `tier_outcomes` loop" (rather than "suppress only entries whose `RegistryEntry.name`
    is one of the two named gates, even when the override value is `all`") would silently
    defeat fail-closed enforcement for `validate-factory-path-root`/`validate-input-hash`/
    `validate-template-compliance` with NO audit trail — a compound CWE-636 (Not Failing
    Securely — silent fail-open for non-named plugins) AND CWE-863 (Incorrect Authorization —
    the override's authorization scope silently expands beyond its declared two-gate boundary)
    hazard, materially worse than the exact-name case PC8 already covered because `all` is the
    operationally MORE LIKELY value an operator reaches for under a live self-lock.
    **Testable assertion:** an integration test MUST construct a dispatch where
    `VSDD_BREAK_GLASS_GATE_BYPASS=all` is set AND a non-named plugin (e.g.
    `validate-factory-path-root`) returns a blocking outcome in the SAME dispatch, and assert
    BOTH (a) `block_intent=true` (the non-named plugin's block stands, unsuppressed) AND (b) no
    `break_glass.activated` record naming that non-named plugin appears in captured
    dispatcher-internal JSONL output — while the two named gates, if also blocking in the same
    dispatch, are correctly suppressed and audited per PC2/PC3/PC5.

9. **PC9 — Mechanical intra-story ordering gate, DUAL-TRIGGER (ADR-039 §Decision 1
    axes-independence + §Decision 3 sequencing rule, made CI-checkable, mirroring
    ...
    **Detector precision — executable-code read pattern, not bare substring/grep; comment-stripping
    REQUIRED (mirrored from S-21.23 AC-022 hardening, F-S2123-P3-003, LOW-folded-in for POLICY-8
    fidelity; strengthened F-S2123-P4-002, Wave-7 pass-4 remediation):** the break-glass
    consultation code detection MUST recognize the `VSDD_BREAK_GLASS_GATE_BYPASS` reference via an
    EXECUTABLE-CODE read pattern — i.e. the token must appear in a position the Rust compiler would
    actually evaluate (a live `std::env::var_os`/conditional-check expression in the
    block-decision path — the detector is accessor-agnostic and keys on the
    `VSDD_BREAK_GLASS_GATE_BYPASS` token in executable position, not the specific function name;
    `std::env::var_os` is the mandated accessor per Task 3/AC-014/EC-015) — NOT a bare substring
    or line-oriented `grep` match against the source text.
    **Comment-stripping is MANDATORY before applying the executable-code pattern check:** the
    detector MUST strip all `//` and `/* */` comment spans from the source text before searching for
    the `VSDD_BREAK_GLASS_GATE_BYPASS` token or pattern. This is required because a COMPLETE
    `std::env::var_os("VSDD_BREAK_GLASS_GATE_BYPASS")` call appearing only inside a `//` comment is
    syntactically indistinguishable from live code to a bare substring or grep, constituting a
    CWE-636 false-green hazard: the detector would report PASS (the full call IS present as text)
    when the break-glass code is NOT actually live in the block-decision path (F-S2123-P4-002).
    A `VSDD_BREAK_GLASS_GATE_BYPASS` token — or a complete `std::env::var_os(...)` call referencing it
    — appearing ONLY inside a source comment (`//` or `/* */`) MUST NOT satisfy this detection after
    comment-stripping is applied. A bare-grep detector that does not strip comments first
    false-positives on both the token-in-comment and full-call-in-comment variants, reopening exactly
    the self-lock hazard this gate exists to close, silently.
    ...
    **Why the dual trigger, not `failure_policy` alone (Q-A/F-S2123-P2-002):** the prior PC9
    ...
    §Decision 3's ordering rule mechanically un-violatable against BOTH self-lock
    ...
    (e) **VACUITY-CONTROL (corrected — Q-A/F-S2123-P2-002):** synthetic registry with BOTH
        ...
        present."
    (f) **COMMENT-ONLY control (control-letter aligned to story AC-022, F-S2123-P4-001;
        mirrored from S-21.23 AC-022, F-S2123-P3-003; strengthened F-S2123-P4-002, Wave-7
        pass-4 remediation):** synthetic registry with EITHER trigger present (e.g.
        `on_error="block"`, matching the live state) + a synthetic executor-source snippet
        where the `VSDD_BREAK_GLASS_GATE_BYPASS` token appears ONLY inside a `//` or `/* */`
        comment — NO actual executable `std::env::var_os` read or conditional check — → assert
        the gate STILL FIRES RED. This control MUST cover BOTH comment-token sub-cases
        (F-S2123-P4-002):
        **(f.i) token-in-comment sub-case:** `VSDD_BREAK_GLASS_GATE_BYPASS` appears as a bare
            token in a `//` comment (e.g. `// TODO: consult VSDD_BREAK_GLASS_GATE_BYPASS
            here`) — the detector MUST fire RED (bare token inside a comment MUST NOT satisfy
            the executable-code check).
        **(f.ii) full-call-in-comment sub-case (NEW — F-S2123-P4-002):** a syntactically
            complete `std::env::var_os(...)` call referencing the variable appears ONLY inside a
            `//` comment (e.g. `// let _bypass =
            std::env::var_os("VSDD_BREAK_GLASS_GATE_BYPASS");`) — the detector MUST fire
            RED. This is the most dangerous false-green variant: a bare substring/grep
            detector finds a complete, syntactically valid `env::var_os(...)` call and reports
            PASS, while the Rust compiler never evaluates it (CWE-636). Comment-stripping
            before the executable-code pattern check (per detector-precision paragraph above)
            is the required mitigation — after stripping, the `env::var_os` call is removed from
            the search space and the check correctly fails.
        This proves the detector is precise to EXECUTABLE code after comment-stripping, not
        merely present-in-source-text: a detector that does not strip comments first
        incorrectly reports PASS on BOTH sub-cases (f.i) and (f.ii), reopening the self-lock
        hazard silently.
    (g) **LIVE-TREE-CONTROL (control-letter aligned to story AC-022, F-S2123-P4-001):** the
        detector MUST be run against the actual
    ...
    The gate's control count is SEVEN (a)–(g), reconciled to match S-21.23's AC-022-hardened
    control set (F-S2123-P3-003) — the prior six-control enumeration understated the story's
    actual (and correct) test coverage, which this correction folds back into the BC as the
    source of truth. Control-letter partition aligned to story AC-022 (F-S2123-P4-001):
    (a)–(e) synthetic, (f) COMMENT-ONLY, (g) LIVE-TREE.
...
11. **PC11 — Audit fail-closed-on-failure: suppression is granted ONLY IF the
    ...
    §AMD-004, RATIFIED 2026-08-22, architect adjudication Q3 via orchestrator relay):**
...
| `VSDD_BREAK_GLASS_GATE_BYPASS=all`; `validate-factory-path-root` (a NON-named plugin) returns a blocking outcome in the same dispatch | `validate-factory-path-root`'s block STANDS — `block_intent=true` from its contribution, unaffected by `all`; NO `break_glass.activated` event emitted naming `validate-factory-path-root` — `all` is scoped to the two named gates only, not a blanket "suppress every block" switch | negative-control (PC8, `all`×non-named-plugin, F-S2123-P3-001) |
...
| Synthetic registry with `on_error="block"` present (matching live state); synthetic executor-source snippet where `VSDD_BREAK_GLASS_GATE_BYPASS` appears as a bare token ONLY inside a `//` comment (e.g. `// TODO: consult VSDD_BREAK_GLASS_GATE_BYPASS here`) — NO executable `std::env::var_os` read or conditional check | `test_no_agent_gate_fail_closed_flip_without_break_glass` FAILS (RED) — detector correctly recognizes the bare token is NOT in executable code despite textual presence; a bare-grep detector would false-positive PASS on this fixture | migration-window-gate (PC9 COMMENT-ONLY control (f.i) token-in-comment sub-case, F-S2123-P3-003, mirrored from S-21.23 AC-022; control-letter aligned F-S2123-P4-001) |
| Synthetic registry with `on_error="block"` present (matching live state); synthetic executor-source snippet where a syntactically COMPLETE `std::env::var_os("VSDD_BREAK_GLASS_GATE_BYPASS")` call appears ONLY inside a `//` comment (e.g. `// let _bypass = std::env::var_os("VSDD_BREAK_GLASS_GATE_BYPASS");`) — the call is syntactically valid but the Rust compiler never evaluates it | `test_no_agent_gate_fail_closed_flip_without_break_glass` FAILS (RED) — detector must strip comments before applying the executable-code pattern check; after stripping, the `env::var_os(...)` call is absent and the check correctly fails. A bare-grep or substring detector would false-positive PASS on this fixture (the full call IS present as text), which is the CWE-636 false-green hazard this sub-case exists to close (F-S2123-P4-002) | migration-window-gate (PC9 COMMENT-ONLY control (f.ii) full-call-in-comment sub-case, NEW — F-S2123-P4-002, Wave-7 pass-4 remediation; comment-stripping mandate per detector-precision paragraph) |
...
| ADR | ADR-039 §Decision 3 (break-glass requirement first introduced for the two PreToolUse `^Agent$` gates; non-load-bearing: as of v1.9); ADR-039 §Decision 3 (RATIFIED, POLICY 22, 2026-08-19 — concrete minimum-viable definition: environment-variable override, human-operator-only, audited via JSONL; intra-story ordering constraint: break-glass commit precedes or is atomic with the fail-closed-flip commit for these two gates within S-21.11) — this BC IS that minimum-viable definition's behavioral contract. **ADR-039 §AMD-004 (RATIFIED 2026-08-22 — architect adjudication Q3, via orchestrator relay, POLICY 22 ratification-channel; S-21.23 F-001, HIGH, pre-TDD adversarial pass-1, Wave-7 split of CONVERGED S-21.11): break-glass suppression MUST be fail-closed-on-audit-failure — an audit write that fails or is skipped (mount-gate) MUST NOT permit the override; the mandated audit sink (`InternalLog::write`) is, by contract, best-effort and swallows I/O errors while also returning `Ok(())` on a mount-gate skip, so a caller cannot distinguish "durably written" from "silently dropped"/"silently skipped" from that `Result` alone; this amendment mandates the additive `InternalLog::write_checked` tri-state method (added ALONGSIDE, not replacing, the existing best-effort `write()`) and requires the break-glass call site alone to gate suppression on its result — **PC11 (v1.2) is this BC's behavioral-test leg for §AMD-004's ruling; the `InternalLog::write_checked` method itself is S-21.23 Phase-3 TDD implementer scope**.** **ADR-039 §Decision 1 (axes-independence — `on_error` governs crash/host-error outcomes independently of `failure_policy`; Q-A/F-S2123-P2-002, Wave-7 pass-2, RATIFIED 2026-08-22): both named Agent gates ALREADY carry `on_error="block"` in the LIVE registry today, independent of any `failure_policy` annotation, and therefore ALREADY self-lock on crash/timeout under §Decision 1's axes-independence — **PC9 (v1.3) is this BC's DUAL-TRIGGER correction: the intra-story ordering gate now keys on EITHER `failure_policy="fail-closed"` OR `on_error="block"`, independently sufficient, closing the gap where a `failure_policy`-only trigger left the already-live `on_error="block"` self-lock exposure outside the gate's coverage**.** **F-S2123-P2-006 (Q-B, Wave-7 pass-2, audit-loss observability): PC11's suppression-denial is a no-op on a counterfactual clean-pass matched invocation, silently dropping the PC6-mandated audit record when the write additionally fails/is skipped — **PC12 (v1.3) is this BC's behavioral-test leg closing that residual silence with an unconditional stderr diagnostic**.** **F-S2123-P3-001 (HIGH, Wave-7 pass-3): the `all` wildcard's scope-restriction guarantee (PC8) had no proof it preserves a NON-named blocking plugin's fail-closed enforcement and audit-silence — a naive `all` branch applied to every tier outcome (not scoped by `RegistryEntry.name`) would silently defeat `validate-factory-path-root`/`validate-input-hash`/`validate-template-compliance` with no audit trail (CWE-636+CWE-863) — **PC8 (v1.4) extended with the `all`×non-named-plugin assertion is this BC's closure**.** **F-S2123-P3-003 (LOW-folded-in, Wave-7 pass-3, POLICY-8 fidelity): S-21.23's AC-022 hardened PC9's detector with a 7th COMMENT-ONLY control and an executable-code-read-pattern requirement (not bare-grep) that this BC's PC9 did not carry, creating a story-superset-of-BC drift — **PC9 (v1.4) mirrors both, reconciling the control count to SEVEN**.** Sibling: BC-1.03.017 (governs the `failure_policy`/`on_error` exhaustion-decision axes this BC's override suppresses the OUTCOME of, without altering the decision function itself) |
| Security | CWE-636 lineage (Not Failing Securely) closure-at-the-override-layer: PC5/PC6's mandatory-audit requirement prevents the override itself from becoming a new silent-approval vector; PC8's `all`-wildcard scope-restriction extension (F-S2123-P3-001) closes a compound CWE-636+CWE-863 (Incorrect Authorization) hazard where a naive `all` implementation could silently defeat fail-closed enforcement for non-named plugins with no audit trail; PC9's DUAL-TRIGGER correction (Q-A/F-S2123-P2-002) closes a self-lock exposure gap that was ALREADY LIVE (both Agent gates already carry `on_error="block"`, independent of any `failure_policy` flip); PC9's detector-precision requirement (F-S2123-P3-003) closes a false-green gap where a bare-grep detector could be defeated by a comment-only reference; PC11 (§AMD-004) closes the residual gap where the audit EMISSION attempt itself silently fails or is skipped, one layer down; PC12 (Q-B/F-S2123-P2-006) closes the remaining observability gap where PC11's suppression-denial is a silent no-op on a counterfactual clean-pass invocation. Kubernetes/GKE/OPA Gatekeeper admission-webhook self-deadlock precedent (named hazard class; narrow authenticated exemption over blanket fail-open) per ADR-039 §Decision 3 |
| Stories | S-21.11 (original unified delivery vehicle) → PC1–PC7 delivered within S-21.11's absorbed scope (break-glass, AMD-002 wiring fix, per-plugin `timeout_ms` calibration, gated fail-closed flip; the prior follow-up name S-21.17 is retired); PC10 delivered as part of S-21.11 v2.0 adversarial pass-1 remediation (F-006); **PC8's `all`-wildcard extension (F-S2123-P3-001), PC9's DUAL-TRIGGER correction and detector-precision hardening (Q-A/F-S2123-P2-002, F-S2123-P3-003), PC11 (Q3/F-S2123-P1-001, ADR-039 §AMD-004), and PC12 (Q-B/F-S2123-P2-006) are S-21.23's Phase-3 TDD implementer scope — S-21.23 is the Wave-7 split-topology sub-story of CONVERGED S-21.11 governing this BC's break-glass scope/ordering/audit/durability/observability leg** |
...
- ADR-039 §Decision 3 — source of PC1-PC9's minimum-viable definition; F-S2111V2-P1-001-mechanism-adjudication memo (`.factory/cycles/v1.0-brownfield-backfill/F-S2111V2-P1-001-mechanism-adjudication.md`) — source of PC10's F-006 threat-boundary directive and EC-005's F-004 correction
- `crates/factory-dispatcher/src/internal_log.rs::InternalLog::write_checked` — PC11's wiring
  target (ADR-039 §AMD-004, RATIFIED 2026-08-22): a new, additive method returning a tri-state
...
S-21.11 — validator exhaustion fail-closed calibration and enforcement (expanded, unified scope: AMD-002 wiring fix + per-plugin `timeout_ms` calibration + break-glass mechanism + gated fail-closed flip); **S-21.23 — Wave-7 split-topology sub-story of CONVERGED S-21.11, owning PC8's `all`-wildcard non-named-plugin scope test (F-S2123-P3-001), PC9's DUAL-TRIGGER ordering-gate correction (Q-A, ADR-039 §Decision 1) and detector-precision/comment-only control (F-S2123-P3-003), PC11's `InternalLog::write_checked` implementation and the break-glass call-site audit-durability gating (Q3, ADR-039 §AMD-004), and PC12's audit-loss stderr diagnostic (Q-B) — all Phase-3 TDD scope**

## VP Anchors

- VP-TBD — break-glass override dispatch: all twelve postconditions exercised by behavioral tests driving the actual `execute_tiers` path with real env-var state; mandatory audit-event assertion against captured JSONL output; `all`-wildcard non-named-plugin scope-preservation test (PC8, F-S2123-P3-001); DUAL-TRIGGER SEVEN-control intra-story ordering gate with detector-precision (PC9, corrected Q-A/F-S2123-P2-002 — `failure_policy` OR `on_error` independently sufficient — + F-S2123-P3-003 executable-code-vs-comment-only detector precision, mirrored from S-21.23 AC-022); child-process parent-environment-isolation threat-boundary test (PC10); audit-write-durability-gates-suppression test against simulated `Failed`/`SkippedMountGate`/`Written` `InternalLog::write_checked` outcomes (PC11, Q3/ADR-039 §AMD-004); audit-loss-observability stderr-diagnostic test, unconditional on suppression outcome (PC12, Q-B/F-S2123-P2-006) — the real VP is anchored to Phase-6 formal-verifier for VP-NNN assignment and propagation to VP-INDEX/verification-architecture.md/verification-coverage-matrix.md per `vp_index_is_vp_catalog_source_of_truth`

## Changelog

| Version | Date | Author | Change |
|---------|------|--------|--------|
| v1.6 | 2026-08-24 | product-owner | Wave-7 adversary pass-6/R5 remediation, cycle v1.0-brownfield-backfill: (1) F-S2123-P6-002 (LOW) — PC9 detector-precision examples corrected from `std::env::var` to `std::env::var_os` (the mandated accessor per Task 3/AC-014/EC-015 "never `std::env::var`"); added accessor-agnostic clarification that the detector keys on the `VSDD_BREAK_GLASS_GATE_BYPASS` token in executable position, not the specific function name; updated all illustrative examples: detector-precision paragraph, comment-stripping sentence, (f.ii) full-call-in-comment sub-case description (example fixture `// let _bypass = std::env::var_os("VSDD_BREAK_GLASS_GATE_BYPASS");`, `.ok()` removed — `var_os` returns `Option<OsString>` directly) and mitigation sentence, Canonical Test Vectors (f.i) and (f.ii) rows. (f.i)/(f.ii) comment-stripping sub-cases preserved intact in structure. (2) F-S2123-P6-003 (LOW, POLICY 19 HIGH-floor risk) — stripped load-bearing ADR-039 version pins (`v1.9`, `v1.10`, `v1.16`) from all active citation sites: H1 title (`§Decision 3 v1.10` → `§Decision 3`), Preconditions 1 and 4, PC9 header, PC9 dual-trigger rationale paragraph, PC11 header (`, v1.16,` removed), Invariant 1, Traceability ADR row (three pins: `v1.9` amendment, `v1.10` amendment, `v1.16` in `§AMD-004`), Security row (`§Decision 3 v1.9`), Architecture Anchors two sites (`§Decision 3 v1.10` and `§AMD-004, RATIFIED v1.16`); stable `ADR-039 §Decision 3` / `ADR-039 §AMD-004` section-anchor form used throughout active text; version tokens retained only as `(as of vX.Y, non-load-bearing)` parentheticals where historical context has semantic value, or dropped where RATIFIED date provides sufficient context. Changelog entries v1.0–v1.5 treated as immutable historical records, exempt from POLICY-19 active-citation cleanup. POLICY-15 grep attestation: `grep -nE 'ADR-039[^)]*v1\.[0-9]+'` pre-patch stdout = 9 matches on lines 156/164/167/461/557/558/575/577/602; all active-text load-bearing matches removed in this patch. 5-leg parity applied: version v1.5→v1.6. NOTE for downstream (step ③ story-writer): S-21.23 + S-21.24 must re-anchor BC-1.03.018 v1.5→v1.6 per bc_array_changes_propagate_to_body_and_acs policy. BC-1.03.018 v1.6. |
| v1.5 | 2026-08-23 | product-owner | Wave-7 adversary pass-4 remediation (F-S2123-P4-001/002/005), cycle v1.0-brownfield-backfill: (1) F-S2123-P4-001 (MED) — PC9 control-letter drift corrected: (f)=COMMENT-ONLY and (g)=LIVE-TREE, aligned to story AC-022's LIVE-TREE=(g) partition; prior BC had (f)=LIVE-TREE and (g)=COMMENT-ONLY, causing a reader to defer to the wrong control letter. Swept VP-TBD (Verification Properties + VP Anchors), Traceability ADR row, Story Anchor, VP Anchors. (2) F-S2123-P4-002 (MED) — PC9 detector false-green closed for commented-out full calls: detector-precision requirement mandates comment-stripping (removal of all `//` and `/* */` comment spans) BEFORE applying the executable-code pattern check, so a complete `std::env::var("VSDD_BREAK_GLASS_GATE_BYPASS")` call inside a comment does not satisfy detection — a bare-substring or grep detector would report PASS, CWE-636 false-green. COMMENT-ONLY control description strengthened to cover the full-call-in-comment sub-case. New Canonical Test Vector row added (full-call-in-comment → gate FIRES RED, F-S2123-P4-002). (3) F-S2123-P4-005 (LOW, PO confirm) — PC3 `all` explicitly stated as a trimmed comma-token (same comma-split-and-trim rule as PC2), confirming `all` is name-scoped shorthand detected by the identical token-matching path as any named gate, not a separate parsing mode. H1 enriched per POLICY 7 with all three corrections. No PC renumbering; PC3/PC8/PC9 amendments corrective/additive within existing scope; PC1-PC2, PC4-PC7, PC10-PC12 UNCHANGED. Does NOT touch BC-1.03.017 (separate product-owner burst, same task), S-21.x story bodies/ACs/frontmatter (story-writer domain, dispatched separately per POLICY 8 propagation), or any INDEX/STATE.md (state-manager domain). Input-hash flagged stale for state-manager reconciliation. BC-1.03.018 v1.5. |
| v1.4 | 2026-08-22 | product-owner | Wave-7 adversary pass-3 remediation, cycle v1.0-brownfield-backfill: (2a) F-S2123-P3-001 (HIGH) — PC8 extended to explicitly state the `all` wildcard is name-scoped shorthand for the two named gates only, NOT a blanket suppress-every-block switch; under `all`, a non-named blocking plugin MUST have its block STAND and MUST NOT receive a `break_glass.activated` audit record — closes a compound CWE-636+CWE-863 hazard a naive `all` implementation would introduce with no audit trail. New Canonical Test Vector row (`all`×non-named-plugin negative control). (2b) F-S2123-P3-003 (LOW-folded-in) — mirrored S-21.23's AC-022 hardening into PC9: added an explicit executable-code-read-pattern detector-precision requirement (a `VSDD_BREAK_GLASS_GATE_BYPASS` token inside a comment does NOT satisfy detection) and a 7th COMMENT-ONLY control, reconciling PC9's control count from six to seven to match the story's actual coverage. New Canonical Test Vector row (comment-only control). Swept sibling sites: VP-TBD (Verification Properties + VP Anchors), Traceability ADR row (new F-S2123-P3-001/003 citations), Security row, Stories row, Story Anchor section. H1 enriched per POLICY 7. No PC renumbering; PC8/PC9 amendments are corrective/additive within existing scope; PC1-PC7, PC10-PC12 UNCHANGED. Does NOT touch BC-1.03.017 (separate product-owner burst, same task), the S-21.x story bodies/ACs/frontmatter (story-writer's domain, dispatched separately this burst per POLICY 8 propagation), or any INDEX/STATE.md (state-manager's domain). This content change means this BC's own `input-hash` will go stale as a result and is flagged for state-manager reconciliation same-burst. BC-1.03.018 v1.4. |
