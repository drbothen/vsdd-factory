...
6. **PC6 — Required event fields:**
   The emitted event MUST carry exactly these fields, matching ADR-039 §Decision 5 Mitigation 1:
   ...
   (`type`, `trace_id`, `session_id`, `ts`, `schema_version` — DI-007 / BC-1.12.002 convention),
   and a `level` field per PC8.
...
8. **PC8 — WARN-level classification and exact message text:**
   ...
   carry a `message` field equal, verbatim, to ADR-039 §Decision 5's mandated text: `"fuel-headroom-warning:
...
| ADR | ADR-039 §Decision 5 Mitigation 1 ("Near-term mitigations: fuel-headroom warning," explicitly independent of the fail-closed policy change in §Decision 1–4/§AMD-001–003) |
