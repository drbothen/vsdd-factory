...
- `crates/factory-dispatcher/src/host/emit_event.rs` — new `emit_plugin_fuel_headroom_warning(ctx:
...
| ADR | ADR-039 §Decision 5 Mitigation 1 (WARN message per §Erratum E-006) ("Near-term mitigations: fuel-headroom warning," explicitly independent of the fail-closed policy change in §Decision 1–4/§AMD-001–003). |
...
|---------|------|--------|--------|
| v1.3 | 2026-08-21 | product-owner | D-1064 Wave-6 pass-6 remediation burst — **F-S2125-P6-001** (HIGH, POLICY 19 `adr_version_cite_volatile_pin_prohibition`): the Traceability ADR row carried a load-bearing `ADR-039 v1.15 §Decision 5 Mitigation 1` version pin, violating POLICY 19/TD-VSDD-091 (stable section-anchor form required; no load-bearing version number). Replaced with the stable form `ADR-039 §Decision 5 Mitigation 1 (WARN message per §Erratum E-006)` — the E-006 parenthetical is explicitly informational/non-load-bearing, naming which erratum corrected the WARN-message text rather than gating the citation on an ADR version number. The `v1.14→v1.15` version-delta provenance narrative (this BC's v1.1 fix burst tracking ADR-039 E-006's `≥90%`→`>90%` WARN-message correction, reproduced verbatim in PC8) is relocated out of the Traceability cell into this Changelog row, where version-delta history belongs per the fix directive — that provenance remains independently documented in the v1.1 row below (F-S2125-P1-006 cascade entry). No postcondition, precondition, or test-vector content changed; this is a Traceability-cell precision fix only. |
| v1.2 | 2026-08-20 | product-owner | S-21.25 adversarial pass-2 (`v1.0-brownfield-backfill`) fix burst — **F-S2125-P2-002** (MEDIUM): §Architecture Anchors named the emitter `emit_fuel_headroom_warning` (no `plugin_` infix), diverging from (a) every sibling emitter in `crates/factory-dispatcher/src/host/emit_event.rs`, which uses the `emit_plugin_*` form (`emit_plugin_timeout_async`, `emit_plugin_completed_async`, `emit_plugin_abandoned`), and (b) VP-079 v1.20 SITE_7, which already names the function `emit_plugin_fuel_headroom_warning` and whose mutation-counter-proof grep predicate searches that exact name — the mismatch would have silently voided SITE_7's coverage. Renamed to **`emit_plugin_fuel_headroom_warning`** at both occurrences (§Architecture Anchors emission-point bullet; v1.0 Changelog row's "new `emit_fuel_headroom_warning()` helper proposed" clause) to align with the sibling convention and VP-079 v1.20 (VP-079 itself is unchanged — it was already correct). Downstream cascade note (not performed here, story-writer's scope): S-21.25's Architecture Mapping, Tasks, and AC-005 test target cite this emitter name and must be swept to `emit_plugin_fuel_headroom_warning` in the same story-writer burst that re-anchors the story's BC-1.03.019 cite v1.1→v1.2. |
