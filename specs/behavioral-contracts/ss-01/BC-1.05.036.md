...
| `caps.cwd_allow = ["/tmp/safe"]`; `ctx.cwd = "/etc"`; capability passes (binary on allow-list); subprocess spawns successfully | Subprocess receives `cwd = "/etc"` — NOT restricted to `/tmp/safe`. No denial event emitted. No error returned. `host.exec_subprocess.completed` IS emitted. **v1 known limitation per EC-012:** `cwd_allow` is declared but not enforced at exec_subprocess.rs:248-250. Test MUST assert NO `internal.capability_denied` event with reason `cwd_not_on_allow_list`. Witnesses EC-012 silent-no-op behavior. | cwd-allow-no-op-witness [LOW-P42-001] |
| `caps.env_allow = ["MY_VAR"]`; `ctx.env_view` does NOT contain `"MY_VAR"`; capability passes; subprocess spawns and exits 0 | Subprocess environment does NOT include `MY_VAR` — name is silently omitted at exec_subprocess.rs:243-247 (`if let Some(val) = env_view.get(name)` branch not taken). No event emitted; no error returned. `host.exec_subprocess.completed` IS emitted. Test MUST assert subprocess did not receive `MY_VAR` (e.g., subprocess echoes env and `MY_VAR` is absent from stdout). Witnesses EC-014 silent-no-op behavior. | env-allow-absent-no-op-witness [LOW-P42-001] |
