...
bc_id: BC-1.13.001
section: "1.13"
last_amended: "2026-06-22 (v1.11) — S-18.14 pass-16 fix burst F-1 (MAJOR POLICY 19): Architecture Anchors line-294 inverse-word-order version-pin `v1.3 Decision 1 Addendum ... and Decision 5` → `§Decision 1 Addendum ... and §Decision 5`. Dual-word-order sweep confirms zero residual normative version-pins in either word-order; all other hits are in exempt sections (modified: array, last_amended:, Changelog table rows). Corrects the false v1.10 attestation."
---
...
8. **INV-8 (Resolver WASM path resolution base):** Relative `plugin` paths in
   ...
   ```
   The `path` parameter of `load_registry` (`pub fn load_registry(&self, path: &Path, ...)`)
