...
last_amended: "2026-08-22 (v1.22) — Wave-7 adversary pass-3 remediation, cycle
...
6. **Calibration-sufficiency precondition — machine-checkable, not merely the 50M floor
   ...
   **Machine-checkable, not assertion-only (mandated form for the standing gate):** this
   ...
   50M floor (PC8's structural gate) while its `observed_max × 1.5` against the frozen
...
| Plugin annotated `fuel_cap=50_000_000` (clears PC8's floor); standing CI gate re-runs against the FROZEN `pc6-sufficiency-snapshot/` fixture, yielding `observed_max`=`40_000_000` → `observed_max × 1.5 = 60_000_000 > 50_000_000` | Sufficiency regression assertion (`fuel_consumed × 1.5 <= registry fuel_cap`) FAILS even though PC8's floor gate PASSES — floor-clearing alone is not calibration-sufficient | half-state-rejected (Precondition 6 sufficiency assertion, Q2/F-S2122-P1-002, ADR-039 §Decision 4 v1.16/§Erratum E-007) |
| Plugin annotated `fuel_cap=75_000_000`; standing CI gate re-runs against the FROZEN `pc6-sufficiency-snapshot/` fixture, yielding `observed_max`=`40_000_000` → `observed_max × 1.5 = 60_000_000 <= 75_000_000` | Sufficiency regression assertion PASSES (`fuel_consumed × 1.5 <= registry fuel_cap` holds) | negative-control (Precondition 6 sufficiency assertion, Q2/F-S2122-P1-002) |
