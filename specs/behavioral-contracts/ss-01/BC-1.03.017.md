...
last_amended: "2026-08-20 (v1.19) — S-21.19 pass-2 remediation of F-S2119-P2-001 (MEDIUM,
...
- `crates/factory-dispatcher/src/executor.rs` — enforcement-active decision path: `plugin_fail_closed`
  ...
  **PC13 extension (ADR-039 §AMD-003, RATIFIED v1.11; narrowed to the correct predicate per
  ADR-039 v1.13 / Erratum E-005, F-S2111V2-P3-001 remediation):** the block-decision function's
  ...
  `PluginResult::Ok { exit_code: 1 }` gap left open by the `Crashed | Timeout` matching above;
  the fix is a single predicate change at the same decision-site this Architecture Anchor
  already governs. The extension fires ONLY on a non-zero-exit `Ok` outcome; it MUST NOT be
  written as a negation of `Ok { exit_code: 0, .. }` — that broad negation wrongly captures
  `Timeout { cause: Fuel | Epoch }` and `Crashed` outcomes as well, collapsing the `on_error`
  axis into deciding exhaustion outcomes and reintroducing the CWE-636 self-lock this story
  prevents (whether a `Timeout` blocks under `failure_policy = FailOpen` is decided exclusively
  by the `failure_policy` axis per PC5/EC-009/Invariant 1, never by `on_error` alone). The base
  rules for `Crashed` and `Timeout` are UNCHANGED, but they are NOT governed by one shared
  predicate (PC13 does not touch either): `Crashed{..}` remains governed SOLELY by `on_error`
  per PC4 (`on_error = Block` blocks unconditionally); `Timeout{..}` remains governed
  EXCLUSIVELY by the `failure_policy` axis per PC1/PC5/PC6/PC10 — `on_error = Block` alone does
  NOT block a `Timeout` when `failure_policy = FailOpen` (PC5/PC10(a)).
