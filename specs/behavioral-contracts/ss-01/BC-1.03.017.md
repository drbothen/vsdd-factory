...
| EC-011 | `legacy-bash-adapter.wasm`-hosted plugin's script runs LONGER than the calibrated `timeout_ms` but SHORTER than the hardcoded 60,000 ms constant (e.g., `timeout_ms=30_000`, script runs 45 s) — the highest-risk AMD-002 defect window | **Pre-wiring-fix (current implementation):** NOT killed — the script silently runs to completion at 45 s, past the intended 30 s budget, and the dispatcher reports a clean pass. This is a silent false clean-pass: the calibrated budget is violated but no `Timeout` outcome is ever produced, because the hardcoded 60 s constant never fires. **Post-wiring-fix (PC12):** killed at ≈30 s; `Timeout{cause: Epoch}` fires; PC1/PC6 enforcement applies normally |
...
| `legacy-bash-adapter.wasm` entry `timeout_ms=30_000`, bash script runs 45 s (CURRENT pre-fix code path) | Script completes normally at 45 s — NOT killed; silent false clean-pass past the calibrated 30 s budget (highest-risk defect window) | AMD-002-silent-false-pass (EC-011, PC12) |
