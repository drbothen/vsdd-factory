...
last_amended: "2026-08-22 (v1.20) — Wave-7 adversary pass-1 remediation, cycle
...
# BC-1.03.017: factory-dispatcher::executor::failure_policy enforcement — exhaustion-outcome dispatch (fail-closed→block; fail-open→advisory), on_error axes independence, crash-versus-exhaustion distinct paths, Phase-3-before-Phase-4 structural half-state gate, migration-window on_error=block completeness gate, legacy-bash-adapter runtime-timeout-wiring verification, and observed_max×1.5 fuel_cap calibration-sufficiency gate (ADR-039 §Decision 3+6 Phase 4 enforcement leg + §AMD-002 wiring leg + §AMD-003 on_error=Block plugin-error fail-closed leg + §Decision 4 v1.16/§Erratum E-007 target-statistic correction; PC11 CWE-636 static gate; PC12 AMD-002 runtime gate; PC13 AMD-003 plugin-error-exit fail-closed gate covering the FULL 18-entry on_error=Block registry set, not a 6-plugin sample; Precondition 6 machine-checkable observed_max×1.5 sufficiency regression assertion; decision function split per ADR-044 v1.1 into `plugin_fail_closed_on_exhaustion` (S-21.24-owned wiring) and `plugin_fail_closed_on_error_exit` (S-21.21-owned wiring))
...
3. For each targeted plugin, the calibrated value has been set per its adapter class from
   ...
   - Native-WASM plugin: `fuel_cap` set to `max(observed_max × 1.5, 50_000_000)`. `50_000_000`
     is an INCLUSIVE FLOOR beneath the `observed_max × 1.5` target, not itself the calibration
     target — a plugin whose `observed_max × 1.5` exceeds `50_000_000` MUST calibrate to the
     higher `observed_max × 1.5` value; clearing the floor alone does not satisfy this
     Precondition (see Precondition 6 for the machine-checkable sufficiency assertion this
     distinction backs).
   - `legacy-bash-adapter.wasm`-hosted plugins: `fuel_cap` set to
     `max(observed_max × 1.5, 50_000_000)` (same target-statistic correction and floor-not-target
...
6. **Calibration-sufficiency precondition — machine-checkable, not merely the 50M floor
   ...
   `fuel_cap` satisfies `fuel_cap >= observed_max × 1.5`, where `observed_max` is measured
   against the mandated calibration corpus (Precondition 2: `lessons.md` at ≥4000 lines;
   `STATE.md` at current live size; `decision-log.md` at current live size; and the ≥574 KB
   (576,396-byte) production-scale fixture at
   `plugins/vsdd-factory/tests/fixtures/validate-cross-site-correspondence/a1-production-scale/factory/specs/behavioral-contracts/BC-INDEX.md`,
   ...
   `observed_max × 1.5` exceeds it.
   **Machine-checkable, not assertion-only:** this sufficiency requirement MUST be enforced by
   a regression assertion, not documented as a one-time calibration note: re-run the plugin
   against the mandated calibration corpus and assert `fuel_consumed × 1.5 <= registry
   fuel_cap` for the corpus's observed `fuel_consumed` reading. A calibration record (PR
   description or artifact) that states a `fuel_cap` value without a corresponding passing
   regression assertion of this inequality is NOT calibration-sufficient evidence — this
   closes the gap where a plugin clears the fixed 50M floor (PC8's structural gate) while its
   `observed_max × 1.5` for the live corpus silently exceeds the annotated cap, leaving the
   fail-closed flip under-calibrated against its own governing formula.
   **If `observed_max × 1.5 >= PRACTICAL_FUEL_CEILING` (`500_000_000`, ADR-039 §Decision 4
...
9. **PC9 — All targeted validator-class plugins carry `failure_policy = "fail-closed"` with
   ...
   `observed_max × 1.5` sufficiency target this floor sits beneath); each
   ...
   **Precondition 6 dependency (calibration-sufficiency, ADR-039 §Decision 4 v1.16 / §Erratum
   ...
   machine-checkable sufficiency assertion (`fuel_cap >= observed_max × 1.5` against the
...
12. **PC12 — AMD-002 runtime wiring: the effective bash-subprocess wall-clock kill deadline
    ...
    **POSITIVE control (the wiring fix's target behavior):** a `hooks-registry.toml` entry
    ...
    ≈2 s plus the `exec_subprocess.rs::run()` poll interval (~5 ms) — well under the hardcoded
    60 s — proving the wiring fix reads and applies the registry's calibrated value rather than
    the hardcoded constant. The dispatcher MUST report `PluginResult::Ok { exit_code: 1, .. }`
    (via `adapter_logic`'s `HookResult::Error` mapping, per the propagation path traced in
    PC13(a)) for that invocation, **NEVER** `PluginResult::Timeout { cause: TimeoutCause::Epoch }`
    — that variant is constructed only by `classify_trap` on a genuine guest `Trap::Interrupt`
    and cannot fire while the guest is blocked inside the synchronous `exec_subprocess` host
    call. PC12 owns the timing/wiring guarantee asserted here — that the kill fires at the
    registry's calibrated `timeout_ms`, not the hardcoded constant; PC13 owns the complementary,
    non-redundant guarantee that for `on_error = Block` entries this resulting
...
2. **No-half-state invariant (ADR-039 §Decision 3):** No `failure_policy = "fail-closed"`
   ...
   calibration TARGET (`max(observed_max × 1.5, 50M)` — v1.16/§Erratum E-007 target-statistic
   correction; `observed_max`, not `measured_p99`); exactly 50_000_000 is the inclusive minimum
   VALID value (`fuel_cap >= 50_000_000` is required; `fuel_cap < 50_000_000` is prohibited),
   but clearing the floor alone is a necessary schema-level minimum, NOT independently
   sufficient calibration evidence — see Precondition 6 for the machine-checkable
   `observed_max × 1.5` sufficiency assertion. The factory default of 20_000_000
...
| Plugin annotated `fuel_cap=50_000_000` (clears PC8's floor); re-run against the mandated calibration corpus yields `observed_max`=`40_000_000` → `observed_max × 1.5 = 60_000_000 > 50_000_000` | Sufficiency regression assertion (`fuel_consumed × 1.5 <= registry fuel_cap`) FAILS even though PC8's floor gate PASSES — floor-clearing alone is not calibration-sufficient | half-state-rejected (Precondition 6 sufficiency assertion, Q2/F-S2122-P1-002, ADR-039 §Decision 4 v1.16/§Erratum E-007) |
| Plugin annotated `fuel_cap=75_000_000`; re-run against the mandated calibration corpus yields `observed_max`=`40_000_000` → `observed_max × 1.5 = 60_000_000 <= 75_000_000` | Sufficiency regression assertion PASSES (`fuel_consumed × 1.5 <= registry fuel_cap` holds) | negative-control (Precondition 6 sufficiency assertion, Q2/F-S2122-P1-002) |
...
| `legacy-bash-adapter.wasm` entry `timeout_ms=2_000`, bash script sleeps 10 s (POST-wiring-fix: `run_bash_via_host` passes registry `timeout_ms`) | Subprocess killed at ≈2_000 ms + ~5 ms poll tolerance; `PluginResult::Ok { exit_code: 1, .. }` (via `HookResult::Error`) observed at that time — NEVER `Timeout{cause: Epoch}` (guest-trap-only variant, cannot fire inside the blocking `exec_subprocess` host call); `test_legacy_bash_adapter_honors_registry_timeout_ms` PASSES | AMD-002-wiring-fixed (PC12 POSITIVE control) |
...
- `crates/factory-dispatcher/src/executor.rs` — enforcement-active decision path, **split into
  ...
  **`plugin_fail_closed_on_error_exit(result, on_error)` — the error-exit leg (ADR-039
  ...
  single-function framing that implied both legs shared S-21.24's wiring timeline.
...
- `plugins/vsdd-factory/hooks-registry.toml` — six targeted plugin entries receive calibrated
  ...
  but for `fuel_cap`, `50_000_000` is a floor beneath the load-bearing `observed_max × 1.5`
  target (ADR-039 §Decision 4 v1.16/§Erratum E-007; Precondition 6's machine-checkable
  sufficiency regression assertion, not PC8's floor gate alone, is what determines calibration
  completeness);
...
- VP-TBD — failure_policy enforcement dispatch: all six postconditions exercised by behavioral
  ...
  validators carry `failure_policy="fail-closed"` with calibrated `fuel_cap >= max(observed_max
  × 1.5, 50M)` (50M is the inclusive floor, `observed_max × 1.5` is the load-bearing target per
  ADR-039 §Decision 4 v1.16/§Erratum E-007); migration-window completeness gate (PC11)
  **VP-TBD hygiene note (S-21.21 O-3; POLICY 9 sanctioned-deferral convention, following the
  ...
  new `observed_max × 1.5` calibration-sufficiency regression-assertion property) is anchored to
