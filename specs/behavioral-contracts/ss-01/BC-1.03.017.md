...
last_amended: "2026-08-24 (v1.27) — Wave-7 pass-8/R7 remediation burst, cycle
v1.0-brownfield-backfill: (F-S2119-R7-001, HIGH) Story Anchors, EC-004 Case A, PC9 descope
narrative, and Traceability Stories row re-anchored from monolithic S-21.11 framing to split
topology per decomposition plan §4: annotation authoring → S-21.19; bash-adapter calibration →
S-21.21; native-WASM calibration → S-21.22; enforcement flip capstone → S-21.24. S-21.13
depends_on updated [S-21.10, S-21.11] → [S-21.10, S-21.22] (split redirect per S-21.19 AC-009
and plan §4). EC-004 Case B options updated: (a) annotate-within → S-21.19 (executor authoring);
(b) block-the-flip → S-21.24 (enforcement flip capstone). PC9 descope narrative updated from
monolithic S-21.11 to split owners (S-21.19/S-21.21/S-21.22/S-21.24). Historical
Changelog/modified:/[Prior:] S-21.11 references preserved (POLICY 1 append-only). Does NOT touch
story bodies/ACs (story-writer step ②) or BC-INDEX/STATE.md (state-manager step ③). Input-hash
flagged stale for state-manager reconciliation. BC-1.03.017 v1.27. [Prior:
2026-08-24 (v1.26) — Wave-7 pass-7/R6 remediation burst, cycle
v1.0-brownfield-backfill: (F-S2121-P7-001, HIGH) Precondition 6 durable-gate flip-conditional
design propagated from decomposition plan §8.8 Addendum (F-S2121-P6-001) to BC SoT. S-21.21's
bash-adapter sufficiency assertion and the general Precondition 6(ii) standing-gate mechanism
now specify that \`ceil(fuel_consumed × 1.5) <= registry fuel_cap\` is asserted ONLY for plugins
whose \`failure_policy == fail-closed\` at test-time (reads LIVE hooks-registry.toml).
Pre-S-21.24-flip the iterated set is empty → gate is vacuously GREEN at S-21.21's own merge.
Post-S-21.24-flip all five plugins are \`fail-closed\` → assertion is load-bearing. Closes the
S-21.21/S-21.24 wave-ordering hazard (same class as F-S2119-P1-001) at the SoT level.
(F-S2121-P7-002/F-S2122-P7-001, MEDIUM ×2) ceil() sweep: four live \`fuel_consumed × 1.5\`
assertion occurrences in Precondition 6 (~L526, ~L562, ~L569) and Canonical Test Vectors
(~L1327-1328) were un-ceil'd despite v1.23 changelog's false-completeness claim; wrapped in
\`ceil(...)\`. TD-VSDD-060 sibling sweep of full BC found two additional occurrences in the VP-TBD
Verification Properties row and the Traceability ADR row; both fixed. H1 updated per POLICY 7.
Does NOT touch BC-1.03.018 (separate file), story bodies/ACs (story-writer step ②), or
INDEX/STATE.md (state-manager step ③). Input-hash flagged stale for state-manager reconciliation.
BC-1.03.017 v1.26. [Prior:
2026-08-24 (v1.25) — Wave-7 floor-breaking consistency remediation, cycle
v1.0-brownfield-backfill (C-W7-004, HIGH, BC-half): S-21.21 durable-gate artifact path and
task reference corrected. H1 and Precondition 6 S-21.21 bullet updated: "Task 5a frozen
`pc6-sufficiency-snapshot/`" was wrong on two counts — Task 5a per §8.7 is S-21.21's
ADDITIVE error-exit wiring step (unrelated to snapshot management), and `pc6-sufficiency-snapshot/`
is S-21.22's native-WASM snapshot, not S-21.21's. Corrected per decomposition plan §8.8 to:
(i) artifact path `pc6-bash-adapter-sufficiency-snapshot/` (dedicated sibling directory to
S-21.22's `pc6-sufficiency-snapshot/`); (ii) stable description "S-21.21's bash-adapter
frozen-corpus durable standing CI regression gate" (TD-VSDD-091 anti-volatile-pin). S-21.22's
durable-gate reference (`pc6-sufficiency-snapshot/`, Task 5a) verified unchanged and correct.
Sibling sweep (TD-VSDD-060): all other occurrences of `pc6-sufficiency-snapshot` and `Task 5a`
are either S-21.22-specific (line ~513), general mechanism descriptions (lines ~529–548),
general VP/test-vector statements not specific to S-21.21, or historical changelog records
(lines ~12–16, ~47, ~1517) — all verified correct or historical, no further live-section
changes required. Does NOT touch BC-1.03.018 (separate file), story bodies/ACs (story-writer
step ③), or INDEX/STATE.md (state-manager step ④). Input-hash flagged stale for state-manager
reconciliation. BC-1.03.017 v1.25. [Prior:
2026-08-23 (v1.24) — Wave-7 adversary pass-5 remediation, cycle
v1.0-brownfield-backfill: (F-S2121-P5-001, HIGH) Precondition 6 calibration-ownership
over-reach corrected: the v1.23 claim that BOTH calibration checks — (i) one-time
live-corpus calibration confirmation (Task 4) and (ii) durable standing CI regression gate
(Task 5a) — are OWNED BY S-21.22 for ALL SIX ADR-039 §Decision 2 validators is
semantically impossible under the decomposition plan's own allocation (§8.8). Corrected
to the architect-adjudicated split-ownership matrix: S-21.21 OWNS BOTH checks for the
FIVE `legacy-bash-adapter.wasm`-hosted plugins (Tasks #6–#15 one-time confirmation, after
AMD-002 wiring fix GREEN, and Task 5a frozen `pc6-sufficiency-snapshot/` durable CI gate);
S-21.22 OWNS BOTH checks for the ONE native-WASM plugin
`validate-cross-site-correspondence` ONLY (Tasks #1–#5 one-time confirmation and Task 5a
frozen snapshot durable CI gate). Per-story ownership preserves wave-7 parallelism (no
`depends_on` edge between S-21.21 and S-21.22); routing S-21.21's five-plugin sufficiency
gates to S-21.22 would create a false sequential DAG dependency and inflate S-21.22's scope
beyond its native-WASM-only mandate (§8.8 rationale). H1 title updated per POLICY 7 to
reflect split ownership (F-S2121-P5-001 corrects F-S2121-P4-002 over-reach). Sibling sweep
(TD-VSDD-060): only Precondition 6 and H1 carried the over-reach claim; Architecture
Anchors, Canonical Test Vectors, VP Anchors, Verification Properties, Traceability, and
historical records verified free of this assertion — no other live-section changes required.
Does NOT touch BC-1.03.018 (separate file), story bodies/ACs (story-writer step ③), or
INDEX/STATE.md (state-manager step ④). Input-hash flagged stale for state-manager
reconciliation. BC-1.03.017 v1.24. [Prior:
2026-08-23 (v1.23) — Wave-7 adversary pass-4 remediation, cycle
...
6. **Calibration-sufficiency precondition — machine-checkable, not merely the 50M floor
   ...
   **SPLIT OWNERSHIP (F-S2121-P5-001 — corrects F-S2121-P4-002 over-reach; §8.8
   adjudication 2026-08-23):** The prior v1.23 claim that BOTH checks are OWNED BY S-21.22
   for ALL SIX validators is incorrect and is closed by F-S2121-P5-001 (§8.8). The
   authoritative split-ownership matrix per architect §8.8:
   - **S-21.21 OWNS BOTH checks** for the FIVE `legacy-bash-adapter.wasm`-hosted plugins
     ...
     (i) one-time live-corpus calibration confirmation — performed during Tasks #6–#15,
     AFTER the AMD-002 `timeout_ms` wiring fix is GREEN, against the wired code path,
     using the `ceil(observed_max × 1.5)` formula; and (ii) S-21.21's bash-adapter
     frozen-corpus durable standing CI regression gate — commits the frozen
     `pc6-bash-adapter-sufficiency-snapshot/` directory alongside the calibration output,
     asserting `ceil(fuel_consumed × 1.5) <= registry fuel_cap` against the frozen snapshot per
     Precondition 6(ii); FLIP-CONDITIONAL (F-S2121-P7-001/§8.8 Addendum F-S2121-P6-001): the
     `ceil(fuel_consumed × 1.5) <= registry fuel_cap` assertion iterates ONLY over plugins whose
     `failure_policy == fail-closed` at test-time (reads LIVE hooks-registry.toml) —
     pre-S-21.24-flip the iterated set is empty and the gate is vacuously GREEN at S-21.21's own
     merge; post-S-21.24-flip all five plugins are `fail-closed`, the set is non-empty, and the
     assertion is load-bearing; not re-run against live-growing files.
   - **S-21.22 OWNS BOTH checks** for the ONE native-WASM plugin
     `validate-cross-site-correspondence` ONLY:
     (i) one-time live-corpus calibration confirmation — Tasks #1–#5, production-scale
     corpus calibration (≥574 KB fixture + live corpus files); and (ii) durable standing CI
     regression gate — Task 5a: frozen `pc6-sufficiency-snapshot/` assertion, per
     Precondition 6(ii).
   Per-story ownership preserves wave-7 parallelism (S-21.21 and S-21.22 have no
   `depends_on` edge between them — they are fully parallel seams). Routing S-21.21's
   five-plugin sufficiency gates to S-21.22 would create a false sequential DAG dependency
   and inflate S-21.22's scope beyond its native-WASM-only mandate (§8.8 rationale per
   decomposition plan).
   **TWO distinct checks — frozen-vs-live split (F-S2122-P3-001 correction, Wave-7 pass-3
   ...
   (ii) **DURABLE, standing CI regression assertion (Task 5a) — the MANDATED form for the
        ...
        `ceil(fuel_consumed(pc6-sufficiency-snapshot/) × 1.5) <= registry fuel_cap` — a stable,
   ...
   **FLIP-CONDITIONAL SCOPE (F-S2121-P7-001/§8.8 Addendum F-S2121-P6-001):** for per-plugin
   durable gates (e.g., S-21.21's `test_bash_adapter_plugins_fuel_cap_sufficiency()`), the
   `ceil(fuel_consumed × 1.5) <= registry fuel_cap` assertion MUST be scoped to plugins whose
   `failure_policy == fail-closed` at test-time (reads LIVE hooks-registry.toml): if no plugin
   in the registry carries `failure_policy=fail-closed` at a given CI-run, the loop body is
   never entered and the gate is vacuously GREEN — preventing the wave-ordering hazard (same
   class as F-S2119-P1-001) where the gate fires RED at merge time because the capstone story
   (S-21.24) has not yet flipped the plugins to `fail-closed`.
   **Machine-checkable, not assertion-only (mandated form for the standing gate):** this
   ...
   assert `ceil(fuel_consumed × 1.5) <= registry fuel_cap` for the snapshot's observed
...
9. **PC9 — All targeted validator-class plugins carry `failure_policy = "fail-closed"` with
   ...
   **PC12 dependency for full protection (ADR-039 §AMD-002, RATIFIED v1.10):** satisfying
   ...
   hardcoded `BASH_TIMEOUT_MS` constant) MUST additionally hold before S-21.24's enforcement
   flip capstone for these five plugins is considered complete. A commit that satisfies
   ...
   (calibration reveals an impractical cap requirement), the annotation for that plugin
   (authored by S-21.19; calibration owned by S-21.21 for bash-adapter plugins or S-21.22 for
   native-WASM) is descoped to the flippable subset via orchestrator-approved spec amendment;
   PC9's asserted set is reduced to the flippable plugins only; the deferred plugin routes to
   named follow-up story S-21.13. PC9
   ...
   **annotate-or-block-the-flip only**: either (a) annotate the plugin
   `failure_policy="fail-closed"` within S-21.19 (executor authoring; even if the cap
   requirement is high; surface to orchestrator and raise the cap), or (b) block the entire
   Phase-4 executor flip (do not ship the enforcement-active decision path in S-21.24
   (enforcement flip capstone)) until the plugin can be annotated in a follow-up. There is NO path that permits the enforcement-active executor to merge while any
...
11. **PC11 — Hard migration-window completeness gate: if the executor is in enforcement-active
    ...
    **EC-004 descope does NOT reduce the PC11 assertion set:** EC-004's reduced-set deferral
    ...
    — they must be annotated `failure_policy="fail-closed"` within S-21.19 (executor authoring;
    S-21.11 superseded) or the Phase-4 flip (S-21.24 enforcement flip capstone) must be blocked
    entirely (see EC-004 amendment v1.4). PC11's five-plugin assertion has no reduced-set escape.
...
12. **PC12 — AMD-002 runtime wiring: the effective bash-subprocess wall-clock kill deadline
    ...
    **Blast-radius scope note (non-restrictive on this PC's assertion; restrictive on
    S-21.24's `failure_policy` flip scope (enforcement flip capstone; S-21.11 superseded)):** the wiring defect this PC closes is global to
    ...
    — the fix is adapter-level, not per-plugin. S-21.19's `failure_policy = "fail-closed"`
    annotation scope (executor authoring; PC9; S-21.11 superseded) remains the five/six named
    plugins only; PC12 does not expand PC9's targeted set.
...
| EC-004 | Calibration reveals a targeted plugin needs `fuel_cap >= PRACTICAL_FUEL_CEILING` (`500_000_000`, ADR-039 §Decision 4 v1.16) for `ceil(observed_max × 1.5)` (Precondition 6's sufficiency target — v1.16/§Erratum E-007 target-statistic correction from the prior `p99×1.5` framing; ceil() per F-S2122-P4-001) | MUST surface to orchestrator; do not annotate with an insufficient cap. **Path diverges by `on_error` value — the two cases are NOT symmetric:** **Case A — `on_error="continue"` plugin (`validate-cross-site-correspondence`) — VALID descope path:** the annotation for `validate-cross-site-correspondence` (authored by S-21.19; calibration owned by S-21.22) is descoped to the flippable subset via orchestrator-approved spec amendment; PC9's enumerated set is reduced to the flippable plugins only; the deferred plugin routes to named follow-up story **S-21.13** (validate-cross-site-correspondence targeted-row lookup eliminating the O(n) fuel ceiling; depends_on [S-21.10, S-21.22]). This is behavior-neutral: `validate-cross-site-correspondence` already fails open on exhaustion under the 2-arg function and continues to do so until annotated. PC11's five-plugin assertion is unaffected because `validate-cross-site-correspondence` is not among the five `on_error="block"` plugins asserted by PC11. **Annotation-landing obligation (F-S2111-P5-005):** S-21.13 (or its named successor) MUST include an explicit mandate to annotate `validate-cross-site-correspondence` with `failure_policy="fail-closed"` and a calibrated `fuel_cap >= 50_000_000` once its O(n) fuel-ceiling algorithmic fix removes the excessive cap requirement. The EC-004 descope is a timing deferral only, not a permanent exemption from fail-closed enforcement — the fail-closed annotation MUST NOT fall through the descope. **Case B — `on_error="block"` plugins (`validate-factory-path-root`, `validate-input-hash`, `validate-template-compliance`, `validate-pr-merge-prerequisites`, `validate-wave-gate-prerequisite`) — EC-004 is NOT a valid descope path (F-S2111-P4-002):** For these five plugins, deferral via EC-004 is **forbidden**. The only permitted resolutions are: **(a) annotate-within-S-21.19:** annotate the plugin `failure_policy="fail-closed"` in S-21.19 (executor authoring; even if the cap requirement is high; surface to orchestrator and raise it); OR **(b) block-the-flip:** do not ship the enforcement-active decision path in S-21.24 (enforcement flip capstone) until the plugin is annotated in a follow-up story. There is NO path that permits the enforcement-active executor to merge while any `on_error="block"` plugin remains at `failure_policy=fail-open` — PC11's CI gate (test_no_on_error_block_without_fail_closed_when_3arg_executor) makes that state mechanically un-mergeable. Routing an `on_error="block"` plugin to S-21.13 is a mis-route: S-21.13 is scoped exclusively to `validate-cross-site-correspondence`'s O(n) fuel-ceiling algorithmic fix and has no mandate to annotate `on_error="block"` plugins. |
...
| EC-007 | New validator-class plugin added after S-21.24 (enforcement flip capstone; S-21.11 superseded) merges (without `failure_policy = "fail-closed"`) | Defaults to `fail-open` per BC-1.01.016 backward-compat; PC8 gate only fires for annotated-but-uncalibrated entries; classification of new plugins is a future-story concern |
...
- **BC-1.01.016** — prerequisite schema: provides `FailurePolicy` enum and
  `RegistryEntry.failure_policy` field; S-21.10 (BC-1.01.016) MUST merge before any of S-21.19/S-21.21/S-21.22/S-21.24 (split sub-stories implementing this BC; S-21.11 superseded)
...
- S-21.19 (executor authoring: extended decision function + inert PluginOutcome.failure_policy plumbing — NOT the enforcement flip; ADR-044 §Decision 1)
- S-21.21 (bash-adapter calibration + error-exit wiring fix for five legacy-bash-adapter.wasm-hosted plugins; ADR-039 §AMD-001/§AMD-002)
- S-21.22 (native-WASM calibration for validate-cross-site-correspondence; ADR-039 §Decision 4)
- S-21.24 (enforcement flip capstone: Phase 4 wiring of plugin_fail_closed_on_exhaustion into executor block-decision chain; depends_on [S-21.19, S-21.20, S-21.21, S-21.22, S-21.23]; ADR-044)
- S-21.13 (EC-004 follow-up for `validate-cross-site-correspondence` only: targeted-row lookup eliminating O(n) fuel ceiling; depends_on [S-21.10, S-21.22]; scoped exclusively to the on_error=continue plugin; on_error=block plugins are NOT routed here per EC-004 amendment v1.4; MUST include explicit mandate to annotate `validate-cross-site-correspondence` `failure_policy="fail-closed"` once O(n) fuel ceiling is removed — annotation-landing obligation per F-S2111-P5-005)
...
| Stories | S-21.10 (prerequisite), S-21.19 (executor authoring), S-21.21 (bash-adapter calibration + error-exit wiring), S-21.22 (native-WASM calibration), S-21.24 (enforcement flip capstone), S-21.13 (EC-004 follow-up for `validate-cross-site-correspondence` only; depends_on [S-21.10, S-21.22]; on_error=block plugins are NOT routed here per EC-004 amendment v1.4; MUST annotate `validate-cross-site-correspondence` fail-closed once O(n) fuel ceiling removed per F-S2111-P5-005) |
...
|---------|------|--------|--------|
| v1.27 | 2026-08-24 | product-owner | Wave-7 pass-8/R7 remediation burst, cycle v1.0-brownfield-backfill: (F-S2119-R7-001, HIGH) Story Anchors, EC-004 Case A, PC9 descope narrative, and Traceability Stories row re-anchored from monolithic S-21.11 framing to split topology per decomposition plan §4: annotation authoring → S-21.19; bash-adapter calibration → S-21.21; native-WASM calibration → S-21.22; enforcement flip capstone → S-21.24. S-21.13 depends_on updated [S-21.10, S-21.11] → [S-21.10, S-21.22] (split redirect per S-21.19 AC-009 and plan §4). EC-004 Case B option (a) updated from "annotate-within-S-21.11" to "annotate-within-S-21.19"; option (b) "block-the-flip" updated from "do not ship...in S-21.11" to "do not ship...in S-21.24 (enforcement flip capstone)". PC9 descope narrative updated: "S-21.11 is descoped..." → "the annotation for that plugin (authored by S-21.19; calibration owned by S-21.21/S-21.22) is descoped...". Historical Changelog/modified:/[Prior:] S-21.11 references preserved per POLICY 1 (append-only). Anti-straggler comprehensiveness pass (same-version, no bump): additional live-body S-21.11 occurrences re-anchored: (a) PC11 "annotated within S-21.11" → S-21.19 (executor authoring); (b) PC12 blast-radius note "S-21.11's `failure_policy` flip scope" → S-21.24; (c) PC12 "S-21.11's annotation scope" → S-21.19; (d) EC-007 "after S-21.11 merges" → S-21.24 (enforcement flip capstone); (e) Related BCs "MUST merge before S-21.11 (this BC)" → split sub-stories S-21.19/S-21.21/S-21.22/S-21.24; (f) Traceability ADR row "delivered WITHIN S-21.11" (§AMD-002 wiring) → S-21.21; (g) Traceability ADR row "delivered WITHIN S-21.11, intra-story ordering" (break-glass) → S-21.23/S-21.24; (h) Traceability ADR row §Decision 3 "delivered within S-21.11 (prior follow-up name S-21.17 retired)" → S-21.23. One correct superseded-reference left intact: ADR-044 "when S-21.11 is partitioned across independently-mergeable sub-stories" (historical ADR rationale). Does NOT touch story bodies/ACs (story-writer step ②) or BC-INDEX/STATE.md (state-manager step ③). Input-hash flagged stale for state-manager reconciliation. BC-1.03.017 v1.27. |
| v1.26 | 2026-08-24 | product-owner | Wave-7 pass-7/R6 remediation burst, cycle v1.0-brownfield-backfill: (F-S2121-P7-001, HIGH) Precondition 6 S-21.21-specific bullet and general (ii) mechanism updated to flip-conditional design per decomposition plan §8.8 Addendum (F-S2121-P6-001): the `ceil(fuel_consumed × 1.5) <= registry fuel_cap` assertion in S-21.21's `test_bash_adapter_plugins_fuel_cap_sufficiency()` and in Precondition 6(ii)'s standing-gate description iterates ONLY over plugins whose `failure_policy == fail-closed` at test-time (reads LIVE hooks-registry.toml); pre-S-21.24-flip the iterated set is empty → gate is vacuously GREEN at S-21.21's own merge; post-S-21.24-flip all five plugins are `fail-closed` → assertion is load-bearing. Closes the S-21.21/S-21.24 wave-ordering hazard (same class as F-S2119-P1-001) at the SoT level. S-21.22's gate (Task 5a, `validate-cross-site-correspondence`) evaluated for applicability — not applicable (S-21.22 sets its own calibrated `fuel_cap` before Task 5a asserts it; no wave-ordering hazard). (F-S2121-P7-002/F-S2122-P7-001, MEDIUM ×2) ceil() sweep: four named live `fuel_consumed × 1.5` assertion occurrences (~L526, ~L562, ~L569, Canonical Test Vectors ~L1327-1328) wrapped in `ceil(...)`. TD-VSDD-060 sibling sweep of full BC found two additional occurrences (VP-TBD Verification Properties row, Traceability ADR row); both fixed. H1 updated per POLICY 7. Does NOT touch BC-1.03.018 (separate file), story bodies/ACs (story-writer step ②), or INDEX/STATE.md (state-manager step ③). Input-hash flagged stale for state-manager reconciliation. BC-1.03.017 v1.26. |
| v1.25 | 2026-08-24 | product-owner | Wave-7 floor-breaking consistency remediation, cycle v1.0-brownfield-backfill: (C-W7-004, HIGH, BC-half) S-21.21 durable-gate artifact path and task reference corrected in H1 and Precondition 6. The v1.24 split-ownership text correctly assigned S-21.21 as owner of the five bash-adapter plugins' durable CI gate, but used the wrong artifact path (`pc6-sufficiency-snapshot/` — S-21.22's native-WASM snapshot) and wrong task label ("Task 5a" — per §8.7, that label is allocated to S-21.21's ADDITIVE error-exit wiring step, unrelated to snapshot management). Corrected per decomposition plan §8.8: (i) artifact path → `pc6-bash-adapter-sufficiency-snapshot/` (dedicated sibling directory to S-21.22's `pc6-sufficiency-snapshot/`); (ii) stable description → "S-21.21's bash-adapter frozen-corpus durable standing CI regression gate" (TD-VSDD-091 anti-volatile-pin). S-21.22's durable-gate reference (`pc6-sufficiency-snapshot/`, Task 5a) verified unchanged and correct. Sibling sweep (TD-VSDD-060): all other occurrences of `pc6-sufficiency-snapshot` and `Task 5a` are either S-21.22-specific (~line 513), general mechanism descriptions (~lines 529–548), general VP/test-vector statements, or historical changelog records (~lines 12–16, ~47, ~1517) — all verified correct or historical, no further live-section changes required. Does NOT touch BC-1.03.018 (separate file), story bodies/ACs (story-writer step ③), or INDEX/STATE.md (state-manager step ④). Input-hash flagged stale for state-manager reconciliation. BC-1.03.017 v1.25. |
| v1.24 | 2026-08-23 | product-owner | Wave-7 adversary pass-5 remediation, cycle v1.0-brownfield-backfill: (F-S2121-P5-001, HIGH) Precondition 6 calibration-ownership over-reach corrected. The v1.23 claim that BOTH calibration checks — (i) one-time live-corpus calibration confirmation and (ii) durable standing CI regression gate — are OWNED BY S-21.22 for ALL SIX ADR-039 §Decision 2 validators is semantically impossible under the plan's own allocation and is closed by F-S2121-P5-001 (§8.8 adjudication 2026-08-23). Corrected to the authoritative split-ownership matrix: S-21.21 OWNS BOTH checks for the FIVE `legacy-bash-adapter.wasm`-hosted plugins (Tasks #6–#15 one-time calibration confirmation, after AMD-002 wiring fix GREEN, and Task 5a frozen `pc6-sufficiency-snapshot/` durable CI gate); S-21.22 OWNS BOTH checks for the ONE native-WASM plugin `validate-cross-site-correspondence` ONLY (Tasks #1–#5 one-time confirmation and Task 5a frozen snapshot durable CI gate). Per-story ownership preserves wave-7 parallelism (no `depends_on` edge between S-21.21 and S-21.22). H1 title updated per POLICY 7 to reflect split ownership. Sibling sweep (TD-VSDD-060): only Precondition 6 (~lines 473–478 in v1.23) and H1 carried the over-reach claim; Architecture Anchors, Canonical Test Vectors, VP Anchors, Verification Properties, Traceability, and all historical records verified free of this assertion — no other live-section changes required. Does NOT touch BC-1.03.018 (separate file), story bodies/ACs (story-writer step ③), or INDEX/STATE.md (state-manager step ④). Input-hash flagged stale for state-manager reconciliation. BC-1.03.017 v1.24. |
