...
last_amended: "2026-08-19 (v1.16) — S-21.11 v2.3 PRE-TDD spec-convergence cascade remediation
...
- `crates/factory-dispatcher/src/executor.rs` — enforcement-active decision path: `plugin_fail_closed`
  ...
  **PC13 extension (ADR-039 §AMD-003, RATIFIED v1.11; narrowed to the correct predicate per
  ...
  rule — `on_error == OnError::Block` AND `result` is `Crashed{..}` or `Timeout{..}` — is
  UNCHANGED and remains governed solely by `on_error` per PC4/PC10.
