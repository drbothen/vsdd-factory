...
last_amended: "2026-08-23 (v1.24) — Wave-7 adversary pass-5 remediation, cycle
...
6. **Calibration-sufficiency precondition — machine-checkable, not merely the 50M floor
   ...
   **SPLIT OWNERSHIP (F-S2121-P5-001 — corrects F-S2121-P4-002 over-reach; §8.8
   ...
   - **S-21.21 OWNS BOTH checks** for the FIVE `legacy-bash-adapter.wasm`-hosted plugins
     ...
     using the `ceil(observed_max × 1.5)` formula; and (ii) durable standing CI regression
     gate — Task 5a: frozen `pc6-sufficiency-snapshot/` assertion committed alongside the
     calibration, asserting `fuel_consumed × 1.5 <= registry fuel_cap` against the frozen
     snapshot per Precondition 6(ii); not re-run against live-growing files.
   - **S-21.22 OWNS BOTH checks** for the ONE native-WASM plugin
