...
origin: brownfield
extracted_from: "pass-3-deep-templates-tools-rules.md:1293"
subsystem: "SS-10"
capability: "TBD"
...
# Behavioral Contract BC-10.02.007: emit-event: atomic JSONL append exploits POSIX PIPE_BUF guarantee

> Section: emit-event CLI
> Source BC (audit ID): BC-AUDIT-2111
...
1. Concurrent invocations produce non-interleaved JSONL.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Single `printf '%s\n' "$EVENT" >> "$LOGFILE"`. POSIX guarantees writes under PIPE_BUF (~4KB) on a single `write()` sysca | manual (existing test: `plugins/vsdd-factory/bin/emit-event`) |
...
| **Source line(s)** | 39–42, 142–144 |
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-2111 |
| **Pass-3 source** | `pass-3-deep-templates-tools-rules.md:1293` |
