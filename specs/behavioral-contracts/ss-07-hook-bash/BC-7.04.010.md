...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1133"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.010: validate-changelog-monotonicity: blocks duplicate version rows

## Description

validate-changelog-monotonicity: blocks duplicate version rows. Adds error "duplicate version 'X' (also at prior row)"; ultimately emits hook.block (`changelog_not_monotonic`), exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1101` (extracted from `pass-3-deep-hooks.md` line 1133).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-changelog-monotonicity.sh``.

## Preconditions

1. Trigger: Two consecutive changelog rows have identical version field.

## Postconditions

1. Behavior: Adds error "duplicate version 'X' (also at prior row)"; ultimately emits hook.block (`changelog_not_monotonic`), exit 2.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1133 |
| **Audit ID** | `BC-AUDIT-1101` |
| **Source Line(s) (within hook)** | 124-127. |
