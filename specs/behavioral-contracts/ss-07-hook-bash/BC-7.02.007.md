...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts-deep-r1.md:234"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.02.007: Validator hooks at `tool = "Edit\

## Description

Validator hooks at `tool = "Edit|Write"` regex run on EVERY post-edit / post-write event regardless of file path. All 23 distinct validator hooks are routed via the `tool = "Edit|Write"` regex match (per `routing.rs::tool_matches` BC-AUDIT-005). The validators must therefore self-narrow via the `case "$FILE_PATH"` pattern from BC-AUDIT-102 to avoid burning latency on irrelevant writes.

**Source category:** Validator hook class contracts.
**Audit ID:** `BC-AUDIT-105` (extracted from `pass-3-behavioral-contracts-deep-r1.md` line 234).

## Preconditions

1. PostToolUse fires after Edit or Write.

## Postconditions

1. All 23 distinct validator hooks are routed via the `tool = "Edit|Write"` regex match (per `routing.rs::tool_matches` BC-AUDIT-005). The validators must therefore self-narrow via the `case "$FILE_PATH"` pattern from BC-AUDIT-102 to avoid burning latency on irrelevant writes.
...
| **Source Document** | `pass-3-behavioral-contracts-deep-r1.md` line 234 |
| **Audit ID** | `BC-AUDIT-105` |
...
> `hooks-registry.toml` validate-* stanzas all have `tool = "Edit|Write"` (read 100 LOC + spot-checks); `routing.rs::tests::match_respects_tool_regex_anchoring` (cited by Pass 3 BC-AUDIT-005).
