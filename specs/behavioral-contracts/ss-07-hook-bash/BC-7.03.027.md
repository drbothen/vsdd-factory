...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:332"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.027: destructive-command-guard: blocks `rm -rf` on catastrophic roots (8 patterns)

## Description

destructive-command-guard: blocks `rm -rf` on catastrophic roots (8 patterns). Block code `catastrophic_root`, emits hook.block, exits 2 with "Catastrophic rm target detected".

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1026` (extracted from `pass-3-deep-hooks.md` line 332).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: `rm` with recursive flag (`-r`, `-R`, `-rf`, `-fr`, `-Rf`, `-fR`, `--recursive`) targeting one of 8 catastrophic roots: `/`, `/*`, `~`, `~/`, `$HOME`, `$HOME/`, `*`, `.*`.

## Postconditions

1. Behavior: Block code `catastrophic_root`, emits hook.block, exits 2 with "Catastrophic rm target detected".
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `rm -rf /`, `rm -rf $HOME`, `rm -rf .*` all blocked. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 332 |
| **Audit ID** | `BC-AUDIT-1026` |
| **Source Line(s) (within hook)** | 67-84. |
