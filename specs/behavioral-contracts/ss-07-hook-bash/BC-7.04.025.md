...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1290"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.025: validate-input-hash: BLOCKS on hash length != 7

## Description

validate-input-hash: BLOCKS on hash length != 7. Emits hook.block (`input_hash_invalid_format`, issue=length, fields stored_hash + hash_len). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1116` (extracted from `pass-3-deep-hooks.md` line 1290).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-input-hash.sh``.

## Preconditions

1. Trigger: STORED_HASH non-placeholder AND length != 7.

## Postconditions

1. Behavior: Emits hook.block (`input_hash_invalid_format`, issue=length, fields stored_hash + hash_len). Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Hash `abcdef` (6 chars) blocked; hash `abc1234` (7 chars hex) passes length check. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1290 |
| **Audit ID** | `BC-AUDIT-1116` |
| **Source Line(s) (within hook)** | 88-96. |
