...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:199"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.015: capture-pr-activity: emits `pr.opened` from `gh pr create` stdout

## Description

capture-pr-activity: emits `pr.opened` from `gh pr create` stdout. Extracts PR URL via `https://github\.com/[^/]+/[^/]+/pull/[0-9]+` regex from stdout, derives pr_number and pr_repo, optionally extracts `--title "..."` text, emits `pr.opened` event. If no URL in stdout, no event.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1014` (extracted from `pass-3-deep-hooks.md` line 199).
**Hook script:** ``plugins/vsdd-factory/hooks/capture-pr-activity.sh``.

## Preconditions

1. Trigger: Successful Bash command matching `(^|[;&|[:space:]])gh[[:space:]]+pr[[:space:]]+create([[:space:]]|$)` regex.

## Postconditions

1. Behavior: Extracts PR URL via `https://github\.com/[^/]+/[^/]+/pull/[0-9]+` regex from stdout, derives pr_number and pr_repo, optionally extracts `--title "..."` text, emits `pr.opened` event. If no URL in stdout, no event.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `gh pr create --title "fix" --body "..."` returning a github URL produces a `pr.opened` event with that URL. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 199 |
| **Audit ID** | `BC-AUDIT-1014` |
| **Source Line(s) (within hook)** | 121-138. |
