...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:278"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.022: convergence-tracker: BLOCKS premature CONVERGENCE_REACHED with novelty>0.15

## Description

convergence-tracker: BLOCKS premature CONVERGENCE_REACHED with novelty>0.15. Multiplies score by 100, treats as int, blocks if >15. Emits `hook.block` event with reason `convergence_rule_violation` and exits 2 with diagnostic.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1021` (extracted from `pass-3-deep-hooks.md` line 278).
**Hook script:** ``plugins/vsdd-factory/hooks/convergence-tracker.sh``.

## Preconditions

1. Trigger: Verdict == `CONVERGENCE_REACHED` AND novelty score numeric.

## Postconditions

1. Behavior: Multiplies score by 100, treats as int, blocks if >15. Emits `hook.block` event with reason `convergence_rule_violation` and exits 2 with diagnostic.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Verdict CONVERGENCE_REACHED + novelty score 0.20 → exit 2 + "novelty score is 0.20 (must be ≤ 0.15)". | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 278 |
| **Audit ID** | `BC-AUDIT-1021` |
| **Source Line(s) (within hook)** | 123-131. |
