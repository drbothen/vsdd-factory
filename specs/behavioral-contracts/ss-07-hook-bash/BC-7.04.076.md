...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1789"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.076: validate-wave-gate-completeness: scopes to wave-state.yaml writes; finds newly-passed waves

## Description

validate-wave-gate-completeness: scopes to wave-state.yaml writes; finds newly-passed waves. Other paths exit 0; no newly-passed waves exit 0.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1167` (extracted from `pass-3-deep-hooks.md` line 1789).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-wave-gate-completeness.sh``.

## Preconditions

1. Trigger: file_path matches `*wave-state.yaml`. Python parses YAML, walks `waves`, prints `<wave_name>:<gate_report-path>` for each `gate_status == passed`.

## Postconditions

1. Behavior: Other paths exit 0; no newly-passed waves exit 0.
2. Exit codes: 0 (skip).
...
| **Source Document** | `pass-3-deep-hooks.md` line 1789 |
| **Audit ID** | `BC-AUDIT-1167` |
| **Source Line(s) (within hook)** | 46-76. |
