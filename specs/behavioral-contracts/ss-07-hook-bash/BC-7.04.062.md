...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1654"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.062: validate-table-cell-count: blocks rows with pipe count != header pipe count

## Description

validate-table-cell-count: blocks rows with pipe count != header pipe count. Emit hook.block (`table_cell_count_mismatch`). Stderr "TABLE CELL COUNT VIOLATION", per-line error with line number and counts. Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1153` (extracted from `pass-3-deep-hooks.md` line 1654).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-table-cell-count.sh``.

## Preconditions

1. Trigger: State machine: header (line with pipes) → separator (`^\|[-: |]+\|$`) confirms table → data rows. Counts unescaped `|` (after stripping `\|`). Per row, if pipe_count != header_pipes → error.

## Postconditions

1. Behavior: Emit hook.block (`table_cell_count_mismatch`). Stderr "TABLE CELL COUNT VIOLATION", per-line error with line number and counts. Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Header has 4 pipes; data row has 5 (unescaped `\|` in cell text) → block with line number. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-table-cell-count.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1654 |
| **Audit ID** | `BC-AUDIT-1153` |
| **Source Line(s) (within hook)** | 40-93. |
