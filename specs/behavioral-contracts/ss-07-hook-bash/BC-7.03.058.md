...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:656"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.058: protect-secrets: blocks `echo\

## Description

protect-secrets: blocks `echo|printf` of secret-shaped variables. Block code `secret_var_echo`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1057` (extracted from `pass-3-deep-hooks.md` line 656).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-secrets.sh``.

## Preconditions

1. Trigger: Bash command contains `echo|printf` AND `$VAR` or `${VAR}` where VAR contains `TOKEN|SECRET|PASSWORD|PASSWD|API[_-]?KEY|PRIVATE[_-]?KEY|ACCESS[_-]?KEY|CREDENTIAL|AUTH` (case-insensitive).

## Postconditions

1. Behavior: Block code `secret_var_echo`.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `echo $GITHUB_TOKEN` blocked; `echo $PATH` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 656 |
| **Audit ID** | `BC-AUDIT-1057` |
| **Source Line(s) (within hook)** | 175-183. |
