...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1428"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.039: validate-pr-merge-prerequisites: missing delivery dir is warn-only (graceful early pipeline)

## Description

validate-pr-merge-prerequisites: missing delivery dir is warn-only (graceful early pipeline). Stderr WARN "cannot find ..."; exit 0 (does not block — directory may not exist yet).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1130` (extracted from `pass-3-deep-hooks.md` line 1428).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-pr-merge-prerequisites.sh``.

## Preconditions

1. Trigger: Cannot find `.factory/code-delivery/<STORY>/`.

## Postconditions

1. Behavior: Stderr WARN "cannot find ..."; exit 0 (does not block — directory may not exist yet).
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1428 |
| **Audit ID** | `BC-AUDIT-1130` |
| **Source Line(s) (within hook)** | 70-76. |
