...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1005"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.090: verify-git-push: emits additionalContext on allowed pushes

## Description

verify-git-push: emits additionalContext on allowed pushes. Stdout `{"additionalContext": "Push requested. Ensure tests pass and code is reviewed before pushing."}`. Exit 0. Atypical for a PreToolUse hook to emit additionalContext (PostToolUse style), but Claude Code accepts it on either event.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1089` (extracted from `pass-3-deep-hooks.md` line 1005).
**Hook script:** ``plugins/vsdd-factory/hooks/verify-git-push.sh``.

## Preconditions

1. Trigger: All checks passed.

## Postconditions

1. Behavior: Stdout `{"additionalContext": "Push requested. Ensure tests pass and code is reviewed before pushing."}`. Exit 0. Atypical for a PreToolUse hook to emit additionalContext (PostToolUse style), but Claude Code accepts it on either event.
2. Exit codes: 0.
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/verify-git-push.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1005 |
| **Audit ID** | `BC-AUDIT-1089` |
| **Source Line(s) (within hook)** | 85-86. |
