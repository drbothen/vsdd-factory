...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:593"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.052: protect-bc: missing file or non-BC path → allow

## Description

protect-bc: missing file or non-BC path → allow. emit_allow → JSON `{"...permissionDecision":"allow"}`, exit 0.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1051` (extracted from `pass-3-deep-hooks.md` line 593).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-bc.sh``.

## Preconditions

1. Trigger: Path does not match BC pattern, or file does not exist (new file).

## Postconditions

1. Behavior: emit_allow → JSON `{"...permissionDecision":"allow"}`, exit 0.
2. Exit codes: 0.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | New BC file (not yet on disk) is allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 593 |
| **Audit ID** | `BC-AUDIT-1051` |
| **Source Line(s) (within hook)** | 49-55. |
