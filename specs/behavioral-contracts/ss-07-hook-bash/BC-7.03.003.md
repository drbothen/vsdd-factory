...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:64"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.003: block-ai-attribution: blocks Co-Authored-By with AI tool name

## Description

block-ai-attribution: blocks Co-Authored-By with AI tool name. Case-insensitive grep -E for `Co-Authored-By:.*Claude|Co-Authored-By:.*Anthropic|Co-Authored-By:.*GPT|Co-Authored-By:.*OpenAI|Co-Authored-By:.*Gemini|Co-Authored-By:.*Google AI`. On match emits `hook.block` event with reason `ai_attribution_coauthored`, prints to stderr, exits 2.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1002` (extracted from `pass-3-deep-hooks.md` line 64).
**Hook script:** ``plugins/vsdd-factory/hooks/block-ai-attribution.sh``.

## Preconditions

1. Trigger: Substring gate matched (BC-1001).

## Postconditions

1. Behavior: Case-insensitive grep -E for `Co-Authored-By:.*Claude|Co-Authored-By:.*Anthropic|Co-Authored-By:.*GPT|Co-Authored-By:.*OpenAI|Co-Authored-By:.*Gemini|Co-Authored-By:.*Google AI`. On match emits `hook.block` event with reason `ai_attribution_coauthored`, prints to stderr, exits 2.
2. Exit codes: 2 on match.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Commit command containing any `Co-Authored-By: Claude` line is rejected with exit 2 and AI ATTRIBUTION BLOCKED diagnostic. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/block-ai-attribution.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 64 |
| **Audit ID** | `BC-AUDIT-1002` |
| **Source Line(s) (within hook)** | 50-54. |
