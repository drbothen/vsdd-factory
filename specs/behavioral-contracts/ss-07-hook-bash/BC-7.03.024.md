...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:298"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.024: convergence-tracker: BLOCKS CONVERGENCE_REACHED with <3 consecutive clean passes

## Description

convergence-tracker: BLOCKS CONVERGENCE_REACHED with <3 consecutive clean passes. Walks sibling `pass-*.md` files in same dir (sorted descending by number), counts consecutive passes with 0 CRIT and 0 HIGH from the top. Block if streak < 3.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1023` (extracted from `pass-3-deep-hooks.md` line 298).
**Hook script:** ``plugins/vsdd-factory/hooks/convergence-tracker.sh``.

## Preconditions

1. Trigger: Verdict CONVERGENCE_REACHED AND no CRIT/HIGH errors AND novelty OK.

## Postconditions

1. Behavior: Walks sibling `pass-*.md` files in same dir (sorted descending by number), counts consecutive passes with 0 CRIT and 0 HIGH from the top. Block if streak < 3.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Pass-3 declares CONVERGENCE_REACHED but pass-2 had 1 HIGH → streak=1 → exit 2. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 298 |
| **Audit ID** | `BC-AUDIT-1023` |
| **Source Line(s) (within hook)** | 141-167. |
