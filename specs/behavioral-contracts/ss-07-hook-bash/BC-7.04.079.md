...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1816"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.079: validate-wave-gate-completeness: blocks gate report missing evidence for any of 6 gates

## Description

validate-wave-gate-completeness: blocks gate report missing evidence for any of 6 gates. Gate names: `Test Suite`, `DTU Validation`, `Adversarial Review`, `Demo Evidence`, `Holdout Evaluation`, `State Update`. Block (`wave_gate_incomplete`). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1170` (extracted from `pass-3-deep-hooks.md` line 1816).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-wave-gate-completeness.sh``.

## Preconditions

1. Trigger: For each gate 1-6, grep -iE for `Gate <N>[^0-9]|GATE_CHECK:.*gate=<N>`. Per missing gate, error.

## Postconditions

1. Behavior: Gate names: `Test Suite`, `DTU Validation`, `Adversarial Review`, `Demo Evidence`, `Holdout Evaluation`, `State Update`. Block (`wave_gate_incomplete`). Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Gate report covering only Gates 1, 3, 5 → error listing Gates 2, 4, 6. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-wave-gate-completeness.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1816 |
| **Audit ID** | `BC-AUDIT-1170` |
| **Source Line(s) (within hook)** | 104-124. |
