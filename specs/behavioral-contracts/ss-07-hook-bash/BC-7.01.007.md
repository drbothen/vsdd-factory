...
origin: brownfield
extracted_from: "pass-3-behavioral-contracts.md:458"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.01.007: track-agent-{start,stop} (PreToolUse:Agent / SubagentStop) records agent lifecycle

## Description

track-agent-{start,stop} (PreToolUse:Agent / SubagentStop) records agent lifecycle. Emits `agent.started` / `agent.stopped` event with `agent_id`, `subagent_type`.

**Source category:** Bash hook script contracts (broad-sweep).
**Audit ID:** `BC-AUDIT-069` (extracted from `pass-3-behavioral-contracts.md` line 458).

## Preconditions

1. Agent dispatched / completed.

## Postconditions

1. Emits `agent.started` / `agent.stopped` event with `agent_id`, `subagent_type`.
...
| **Source Document** | `pass-3-behavioral-contracts.md` line 458 |
| **Audit ID** | `BC-AUDIT-069` |
...
> `hooks/track-agent-{start,stop}.sh`.
