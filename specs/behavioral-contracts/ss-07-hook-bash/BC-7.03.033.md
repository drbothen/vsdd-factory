...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:392"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.033: destructive-command-guard: blocks `--no-verify` and `--no-gpg-sign`

## Description

destructive-command-guard: blocks `--no-verify` and `--no-gpg-sign`. Block codes `git_no_verify` and `git_no_gpg_sign`. Stops agent from bypassing the same hooks vsdd-factory ships.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1032` (extracted from `pass-3-deep-hooks.md` line 392).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: `git commit|merge|rebase|cherry-pick|am --no-verify` or `git commit --no-gpg-sign`.

## Postconditions

1. Behavior: Block codes `git_no_verify` and `git_no_gpg_sign`. Stops agent from bypassing the same hooks vsdd-factory ships.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `git commit --no-verify -m foo` blocked. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 392 |
| **Audit ID** | `BC-AUDIT-1032` |
| **Source Line(s) (within hook)** | 276-288. |
