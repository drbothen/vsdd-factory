...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:288"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.023: convergence-tracker: BLOCKS CONVERGENCE_REACHED when CRIT or HIGH count > 0

## Description

convergence-tracker: BLOCKS CONVERGENCE_REACHED when CRIT or HIGH count > 0. Block with separate ERROR per finding-class: "$CRIT_COUNT CRITICAL findings" and/or "$HIGH_COUNT HIGH findings".

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1022` (extracted from `pass-3-deep-hooks.md` line 288).
**Hook script:** ``plugins/vsdd-factory/hooks/convergence-tracker.sh``.

## Preconditions

1. Trigger: Verdict CONVERGENCE_REACHED AND CRITICAL count > 0 OR HIGH count > 0 (parsed from `| CRITICAL | N |` table).

## Postconditions

1. Behavior: Block with separate ERROR per finding-class: "$CRIT_COUNT CRITICAL findings" and/or "$HIGH_COUNT HIGH findings".
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | CONVERGENCE_REACHED + 1 HIGH = exit 2. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 288 |
| **Audit ID** | `BC-AUDIT-1022` |
| **Source Line(s) (within hook)** | 134-139. |
