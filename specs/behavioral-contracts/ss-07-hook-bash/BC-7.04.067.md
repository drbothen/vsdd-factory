...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1703"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.067: validate-template-compliance: blocks missing H2 sections vs template

## Description

validate-template-compliance: blocks missing H2 sections vs template. MISSING_SECTIONS `||`-joined; printed line-by-line on stderr. Block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1158` (extracted from `pass-3-deep-hooks.md` line 1703).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-template-compliance.sh``.

## Preconditions

1. Trigger: Template H2 sections (filtered via grep -vi `only|optional|recommended|conditional`) not present in file H2 sections (case-insensitive match, after stripping `(MANDATORY)` suffix).

## Postconditions

1. Behavior: MISSING_SECTIONS `||`-joined; printed line-by-line on stderr. Block.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | BC file missing `## Postconditions` (required H2 in template) → block. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1703 |
| **Audit ID** | `BC-AUDIT-1158` |
| **Source Line(s) (within hook)** | 159-176. |
