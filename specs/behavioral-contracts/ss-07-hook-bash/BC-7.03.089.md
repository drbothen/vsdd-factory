...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:995"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.089: verify-git-push: blocks pushes to main\

## Description

verify-git-push: blocks pushes to main|master|develop. Block code `git_push_protected` with hint to use feature branch + PR.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1088` (extracted from `pass-3-deep-hooks.md` line 995).
**Hook script:** ``plugins/vsdd-factory/hooks/verify-git-push.sh``.

## Preconditions

1. Trigger: Regex `git push <remote-name> (main|master|develop)($|space|colon)` matches.

## Postconditions

1. Behavior: Block code `git_push_protected` with hint to use feature branch + PR.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `git push origin main` blocked; `git push origin feature/STORY-001` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 995 |
| **Audit ID** | `BC-AUDIT-1088` |
| **Source Line(s) (within hook)** | 68-82. |
