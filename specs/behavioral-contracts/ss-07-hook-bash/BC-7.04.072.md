...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1750"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.072: validate-vp-consistency: blocks VP-INDEX summary totals != row counts

## Description

validate-vp-consistency: blocks VP-INDEX summary totals != row counts. Per tool, error "VP-INDEX declares X total = N but row count = M". Block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1163` (extracted from `pass-3-deep-hooks.md` line 1750).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-vp-consistency.sh``.

## Preconditions

1. Trigger: awk parses VP-INDEX rows by tool column ($5), normalizes labels to snake_case, counts. Awk also extracts Summary section declared totals. Per tool, if declared != actual row count → error.

## Postconditions

1. Behavior: Per tool, error "VP-INDEX declares X total = N but row count = M". Block.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1750 |
| **Audit ID** | `BC-AUDIT-1163` |
| **Source Line(s) (within hook)** | 95-141. |
