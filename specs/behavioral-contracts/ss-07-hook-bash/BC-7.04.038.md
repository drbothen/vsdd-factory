...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1418"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.038: validate-pr-merge-prerequisites: blocks when 3 evidence files missing

## Description

validate-pr-merge-prerequisites: blocks when 3 evidence files missing. For each missing file, append error + missing-files list. Emit hook.block (`pr_merge_evidence_missing`, fields story_id/delivery_dir/missing). Stderr "PR MERGE PREREQUISITES NOT MET". Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1129` (extracted from `pass-3-deep-hooks.md` line 1418).
...
1. Behavior: For each missing file, append error + missing-files list. Emit hook.block (`pr_merge_evidence_missing`, fields story_id/delivery_dir/missing). Stderr "PR MERGE PREREQUISITES NOT MET". Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Missing pr-review.md → block with "Missing: ... pr-review.md (PR review not conducted)". | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1418 |
| **Audit ID** | `BC-AUDIT-1129` |
| **Source Line(s) (within hook)** | 78-121. |
