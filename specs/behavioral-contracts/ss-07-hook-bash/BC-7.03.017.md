...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:219"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.017: capture-pr-activity: log directory resolution order

## Description

capture-pr-activity: log directory resolution order. Resolution priority: (1) `$VSDD_LOG_DIR` if set; (2) `<git-main-worktree>/.factory/logs`; (3) literal `.factory/logs`. Bounded by `git worktree list --porcelain` parsing.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1016` (extracted from `pass-3-deep-hooks.md` line 219).
**Hook script:** ``plugins/vsdd-factory/hooks/capture-pr-activity.sh``.

## Preconditions

1. Trigger: `_resolve_log_dir` called when computing duration on merge.

## Postconditions

1. Behavior: Resolution priority: (1) `$VSDD_LOG_DIR` if set; (2) `<git-main-worktree>/.factory/logs`; (3) literal `.factory/logs`. Bounded by `git worktree list --porcelain` parsing.
2. Exit codes: N/A helper.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Log dir resolution matches the priority order above. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 219 |
| **Audit ID** | `BC-AUDIT-1016` |
| **Source Line(s) (within hook)** | 65-77. |
