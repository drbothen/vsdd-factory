...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1310"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.027: validate-input-hash: warns on stored ≠ computed (advisory)

## Description

validate-input-hash: warns on stored ≠ computed (advisory). Stderr DRIFT warn — does NOT block. Exit 0.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1118` (extracted from `pass-3-deep-hooks.md` line 1310).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-input-hash.sh``.

## Preconditions

1. Trigger: Hash format valid; `bin/compute-input-hash` available; computed hash differs from stored.

## Postconditions

1. Behavior: Stderr DRIFT warn — does NOT block. Exit 0.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1310 |
| **Audit ID** | `BC-AUDIT-1118` |
| **Source Line(s) (within hook)** | 106-114. |
