...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:231"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.018: check-factory-commit: identity & registry binding (with semantic mismatch)

## Description

check-factory-commit: identity & registry binding (with semantic mismatch). **Registry/script mismatch:** script emits `additionalContext` JSON which is meaningful for PostToolUse (advisory context for next agent step) but registered as PreToolUse with `on_error=block`. See cross-walk section.
...
1. Hook event/tool match: PreToolUse on Bash (per registry)..
2. Trigger: PreToolUse on Bash.

## Postconditions

1. Behavior: **Registry/script mismatch:** script emits `additionalContext` JSON which is meaningful for PostToolUse (advisory context for next agent step) but registered as PreToolUse with `on_error=block`. See cross-walk section.
2. Exit codes: 0 always.
3. Error policy: block (registry) — but script never exits non-zero.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Cross-walk DRIFT noted: the script's documented intent (advisory after-the-fact reminder) does not match its registered binding (PreToolUse blocking band). | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/check-factory-commit.sh`` |
| **Confidence** | high (registry entry is authoritative); medium (script content). |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 231 |
| **Audit ID** | `BC-AUDIT-1017` |
| **Source Line(s) (within hook)** | Header line 2 says "PostToolUse hook"; registry says PreToolUse. |
