...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1349"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.031: validate-novelty-assessment: requires Pass / Novelty score / Verdict / Trajectory fields

## Description

validate-novelty-assessment: requires Pass / Novelty score / Verdict / Trajectory fields. One error per missing field; block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1122` (extracted from `pass-3-deep-hooks.md` line 1349).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-novelty-assessment.sh``.

## Preconditions

1. Trigger: Section exists. Checks: Pass field with `**Pass**`, `novelty score` substring, verdict matches `CONVERGENCE_REACHED|FINDINGS_REMAIN`, `trajectory` substring.

## Postconditions

1. Behavior: One error per missing field; block.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1349 |
| **Audit ID** | `BC-AUDIT-1122` |
| **Source Line(s) (within hook)** | 60-82. |
