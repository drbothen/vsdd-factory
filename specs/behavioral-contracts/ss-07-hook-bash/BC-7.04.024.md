...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1281"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.024: validate-input-hash: warns on missing/template/null hash

## Description

validate-input-hash: warns on missing/template/null hash. Stderr WARN "artifact has inputs: but no computed input-hash". Suggests `compute-input-hash <file> --update`. Exit 0 (advisory).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1115` (extracted from `pass-3-deep-hooks.md` line 1281).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-input-hash.sh``.

## Preconditions

1. Trigger: STORED_HASH empty OR `[md5]` OR `null` AND file has `inputs:` field.

## Postconditions

1. Behavior: Stderr WARN "artifact has inputs: but no computed input-hash". Suggests `compute-input-hash <file> --update`. Exit 0 (advisory).
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1281 |
| **Audit ID** | `BC-AUDIT-1115` |
| **Source Line(s) (within hook)** | 80-85. |
