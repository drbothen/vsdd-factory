...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1272"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.023: validate-input-hash: skips intentional placeholders [live-state] and [pending-recompute]

## Description

validate-input-hash: skips intentional placeholders [live-state] and [pending-recompute]. Exit 0 silently — these placeholders are documented escape hatches.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1114` (extracted from `pass-3-deep-hooks.md` line 1272).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-input-hash.sh``.

## Preconditions

1. Trigger: STORED_HASH equals one of `[live-state]` or `[pending-recompute]`.

## Postconditions

1. Behavior: Exit 0 silently — these placeholders are documented escape hatches.
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1272 |
| **Audit ID** | `BC-AUDIT-1114` |
| **Source Line(s) (within hook)** | 75-78. |
