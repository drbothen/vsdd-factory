...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1768"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.074: validate-vp-consistency: blocks VPs in coverage-matrix missing from VP-INDEX

## Description

validate-vp-consistency: blocks VPs in coverage-matrix missing from VP-INDEX. Error "VP X referenced in verification-coverage-matrix.md but not in VP-INDEX". Block.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1165` (extracted from `pass-3-deep-hooks.md` line 1768).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-vp-consistency.sh``.

## Preconditions

1. Trigger: VP referenced in coverage-matrix not in VP-INDEX.

## Postconditions

1. Behavior: Error "VP X referenced in verification-coverage-matrix.md but not in VP-INDEX". Block.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1768 |
| **Audit ID** | `BC-AUDIT-1165` |
| **Source Line(s) (within hook)** | 228-234. |
