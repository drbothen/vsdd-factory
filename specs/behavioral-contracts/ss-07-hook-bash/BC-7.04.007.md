...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1102"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.007: validate-bc-title: blocks when BC H1 != BC-INDEX row title

## Description

validate-bc-title: blocks when BC H1 != BC-INDEX row title. If H1 title != BC-INDEX title (and both non-empty), emit hook.block (`policy7_bc_title_mismatch`) with `bc_id`, `h1_title`, `index_title`. Stderr "POLICY 7 VIOLATION".

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1098` (extracted from `pass-3-deep-hooks.md` line 1102).
...
1. Behavior: If H1 title != BC-INDEX title (and both non-empty), emit hook.block (`policy7_bc_title_mismatch`) with `bc_id`, `h1_title`, `index_title`. Stderr "POLICY 7 VIOLATION".
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | BC file H1 says `Plugin compiles to wasm32-wasip2`, BC-INDEX row says `Plugin compiles wasm` → block. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-bc-title.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1102 |
| **Audit ID** | `BC-AUDIT-1098` |
| **Source Line(s) (within hook)** | 45-98. |
