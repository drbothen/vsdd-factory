...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1218"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.018: validate-finding-format: blocks legacy ADV-NNN and ADV-PN-NNN formats

## Description

validate-finding-format: blocks legacy ADV-NNN and ADV-PN-NNN formats. For each legacy match, error with the offending ID + correct format. Block with reason `finding_id_legacy_format`. Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1109` (extracted from `pass-3-deep-hooks.md` line 1218).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-finding-format.sh``.

## Preconditions

1. Trigger: File path is adversarial-review/pass-/FIX-/fix-. Greps for `ADV-[A-Z0-9_-]+` tokens; current format is `ADV-<CYCLE>-P[N]-[SEV]-NNN`; legacy formats include `ADV-NNN` (just digits) or `ADV-P[N]-NNN` (no cycle/severity).

## Postconditions

1. Behavior: For each legacy match, error with the offending ID + correct format. Block with reason `finding_id_legacy_format`. Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Bare `ADV-<digits>` IDs flagged; `ADV-P1CONV-P03-CRIT-001` allowed. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-finding-format.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1218 |
| **Audit ID** | `BC-AUDIT-1109` |
| **Source Line(s) (within hook)** | 47-64. |
