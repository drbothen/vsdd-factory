...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:628"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.055: protect-secrets: blocks shell content reads of .env files

## Description

protect-secrets: blocks shell content reads of .env files. Block code `env_file_read_shell`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1054` (extracted from `pass-3-deep-hooks.md` line 628).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-secrets.sh``.

## Preconditions

1. Trigger: Tool == Bash AND command contains one of `cat|less|more|head|tail|bat|xxd|od|strings|grep|awk|sed` AND `command_references_secret_env` returns true (any `.env*` token that is not `.example|.sample|.template`).

## Postconditions

1. Behavior: Block code `env_file_read_shell`.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `cat .env` blocked; `cat .env.example` allowed. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 628 |
| **Audit ID** | `BC-AUDIT-1054` |
| **Source Line(s) (within hook)** | 113-126. |
