...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:583"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.051: protect-bc: jq-missing fail-closed (exit 1)

## Description

protect-bc: jq-missing fail-closed (exit 1). Stderr "jq is required but not found"; exit 1. With on_error=block this fails the edit.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1050` (extracted from `pass-3-deep-hooks.md` line 583).
...
1. Behavior: Stderr "jq is required but not found"; exit 1. With on_error=block this fails the edit.
2. Exit codes: 1.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Hook treats jq-missing as a block-able error rather than degrading silently — different from most hooks. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 583 |
| **Audit ID** | `BC-AUDIT-1050` |
| **Source Line(s) (within hook)** | 14-17. |
