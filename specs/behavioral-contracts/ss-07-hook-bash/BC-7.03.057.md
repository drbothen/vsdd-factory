...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:647"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.057: protect-secrets: blocks tar/zip including .env

## Description

protect-secrets: blocks tar/zip including .env. Block code `env_file_archive`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1056` (extracted from `pass-3-deep-hooks.md` line 647).
**Hook script:** ``plugins/vsdd-factory/hooks/protect-secrets.sh``.

## Preconditions

1. Trigger: Bash command contains `tar|zip` AND mentions a non-template `.env*`.

## Postconditions

1. Behavior: Block code `env_file_archive`.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 647 |
| **Audit ID** | `BC-AUDIT-1056` |
| **Source Line(s) (within hook)** | 163-171. |
