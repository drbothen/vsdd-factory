...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:268"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.021: convergence-tracker: enforces trajectory monotonicity (warn-only)

## Description

convergence-tracker: enforces trajectory monotonicity (warn-only). Parses arrow-delimited number list; if any later number > earlier number, prints WARNING ("Trajectory monotonicity violation — finding count increased"). Does NOT block — appends to WARNINGS variable only.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1020` (extracted from `pass-3-deep-hooks.md` line 268).
**Hook script:** ``plugins/vsdd-factory/hooks/convergence-tracker.sh``.

## Preconditions

1. Trigger: File contains a `Trajectory` field with `→` separator.

## Postconditions

1. Behavior: Parses arrow-delimited number list; if any later number > earlier number, prints WARNING ("Trajectory monotonicity violation — finding count increased"). Does NOT block — appends to WARNINGS variable only.
2. Exit codes: 0 (warning path).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Trajectory `29→24→7→9` produces a WARN; `29→24→7→4` does not. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 268 |
| **Audit ID** | `BC-AUDIT-1020` |
| **Source Line(s) (within hook)** | 100-119. |
