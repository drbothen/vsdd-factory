...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1475"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.044: validate-pr-review-posted: blocks when no formal review posted

## Description

validate-pr-review-posted: blocks when no formal review posted. Errors stack; emit hook.block (`pr_review_not_posted`).

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1135` (extracted from `pass-3-deep-hooks.md` line 1475).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-pr-review-posted.sh``.

## Preconditions

1. Trigger: Result does not mention `gh pr review|pr review.*posted|review.*posted.*GitHub|APPROVE|REQUEST_CHANGES`. AND if `gh pr review` mentioned, must include `approve|request-changes|APPROVE|REQUEST_CHANGES`.

## Postconditions

1. Behavior: Errors stack; emit hook.block (`pr_review_not_posted`).
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Result with `gh pr review` but no `--approve\|--request-changes` → blocked. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 1475 |
| **Audit ID** | `BC-AUDIT-1135` |
| **Source Line(s) (within hook)** | 48-56. |
