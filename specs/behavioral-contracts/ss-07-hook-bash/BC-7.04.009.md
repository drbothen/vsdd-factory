...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1125"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.009: validate-changelog-monotonicity: scopes to .factory/*.md, skips known no-changelog files

## Description

validate-changelog-monotonicity: scopes to .factory/*.md, skips known no-changelog files

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1100` (extracted from `pass-3-deep-hooks.md` line 1125).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-changelog-monotonicity.sh``.

## Preconditions

1. Trigger: file_path matches `*.factory/*.md` AND not STATE.md/INDEX.md/burst-log/convergence-trajectory/session-checkpoint/lessons; AND file contains `## Changelog` heading.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1125 |
| **Audit ID** | `BC-AUDIT-1100` |
| **Source Line(s) (within hook)** | 35-48. |
