...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1527"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.049: validate-state-pin-freshness: blocks when STATE pin != artifact frontmatter version (5 fields)

## Description

validate-state-pin-freshness: blocks when STATE pin != artifact frontmatter version (5 fields). For each pair, if STATE.md cites a non-empty version AND artifact exists AND its frontmatter `version:` differs (after stripping `v` prefix), error "field: STATE.md cites 'X' but Y has version 'Z'". Block (`state_version_pin_drift`). Exit 2.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1140` (extracted from `pass-3-deep-hooks.md` line 1527).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-state-pin-freshness.sh``.

## Preconditions

1. Trigger: Path is STATE.md. VERSION_PAIRS list: `bc_index_version → specs/behavioral-contracts/BC-INDEX.md`, `story_index_version → stories/STORY-INDEX.md`, `vp_index_version → specs/verification-properties/VP-INDEX.md`, `test_vectors_version → specs/prd-supplements/test-vectors.md`, `arch_index_version → specs/architecture/ARCH-INDEX.md`.

## Postconditions

1. Behavior: For each pair, if STATE.md cites a non-empty version AND artifact exists AND its frontmatter `version:` differs (after stripping `v` prefix), error "field: STATE.md cites 'X' but Y has version 'Z'". Block (`state_version_pin_drift`). Exit 2.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | STATE `bc_index_version: 1.5.0` + BC-INDEX.md frontmatter `version: 1.6.0` → block. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-state-pin-freshness.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1527 |
| **Audit ID** | `BC-AUDIT-1140` |
| **Source Line(s) (within hook)** | 41-95. |
