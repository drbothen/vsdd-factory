...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:430"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.037: destructive-command-guard: blocks recursive chmod/chown on protected paths

## Description

destructive-command-guard: blocks recursive chmod/chown on protected paths. Block code `recursive_permission_change`. Cites risk of breaking git metadata and hook executability.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1036` (extracted from `pass-3-deep-hooks.md` line 430).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: `chmod|chown -R|--recursive` AND command mentions `.factory`, `src/`, `tests/`, or `.git/`.

## Postconditions

1. Behavior: Block code `recursive_permission_change`. Cites risk of breaking git metadata and hook executability.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 430 |
| **Audit ID** | `BC-AUDIT-1036` |
| **Source Line(s) (within hook)** | 344-354. |
