...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:746"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.066: red-gate: skips test files always

## Description

red-gate: skips test files always. Exit 0 (test-first is the whole point).

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1065` (extracted from `pass-3-deep-hooks.md` line 746).
**Hook script:** ``plugins/vsdd-factory/hooks/red-gate.sh``.

## Preconditions

1. Trigger: file_path under `tests/`, `__tests__/`, `spec/`, or matches `*.test.<ext>`, `*.spec.<ext>`, `*_test.rs|go|py`.

## Postconditions

1. Behavior: Exit 0 (test-first is the whole point).
2. Exit codes: 0.
...
| **Source Document** | `pass-3-deep-hooks.md` line 746 |
| **Audit ID** | `BC-AUDIT-1065` |
| **Source Line(s) (within hook)** | 38-44. |
