...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1548"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.051: validate-state-size: warns at >200 lines, blocks at >500 unless reducing

## Description

validate-state-size: warns at >200 lines, blocks at >500 unless reducing. If LINE_COUNT < PRIOR_COUNT → exit 0 (compaction allowed unconditionally). If >500 lines AND not reducing → emit hook.block (`state_bloat`, line_count, limit=500). Stderr STATE.md BLOAT — BLOCKED with hint to run `/vsdd-factory:compact-state`. Exit 2. If 200-500 → stderr WARN, exit 0.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1142` (extracted from `pass-3-deep-hooks.md` line 1548).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-state-size.sh``.

## Preconditions

1. Trigger: Path ends in `.factory/STATE.md`. Counts wc -l. Compares against `git -C $PARENT_DIR show HEAD:STATE.md` line count.

## Postconditions

1. Behavior: If LINE_COUNT < PRIOR_COUNT → exit 0 (compaction allowed unconditionally). If >500 lines AND not reducing → emit hook.block (`state_bloat`, line_count, limit=500). Stderr STATE.md BLOAT — BLOCKED with hint to run `/vsdd-factory:compact-state`. Exit 2. If 200-500 → stderr WARN, exit 0.
2. Exit codes: 0 (≤500 or shrinking) / 2 (>500 and growing).
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | STATE.md grew from 480 to 510 → block; STATE.md compacted from 510 to 100 → allow. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/validate-state-size.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 1548 |
| **Audit ID** | `BC-AUDIT-1142` |
| **Source Line(s) (within hook)** | 43-74. |
