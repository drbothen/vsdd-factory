...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:244"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.019: check-factory-commit: emits additionalContext when .factory commit lacks STATE.md

## Description

check-factory-commit: emits additionalContext when .factory commit lacks STATE.md. Emits JSON `{"additionalContext": "Factory artifact committed but STATE.md was not updated. ..."}` to stdout. On all other paths emits `{}`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1018` (extracted from `pass-3-deep-hooks.md` line 244).
**Hook script:** ``plugins/vsdd-factory/hooks/check-factory-commit.sh``.

## Preconditions

1. Trigger: Bash command contains `git commit` AND contains `.factory` or `cd .factory` AND does NOT contain `STATE.md`.

## Postconditions

1. Behavior: Emits JSON `{"additionalContext": "Factory artifact committed but STATE.md was not updated. ..."}` to stdout. On all other paths emits `{}`.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | `git commit -m "..." -- .factory/specs/foo.md` produces additionalContext nudge; `git commit -- .factory/STATE.md other.md` produces `{}`. | happy-path |
...
| **Source Document** | `pass-3-deep-hooks.md` line 244 |
| **Audit ID** | `BC-AUDIT-1018` |
| **Source Line(s) (within hook)** | 7-22. |
