...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:382"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.03.032: destructive-command-guard: blocks irreversible git operations (8 patterns)

## Description

destructive-command-guard: blocks irreversible git operations (8 patterns). Block codes: `git_reset_hard`, `git_clean_force`, `git_checkout_dot`, `git_restore_dot`, `git_stash_discard`, `git_branch_d_protected`, `git_filter_history`, `git_reflog_expire`, `git_gc_prune_now`, `git_worktree_force`.

**Source category:** Routing hooks (PreToolUse, PostToolUse, lifecycle).
**Audit ID:** `BC-AUDIT-1031` (extracted from `pass-3-deep-hooks.md` line 382).
**Hook script:** ``plugins/vsdd-factory/hooks/destructive-command-guard.sh``.

## Preconditions

1. Trigger: Each of: `git reset --hard`, `git clean -f|--force` (allowing `-n|--dry-run`), `git checkout -- .`, `git restore .`, `git stash drop|clear`, `git branch -D main|master|develop`, `git filter-branch|filter-repo`, `git reflog expire --expire=now`, `git gc --prune=now`, `git worktree remove --force` outside `.worktrees/`.

## Postconditions

1. Behavior: Block codes: `git_reset_hard`, `git_clean_force`, `git_checkout_dot`, `git_restore_dot`, `git_stash_discard`, `git_branch_d_protected`, `git_filter_history`, `git_reflog_expire`, `git_gc_prune_now`, `git_worktree_force`.
2. Exit codes: 2.
...
|-------|-----------------|----------|
| Acceptance criterion (see below) | Each pattern reproduces a unique block code. | happy-path |
...
| Property | Value |
|----------|-------|
| **Path** | ``plugins/vsdd-factory/hooks/destructive-command-guard.sh`` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Document** | `pass-3-deep-hooks.md` line 382 |
| **Audit ID** | `BC-AUDIT-1031` |
| **Source Line(s) (within hook)** | 178-272. |
