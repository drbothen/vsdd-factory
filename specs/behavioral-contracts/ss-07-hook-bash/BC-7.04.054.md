...
origin: brownfield
extracted_from: "pass-3-deep-hooks.md:1580"
subsystem: "SS-07"
capability: "TBD"
...
# Behavioral Contract BC-7.04.054: validate-story-bc-sync: blocks frontmatter BC missing AC trace annotation

## Description

validate-story-bc-sync: blocks frontmatter BC missing AC trace annotation. Error per missing BC trace.

**Source category:** Validator hook scripts (validate-* and verify-*).
**Audit ID:** `BC-AUDIT-1145` (extracted from `pass-3-deep-hooks.md` line 1580).
**Hook script:** ``plugins/vsdd-factory/hooks/validate-story-bc-sync.sh``.

## Preconditions

1. Trigger: BC in frontmatter array but no `traces to BC-X.YY.ZZZ` annotation references it.

## Postconditions

1. Behavior: Error per missing BC trace.
2. Exit codes: 2.
...
| **Source Document** | `pass-3-deep-hooks.md` line 1580 |
| **Audit ID** | `BC-AUDIT-1145` |
| **Source Line(s) (within hook)** | 97-105. |
