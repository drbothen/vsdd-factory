...
origin: brownfield
extracted_from: "pass-3-deep-rust-tests.md:1022"
subsystem: "SS-03"
capability: "TBD"
...
# Behavioral Contract BC-3.02.016: sink-file::config_deserializes_from_toml: minimal TOML config parses with queue_depth defaulting

> Section: File sink behavior
> Source BC (audit ID): BC-AUDIT-2383

## Description

Given Minimal TOML `name="local-events", enabled=true, path_template="..."`. When Parsed via `toml::from_str::<FileSinkConfig>`.. Then `cfg.queue_depth == DEFAULT_QUEUE_DEPTH (1000)`.

## Preconditions

1. Minimal TOML `name="local-events", enabled=true, path_template="..."`.

## Postconditions

1. `cfg.queue_depth == DEFAULT_QUEUE_DEPTH (1000)`.

## Invariants

1. Operators omit queue_depth and get the documented default.
...
|-------|----------------|----------|
| Minimal TOML `name="local-events", enabled=true, path_template="..."`. | `cfg.queue_depth == DEFAULT_QUEUE_DEPTH (1000)`. | happy-path |
| TBD | TBD | edge-case |
| TBD | TBD | error |
...
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-2383 |
| **Pass-3 source** | `pass-3-deep-rust-tests.md:1022` |
| **Extraction Date** | 2026-04-25 |
| **Pinned test** | ``crates/sink-file/src/lib.rs::tests::config_deserializes_from_toml`` |
| **Test type** | unit |
