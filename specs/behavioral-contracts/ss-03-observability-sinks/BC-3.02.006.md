...
origin: brownfield
extracted_from: "pass-3-deep-rust-tests.md:912"
subsystem: "SS-03"
capability: "TBD"
...
# Behavioral Contract BC-3.02.006: sink-file::template_unbalanced_brace_treated_literally: opening `{` without closing `}` is treate

> Section: File sink behavior
> Source BC (audit ID): BC-AUDIT-2373

## Description

Given Template `weird-{date-only.jsonl` (no closing brace). When Resolved.. Then Returns `weird-{date-only.jsonl` literally (no substitution attempted on unclosed brace).

## Preconditions

1. Template `weird-{date-only.jsonl` (no closing brace).

## Postconditions

1. Returns `weird-{date-only.jsonl` literally (no substitution attempted on unclosed brace).

## Invariants

1. Operator typos in templates produce a recognizable filename rather than a parse error or partial substitution.
...
|-------|----------------|----------|
| Template `weird-{date-only.jsonl` (no closing brace). | Returns `weird-{date-only.jsonl` literally (no substitution attempted on unclosed brace). | happy-path |
| TBD | TBD | edge-case |
| TBD | TBD | error |
...
| **Confidence** | high |
| **Original audit ID** | BC-AUDIT-2373 |
| **Pass-3 source** | `pass-3-deep-rust-tests.md:912` |
| **Extraction Date** | 2026-04-25 |
| **Pinned test** | ``crates/sink-file/src/lib.rs::tests::template_unbalanced_brace_treated_literally`` |
| **Test type** | unit |
