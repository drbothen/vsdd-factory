...
| EC-011 | `precompact_flush_sha: null` in HANDOFF.md but `.factory/hooks/precompact-flush-log` last line contains a valid commit SHA (hard block determined by FIELD-4 of the last precompact-flush-log line equalling `commit` per PC5's FIELD-4 corroboration method — not by a live `git cat-file` call) | HARD BLOCK: `PrecompactShaMismatch`; operator must populate `precompact_flush_sha` with the SHA (FIELD-2) from the last line of the log |
