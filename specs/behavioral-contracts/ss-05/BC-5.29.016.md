...
1. Step `phase-0-human-approval (line 169; lobster carve-out: stable anchor is step name `phase-0-human-approval`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-0-human-approval`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-0-human-approval (line 169; lobster carve-out: stable anchor is step name `phase-0-human-approval`, not line number)` | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-0-human-approval (line 169; lobster carve-out: stable anchor is step name `phase-0-human-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 974; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1555` |
| **Confidence** | high |
...
- **Step:** phase-0-human-approval (line 169; lobster carve-out: stable anchor is step name `phase-0-human-approval`, not line number)
- **Type:** human-approval
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-0-human-approval (line 169)`). Stable anchor is step name `phase-0-human-approval`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-0-human-approval`. |
