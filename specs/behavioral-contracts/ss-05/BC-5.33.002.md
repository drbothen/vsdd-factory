...
# Behavioral Contract BC-5.33.002: maintenance: entry-point

## Description

Entry-point step `load-config` (line 27; lobster carve-out: stable anchor is step name `load-config`, not line number) with `depends_on: []`. Agent: orchestrator.
...
- BC-5.33.006 — load-config (per-step)
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/maintenance.lobster` (lines 27; lobster path carve-out: line range is unstable as lobster files evolve) |
| **Confidence** | high |
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — desc step `load-config` (line 27); lobster path lines 27
