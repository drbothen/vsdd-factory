...
1. Step `verification-convergence (line 59; lobster carve-out: stable anchor is step name `verification-convergence`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `verification-convergence`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `verification-convergence (line 59; lobster carve-out: stable anchor is step name `verification-convergence`, not line number)` | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `verification-convergence (line 59; lobster carve-out: stable anchor is step name `verification-convergence`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 623; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1441` |
| **Confidence** | high |
...
- **Step:** verification-convergence (line 59; lobster carve-out: stable anchor is step name `verification-convergence`, not line number)
- **Type:** skill
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`verification-convergence (line 59)`). Stable anchor is step name `verification-convergence`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `verification-convergence`. |
