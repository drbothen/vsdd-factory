...
# Behavioral Contract BC-5.27.023: phase-7:visual-review

## Description

Workflow contract: phase-7:visual-review.
...
1. Step `visual-review (line 151; lobster carve-out: stable anchor is step name `visual-review`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `visual-review`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `visual-review (line 151; lobster carve-out: stable anchor is step name `visual-review`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1452 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `visual-review (line 151; lobster carve-out: stable anchor is step name `visual-review`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 658; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1452` |
| **Confidence** | high |
...
- **Step:** visual-review (line 151; lobster carve-out: stable anchor is step name `visual-review`, not line number)
- **Type:** agent
- **Agent:** visual-reviewer
- **Condition:** `config.demo_recording.enabled != false`
- **Source line(s) in workflow YAML:** 151-160
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`visual-review (line 151)`). Stable anchor is step name `visual-review`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `visual-review`. |
