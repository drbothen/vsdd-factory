...
# Behavioral Contract BC-5.21.003: phase-1: terminal-step

## Description

Final approval over `prd.md`, `architecture/`, `verification-properties/`, `behavioral-contracts/`, `ux-spec.md`, `module-criticality.md`.
...
1. Final approval over `prd.md`, `architecture/`, `verification-properties/`, `behavioral-contracts/`, `ux-spec.md`, `module-criticality.md`.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``human-approval` (line 148; lobster carve-out: stable anchor is step name `human-approval`, not line number) — depends on `[input-hash-drift-check]`; 24h timeout` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1322 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``human-approval` (line 148; lobster carve-out: stable anchor is step name `human-approval`, not line number) — depends on `[input-hash-drift-check]`; 24h timeout`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 212; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1322` |
| **Confidence** | high |
...
- **Step:** `human-approval` (line 148; lobster carve-out: stable anchor is step name `human-approval`, not line number) — depends on `[input-hash-drift-check]`; 24h timeout
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`human-approval (line 148)`). Stable anchor is step name `human-approval`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — **Step:** bt form: step `human-approval` (line 148)
