...
# Behavioral Contract BC-5.26.008: phase-6:fuzz-testing

## Description

Workflow contract: phase-6:fuzz-testing.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-kani-proofs]`.

## Postconditions

1. Step `fuzz-testing (line 33; lobster carve-out: stable anchor is step name `fuzz-testing`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `fuzz-testing`; line number is unstable as lobster files evolve -->

## Invariants

1. Step execution must complete within timeout: 2h.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `fuzz-testing (line 33; lobster carve-out: stable anchor is step name `fuzz-testing`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1417 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `fuzz-testing (line 33; lobster carve-out: stable anchor is step name `fuzz-testing`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 562; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1417` |
| **Confidence** | high |
...
- **Step:** fuzz-testing (line 33; lobster carve-out: stable anchor is step name `fuzz-testing`, not line number)
- **Type:** skill
- **Skill:** `formal-verify/steps/step-b-fuzz-testing.md`
- **Depends on:** `[backup-kani-proofs]`
- **Timeout:** 2h
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`fuzz-testing (line 33)`). Stable anchor is step name `fuzz-testing`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `fuzz-testing`. |
