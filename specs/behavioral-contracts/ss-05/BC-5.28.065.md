...
# Behavioral Contract BC-5.28.065: greenfield:phase-6-ui-quality-gate

## Description

Workflow contract: greenfield:phase-6-ui-quality-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-6-heuristic-evaluation, phase-6-ui-completeness-final, phase-6-responsive-final]`.
2. Conditional gate evaluates true: `same UI`.

## Postconditions

1. Step `phase-6-ui-quality-gate (line 1229; lobster carve-out: stable anchor is step name `phase-6-ui-quality-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-6-ui-quality-gate`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `same UI` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-6-ui-quality-gate (line 1229; lobster carve-out: stable anchor is step name `phase-6-ui-quality-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1524 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-6-ui-quality-gate (line 1229; lobster carve-out: stable anchor is step name `phase-6-ui-quality-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 872; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1524` |
| **Confidence** | high |
...
- **Step:** phase-6-ui-quality-gate (line 1229; lobster carve-out: stable anchor is step name `phase-6-ui-quality-gate`, not line number)
- **Type:** skill
- **Skill:** `skills/ui-quality-gate/SKILL.md`
- **Depends on:** `[phase-6-heuristic-evaluation, phase-6-ui-completeness-final, phase-6-responsive-final]`
- **Condition:** same UI
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-6-ui-quality-gate (line 1229)`). Stable anchor is step name `phase-6-ui-quality-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-6-ui-quality-gate`. |
