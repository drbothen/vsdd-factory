...
# Behavioral Contract BC-5.29.017: brownfield:post-phase-0-routing

## Description

Routes per human choice: 1) feature in mind → continue, 2) help me figure out → planning.lobster, 3) nothing → park (STATE.md `phase: 0-complete, awaiting: feature-request`).

## Preconditions

1. Upstream dependencies completed successfully: `[phase-0-human-approval]`.

## Postconditions

1. Routes per human choice: 1) feature in mind → continue, 2) help me figure out → planning.lobster, 3) nothing → park (STATE.md `phase: 0-complete, awaiting: feature-request`).
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `post-phase-0-routing (line 194; lobster carve-out: stable anchor is step name `post-phase-0-routing`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1556 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `post-phase-0-routing (line 194; lobster carve-out: stable anchor is step name `post-phase-0-routing`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 977; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1556` |
| **Confidence** | high |
...
- **Step:** post-phase-0-routing (line 194; lobster carve-out: stable anchor is step name `post-phase-0-routing`, not line number)
- **Type:** agent
- **Agent:** orchestrator
- **Depends on:** `[phase-0-human-approval]`
- **Source line(s) in workflow YAML:** 194-209
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`post-phase-0-routing (line 194)`). Stable anchor is step name `post-phase-0-routing`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `post-phase-0-routing`. |
