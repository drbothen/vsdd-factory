...
# Behavioral Contract BC-5.29.001: brownfield-vsdd: identity

## Description

v3.0.0. VSDD pipeline for existing codebases. Adds Phase 0 (Codebase Ingestion) before greenfield. After Phase 0 + human approval, routes through post-Phase 0 routing → market intelligence → cross-language porting detection (semport) → design-system extraction → greenfield sub-workflow → optional multi-repo handoff.
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1540 |
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 922; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1540` |
| **Confidence** | high |
...
- **Source line(s) in workflow YAML:** 1-22
- **Behavior:** v3.0.0. VSDD pipeline for existing codebases. Adds Phase 0 (Codebase Ingestion) before greenfield. After Phase 0 + human approval, routes through post-Phase 0 routing → market intelligence → cross-language porting detection (semport) → design-system extraction → greenfield sub-workflow → optional multi-repo handoff.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — source-doc line 922
