...
# Behavioral Contract BC-5.30.038: feature:phase-f2-gene-transfusion-assessment

## Description

Workflow contract: feature:phase-f2-gene-transfusion-assessment.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f2-spec-evolution]`.
2. Conditional gate evaluates true: `standard + `feature.has_reference_impl == true`.

## Postconditions

1. Step `phase-f2-gene-transfusion-assessment (line 482; lobster carve-out: stable anchor is step name `phase-f2-gene-transfusion-assessment`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f2-gene-transfusion-assessment`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `standard + `feature.has_reference_impl == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f2-gene-transfusion-assessment (line 482; lobster carve-out: stable anchor is step name `phase-f2-gene-transfusion-assessment`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1617 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f2-gene-transfusion-assessment (line 482; lobster carve-out: stable anchor is step name `phase-f2-gene-transfusion-assessment`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1152; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1617` |
| **Confidence** | high |
...
- **Step:** phase-f2-gene-transfusion-assessment (line 482; lobster carve-out: stable anchor is step name `phase-f2-gene-transfusion-assessment`, not line number)
- **Type:** agent
- **Agent:** architect
- **Depends on:** `[phase-f2-spec-evolution]`
- **Condition:** standard + `feature.has_reference_impl == true`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f2-gene-transfusion-assessment (line 482)`). Stable anchor is step name `phase-f2-gene-transfusion-assessment`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f2-gene-transfusion-assessment`. |
