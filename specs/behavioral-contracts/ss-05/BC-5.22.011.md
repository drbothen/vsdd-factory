...
# Behavioral Contract BC-5.22.011: phase-2:backup-dependency-graph

## Description

Workflow contract: phase-2:backup-dependency-graph.
...
1. Step `backup-dependency-graph (line 54; lobster carve-out: stable anchor is step name `backup-dependency-graph`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `backup-dependency-graph`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `backup-dependency-graph (line 54; lobster carve-out: stable anchor is step name `backup-dependency-graph`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1350 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `backup-dependency-graph (line 54; lobster carve-out: stable anchor is step name `backup-dependency-graph`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 362; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1350` |
| **Confidence** | high |
...
- **Step:** backup-dependency-graph (line 54; lobster carve-out: stable anchor is step name `backup-dependency-graph`, not line number)
- **Agent:** state-manager
- **Source line(s) in workflow YAML:** 54-60
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`backup-dependency-graph (line 54)`). Stable anchor is step name `backup-dependency-graph`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `backup-dependency-graph`. |
