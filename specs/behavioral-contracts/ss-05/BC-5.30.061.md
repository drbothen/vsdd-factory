...
# Behavioral Contract BC-5.30.061: feature:holdout-gate

## Description

Workflow contract: feature:holdout-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[holdout-evaluation]`.

## Postconditions

1. Step `holdout-gate (line 1053; lobster carve-out: stable anchor is step name `holdout-gate`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `holdout-gate (line 1053; lobster carve-out: stable anchor is step name `holdout-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1640 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `holdout-gate (line 1053; lobster carve-out: stable anchor is step name `holdout-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1222; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1640` |
| **Confidence** | high |
...
- **Step:** holdout-gate (line 1053; lobster carve-out: stable anchor is step name `holdout-gate`, not line number)
- **Type:** gate
- **Depends on:** `[holdout-evaluation]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`holdout-gate (line 1053)`). Stable anchor is step name `holdout-gate`.
