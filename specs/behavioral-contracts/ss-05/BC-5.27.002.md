...
# Behavioral Contract BC-5.27.002: phase-7: entry-point

## Description

Workflow contract: phase-7: entry-point.
...
1. Step ``spec-convergence` (line 17; lobster carve-out: stable anchor is step name `spec-convergence`, not line number) — `depends_on: []`.` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `spec-convergence`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``spec-convergence` (line 17; lobster carve-out: stable anchor is step name `spec-convergence`, not line number) — `depends_on: []`.` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1431 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``spec-convergence` (line 17; lobster carve-out: stable anchor is step name `spec-convergence`, not line number) — `depends_on: []`.`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 593; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1431` |
| **Confidence** | high |
...
- **Step:** `spec-convergence` (line 17; lobster carve-out: stable anchor is step name `spec-convergence`, not line number) — `depends_on: []`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``spec-convergence` (line 17)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `spec-convergence`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `spec-convergence`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `spec-convergence`. |
