...
# Behavioral Contract BC-5.30.049: feature:phase-f3-incremental-stories

## Description

Workflow contract: feature:phase-f3-incremental-stories.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f2-human-approval]`.

## Postconditions

1. Step `phase-f3-incremental-stories (line 657; lobster carve-out: stable anchor is step name `phase-f3-incremental-stories`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f3-incremental-stories`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f3-incremental-stories (line 657; lobster carve-out: stable anchor is step name `phase-f3-incremental-stories`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1628 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f3-incremental-stories (line 657; lobster carve-out: stable anchor is step name `phase-f3-incremental-stories`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1185; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1628` |
| **Confidence** | high |
...
- **Step:** phase-f3-incremental-stories (line 657; lobster carve-out: stable anchor is step name `phase-f3-incremental-stories`, not line number)
- **Type:** skill
- **Skill:** `skills/phase-f3-incremental-stories/SKILL.md`
- **Depends on:** `[phase-f2-human-approval]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f3-incremental-stories (line 657)`). Stable anchor is step name `phase-f3-incremental-stories`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f3-incremental-stories`. |
