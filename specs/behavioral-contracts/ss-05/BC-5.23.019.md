...
# Behavioral Contract BC-5.23.019: phase-3:backup-cleanup

## Description

Updates STATE.md and `sprint-state.yaml` with story status `completed`.
...
1. Updates STATE.md and `sprint-state.yaml` with story status `completed`.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `backup-cleanup (line 111; lobster carve-out: stable anchor is step name `backup-cleanup`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1378 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `backup-cleanup (line 111; lobster carve-out: stable anchor is step name `backup-cleanup`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 456; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1378` |
| **Confidence** | high |
...
- **Step:** backup-cleanup (line 111; lobster carve-out: stable anchor is step name `backup-cleanup`, not line number)
- **Agent:** state-manager
- **Source line(s) in workflow YAML:** 111-117
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`backup-cleanup (line 111)`). Stable anchor is step name `backup-cleanup`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `backup-cleanup`. |
