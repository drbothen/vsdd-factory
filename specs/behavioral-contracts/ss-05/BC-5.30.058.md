...
# Behavioral Contract BC-5.30.058: feature:build-verification

## Description

Workflow contract: feature:build-verification.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f4-gate]`.

## Postconditions

1. Step `build-verification (line 1020; lobster carve-out: stable anchor is step name `build-verification`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `build-verification`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `build-verification (line 1020; lobster carve-out: stable anchor is step name `build-verification`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1637 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `build-verification (line 1020; lobster carve-out: stable anchor is step name `build-verification`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1213; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1637` |
| **Confidence** | high |
...
- **Step:** build-verification (line 1020; lobster carve-out: stable anchor is step name `build-verification`, not line number)
- **Type:** agent
- **Agent:** implementer
- **Depends on:** `[phase-f4-gate]`
- **Timeout:** 30m
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`build-verification (line 1020)`). Stable anchor is step name `build-verification`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `build-verification`. |
