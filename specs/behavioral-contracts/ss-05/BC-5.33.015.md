...
Step `holdout-freshness-check` (line 113; lobster carve-out: stable anchor is step name `holdout-freshness-check`, not line number). Type: agent. Agent: holdout-evaluator. Depends: `[state-init]`. Condition: `state.has_holdout_scenarios == true`. Source 113-126. <!-- F-P22-001: lobster-line-cite deferred per pass-21/22 carve-out; line range preserved as source evidence; lobster files have stable section structure (step `holdout-freshness-check` is the stable anchor); migrate to step-name anchor in future sweep -->
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/maintenance.lobster` (lines 113-126; lobster path carve-out: line range is unstable as lobster files evolve) |
| **Confidence** | high |
...
**Driver:** F-P22-001 pass-22 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 semantic-pattern-class discipline) — §Description cited lobster step by line number (`(line 113)`) and source range (`Source 113-126.`). These are lobster-file references and fall under the lobster-line-cite carve-out exception; the stable anchor is the step name `holdout-freshness-check`, not the line number.

**Change made:**
- §Description: inline annotation added noting lobster carve-out (stable anchor = step name `holdout-freshness-check`); HTML carve-out comment added citing F-P22-001 deferral.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster path lines 113-126
