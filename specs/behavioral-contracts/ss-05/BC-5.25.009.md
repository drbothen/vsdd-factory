...
# Behavioral Contract BC-5.25.009: phase-5:gemini-secondary-review

## Description

Secondary review using Gemini 3.1 Pro on fresh context; targets patterns the GPT-5.4 adversary may have missed.
...
1. Secondary review using Gemini 3.1 Pro on fresh context; targets patterns the GPT-5.4 adversary may have missed.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `gemini-secondary-review (line 41; lobster carve-out: stable anchor is step name `gemini-secondary-review`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1408 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `gemini-secondary-review (line 41; lobster carve-out: stable anchor is step name `gemini-secondary-review`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 531; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1408` |
| **Confidence** | high |
...
- **Step:** gemini-secondary-review (line 41; lobster carve-out: stable anchor is step name `gemini-secondary-review`, not line number)
- **Type:** agent
- **Agent:** code-reviewer
- **Condition:** `config.enable_secondary_adversary == true`
- **Source line(s) in workflow YAML:** 41-55
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`gemini-secondary-review (line 41)`). Stable anchor is step name `gemini-secondary-review`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `gemini-secondary-review`. |
