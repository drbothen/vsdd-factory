...
# Behavioral Contract BC-5.28.002: greenfield: entry-point

## Description

Workflow contract: greenfield: entry-point.
...
1. Step ``repo-initialization` (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number) — `depends_on: []`. Type: skill, skill: `skills/repo-initialization/SKILL.md`. Sole zero-in-degree node.` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `repo-initialization`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``repo-initialization` (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number) — `depends_on: []`. Type: skill, skill: `skills/repo-initialization/SKILL.md`. Sole zero-in-degree node.` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1461 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``repo-initialization` (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number) — `depends_on: []`. Type: skill, skill: `skills/repo-initialization/SKILL.md`. Sole zero-in-degree node.`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 677; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1461` |
| **Confidence** | high |
...
- **Step:** `repo-initialization` (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number) — `depends_on: []`. Type: skill, skill: `skills/repo-initialization/SKILL.md`. Sole zero-in-degree node.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``repo-initialization` (line 37)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `repo-initialization`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `repo-initialization`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `repo-initialization`. |
