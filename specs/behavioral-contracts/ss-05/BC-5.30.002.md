...
# Behavioral Contract BC-5.30.002: feature: entry-point

## Description

Workflow contract: feature: entry-point.
...
1. Step ``factory-worktree-health` (line 86; lobster carve-out: stable anchor is step name `factory-worktree-health`, not line number) — `depends_on: []`. Note: there are TWO zero-in-degree steps in this file: `factory-worktree-health` (line 86; lobster carve-out: same file, same annotation) AND `establish-demo-baseline` (line 162; lobster carve-out: stable anchor is step name `establish-demo-baseline`, `depends_on: []` per JSON line 82; lobster carve-out). The latter runs in parallel as an optional preflight conditional on `config.demo_recording.enabled != false`.` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `factory-worktree-health`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``factory-worktree-health` (line 86; lobster carve-out: stable anchor is step name `factory-worktree-health`, not line number) — `depends_on: []`. Note: there are TWO zero-in-degree steps in this file: `factory-worktree-health` (line 86; lobster carve-out: same file, same annotation) AND `establish-demo-baseline` (line 162; lobster carve-out: stable anchor is step name `establish-demo-baseline`, `depends_on: []` per JSON line 82; lobster carve-out). The latter runs in parallel as an optional preflight conditional on `config.demo_recording.enabled != false`.` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1581 |
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1036; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1581` |
| **Confidence** | high |
...
- **Step:** `factory-worktree-health` (line 86; lobster carve-out: stable anchor is step name `factory-worktree-health`, not line number) — `depends_on: []`. Note: there are TWO zero-in-degree steps in this file: `factory-worktree-health` (line 86; lobster carve-out: same file, same annotation) AND `establish-demo-baseline` (line 162; lobster carve-out: stable anchor is step name `establish-demo-baseline`, `depends_on: []` per JSON line 82; lobster carve-out). The latter runs in parallel as an optional preflight conditional on `config.demo_recording.enabled != false`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``factory-worktree-health` (line 86)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `factory-worktree-health`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `factory-worktree-health`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `factory-worktree-health`. |
