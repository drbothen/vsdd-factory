...
|--------|----------|-------------|
| VP-001 | Workflow YAML field shape matches declared schema for ``factory-worktree-health` (line 86) — `depends_on: []`. Note: there are TWO zero-in-degree steps in this file: `factory-worktree-health` (line 86) AND `establish-demo-baseline` (line 162, `depends_on: []` per JSON line 82). The latter runs in parallel as an optional preflight conditional on `config.demo_recording.enabled != false`.` | manual |
