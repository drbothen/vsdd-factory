...
modified:
  - "2026-06-24 (v2.0) — S-18.04b-prereq BC authoring burst (product-owner): ADR-029 exec-free re-architecture: (A) §Architecture Anchors exec-free clause + dispatcher git_context injection model (ADR-029 §Decision 3); (B) §Postconditions PC1 addendum: git_context.head_subject/head_sha/head_parent_subject/head_parent_sha injected host-side, read WASM-side; neither WASM hook calls exec_subprocess (ADR-029 §Decision 1 + §Decision 3); (C) §Verification Properties VP-084 proof-model: PostToolUse Bash envelope with git_context fields; negative control via sentinel-containing subjects in git_context; AC-007 preserved."
  - "2026-06-16 (v1.9) — fix burst (product-owner): (F-PASS2-002 + O-P2-004) Story-anchor traceability corrections per D-611 S-18.04 split: §Story Anchor + §Traceability Stories row corrected from S-18.04 → S-18.04b. No behavioral content changed."
...
1. **Exemption by prefix match + SHA corroboration (F-8)**: Both `validate-burst-log` and `validate-dispatch-advance` treat any commit whose subject matches the pattern `^PreCompact flush ` as lifecycle-orthogonal, BUT ONLY when the commit's SHA can be corroborated against the LAST LINE of the append-log file `.factory/hooks/precompact-flush-log`. The log line has four space-separated fields: `<ISO-timestamp> <SHA> <cycle>/<step> commit`. The WASM gate reads the SECOND FIELD (the SHA) and the FOURTH FIELD (the object-type token) of the last line. **The WASM gate does NOT exec `git cat-file -t` — it has no exec_subprocess capability.** Instead, it reads the FOURTH FIELD of the log line: (a) if the append-log exists and its last line has FOURTH FIELD == `commit`, the exempted commit's SHA MUST equal the SECOND FIELD SHA (exact match; no partial match). **SHA-mismatch with valid field-4=`commit` is NOT EXEMPT** — this outcome is enforced in-body here (not only in EC-003) because allowing exemption on SHA-mismatch-with-valid-field-4 would permit arbitrary-prefix bypass (an agent authors a commit with `PreCompact flush ` subject while the log records a different SHA; the mismatch is detectable and is treated as suspicious; chain detection applies normally); (b) if the last line's FOURTH FIELD is absent, empty, or not the literal string `commit` (indicating log corruption or truncation), treat the log entry as stale and fall through to case (c); (c) if the append-log is genuinely absent (`test -f` returns false), or the last line is empty, the prefix match alone is sufficient for exemption. This prevents an arbitrary agent from authoring a commit with a `PreCompact flush ` subject to bypass TD-VSDD-053 without having actually run the flush hook. Exempt commits are excluded from the HEAD/HEAD^ chain comparison that detects `MULTI_COMMIT_CHAIN_NOT_ALLOWED`.
   ...
   **git_context source (ADR-029 §Decision 1 + §Decision 3)**: The commit subjects and SHAs evaluated by the exemption check are obtained HOST-SIDE by the dispatcher and injected into `payload.extra` as a `git_context` JSON object before the WASM plugins execute. The WASM gate reads `payload.extra.get("git_context")` and extracts four fields: `head_subject`, `head_sha`, `head_parent_subject`, `head_parent_sha`. **Neither `validate-burst-log` nor `validate-dispatch-advance` calls `host::exec_subprocess` for commit-context acquisition.** If `git_context` is absent or all fields are empty (dispatcher fail-open), the WASM gate treats the situation as "no chain context — skip check" (fail-open, consistent with Invariant 5). The trigger event for chain-detection evaluation is PostToolUse Bash (tool='Bash', `tool_input.command` containing a `git commit` targeting the factory-artifacts worktree), NOT PostToolUse Edit/Write (ADR-029 §Decision 1).
...
- BC-1.16.001 — depends on: dispatcher git_context injection (host-side); BC-5.41.003 is the WASM-side consumption contract; BC-1.16.001 is the host-side injection contract (ADR-029 §Decision 3 host/WASM split)

## Architecture Anchors

- `plugins/vsdd-factory/hook-plugins/validate-burst-log.wasm` — native WASM plugin (crates/hook-plugins/); must be amended with `PreCompact flush ` prefix exemption + precompact-flush-log last-line FOURTH FIELD (`commit` token) corroboration; WASM does NOT exec git; no bash equivalent exists. **Exec-free constraint (ADR-029 §Decision 3):** this plugin MUST NOT call `host::exec_subprocess` for commit-context acquisition; the dispatcher (host layer, `crates/factory-dispatcher/src/invoke.rs`) execs git host-side on PostToolUse Bash git-commit events and injects `git_context` into `payload.extra`; the WASM gate reads `payload.extra.get("git_context")`.
- `plugins/vsdd-factory/hook-plugins/validate-dispatch-advance.wasm` — native WASM plugin (crates/hook-plugins/); must be amended symmetrically; same fourth-field corroboration logic; WASM does NOT exec git; no bash equivalent exists. **Exec-free constraint (ADR-029 §Decision 3):** same host-injection model as validate-burst-log.wasm; `host::exec_subprocess` MUST NOT be called for commit-context acquisition.
- `crates/factory-dispatcher/src/invoke.rs` — dispatcher implementation responsible for host-side git execution and `git_context` injection into `payload.extra` on qualifying PostToolUse Bash git-commit events (ADR-029 §Decision 3 + §Decision 1); see BC-1.16.001 for the dispatcher-side injection contract.
...
- ADR-029 §Decision 1 — chain-detection gate trigger: PostToolUse Bash (git commit), not PostToolUse Edit/Write; dispatcher injects git_context before routing to chain-detection plugins
- ADR-029 §Decision 3 — dispatcher (host) execs git host-side; WASM plugins are exec-free; git_context injected into payload.extra

## Story Anchor

S-18.04b (validate-burst-log + validate-dispatch-advance exemption; depends_on: S-18.04a precompact-flush.sh core)
...
|--------|----------|-------------|
| VP-084 | validate-burst-log exempts commits with "PreCompact flush " prefix from MULTI_COMMIT_CHAIN_NOT_ALLOWED detector; validate-dispatch-advance applies same exemption symmetrically. **Proof model (ADR-029 §Decision 6):** bats proof uses a PostToolUse Bash envelope (`tool='Bash'`, `tool_input.command` containing `git commit`) with `git_context` carrying HEAD/HEAD^ fields (`head_subject`, `head_sha`, `head_parent_subject`, `head_parent_sha`); negative control supplies sentinel-containing subjects (`head_subject: "stage 1 backfill"`, `head_parent_subject: "stage 2 backfill"`) via `git_context` to exercise the MULTI_COMMIT_CHAIN_NOT_ALLOWED detection path; the dispatcher's `git_context` injection is exercised end-to-end (not bypassed); AC-007 (tool under test is factory-dispatcher, not raw wasmtime) is preserved. | integration |
...
| ADR | ADR-026 §Decision 10 (PreCompact flush lifecycle distinct from state-manager burst lifecycle; validate-burst-log + validate-dispatch-advance must exempt commits with `PreCompact flush ` prefix + SHA corroboration via precompact-flush-log last-line fourth-field `commit` token; WASM reads embedded field, does not exec git; exemption is case-sensitive; stale-entry label is log corruption or truncation, not write-before-push-crash per F-P5-002); ADR-029 §Decision 1 (trigger: PostToolUse Bash git-commit event, not Edit/Write; dispatcher injects git_context before routing); ADR-029 §Decision 3 (dispatcher execs git host-side; WASM plugins exec-free; git_context in payload.extra) |
| Stories | S-18.04b |
...
|---------|------|--------|--------|
| v2.0 | 2026-06-24 | product-owner | (ADR-029 re-architecture) Three amendments for exec-free WASM via dispatcher git_context injection: (A) §Architecture Anchors: exec-free clause (WASM MUST NOT call host::exec_subprocess; dispatcher injects git_context into payload.extra via invoke.rs; ADR-029 §Decision 3); (B) §Postconditions PC1 addendum: git_context.head_subject/head_sha/head_parent_subject/head_parent_sha injected host-side, WASM reads via payload.extra.get("git_context"); trigger event is PostToolUse Bash not Edit/Write (ADR-029 §Decision 1 + §Decision 3); fail-open on absent/empty git_context; (C) §Verification Properties VP-084 proof-model: PostToolUse Bash envelope with git_context HEAD/HEAD^ fields; sentinel subjects via git_context for negative control; end-to-end dispatcher injection exercised; AC-007 preserved. H1 updated with ADR-029 injection model summary. |
| v1.9 | 2026-06-16 | product-owner | (F-PASS2-002 + O-P2-004) Story-anchor traceability corrections per D-611 S-18.04 split: §Story Anchor corrected from S-18.04 → S-18.04b (validate-burst-log + validate-dispatch-advance exemption is S-18.04b deliverable); §Traceability Stories row corrected from S-18.04 → S-18.04b. No postcondition/invariant/edge-case behavioral text changed. |
