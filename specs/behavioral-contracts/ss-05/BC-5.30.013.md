...
# Behavioral Contract BC-5.30.013: feature:phase-f1-delta-analysis

## Description

Workflow contract: feature:phase-f1-delta-analysis.

## Preconditions

1. Upstream dependencies completed successfully: `[feature-market-review, environment-check]`.

## Postconditions

1. Step `phase-f1-delta-analysis (line 179; lobster carve-out: stable anchor is step name `phase-f1-delta-analysis`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f1-delta-analysis`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f1-delta-analysis (line 179; lobster carve-out: stable anchor is step name `phase-f1-delta-analysis`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1592 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f1-delta-analysis (line 179; lobster carve-out: stable anchor is step name `phase-f1-delta-analysis`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1077; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1592` |
| **Confidence** | high |
...
- **Step:** phase-f1-delta-analysis (line 179; lobster carve-out: stable anchor is step name `phase-f1-delta-analysis`, not line number)
- **Type:** skill
- **Skill:** `skills/phase-f1-delta-analysis/SKILL.md`
- **Depends on:** `[feature-market-review, environment-check]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f1-delta-analysis (line 179)`). Stable anchor is step name `phase-f1-delta-analysis`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f1-delta-analysis`. |
