...
# Behavioral Contract BC-5.20.014: phase-0:extraction-validation

## Description

Validates the extracted entities/contracts against the source — sanity check that nothing fabricated.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-coverage-audit]`.

## Postconditions

1. Validates the extracted entities/contracts against the source — sanity check that nothing fabricated.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``extraction-validation` (line 81; lobster carve-out: stable anchor is step name `extraction-validation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1313 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``extraction-validation` (line 81; lobster carve-out: stable anchor is step name `extraction-validation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 147; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1313` |
| **Confidence** | high |
...
- **Step:** `extraction-validation` (line 81; lobster carve-out: stable anchor is step name `extraction-validation`, not line number)
- **Type:** skill
- **Skill:** `step-e-extraction-validation.md`
- **Depends on:** `[backup-coverage-audit]`
- **Source line(s) in workflow YAML:** 81-85
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`extraction-validation (line 81)`). Stable anchor is step name `extraction-validation`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `extraction-validation` (line 81); double-backtick VP/anchor: step `extraction-validation` (line 81); **Step:** bt form: step `extraction-validation` (line 81)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `extraction-validation`. |
