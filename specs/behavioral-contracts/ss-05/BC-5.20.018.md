...
# Behavioral Contract BC-5.20.018: phase-0:phase-0-gate

## Description

Blocking gate. Verifies 7 criteria: project-context.md exists, module-criticality.md exists, ≥1 BC in `behavioral-contracts/`, BCs use `BC-S.SS.NNN` with `origin: recovered`, security-audit.md exists, validation-report.md passed, holdout scenarios seeded.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-final-synthesis]`.

## Postconditions

1. Blocking gate.
2. Acceptance: All 7 criteria satisfied or `fail_action: block` halts the pipeline.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``phase-0-gate` (line 113; lobster carve-out: stable anchor is step name `phase-0-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1317 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``phase-0-gate` (line 113; lobster carve-out: stable anchor is step name `phase-0-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 171; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1317` |
| **Confidence** | high |
...
- **Step:** `phase-0-gate` (line 113; lobster carve-out: stable anchor is step name `phase-0-gate`, not line number)
- **Type:** gate
- **Depends on:** `[backup-final-synthesis]`
- **Source line(s) in workflow YAML:** 113-125
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-0-gate (line 113)`). Stable anchor is step name `phase-0-gate`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `phase-0-gate` (line 113); double-backtick VP/anchor: step `phase-0-gate` (line 113); **Step:** bt form: step `phase-0-gate` (line 113)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-0-gate`. |
