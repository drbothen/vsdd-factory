...
# Behavioral Contract BC-5.28.003: greenfield: terminal-step

## Description

Workflow contract: greenfield: terminal-step.
...
1. Step ``process-review-decisions` (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`. Final post-pipeline housekeeping that processes human review decisions.` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``process-review-decisions` (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`. Final post-pipeline housekeeping that processes human review decisions.` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1462 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``process-review-decisions` (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`. Final post-pipeline housekeeping that processes human review decisions.`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 680; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1462` |
| **Confidence** | high |
...
- **Step:** `process-review-decisions` (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`. Final post-pipeline housekeeping that processes human review decisions.
- **Source line(s) in workflow YAML:** 1398-1410
