...
# Behavioral Contract BC-5.29.027: brownfield:multi-repo-handoff-check

## Description

Detects whether greenfield's multi-repo-state-migration emitted `project_type: multi-repo` and `project.yaml`; sets MULTI_REPO_HANDOFF flag.
...
1. Detects whether greenfield's multi-repo-state-migration emitted `project_type: multi-repo` and `project.yaml`; sets MULTI_REPO_HANDOFF flag.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `multi-repo-handoff-check (line 348; lobster carve-out: stable anchor is step name `multi-repo-handoff-check`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1566 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `multi-repo-handoff-check (line 348; lobster carve-out: stable anchor is step name `multi-repo-handoff-check`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1009; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1566` |
| **Confidence** | high |
...
- **Step:** multi-repo-handoff-check (line 348; lobster carve-out: stable anchor is step name `multi-repo-handoff-check`, not line number)
- **Type:** agent
- **Agent:** orchestrator
- **Depends on:** `[greenfield-pipeline]`
- **Source line(s) in workflow YAML:** 348-357
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`multi-repo-handoff-check (line 348)`). Stable anchor is step name `multi-repo-handoff-check`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `multi-repo-handoff-check`. |
