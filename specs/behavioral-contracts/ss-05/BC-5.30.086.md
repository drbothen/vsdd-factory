...
# Behavioral Contract BC-5.30.086: feature:session-review-approval

## Description

Workflow contract: feature:session-review-approval.
...
1. Step `session-review-approval (line 1466; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `session-review-approval (line 1466; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1665 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `session-review-approval (line 1466; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1297; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1665` |
| **Confidence** | high |
...
- **Step:** session-review-approval (line 1466; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)
- **Type:** human-approval
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`session-review-approval (line 1466)`). Stable anchor is step name `session-review-approval`.
