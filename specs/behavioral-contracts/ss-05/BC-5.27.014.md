...
# Behavioral Contract BC-5.27.014: phase-7:visual-convergence

## Description

Workflow contract: phase-7:visual-convergence.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-verification-convergence]`.

## Postconditions

1. Step `visual-convergence (line 73; lobster carve-out: stable anchor is step name `visual-convergence`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `visual-convergence`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `visual-convergence (line 73; lobster carve-out: stable anchor is step name `visual-convergence`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1443 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `visual-convergence (line 73; lobster carve-out: stable anchor is step name `visual-convergence`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 629; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1443` |
| **Confidence** | high |
...
- **Step:** visual-convergence (line 73; lobster carve-out: stable anchor is step name `visual-convergence`, not line number)
- **Type:** skill
- **Skill:** `step-e-visual-convergence.md`
- **Depends on:** `[backup-verification-convergence]`
- **Source line(s) in workflow YAML:** 73-76
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`visual-convergence (line 73)`). Stable anchor is step name `visual-convergence`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `visual-convergence`. |
