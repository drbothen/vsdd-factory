...
# Behavioral Contract BC-5.29.024: brownfield:brownfield-design-system-approval

## Description

Workflow contract: brownfield:brownfield-design-system-approval.

## Preconditions

1. Upstream dependencies completed successfully: `[brownfield-design-system-extract]`.
2. Conditional gate evaluates true: `same`.

## Postconditions

1. Step `brownfield-design-system-approval (line 289; lobster carve-out: stable anchor is step name `brownfield-design-system-approval`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `brownfield-design-system-approval`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `brownfield-design-system-approval (line 289; lobster carve-out: stable anchor is step name `brownfield-design-system-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1563 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `brownfield-design-system-approval (line 289; lobster carve-out: stable anchor is step name `brownfield-design-system-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 999; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1563` |
| **Confidence** | high |
...
- **Step:** brownfield-design-system-approval (line 289; lobster carve-out: stable anchor is step name `brownfield-design-system-approval`, not line number)
- **Type:** human-approval
- **Depends on:** `[brownfield-design-system-extract]`
- **Condition:** same
- **Timeout:** 24h
- **Source line(s) in workflow YAML:** 289-299
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`brownfield-design-system-approval (line 289)`). Stable anchor is step name `brownfield-design-system-approval`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `brownfield-design-system-approval`. |
