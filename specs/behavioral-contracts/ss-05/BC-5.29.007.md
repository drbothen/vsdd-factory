...
# Behavioral Contract BC-5.29.007: brownfield:environment-gate

## Description

Workflow contract: brownfield:environment-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[environment-setup]`.

## Postconditions

1. Step `environment-gate (line 43; lobster carve-out: stable anchor is step name `environment-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `environment-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `environment-gate (line 43; lobster carve-out: stable anchor is step name `environment-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1546 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `environment-gate (line 43; lobster carve-out: stable anchor is step name `environment-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 944; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1546` |
| **Confidence** | high |
...
- **Step:** environment-gate (line 43; lobster carve-out: stable anchor is step name `environment-gate`, not line number)
- **Type:** gate
- **Depends on:** `[environment-setup]`
- **Source line(s) in workflow YAML:** 43-52
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`environment-gate (line 43)`). Stable anchor is step name `environment-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `environment-gate`. |
