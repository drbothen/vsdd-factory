...
# Behavioral Contract BC-5.20.008: phase-0:broad-sweep

## Description

Runs the 7-pass broad sweep (Pass 0 through 6) against the acquired source, producing the initial pass files. 7 broad-sweep pass files produced (one per pass).

## Preconditions

1. Upstream dependencies completed successfully: `[backup-source-acquisition]`.

## Postconditions

1. Runs the 7-pass broad sweep (Pass 0 through 6) against the acquired source, producing the initial pass files.
2. Acceptance: 7 broad-sweep pass files produced (one per pass).
