...
# Behavioral Contract BC-5.21.012: phase-1:create-architecture

## Description

Produces architecture sections + verification-properties; may emit a feasibility report that triggers prd-revision.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-create-prd]`.

## Postconditions

1. Produces architecture sections + verification-properties; may emit a feasibility report that triggers prd-revision.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``create-architecture` (line 63; lobster carve-out: stable anchor is step name `create-architecture`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1331 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``create-architecture` (line 63; lobster carve-out: stable anchor is step name `create-architecture`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 267; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1331` |
| **Confidence** | high |
...
- **Step:** `create-architecture` (line 63; lobster carve-out: stable anchor is step name `create-architecture`, not line number)
- **Type:** skill
- **Skill:** `skills/create-architecture/SKILL.md`
- **Depends on:** `[backup-create-prd]`
- **Source line(s) in workflow YAML:** 63-66
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`create-architecture (line 63)`). Stable anchor is step name `create-architecture`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `create-architecture` (line 63); double-backtick VP/anchor: step `create-architecture` (line 63); **Step:** bt form: step `create-architecture` (line 63)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `create-architecture`. |
