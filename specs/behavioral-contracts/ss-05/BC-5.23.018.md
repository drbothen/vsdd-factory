...
# Behavioral Contract BC-5.23.018: phase-3:cleanup

## Description

Workflow contract: phase-3:cleanup.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-pr-lifecycle]`.

## Postconditions

1. Step `cleanup (line 106; lobster carve-out: stable anchor is step name `cleanup`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `cleanup`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `cleanup (line 106; lobster carve-out: stable anchor is step name `cleanup`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1377 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `cleanup (line 106; lobster carve-out: stable anchor is step name `cleanup`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 453; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1377` |
| **Confidence** | high |
...
- **Step:** cleanup (line 106; lobster carve-out: stable anchor is step name `cleanup`, not line number)
- **Type:** skill
- **Skill:** `step-g-cleanup.md`
- **Depends on:** `[backup-pr-lifecycle]`
- **Source line(s) in workflow YAML:** 106-109
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`cleanup (line 106)`). Stable anchor is step name `cleanup`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `cleanup`. |
