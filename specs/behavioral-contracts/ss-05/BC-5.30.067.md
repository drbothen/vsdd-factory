...
# Behavioral Contract BC-5.30.067: feature:phase-f6-a11y-recheck

## Description

Workflow contract: feature:phase-f6-a11y-recheck.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f6-targeted-hardening]`.
2. Conditional gate evaluates true: `UI feature_type`.

## Postconditions

1. Step `phase-f6-a11y-recheck (line 1193; lobster carve-out: stable anchor is step name `phase-f6-a11y-recheck`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f6-a11y-recheck`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `UI feature_type` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f6-a11y-recheck (line 1193; lobster carve-out: stable anchor is step name `phase-f6-a11y-recheck`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1646 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f6-a11y-recheck (line 1193; lobster carve-out: stable anchor is step name `phase-f6-a11y-recheck`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1240; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1646` |
| **Confidence** | high |
...
- **Step:** phase-f6-a11y-recheck (line 1193; lobster carve-out: stable anchor is step name `phase-f6-a11y-recheck`, not line number)
- **Type:** agent
- **Agent:** accessibility-auditor
- **Depends on:** `[phase-f6-targeted-hardening]`
- **Condition:** UI feature_type
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f6-a11y-recheck (line 1193)`). Stable anchor is step name `phase-f6-a11y-recheck`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f6-a11y-recheck`. |
