...
# Behavioral Contract BC-5.28.031: greenfield:multi-repo-human-confirmation

## Description

Workflow contract: greenfield:multi-repo-human-confirmation.

## Preconditions

1. Upstream dependencies completed successfully: `[multi-repo-topology-check]`.
2. Conditional gate evaluates true: `deployment_topology == 'multi-service'`.

## Postconditions

1. Step `multi-repo-human-confirmation (line 375; lobster carve-out: stable anchor is step name `multi-repo-human-confirmation`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `multi-repo-human-confirmation`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `deployment_topology == 'multi-service'` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `multi-repo-human-confirmation (line 375; lobster carve-out: stable anchor is step name `multi-repo-human-confirmation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1490 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `multi-repo-human-confirmation (line 375; lobster carve-out: stable anchor is step name `multi-repo-human-confirmation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 769; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1490` |
| **Confidence** | high |
...
- **Step:** multi-repo-human-confirmation (line 375; lobster carve-out: stable anchor is step name `multi-repo-human-confirmation`, not line number)
- **Type:** human-approval
- **Depends on:** `[multi-repo-topology-check]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`multi-repo-human-confirmation (line 375)`). Stable anchor is step name `multi-repo-human-confirmation`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `multi-repo-human-confirmation`. |
