...
# Behavioral Contract BC-5.21.019: phase-1:human-approval

## Description

Workflow contract: phase-1:human-approval.
...
1. Step ``human-approval` (line 148; lobster carve-out: stable anchor is step name `human-approval`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``human-approval` (line 148; lobster carve-out: stable anchor is step name `human-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1338 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``human-approval` (line 148; lobster carve-out: stable anchor is step name `human-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 311; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1338` |
| **Confidence** | high |
...
- **Step:** `human-approval` (line 148; lobster carve-out: stable anchor is step name `human-approval`, not line number)
- **Timeout:** 24h
- **Source line(s) in workflow YAML:** 148-161
