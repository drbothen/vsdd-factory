...
# Behavioral Contract BC-5.28.044: greenfield:pre-phase-4-dtu-gate

## Description

Workflow contract: greenfield:pre-phase-4-dtu-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[dx-engineer-preflight-gate]`.
2. Conditional gate evaluates true: `dtu_assessment.DTU_REQUIRED == true`.

## Postconditions

1. Step `pre-phase-4-dtu-gate (line 605; lobster carve-out: stable anchor is step name `pre-phase-4-dtu-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `pre-phase-4-dtu-gate`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `dtu_assessment.DTU_REQUIRED == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `pre-phase-4-dtu-gate (line 605; lobster carve-out: stable anchor is step name `pre-phase-4-dtu-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1503 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `pre-phase-4-dtu-gate (line 605; lobster carve-out: stable anchor is step name `pre-phase-4-dtu-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 808; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1503` |
| **Confidence** | high |
...
- **Step:** pre-phase-4-dtu-gate (line 605; lobster carve-out: stable anchor is step name `pre-phase-4-dtu-gate`, not line number)
- **Type:** gate
- **Depends on:** `[dx-engineer-preflight-gate]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`pre-phase-4-dtu-gate (line 605)`). Stable anchor is step name `pre-phase-4-dtu-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `pre-phase-4-dtu-gate`. |
