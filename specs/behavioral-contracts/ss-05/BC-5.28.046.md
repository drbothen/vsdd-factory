...
# Behavioral Contract BC-5.28.046: greenfield:phase-3-per-story-delivery

## Description

Wave-level loop iterating over stories; per-story delivery delegates to `code-delivery.lobster` sub-workflow.

## Preconditions

1. Upstream dependencies completed successfully: `[dx-engineer-preflight-gate, pre-phase-4-dtu-gate, pre-phase-4-cicd-gate]`.

## Postconditions

1. Wave-level loop iterating over stories; per-story delivery delegates to `code-delivery.lobster` sub-workflow.

## Invariants

1. Step execution must complete within timeout: 12h.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-3-per-story-delivery (line 634; lobster carve-out: stable anchor is step name `phase-3-per-story-delivery`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1505 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-3-per-story-delivery (line 634; lobster carve-out: stable anchor is step name `phase-3-per-story-delivery`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 814; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1505` |
| **Confidence** | high |
...
- **Step:** phase-3-per-story-delivery (line 634; lobster carve-out: stable anchor is step name `phase-3-per-story-delivery`, not line number)
- **Type:** loop
- **Depends on:** `[dx-engineer-preflight-gate, pre-phase-4-dtu-gate, pre-phase-4-cicd-gate]`
- **Timeout:** 12h
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-3-per-story-delivery (line 634)`). Stable anchor is step name `phase-3-per-story-delivery`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-3-per-story-delivery`. |
