...
# Behavioral Contract BC-5.30.048: feature:phase-f2-human-approval

## Description

Workflow contract: feature:phase-f2-human-approval.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f2-gate]`.

## Postconditions

1. Step `phase-f2-human-approval (line 640; lobster carve-out: stable anchor is step name `phase-f2-human-approval`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f2-human-approval`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f2-human-approval (line 640; lobster carve-out: stable anchor is step name `phase-f2-human-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1627 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f2-human-approval (line 640; lobster carve-out: stable anchor is step name `phase-f2-human-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1182; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1627` |
| **Confidence** | high |
...
- **Step:** phase-f2-human-approval (line 640; lobster carve-out: stable anchor is step name `phase-f2-human-approval`, not line number)
- **Type:** human-approval
- **Depends on:** `[phase-f2-gate]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f2-human-approval (line 640)`). Stable anchor is step name `phase-f2-human-approval`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f2-human-approval`. |
