...
# Behavioral Contract BC-5.21.016: phase-1:spec-gate

## Description

Blocks unless: all requirements have unique IDs and numerical targets; Provable Properties Catalog covers all security boundaries; Purity Boundary Map is complete; verification tooling selected; module-criticality.md written.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-create-architecture, backup-prd-revision]`.

## Postconditions

1. Blocks unless: all requirements have unique IDs and numerical targets; Provable Properties Catalog covers all security boundaries; Purity Boundary Map is complete; verification tooling selected; module-criticality.md written.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``spec-gate` (line 94; lobster carve-out: stable anchor is step name `spec-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1335 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``spec-gate` (line 94; lobster carve-out: stable anchor is step name `spec-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 291; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1335` |
| **Confidence** | high |
...
- **Step:** `spec-gate` (line 94; lobster carve-out: stable anchor is step name `spec-gate`, not line number)
- **Type:** gate
- **Depends on:** `[backup-create-architecture, backup-prd-revision]`
- **Source line(s) in workflow YAML:** 94-104
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`spec-gate (line 94)`). Stable anchor is step name `spec-gate`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `spec-gate` (line 94); double-backtick VP/anchor: step `spec-gate` (line 94); **Step:** bt form: step `spec-gate` (line 94)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `spec-gate`. |
