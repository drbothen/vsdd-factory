...
# Behavioral Contract BC-5.28.067: greenfield:phase-7-convergence

## Description

Workflow contract: greenfield:phase-7-convergence.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-5-gate, phase-6-ui-fix-delivery]`.

## Postconditions

1. Step `phase-7-convergence (line 1246; lobster carve-out: stable anchor is step name `phase-7-convergence`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-7-convergence`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-7-convergence (line 1246; lobster carve-out: stable anchor is step name `phase-7-convergence`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1526 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-7-convergence (line 1246; lobster carve-out: stable anchor is step name `phase-7-convergence`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 878; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1526` |
| **Confidence** | high |
...
- **Step:** phase-7-convergence (line 1246; lobster carve-out: stable anchor is step name `phase-7-convergence`, not line number)
- **Type:** skill
- **Skill:** `skills/phase-7-convergence/SKILL.md`
- **Depends on:** `[phase-5-gate, phase-6-ui-fix-delivery]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-7-convergence (line 1246)`). Stable anchor is step name `phase-7-convergence`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-7-convergence`. |
