...
1. Step `bugfix-release (line 422; lobster carve-out: stable anchor is step name `bugfix-release`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `bugfix-release`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `bugfix-release (line 422; lobster carve-out: stable anchor is step name `bugfix-release`, not line number)` | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `bugfix-release (line 422; lobster carve-out: stable anchor is step name `bugfix-release`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1137; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1612` |
| **Confidence** | high |
...
- **Step:** bugfix-release (line 422; lobster carve-out: stable anchor is step name `bugfix-release`, not line number)
- **Type:** agent
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`bugfix-release (line 422)`). Stable anchor is step name `bugfix-release`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `bugfix-release`. |
