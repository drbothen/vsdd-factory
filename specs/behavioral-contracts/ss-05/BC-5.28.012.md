...
# Behavioral Contract BC-5.28.012: greenfield:adaptive-planning

## Description

Workflow contract: greenfield:adaptive-planning.

## Preconditions

1. Upstream dependencies completed successfully: `[state-initialization]`.
2. Conditional gate evaluates true: `config.skip_planning != true`.

## Postconditions

1. Step `adaptive-planning (line 97; lobster carve-out: stable anchor is step name `adaptive-planning`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `adaptive-planning`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `config.skip_planning != true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `adaptive-planning (line 97; lobster carve-out: stable anchor is step name `adaptive-planning`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1471 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `adaptive-planning (line 97; lobster carve-out: stable anchor is step name `adaptive-planning`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 711; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1471` |
| **Confidence** | high |
...
- **Step:** adaptive-planning (line 97; lobster carve-out: stable anchor is step name `adaptive-planning`, not line number)
- **Type:** sub-workflow
- **Depends on:** `[state-initialization]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`adaptive-planning (line 97)`). Stable anchor is step name `adaptive-planning`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `adaptive-planning`. |
