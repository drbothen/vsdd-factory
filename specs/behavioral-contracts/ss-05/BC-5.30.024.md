...
# Behavioral Contract BC-5.30.024: feature:bugfix-single-story-delivery

## Description

Workflow contract: feature:bugfix-single-story-delivery.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f1-human-approval, bugfix-demo-baseline]`.
2. Conditional gate evaluates true: `delta.intent == 'bug-fix'`.

## Postconditions

1. Step `bugfix-single-story-delivery (line 301; lobster carve-out: stable anchor is step name `bugfix-single-story-delivery`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `bugfix-single-story-delivery`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `delta.intent == 'bug-fix'` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `bugfix-single-story-delivery (line 301; lobster carve-out: stable anchor is step name `bugfix-single-story-delivery`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1603 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `bugfix-single-story-delivery (line 301; lobster carve-out: stable anchor is step name `bugfix-single-story-delivery`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1110; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1603` |
| **Confidence** | high |
...
- **Step:** bugfix-single-story-delivery (line 301; lobster carve-out: stable anchor is step name `bugfix-single-story-delivery`, not line number)
- **Type:** sub-workflow
- **Depends on:** `[phase-f1-human-approval, bugfix-demo-baseline]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`bugfix-single-story-delivery (line 301)`). Stable anchor is step name `bugfix-single-story-delivery`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `bugfix-single-story-delivery`. |
