...
# Behavioral Contract BC-5.28.068: greenfield:phase-6-gate

## Description

Workflow contract: greenfield:phase-6-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-7-convergence]`.

## Postconditions

1. Step `phase-6-gate (line 1251; lobster carve-out: stable anchor is step name `phase-6-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-6-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-6-gate (line 1251; lobster carve-out: stable anchor is step name `phase-6-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1527 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-6-gate (line 1251; lobster carve-out: stable anchor is step name `phase-6-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 881; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1527` |
| **Confidence** | high |
...
- **Step:** phase-6-gate (line 1251; lobster carve-out: stable anchor is step name `phase-6-gate`, not line number)
- **Type:** gate
- **Depends on:** `[phase-7-convergence]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-6-gate (line 1251)`). Stable anchor is step name `phase-6-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-6-gate`. |
