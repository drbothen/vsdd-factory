...
# Behavioral Contract BC-5.28.020: greenfield:phase-1-design-system-approval

## Description

Workflow contract: greenfield:phase-1-design-system-approval.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-1-design-system-bootstrap]`.
2. Conditional gate evaluates true: `same as above`.

## Postconditions

1. Step `phase-1-design-system-approval (line 180; lobster carve-out: stable anchor is step name `phase-1-design-system-approval`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-1-design-system-approval`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `same as above` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-1-design-system-approval (line 180; lobster carve-out: stable anchor is step name `phase-1-design-system-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1479 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-1-design-system-approval (line 180; lobster carve-out: stable anchor is step name `phase-1-design-system-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 735; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1479` |
| **Confidence** | high |
...
- **Step:** phase-1-design-system-approval (line 180; lobster carve-out: stable anchor is step name `phase-1-design-system-approval`, not line number)
- **Type:** human-approval
- **Depends on:** `[phase-1-design-system-bootstrap]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-1-design-system-approval (line 180)`). Stable anchor is step name `phase-1-design-system-approval`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-1-design-system-approval`. |
