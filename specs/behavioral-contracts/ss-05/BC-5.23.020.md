...
# Behavioral Contract BC-5.23.020: phase-3:implementation-gate

## Description

Verifies Red Gate proof, all tests pass, purity boundary respected, no `unwrap()` in production, clippy/lint zero warnings, demo evidence per AC, PR merged to develop, worktree cleaned.
...
1. Verifies Red Gate proof, all tests pass, purity boundary respected, no `unwrap()` in production, clippy/lint zero warnings, demo evidence per AC, PR merged to develop, worktree cleaned.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `implementation-gate (line 121; lobster carve-out: stable anchor is step name `implementation-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1379 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `implementation-gate (line 121; lobster carve-out: stable anchor is step name `implementation-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 460; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1379` |
| **Confidence** | high |
...
- **Step:** implementation-gate (line 121; lobster carve-out: stable anchor is step name `implementation-gate`, not line number)
- **Type:** gate
- **Source line(s) in workflow YAML:** 121-134
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`implementation-gate (line 121)`). Stable anchor is step name `implementation-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `implementation-gate`. |
