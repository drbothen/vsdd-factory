...
# Behavioral Contract BC-5.20.005: phase-0-codebase-ingestion: failure semantics
...
1. Default `on_failure: escalate`, `max_retries: 2`, `timeout: 4h`.
2. Acceptance: Reading the YAML produces `defaults.on_failure == "escalate"`. The phase-0-gate has `gate.fail_action == "block"`.
