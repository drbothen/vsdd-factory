...
# Behavioral Contract BC-5.22.010: phase-2:dependency-graph

## Description

Workflow contract: phase-2:dependency-graph.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-create-stories]`.

## Postconditions

1. Step `dependency-graph (line 49; lobster carve-out: stable anchor is step name `dependency-graph`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `dependency-graph`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `dependency-graph (line 49; lobster carve-out: stable anchor is step name `dependency-graph`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1349 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `dependency-graph (line 49; lobster carve-out: stable anchor is step name `dependency-graph`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 359; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1349` |
| **Confidence** | high |
...
- **Step:** dependency-graph (line 49; lobster carve-out: stable anchor is step name `dependency-graph`, not line number)
- **Type:** skill
- **Skill:** `decompose-stories/steps/step-c-dependency-graph.md`
- **Depends on:** `[backup-create-stories]`
- **Source line(s) in workflow YAML:** 49-52
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`dependency-graph (line 49)`). Stable anchor is step name `dependency-graph`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `dependency-graph`. |
