...
# Behavioral Contract BC-5.26.012: phase-6:security-scan

## Description

Workflow contract: phase-6:security-scan.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-mutation-testing]`.

## Postconditions

1. Step `security-scan (line 65; lobster carve-out: stable anchor is step name `security-scan`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `security-scan`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `security-scan (line 65; lobster carve-out: stable anchor is step name `security-scan`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1421 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `security-scan (line 65; lobster carve-out: stable anchor is step name `security-scan`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 574; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1421` |
| **Confidence** | high |
...
- **Step:** security-scan (line 65; lobster carve-out: stable anchor is step name `security-scan`, not line number)
- **Type:** skill
- **Skill:** `formal-verify/steps/step-d-security-scan.md`
- **Depends on:** `[backup-mutation-testing]`
- **Source line(s) in workflow YAML:** 65-68
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`security-scan (line 65)`). Stable anchor is step name `security-scan`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `security-scan`. |
