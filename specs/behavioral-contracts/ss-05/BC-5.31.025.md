...
Step `dependency-merge-check` (line 394; lobster carve-out: stable anchor is step name `dependency-merge-check`, not line number). Type: agent. Agent: pr-manager. Depends: `[wait-for-ci]`. Source 394-402. Verifies upstream/downstream dependency PRs are mergeable in the right order. <!-- F-P22-001: lobster-line-cite deferred per pass-21/22 carve-out; line range preserved as source evidence; lobster files have stable section structure (step `dependency-merge-check` is the stable anchor); migrate to step-name anchor in future sweep -->
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/code-delivery.lobster` (lines 394-402; lobster path carve-out: line range is unstable as lobster files evolve) |
| **Confidence** | high |
...
**Driver:** F-P22-001 pass-22 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 semantic-pattern-class discipline) — §Description cited lobster step by line number (`(line 394)`) and source range (`Source 394-402.`). These are lobster-file references and fall under the lobster-line-cite carve-out exception; the stable anchor is the step name `dependency-merge-check`, not the line number.

**Change made:**
- §Description: inline annotation added noting lobster carve-out (stable anchor = step name `dependency-merge-check`); HTML carve-out comment added citing F-P22-001 deferral.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster path lines 394-402
