...
# Behavioral Contract BC-5.20.009: phase-0:backup-broad-sweep

## Description

Commits the 7 pass files from broad-sweep; updates STATE.md.

## Preconditions

1. Upstream dependencies completed successfully: `[broad-sweep]`.

## Postconditions

1. Commits the 7 pass files from broad-sweep; updates STATE.md.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``backup-broad-sweep` (line 39; lobster carve-out: stable anchor is step name `backup-broad-sweep`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1308 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``backup-broad-sweep` (line 39; lobster carve-out: stable anchor is step name `backup-broad-sweep`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 115; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1308` |
| **Confidence** | high |
...
- **Step:** `backup-broad-sweep` (line 39; lobster carve-out: stable anchor is step name `backup-broad-sweep`, not line number)
- **Type:** agent
- **Agent:** state-manager
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`backup-broad-sweep (line 39)`). Stable anchor is step name `backup-broad-sweep`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `backup-broad-sweep` (line 39); double-backtick VP/anchor: step `backup-broad-sweep` (line 39); **Step:** bt form: step `backup-broad-sweep` (line 39)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `backup-broad-sweep`. |
