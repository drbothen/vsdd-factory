...
# Behavioral Contract BC-5.29.028: brownfield:multi-repo-pipeline

## Description

Workflow contract: brownfield:multi-repo-pipeline.

## Preconditions

1. Upstream dependencies completed successfully: `[multi-repo-handoff-check]`.
2. Conditional gate evaluates true: `MULTI_REPO_HANDOFF == true`.

## Postconditions

1. Step `multi-repo-pipeline (line 359; lobster carve-out: stable anchor is step name `multi-repo-pipeline`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `multi-repo-pipeline`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `MULTI_REPO_HANDOFF == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `multi-repo-pipeline (line 359; lobster carve-out: stable anchor is step name `multi-repo-pipeline`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1567 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `multi-repo-pipeline (line 359; lobster carve-out: stable anchor is step name `multi-repo-pipeline`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1013; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1567` |
| **Confidence** | high |
...
- **Step:** multi-repo-pipeline (line 359; lobster carve-out: stable anchor is step name `multi-repo-pipeline`, not line number)
- **Type:** sub-workflow
- **Depends on:** `[multi-repo-handoff-check]`
- **Condition:** `MULTI_REPO_HANDOFF == true`
- **Source line(s) in workflow YAML:** 359-363
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`multi-repo-pipeline (line 359)`). Stable anchor is step name `multi-repo-pipeline`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `multi-repo-pipeline`. |
