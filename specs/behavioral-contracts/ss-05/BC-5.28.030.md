...
# Behavioral Contract BC-5.28.030: greenfield:multi-repo-topology-check

## Description

Workflow contract: greenfield:multi-repo-topology-check.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-1-human-approval]`.

## Postconditions

1. Step `multi-repo-topology-check (line 362; lobster carve-out: stable anchor is step name `multi-repo-topology-check`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `multi-repo-topology-check`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `multi-repo-topology-check (line 362; lobster carve-out: stable anchor is step name `multi-repo-topology-check`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1489 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `multi-repo-topology-check (line 362; lobster carve-out: stable anchor is step name `multi-repo-topology-check`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 766; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1489` |
| **Confidence** | high |
...
- **Step:** multi-repo-topology-check (line 362; lobster carve-out: stable anchor is step name `multi-repo-topology-check`, not line number)
- **Type:** agent
- **Agent:** orchestrator
- **Depends on:** `[phase-1-human-approval]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`multi-repo-topology-check (line 362)`). Stable anchor is step name `multi-repo-topology-check`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `multi-repo-topology-check`. |
