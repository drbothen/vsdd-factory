...
# Behavioral Contract BC-5.21.018: phase-1:input-hash-drift-check

## Description

Workflow contract: phase-1:input-hash-drift-check.

## Preconditions

1. Upstream dependencies completed successfully: `[adversarial-spec-review]`.
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1337 |
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 305; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1337` |
| **Confidence** | high |
...
- **Source line(s) in workflow YAML:** 138-145
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``input-hash-drift-check` (line 138)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `input-hash-drift-check`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `input-hash-drift-check`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `input-hash-drift-check`. |
