...
# Behavioral Contract BC-5.28.010: greenfield:scaffold-claude-md

## Description

Workflow contract: greenfield:scaffold-claude-md.

## Preconditions

1. Upstream dependencies completed successfully: `[factory-worktree-gate]`.
2. Conditional gate evaluates true: `!file_exists('CLAUDE.md')`.

## Postconditions

1. Step `scaffold-claude-md (line 70; lobster carve-out: stable anchor is step name `scaffold-claude-md`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `scaffold-claude-md`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `!file_exists('CLAUDE.md')` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `scaffold-claude-md (line 70; lobster carve-out: stable anchor is step name `scaffold-claude-md`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1469 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `scaffold-claude-md (line 70; lobster carve-out: stable anchor is step name `scaffold-claude-md`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 705; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1469` |
| **Confidence** | high |
...
- **Step:** scaffold-claude-md (line 70; lobster carve-out: stable anchor is step name `scaffold-claude-md`, not line number)
- **Type:** skill
- **Skill:** `skills/scaffold-claude-md/SKILL.md`
- **Depends on:** `[factory-worktree-gate]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`scaffold-claude-md (line 70)`). Stable anchor is step name `scaffold-claude-md`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `scaffold-claude-md`. |
