...
# Behavioral Contract BC-5.27.021: phase-7:input-hash-drift-check

## Description

Workflow contract: phase-7:input-hash-drift-check.
...
1. Step `input-hash-drift-check (line 134; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `input-hash-drift-check (line 134; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1450 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `input-hash-drift-check (line 134; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 652; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1450` |
| **Confidence** | high |
...
- **Step:** input-hash-drift-check (line 134; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)
- **Type:** skill
- **Source line(s) in workflow YAML:** 134-140
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`input-hash-drift-check (line 134)`). Stable anchor is step name `input-hash-drift-check`.
