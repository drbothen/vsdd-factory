...
# Behavioral Contract BC-5.22.005: phase-2: failure semantics

## Description

Defaults `on_failure: escalate`, retries 2, timeout 2h. `decomposition-gate.fail_action: block`. Adversarial story review is a `loop` with max_iterations 10 and exit on CONVERGENCE_REACHED. ### Per-step BCs (phase-2)
...
1. Defaults `on_failure: escalate`, retries 2, timeout 2h.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-2: failure semantics` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1344 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-2: failure semantics`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 340; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1344` |
| **Confidence** | high |
...
- **Source line(s) in workflow YAML:** 10-13, 105
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — source-doc line 340
