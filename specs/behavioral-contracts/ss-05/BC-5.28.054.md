...
# Behavioral Contract BC-5.28.054: greenfield:phase-4-demo-recording

## Description

Workflow contract: greenfield:phase-4-demo-recording.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-4-gate]`.
2. Conditional gate evaluates true: `config.demo_recording.enabled != false`.

## Postconditions

1. Step `phase-4-demo-recording (line 1044; lobster carve-out: stable anchor is step name `phase-4-demo-recording`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-4-demo-recording`; line number is unstable as lobster files evolve -->

## Invariants

1. Step execution must complete within timeout: 30m.
...
| EC-003 | Condition `config.demo_recording.enabled != false` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-4-demo-recording (line 1044; lobster carve-out: stable anchor is step name `phase-4-demo-recording`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1513 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-4-demo-recording (line 1044; lobster carve-out: stable anchor is step name `phase-4-demo-recording`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 839; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1513` |
| **Confidence** | high |
...
- **Step:** phase-4-demo-recording (line 1044; lobster carve-out: stable anchor is step name `phase-4-demo-recording`, not line number)
- **Type:** skill
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-4-demo-recording (line 1044)`). Stable anchor is step name `phase-4-demo-recording`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-4-demo-recording`. |
