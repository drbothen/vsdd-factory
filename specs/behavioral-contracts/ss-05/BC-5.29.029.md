...
# Behavioral Contract BC-5.29.029: brownfield:session-review

## Description

Workflow contract: brownfield:session-review.

## Preconditions

1. Upstream dependencies completed successfully: `[greenfield-pipeline]`.

## Postconditions

1. Step `session-review (line 371; lobster carve-out: stable anchor is step name `session-review`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `session-review (line 371; lobster carve-out: stable anchor is step name `session-review`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1568 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `session-review (line 371; lobster carve-out: stable anchor is step name `session-review`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1016; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1568` |
| **Confidence** | high |
...
- **Step:** session-review (line 371; lobster carve-out: stable anchor is step name `session-review`, not line number)
- **Type:** skill
- **Skill:** `skills/session-review/SKILL.md`
- **Depends on:** `[greenfield-pipeline]`
- **Source line(s) in workflow YAML:** 371-375
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`session-review (line 371)`). Stable anchor is step name `session-review`.
