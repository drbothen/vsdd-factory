...
Step `session-review-approval` (line 412; lobster carve-out: stable anchor is step name `session-review-approval`, not line number). Type: human-approval. Timeout: 72h. Depends: `[session-review]`. Source 412-423. <!-- F-P22-001: lobster-line-cite deferred per pass-21/22 carve-out; line range preserved as source evidence; lobster files have stable section structure (step `session-review-approval` is the stable anchor); migrate to step-name anchor in future sweep -->
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/discovery.lobster` (lines 412-423; lobster path carve-out: line range is unstable as lobster files evolve) |
| **Confidence** | high |
...
**Driver:** F-P22-001 pass-22 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 semantic-pattern-class discipline) — §Description cited lobster step by line number (`(line 412)`) and source range (`Source 412-423.`). These are lobster-file references and fall under the lobster-line-cite carve-out exception; the stable anchor is the step name `session-review-approval`, not the line number.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster path lines 412-423
