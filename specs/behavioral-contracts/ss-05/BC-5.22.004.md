...
# Behavioral Contract BC-5.22.004: phase-2: DAG integrity

## Description

14 steps acyclic; linear backup chain; final flow define-epics → create-stories → dependency-graph → wave-schedule → holdout-scenarios → decomposition-gate → adversarial-story-review (loop) → input-hash-drift-check → human-approval.
...
1. 14 steps acyclic; linear backup chain; final flow define-epics → create-stories → dependency-graph → wave-schedule → holdout-scenarios → decomposition-gate → adversarial-story-review (loop) → input-hash-drift-check → human-approval.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-2: DAG integrity` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1343 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-2: DAG integrity`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 335; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1343` |
| **Confidence** | high |
...
- **Source line(s) in workflow YAML:** 15-171
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — source-doc line 335
