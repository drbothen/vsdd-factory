...
# Behavioral Contract BC-5.23.010: phase-3:failing-tests

## Description

Writes failing tests + Red Gate (compile pass, all assertions fail).

## Preconditions

1. Upstream dependencies completed successfully: `[backup-generate-stubs]`.

## Postconditions

1. Writes failing tests + Red Gate (compile pass, all assertions fail).
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `failing-tests (line 47; lobster carve-out: stable anchor is step name `failing-tests`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1369 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `failing-tests (line 47; lobster carve-out: stable anchor is step name `failing-tests`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 427; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1369` |
| **Confidence** | high |
...
- **Step:** failing-tests (line 47; lobster carve-out: stable anchor is step name `failing-tests`, not line number)
- **Type:** skill
- **Skill:** `step-c-failing-tests.md`
- **Depends on:** `[backup-generate-stubs]`
- **Source line(s) in workflow YAML:** 47-50
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`failing-tests (line 47)`). Stable anchor is step name `failing-tests`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `failing-tests`. |
