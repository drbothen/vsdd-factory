...
# Behavioral Contract BC-5.30.060: feature:holdout-evaluation

## Description

Workflow contract: feature:holdout-evaluation.

## Preconditions

1. Upstream dependencies completed successfully: `[build-gate]`.

## Postconditions

1. Step `holdout-evaluation (line 1042; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `holdout-evaluation (line 1042; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1639 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `holdout-evaluation (line 1042; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1219; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1639` |
| **Confidence** | high |
...
- **Step:** holdout-evaluation (line 1042; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)
- **Type:** skill
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`holdout-evaluation (line 1042)`). Stable anchor is step name `holdout-evaluation`.
