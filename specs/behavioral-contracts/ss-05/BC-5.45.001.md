...
Every `.factory/` artifact that carries a `last_amended:` frontmatter scalar (most load-bearingly
the five files named in D-1149 — `STORY-INDEX.md`, `BC-INDEX.md`, `ARCH-INDEX.md`, `VP-INDEX.md`,
`STATE.md`) MUST have that field written using the current-entry-only discipline ratified by
ADR-049: the executing agent OVERWRITES `last_amended` with ONLY the new entry, and the entry it
displaces is PREPENDED as a new `changelog:` sequence list item (or, for `STATE.md`, superseded by
the file's own body-level `## Decisions Log`/`## Phase Progress`, since that file carries no
frontmatter `changelog:`). The prior discipline — read the existing value, wrap it in a
`[Prior: ...]` bracket, and write the whole concatenation back as one larger scalar — is what
produced a 323,499-char single physical line on `STORY-INDEX.md` (and comparable growth on the
other four files) and repeatedly exhausted the bash-adapter WASM validators' fuel budget (743
fuel-timeouts/day, 2026-09-02). This BC is the write-path contract; BC-10.13.001 governs the
one-time migration/rotation tool that brings existing files into this shape, and BC-4.18.001
governs the fuel-budget-relief regression proof.

## Preconditions

1. An executing agent (state-manager for the four index files and STATE.md; any agent for other
   `.factory/` artifacts carrying `last_amended:`) is about to write a new history entry to a
   file's `last_amended:` frontmatter field.
...
For every one of the five D-1149 files and every other `.factory/` artifact carrying
`last_amended:`, no write performed under this BC's discipline ever produces a `last_amended`
value containing a nested `[Prior: <date> (vX.Y) — ...]` bracket referring to a DIFFERENT dated
entry than the current one. (The D-1149-era trailing pointer note — `[Prior history →
