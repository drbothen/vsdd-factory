...
# Behavioral Contract BC-5.28.061: greenfield:phase-5-gate

## Description

Workflow contract: greenfield:phase-5-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-6-formal-hardening, phase-6-dtu-adversarial]`.

## Postconditions

1. Step `phase-5-gate (line 1190; lobster carve-out: stable anchor is step name `phase-5-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-5-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-5-gate (line 1190; lobster carve-out: stable anchor is step name `phase-5-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1520 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-5-gate (line 1190; lobster carve-out: stable anchor is step name `phase-5-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 860; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1520` |
| **Confidence** | high |
...
- **Step:** phase-5-gate (line 1190; lobster carve-out: stable anchor is step name `phase-5-gate`, not line number)
- **Type:** gate
- **Depends on:** `[phase-6-formal-hardening, phase-6-dtu-adversarial]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-5-gate (line 1190)`). Stable anchor is step name `phase-5-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-5-gate`. |
