...
# Behavioral Contract BC-5.25.008: phase-5:triage-and-fix (nested)

## Description

Workflow contract: phase-5:triage-and-fix (nested).

## Preconditions

1. Upstream dependencies completed successfully: `[adversary-code-review]`.

## Postconditions

1. Step `triage-and-fix (line 36; lobster carve-out: stable anchor is step name `triage-and-fix`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `triage-and-fix`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `triage-and-fix (line 36; lobster carve-out: stable anchor is step name `triage-and-fix`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1407 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `triage-and-fix (line 36; lobster carve-out: stable anchor is step name `triage-and-fix`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 528; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1407` |
| **Confidence** | high |
...
- **Step:** triage-and-fix (line 36; lobster carve-out: stable anchor is step name `triage-and-fix`, not line number)
- **Type:** skill
- **Skill:** `skills/adversarial-review/SKILL.md`
- **Depends on:** `[adversary-code-review]`
- **Source line(s) in workflow YAML:** 36-39
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`triage-and-fix (line 36)`). Stable anchor is step name `triage-and-fix`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `triage-and-fix`. |
