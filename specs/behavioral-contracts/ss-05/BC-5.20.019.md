...
# Behavioral Contract BC-5.20.019: phase-0:input-hash-drift-check

## Description

Scans Phase 0 artifacts for stale `input-hash` frontmatter before human review.
...
1. Scans Phase 0 artifacts for stale `input-hash` frontmatter before human review.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``input-hash-drift-check` (line 127; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1318 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``input-hash-drift-check` (line 127; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 178; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1318` |
| **Confidence** | high |
...
- **Step:** `input-hash-drift-check` (line 127; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)
- **Type:** skill
- **Skill:** `skills/check-input-drift/SKILL.md`
- **Depends on:** `[phase-0-gate]`
- **Source line(s) in workflow YAML:** 127-132
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`input-hash-drift-check (line 127)`). Stable anchor is step name `input-hash-drift-check`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `input-hash-drift-check` (line 127); double-backtick VP/anchor: step `input-hash-drift-check` (line 127); **Step:** bt form: step `input-hash-drift-check` (line 127)
