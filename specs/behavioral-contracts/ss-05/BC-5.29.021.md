...
# Behavioral Contract BC-5.29.021: brownfield:semport-translation

## Description

Workflow contract: brownfield:semport-translation.

## Preconditions

1. Upstream dependencies completed successfully: `[detect-cross-language-porting]`.
2. Conditional gate evaluates true: `brownfield.needs_semport == true`.

## Postconditions

1. Step `semport-translation (line 257; lobster carve-out: stable anchor is step name `semport-translation`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `semport-translation`; line number is unstable as lobster files evolve -->

## Invariants

1. Step execution must complete within timeout: 4h.
...
| EC-003 | Condition `brownfield.needs_semport == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `semport-translation (line 257; lobster carve-out: stable anchor is step name `semport-translation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1560 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `semport-translation (line 257; lobster carve-out: stable anchor is step name `semport-translation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 990; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1560` |
| **Confidence** | high |
...
- **Step:** semport-translation (line 257; lobster carve-out: stable anchor is step name `semport-translation`, not line number)
- **Type:** skill
- **Skill:** `skills/semport-analyze/SKILL.md`
- **Depends on:** `[detect-cross-language-porting]`
- **Condition:** `brownfield.needs_semport == true`
- **Timeout:** 4h
- **Source line(s) in workflow YAML:** 257-262
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`semport-translation (line 257)`). Stable anchor is step name `semport-translation`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `semport-translation`. |
