...
# Behavioral Contract BC-5.22.018: phase-2:input-hash-drift-check

## Description

Workflow contract: phase-2:input-hash-drift-check.
...
1. Step `input-hash-drift-check (line 149; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `input-hash-drift-check (line 149; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1357 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `input-hash-drift-check (line 149; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 385; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1357` |
| **Confidence** | high |
...
- **Step:** input-hash-drift-check (line 149; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)
- **Type:** skill
- **Source line(s) in workflow YAML:** 149-154
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`input-hash-drift-check (line 149)`). Stable anchor is step name `input-hash-drift-check`.
