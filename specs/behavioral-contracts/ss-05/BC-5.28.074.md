...
# Behavioral Contract BC-5.28.074: greenfield:steady-state-handoff

## Description

Workflow contract: greenfield:steady-state-handoff.

## Preconditions

1. Upstream dependencies completed successfully: `[release]`.

## Postconditions

1. Step `steady-state-handoff (line 1343; lobster carve-out: stable anchor is step name `steady-state-handoff`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `steady-state-handoff`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `steady-state-handoff (line 1343; lobster carve-out: stable anchor is step name `steady-state-handoff`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1533 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `steady-state-handoff (line 1343; lobster carve-out: stable anchor is step name `steady-state-handoff`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 899; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1533` |
| **Confidence** | high |
...
- **Step:** steady-state-handoff (line 1343; lobster carve-out: stable anchor is step name `steady-state-handoff`, not line number)
- **Type:** agent
- **Agent:** orchestrator
- **Depends on:** `[release]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`steady-state-handoff (line 1343)`). Stable anchor is step name `steady-state-handoff`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `steady-state-handoff`. |
