...
# Behavioral Contract BC-5.24.003: phase-4: terminal-step

## Description

Workflow contract: phase-4: terminal-step.
...
1. Step ``holdout-gate` (line 28; lobster carve-out: stable anchor is step name `holdout-gate`, not line number) — gate type, depends `[holdout-evaluation]`. Source 28-38.` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `holdout-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``holdout-gate` (line 28; lobster carve-out: stable anchor is step name `holdout-gate`, not line number) — gate type, depends `[holdout-evaluation]`. Source 28-38.` | manual |
...
| Architecture Module | SS-05 Pipeline Orchestration (plugins/vsdd-factory/workflows/phases/phase-4-holdout-evaluation.lobster) |
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1392 |

## Related BCs (Recommended)

- TBD — sibling step contracts in section BC-5.24

## Architecture Anchors (Recommended)

- `architecture/ss-05-pipeline-orchestration.md#workflow-phase-4-holdout-evaluation` — workflow declaration source
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``holdout-gate` (line 28; lobster carve-out: stable anchor is step name `holdout-gate`, not line number) — gate type, depends `[holdout-evaluation]`. Source 28-38.`
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/phases/phase-4-holdout-evaluation.lobster` |
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 479; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1392` |
| **Confidence** | high |
...
- **Step:** `holdout-gate` (line 28; lobster carve-out: stable anchor is step name `holdout-gate`, not line number) — gate type, depends `[holdout-evaluation]`. Source 28-38.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``holdout-gate` (line 28)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `holdout-gate`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `holdout-gate`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `holdout-gate`. |
