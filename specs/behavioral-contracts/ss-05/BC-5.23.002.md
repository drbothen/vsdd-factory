...
origin: brownfield
extracted_from: "plugins/vsdd-factory/workflows/phases/phase-3-tdd-implementation.lobster"
subsystem: "SS-05"
capability: "CAP-001"
...
# Behavioral Contract BC-5.23.002: phase-3: entry-point

## Description

Workflow contract: phase-3: entry-point.
...
1. Step ``create-worktree` (line 18; lobster carve-out: stable anchor is step name `create-worktree`, not line number) — `depends_on: []`. Type: skill, skill: `deliver-story/steps/step-a-create-worktree.md`.` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `create-worktree`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``create-worktree` (line 18; lobster carve-out: stable anchor is step name `create-worktree`, not line number) — `depends_on: []`. Type: skill, skill: `deliver-story/steps/step-a-create-worktree.md`.` | manual |
...
| Architecture Module | SS-05 Pipeline Orchestration (plugins/vsdd-factory/workflows/phases/phase-3-tdd-implementation.lobster) |
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1361 |

## Related BCs (Recommended)

- TBD — sibling step contracts in section BC-5.23

## Architecture Anchors (Recommended)

- `architecture/ss-05-pipeline-orchestration.md#workflow-phase-3-tdd-implementation` — workflow declaration source
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``create-worktree` (line 18; lobster carve-out: stable anchor is step name `create-worktree`, not line number) — `depends_on: []`. Type: skill, skill: `deliver-story/steps/step-a-create-worktree.md`.`
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/phases/phase-3-tdd-implementation.lobster` |
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 400; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1361` |
| **Confidence** | high |
...
- **Step:** `create-worktree` (line 18; lobster carve-out: stable anchor is step name `create-worktree`, not line number) — `depends_on: []`. Type: skill, skill: `deliver-story/steps/step-a-create-worktree.md`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``create-worktree` (line 18)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `create-worktree`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `create-worktree`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `create-worktree`. |
