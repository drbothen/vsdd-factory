...
# Behavioral Contract BC-5.28.056: greenfield:phase-5-gemini-review

## Description

Workflow contract: greenfield:phase-5-gemini-review.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-5-adversarial-refinement]`.
2. Conditional gate evaluates true: `config.enable_secondary_adversary == true`.

## Postconditions

1. Step `phase-5-gemini-review (line 1109; lobster carve-out: stable anchor is step name `phase-5-gemini-review`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-5-gemini-review`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `config.enable_secondary_adversary == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-5-gemini-review (line 1109; lobster carve-out: stable anchor is step name `phase-5-gemini-review`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1515 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-5-gemini-review (line 1109; lobster carve-out: stable anchor is step name `phase-5-gemini-review`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 845; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1515` |
| **Confidence** | high |
...
- **Step:** phase-5-gemini-review (line 1109; lobster carve-out: stable anchor is step name `phase-5-gemini-review`, not line number)
- **Type:** agent
- **Agent:** code-reviewer
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-5-gemini-review (line 1109)`). Stable anchor is step name `phase-5-gemini-review`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-5-gemini-review`. |
