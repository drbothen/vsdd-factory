...
# Behavioral Contract BC-5.28.011: greenfield:state-initialization

## Description

Workflow contract: greenfield:state-initialization.

## Preconditions

1. Upstream dependencies completed successfully: `[factory-worktree-gate, scaffold-claude-md]`.

## Postconditions

1. Step `state-initialization (line 81; lobster carve-out: stable anchor is step name `state-initialization`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `state-initialization`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `state-initialization (line 81; lobster carve-out: stable anchor is step name `state-initialization`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1470 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `state-initialization (line 81; lobster carve-out: stable anchor is step name `state-initialization`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 708; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1470` |
| **Confidence** | high |
...
- **Step:** state-initialization (line 81; lobster carve-out: stable anchor is step name `state-initialization`, not line number)
- **Type:** agent
- **Agent:** state-manager
- **Depends on:** `[factory-worktree-gate, scaffold-claude-md]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`state-initialization (line 81)`). Stable anchor is step name `state-initialization`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `state-initialization`. |
