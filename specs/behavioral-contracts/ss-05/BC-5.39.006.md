...
7. **(v1.8, narrative-literal hardening; see
   ...
   **(v1.9, separator-row-detection tightening — closes adversary pass-1 finding LOW-1)** A row is
   classified as a GFM separator row — and skipped — **if and only if**, after splitting the row
   on `|`, **every** resulting cell's trimmed content consists solely of the characters `-`, `:`,
   and whitespace (the canonical GFM table-separator form, e.g. `---`, `:---`, `---:`, `:---:`). A
   row is NOT classified as a separator merely because some cell's content contains the substring
   `---` — a genuine `## Decisions Log` DATA row whose non-ID cell contains `---` (an em-dash
   rendering, a `D-441---D-449` range citation, etc.) is retained, not skipped, and its ID cell is
   evaluated normally against the candidate set. The prior (v1.8) rule — skip any row "containing
   `---`" — is superseded; it wrongly matched on substring-containment rather than
   whole-cell-composition, which would drop `max_in_file` and let a genuinely stale
   `current_step:` D-chain cite escape detection whenever a legitimate data row's prose/summary
   cell happened to contain `---`.

   For each retained (non-separator) row, the hook takes the **first** pipe-delimited cell and
   trims it. **(v1.9, ID-cell normalization — closes adversary pass-1 finding LOW-2)** Before the
   whole-cell match, the trimmed cell is further stripped of surrounding markdown emphasis
   delimiters: leading and trailing runs of `*`, `_`, and backtick `` ` `` characters are removed
   (so `**D-1164**`, `` `D-1164` ``, and `_D-1164_` all normalize to `D-1164`). The hook then —
   **only if the normalized cell is a whole-string match to `D-\d+` (the entire normalized cell,
   not a substring)** — includes that integer in the candidate set. Interior or partial decoration
   that does not normalize cleanly to a whole-string `D-\d+` (e.g. `D-**1164**`, `**D-11**64`) is
   still rejected: normalization strips only symmetric leading/trailing emphasis runs, it does not
   repair malformed interior markup. The prior (v1.8) rule — whole-cell match against the raw,
   undecorated cell — wrongly rejected a decoration-wrapped ID cell like `| **D-1164** |` (the
   `**...**` bold convention is actively used elsewhere in STATE.md, e.g. the Phase-Progress
   table), silently excluding that row's ID from the candidate set and again letting a stale cite
   escape detection.

   `max_in_file` is the maximum of the resulting candidate set (0 if the section is absent or
   contains no matching rows) — the fail-open-to-0-on-absent-section and max-reduction semantics
   are unchanged from v1.8.
...
| EC-025 | A `## Decisions Log` row's first pipe-cell is emphasis-decorated, e.g. `\| **D-1164** \|` (bold), `\| \`D-1164\` \|` (backtick), or `\| _D-1164_ \|` (underscore-italic) | Continue — the cell is normalized by stripping surrounding `*`/`_`/backtick runs to `D-1164`; the normalized cell whole-string matches `D-\d+`; the row IS counted (`max_in_file` includes `1164`), not silently dropped |
| EC-026 | A `## Decisions Log` DATA row's non-ID cell contains the substring `---` (e.g. a summary cell reading `Closes D-441---D-449 range`, or an em-dash-style `---` rendering) while its first cell is a clean `D-\d+` | Continue — the row is NOT treated as a GFM separator row (not every cell's trimmed content is composed solely of `-`/`:`/whitespace); the row is retained and its ID cell is counted normally toward `max_in_file` |
...
| Emphasis-decorated Decisions Log ID cell (v1.9, LOW-2 closure) | `## Decisions Log` row's first cell is `\| **D-1164** \|` (bold-wrapped); `current_step:` cites `D-1164` | `HookResult::Continue` — ID-cell normalization strips the surrounding `**` delimiters before matching; normalized cell `D-1164` whole-string matches `D-\d+`; `max_in_file=1164`; `max_cited=1164 >= max_in_file=1164` | PASS (cites S-15.03 wave-m2 dispatch adversary pass-1 finding LOW-2 as prior art; pre-v1.9 behavior would have silently excluded this row's ID from the candidate set) |
| Decisions Log data row containing `---` in a non-ID cell (v1.9, LOW-1 closure) | `## Decisions Log` row is `\| D-1165 \| 2026-09-05 \| Closes D-441---D-449 range \| ... \|` (a genuine data row, not a GFM separator); `current_step:` cites `D-1165` | `HookResult::Continue` — the row is NOT classified as a separator because not every cell's trimmed content is composed solely of `-`/`:`/whitespace (the summary cell contains prose plus `---`); the row is retained and its ID cell `D-1165` is counted; `max_in_file=1165`; `max_cited=1165 >= max_in_file=1165` | PASS (cites S-15.03 wave-m2 dispatch adversary pass-1 finding LOW-1 as prior art; pre-v1.9 behavior would have skipped this row as a false-positive separator, dropping `max_in_file` and letting a stale `current_step:` cite escape detection) |
...
|---------|------|-------------|
| 1.9 | 2026-09-05 | Spec-first amendment (product-owner) hardening invariant 7 Side B's `## Decisions Log` scan against two latent false-NEGATIVE (stale-cite-escape) vectors surfaced by adversary pass-1 review of `.factory/cycles/v1.0-brownfield-backfill/s-15.03-wave-m2-dispatch.md`. Closes LOW-1 (the v1.8 rule skipped any row "containing `---`" — too broad, since a legitimate Decisions Log DATA row whose prose/summary cell contains `---` (em-dash, a `D-441---D-449` range citation, etc.) would be wrongly skipped, dropping `max_in_file` and letting a genuinely stale `current_step:` D-chain cite escape detection). Closes LOW-2 (the v1.8 whole-cell match rejected a decoration-wrapped ID cell like `\| **D-1164** \|` — the `**...**` bold convention is actively used elsewhere in STATE.md, e.g. the Phase-Progress table — silently excluding that row's ID from the candidate set and again letting a stale cite escape). (a) Separator-row detection replaced: a row is a GFM separator row — and skipped — if and only if, after splitting on `\|`, EVERY cell's trimmed content consists solely of `-`, `:`, and whitespace; a data row containing `---` inside a cell is now retained and its ID counted. (b) ID-cell normalization added: before the whole-cell `D-\d+` match, the first pipe-cell is trimmed and further stripped of surrounding markdown emphasis delimiters (leading/trailing runs of `*`, `_`, and backtick `` ` ``), so `**D-1164**`, `` `D-1164` ``, and `_D-1164_` all normalize to `D-1164` before matching; interior/partial decoration that does not normalize to a clean `D-\d+` is still rejected. Fail-open-to-0-on-absent-section and max-reduction semantics unchanged. Added EC-025 (emphasis-decorated ID cell normalized and counted) and EC-026 (Decisions Log data row with `---` in a non-ID cell retained, not treated as a separator). Added two Canonical Test Vector rows exercising both fixes, citing the adversary pass-1 LOW-1/LOW-2 findings as prior art. This amendment is spec-only — no code, STATE.md, or story-frontmatter `bcs:` arrays were modified; Red Gate tests (test-writer) and the `scan_max_decision_log_id` separator/normalization refinement (implementer) are a follow-up fix-burst. |
