...
# Behavioral Contract BC-5.29.008: brownfield:repo-verification

## Description

Verifies (NOT creates) existing repo: git remote, default branch, CI/CD, branch protection, .factory/ worktree, git rerere, merge-config.yaml. No destructive changes.

## Preconditions

1. Upstream dependencies completed successfully: `[environment-gate]`.

## Postconditions

1. Verifies (NOT creates) existing repo: git remote, default branch, CI/CD, branch protection, .factory/ worktree, git rerere, merge-config.yaml.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `repo-verification (line 58; lobster carve-out: stable anchor is step name `repo-verification`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1547 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `repo-verification (line 58; lobster carve-out: stable anchor is step name `repo-verification`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 947; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1547` |
| **Confidence** | high |
...
- **Step:** repo-verification (line 58; lobster carve-out: stable anchor is step name `repo-verification`, not line number)
- **Type:** agent
- **Agent:** devops-engineer
- **Depends on:** `[environment-gate]`
- **Timeout:** 15m
- **Source line(s) in workflow YAML:** 58-76
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`repo-verification (line 58)`). Stable anchor is step name `repo-verification`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `repo-verification`. |
