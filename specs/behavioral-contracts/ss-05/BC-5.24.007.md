...
# Behavioral Contract BC-5.24.007: phase-4:holdout-evaluation

## Description

Workflow contract: phase-4:holdout-evaluation.

## Preconditions

1. Upstream dependencies completed successfully: `[scenario-rotation]`.

## Postconditions

1. Step `holdout-evaluation (line 23; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `holdout-evaluation`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `holdout-evaluation (line 23; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1396 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `holdout-evaluation (line 23; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 492; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1396` |
| **Confidence** | high |
...
- **Step:** holdout-evaluation (line 23; lobster carve-out: stable anchor is step name `holdout-evaluation`, not line number)
- **Type:** skill
- **Skill:** `skills/holdout-eval/SKILL.md`
- **Depends on:** `[scenario-rotation]`
- **Source line(s) in workflow YAML:** 23-26
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`holdout-evaluation (line 23)`). Stable anchor is step name `holdout-evaluation`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `holdout-evaluation`. |
