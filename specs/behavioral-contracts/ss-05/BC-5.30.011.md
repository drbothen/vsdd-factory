...
# Behavioral Contract BC-5.30.011: feature:feature-market-review

## Description

Workflow contract: feature:feature-market-review.

## Preconditions

1. Upstream dependencies completed successfully: `[feature-market-intel]`.
2. Conditional gate evaluates true: `same`.

## Postconditions

1. Step `feature-market-review (line 146; lobster carve-out: stable anchor is step name `feature-market-review`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `feature-market-review`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `feature-market-review (line 146; lobster carve-out: stable anchor is step name `feature-market-review`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1590 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `feature-market-review (line 146; lobster carve-out: stable anchor is step name `feature-market-review`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1070; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1590` |
| **Confidence** | high |
...
- **Step:** feature-market-review (line 146; lobster carve-out: stable anchor is step name `feature-market-review`, not line number)
- **Type:** human-approval
- **Depends on:** `[feature-market-intel]`
- **Condition:** same
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`feature-market-review (line 146)`). Stable anchor is step name `feature-market-review`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `feature-market-review`. |
