...
# Behavioral Contract BC-5.26.007: phase-6:backup-kani-proofs

## Description

Workflow contract: phase-6:backup-kani-proofs.
...
1. Step `backup-kani-proofs (line 23; lobster carve-out: stable anchor is step name `backup-kani-proofs`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `backup-kani-proofs`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `backup-kani-proofs (line 23; lobster carve-out: stable anchor is step name `backup-kani-proofs`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1416 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `backup-kani-proofs (line 23; lobster carve-out: stable anchor is step name `backup-kani-proofs`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 559; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1416` |
| **Confidence** | high |
...
- **Step:** backup-kani-proofs (line 23; lobster carve-out: stable anchor is step name `backup-kani-proofs`, not line number)
- **Agent:** state-manager
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`backup-kani-proofs (line 23)`). Stable anchor is step name `backup-kani-proofs`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `backup-kani-proofs`. |
