...
# Behavioral Contract BC-5.30.059: feature:build-gate

## Description

Workflow contract: feature:build-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[build-verification]`.

## Postconditions

1. Step `build-gate (line 1029; lobster carve-out: stable anchor is step name `build-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `build-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `build-gate (line 1029; lobster carve-out: stable anchor is step name `build-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1638 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `build-gate (line 1029; lobster carve-out: stable anchor is step name `build-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1216; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1638` |
| **Confidence** | high |
...
- **Step:** build-gate (line 1029; lobster carve-out: stable anchor is step name `build-gate`, not line number)
- **Type:** gate
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`build-gate (line 1029)`). Stable anchor is step name `build-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `build-gate`. |
