...
# Behavioral Contract BC-5.29.004: brownfield: DAG integrity

## Description

26 steps acyclic. Sequence: environment-setup → environment-gate → repo-verification → factory-worktree-health → factory-worktree-gate → optional scaffold-claude-md → state-initialization → phase-0-codebase-ingestion → phase-0-artifact-backup → phase-0-gate → phase-0-human-approval → post-phase-0-routing → conditional brownfield-market-intel → brownfield-market-review → detect-cross-language-porti
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `brownfield: DAG integrity` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1543 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `brownfield: DAG integrity`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 931; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1543` |
| **Confidence** | high |
...
- **Source line(s) in workflow YAML:** 24-401
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — source-doc line 931
