...
# Behavioral Contract BC-5.27.024: phase-7:human-approval

## Description

Final ZERO-SLOP achievement gate.

## Preconditions

1. Upstream dependencies completed successfully: `[convergence-gate, visual-review]`.

## Postconditions

1. Final ZERO-SLOP achievement gate.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `human-approval (line 164; lobster carve-out: stable anchor is step name `human-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1453 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `human-approval (line 164; lobster carve-out: stable anchor is step name `human-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 661; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1453` |
| **Confidence** | high |
...
- **Step:** human-approval (line 164; lobster carve-out: stable anchor is step name `human-approval`, not line number)
- **Type:** human-approval
- **Depends on:** `[convergence-gate, visual-review]`
- **Timeout:** 48h
- **Source line(s) in workflow YAML:** 164-175
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`human-approval (line 164)`). Stable anchor is step name `human-approval`.
