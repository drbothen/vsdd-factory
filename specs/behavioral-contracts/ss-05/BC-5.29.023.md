...
# Behavioral Contract BC-5.29.023: brownfield:brownfield-design-system-extract

## Description

Workflow contract: brownfield:brownfield-design-system-extract.

## Preconditions

1. Upstream dependencies completed successfully: `[brownfield-market-review]`.
2. Conditional gate evaluates true: `routing.choice == 'feature' and feature_type in ['ui','full-stack']`.

## Postconditions

1. Step `brownfield-design-system-extract (line 280; lobster carve-out: stable anchor is step name `brownfield-design-system-extract`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `brownfield-design-system-extract`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `routing.choice == 'feature' and feature_type in ['ui','full-stack']` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `brownfield-design-system-extract (line 280; lobster carve-out: stable anchor is step name `brownfield-design-system-extract`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1562 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `brownfield-design-system-extract (line 280; lobster carve-out: stable anchor is step name `brownfield-design-system-extract`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 996; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1562` |
| **Confidence** | high |
...
- **Step:** brownfield-design-system-extract (line 280; lobster carve-out: stable anchor is step name `brownfield-design-system-extract`, not line number)
- **Type:** skill
...
- **Source line(s) in workflow YAML:** 280-287
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`brownfield-design-system-extract (line 280)`). Stable anchor is step name `brownfield-design-system-extract`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `brownfield-design-system-extract`. |
