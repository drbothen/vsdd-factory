...
# Behavioral Contract BC-5.30.076: feature:phase-f7-ui-quality-gate

## Description

Workflow contract: feature:phase-f7-ui-quality-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f7-heuristic-evaluation, phase-f7-ui-completeness-final, phase-f7-responsive-final]`.
2. Conditional gate evaluates true: `UI`.

## Postconditions

1. Step `phase-f7-ui-quality-gate (line 1297; lobster carve-out: stable anchor is step name `phase-f7-ui-quality-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f7-ui-quality-gate`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `UI` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f7-ui-quality-gate (line 1297; lobster carve-out: stable anchor is step name `phase-f7-ui-quality-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1655 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f7-ui-quality-gate (line 1297; lobster carve-out: stable anchor is step name `phase-f7-ui-quality-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1267; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1655` |
| **Confidence** | high |
...
- **Step:** phase-f7-ui-quality-gate (line 1297; lobster carve-out: stable anchor is step name `phase-f7-ui-quality-gate`, not line number)
- **Type:** skill
- **Skill:** `skills/ui-quality-gate/SKILL.md`
- **Depends on:** `[phase-f7-heuristic-evaluation, phase-f7-ui-completeness-final, phase-f7-responsive-final]`
- **Condition:** UI
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f7-ui-quality-gate (line 1297)`). Stable anchor is step name `phase-f7-ui-quality-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f7-ui-quality-gate`. |
