...
# Behavioral Contract BC-5.30.031: feature:bugfix-f7-verification

## Description

Workflow contract: feature:bugfix-f7-verification.

## Preconditions

1. Upstream dependencies completed successfully: `[bugfix-f6-scoped, bugfix-demo-comparison]`.
2. Conditional gate evaluates true: `delta.intent == 'bug-fix'`.

## Postconditions

1. Step `bugfix-f7-verification (line 401; lobster carve-out: stable anchor is step name `bugfix-f7-verification`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `bugfix-f7-verification`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `bugfix-f7-verification (line 401; lobster carve-out: stable anchor is step name `bugfix-f7-verification`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1610 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `bugfix-f7-verification (line 401; lobster carve-out: stable anchor is step name `bugfix-f7-verification`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1131; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1610` |
| **Confidence** | high |
...
- **Step:** bugfix-f7-verification (line 401; lobster carve-out: stable anchor is step name `bugfix-f7-verification`, not line number)
- **Type:** skill
- **Skill:** `skills/phase-f7-delta-convergence/SKILL.md`
- **Depends on:** `[bugfix-f6-scoped, bugfix-demo-comparison]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`bugfix-f7-verification (line 401)`). Stable anchor is step name `bugfix-f7-verification`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `bugfix-f7-verification`. |
