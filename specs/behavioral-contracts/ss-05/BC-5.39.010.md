...
13. Within the story file content, **the scan is CONFINED to the two BC-citation sections**,
    ...
    **Phase 1 — Pure-version field**: scan all pipe-delimited fields of the row. If any field's
    ...
    `1.7` or `v1.6`. Corpus count (2026-08-06): **58 rows** across all story files.
...
— **99.21% of the fuel budget consumed**. Headroom: **79,087 fuel = 0.79% of budget**.

Exhaustion bisection (real BC-INDEX rows appended to fixture): threshold between **593,525 and
601,548 bytes** — approximately **110 additional BC-INDEX rows** above the current corpus,
**~3–4% above the current 576,396-byte index**. `total_bcs` moved 1983→1985 in a single burst
during this cascade; BC-INDEX grows monotonically each burst.

**The prior claim — "The `max_bytes` caps in PC4 (1 MiB), PC10/12/15/35 (512 KiB), and PC19/23/29
(2 MiB) are calibrated to bound reads inside the fuel budget at current artifact sizes" — is FALSE
at HEAD and is retracted.** The 1 MiB PC4 cap permits a file of 1,048,576 bytes; exhaustion occurs
at approximately 594 KB — the cap is approximately **76% above the exhaustion point**. The caps
bound reads that are structurally larger than the budget can handle; they do not bound reads
inside the budget.

**On exhaustion the dispatcher exits 0 with empty stdout** — output-identical to a clean scan. This
is the silent-guard-failure class this BC exists to prevent, applied to this BC itself. With
`on_error = "continue"`, fuel exhaustion is silenced at the registry level; there is no WASM-side
handling for it in this hook. The effect: Arm A1/A2/B/E validation stops entirely and
undetectably as BC-INDEX grows past ~594 KB, while `cargo test`, `bats`, and the dispatcher all
exit 0.

**NORMATIVE REQUIREMENT — Exhaustion MUST surface as an observable failure, not a silent pass.**
The production-scale bats gate for this hook MUST include both of the following assertions
(test-writer obligation per F-S2107-P9-002 routing):
...
1. **Margin gate**: execute the real dispatcher against the production-scale BC-INDEX fixture,
   ...
   `fuel_consumed ≤ 6,000,000` (60% of the 10,000,000 cap). A 60% ceiling fires when the corpus
   reaches approximately 350 KB, giving multiple burst-cycles of warning before the ~594 KB
   exhaustion threshold. **A gate that passes at 99.21% fuel consumed is NON-CONFORMING under
   this BC even if bats exits 0 today.**
...
2. **Fixture drift gate**: assert that the production-scale fixture's `BC-INDEX.md` sha256 matches
   ...
   was added to close, displaced one level up.

**Operational consequence.** With the fuel budget unchanged (human ruling 2026-08-08: do NOT raise
the cap), exhaustion is expected to begin occurring within approximately 110 BC-INDEX rows of
ordinary registration traffic. Once the margin gate and drift gate are in place, this will surface
as a bats test failure — loud and operator-visible — rather than a silent validation stop. That
is the intended trade: the corpus-growth risk is now VISIBLE rather than implied.

**`fuel_cap` field**: ADR-035 §Decision 5 introduced a per-plugin `fuel_cap` registry field. As of
2026-08-08, the evidentiary precondition for adding a cap — confirming that `max_bytes` caps are
genuinely insufficient — **is now met**: 9,920,913/10,000,000 consumed at current scale,
exhaustion ~110 rows away. However, the human ruling (2026-08-08) explicitly chose to address
exhaustion via loud observable margin and drift gates rather than by raising the cap. A future
implementer MUST NOT add a `fuel_cap` entry without re-confirming this ruling with the operator.
The authorized approach is the margin gate + drift gate mandated above; `fuel_cap` remains
unauthorized until the operator explicitly re-authorizes it.
