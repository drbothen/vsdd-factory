...
section: "5.39"
last_amended: |-
  2026-08-15 (v1.24) — POL-14 auto-promotion at merge (state-manager; S-21.07 PR #776
  squash-merged to `develop` at commit `e94767bc` 2026-08-15). Top-level `status:` and
  `lifecycle_status:` fields draft→active. No behavioral/content change — Description,
  Preconditions, Postconditions, Invariants, Edge Cases, Canonical Test Vectors byte-identical
  to v1.23. BC-INDEX v4.62→v4.63 (row status column draft→active; version-chain cell +v1.24).
  [Prior: 2026-08-15 (v1.23) — SEC-001 / CWE-697 contiguity-anchoring amendment, human-authorized spec
  amendment (human directed: research-validate the shipped `chain_immediately_precedes_filename`
  implementation → verdict CORRECT, MITRE/OWASP/Rust-std cited → amend BC to ratify). Research
  verdict: the pre-v1.23 preconditions codified the CWE-697 "insufficient comparison" /
  independent-component-presence anti-pattern (`components().any(|c| ...)` evaluated separately
  per required directory segment), which the validated implementation in
  `crates/hook-plugins/validate-cross-site-correspondence/src/dispatch.rs` has already replaced
  with `chain_immediately_precedes_filename(components, chain, gap)`: the required directory
  chain MUST occupy a single CONTIGUOUS, correctly-ORDERED window of `Component::Normal`
  segments, positioned so it is followed by exactly `gap` further `Normal`-only directory
  segments and then the filename (never independently searched per segment name across the
  whole path), AND the path is rejected outright if it contains a `Component::ParentDir` (`..`)
  component ANYWHERE — including the otherwise-unconstrained prefix before the anchored chain.
  This amendment RATIFIES the existing flat-corpus reality — it does not change hook behavior;
  the implementation converged first and is validated correct, so the spec is amended to match
  per the Standing Rule for code-vs-spec conflicts (CLAUDE.md §Architectural Authority item 12).
  Precondition 1 (Arm A1 BC-file trigger) amended: the BC-INDEX chain check is replaced from
  `components().any(|c| c.as_os_str() == "behavioral-contracts")` to
  `chain_immediately_precedes_filename(components, [".factory", "specs",
  "behavioral-contracts"], gap=1)`, plus an explicit `..`-rejection clause. Precondition 9 (Arm
  A2 / B1 story-file trigger) amended: the `stories` single-component presence check is replaced
  with `chain_immediately_precedes_filename(components, [".factory", "stories"], gap=0)`, plus
  the same `..`-rejection clause. Precondition 34 (Class E frontmatter-parity trigger) amended on
  both the VP arm (`.factory/specs/verification-properties` chain, `gap=0`) and the epic arm
  (`.factory/stories/epics` chain, `gap=0`), each replacing independent `components().any(…)`
  per-segment presence checks with the same contiguity-anchored form plus `..`-rejection.
  Invariant 3 extended: "path-component-strict guards; raw string `ends_with` forbidden" now
  normatively requires contiguity-anchored chain matching (NOT independent per-segment
  presence) AND rejection of any path containing a `Component::ParentDir` (`..`) component —
  codifying the SEC-001/CWE-697 fix as a spec-level MUST, not merely an implementation choice. A
  new exact-gap sub-clause is added within invariant 3: BC files are ALWAYS exactly one `ss-NN`
  subdirectory under `.factory/specs/behavioral-contracts/` (`gap=1`); VP files, epic files, and
  active story files are FLAT (`gap=0`) — making the hardcoded gap values a checked spec
  invariant, so a future deeper-nesting layout change is a spec violation requiring a
  coordinated BC amendment, not a silent classifier no-op that drops governed files. Nested
  `.factory/stories/v1.0-legacy/S-*.md` archival stories are noted as intentionally OUT of the
  flat-corpus classification scope — the operative corpus is the non-recursive
  `.factory/stories/*.md` glob; the anchoring's rejection of the legacy-archive depth is
  intended, not a gap. New Canonical Test Vector row added ("Contiguity-anchored decoy
  rejection"): a non-contiguous decoy, a `..`-after-chain decoy, and a leading-`..` decoy are all
  correctly NOT classified as governed, against a positive absolute/worktree-path control that
  IS classified; the mutant is the pre-v1.23 independent-`.any()`-per-segment implementation,
  which misclassifies the non-contiguous decoy as governed. No new edge case (EC-NNN) added — the
  Canonical Test Vector row is judged sufficient coverage for this precondition-scoping
  (not-classified / hook-does-not-fire) behavior class, consistent with the existing EC-023
  precondition-scoping precedent, keeping propagation minimal per scope discipline. No BC H1
  title change (POLICY 7): the title's promise — blocking on stale cross-site version-cites
  across the four gated classes — is unaffected; this is path-classification-correctness
  hardening within the existing scope. No renumbering of any existing ID (POLICY 1 append-only):
  precondition 1, precondition 9, precondition 34, and invariant 3 text are amended in place; the
  new Canonical Test Vector row is a net-new appended row. BC-INDEX and the S-21.07 story are NOT
  amended in this burst — BC-INDEX version-cell sync is owed to state-manager; S-21.07 story
  BC-table/Token Budget version cite is owed to story-writer. No implementer/test-writer
  obligation is created by this amendment: `chain_immediately_precedes_filename` in `dispatch.rs`
  already implements the amended behavior in full (including the exact-gap values and the `..`
  rejection) — this burst is a pure spec-side ratification of already-shipped, already-validated
  code; a RED-gate/pinning unit test exercising the new Canonical Test Vector's decoy paths
  against the shipped classifier is recommended for test-writer as forward-looking regression
  protection but is not required to close this amendment, since the classifier already passes
  it. (product-owner; human-authorized SEC-001/CWE-697 spec-ratification amendment.)
  [Prior: 2026-08-14 (v1.22) — ADV-RECON11-001 (LOW, near-zero-reachability) architect-adjudicated
...
1. A PostToolUse Edit/Write event has fired on a file satisfying ALL of:
   ...
   - **(v1.23 / SEC-001 / CWE-697 contiguity-anchoring)** The file's path components contain
     the `.factory` / `specs` / `behavioral-contracts` chain as a CONTIGUOUS, correctly-ORDERED
     window of `Component::Normal` segments, positioned so that it is followed by exactly ONE
     further `Normal`-only directory segment (the `ss-NN` subdirectory; see the exact-gap
     invariant under Invariant 3) and then the filename —
     `chain_immediately_precedes_filename(components, [".factory", "specs",
     "behavioral-contracts"], gap=1)`. An independent per-segment presence check (e.g.
     `components().any(|c| c.as_os_str() == "behavioral-contracts")`, checked in isolation from
     the other chain segments' positions) is **NON-CONFORMING**: it admits a decoy path where
     the required directory names are present but non-contiguous, out of order, or separated by
     `..` traversal segments (e.g. `x/.factory/y/specs/z/behavioral-contracts/BC-1.2.3.md`) as a
     governed BC file. See Invariant 3.
   - **(v1.23 / SEC-001 / CWE-697)** The path contains NO `Component::ParentDir` (`..`)
     component ANYWHERE — including in the position(s) before the anchored chain, which are
     otherwise unconstrained to permit real absolute/worktree paths. A path containing any `..`
     component is rejected outright as non-governed, regardless of what the rest of the path
     contains (closes the leading-`..`-before-anchor residual, e.g.
     `../.factory/specs/behavioral-contracts/ss-05/BC-1.2.3.md`; see Invariant 3).
...
9. A PostToolUse Edit/Write event has fired on a file satisfying ALL of:
   ...
   - **(v1.23 / SEC-001 / CWE-697 contiguity-anchoring)** The file's path components contain
     the `.factory` / `stories` chain as a CONTIGUOUS, correctly-ORDERED window immediately
     preceding the filename — i.e. exactly ZERO further directory segments between the chain
     and the filename (`gap=0`; active story files sit FLAT under `.factory/stories/`; see the
     exact-gap invariant under Invariant 3) —
     `chain_immediately_precedes_filename(components, [".factory", "stories"], gap=0)`. An
     isolated single-component presence check (e.g. testing only for a `stories` path
     component, without anchoring its position relative to `.factory` and the filename) is
     **NON-CONFORMING** per the same rationale as precondition 1. This is also why
     `.factory/stories/v1.0-legacy/S-*.md` archival stories are intentionally excluded — a
     non-flat path fails the `gap=0` window.
   - **(v1.23 / SEC-001 / CWE-697)** The path contains NO `Component::ParentDir` (`..`)
     component ANYWHERE — rejected outright as non-governed. See Invariant 3.
...
34. PostToolUse on a file satisfying ANY of:
    - Under `.factory/specs/behavioral-contracts/ss-*/` with basename `BC-*.md` (not
      `BC-INDEX.md`)
    - **(v1.23 / SEC-001 / CWE-697 contiguity-anchoring)** Under
      `.factory/specs/verification-properties/`: the file's path components contain the
      `.factory` / `specs` / `verification-properties` chain as a CONTIGUOUS, correctly-ORDERED
      window immediately preceding the filename (`gap=0` — VP files sit FLAT under
      `.factory/specs/verification-properties/`; see the exact-gap invariant under Invariant 3)
      — `chain_immediately_precedes_filename(components, [".factory", "specs",
      "verification-properties"], gap=0)`, AND the path contains NO `Component::ParentDir`
      (`..`) component ANYWHERE. An independent per-segment presence check (e.g.
      `components().any(|c| c.as_os_str() == "verification-properties")`, checked in isolation)
      is **NON-CONFORMING** per the same rationale as precondition 1. With basename
    ...
    - **(v1.23 / SEC-001 / CWE-697 contiguity-anchoring)** Under `.factory/stories/epics/`: the
      file's path components contain the `.factory` / `stories` / `epics` chain as a
      CONTIGUOUS, correctly-ORDERED window immediately preceding the filename (`gap=0` — epic
      files sit FLAT under `.factory/stories/epics/`; see the exact-gap invariant under
      Invariant 3) — `chain_immediately_precedes_filename(components, [".factory", "stories",
      "epics"], gap=0)`, AND the path contains NO `Component::ParentDir` (`..`) component
      ANYWHERE. Independent per-segment presence checks (each of `stories` and `epics` verified
      via `components().any(…)` in isolation from the other's position) are **NON-CONFORMING**
      per the same rationale as precondition 1 — they admit a non-contiguous or
      out-of-order-segment decoy as a governed epic path. With basename matching
...
3. All file-path matching uses path-component-strict guards. Raw string `ends_with` is
   forbidden. **(v1.23 / SEC-001 / CWE-697 extension)** "Path-component-strict" is NOT
   satisfied by an independent per-segment presence check — e.g.
   `components().any(|c| c.as_os_str() == "<segment>")` evaluated separately for each required
   directory name, with no constraint on the segments' relative order or position. That form
   admits a decoy path where the required segment names are present but non-contiguous, out of
   order, or separated by `..` traversal — e.g.
   `x/.factory/y/specs/z/behavioral-contracts/BC-1.2.3.md` — as a governed path. Path matching
   MUST instead be **contiguity-anchored**: the governed directory chain for a given artifact
   class (e.g. `.factory` / `specs` / `behavioral-contracts` for BC files) MUST appear as a
   single CONTIGUOUS, correctly-ORDERED window of `Component::Normal` segments, positioned so
   that it is followed by exactly the artifact class's normative `gap` of further `Normal`-only
   directory segments and then the filename — never searched independently for each segment
   name across the whole path. Additionally, a path MUST be rejected outright as non-governed if
   it contains a `Component::ParentDir` (`..`) component ANYWHERE — including in the
   unconstrained prefix before the anchored chain (which otherwise must remain unconstrained so
   that real absolute/worktree paths, where `.factory` is not path component 0, still classify
   correctly) — closing the residual where a leading `..` (e.g.
   `../.factory/specs/behavioral-contracts/ss-05/BC-x.md`) would otherwise sit in that
   unconstrained prefix and still classify as governed. This is a classification-correctness
   requirement on this hook's own dispatch-side guards; it does not substitute for the
   sandbox's own traversal defense (`factory-dispatcher/src/host/path_util.rs::check_path_allowed`,
   Kani-proven VP-097), which independently canonicalizes and allow-lists paths downstream and
   fails closed on any real escape regardless of this hook's classification answer.

   **Exact-gap invariant (normative, v1.23).** The governed layout depth is fixed per artifact
   class, and this depth is itself a checked spec invariant, not an incidental implementation
   detail: BC files are ALWAYS exactly one `ss-NN` subdirectory under
   `.factory/specs/behavioral-contracts/` (`gap=1`); VP files sit FLAT directly under
   `.factory/specs/verification-properties/` (`gap=0`); epic files sit FLAT directly under
   `.factory/stories/epics/` (`gap=0`); and active story files sit FLAT directly under
   `.factory/stories/` (`gap=0`). A future change to the governed corpus layout (e.g. BC files
   nested two directories deep, or VP files moved into per-subsystem subdirectories) is a SPEC
   VIOLATION requiring a coordinated BC amendment plus a corresponding `gap` change in the
   implementation in the same burst — NOT a silent classifier no-op. Silently widening or
   narrowing `gap` without a corresponding BC amendment would either (a) stop matching real
   governed files at the new depth (silent file-drop — the exact failure class this
   contiguity-anchoring amendment exists to prevent, applied to the hook's own dispatch layer),
   or (b) admit files at the wrong depth as governed. Nested `.factory/stories/v1.0-legacy/
   S-*.md` archival stories are intentionally OUT of the flat-corpus (`gap=0`) classification
   scope for Class A Arm2 / Class B Arm1 / Class E story matching — the operative corpus is the
   non-recursive `.factory/stories/*.md` glob; the contiguity-anchored matcher's rejection of
   the deeper legacy-archive path is intended behavior, not a coverage gap.
...
| Contiguity-anchored decoy rejection (v1.23 / SEC-001 / CWE-697) | (a) Non-contiguous decoy `x/.factory/y/specs/z/behavioral-contracts/BC-1.2.3.md` (all three required chain segment names present, but interleaved with other segments and not immediately preceding an `ss-NN`/filename window). (b) `..`-after-chain decoy `.factory/specs/behavioral-contracts/../../../../tmp/evil/BC-1.2.3.md` (chain present and contiguous, but a `ParentDir` component appears after it). (c) Leading-`..` decoy `../.factory/specs/behavioral-contracts/ss-05/BC-1.2.3.md` (chain correctly anchored immediately before `ss-05`/filename, but a `ParentDir` component sits in the otherwise-unconstrained prefix) | NOT classified as a governed BC file for (a), (b), AND (c): `is_bc_file` returns `false` for all three; precondition 1 does not match; the hook does not fire on the write | A Arm1 / Invariant 3 (`chain_immediately_precedes_filename`) | Pre-v1.23 independent `.any()`-per-segment presence check (NON-CONFORMING): classifies (a) as a governed BC file because all three segment names are present somewhere in the path, regardless of contiguity, order, or intervening `..` — false positive on a decoy path | Positive control: real absolute/worktree path (e.g. `/Users/…/.worktrees/S-21.07/.factory/specs/behavioral-contracts/ss-05/BC-5.39.010.md`) → `is_bc_file` returns `true`; the `.factory`/`specs`/`behavioral-contracts` chain occupies the position immediately before the `ss-05` gap segment and the filename, with no `ParentDir` component anywhere in the path |
...
|---------|------|-------------|
| 1.24 | 2026-08-15 | **POL-14 auto-promotion at merge** (state-manager; S-21.07 PR #776 squash-merged to `develop` at commit `e94767bc` 2026-08-15). Top-level `status:` and `lifecycle_status:` fields draft→active. No behavioral/content change — Description, Preconditions, Postconditions, Invariants, Edge Cases, Canonical Test Vectors byte-identical to v1.23. BC-INDEX v4.62→v4.63 (row status column draft→active; version-chain cell +v1.24). |
| 1.23 | 2026-08-15 | **SEC-001 / CWE-697 contiguity-anchoring amendment** (human-authorized spec ratification: human directed research-validation of the shipped `chain_immediately_precedes_filename` implementation → verdict CORRECT, MITRE/OWASP/Rust-std cited → BC amended to ratify). The pre-v1.23 preconditions codified the CWE-697 "insufficient comparison" / independent-component-presence anti-pattern (`components().any(|c| ...)` evaluated separately per required directory segment); the validated implementation in `crates/hook-plugins/validate-cross-site-correspondence/src/dispatch.rs` already replaces this with contiguity-anchored, correctly-ordered chain matching plus outright rejection of any `Component::ParentDir` (`..`) anywhere in the path. Precondition 1 (Arm A1), precondition 9 (Arm A2/B1), and precondition 34 (Class E, both VP and epic arms) amended in place to require `chain_immediately_precedes_filename` contiguity-anchoring with `..`-rejection, replacing the independent `.any()` per-segment presence form. Invariant 3 extended to normatively require contiguity-anchored matching (not independent per-segment presence) plus `..`-rejection, and gains a new exact-gap sub-clause fixing the hardcoded gap values (BC files `gap=1`; VP/epic/active-story files `gap=0`) as a checked spec invariant. New Canonical Test Vector row added ("Contiguity-anchored decoy rejection") covering non-contiguous, `..`-after-chain, and leading-`..` decoys against a positive control, with the pre-v1.23 independent-`.any()` implementation as the killed mutant. No new EC (existing EC-023 precondition-scoping precedent judged sufficient). No BC H1 change (POLICY 7); no ID renumbering (POLICY 1 append-only) — this is a pure spec-side ratification of already-shipped, already-validated code; no implementer/test-writer obligation created. BC-INDEX version-cell sync owed to state-manager; S-21.07 story BC-table/Token Budget cite owed to story-writer. (product-owner; human-authorized SEC-001/CWE-697 spec-ratification amendment.) |
| 1.22 | 2026-08-14 | **ADV-RECON11-001 (LOW, near-zero-reachability)**: architect-adjudicated amendment adding a "Secondary Index-File UTF-8 Decode Failure" clause, DISTINCT from precondition 15a / postcondition 25 (which govern PRIMARY-target decode failure only). Gap: BC-INDEX.md (Arm A1 secondary read) and STORY-INDEX.md (Arm B1 secondary read only — NOT Arm B2, where STORY-INDEX.md is the primary target already covered by 15a/25) had no specified UTF-8 decode disposition — a non-UTF-8 BC-INDEX silently degraded to `RowAbsent`, triggering postcondition 4's BLOCK with a MISLEADING "dropped registration" message for any BC with `version:` > "1.0" (actual root cause: index-file corruption, not a missing row); a non-UTF-8 STORY-INDEX (Arm B1) silently degraded to `(None, None)`, triggering postcondition 12's fail-open "not yet registered" advisory (silently disabling three-way hash checking). Same diagnostic-accuracy defect class as F-S2107-P1B-016 (fixed for Arm A2's secondary BC-FILE read), applied asymmetrically to Arm A1/B1's secondary INDEX-FILE reads. New precondition 15b ("Secondary Index-File UTF-8 Decode Failure") added immediately after precondition 15a. New postcondition 26 requires a distinct `host::log_warn` advisory naming the index-file path and stating the row/hash state is INDETERMINATE (not confirmed-absent) + `HookResult::Continue` — MUST NOT fall through into `RowAbsent` or `(None, None)`. Disposition is ADVISORY, not block — deliberately lower severity than precondition 15a / postcondition 25's primary-target BLOCK, preserving the low-disruption posture appropriate to this LOW/near-zero-reachability gap. New EC-040 (combined Arm A1 + Arm B1 scenario) and new Canonical Test Vector row ("Secondary Index-File UTF-8 Decode Failure") added. Invariant 5 extended with a parenthetical clarifying its "NotFound only" title no longer fully describes secondary-target disposition scope. No BC H1 change (POLICY 7); no ID renumbering (POLICY 1 append-only) — precondition 15b, postcondition 26, EC-040, and the new test-vector row are all net-new appended IDs. BC-INDEX, the S-21.07 story, and the implementation (`extract_bc_index_version_state`'s UTF-8 fallback in `arm_a1.rs`; `parse_story_index_catalog_hash`/`parse_story_index_blockquote_hash`'s UTF-8 fallbacks in `arm_b.rs`) are NOT amended in this burst — BC-INDEX version-cell sync owed to state-manager; S-21.07 story BC-table/Token Budget cite + new AC owed to story-writer; IndexUnreadable-disposition implementation owed to implementer; RED-gate unit tests for both the Arm A1 non-UTF-8-BC-INDEX and Arm B1 non-UTF-8-STORY-INDEX scenarios owed to test-writer. (product-owner; architect ruling on ADV-RECON11-001; closes ADV-RECON11-001.) |
