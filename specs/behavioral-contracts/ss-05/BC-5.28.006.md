...
# Behavioral Contract BC-5.28.006: greenfield: cost monitoring (workflow-level)

## Description

Cost monitoring continuous through pipeline (per workflow description line 9-15). Per-step `model_tier` annotations route to T1/T2/T3 model tiers. ### Per-step BCs (greenfield)
...
1. Cost monitoring continuous through pipeline (per workflow description line 9-15).
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1465 |
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 691; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1465` |
| **Confidence** | high |
...
- **Source line(s) in workflow YAML:** workflow-level cost-monitoring (description claim, no explicit YAML field)
- **Behavior:** Cost monitoring continuous through pipeline (per workflow description line 9-15). Per-step `model_tier` annotations route to T1/T2/T3 model tiers.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — source-doc line 691
