...
# Behavioral Contract BC-5.25.007: phase-5:adversary-code-review (nested)

## Description

Reviews codebase for spec fidelity, test quality, code quality, security surface, spec gaps. Reports CONVERGENCE_REACHED if findings cosmetic only.
...
1. Reviews codebase for spec fidelity, test quality, code quality, security surface, spec gaps.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `adversary-code-review (line 21; lobster carve-out: stable anchor is step name `adversary-code-review`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1406 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `adversary-code-review (line 21; lobster carve-out: stable anchor is step name `adversary-code-review`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 524; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1406` |
| **Confidence** | high |
...
- **Step:** adversary-code-review (line 21; lobster carve-out: stable anchor is step name `adversary-code-review`, not line number)
- **Type:** agent
- **Agent:** adversary
- **Source line(s) in workflow YAML:** 21-35
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`adversary-code-review (line 21)`). Stable anchor is step name `adversary-code-review`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `adversary-code-review`. |
