...
# Behavioral Contract BC-5.30.010: feature:feature-market-intel

## Description

Workflow contract: feature:feature-market-intel.

## Preconditions

1. Upstream dependencies completed successfully: `[environment-check]`.
2. Conditional gate evaluates true: `request.intent != 'bug-fix'`.

## Postconditions

1. Step `feature-market-intel (line 136; lobster carve-out: stable anchor is step name `feature-market-intel`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `feature-market-intel`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `request.intent != 'bug-fix'` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `feature-market-intel (line 136; lobster carve-out: stable anchor is step name `feature-market-intel`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1589 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `feature-market-intel (line 136; lobster carve-out: stable anchor is step name `feature-market-intel`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1067; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1589` |
| **Confidence** | high |
...
- **Step:** feature-market-intel (line 136; lobster carve-out: stable anchor is step name `feature-market-intel`, not line number)
- **Type:** skill
- **Skill:** `skills/market-intelligence-assessment/SKILL.md`
- **Depends on:** `[environment-check]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`feature-market-intel (line 136)`). Stable anchor is step name `feature-market-intel`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `feature-market-intel`. |
