...
# Behavioral Contract BC-5.28.076: greenfield:session-review

## Description

Workflow contract: greenfield:session-review.

## Preconditions

1. Upstream dependencies completed successfully: `[post-feature-validation, release]`.

## Postconditions

1. Step `session-review (line 1379; lobster carve-out: stable anchor is step name `session-review`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `session-review`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `session-review (line 1379; lobster carve-out: stable anchor is step name `session-review`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1535 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `session-review (line 1379; lobster carve-out: stable anchor is step name `session-review`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 905; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1535` |
| **Confidence** | high |
...
- **Step:** session-review (line 1379; lobster carve-out: stable anchor is step name `session-review`, not line number)
- **Type:** skill
- **Skill:** `skills/session-review/SKILL.md`
- **Depends on:** `[post-feature-validation, release]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`session-review (line 1379)`). Stable anchor is step name `session-review`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `session-review`. |
