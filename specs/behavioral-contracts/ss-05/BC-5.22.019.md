...
# Behavioral Contract BC-5.22.019: phase-2:human-approval

## Description

Workflow contract: phase-2:human-approval.
...
1. Step `human-approval (line 158; lobster carve-out: stable anchor is step name `human-approval`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `human-approval (line 158; lobster carve-out: stable anchor is step name `human-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1358 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `human-approval (line 158; lobster carve-out: stable anchor is step name `human-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 388; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1358` |
| **Confidence** | high |
...
- **Step:** human-approval (line 158; lobster carve-out: stable anchor is step name `human-approval`, not line number)
- **Timeout:** 24h
- **Source line(s) in workflow YAML:** 158-171
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`human-approval (line 158)`). Stable anchor is step name `human-approval`.
