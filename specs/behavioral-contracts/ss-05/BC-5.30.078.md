...
# Behavioral Contract BC-5.30.078: feature:phase-f7-delta-convergence

## Description

Workflow contract: feature:phase-f7-delta-convergence.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f6-gate, phase-f7-ui-fix-delivery]`.

## Postconditions

1. Step `phase-f7-delta-convergence (line 1319; lobster carve-out: stable anchor is step name `phase-f7-delta-convergence`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f7-delta-convergence`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f7-delta-convergence (line 1319; lobster carve-out: stable anchor is step name `phase-f7-delta-convergence`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1657 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f7-delta-convergence (line 1319; lobster carve-out: stable anchor is step name `phase-f7-delta-convergence`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1273; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1657` |
| **Confidence** | high |
...
- **Step:** phase-f7-delta-convergence (line 1319; lobster carve-out: stable anchor is step name `phase-f7-delta-convergence`, not line number)
- **Type:** loop
- **Depends on:** `[phase-f6-gate, phase-f7-ui-fix-delivery]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f7-delta-convergence (line 1319)`). Stable anchor is step name `phase-f7-delta-convergence`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f7-delta-convergence`. |
