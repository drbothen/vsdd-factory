...
# Behavioral Contract BC-5.30.041: feature:phase-f2-multi-variant

## Description

Workflow contract: feature:phase-f2-multi-variant.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f2-ux-design]`.
2. Conditional gate evaluates true: `UI + `ux_delta.has_complex_screens == true`.

## Postconditions

1. Step `phase-f2-multi-variant (line 523; lobster carve-out: stable anchor is step name `phase-f2-multi-variant`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f2-multi-variant`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `UI + `ux_delta.has_complex_screens == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f2-multi-variant (line 523; lobster carve-out: stable anchor is step name `phase-f2-multi-variant`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1620 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f2-multi-variant (line 523; lobster carve-out: stable anchor is step name `phase-f2-multi-variant`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1161; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1620` |
| **Confidence** | high |
...
- **Step:** phase-f2-multi-variant (line 523; lobster carve-out: stable anchor is step name `phase-f2-multi-variant`, not line number)
- **Type:** skill
- **Skill:** `skills/multi-variant-design/SKILL.md`
- **Depends on:** `[phase-f2-ux-design]`
- **Condition:** UI + `ux_delta.has_complex_screens == true`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f2-multi-variant (line 523)`). Stable anchor is step name `phase-f2-multi-variant`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f2-multi-variant`. |
