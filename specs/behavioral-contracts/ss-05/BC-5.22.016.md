...
# Behavioral Contract BC-5.22.016: phase-2:decomposition-gate

## Description

Verifies BC traceability, no TBD/TODO in ACs, acyclic dependency graph, wave assignments respect dependencies, STORY-INDEX matches, ≥1 holdout scenario per wave.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-holdout-scenarios]`.

## Postconditions

1. Verifies BC traceability, no TBD/TODO in ACs, acyclic dependency graph, wave assignments respect dependencies, STORY-INDEX matches, ≥1 holdout scenario per wave.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `decomposition-gate (line 94; lobster carve-out: stable anchor is step name `decomposition-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1355 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `decomposition-gate (line 94; lobster carve-out: stable anchor is step name `decomposition-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 377; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1355` |
| **Confidence** | high |
...
- **Step:** decomposition-gate (line 94; lobster carve-out: stable anchor is step name `decomposition-gate`, not line number)
- **Type:** gate
- **Depends on:** `[backup-holdout-scenarios]`
- **Source line(s) in workflow YAML:** 94-105
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`decomposition-gate (line 94)`). Stable anchor is step name `decomposition-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `decomposition-gate`. |
