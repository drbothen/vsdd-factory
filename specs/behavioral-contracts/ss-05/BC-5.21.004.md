...
# Behavioral Contract BC-5.21.004: phase-1: DAG integrity
...
1. 14 steps acyclic.
2. Acceptance: Topological sort succeeds; spec-gate's two-parent fan-in correctly handles either presence/absence of `prd-revision`.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-1: DAG integrity` | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-1: DAG integrity`
