...
# Behavioral Contract BC-5.29.012: brownfield:state-initialization

## Description

Workflow contract: brownfield:state-initialization.
...
1. Step `state-initialization (line 110; lobster carve-out: stable anchor is step name `state-initialization`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `state-initialization (line 110; lobster carve-out: stable anchor is step name `state-initialization`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1551 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `state-initialization (line 110; lobster carve-out: stable anchor is step name `state-initialization`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 960; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1551` |
| **Confidence** | high |
...
- **Step:** state-initialization (line 110; lobster carve-out: stable anchor is step name `state-initialization`, not line number)
- **Type:** agent
- **Agent:** state-manager
- **Depends on:** `[factory-worktree-gate, scaffold-claude-md]`
- **Source line(s) in workflow YAML:** 110-118
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`state-initialization (line 110)`). Stable anchor is step name `state-initialization`.
