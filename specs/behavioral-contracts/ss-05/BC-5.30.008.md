...
# Behavioral Contract BC-5.30.008: feature:feature-cycle-init

## Description

Workflow contract: feature:feature-cycle-init.
...
1. Step `feature-cycle-init (line 105; lobster carve-out: stable anchor is step name `feature-cycle-init`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `feature-cycle-init`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `feature-cycle-init (line 105; lobster carve-out: stable anchor is step name `feature-cycle-init`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1587 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `feature-cycle-init (line 105; lobster carve-out: stable anchor is step name `feature-cycle-init`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1061; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1587` |
| **Confidence** | high |
...
- **Step:** feature-cycle-init (line 105; lobster carve-out: stable anchor is step name `feature-cycle-init`, not line number)
- **Type:** agent
- **Agent:** state-manager
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`feature-cycle-init (line 105)`). Stable anchor is step name `feature-cycle-init`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `feature-cycle-init`. |
