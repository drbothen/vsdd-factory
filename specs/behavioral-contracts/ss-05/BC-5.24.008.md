...
# Behavioral Contract BC-5.24.008: phase-4:holdout-gate

## Description

Verifies different model family used (GPT-5.4 not Claude), mean satisfaction ≥0.85, no must-pass scenario <0.6, std-dev <0.15, scenario rotation applied.
...
1. Verifies different model family used (GPT-5.4 not Claude), mean satisfaction ≥0.85, no must-pass scenario <0.6, std-dev <0.15, scenario rotation applied.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `holdout-gate (line 28; lobster carve-out: stable anchor is step name `holdout-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1397 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `holdout-gate (line 28; lobster carve-out: stable anchor is step name `holdout-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 495; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1397` |
| **Confidence** | high |
...
- **Step:** holdout-gate (line 28; lobster carve-out: stable anchor is step name `holdout-gate`, not line number)
- **Type:** gate
- **Source line(s) in workflow YAML:** 28-38
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`holdout-gate (line 28)`). Stable anchor is step name `holdout-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `holdout-gate`. |
