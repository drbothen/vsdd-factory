...
# Behavioral Contract BC-5.28.060: greenfield:phase-6-dtu-adversarial

## Description

Workflow contract: greenfield:phase-6-dtu-adversarial.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-6-formal-hardening]`.
2. Conditional gate evaluates true: `dtu_clones.has_l4 == true`.

## Postconditions

1. Step `phase-6-dtu-adversarial (line 1180; lobster carve-out: stable anchor is step name `phase-6-dtu-adversarial`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-6-dtu-adversarial`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `dtu_clones.has_l4 == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-6-dtu-adversarial (line 1180; lobster carve-out: stable anchor is step name `phase-6-dtu-adversarial`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1519 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-6-dtu-adversarial (line 1180; lobster carve-out: stable anchor is step name `phase-6-dtu-adversarial`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 857; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1519` |
| **Confidence** | high |
...
- **Step:** phase-6-dtu-adversarial (line 1180; lobster carve-out: stable anchor is step name `phase-6-dtu-adversarial`, not line number)
- **Type:** agent
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-6-dtu-adversarial (line 1180)`). Stable anchor is step name `phase-6-dtu-adversarial`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-6-dtu-adversarial`. |
