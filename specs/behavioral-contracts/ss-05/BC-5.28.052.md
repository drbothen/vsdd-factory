...
# Behavioral Contract BC-5.28.052: greenfield:phase-4-holdout-evaluation

## Description

Workflow contract: greenfield:phase-4-holdout-evaluation.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-4-scenario-rotation, phase-4-dtu-startup]`.

## Postconditions

1. Step `phase-4-holdout-evaluation (line 1020; lobster carve-out: stable anchor is step name `phase-4-holdout-evaluation`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-4-holdout-evaluation`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-4-holdout-evaluation (line 1020; lobster carve-out: stable anchor is step name `phase-4-holdout-evaluation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1511 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-4-holdout-evaluation (line 1020; lobster carve-out: stable anchor is step name `phase-4-holdout-evaluation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 833; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1511` |
| **Confidence** | high |
...
- **Step:** phase-4-holdout-evaluation (line 1020; lobster carve-out: stable anchor is step name `phase-4-holdout-evaluation`, not line number)
- **Type:** skill
- **Skill:** `skills/phase-4-holdout-evaluation/SKILL.md`
- **Depends on:** `[phase-4-scenario-rotation, phase-4-dtu-startup]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-4-holdout-evaluation (line 1020)`). Stable anchor is step name `phase-4-holdout-evaluation`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-4-holdout-evaluation`. |
