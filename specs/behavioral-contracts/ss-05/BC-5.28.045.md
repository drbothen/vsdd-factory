...
# Behavioral Contract BC-5.28.045: greenfield:pre-phase-4-cicd-gate

## Description

Workflow contract: greenfield:pre-phase-4-cicd-gate.
...
1. Step `pre-phase-4-cicd-gate (line 616; lobster carve-out: stable anchor is step name `pre-phase-4-cicd-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `pre-phase-4-cicd-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `pre-phase-4-cicd-gate (line 616; lobster carve-out: stable anchor is step name `pre-phase-4-cicd-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1504 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `pre-phase-4-cicd-gate (line 616; lobster carve-out: stable anchor is step name `pre-phase-4-cicd-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 811; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1504` |
| **Confidence** | high |
...
- **Step:** pre-phase-4-cicd-gate (line 616; lobster carve-out: stable anchor is step name `pre-phase-4-cicd-gate`, not line number)
- **Type:** gate
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`pre-phase-4-cicd-gate (line 616)`). Stable anchor is step name `pre-phase-4-cicd-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `pre-phase-4-cicd-gate`. |
