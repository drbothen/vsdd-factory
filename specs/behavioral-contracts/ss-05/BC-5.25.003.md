...
# Behavioral Contract BC-5.25.003: phase-5: terminal-step

## Description

Workflow contract: phase-5: terminal-step.
...
1. Step ``gemini-secondary-review` (line 41; lobster carve-out: stable anchor is step name `gemini-secondary-review`, not line number) — depends `[adversarial-review-loop]`. Conditional: `config.enable_secondary_adversary == true`.` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `gemini-secondary-review`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``gemini-secondary-review` (line 41; lobster carve-out: stable anchor is step name `gemini-secondary-review`, not line number) — depends `[adversarial-review-loop]`. Conditional: `config.enable_secondary_adversary == true`.` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1402 |

## Related BCs (Recommended)

- TBD — sibling step contracts in section BC-5.25

## Architecture Anchors (Recommended)

- `architecture/ss-05-pipeline-orchestration.md#workflow-phase-5-adversarial-refinement` — workflow declaration source
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``gemini-secondary-review` (line 41; lobster carve-out: stable anchor is step name `gemini-secondary-review`, not line number) — depends `[adversarial-review-loop]`. Conditional: `config.enable_secondary_adversary == true`.`
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/phases/phase-5-adversarial-refinement.lobster` |
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 511; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1402` |
| **Confidence** | high |
...
- **Step:** `gemini-secondary-review` (line 41; lobster carve-out: stable anchor is step name `gemini-secondary-review`, not line number) — depends `[adversarial-review-loop]`. Conditional: `config.enable_secondary_adversary == true`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``gemini-secondary-review` (line 41)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `gemini-secondary-review`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `gemini-secondary-review`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `gemini-secondary-review`. |
