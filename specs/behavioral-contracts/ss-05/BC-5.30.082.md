...
# Behavioral Contract BC-5.30.082: feature:release

## Description

Workflow contract: feature:release.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f7-human-approval]`.

## Postconditions

1. Step `release (line 1404; lobster carve-out: stable anchor is step name `release`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `release (line 1404; lobster carve-out: stable anchor is step name `release`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1661 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `release (line 1404; lobster carve-out: stable anchor is step name `release`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1285; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1661` |
| **Confidence** | high |
...
- **Step:** release (line 1404; lobster carve-out: stable anchor is step name `release`, not line number)
- **Type:** skill
- **Skill:** `skills/release/SKILL.md`
- **Depends on:** `[phase-f7-human-approval]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`release (line 1404)`). Stable anchor is step name `release`.
