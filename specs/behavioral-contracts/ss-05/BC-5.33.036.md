...
Step `maintenance-gate` (line 373; lobster carve-out: stable anchor is step name `maintenance-gate`, not line number). Type: gate. Depends: `[maintenance-report, state-final]`. fail_action: warn. Source 373-381. <!-- F-P22-001: lobster-line-cite deferred per pass-21/22 carve-out; line range preserved as source evidence; lobster files have stable section structure (step `maintenance-gate` is the stable anchor); migrate to step-name anchor in future sweep -->
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/maintenance.lobster` (lines 373-381; lobster path carve-out: line range is unstable as lobster files evolve) |
| **Confidence** | high |
...
**Driver:** F-P22-001 pass-22 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 semantic-pattern-class discipline) — §Description cited lobster step by line number (`(line 373)`) and source range (`Source 373-381.`). These are lobster-file references and fall under the lobster-line-cite carve-out exception; the stable anchor is the step name `maintenance-gate`, not the line number.

**Change made:**
- §Description: inline annotation added noting lobster carve-out (stable anchor = step name `maintenance-gate`); HTML carve-out comment added citing F-P22-001 deferral.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster path lines 373-381
