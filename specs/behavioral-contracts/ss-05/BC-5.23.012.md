...
# Behavioral Contract BC-5.23.012: phase-3:implement

## Description

TDD inner loop — pick next failing test, write minimum code, repeat.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-failing-tests]`.

## Postconditions

1. TDD inner loop — pick next failing test, write minimum code, repeat.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `implement (line 62; lobster carve-out: stable anchor is step name `implement`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1371 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `implement (line 62; lobster carve-out: stable anchor is step name `implement`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 434; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1371` |
| **Confidence** | high |
...
- **Step:** implement (line 62; lobster carve-out: stable anchor is step name `implement`, not line number)
- **Type:** skill
- **Skill:** `step-d-implement.md`
- **Depends on:** `[backup-failing-tests]`
- **Timeout:** 2h
- **Source line(s) in workflow YAML:** 62-66
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`implement (line 62)`). Stable anchor is step name `implement`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `implement`. |
