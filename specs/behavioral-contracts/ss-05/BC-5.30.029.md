...
# Behavioral Contract BC-5.30.029: feature:bugfix-f6-a11y

## Description

Workflow contract: feature:bugfix-f6-a11y.

## Preconditions

1. Upstream dependencies completed successfully: `[bugfix-f6-scoped]`.
2. Conditional gate evaluates true: `delta.intent == 'bug-fix' and f1.feature_type in ['ui','full-stack']`.

## Postconditions

1. Step `bugfix-f6-a11y (line 381; lobster carve-out: stable anchor is step name `bugfix-f6-a11y`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `bugfix-f6-a11y`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `delta.intent == 'bug-fix' and f1.feature_type in ['ui','full-stack']` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `bugfix-f6-a11y (line 381; lobster carve-out: stable anchor is step name `bugfix-f6-a11y`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1608 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `bugfix-f6-a11y (line 381; lobster carve-out: stable anchor is step name `bugfix-f6-a11y`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1125; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1608` |
| **Confidence** | high |
...
- **Step:** bugfix-f6-a11y (line 381; lobster carve-out: stable anchor is step name `bugfix-f6-a11y`, not line number)
- **Type:** agent
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`bugfix-f6-a11y (line 381)`). Stable anchor is step name `bugfix-f6-a11y`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `bugfix-f6-a11y`. |
