...
# Behavioral Contract BC-5.30.071: feature:phase-f6-demo-recording

## Description

Workflow contract: feature:phase-f6-demo-recording.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f6-gate]`.
2. Conditional gate evaluates true: `config.demo_recording.enabled != false`.

## Postconditions

1. Step `phase-f6-demo-recording (line 1241; lobster carve-out: stable anchor is step name `phase-f6-demo-recording`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f6-demo-recording`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f6-demo-recording (line 1241; lobster carve-out: stable anchor is step name `phase-f6-demo-recording`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1650 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f6-demo-recording (line 1241; lobster carve-out: stable anchor is step name `phase-f6-demo-recording`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1252; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1650` |
| **Confidence** | high |
...
- **Step:** phase-f6-demo-recording (line 1241; lobster carve-out: stable anchor is step name `phase-f6-demo-recording`, not line number)
- **Type:** skill
- **Skill:** `skills/demo-recording/SKILL.md`
- **Depends on:** `[phase-f6-gate]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f6-demo-recording (line 1241)`). Stable anchor is step name `phase-f6-demo-recording`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f6-demo-recording`. |
