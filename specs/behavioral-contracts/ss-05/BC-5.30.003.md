...
# Behavioral Contract BC-5.30.003: feature: terminal-step

## Description

Workflow contract: feature: terminal-step.
...
1. Step ``process-review-decisions` (line 1479; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`.` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``process-review-decisions` (line 1479; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`.` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1582 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``process-review-decisions` (line 1479; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`.`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1039; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1582` |
| **Confidence** | high |
...
- **Step:** `process-review-decisions` (line 1479; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number) — agent: state-manager, depends `[session-review-approval]`.
