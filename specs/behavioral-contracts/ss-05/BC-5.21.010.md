...
# Behavioral Contract BC-5.21.010: phase-1:create-prd

## Description

Produces PRD and behavioral contracts (BC-S.SS.NNN format).

## Preconditions

1. Upstream dependencies completed successfully: `[backup-create-domain-spec]`.

## Postconditions

1. Produces PRD and behavioral contracts (BC-S.SS.NNN format).
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``create-prd` (line 48; lobster carve-out: stable anchor is step name `create-prd`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1329 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``create-prd` (line 48; lobster carve-out: stable anchor is step name `create-prd`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 255; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1329` |
| **Confidence** | high |
...
- **Step:** `create-prd` (line 48; lobster carve-out: stable anchor is step name `create-prd`, not line number)
- **Type:** skill
- **Skill:** `skills/create-prd/SKILL.md`
- **Depends on:** `[backup-create-domain-spec]`
- **Source line(s) in workflow YAML:** 48-51
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`create-prd (line 48)`). Stable anchor is step name `create-prd`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `create-prd` (line 48); double-backtick VP/anchor: step `create-prd` (line 48); **Step:** bt form: step `create-prd` (line 48)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `create-prd`. |
