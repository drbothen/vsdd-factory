...
# Behavioral Contract BC-5.30.018: feature:quick-dev-regression

## Description

Workflow contract: feature:quick-dev-regression.

## Preconditions

1. Upstream dependencies completed successfully: `[quick-dev-single-story]`.
2. Conditional gate evaluates true: `same`.

## Postconditions

1. Step `quick-dev-regression (line 247; lobster carve-out: stable anchor is step name `quick-dev-regression`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `quick-dev-regression`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `quick-dev-regression (line 247; lobster carve-out: stable anchor is step name `quick-dev-regression`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1597 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `quick-dev-regression (line 247; lobster carve-out: stable anchor is step name `quick-dev-regression`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1092; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1597` |
| **Confidence** | high |
...
- **Step:** quick-dev-regression (line 247; lobster carve-out: stable anchor is step name `quick-dev-regression`, not line number)
- **Type:** agent
- **Agent:** implementer
- **Depends on:** `[quick-dev-single-story]`
- **Condition:** same
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`quick-dev-regression (line 247)`). Stable anchor is step name `quick-dev-regression`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `quick-dev-regression`. |
