...
# Behavioral Contract BC-5.28.015: greenfield:prd-revision

## Description

Workflow contract: greenfield:prd-revision.

## Preconditions

1. Upstream dependencies completed successfully: `[architect-feasibility-review]`.
2. Conditional gate evaluates true: `architect.verdict == 'request-changes'`.

## Postconditions

1. Step `prd-revision (line 124; lobster carve-out: stable anchor is step name `prd-revision`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `prd-revision`; line number is unstable as lobster files evolve -->
...
| EC-003 | Condition `architect.verdict == 'request-changes'` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `prd-revision (line 124; lobster carve-out: stable anchor is step name `prd-revision`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1474 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `prd-revision (line 124; lobster carve-out: stable anchor is step name `prd-revision`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 720; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1474` |
| **Confidence** | high |
...
- **Step:** prd-revision (line 124; lobster carve-out: stable anchor is step name `prd-revision`, not line number)
- **Type:** skill
- **Skill:** `skills/phase-1-prd-revision/SKILL.md`
- **Depends on:** `[architect-feasibility-review]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`prd-revision (line 124)`). Stable anchor is step name `prd-revision`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `prd-revision`. |
