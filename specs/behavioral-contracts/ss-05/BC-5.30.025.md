...
# Behavioral Contract BC-5.30.025: feature:bugfix-build-verification

## Description

Workflow contract: feature:bugfix-build-verification.

## Preconditions

1. Upstream dependencies completed successfully: `[bugfix-single-story-delivery]`.
2. Conditional gate evaluates true: `delta.intent == 'bug-fix'`.

## Postconditions

1. Step `bugfix-build-verification (line 319; lobster carve-out: stable anchor is step name `bugfix-build-verification`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `bugfix-build-verification`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `bugfix-build-verification (line 319; lobster carve-out: stable anchor is step name `bugfix-build-verification`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1604 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `bugfix-build-verification (line 319; lobster carve-out: stable anchor is step name `bugfix-build-verification`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1113; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1604` |
| **Confidence** | high |
...
- **Step:** bugfix-build-verification (line 319; lobster carve-out: stable anchor is step name `bugfix-build-verification`, not line number)
- **Type:** agent
- **Agent:** implementer
- **Depends on:** `[bugfix-single-story-delivery]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`bugfix-build-verification (line 319)`). Stable anchor is step name `bugfix-build-verification`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `bugfix-build-verification`. |
