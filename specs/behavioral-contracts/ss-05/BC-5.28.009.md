...
# Behavioral Contract BC-5.28.009: greenfield:factory-worktree-gate

## Description

Workflow contract: greenfield:factory-worktree-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[factory-worktree-health]`.

## Postconditions

1. Step `factory-worktree-gate (line 57; lobster carve-out: stable anchor is step name `factory-worktree-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `factory-worktree-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `factory-worktree-gate (line 57; lobster carve-out: stable anchor is step name `factory-worktree-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1468 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `factory-worktree-gate (line 57; lobster carve-out: stable anchor is step name `factory-worktree-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 702; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1468` |
| **Confidence** | high |
...
- **Step:** factory-worktree-gate (line 57; lobster carve-out: stable anchor is step name `factory-worktree-gate`, not line number)
- **Type:** gate
- **Depends on:** `[factory-worktree-health]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`factory-worktree-gate (line 57)`). Stable anchor is step name `factory-worktree-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `factory-worktree-gate`. |
