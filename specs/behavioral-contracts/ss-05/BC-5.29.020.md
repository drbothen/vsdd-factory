...
# Behavioral Contract BC-5.29.020: brownfield:detect-cross-language-porting

## Description

Workflow contract: brownfield:detect-cross-language-porting.
...
1. Step `detect-cross-language-porting (line 242; lobster carve-out: stable anchor is step name `detect-cross-language-porting`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `detect-cross-language-porting`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `detect-cross-language-porting (line 242; lobster carve-out: stable anchor is step name `detect-cross-language-porting`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1559 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `detect-cross-language-porting (line 242; lobster carve-out: stable anchor is step name `detect-cross-language-porting`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 987; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1559` |
| **Confidence** | high |
...
- **Step:** detect-cross-language-porting (line 242; lobster carve-out: stable anchor is step name `detect-cross-language-porting`, not line number)
- **Type:** agent
- **Agent:** architect
- **Depends on:** `[brownfield-market-review]`
- **Condition:** `routing.choice == 'feature'`
- **Source line(s) in workflow YAML:** 242-255
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`detect-cross-language-porting (line 242)`). Stable anchor is step name `detect-cross-language-porting`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `detect-cross-language-porting`. |
