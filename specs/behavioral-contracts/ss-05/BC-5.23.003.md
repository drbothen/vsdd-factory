...
# Behavioral Contract BC-5.23.003: phase-3: terminal-step

## Description

Final step verifies spec artifacts have not drifted since implementation began; no human-approval (per-story flow).

## Preconditions

1. Upstream dependencies completed successfully: `[implementation-gate]`.

## Postconditions

1. Final step verifies spec artifacts have not drifted since implementation began; no human-approval (per-story flow).
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``input-hash-drift-check` (line 138; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1362 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``input-hash-drift-check` (line 138; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 403; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1362` |
| **Confidence** | high |
...
- **Step:** `input-hash-drift-check` (line 138; lobster carve-out: stable anchor is step name `input-hash-drift-check`, not line number)
- **Depends on:** `[implementation-gate]`
- **Source line(s) in workflow YAML:** 138-144
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`input-hash-drift-check (line 138)`). Stable anchor is step name `input-hash-drift-check`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `input-hash-drift-check` (line 138); double-backtick VP/anchor: step `input-hash-drift-check` (line 138); **Step:** bt form: step `input-hash-drift-check` (line 138)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `input-hash-drift-check`. |
