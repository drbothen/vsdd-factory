...
# Behavioral Contract BC-5.24.002: phase-4: entry-point

## Description

Workflow contract: phase-4: entry-point.
...
1. Step ``scenario-rotation` (line 14; lobster carve-out: stable anchor is step name `scenario-rotation`, not line number) — `depends_on: []`. Agent: orchestrator.` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `scenario-rotation`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``scenario-rotation` (line 14; lobster carve-out: stable anchor is step name `scenario-rotation`, not line number) — `depends_on: []`. Agent: orchestrator.` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1391 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``scenario-rotation` (line 14; lobster carve-out: stable anchor is step name `scenario-rotation`, not line number) — `depends_on: []`. Agent: orchestrator.`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 476; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1391` |
| **Confidence** | high |
...
- **Step:** `scenario-rotation` (line 14; lobster carve-out: stable anchor is step name `scenario-rotation`, not line number) — `depends_on: []`. Agent: orchestrator.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``scenario-rotation` (line 14)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `scenario-rotation`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `scenario-rotation`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `scenario-rotation`. |
