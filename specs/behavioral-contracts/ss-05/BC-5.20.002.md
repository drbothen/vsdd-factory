...
# Behavioral Contract BC-5.20.002: phase-0-codebase-ingestion: entry-point

## Description

The single root step (no upstream dependencies). It is the first invocation when this phase begins. Topological sort of this workflow has `source-acquisition` as the unique zero-in-degree node.
...
1. The single root step (no upstream dependencies).
2. Acceptance: Topological sort of this workflow has `source-acquisition` as the unique zero-in-degree node.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``source-acquisition` (line 18; lobster carve-out: stable anchor is step name `source-acquisition`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1301 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``source-acquisition` (line 18; lobster carve-out: stable anchor is step name `source-acquisition`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 47; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1301` |
| **Confidence** | high |
...
- **Step:** `source-acquisition` (line 18; lobster carve-out: stable anchor is step name `source-acquisition`, not line number)
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`source-acquisition (line 18)`). Stable anchor is step name `source-acquisition`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `source-acquisition` (line 18); double-backtick VP/anchor: step `source-acquisition` (line 18); **Step:** bt form: step `source-acquisition` (line 18)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `source-acquisition`. |
