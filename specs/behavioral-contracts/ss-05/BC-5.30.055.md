...
# Behavioral Contract BC-5.30.055: feature:phase-f4-delta-implementation

## Description

Per-wave loop iterating over stories; per-story delivery via `code-delivery.lobster`.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f3-human-approval, toolchain-preflight]`.

## Postconditions

1. Per-wave loop iterating over stories; per-story delivery via `code-delivery.lobster`.

## Invariants

1. Step execution must complete within timeout: 8h.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f4-delta-implementation (line 764; lobster carve-out: stable anchor is step name `phase-f4-delta-implementation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1634 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f4-delta-implementation (line 764; lobster carve-out: stable anchor is step name `phase-f4-delta-implementation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1203; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1634` |
| **Confidence** | high |
...
- **Step:** phase-f4-delta-implementation (line 764; lobster carve-out: stable anchor is step name `phase-f4-delta-implementation`, not line number)
- **Type:** loop
- **Depends on:** `[phase-f3-human-approval, toolchain-preflight]`
- **Timeout:** 8h
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f4-delta-implementation (line 764)`). Stable anchor is step name `phase-f4-delta-implementation`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f4-delta-implementation`. |
