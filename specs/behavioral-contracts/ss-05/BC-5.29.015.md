...
# Behavioral Contract BC-5.29.015: brownfield:phase-0-gate

## Description

Verifies 11 criteria (project-context, recovered-architecture, conventions, BCs in BC-S.SS.NNN with `origin: recovered`, verification-gap-analysis, module-criticality, security-audit, adversarial-review, validation-report, holdout scenarios seeded).

## Preconditions

1. Upstream dependencies completed successfully: `[phase-0-artifact-backup]`.

## Postconditions

1. Verifies 11 criteria (project-context, recovered-architecture, conventions, BCs in BC-S.SS.NNN with `origin: recovered`, verification-gap-analysis, module-criticality, security-audit, adversarial-review, validation-report, holdout scenarios seeded).
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-0-gate (line 151; lobster carve-out: stable anchor is step name `phase-0-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1554 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-0-gate (line 151; lobster carve-out: stable anchor is step name `phase-0-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 970; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1554` |
| **Confidence** | high |
...
- **Step:** phase-0-gate (line 151; lobster carve-out: stable anchor is step name `phase-0-gate`, not line number)
- **Type:** gate
- **Depends on:** `[phase-0-artifact-backup]`
- **Source line(s) in workflow YAML:** 151-167
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-0-gate (line 151)`). Stable anchor is step name `phase-0-gate`.
