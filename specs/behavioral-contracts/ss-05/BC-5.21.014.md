...
# Behavioral Contract BC-5.21.014: phase-1:prd-revision

## Description

Conditional architect-feedback PRD revision loop.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-create-architecture]`.
2. Conditional gate evaluates true: `exists('.factory/specs/architecture-feasibility-report.md')`.

## Postconditions

1. Conditional architect-feedback PRD revision loop.
...
| EC-003 | Condition `exists('.factory/specs/architecture-feasibility-report.md')` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``prd-revision` (line 78; lobster carve-out: stable anchor is step name `prd-revision`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1333 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``prd-revision` (line 78; lobster carve-out: stable anchor is step name `prd-revision`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 279; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1333` |
| **Confidence** | high |
...
- **Step:** `prd-revision` (line 78; lobster carve-out: stable anchor is step name `prd-revision`, not line number)
- **Type:** skill
- **Skill:** `skills/phase-1-prd-revision/SKILL.md`
- **Depends on:** `[backup-create-architecture]`
- **Condition:** `exists('.factory/specs/architecture-feasibility-report.md')`
- **Source line(s) in workflow YAML:** 78-82
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`prd-revision (line 78)`). Stable anchor is step name `prd-revision`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `prd-revision` (line 78); double-backtick VP/anchor: step `prd-revision` (line 78); **Step:** bt form: step `prd-revision` (line 78)
