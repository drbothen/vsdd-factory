...
# Behavioral Contract BC-5.30.054: feature:toolchain-preflight

## Description

Workflow contract: feature:toolchain-preflight.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f3-human-approval]`.
2. Conditional gate evaluates true: `toolchain.recently_checked == false`.

## Postconditions

1. Step `toolchain-preflight (line 747; lobster carve-out: stable anchor is step name `toolchain-preflight`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `toolchain-preflight`; line number is unstable as lobster files evolve -->

## Invariants

1. Step execution must complete within timeout: 15m.
...
| EC-003 | Condition `toolchain.recently_checked == false` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `toolchain-preflight (line 747; lobster carve-out: stable anchor is step name `toolchain-preflight`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1633 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `toolchain-preflight (line 747; lobster carve-out: stable anchor is step name `toolchain-preflight`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1200; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1633` |
| **Confidence** | high |
...
- **Step:** toolchain-preflight (line 747; lobster carve-out: stable anchor is step name `toolchain-preflight`, not line number)
- **Type:** agent
- **Agent:** dx-engineer
- **Depends on:** `[phase-f3-human-approval]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`toolchain-preflight (line 747)`). Stable anchor is step name `toolchain-preflight`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `toolchain-preflight`. |
