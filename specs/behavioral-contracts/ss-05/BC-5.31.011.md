...
Step `per-story-adversarial-review` (line 101; lobster carve-out: stable anchor is step name `per-story-adversarial-review`, not line number). Type: loop. max_iterations: 10. Depends: `[implement]`. Source 101-145. Behavior: spawns adversary on changed files only with extensive context exclusions (no implementer notes, red-gate-log, prior adversary history, semport history, holdout scenarios). <!-- F-P22-001: lobster-line-cite deferred per pass-21/22 carve-out; line range preserved as source evidence; lobster files have stable section structure (step `per-story-adversarial-review` is the stable anchor); migrate to step-name anchor in future sweep -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Loop terminates in ≤10 iterations | manual |
...
**Driver:** F-P22-001 pass-22 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 semantic-pattern-class discipline) — §Description cited lobster step by line number (`(line 101)`) and source range (`Source 101-145.`). These are lobster-file references and fall under the lobster-line-cite carve-out exception; the stable anchor is the step name `per-story-adversarial-review`, not the line number.
