...
# Behavioral Contract BC-5.28.022: greenfield:phase-1-multi-variant-approval

## Description

Workflow contract: greenfield:phase-1-multi-variant-approval.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-1-multi-variant-design]`.
2. Conditional gate evaluates true: `same`.

## Postconditions

1. Step `phase-1-multi-variant-approval (line 200; lobster carve-out: stable anchor is step name `phase-1-multi-variant-approval`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-1-multi-variant-approval`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-1-multi-variant-approval (line 200; lobster carve-out: stable anchor is step name `phase-1-multi-variant-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1481 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-1-multi-variant-approval (line 200; lobster carve-out: stable anchor is step name `phase-1-multi-variant-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 741; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1481` |
| **Confidence** | high |
...
- **Step:** phase-1-multi-variant-approval (line 200; lobster carve-out: stable anchor is step name `phase-1-multi-variant-approval`, not line number)
- **Type:** human-approval
- **Depends on:** `[phase-1-multi-variant-design]`
- **Condition:** same
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-1-multi-variant-approval (line 200)`). Stable anchor is step name `phase-1-multi-variant-approval`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-1-multi-variant-approval`. |
