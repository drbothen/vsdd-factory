...
Step `compute-repo-waves` (line 79; lobster carve-out: stable anchor is step name `compute-repo-waves`, not line number). Type: agent. Agent: orchestrator. Depends: `[read-project-manifest]`. Source 79-89. Behavior: Kahn's algorithm at repo level. Detects circular dependencies. Distinguishes contract dependencies from generation dependencies. <!-- F-P22-001: lobster-line-cite deferred per pass-21/22 carve-out; line range preserved as source evidence; lobster files have stable section structure (step `compute-repo-waves` is the stable anchor); migrate to step-name anchor in future sweep -->
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/workflows/multi-repo.lobster` (lines 79-89; lobster path carve-out: line range is unstable as lobster files evolve) |
| **Confidence** | high |
...
**Driver:** F-P22-001 pass-22 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 semantic-pattern-class discipline) — §Description cited lobster step by line number (`(line 79)`) and source range (`Source 79-89.`). These are lobster-file references and fall under the lobster-line-cite carve-out exception; the stable anchor is the step name `compute-repo-waves`, not the line number.

**Change made:**
- §Description: inline annotation added noting lobster carve-out (stable anchor = step name `compute-repo-waves`); HTML carve-out comment added citing F-P22-001 deferral.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster path lines 79-89
