...
# Behavioral Contract BC-5.28.078: greenfield:process-review-decisions

## Description

Workflow contract: greenfield:process-review-decisions.

## Preconditions

1. Upstream dependencies completed successfully: `[session-review-approval]`.

## Postconditions

1. Step `process-review-decisions (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `process-review-decisions (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1537 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `process-review-decisions (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 911; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1537` |
| **Confidence** | high |
...
- **Step:** process-review-decisions (line 1398; lobster carve-out: stable anchor is step name `process-review-decisions`, not line number)
- **Type:** agent
- **Agent:** state-manager
- **Depends on:** `[session-review-approval]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`process-review-decisions (line 1398)`). Stable anchor is step name `process-review-decisions`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `process-review-decisions`. |
