...
# Behavioral Contract BC-5.20.016: phase-0:final-synthesis

## Description

Produces the unified pass-8 synthesis document and project-context artifacts.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-extraction-validation]`.

## Postconditions

1. Produces the unified pass-8 synthesis document and project-context artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``final-synthesis` (line 97; lobster carve-out: stable anchor is step name `final-synthesis`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1315 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``final-synthesis` (line 97; lobster carve-out: stable anchor is step name `final-synthesis`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 159; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1315` |
| **Confidence** | high |
...
- **Step:** `final-synthesis` (line 97; lobster carve-out: stable anchor is step name `final-synthesis`, not line number)
- **Type:** skill
- **Skill:** `step-f-final-synthesis.md`
- **Depends on:** `[backup-extraction-validation]`
- **Source line(s) in workflow YAML:** 97-101
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`final-synthesis (line 97)`). Stable anchor is step name `final-synthesis`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `final-synthesis` (line 97); double-backtick VP/anchor: step `final-synthesis` (line 97); **Step:** bt form: step `final-synthesis` (line 97)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `final-synthesis`. |
