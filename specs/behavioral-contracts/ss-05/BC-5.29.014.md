...
# Behavioral Contract BC-5.29.014: brownfield:phase-0-artifact-backup

## Description

Workflow contract: brownfield:phase-0-artifact-backup.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-0-codebase-ingestion]`.

## Postconditions

1. Step `phase-0-artifact-backup (line 143; lobster carve-out: stable anchor is step name `phase-0-artifact-backup`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-0-artifact-backup`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-0-artifact-backup (line 143; lobster carve-out: stable anchor is step name `phase-0-artifact-backup`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1553 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-0-artifact-backup (line 143; lobster carve-out: stable anchor is step name `phase-0-artifact-backup`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 967; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1553` |
| **Confidence** | high |
...
- **Step:** phase-0-artifact-backup (line 143; lobster carve-out: stable anchor is step name `phase-0-artifact-backup`, not line number)
- **Type:** agent
- **Agent:** state-manager
- **Depends on:** `[phase-0-codebase-ingestion]`
- **Source line(s) in workflow YAML:** 143-149
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-0-artifact-backup (line 143)`). Stable anchor is step name `phase-0-artifact-backup`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-0-artifact-backup`. |
