...
# Behavioral Contract BC-5.29.026: brownfield:greenfield-pipeline

## Description

Workflow contract: brownfield:greenfield-pipeline.

## Preconditions

1. Upstream dependencies completed successfully: `[brownfield-to-greenfield-transition]`.
2. Conditional gate evaluates true: `routing.choice == 'feature'`.

## Postconditions

1. Step `greenfield-pipeline (line 335; lobster carve-out: stable anchor is step name `greenfield-pipeline`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `greenfield-pipeline`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `greenfield-pipeline (line 335; lobster carve-out: stable anchor is step name `greenfield-pipeline`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1565 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `greenfield-pipeline (line 335; lobster carve-out: stable anchor is step name `greenfield-pipeline`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1006; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1565` |
| **Confidence** | high |
...
- **Step:** greenfield-pipeline (line 335; lobster carve-out: stable anchor is step name `greenfield-pipeline`, not line number)
- **Type:** sub-workflow
- **Depends on:** `[brownfield-to-greenfield-transition]`
- **Condition:** `routing.choice == 'feature'`
- **Source line(s) in workflow YAML:** 335-339
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`greenfield-pipeline (line 335)`). Stable anchor is step name `greenfield-pipeline`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `greenfield-pipeline`. |
