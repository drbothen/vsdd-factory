...
# Behavioral Contract BC-5.28.051: greenfield:phase-4-dtu-startup

## Description

Workflow contract: greenfield:phase-4-dtu-startup.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-4-scenario-rotation]`.
2. Conditional gate evaluates true: `dtu_assessment.has_candidates == true`.

## Postconditions

1. Step `phase-4-dtu-startup (line 1012; lobster carve-out: stable anchor is step name `phase-4-dtu-startup`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-4-dtu-startup`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-4-dtu-startup (line 1012; lobster carve-out: stable anchor is step name `phase-4-dtu-startup`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1510 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-4-dtu-startup (line 1012; lobster carve-out: stable anchor is step name `phase-4-dtu-startup`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 830; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1510` |
| **Confidence** | high |
...
- **Step:** phase-4-dtu-startup (line 1012; lobster carve-out: stable anchor is step name `phase-4-dtu-startup`, not line number)
- **Type:** agent
- **Agent:** devops-engineer
- **Depends on:** `[phase-4-scenario-rotation]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-4-dtu-startup (line 1012)`). Stable anchor is step name `phase-4-dtu-startup`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-4-dtu-startup`. |
