...
# Behavioral Contract BC-5.20.003: phase-0-codebase-ingestion: terminal-step

## Description

Final gate. Presents the project context document, recovered architecture, conventions, security audit, validation report, and module-criticality classification for human approval; 24h timeout. No step in the workflow has `human-approval` in its `depends_on` list. The step prompt asks "Review the project context document. Is the recovered architecture accurate?".

## Preconditions

1. Upstream dependencies completed successfully: `[input-hash-drift-check]`.

## Postconditions

1. Final gate.
2. Acceptance: No step in the workflow has `human-approval` in its `depends_on` list. The step prompt asks "Review the project context document. Is the recovered architecture accurate?".
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``human-approval` (line 134; lobster carve-out: stable anchor is step name `human-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1302 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``human-approval` (line 134; lobster carve-out: stable anchor is step name `human-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 58; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1302` |
| **Confidence** | high |
...
- **Step:** `human-approval` (line 134; lobster carve-out: stable anchor is step name `human-approval`, not line number)
- **Type:** human-approval
- **Depends on:** `[input-hash-drift-check]`
- **Source line(s) in workflow YAML:** 134-146
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`human-approval (line 134)`). Stable anchor is step name `human-approval`.
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — double-backtick VP/anchor: step `human-approval` (line 134); double-backtick VP/anchor: step `human-approval` (line 134); **Step:** bt form: step `human-approval` (line 134)
...
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `human-approval`. |
