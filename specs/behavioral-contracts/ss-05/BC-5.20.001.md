...
# Behavioral Contract BC-5.20.001: phase-0-codebase-ingestion: identity

## Description

Declares a brownfield-mode Phase 0 workflow named `phase-0-codebase-ingestion` (v3.0.0) that analyses an existing codebase via the broad-then-converge protocol — source acquisition, broad sweep (7 passes), convergence deepening, coverage audit, extraction validation, final synthesis — with state-manager commits between every step for crash recovery.
...
1. Declares a brownfield-mode Phase 0 workflow named `phase-0-codebase-ingestion` (v3.0.0) that analyses an existing codebase via the broad-then-converge protocol — source acquisition, broad sweep (7 passes), convergence deepening, coverage audit, extraction validation, final synthesis — with state-manager commits between every step for crash recovery.
2. Acceptance: A loader that reads this workflow exposes `name=phase-0-codebase-ingestion`, `version=3.0.0`, and the description string.
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1300 |
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 37; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1300` |
| **Confidence** | high |
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — source-doc line 37
