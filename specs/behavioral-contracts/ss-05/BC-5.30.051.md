...
# Behavioral Contract BC-5.30.051: feature:phase-f3-state-backup

## Description

Workflow contract: feature:phase-f3-state-backup.

## Preconditions

1. Upstream dependencies completed successfully: `[phase-f3-spec-review-gemini]`.

## Postconditions

1. Step `phase-f3-state-backup (line 695; lobster carve-out: stable anchor is step name `phase-f3-state-backup`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `phase-f3-state-backup`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `phase-f3-state-backup (line 695; lobster carve-out: stable anchor is step name `phase-f3-state-backup`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1630 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `phase-f3-state-backup (line 695; lobster carve-out: stable anchor is step name `phase-f3-state-backup`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1191; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1630` |
| **Confidence** | high |
...
- **Step:** phase-f3-state-backup (line 695; lobster carve-out: stable anchor is step name `phase-f3-state-backup`, not line number)
- **Type:** agent
- **Agent:** state-manager
- **Depends on:** `[phase-f3-spec-review-gemini]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`phase-f3-state-backup (line 695)`). Stable anchor is step name `phase-f3-state-backup`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `phase-f3-state-backup`. |
