...
5. When STATE.md is written, the "Session Resume Checkpoint" section's `### §1. Where We Are`
   ...
   marker-prefix check is applied to this sub-section text:
...
**Snapshot:** Captured at D-518 parent-commit `f189b45b` 2026-05-29. The trajectory tail values shown in this stdout (`→9→9→9→11`) evolve monotonically per cycle; re-execute against the latest STATE.md to verify the inv-4 marker-prefix invariant holds across cycles. The scoped-segment count==4 property is STRUCTURAL and survives trajectory growth — the `trajectory-tail ` marker prefix followed by `→N→N→N→N; ` pattern is enforced by state-manager Commit E discipline independent of the specific axis values. To verify freshness: run `grep "trajectory-tail" /path/to/STATE.md | head -10` against current factory-artifacts HEAD and confirm the marker-prefix + count==4 invariant holds in the first semicolon-segment. Historical D-518 snapshot retained for audit traceability.
