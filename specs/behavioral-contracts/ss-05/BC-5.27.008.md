...
# Behavioral Contract BC-5.27.008: phase-7:test-convergence

## Description

Workflow contract: phase-7:test-convergence.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-spec-convergence]`.

## Postconditions

1. Step `test-convergence (line 31; lobster carve-out: stable anchor is step name `test-convergence`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `test-convergence`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `test-convergence (line 31; lobster carve-out: stable anchor is step name `test-convergence`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1437 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `test-convergence (line 31; lobster carve-out: stable anchor is step name `test-convergence`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 611; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1437` |
| **Confidence** | high |
...
- **Step:** test-convergence (line 31; lobster carve-out: stable anchor is step name `test-convergence`, not line number)
- **Type:** skill
- **Skill:** `step-b-test-convergence.md`
- **Depends on:** `[backup-spec-convergence]`
- **Source line(s) in workflow YAML:** 31-34
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`test-convergence (line 31)`). Stable anchor is step name `test-convergence`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `test-convergence`. |
