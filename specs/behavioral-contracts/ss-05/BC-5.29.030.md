...
# Behavioral Contract BC-5.29.030: brownfield:session-review-approval

## Description

Workflow contract: brownfield:session-review-approval.
...
1. Step `session-review-approval (line 377; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)` has run to completion and emitted any declared artifacts.
...
1. Step execution must complete within timeout: 72h.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `session-review-approval (line 377; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1569 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `session-review-approval (line 377; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1019; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1569` |
| **Confidence** | high |
...
- **Step:** session-review-approval (line 377; lobster carve-out: stable anchor is step name `session-review-approval`, not line number)
- **Type:** human-approval
- **Depends on:** `[session-review]`
- **Timeout:** 72h
- **Source line(s) in workflow YAML:** 377-388
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`session-review-approval (line 377)`). Stable anchor is step name `session-review-approval`.
