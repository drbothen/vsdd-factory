...
# Behavioral Contract BC-5.29.022: brownfield:semport-validation-gate

## Description

Workflow contract: brownfield:semport-validation-gate.

## Preconditions

1. Upstream dependencies completed successfully: `[semport-translation]`.
2. Conditional gate evaluates true: `same`.

## Postconditions

1. Step `semport-validation-gate (line 264; lobster carve-out: stable anchor is step name `semport-validation-gate`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `semport-validation-gate`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `semport-validation-gate (line 264; lobster carve-out: stable anchor is step name `semport-validation-gate`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1561 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `semport-validation-gate (line 264; lobster carve-out: stable anchor is step name `semport-validation-gate`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 993; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1561` |
| **Confidence** | high |
...
- **Step:** semport-validation-gate (line 264; lobster carve-out: stable anchor is step name `semport-validation-gate`, not line number)
- **Type:** gate
- **Depends on:** `[semport-translation]`
- **Condition:** same
- **Source line(s) in workflow YAML:** 264-273
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`semport-validation-gate (line 264)`). Stable anchor is step name `semport-validation-gate`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `semport-validation-gate`. |
