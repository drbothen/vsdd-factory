...
# Behavioral Contract BC-5.30.084: feature:post-feature-validation

## Description

Workflow contract: feature:post-feature-validation.

## Preconditions

1. Upstream dependencies completed successfully: `[feature-cycle-handoff]`.
2. Conditional gate evaluates true: `config.post_feature_validation.enabled == true`.

## Postconditions

1. Step `post-feature-validation (line 1446; lobster carve-out: stable anchor is step name `post-feature-validation`, not line number)` has run to completion and emitted any declared artifacts.
...
| EC-003 | Condition `config.post_feature_validation.enabled == true` is false | Step is skipped; downstream `depends_on` consumers proceed without this step's outputs. |
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `post-feature-validation (line 1446; lobster carve-out: stable anchor is step name `post-feature-validation`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1663 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `post-feature-validation (line 1446; lobster carve-out: stable anchor is step name `post-feature-validation`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 1291; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1663` |
| **Confidence** | high |
...
- **Step:** post-feature-validation (line 1446; lobster carve-out: stable anchor is step name `post-feature-validation`, not line number)
- **Type:** skill
- **Skill:** `skills/post-feature-validation/SKILL.md`
- **Depends on:** `[feature-cycle-handoff]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`post-feature-validation (line 1446)`). Stable anchor is step name `post-feature-validation`.
