...
# Behavioral Contract BC-5.29.018: brownfield:brownfield-market-intel

## Description

Workflow contract: brownfield:brownfield-market-intel.

## Preconditions

1. Upstream dependencies completed successfully: `[post-phase-0-routing]`.
2. Conditional gate evaluates true: `routing.choice == 'feature'`.

## Postconditions

1. Step `brownfield-market-intel (line 216; lobster carve-out: stable anchor is step name `brownfield-market-intel`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `brownfield-market-intel`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `brownfield-market-intel (line 216; lobster carve-out: stable anchor is step name `brownfield-market-intel`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1557 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `brownfield-market-intel (line 216; lobster carve-out: stable anchor is step name `brownfield-market-intel`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 981; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1557` |
| **Confidence** | high |
...
- **Step:** brownfield-market-intel (line 216; lobster carve-out: stable anchor is step name `brownfield-market-intel`, not line number)
- **Type:** skill
- **Skill:** `skills/market-intelligence-assessment/SKILL.md`
- **Depends on:** `[post-phase-0-routing]`
- **Condition:** `routing.choice == 'feature'`
- **Source line(s) in workflow YAML:** 216-224
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`brownfield-market-intel (line 216)`). Stable anchor is step name `brownfield-market-intel`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `brownfield-market-intel`. |
