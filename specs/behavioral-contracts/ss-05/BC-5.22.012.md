...
# Behavioral Contract BC-5.22.012: phase-2:wave-schedule

## Description

Workflow contract: phase-2:wave-schedule.

## Preconditions

1. Upstream dependencies completed successfully: `[backup-dependency-graph]`.

## Postconditions

1. Step `wave-schedule (line 64; lobster carve-out: stable anchor is step name `wave-schedule`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `wave-schedule`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `wave-schedule (line 64; lobster carve-out: stable anchor is step name `wave-schedule`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1351 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `wave-schedule (line 64; lobster carve-out: stable anchor is step name `wave-schedule`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 365; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1351` |
| **Confidence** | high |
...
- **Step:** wave-schedule (line 64; lobster carve-out: stable anchor is step name `wave-schedule`, not line number)
- **Type:** skill
- **Skill:** `decompose-stories/steps/step-d-wave-schedule.md`
- **Depends on:** `[backup-dependency-graph]`
- **Source line(s) in workflow YAML:** 64-67
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`wave-schedule (line 64)`). Stable anchor is step name `wave-schedule`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `wave-schedule`. |
