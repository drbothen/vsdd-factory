...
# Behavioral Contract BC-5.22.006: phase-2:define-epics

## Description

Workflow contract: phase-2:define-epics.
...
1. Step `define-epics (line 18; lobster carve-out: stable anchor is step name `define-epics`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `define-epics (line 18; lobster carve-out: stable anchor is step name `define-epics`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1345 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `define-epics (line 18; lobster carve-out: stable anchor is step name `define-epics`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 347; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1345` |
| **Confidence** | high |
...
- **Step:** define-epics (line 18; lobster carve-out: stable anchor is step name `define-epics`, not line number)
- **Type:** skill
- **Skill:** `decompose-stories/steps/step-a-define-epics.md`
- **Depends on:** `[]`
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`define-epics (line 18)`). Stable anchor is step name `define-epics`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `define-epics`. |
