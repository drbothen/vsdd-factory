...
# Behavioral Contract BC-5.22.015: phase-2:backup-holdout-scenarios

## Description

Workflow contract: phase-2:backup-holdout-scenarios.
...
1. Step `backup-holdout-scenarios (line 84; lobster carve-out: stable anchor is step name `backup-holdout-scenarios`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `backup-holdout-scenarios`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `backup-holdout-scenarios (line 84; lobster carve-out: stable anchor is step name `backup-holdout-scenarios`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1354 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `backup-holdout-scenarios (line 84; lobster carve-out: stable anchor is step name `backup-holdout-scenarios`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 374; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1354` |
| **Confidence** | high |
...
- **Step:** backup-holdout-scenarios (line 84; lobster carve-out: stable anchor is step name `backup-holdout-scenarios`, not line number)
- **Agent:** state-manager
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`backup-holdout-scenarios (line 84)`). Stable anchor is step name `backup-holdout-scenarios`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `backup-holdout-scenarios`. |
