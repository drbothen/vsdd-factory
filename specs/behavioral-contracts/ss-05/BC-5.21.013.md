...
# Behavioral Contract BC-5.21.013: phase-1:backup-create-architecture

## Description

Workflow contract: phase-1:backup-create-architecture.
...
1. Step ``backup-create-architecture` (line 68; lobster carve-out: stable anchor is step name `backup-create-architecture`, not line number)` has run to completion and emitted any declared artifacts.
<!-- F-P23-001: lobster-line-cite annotated per pass-23 carve-out; stable anchor is step name `backup-create-architecture`; line number is unstable as lobster files evolve -->
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for ``backup-create-architecture` (line 68; lobster carve-out: stable anchor is step name `backup-create-architecture`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1332 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for ``backup-create-architecture` (line 68; lobster carve-out: stable anchor is step name `backup-create-architecture`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 274; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1332` |
| **Confidence** | high |
...
- **Step:** `backup-create-architecture` (line 68; lobster carve-out: stable anchor is step name `backup-create-architecture`, not line number)
- **Agent:** state-manager
- **Source line(s) in workflow YAML:** 68-74
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001 semantic-pattern-class discipline with FULL syntactic variant scope) — §Postconditions cited lobster step by line number using double-backtick form (`` ``backup-create-architecture` (line 68)` ``). This is a lobster-file reference and falls under the lobster-line-cite carve-out exception; the stable anchor is the step name `backup-create-architecture`, not the line number. Pass-23 adversary identified this as the sibling-class variant missed by F-P22-001's single-backtick-description-form sweep.

**Changes made:**
- §Postconditions: inline annotation added noting lobster carve-out (stable anchor = step name `backup-create-architecture`); HTML carve-out comment added citing F-P23-001 deferral.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: postcondition-form lobster-line-cite (double-backtick variant) annotated with carve-out per L-P19-001 + L-P20-001 + L-P22-001. Stable anchor is step name `backup-create-architecture`. |
