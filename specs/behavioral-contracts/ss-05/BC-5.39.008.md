...
4. If any policy entry has an `id` field that is not a YAML integer scalar in range [1, 999],
   ...
   **Range rationale (v1.3, F-BC008P3-002 fix):** The upper bound of 999 supports
   future-proofing per the three-digit-ID canonical form (POLICY 15 / F-PASS14-006).
   Current production ids are 1..18 (18 policies as of 2026-05-19). Expected growth is
   well under 100 over the next 5 years. Hard cap of 999 enables consistent 3-digit-maximum
   display formatting in tables and UI tooling without imposing an artificially low ceiling.
   INV-018 evidence — current id range: `grep -nE '^  - id:' .factory/policies.yaml | head -5`
   → `33: - id: 1` ... `47: - id: 2` (first 5; bare integer format confirmed).
   `grep -nE '^  - id:' .factory/policies.yaml | tail -3` → `268: - id: 17`, `282: - id: 18`
   (current max = 18; well within [1, 999]). Residual-class sweep for 4+ digit ids:
...
|----|-------------|-------------------|
| EC-001 | policies.yaml has invalid YAML syntax (unmatched `{`) | `HookResult::block_with_fix(...)` with parse-error location (line N); Part A postcondition 1 |
| EC-002 | policies.yaml missing `version:` header field | `HookResult::block_with_fix(...)` naming `version` as missing; citing F-PASS14-004 |
| EC-003 | Policy entry has `id: "POLICY 01"` (string, not integer) | `HookResult::block_with_fix(...)` citing non-conforming id format; expected bare YAML integer in [1,999] |
| EC-004 | Policy entry has `id: 1` (bare integer, canonical) | `HookResult::Continue` for ID format check |
| EC-005 | Two policy entries both have `id: 3` | `HookResult::block_with_fix(...)` citing duplicate ID |
...
| EC-007 | Policy entry has `lint_hook: "nonexistent-plugin"` (not in hooks-registry.toml) | `HookResult::block_with_fix(...)` naming missing plugin reference |
...
| EC-010 | Policy entry has `codified_at: "pass-72"` (non-D-NNN form) | `HookResult::block_with_fix(...)` citing malformed codified_at value |
...
| YAML parse error | `policies.yaml` has unmatched brace | `HookResult::block_with_fix(...)` with parse-error location | PC5 violated (parse failure) | BLOCK |
| Missing header field | policies.yaml lacks `version:` header | `HookResult::block_with_fix(...)` naming `version`; F-PASS14-004 | PC5 satisfied, PC2 header check violated | BLOCK |
| Non-conforming ID (string) | `id: "POLICY 01"` (quoted string, not integer) | `HookResult::block_with_fix(...)` citing non-conforming id format; expected integer in [1,999] | PC5 satisfied; ID format violated | BLOCK |
| Duplicate ID | Two entries with `id: 3` | `HookResult::block_with_fix(...)` naming duplicated ID | PC5 satisfied; duplicate check violated | BLOCK |
| Missing lint_hook field | Policy entry has no `lint_hook` key | `HookResult::block_with_fix(...)` naming missing field | PC2 (required field absent) | BLOCK |
| Nonexistent lint_hook plugin | `lint_hook: "ghost-plugin"` not in hooks-registry.toml | `HookResult::block_with_fix(...)` naming missing plugin | PC8 satisfied; lint_hook existence violated | BLOCK |
| null lint_hook | `lint_hook: null` | `HookResult::Continue` for that field | PC2 satisfied (null valid) | PASS |
| Malformed codified_at | `codified_at: "pass-72"` | `HookResult::block_with_fix(...)` citing malformed value | PC7 rationale: lint_hook non-null; codified_at invalid | BLOCK |
...
The "Postcondition" column cites postcondition ordinals (1–14 for Part A/B/C) or "PC-label"
for precondition-level gates. POLICY-row corrections per v1.3 (F-BC008P3-001): POLICY 13
enforces postcondition 6 (lint_hook field existence check); POLICY 16 enforces postcondition 7
(codified_at coupling validation). PC3 (tool_input.content is not source of truth) is
semantically unrelated to POLICY 13/16 and is NOT cited in those rows.

| D-NNN Sub-Clause | Gate Enforced | Postcondition |
|-----------------|---------------|---------------|
| F-PASS14-004 | policies.yaml frontmatter header required fields | 2 |
| F-PASS14-006 | Three-digit POLICY ID canonical format (human direction 2026-05-15) | 4 |
