...
# Behavioral Contract BC-5.28.007: greenfield:repo-initialization

## Description

Workflow contract: greenfield:repo-initialization.
...
1. Step `repo-initialization (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number)` has run to completion and emitted any declared artifacts.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Workflow YAML field shape matches declared schema for `repo-initialization (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number)` | manual |
...
| Stories | TBD |
| Source BC-AUDIT ID | BC-AUDIT-1466 |
...
- [TBD — to be assigned in Phase 1.6c] — Workflow YAML field shape matches declared schema for `repo-initialization (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number)`
...
| **Source Document** | `.factory/phase-0-ingestion/pass-3-deep-workflows.md` (line 696; source-doc carve-out: line in phase-0 ingestion doc, not lobster step line) |
| **Source BC-AUDIT ID** | `BC-AUDIT-1466` |
| **Confidence** | high |
...
- **Step:** repo-initialization (line 37; lobster carve-out: stable anchor is step name `repo-initialization`, not line number)
- **Type:** skill
...
**Driver:** F-P23-001 pass-23 retroactive corpus-wide sweep (per L-P19-001 / L-P20-001 / L-P22-001) — lobster step cited by line number (`repo-initialization (line 37)`). Stable anchor is step name `repo-initialization`.
...
|---------|------|--------|--------|
| v1.2 | 2026-05-08 | state-manager | F-P23-001 corpus-wide sweep: lobster-line-cite annotated with carve-out. Stable anchor is step name `repo-initialization`. |
