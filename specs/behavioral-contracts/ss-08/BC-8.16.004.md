...
origin: brownfield
extracted_from: "plugins/vsdd-factory/templates/discovery-config-template.yaml"
subsystem: SS-08
capability: ""
...
removed: null
removal_reason: null
audit_id: BC-AUDIT-1915
section: "Discovery, project, config templates"
type: template
---

# Behavioral Contract BC-8.16.004: discovery-config-template: discovery-engine ingestion config

## Description

`.factory/discovery-config.yaml`. Top-level: `schedule` (5 channels with cadence — market_research weekly, feedback_ingestion daily, etc.), `user_channels` (github / slack / discord, all optional), market_research, competitive_monitoring, analytics_integration. Unconfigured channels skipped gracefully.

## Preconditions

1. Template file at `plugins/vsdd-factory/templates/discovery-config-template.yaml` is being authored or validated.
2. Authoring agent or validation tool is reading the template definition.

## Postconditions

1. Top-level keys parse as YAML.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/templates/discovery-config-template.yaml` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/templates/discovery-config-template.yaml` | **Type:** template | **Confidence:** HIGH | **Source line(s):** 1–30
