...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2284
section: "Rules: worktree-protocol (branch hierarchy, merge protocol)"
type: rule
---

# Behavioral Contract BC-8.27.008: rules/worktree-protocol.md: cleanup rules — remove worktrees promptly, never force-remove with uncommitted changes, audit via `git worktree list`

## Description

Remove worktrees promptly after merge — stale worktrees waste disk and cause confusion. NEVER force-remove a worktree with uncommitted changes — commit or stash first. `git worktree list` to audit active worktrees. `.worktrees/` is gitignored — worktrees are ephemeral, not tracked.
...
1. No stale worktrees in `git worktree list`; `.worktrees/` gitignored.
...
**Source metadata:** `plugins/vsdd-factory/rules/worktree-protocol.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 73–77
