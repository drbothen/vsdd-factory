...
origin: brownfield
extracted_from: "plugins/vsdd-factory/templates/project-manifest-template.yaml"
subsystem: SS-08
capability: ""
...
removed: null
removal_reason: null
audit_id: BC-AUDIT-1916
section: "Discovery, project, config templates"
type: template
---

# Behavioral Contract BC-8.16.005: project-manifest-template: multi-repo project.yaml schema

## Description

Multi-repo `project.yaml`. OPTIONAL for single-repo projects. Top-level: `project` (name, description, version), `repos` (map of repo-id → {`path`, `language`, `framework`, `role` enum {primary | consumer | generated}, `contracts.{produces[],consumes[]}`}). Cross-repo contracts wire via `consumes[].source_repo` matching another repo's `produces[].type`.

## Preconditions

1. Template file at `plugins/vsdd-factory/templates/project-manifest-template.yaml` is being authored or validated.
2. Authoring agent or validation tool is reading the template definition.

## Postconditions

1. YAML parses; per-repo `role` is in enum.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/templates/project-manifest-template.yaml` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/templates/project-manifest-template.yaml` | **Type:** template | **Confidence:** HIGH | **Source line(s):** 1–40
