...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2229
section: "Rules: rust (forbid unsafe, edition, lints)"
type: rule
---

# Behavioral Contract BC-8.23.002: rules/rust.md: NO `unwrap()` in production code — use `?` or `expect("actionable msg")`

## Description

No `unwrap()` in production code paths — use `?` for propagation or `expect("actionable message")` when panic is desired with context. Tests may use `unwrap()` freely.
...
1. Code-grep finds no `.unwrap()` outside `#[cfg(test)]` modules.
...
**Source metadata:** `plugins/vsdd-factory/rules/rust.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 10
