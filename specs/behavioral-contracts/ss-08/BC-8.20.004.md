...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2204
section: "Rules: bash (no stderr suppression, no eval, dependency checks)"
type: rule
---

# Behavioral Contract BC-8.20.004: rules/bash.md: test files MUST verify tool dependencies at the top before any assertions

## Description

Test files: top-of-file dependency check pattern: `if ! command -v required-tool &>/dev/null; then echo "ERROR: required-tool is required but not found." >&2; echo "Install: cargo install required-tool" >&2; exit 1; fi`.
...
1. Each test file has the prerequisite check before fixtures.
...
**Source metadata:** `plugins/vsdd-factory/rules/bash.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 62–72
