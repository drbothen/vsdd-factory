...
origin: brownfield
extracted_from: "plugins/vsdd-factory/rules/factory-protocol.md"
subsystem: SS-08
capability: ""
...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2213
section: "Rules: factory-protocol (.factory worktree, governance)"
type: rule
---

# Behavioral Contract BC-8.21.003: rules/factory-protocol.md: all `.factory/` changes commit to `factory-artifacts`, NOT main/develop

## Description

Commit from within the `.factory/` directory: `cd .factory && git add -A && git commit -m "..."`. Commit at every phase gate transition. Format: `factory(<phase>): <description>` (e.g., `factory(phase-1): add PRD and architecture specs`).

## Preconditions

1. The rule from `plugins/vsdd-factory/rules/factory-protocol.md` is being applied to a code change, commit, or artifact.
2. The artifact under review falls within the scope of the rule ("Used by" field below).

## Postconditions

1. Commits land on `factory-artifacts` branch with documented message format.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/rules/factory-protocol.md` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/rules/factory-protocol.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 60–63
