...
origin: brownfield
extracted_from: "plugins/vsdd-factory/templates/demo-playwright-template.spec.ts"
subsystem: SS-08
capability: ""
...
removed: null
removal_reason: null
audit_id: BC-AUDIT-1871
section: "Demo evidence templates"
type: template
---

# Behavioral Contract BC-8.08.003: demo-playwright-template: Playwright per-AC video+screenshot demo

## Description

TypeScript Playwright spec scaffold. Best practices: ONE test per AC, slowMo 200ms, video `'on'` (always), screenshots at verify points, `data-testid` locators, BOTH success+error path per test, recordings under 15s, viewport 1280x720. Output `.webm` + `.png` per AC.

## Preconditions

1. Template file at `plugins/vsdd-factory/templates/demo-playwright-template.spec.ts` is being authored or validated.
2. Authoring agent or validation tool is reading the template definition.

## Postconditions

1. File begins with documentation block + `import { test, expect } from '@playwright/test'`.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/templates/demo-playwright-template.spec.ts` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/templates/demo-playwright-template.spec.ts` | **Type:** template | **Confidence:** HIGH | **Source line(s):** 1–25
