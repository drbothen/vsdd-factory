...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2201
section: "Rules: bash (no stderr suppression, no eval, dependency checks)"
type: rule
---

# Behavioral Contract BC-8.20.001: rules/bash.md: SHALL NOT suppress stderr with `2>/dev/null` in production scripts

## Description

Suppressing stderr makes tool crashes indistinguishable from "value not found." Capture stderr and check it. Use `_run_with_stderr_guard` from `assertions.sh`, OR a manual `mktemp` stderr file + `[[ -s ]]` check pattern.
...
1. No untagged `2>/dev/null` in prod scripts; allowed only with `# STDERR-EXEMPT: <rationale>` tag.
...
**Source metadata:** `plugins/vsdd-factory/rules/bash.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 7–29
