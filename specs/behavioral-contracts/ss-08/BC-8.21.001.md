...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2211
section: "Rules: factory-protocol (.factory worktree, governance)"
type: rule
---

# Behavioral Contract BC-8.21.001: rules/factory-protocol.md: `.factory/` is a git worktree on the orphan `factory-artifacts` branch

## Description

`.factory/` is a git worktree mounted on the orphan `factory-artifacts` branch. It holds all pipeline state, specs, stories, and evaluation artifacts. NEVER on `main` or `develop` — lives on its own branch with its own commit history.
...
1. `.factory/` is a separate worktree on `factory-artifacts`.
...
**Source metadata:** `plugins/vsdd-factory/rules/factory-protocol.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 5–7
