...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2205
section: "Rules: bash (no stderr suppression, no eval, dependency checks)"
type: rule
---

# Behavioral Contract BC-8.20.005: rules/bash.md: negative assertions MUST verify the search tool ran successfully

## Description

Tests asserting something does NOT exist must FIRST verify the search tool works. A tool crash must not be interpreted as "pattern absent." This is the most common false-pass class. Pattern: capture stderr, `[[ -n "$stderr" ]] && _fail` before interpreting empty result as absent.
...
1. Negative-assertion tests have the stderr-check pattern.
...
**Source metadata:** `plugins/vsdd-factory/rules/bash.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 74–92
