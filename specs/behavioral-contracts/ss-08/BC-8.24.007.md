...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2243
section: "Rules: spec-format (4-level hierarchy, BC/VP shape)"
type: rule
---

# Behavioral Contract BC-8.24.007: rules/spec-format.md: green VPs are IMMUTABLE — modification requires new VP supersedes old

## Description

Once a VP reaches `green` status, it CANNOT be modified without explicit approval. The spec-steward hook enforces this. To change a green VP, create a new VP that supersedes it AND retire the old one (via vp-withdrawal-template).
...
1. Green VP files unchanged in subsequent commits; supersession via vp-withdrawal-template.
...
**Source metadata:** `plugins/vsdd-factory/rules/spec-format.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 96–98
