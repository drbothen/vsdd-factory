...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2202
section: "Rules: bash (no stderr suppression, no eval, dependency checks)"
type: rule
---

# Behavioral Contract BC-8.20.002: rules/bash.md: SHALL NOT use `eval` in shell helpers

## Description

`eval "$cmd"` is a command-injection vector (CWE-78). Use `"$@"` parameter expansion: pass command and args as separate parameters.
...
1. No `eval` in shell helpers; `"$@"` pattern instead.
...
**Source metadata:** `plugins/vsdd-factory/rules/bash.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 31–45
