...
origin: brownfield
extracted_from: "plugins/vsdd-factory/templates/reference-manifest-template.yaml"
subsystem: SS-08
capability: ""
...
removed: null
removal_reason: null
audit_id: BC-AUDIT-1917
section: "Discovery, project, config templates"
type: template
---

# Behavioral Contract BC-8.16.006: reference-manifest-template: .reference/ rebuild manifest

## Description

Source of truth for rebuilding `.reference/` on a new system. Per-repo schema: `url` (HTTPS or SSH clone URL), `commit` (SHA at ingestion), `ingested` (YYYY-MM-DD), `depth` (1=shallow / `full`), `focus` (full | plugins | src | <dir>), `status` (pending | ingested | analyzed), `note`. Updated by `/brownfield-ingest` Step 0 when new repos added.

## Preconditions

1. Template file at `plugins/vsdd-factory/templates/reference-manifest-template.yaml` is being authored or validated.
2. Authoring agent or validation tool is reading the template definition.

## Postconditions

1. Top-level `repos:` map present; per-entry keys present.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/templates/reference-manifest-template.yaml` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/templates/reference-manifest-template.yaml` | **Type:** template | **Confidence:** HIGH | **Source line(s):** 1–24
