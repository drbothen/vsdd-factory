...
origin: brownfield
extracted_from: "plugins/vsdd-factory/templates/project-justfile-template"
subsystem: SS-08
capability: ""
...
removed: null
removal_reason: null
audit_id: BC-AUDIT-1922
section: "Discovery, project, config templates"
type: template
---

# Behavioral Contract BC-8.16.011: project-justfile-template: per-project justfile bootstrap

## Description

Generated at `dark-factory --init`. Manages YOUR PROJECT (not the engine). Sets `dotenv-load`, `positional-arguments`. Variables: `engine_home := env("DARK_FACTORY_HOME", ...)`, `project_name`, `project_mode`. Default recipe shows `just --list`.

## Preconditions

1. Template file at `plugins/vsdd-factory/templates/project-justfile-template` is being authored or validated.
2. Authoring agent or validation tool is reading the template definition.

## Postconditions

1. Header comment + `set` directives + `start` recipe present.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/templates/project-justfile-template` |
| **Confidence** | HIGH |
| **Extraction Date** | 2026-04-25 |

**Source metadata:** `plugins/vsdd-factory/templates/project-justfile-template` | **Type:** template | **Confidence:** HIGH | **Source line(s):** 1–25
