...
removed: null
removal_reason: null
audit_id: BC-AUDIT-2231
section: "Rules: rust (forbid unsafe, edition, lints)"
type: rule
---

# Behavioral Contract BC-8.23.004: rules/rust.md: type design — newtypes for IDs, validated constructors at trust boundaries, `#[non_exhaustive]` on growing enums, UUID v7 for time-ordered IDs

## Description

Newtypes for IDs (prevents mixing). Validated constructors at trust boundaries (`new()` validates API input/deserialization; `new_unchecked()` for tests/trusted internal). `#[non_exhaustive]` on enums that will grow. UUID v7 (`Uuid::now_v7()`) for time-sortable ordering. Private fields with getters on security-critical types.
...
1. ID types are newtypes; growing enums are non_exhaustive.
...
**Source metadata:** `plugins/vsdd-factory/rules/rust.md` | **Type:** rule | **Confidence:** HIGH | **Source line(s):** 13–20
