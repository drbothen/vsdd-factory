...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-562"
ss_section: "Feature-mode phase skills (f1-f7)"
skill: "phase-f6-targeted-hardening"
---

# Behavioral Contract BC-6.17.035: phase-f6-targeted-hardening: Hardening scope per-tool varies — Kani/fuzz/mutation/Semgrep delta; regression+audit full tree

## Description

Scope table: Kani proofs → new/modified modules only; Fuzz testing → new code paths only; Mutation testing → changed files only; Semgrep → changed + new files; Regression tests → full existing test suite; cargo audit / npm audit → full dependency tree. Acceptance: Per-tool scope table.
...
1. Scope table: Kani proofs → new/modified modules only; Fuzz testing → new code paths only; Mutation testing → changed files only; Semgrep → changed + new files; Regression tests → full existing test suite; cargo audit / npm audit → full dependency tree.

## Invariants

1. Per-tool scope table.
