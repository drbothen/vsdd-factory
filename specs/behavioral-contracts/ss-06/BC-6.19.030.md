...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1229"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.19.030: toolchain-provisioning: integration with formal-hardening + multi-repo + new-language

## Description

toolchain-provisioning: integration with formal-hardening + multi-repo + new-language. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/toolchain-provisioning/SKILL.md` (lines 275-313; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Formal hardening start; project.yaml multi-repo; adding new language.

## Postconditions

1. formal-verifier reads toolchain-state.yaml first; missing required tool triggers re-provisioning. Multi-repo: per-repo language detection. Adding new language: edit `config/verification-toolchains.yaml` only (no skill change).
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Acceptance: formal-verifier never executes without toolchain-state.yaml; multi-repo writes per-repo state. | manual |
...
- [TBD — to be assigned in Phase 1.6c] — formal-verifier never executes without toolchain-state.yaml; multi-repo writes per-repo state.
...
| **Source Lines** | 275-313 |
| **Audit ID** | BC-AUDIT-723 |
