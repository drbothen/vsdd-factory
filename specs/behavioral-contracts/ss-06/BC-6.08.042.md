...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1416"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.08.042: ui-quality-gate: 4 strictness levels (per-story/wave/build/convergence)

## Description

ui-quality-gate: 4 strictness levels (per-story/wave/build/convergence). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/ui-quality-gate/SKILL.md` (lines 53-96; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. Per-story → token+a11y+component test (block merge). Wave gate → +responsive+perf+states (block next wave). Build verification → all (block release). Convergence → 100% on every dimension; zero traceability gaps; 100% fidelity (block convergence).
