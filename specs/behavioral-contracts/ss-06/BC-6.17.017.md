...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-544"
ss_section: "Feature-mode phase skills (f1-f7)"
skill: "phase-f3-incremental-stories"
---

# Behavioral Contract BC-6.17.017: phase-f3-incremental-stories: Cycle detection via Kahn's algorithm topological sort

## Description

Build adjacency list (existing + new stories). Run topological sort (Kahn's algorithm). If sort completes: no cycles, proceed. If fails: identify cycle, report to human. Common fixes: merge stories, split differently, remove dependency. Acceptance: Algorithm name; failure handling.
...
1. Build adjacency list (existing + new stories). Run topological sort (Kahn's algorithm). If sort completes: no cycles, proceed. If fails: identify cycle, report to human. Common fixes: merge stories, split differently, remove dependency.

## Invariants

1. Algorithm name; failure handling.
