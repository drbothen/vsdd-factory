...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-1.md#L315"
subsystem: SS-06
capability: "TBD"
...
# Behavioral Contract BC-6.14.005: artifact-detection: failure modes (no .factory/, corruption, legacy)

> Source: `pass-3-deep-skills-batch-1.md` line 315 (was `BC-AUDIT-226`)
> Subsystem: SS-06 — Skill Catalog
> Section: Artifact and verification skills

## Description

artifact-detection: failure modes (no .factory/, corruption, legacy). Missing `.factory/` → report L0 → route Discovery; corrupt artifacts (unparseable frontmatter, truncated) → flag specific corruption + recommend re-creation; legacy FR-NNN → flag for migration, do not silently skip.

## Preconditions

1. Various failure conditions

## Postconditions

1. Each failure mode is handled per the documented branch rather than crashing or silent fallback.
...
- `architecture/ss-06-skill-catalog.md#artifact-detection-failure-modes-(no-.factory/,-corruption,-legacy)` — TBD anchor
...
| **Source line(s) within skill** | 171-175 |
