...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1144"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.14.024: systematic-debugging: 4-phase sequence (root cause → pattern → hypothesis → implementation)

## Description

systematic-debugging: 4-phase sequence (root cause → pattern → hypothesis → implementation). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/systematic-debugging/SKILL.md` (lines 38-87; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Bug investigation start.

## Postconditions

1. Phase 1 (read errors, reproduce, check changes, trace data); Phase 2 (find working examples, compare, identify diffs); Phase 3 (single hypothesis, minimal test, one variable); Phase 4 (failing test FIRST per Red Gate, single fix, verify).
