...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-behavioral-contracts-deep-r1.md#L125"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.02.004: Skills with `disable-model-invocation: true` are dispatcher-only — model cannot self-invoke

## Description

Skills with `disable-model-invocation: true` are dispatcher-only — model cannot self-invoke. Skill-class meta-contract — applies to every SKILL.md across the vsdd-factory skills catalog. Confidence: HIGH.

## Preconditions

1. Skill frontmatter contains `disable-model-invocation: true`.

## Postconditions

1. Model cannot launch the skill via reasoning; only orchestrator dispatch (Task tool + agent handoff) or explicit slash command invocation activates it.
...
- [TBD — to be assigned in Phase 1.6c] — Model cannot launch the skill via reasoning; only orchestrator dispatch (Task tool + agent handoff) or explicit slash command invocation activates it.
...
| **Source Line** | 125 |
| **Audit ID** | BC-AUDIT-090 |
| **Evidence (verbatim)** | `skills/wave-gate/SKILL.md:6` (`disable-model-invocation: true`); `skills/create-prd/SKILL.md:4`; `skills/deliver-story/SKILL.md:5`. Pattern is used for high-stakes orchestration skills to prevent inadvertent self-dispatch. |
| **Confidence (verbatim)** | HIGH (consistent flag usage across high-risk skills). |
