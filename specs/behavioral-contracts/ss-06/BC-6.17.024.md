...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-551"
ss_section: "Feature-mode phase skills (f1-f7)"
skill: "phase-f4-delta-implementation"
---

# Behavioral Contract BC-6.17.024: phase-f4-delta-implementation: Regression failure — fix the implementation, not the test

## Description

"Log to regression-log.md, identify which implementation change caused the regression, fix the implementation (not the test), re-run until clean." Hook `plugins/src/regression-gate.ts` automates regression detection. Acceptance: Implementation-not-test rule.
...
1. "Log to regression-log.md, identify which implementation change caused the regression, fix the implementation (not the test), re-run until clean." Hook `plugins/src/regression-gate.ts` automates regression detection.

## Invariants

1. Implementation-not-test rule.
