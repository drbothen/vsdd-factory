...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1683"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.08.052: visual-companion: server lifecycle (start/url/state-dir/stop, 30-min auto-exit)

## Description

visual-companion: server lifecycle (start/url/state-dir/stop, 30-min auto-exit). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/visual-companion/SKILL.md` (lines 30-65; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. `start-server.sh --project-dir <path>` returns JSON {type:server-started, port, url, screen_dir, state_dir}; user opens URL; stop with `stop-server.sh <session_dir>`; auto-exits after 30 min inactivity.
