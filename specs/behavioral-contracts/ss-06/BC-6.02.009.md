...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-behavioral-contracts-deep-r1.md#L160"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.02.009: Skill bodies MAY include a "Red Flags" table to enumerate fabrication / shortcut anti-patterns

## Description

Skill bodies MAY include a "Red Flags" table to enumerate fabrication / shortcut anti-patterns. Skill-class meta-contract — applies to every SKILL.md across the vsdd-factory skills catalog. Confidence: HIGH.

## Preconditions

1. Skill is high-risk for self-deception or operator shortcut.

## Postconditions

1. A markdown table maps "Thought" (operator's tempting deviation) to "Reality" (the canonical correction). Used as an in-skill counter-narrative tool.
...
| **Source Line** | 160 |
| **Audit ID** | BC-AUDIT-095 |
| **Evidence (verbatim)** | `brownfield-ingest/SKILL.md:25-39` (10-row table); `wave-gate/SKILL.md:27-39` (8-row table). |
| **Confidence (verbatim)** | HIGH (pattern present on the two skills with the highest fabrication risk; not universal — release/SKILL.md doesn't use it). |
