...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L614"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.16.042: run-phase: topological execution honors depends_on

## Description

run-phase: topological execution honors depends_on. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/run-phase/SKILL.md` (lines 35-46; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 3-4 (Enumerate / Execute).

## Postconditions

1. Extracts steps in dependency order; per type: `agent` → spawn via Task tool with step.task as prompt; `skill` → invoke Skill; `command` → run via Bash; on failure honors `on_failure` (escalate/retry/skip) and `max_retries`.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Acceptance: No step runs before its `depends_on` predecessors. | manual |
...
| Architecture Module | plugins/vsdd-factory/skills/run-phase/SKILL.md |
| Stories | TBD |

## Related BCs (Recommended)

- TBD — sibling BCs in same skill (see other BCs for `plugins/vsdd-factory/skills/run-phase/SKILL.md` in this directory)

## Architecture Anchors (Recommended)

- `plugins/vsdd-factory/skills/run-phase/SKILL.md` — defining skill body for this contract
...
- [TBD — to be assigned in Phase 1.6c] — No step runs before its `depends_on` predecessors.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/skills/run-phase/SKILL.md` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Source Lines** | 35-46 |
| **Audit ID** | BC-AUDIT-660 |
