...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-530"
ss_section: "Feature-mode phase skills (f1-f7)"
skill: "phase-f1-delta-analysis"
---

# Behavioral Contract BC-6.17.003: phase-f1-delta-analysis: Intent classification — feature, enhancement, bug-fix maps to F1-F7 vs bug-fix route

## Description

Intent table: feature ("add"/"build"/"new") → Full F1-F7. enhancement ("improve"/"update"/"change") → Full F1-F7 (may be quick dev if trivial). bug-fix ("fix"/"bug"/"broken"/"regression") → Bug fix route (skip F2, F3). Recorded in delta analysis report and STATE.md. Acceptance: Three intent classes; routing rules.
...
1. Intent table: feature ("add"/"build"/"new") → Full F1-F7. enhancement ("improve"/"update"/"change") → Full F1-F7 (may be quick dev if trivial). bug-fix ("fix"/"bug"/"broken"/"regression") → Bug fix route (skip F2, F3). Recorded in delta analysis report and STATE.md.

## Invariants

1. Three intent classes; routing rules.
