...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-420"
ss_section: "Factory operations and dashboards skills"
skill: "generate-pdf"
---

# Behavioral Contract BC-6.11.021: generate-pdf: Errors must be reported with specific solutions per error class

## Description

Reports specific error and suggests fix (per error/solution table: pandoc not found → `brew install pandoc`, weasyprint not found → `brew install weasyprint`, etc.). Acceptance: Failures map to one of the listed error categories with stated remediation.

## Preconditions

1. Any failure during PDF generation.

## Postconditions

1. Reports specific error and suggests fix (per error/solution table: pandoc not found → `brew install pandoc`, weasyprint not found → `brew install weasyprint`, etc.).

## Invariants

1. Failures map to one of the listed error categories with stated remediation.
...
| Property | Value |
|----------|-------|
| **Path** | `plugins/vsdd-factory/skills/guided-brief-creation/SKILL.md` (172 LOC)` |
| **Confidence** | high |
| **Extraction Date** | 2026-04-25 |
| **Audit Source ID** | BC-AUDIT-420 |
