...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L743"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.05.053: semport-analyze: identity, two modes, reference resolution

## Description

semport-analyze: identity, two modes, reference resolution. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/semport-analyze/SKILL.md` (lines 1-28; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. `/semport-analyze [source-path] [target-language] [--incremental <module>]`.

## Postconditions

1. Two modes: Full Ingestion (default) or Incremental (`--incremental`). If arg matches `.reference/` directory name, resolves to `.reference/<project>/`; canonical inventory in `.factory/reference-manifest.yaml`.
