...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L877"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.19.023: setup-env: MCP env-var prefix check + git config (rerere on)

## Description

setup-env: MCP env-var prefix check + git config (rerere on). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/setup-env/SKILL.md` (lines 43-66; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. `echo $PERPLEXITY_API_KEY | head -c 4` (non-secret prefix); set `git config rerere.enabled true` if not enabled.
