...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1077"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.08.028: storybook-mcp-integration: install + config + register procedure

## Description

storybook-mcp-integration: install + config + register procedure. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/storybook-mcp-integration/SKILL.md` (lines 22-58; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. dx-engineer invocation.

## Postconditions

1. `npx storybook@latest init`; `npm install -D @storybook/addon-mcp`; configure `.storybook/main.ts` with addons + `experimentalComponentsManifest: true` + `experimentalCodeExamples: true`; start dev server (`npm run storybook`); register MCP server JSON config at http://localhost:6006/mcp.
