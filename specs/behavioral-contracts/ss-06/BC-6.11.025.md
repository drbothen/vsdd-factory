...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-447"
ss_section: "Factory operations and dashboards skills"
skill: "maintenance-sweep"
---

# Behavioral Contract BC-6.11.025: maintenance-sweep: Dependency audit splits T3 (run scans) and T2 (analyze)

## Description

dx-engineer (T3) runs cargo audit, cargo deny check, npm audit, pip-audit. security-reviewer (T2) analyzes results, classifies severity. CRITICAL/HIGH CVE → immediate fix PR; MEDIUM → next sprint log; LOW → log only. Acceptance: Tier split documented; severity routing.

## Preconditions

1. Dependency audit sweep runs.

## Postconditions

1. dx-engineer (T3) runs cargo audit, cargo deny check, npm audit, pip-audit. security-reviewer (T2) analyzes results, classifies severity. CRITICAL/HIGH CVE → immediate fix PR; MEDIUM → next sprint log; LOW → log only.

## Invariants

1. Tier split documented; severity routing.
