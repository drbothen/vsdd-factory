...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-behavioral-contracts-deep-r1.md#L167"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.02.010: Skills that dispatch sub-agents declare a "Canonical Source" or single-source-of-truth playbook r...

## Description

Skills that dispatch sub-agents declare a "Canonical Source" or single-source-of-truth playbook reference. Skill-class meta-contract — applies to every SKILL.md across the vsdd-factory skills catalog. Confidence: HIGH.

## Preconditions

1. Skill is a dispatch-only entry point (e.g., delegates execution to specialist agents).

## Postconditions

1. Skill body identifies a single load-bearing playbook (typically `agents/orchestrator/<flow>.md`) with explicit "if the two ever disagree, the orchestrator file wins." Re-read at the start of every dispatch.
...
| **Source Line** | 167 |
| **Audit ID** | BC-AUDIT-096 |
| **Evidence (verbatim)** | `deliver-story/SKILL.md:45-49` (`agents/orchestrator/per-story-delivery.md` is the playbook). The skill itself enforces "Read the orchestrator file at the start of every dispatch. Do not cache it between runs." |
| **Confidence (verbatim)** | HIGH (single-source-of-truth pattern is explicit). |
