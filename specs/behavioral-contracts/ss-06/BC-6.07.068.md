...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1661"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.07.068: validate-workflow: collects all errors (no early bail) + exit code

## Description

validate-workflow: collects all errors (no early bail) + exit code. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/validate-workflow/SKILL.md` (lines 26-41; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. Parses with `bin/lobster-parse`; walks step list collecting all errors (does not bail on first); existence checks via `ls`; reports compact table + summary (`<n> checks, <p> passed, <f> failed`); exits 0 on all-pass, non-zero otherwise.
