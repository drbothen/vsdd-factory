...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L957"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.07.046: spec-versioning: L4 immutability rules + locked-VP enforcement

## Description

spec-versioning: L4 immutability rules + locked-VP enforcement. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/spec-versioning/SKILL.md` (lines 107-148; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. VP gets `status: locked` after passing proof.

## Postconditions

1. (1) Locked VPs cannot be modified; (2) refinements via VP-NNN+1 with `amends: VP-NNN`; (3) withdrawal requires justification + impact + human approval, sets `status: withdrawn`; (4) hierarchy version propagation L4→L3→L2→L1. consistency-validator detects modifications to locked VPs and reports CRITICAL findings that block pipeline.
...
| **Source Lines** | 107-148 |
| **Audit ID** | BC-AUDIT-695 |
