...
# Behavioral Contract BC-6.18.001: pr-create: identity & invocation contract

## Description

pr-create: identity & invocation contract. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/pr-create/SKILL.md` (lines 1-7; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Command `/vsdd-factory:pr-create [STORY-NNN]` invoked.

## Postconditions

1. Generates structured PR for completed story; `disable-model-invocation: true`, `allowed-tools: Read, Bash, Glob, Grep`; argument-hint `[STORY-NNN]`.
