...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1006"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.06.029: state-burst: Stage 2 global SHA replace + backfill commit

## Description

state-burst: Stage 2 global SHA replace + backfill commit. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/state-burst/SKILL.md` (lines 119-156; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. After Stage 1 verification.

## Postconditions

1. `STAGE1_SHA=$(git rev-parse HEAD | cut -c1-8)`; portable sed replaces `15fa97e6` across STATE.md/SESSION-HANDOFF.md/wave-state.yaml; verifies single SHA value cited; commits with `chore(<wave>): backfill stage-1 SHA $STAGE1_SHA into pass-N records`. `verify-sha-currency.sh` must report PASS.
...
| **Source Lines** | 119-156 |
| **Audit ID** | BC-AUDIT-700 |
