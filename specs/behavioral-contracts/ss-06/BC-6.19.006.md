...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L445"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.19.006: repo-initialization: multi-repo creates .factory-project/ + project.yaml + per-service repos

## Description

repo-initialization: multi-repo creates .factory-project/ + project.yaml + per-service repos. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/repo-initialization/SKILL.md` (lines 189-274; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Architect's `deployment_topology: multi-service` + human confirms multi-repo.

## Postconditions

1. Creates `.factory-project/` orphan worktree on `factory-project-artifacts`; per-service repos with develop/branch protection/git rerere/.factory worktree each; emits `project.yaml` with primary_repo, repos[], dependency_graph; .factory-project/ has STATE.md, cost/, integration/, specs/, wave-plans/.
...
| **Source Lines** | 189-274 |
| **Audit ID** | BC-AUDIT-643 |
