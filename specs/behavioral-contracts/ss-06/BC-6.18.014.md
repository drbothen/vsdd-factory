...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L342"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.18.014: release: version bump determined from story types when no explicit version

## Description

release: version bump determined from story types when no explicit version. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/release/SKILL.md` (lines 96-107; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Release Mode Step 2 with no explicit version arg.

## Postconditions

1. `type: feat` → MINOR, only `type: fix` → PATCH, any `breaking_change: true` → MAJOR; otherwise prompt user. Never bumps without confirmation.
