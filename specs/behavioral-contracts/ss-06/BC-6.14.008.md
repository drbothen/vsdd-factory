...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-412"
ss_section: "Artifact and verification skills"
skill: "formal-verify"
---

# Behavioral Contract BC-6.14.008: formal-verify: Fuzz saturation requires ≥5 minutes per target with stable coverage

## Description

"Saturation requires 5 minutes per fuzz target AND stable coverage. 30 seconds is not saturation.". Acceptance: `cargo fuzz run <target> -- -max_total_time=300` per target, no crashes.

## Preconditions

1. Fuzz testing step runs.

## Postconditions

1. "Saturation requires 5 minutes per fuzz target AND stable coverage. 30 seconds is not saturation."

## Invariants

1. `cargo fuzz run <target> -- -max_total_time=300` per target, no crashes.
