...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1211"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.19.028: toolchain-provisioning: install-priority (lang-native → brew → system) + pkg-mgr per type

## Description

toolchain-provisioning: install-priority (lang-native → brew → system) + pkg-mgr per type. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/toolchain-provisioning/SKILL.md` (lines 144-176; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 4 (Install Missing Tools).

## Postconditions

1. Priority: language-native pkg manager → Homebrew (macOS) → apt/dnf. Cargo deps (proptest) → project Cargo.toml [dev-dependencies] not global. npm devDeps (fast-check, Stryker) → `npm install --save-dev`. pip deps (Hypothesis, mutmut) → project venv if exists, else global. Verify after each install.
