...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-590"
ss_section: "Story delivery skills"
skill: "post-feature-validation"
---

# Behavioral Contract BC-6.10.014: post-feature-validation: Identity — monitors post-ship feedback at 7/30/90-day intervals; entirely optional

## Description

Monitors feedback channels and analytics for shipped feature reception. Closes the feedback loop: Build → Deploy → Customers → Feedback → Analysis → Next Iteration. Configurable intervals (default 7, 30, 90). Entirely optional — no-op if not configured. Acceptance: Frontmatter `name: post-feature-validation`.

## Preconditions

1. Scheduled (7d/30d/90d after feature ships) or manual ("Check how [feature] is doing").

## Postconditions

1. Monitors feedback channels and analytics for shipped feature reception. Closes the feedback loop: Build → Deploy → Customers → Feedback → Analysis → Next Iteration. Configurable intervals (default 7, 30, 90). Entirely optional — no-op if not configured.

## Invariants

1. Frontmatter `name: post-feature-validation`.
