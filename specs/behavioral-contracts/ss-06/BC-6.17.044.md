...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-571"
ss_section: "Feature-mode phase skills (f1-f7)"
skill: "phase-f7-delta-convergence"
---

# Behavioral Contract BC-6.17.044: phase-f7-delta-convergence: Final human authorization gate; failure routes to specific phase

## Description

Human must explicitly authorize merge. If sent back: identify which phase to re-execute. Spec/Impl not converged → F5; Tests not converged → F6; Holdout not converged → re-evaluation. Fix PRs through code-delivery.lobster. After approval, release: MINOR or PATCH bump → CHANGELOG → tag → gh release → publish. Acceptance: Routing rules per failing dimension; release ordering.

## Preconditions

1. Convergence report ready.

## Postconditions

1. Human must explicitly authorize merge. If sent back: identify which phase to re-execute. Spec/Impl not converged → F5; Tests not converged → F6; Holdout not converged → re-evaluation. Fix PRs through code-delivery.lobster. After approval, release: MINOR or PATCH bump → CHANGELOG → tag → gh release → publish.

## Invariants

1. Routing rules per failing dimension; release ordering.
