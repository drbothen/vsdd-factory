...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1759"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.06.039: wave-gate: GATE_CHECK telemetry lines (validated by hook)

## Description

wave-gate: GATE_CHECK telemetry lines (validated by hook). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/wave-gate/SKILL.md` (lines 118-133; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. Emit `GATE_CHECK: gate=N name=<name> status=<pass|fail|skip> note=<...>`; `validate-wave-gate-completeness` hook validates these are present in gate report before allowing `gate_status: passed` in wave-state.yaml. skip requires reason in note.
...
| **Source Lines** | 118-133 |
| **Audit ID** | BC-AUDIT-777 |
