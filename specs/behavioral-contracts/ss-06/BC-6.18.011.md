...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L315"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.18.011: release: identity, modes, factory worktree pre-flight

## Description

release: identity, modes, factory worktree pre-flight. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/release/SKILL.md` (lines 1-34; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. `/vsdd-factory:release [init|<version>|--dry-run]`.

## Postconditions

1. Three modes: Bootstrap (`init`), Dry Run (`--dry-run`), Release (else). Pre-flight ensures `.factory/` worktree exists; if `factory-artifacts` branch missing, triggers Bootstrap.
