...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L721"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.19.019: sdk-generation: contract evolution (semver + breaking detection)

## Description

sdk-generation: contract evolution (semver + breaking detection). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/sdk-generation/SKILL.md` (lines 298-345; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. MAJOR for breaking (remove endpoint/required field, type change, auth change); MINOR for additions (new endpoint, optional field, optional query param, new error status); PATCH for corrections. Detection: openapi-diff / `buf breaking` / graphql-inspector diff.
...
| **Source Lines** | 298-345 |
| **Audit ID** | BC-AUDIT-671 |
