...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1603"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.07.062: validate-template-compliance: template resolution by document_type then path

## Description

validate-template-compliance: template resolution by document_type then path. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/validate-template-compliance/SKILL.md` (lines 27-58; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. Primary lookup: `document_type` frontmatter → matching template. Fallback: 14-row path-pattern table mapping (BC files, VP files, story files, architecture sections, domain-spec, holdout-scenarios, prd, brief, STATE, dtu-assessment, module-criticality). If neither resolves, skip with explicit message.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Acceptance: Each file gets a template or "no template found" report. | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Each file gets a template or "no template found" report.
