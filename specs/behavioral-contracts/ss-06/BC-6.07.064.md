...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1621"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.07.064: validate-template-compliance: report format (per-file detail + summary table + aggregate counts)

## Description

validate-template-compliance: report format (per-file detail + summary table + aggregate counts). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/validate-template-compliance/SKILL.md` (lines 98-141; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. Per-file detail block; directory summary table (file/template/frontmatter/sections/tables/overall); aggregate counts (PASS/WARN/FAIL with %).
