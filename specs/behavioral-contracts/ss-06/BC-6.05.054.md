...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L752"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.05.054: semport-analyze: incremental protocol is delta-only (5 steps)

## Description

semport-analyze: incremental protocol is delta-only (5 steps). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/semport-analyze/SKILL.md` (lines 39-56; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. `--incremental <module>` mode.

## Postconditions

1. (1) read existing pass-6-synthesis.md, (2) scan only specified module + direct dependencies, (3) produce delta (changed/new/removed), (4) update existing pass files (append, don't replace), (5) write `.factory/semport/<project>/<project>-delta-<module>-<date>.md`.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Acceptance: No re-scan of full codebase; existing pass files only appended. | manual |
...
- [TBD — to be assigned in Phase 1.6c] — No re-scan of full codebase; existing pass files only appended.
