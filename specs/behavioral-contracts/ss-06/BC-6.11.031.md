...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-464"
ss_section: "Factory operations and dashboards skills"
skill: "model-routing"
---

# Behavioral Contract BC-6.11.031: model-routing: Three-tier fallback — primary → standard fallback → reasoning fallback (for adversary/judgment) or fast fallback → standard (for impl/validation)

## Description

Implementation/Validation → fallback/fast (Codestral) → fallback/standard (DeepSeek-V3). Adversary/Judgment → fallback/standard (DeepSeek-V3) → fallback/reasoning (Qwen3-235B thinking). Review → fallback/standard. fallback/standard via OpenRouter falls to local DeepSeek-V3 for sensitive data. Acceptance: Routing chains per tier.

## Preconditions

1. Primary model fails after 2 retries.

## Postconditions

1. Implementation/Validation → fallback/fast (Codestral) → fallback/standard (DeepSeek-V3). Adversary/Judgment → fallback/standard (DeepSeek-V3) → fallback/reasoning (Qwen3-235B thinking). Review → fallback/standard. fallback/standard via OpenRouter falls to local DeepSeek-V3 for sensitive data.

## Invariants

1. Routing chains per tier.
