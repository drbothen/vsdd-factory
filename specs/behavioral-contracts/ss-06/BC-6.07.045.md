...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L948"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.07.045: spec-versioning: per-story spec_version + drift detection

## Description

spec-versioning: per-story spec_version + drift detection. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/spec-versioning/SKILL.md` (lines 60-105; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Story creation, then drift detection at Phase F1 / after Phase F2 / on demand.

## Postconditions

1. Story frontmatter records `spec_version`; commit message references "Spec version: X.Y.Z". Drift report compares story vs current; PATCH = informational, MINOR = check overlap, MAJOR = mandatory review.
