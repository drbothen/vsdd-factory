...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L485"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.05.047: research: identity & sub-agent fork

## Description

research: identity & sub-agent fork. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/research/SKILL.md` (lines 1-9; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. `/research [domain|general] <topic>`.

## Postconditions

1. Spawns research-agent in forked context (disable-model-invocation true, agent: research-agent).
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Acceptance: Frontmatter declares `context: fork` and `agent: research-agent`. | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Frontmatter declares `context: fork` and `agent: research-agent`.
