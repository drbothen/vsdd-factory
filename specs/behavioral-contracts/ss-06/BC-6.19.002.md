...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L409"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.19.002: repo-initialization: workspace-isolation guard refuses dark-factory cwd

## Description

repo-initialization: workspace-isolation guard refuses dark-factory cwd. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/repo-initialization/SKILL.md` (lines 81-91; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. devops-engineer step before any repo/git command.

## Postconditions

1. If `pwd` contains `dark-factory`, exit FATAL with prescribed remedy.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Acceptance: Guard runs before `gh repo create`/`git clone`/`git config`. | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Guard runs before `gh repo create`/`git clone`/`git config`.
