...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L217"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.06.021: recover-state: identity, dry-run, and backup

## Description

recover-state: identity, dry-run, and backup. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/recover-state/SKILL.md` (lines 1-33; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. STATE.md missing/corrupted/out-of-sync; or `--dry-run` flag.

## Postconditions

1. Backs up existing STATE.md to `STATE.md.backup-YYYY-MM-DD-HHMMSS` if present; `--dry-run` skips writes and only reports.
