...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L605"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.16.041: run-phase: validates workflow before execution

## Description

run-phase: validates workflow before execution. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/run-phase/SKILL.md` (lines 28-39; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 2 (Validate the workflow).

## Postconditions

1. Calls `bin/lobster-parse` with `.workflow | {name, version, steps: (.steps | length)}`; confirms name + ≥1 step.
