...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L244"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.06.024: recover-state: requires user approval before write

## Description

recover-state: requires user approval before write. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/recover-state/SKILL.md` (lines 113-148; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 8 (Validate reconstruction) before Step 9 (Write).

## Postconditions

1. Presents reconstructed snapshot to user with phase progress, artifact counts, DTU; prompts approve/adjust; only writes on approval.
...
| **Source Lines** | 113-148 |
| **Audit ID** | BC-AUDIT-622 |
