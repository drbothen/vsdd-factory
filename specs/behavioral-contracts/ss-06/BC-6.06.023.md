...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L235"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.06.023: recover-state: phase decision tree is total and ordered

## Description

recover-state: phase decision tree is total and ordered. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/recover-state/SKILL.md` (lines 53-71; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 3 (Determine current phase).

## Postconditions

1. Applies the explicit decision tree (no brief → pre-pipeline; brief only → 1a; …; convergence-report → 6); story status branches inside Phase 3.
