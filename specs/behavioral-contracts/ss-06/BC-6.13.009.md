...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-484"
ss_section: "Telemetry and analytics integration skills"
skill: "onboard-observability"
---

# Behavioral Contract BC-6.13.009: onboard-observability: Identity — registers project + writes Claude OTel env vars; idempotent

## Description

Two-part setup: (1) `factory-obs register` adds project's `.factory/logs` to collector watch list; (2) writes 5 OTEL_* env vars to `.claude/settings.local.json` so Claude ships tool calls/tokens/costs to Loki+Prometheus. Idempotent. Acceptance: Frontmatter `name: onboard-observability`; `allowed-tools: Bash, Read, Write`.

## Preconditions

1. User asks to "register this project with observability", "set up observability here", "onboard the observability stack", or similar first-time setup phrasing.

## Postconditions

1. Two-part setup: (1) `factory-obs register` adds project's `.factory/logs` to collector watch list; (2) writes 5 OTEL_* env vars to `.claude/settings.local.json` so Claude ships tool calls/tokens/costs to Loki+Prometheus. Idempotent.

## Invariants

1. Frontmatter `name: onboard-observability`; `allowed-tools: Bash, Read, Write`.
