...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1710"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.08.055: visual-companion: excalidraw mode + composed views (screen.json manifest)

## Description

visual-companion: excalidraw mode + composed views (screen.json manifest). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/visual-companion/SKILL.md` (lines 74-104; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. `.excalidraw` file write or split view.

## Postconditions

1. Excalidraw: server auto-switches to interactive canvas; user edits sync via WebSocket to file; agent reads on next turn. Composed: `screen.json` with `{layout:split, panes:[{file,label}]}`.
