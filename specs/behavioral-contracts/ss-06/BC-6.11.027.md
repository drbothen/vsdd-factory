...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-449"
ss_section: "Factory operations and dashboards skills"
skill: "maintenance-sweep"
---

# Behavioral Contract BC-6.11.027: maintenance-sweep: Auto-PR quality gate same as Feature Mode (regression + holdout + adversarial + lint)

## Description

All existing tests pass (regression). Holdout scenarios pass (≥90%). Adversarial review (scoped to changed files). Lint/format clean. Acceptance: "Maintenance PRs go through the SAME quality gates as Feature Mode code.".
...
1. All existing tests pass (regression). Holdout scenarios pass (≥90%). Adversarial review (scoped to changed files). Lint/format clean.

## Invariants

1. "Maintenance PRs go through the SAME quality gates as Feature Mode code."
