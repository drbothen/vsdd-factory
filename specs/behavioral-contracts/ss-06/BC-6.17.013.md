...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-540"
ss_section: "Feature-mode phase skills (f1-f7)"
skill: "phase-f2-spec-evolution"
---

# Behavioral Contract BC-6.17.013: phase-f2-spec-evolution: Quality Gate — BC-S.SS.NNN format, append-only, acyclic deps, version bumped, changelog written, adversary cosmetic only, human approved

## Description

New BCs use BC-S.SS.NNN; new requirements continue ID sequence (no gaps, no collisions); modified retain previous version inline; architecture dependency graph remains acyclic; new VPs have proof strategies; spec version bumped per semver; changelog entry written; adversary findings on delta cosmetic only; DTU re-assessment done if new external deps; gene transfusion assessment done if reference impl exists; UX delta + accessibility review for UI features; human approved. Acceptance: Twelve Quality Gate items.
