...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-406"
ss_section: "Adversarial and review skills"
skill: "fix-pr-delivery"
---

# Behavioral Contract BC-6.04.021: fix-pr-delivery: Demo recording is conditional on behavior-changing fixes

## Description

Demo only if fix is behavior-changing (output, error messages, CLI flags, API responses, security restrictions). Transparent fixes (refactoring, performance, internal error handling) get no demo. Acceptance: Quality Gate: "Demo recorded if behavior-changing fix".

## Preconditions

1. Fix PR is being prepared.

## Postconditions

1. Demo only if fix is behavior-changing (output, error messages, CLI flags, API responses, security restrictions). Transparent fixes (refactoring, performance, internal error handling) get no demo.

## Invariants

1. Quality Gate: "Demo recorded if behavior-changing fix".
