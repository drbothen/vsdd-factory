...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1122"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.08.033: storybook-mcp-integration: non-React fallback (manifest only)

## Description

storybook-mcp-integration: non-React fallback (manifest only). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/storybook-mcp-integration/SKILL.md` (lines 152-158, 180-185).

## Preconditions

1. Non-React framework detected (Vue/Svelte/Angular).

## Postconditions

1. list-all-documentation, preview-stories, run-story-tests still work; component manifest unavailable; agents read source files directly. Failure modes: not installed → skip + flag gap; unreachable → retry once + fallback to file-based.
...
| **Source Lines** | 152-158, 180-185 |
| **Audit ID** | BC-AUDIT-712 |
