...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1046"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.06.033: state-update: 4-step procedure (read → frontmatter → history → commit)

## Description

state-update: 4-step procedure (read → frontmatter → history → commit). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/state-update/SKILL.md` (lines 19-66; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Calling skill provides {phase, status, notes}.

## Postconditions

1. Read STATE.md; update frontmatter (pipeline status, phase, product, mode, timestamp, previous_phase); append row to Phase History table; commit with `factory(<phase>): <status> — <description>`.
