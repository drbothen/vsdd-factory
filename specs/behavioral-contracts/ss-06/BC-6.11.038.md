...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-580"
ss_section: "Factory operations and dashboards skills"
skill: "policy-add"
---

# Behavioral Contract BC-6.11.038: policy-add: Policy name must be snake_case and unique across registry

## Description

Must be snake_case (lowercase, underscores, no spaces, no hyphens). Must not already exist (check `name` field). Invalid or duplicate → report error and stop. Acceptance: Format and uniqueness rule; stop-on-violation.
...
1. Must be snake_case (lowercase, underscores, no spaces, no hyphens). Must not already exist (check `name` field). Invalid or duplicate → report error and stop.

## Invariants

1. Format and uniqueness rule; stop-on-violation.
