...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L899"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.07.040: spec-drift: identity & forked Explore agent

## Description

spec-drift: identity & forked Explore agent. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/spec-drift/SKILL.md` (lines 1-8; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Drift detection request.

## Postconditions

1. disable-model-invocation true; context fork; agent Explore. Compares spec vs implementation in fresh context for objectivity.
...
|--------|----------|-------------|
| (TBD — to be assigned in Phase 1.6c) | Acceptance: Frontmatter `context: fork` + `agent: Explore`. | manual |
...
- [TBD — to be assigned in Phase 1.6c] — Frontmatter `context: fork` + `agent: Explore`.
