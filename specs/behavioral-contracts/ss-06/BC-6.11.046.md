...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-588"
ss_section: "Factory operations and dashboards skills"
skill: "policy-registry"
---

# Behavioral Contract BC-6.11.046: policy-registry: List shows summary table with #/Policy/Severity/Enforced By/Lint Hook

## Description

Reads `.factory/policies.yaml`. If file doesn't exist → "No policy registry found. Run `/vsdd-factory:policy-registry init` to create one." Otherwise summary table. Acceptance: Default-to-list behavior; missing-file message.

## Preconditions

1. `policy-registry list` (default).

## Postconditions

1. Reads `.factory/policies.yaml`. If file doesn't exist → "No policy registry found. Run `/vsdd-factory:policy-registry init` to create one." Otherwise summary table.

## Invariants

1. Default-to-list behavior; missing-file message.
