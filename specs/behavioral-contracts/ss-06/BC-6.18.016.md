...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L360"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.18.016: release: per-format version-bump dispatch (json/toml/yaml/regex)

## Description

release: per-format version-bump dispatch (json/toml/yaml/regex). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/release/SKILL.md` (lines 142-156; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 5 (Bump Versions).

## Postconditions

1. For each entry in `version_sources`/`global_version_sources`: `json`→`jq`, `toml`→`sed`/toml, `yaml`→`yq`, `regex`→`sed`. Reports each file updated.
...
| **Source Lines** | 142-156 |
| **Audit ID** | BC-AUDIT-634 |
