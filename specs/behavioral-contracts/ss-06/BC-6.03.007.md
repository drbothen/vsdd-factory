...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-1.md#L1106"
subsystem: SS-06
capability: "TBD"
...
# Behavioral Contract BC-6.03.007: deactivate: skill identity (inverse of activate)

> Source: `pass-3-deep-skills-batch-1.md` line 1106 (was `BC-AUDIT-305`)
> Subsystem: SS-06 — Skill Catalog
> Section: Activation and deactivation skills

## Description

deactivate: skill identity (inverse of activate). Reverses activate. Clears orchestrator default-agent override in `.claude/settings.local.json`, removes v1.0 platform activation block, deletes per-machine `hooks/hooks.json` (the per-platform variants remain). Plugin itself stays enabled.

## Preconditions

1. User invokes `/vsdd-factory:deactivate`

## Postconditions

1. After deactivation, `.claude/settings.local.json` lacks `agent` and `vsdd-factory` keys; hooks.json removed but `hooks.json.<platform>` files remain.
...
- `architecture/ss-06-skill-catalog.md#deactivate-skill-identity-(inverse-of-activate)` — TBD anchor
