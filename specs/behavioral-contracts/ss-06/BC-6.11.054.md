...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1861"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.11.054: worktree-manage: cleanup refuses dirty + warns unmerged

## Description

worktree-manage: cleanup refuses dirty + warns unmerged. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/worktree-manage/SKILL.md` (lines 68-104; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).
...
1. Check uncommitted (`git status --porcelain`); abort+warn if dirty (user must commit/stash). Check merged to develop; if not merged, warn but allow with confirmation. Remove worktree (`git worktree remove`); delete branch if merged.
