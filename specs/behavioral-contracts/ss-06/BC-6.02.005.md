...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-behavioral-contracts-deep-r1.md#L132"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.02.005: Skills with `allowed-tools:` whitelist restrict tool surface inside the skill body

## Description

Skills with `allowed-tools:` whitelist restrict tool surface inside the skill body. Skill-class meta-contract — applies to every SKILL.md across the vsdd-factory skills catalog. Confidence: HIGH.

## Preconditions

1. Skill frontmatter declares `allowed-tools: Read, Write, Edit, …`.

## Postconditions

1. Inside the skill's procedure, only the listed tools may be invoked. Claude Code enforces the whitelist at tool-dispatch time.
...
| **Source Line** | 132 |
| **Audit ID** | BC-AUDIT-091 |
| **Evidence (verbatim)** | `wave-gate/SKILL.md:7` (`allowed-tools: Read, Write, Edit, Bash, Glob, Grep, AskUserQuestion`); `create-prd/SKILL.md:5` (`allowed-tools: Read, Write, Edit, Bash, AskUserQuestion`); `deliver-story/SKILL.md:6` (`allowed-tools: Read, Bash, Glob, Grep, AskUserQuestion, Task`). |
| **Confidence (verbatim)** | HIGH (frontmatter pattern uniform on the sampled skills that use it). |
