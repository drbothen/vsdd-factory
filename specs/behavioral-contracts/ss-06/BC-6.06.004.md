...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-1.md#L471"
subsystem: SS-06
capability: "TBD"
...
# Behavioral Contract BC-6.06.004: check-input-drift: cluster-drift triage before bulk --update

> Source: `pass-3-deep-skills-batch-1.md` line 471 (was `BC-AUDIT-242`)
> Subsystem: SS-06 — Skill Catalog
> Section: State and convergence skills

## Description

check-input-drift: cluster-drift triage before bulk --update. Skill MUST STOP before running `--update`. Reads upstream diff first; if cosmetic → proceed; if semantic → dispatch producing agent (business-analyst, product-owner, architect, story-writer per artifact type) for content review. After review, run `--update` then re-scan to verify zero drift and no cascading staleness.

## Preconditions

1. Cluster patterns detected (all shards drift, all subsystem files drift, >5 artifacts share upstream, PRD+supplements drift, all VPs in family drift)

## Postconditions

1. Cluster pattern detection triggers triage flow; bulk `--update` is only run after content-review or after explicit "bulk update, no investigation" override.
...
- `architecture/ss-06-skill-catalog.md#check-input-drift-cluster-drift-triage-before-bulk--update` — TBD anchor
...
| **Source line(s) within skill** | 147-216 |
