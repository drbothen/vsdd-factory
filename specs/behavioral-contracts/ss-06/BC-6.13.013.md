...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-488"
ss_section: "Telemetry and analytics integration skills"
skill: "onboard-observability"
---

# Behavioral Contract BC-6.13.013: onboard-observability: Idempotency — register dedupes on absolute path; jq merge overwrites only the 5 keys

## Description

factory-obs register dedupes on absolute path. jq merge preserves existing env keys, only overwrites 5 OTEL_* keys. Acceptance: Both idempotency mechanisms.

## Preconditions

1. Skill re-invoked on already-onboarded project.

## Postconditions

1. factory-obs register dedupes on absolute path. jq merge preserves existing env keys, only overwrites 5 OTEL_* keys.

## Invariants

1. Both idempotency mechanisms.
