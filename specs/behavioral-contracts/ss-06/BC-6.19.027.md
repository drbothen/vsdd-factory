...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L1202"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.19.027: toolchain-provisioning: language detection cascade

## Description

toolchain-provisioning: language detection cascade. Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/toolchain-provisioning/SKILL.md` (lines 58-89; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 1 (Detect Language(s)).

## Postconditions

1. Greenfield → ARCH-INDEX. Brownfield → project-discovery.md. Fallback → file scan (Cargo.toml→Rust, package.json+tsconfig.json→TS, pyproject.toml/setup.py→Python, go.mod→Go). Multiple languages → provision all. Writes `.factory/toolchain-state.yaml`.
