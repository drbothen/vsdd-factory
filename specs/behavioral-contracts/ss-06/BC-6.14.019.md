...
origin: brownfield
extracted_from: ".factory/phase-0-ingestion/pass-3-deep-skills-batch-3.md#L275"
subsystem: "SS-06"
capability: "CAP-TBD"
...
# Behavioral Contract BC-6.14.019: register-artifact: type identification by path pattern (4-row table)

## Description

register-artifact: type identification by path pattern (4-row table). Confidence: HIGH. Extracted from `plugins/vsdd-factory/skills/register-artifact/SKILL.md` (lines 17-28; source-range carve-out: SKILL.md line ranges are unstable; stable anchor is section content).

## Preconditions

1. Step 1 path parsing.

## Postconditions

1. Maps `behavioral-contracts/BC-*.md` → BC; `verification-properties/VP-*.md` → VP; `stories/STORY-*.md` → Story; `holdout-scenarios/HS-*.md` → HS. Unrecognized → reports unrecognized-type error.
