...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-549"
ss_section: "Feature-mode phase skills (f1-f7)"
skill: "phase-f4-delta-implementation"
---

# Behavioral Contract BC-6.17.022: phase-f4-delta-implementation: Per-story delivery uses code-delivery.lobster sub-workflow with 11 stages

## Description

11 stages: stubs (Two-Step Red Gate), tests (Red Gate: compile + fail), implementation (TDD or Semport gene-transfusion), E2E tests (if UI/full-stack), demo (per-AC), Push+PR (squash, force-with-lease), AI review (pr-reviewer Gemini, info asymmetry), security review (CRIT/HIGH only), converge (max 10 cycles), merge (deps check), cleanup (devops-engineer removes worktree). Acceptance: Eleven stages.
