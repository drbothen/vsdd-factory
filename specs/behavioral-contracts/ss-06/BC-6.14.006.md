...
removed: null
removal_reason: null
audit_source_id: "BC-AUDIT-410"
ss_section: "Artifact and verification skills"
skill: "formal-verify"
---

# Behavioral Contract BC-6.14.006: formal-verify: Identity — Phase 6 quality gate runs Kani + cargo-fuzz + cargo-mutants + semgrep

## Description

Runs four verification techniques: Kani proofs (pure core), cargo-fuzz (parsers/handlers), cargo-mutants (mutation kill rate), semgrep (security scanning). Acceptance: Frontmatter `name: formal-verify`; `disable-model-invocation: true`; `allowed-tools: Read, Write, Bash, Glob, Grep`.

## Preconditions

1. Phase 6 (formal hardening) entry.

## Postconditions

1. Runs four verification techniques: Kani proofs (pure core), cargo-fuzz (parsers/handlers), cargo-mutants (mutation kill rate), semgrep (security scanning).

## Invariants

1. Frontmatter `name: formal-verify`; `disable-model-invocation: true`; `allowed-tools: Read, Write, Bash, Glob, Grep`.
